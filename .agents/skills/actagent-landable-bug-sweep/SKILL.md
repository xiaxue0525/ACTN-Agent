---
name: actagent-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary ACTAgent bugfix PRs until five are landable."
---

# ACTAgent Landable Bug Sweep

Autonomous maintainer workflow for producing five landable ACTAgent bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$actagent-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when author trust matters, `$actagent-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
...
5. Patch:
   ...
   - if unwritable or wrong shape, create own PR and preserve useful author credit
   - if no PR exists, create one
   - add regression test when it fits
   - release-note context for user-facing fixes in PR body or commit message; credit human reporter/author when known
...
- If maintainer cannot push to external branch: create own branch/PR, preserve useful commits or credit.

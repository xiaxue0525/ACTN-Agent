---
name: actagent-testing
description: Choose, run, rerun, or debug ACTAgent tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
--provider blacksmith-testbox ... -- env ACTAGENT_CHECK_CHANGED_REMOTE_CHILD=1 ACTAGENT_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
ACTAGENT_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
ACTAGENT_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `ACTAGENT_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `ACTAGENT_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `ACTAgent Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `actagent/releases` evidence ledger.
Use the manual `ACTAgent Release Evidence`
(`actagent-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation actagent/actagent <run-id> blocking
package-acceptance actagent/actagent <run-id> blocking
release-checks actagent/actagent <run-id> blocking
```
...
When `Full Release Validation` completes and `ACTAGENT_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`ACTAgent Release Evidence From Full Validation` workflow. That workflow reads
...
`ACTAgent Release Checks` (`actagent-release-checks.yml`) is the release child
...
```bash
gh workflow run actagent-release-checks.yml \
  --repo actagent/actagent \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`ACTAgent Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/actagent/actagent-live-media-runner:ubuntu-24.04` container so
...
`pnpm actagent qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm actagent qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `ACTAGENT_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `ACTAgent Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`ACTAgent Live And E2E Checks (Reusable)`
(`actagent-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run actagent-live-and-e2e-checks-reusable.yml \
  --repo actagent/actagent \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `ACTAGENT_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`ACTAGENT_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
ACTAGENT_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
ACTAGENT_DOCKER_ALL_DRY_RUN=1 ACTAGENT_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
ACTAGENT_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
ACTAGENT_DOCKER_ALL_LANES=<lane> \
ACTAGENT_DOCKER_ALL_BUILD=0 \
ACTAGENT_DOCKER_ALL_PREFLIGHT=0 \
ACTAGENT_SKIP_DOCKER_BUILD=1 \
ACTAGENT_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
ACTAGENT_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  `npm view actagent dist-tags --json --prefer-online --cache /tmp/actagent-npm-cache-verify-$$`
  and `npm view actagent@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/actagent-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=actagent@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=actagent@YYYY.M.D-beta.N` or
  `package_spec=actagent@YYYY.M.D`.
- For stable package proof, use `package_spec=actagent@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `actagent@beta`,
  `actagent@latest`, or exact ACTAgent release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `actagent update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `actagent@beta`, `actagent@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an ACTAgent branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-actagent-for-docker.mjs` is the
...
`docker_lanes=skill-install` for live ACTAgentHub skill-install validation. The
...
from `actagent skills search`, installs it, and verifies `.actagenthub` origin/lock

...
```bash
export ACTAGENT_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.actagent/actagent.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ACTAGENT_PARALLELS_DISCORD_TOKEN \
...
- Use the `actagent` wrapper for guest `message send/read`; `node actagent.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/actagent-parallels-smoke.*`
...
- host inbound nonce appears in `actagent message read` output

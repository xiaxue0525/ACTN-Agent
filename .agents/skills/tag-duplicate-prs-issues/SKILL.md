...
description: Use gitcrawl to search duplicate ACTAgent PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local ACTAgent history and clustering source.
...
gitcrawl neighbors actagent/actagent --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search actagent/actagent --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search actagent/actagent --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```
...
```bash
gh search prs --repo actagent/actagent --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo actagent/actagent --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo actagent/actagent --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R actagent/actagent "<problem phrase>" --types group --limit 10
prtags search similar -R actagent/actagent "<problem summary>" --types group --limit 10
prtags group list -R actagent/actagent
```
...
```bash
prtags group create -R actagent/actagent \
  --kind mixed \
...
```bash
prtags field ensure -R actagent/actagent --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R actagent/actagent --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R actagent/actagent --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R actagent/actagent --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R actagent/actagent --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R actagent/actagent --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R actagent/actagent --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R actagent/actagent --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R actagent/actagent --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R actagent/actagent <pr-number> \
...
```bash
prtags annotation issue set -R actagent/actagent <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R actagent/actagent
```

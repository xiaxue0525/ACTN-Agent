...
import type { PluginLogger } from "actagent/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "actagent/plugin-sdk/runtime-env";
...
/** Input data used to publish ACTAgent gateway Bonjour records. */
...
// See https://github.com/actagent/actagent/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.ACTAGENT_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "ACTAgent"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "ACTAgent";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ACTAgent\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.ACTAGENT_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "actagent";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or ACTAGENT_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );

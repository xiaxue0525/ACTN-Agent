...
import { createMockIncomingRequest } from "actagent/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "actagent/plugin-sdk/webhook-request-guards";
...
vi.mock("actagent/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/webhook-ingress")>(
    "actagent/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("actagent/plugin-sdk/reply-runtime");
    vi.doUnmock("actagent/plugin-sdk/runtime-env");
    vi.doUnmock("actagent/plugin-sdk/webhook-ingress");

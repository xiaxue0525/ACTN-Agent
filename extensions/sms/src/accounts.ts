...
import { normalizeOptionalAccountId } from "actagent/plugin-sdk/account-id";
import {
  DEFAULT_ACCOUNT_ID,
...
function getChannelConfig(cfg: ACTAgentConfig): SmsChannelConfig | undefined {
...
export function listSmsAccountIds(cfg: ACTAgentConfig): string[] {
...
export function resolveDefaultSmsAccountId(cfg: ACTAgentConfig): string {
  const channelCfg = getChannelConfig(cfg);
...
export function inspectSmsAccount(cfg: ACTAgentConfig, accountId?: string | null) {

...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "actagent/plugin-sdk/provider-auth";
...
import { fetchCodexUsage } from "actagent/plugin-sdk/provider-usage";
import {
 ...
 * entirely when they were written by older ACTAgent versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.actagent.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `actagent models auth login --provider openai` or `actagent configure`.";
}

...
import type { ProviderRuntimeModel } from "actagent/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-actagent-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-actagent-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-actagent-turn-attempt"]).toBe("2");
    expect(state?.metadata?.actagent_session_id).toBe("session-123");
    expect(state?.metadata?.actagent_turn_id).toBe("turn-123");
    expect(state?.metadata?.actagent_turn_attempt).toBe("2");
    expect(state?.metadata?.actagent_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-actagent-session-id"]).toBe("session-123");

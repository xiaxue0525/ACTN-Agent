...
} from "actagent/plugin-sdk/image-generation";
import {
  parseOpenAiCompatibleImageResponse,
  toImageDataUrl,
} from "actagent/plugin-sdk/image-generation";
import { createSubsystemLogger } from "actagent/plugin-sdk/logging-core";
import { resolveClosestSize } from "actagent/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "actagent/plugin-sdk/media-mime";
import {
...
import { resolveApiKeyForProvider } from "actagent/plugin-sdk/provider-auth-runtime";
import {
...
import { isPrivateNetworkOptInEnabled } from "actagent/plugin-sdk/ssrf-runtime";
import {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.ACTAGENT_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: ACTAgentConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: ACTAgentConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: ACTAgentConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: ACTAgentConfig | undefined): ACTAgentConfig | undefined {

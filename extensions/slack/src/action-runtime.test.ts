...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): ACTAgentConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as ACTAgentConfig;
  ...
  function expectLastSlackSend(content: string, cfg: ACTAgentConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: ACTAgentConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: ACTAgentConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/actagent-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/actagent-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/actagent-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });

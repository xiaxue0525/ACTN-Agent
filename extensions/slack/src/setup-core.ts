...
import { hasConfiguredSecretInput } from "actagent/plugin-sdk/secret-input";
import {
...
import { formatDocsLink } from "actagent/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: ACTAgentConfig, accountId: string): ACTAgentConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: ACTAgentConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) =>
    ...
    disable: (cfg: ACTAgentConfig) => setSetupChannelEnabled(cfg, channel, false),

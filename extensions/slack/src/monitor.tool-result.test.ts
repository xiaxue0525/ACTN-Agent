...
import { CURRENT_MESSAGE_MARKER } from "actagent/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "actagent/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "actagent/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bactagent\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("actagent: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("actagent: hello <@U2>");
  });

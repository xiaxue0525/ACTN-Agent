...
type SlackRouteBinding = NonNullable<ACTAgentConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  ACTAgentConfig,
  { bindingsRef: ACTAgentConfig["bindings"]; normalizedCfg: ACTAgentConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: ACTAgentConfig): ACTAgentConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as ACTAgentConfig)
    : cfg;

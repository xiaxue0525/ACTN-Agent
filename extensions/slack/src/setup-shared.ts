...
import { patchChannelConfigForAccount } from "actagent/plugin-sdk/setup-runtime";
import { formatDocsLink } from "actagent/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { ACTAgentConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "ACTAgent") {
  const safeName = botName.trim() || "ACTAgent";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for ACTAgent`,
    },
    features: {
      bot_user: {
      ...
      assistant_view: {
        assistant_description: `${safeName} connects Slack assistant threads to ACTAgent agents.`,
        suggested_prompts: [
          {

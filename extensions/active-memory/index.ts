...
import { closeActiveMemorySearchManager } from "actagent/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "actagent/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type ACTAgentPluginApi } from "actagent/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "actagent/plugin-sdk/routing";
import { isPathInside } from "actagent/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredACTAgentTmpDir } from "actagent/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: ACTAgentConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: ACTAgentConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: ACTAgentPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: ACTAgentPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: ACTAgentConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: ACTAgentPluginApi): ACTAgentConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as ACTAgentConfig | undefined) ??
      (api.config as ACTAgentConfig | undefined)
    );
  } catch {
    return api.config as ACTAgentConfig | undefined;
  }
}
...
export default definePluginEntry({
  ...
  register(api: ACTAgentPluginApi) {
    const readCurrentConfig = (): ACTAgentConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as ACTAgentConfig | undefined) ??
          (api.config as ACTAgentConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as ACTAgentConfig;
          if (action === "status") {
            return {

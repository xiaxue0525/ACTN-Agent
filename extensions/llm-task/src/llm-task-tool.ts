...
import { buildModelAliasIndex, resolveModelRefFromString } from "actagent/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredACTAgentTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<ACTAgentPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via actagent.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<ACTAgentPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: ACTAgentPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<ACTAgentPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredACTAgentTmpDir(), prefix: "actagent-llm-task-" },
        async ({ dir: tmpDir }) => {

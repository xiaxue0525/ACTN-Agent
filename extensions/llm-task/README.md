...
`actagent.invoke --each`) without adding new ACTAgent code per workflow.
...
This extension depends on ACTAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ACTAgent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.actagent/extensions` as a standalone plugin directory.

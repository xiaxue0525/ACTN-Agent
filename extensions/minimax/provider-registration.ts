...
import { buildOauthProviderAuthResult } from "actagent/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "actagent/plugin-sdk/provider-auth-api-key";
...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "actagent/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "actagent/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: ACTAgentConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: ACTAgentConfig): string | undefined {
...
export function registerMinimaxProviders(api: ACTAgentPluginApi) {

...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'actagent matrix verify status --verbose --account ${accountId}' and 'actagent matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another ACTAgent runtime is using the same account. " +
    `Check 'actagent matrix verify status --verbose --account ${accountId}' and 'actagent matrix devices list --account ${accountId}'.`
  );
}

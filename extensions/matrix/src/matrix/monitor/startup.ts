...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleACTAgentDevices.length > 0) {
      params.logger.warn(
        `matrix: stale ACTAgent devices detected for ${params.auth.userId}: ${deviceHealth.staleACTAgentDevices.map((device) => device.deviceId).join(", ")}. Run 'actagent matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {

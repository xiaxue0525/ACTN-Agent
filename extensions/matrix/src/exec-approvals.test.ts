...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<ACTAgentConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<ACTAgentConfig["channels"]>["matrix"]>>,
): ACTAgentConfig {
  return {
    channels: {
      matrix: {

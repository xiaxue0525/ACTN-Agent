...
function hasTrustedACTAgentRootIndicator(packageRoot, packageJson) {
  ...
  const hasACTAgentBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("actagent")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.actagent === "string");
  const hasACTAgentEntrypoint = fs.existsSync(path.join(packageRoot, "actagent.mjs"));
  return hasCliEntryExport || hasACTAgentBin || hasACTAgentEntrypoint;
}

function findACTAgentPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "actagent" && hasTrustedACTAgentRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findACTAgentPackageRoot(moduleDir);
  if (location) {

...
vi.mock("actagent/plugin-sdk/runtime-env", () => ({
...
import type { ACTAgentPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/actagent-diagnostics-otel-test";
...
const ORIGINAL_ACTAGENT_OTEL_PRELOADED = process.env.ACTAGENT_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<ACTAgentPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): ACTAgentPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.ACTAGENT_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_ACTAGENT_OTEL_PRELOADED === undefined) {
      delete process.env.ACTAGENT_OTEL_PRELOADED;
    } else {
      process.env.ACTAGENT_OTEL_PRELOADED = ORIGINAL_ACTAGENT_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["actagent.provider"]).toBe("openai");
    for (const key of [
      "actagent.callId",
      "actagent.call_id",
      "actagent.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("actagent.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "actagent.channel": "telegram",
      "actagent.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("actagent.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "actagent.channel": "telegram",
      "actagent.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("actagent.message.queued")?.add).toHaveBeenCalledWith(1, {
      "actagent.channel": "telegram",
      "actagent.source": "telegram",
    });
    expect(telemetryState.histograms.get("actagent.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("actagent.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "actagent.channel": "telegram",
      "actagent.source": "telegram",
    });
    expect(telemetryState.histograms.get("actagent.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "actagent.lane": "main",
    });
    expect(telemetryState.counters.get("actagent.message.processed")?.add).toHaveBeenCalledWith(1, {
      "actagent.channel": "telegram",
      "actagent.outcome": "completed",
    });
    expect(telemetryState.counters.get("actagent.message.received")?.add).toHaveBeenCalledWith(1, {
      "actagent.channel": "telegram",
      "actagent.source": "webhook",
    });
    expect(telemetryState.counters.get("actagent.message.received")?.add).toHaveBeenCalledWith(1, {
      "actagent.channel": "unknown",
      "actagent.source": "unknown",
    });
    expect(
      telemetryState.counters.get("actagent.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("actagent.message.dispatch.completed")?.add,
    ...
    expect(
      ...
      expect.objectContaining({
        "actagent.channel": "unknown",
        "actagent.reason": "none",
        "actagent.source": "unknown",
      }),
    );
    expect(
      telemetryState.histograms.get("actagent.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "actagent.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("actagent.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "actagent.channel": "telegram",
      "actagent.outcome": "completed",
    });
    expect(telemetryState.histograms.get("actagent.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "actagent.lane": "main",
      },
    );
    expect(telemetryState.counters.get("actagent.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("actagent.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "actagent.state": "processing",
    });
    expect(
      telemetryState.histograms.get("actagent.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "actagent.state": "processing",
    });
    expect(telemetryState.counters.get("actagent.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "actagent.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("actagent.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "actagent.agent": "agent.default",
        "actagent.channel": "telegram",
        "actagent.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("actagent.webhook.processed");
    expect(spanNames).toContain("actagent.message.processed");
    expect(spanNames).toContain("actagent.session.stuck");
    const webhookSpanOptions = startedSpanOptions("actagent.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("actagent.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("actagent.message.processed");
    expect(messageSpanOptions?.attributes?.["actagent.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["actagent.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("actagent.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("actagent.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.ACTAGENT_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("actagent.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["actagent.provider"]).toBe("openai");
    expect(runDurationAttributes?.["actagent.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("actagent.run");
    expect(runSpanOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("actagent.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "actagent.exporter": "diagnostics-otel",
      "actagent.signal": "logs",
      "actagent.status": "started",
      "actagent.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("actagent.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "actagent.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("actagent.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "actagent.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("actagent.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "actagent.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("actagent.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["actagent.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["actagent.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["actagent.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "actagent.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("actagent.payload.large")?.add).toHaveBeenCalledWith(1, {
      "actagent.payload.action": "rejected",
      "actagent.payload.surface": "gateway.frame",
      "actagent.channel": "web",
      "actagent.plugin": "none",
      "actagent.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("actagent.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "actagent.exporter": "diagnostics-otel",
      "actagent.signal": "logs",
      "actagent.status": "failure",
      "actagent.reason": "emit_failed",
      "actagent.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("actagent.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("actagent.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["actagent.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["actagent.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("actagent.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["actagent.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "actagent.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["actagent.content.input_messages"]).toBe("user prompt");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["actagent.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "actagent.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "actagent.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "actagent.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/actagent/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/actagent/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["actagent.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["actagent.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `actagent.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "actagent.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "actagent.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "actagent.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "actagent.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "actagent.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "actagent.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("actagent.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "actagent.channel": "webchat",
      "actagent.agent": "ops",
      "actagent.provider": "openai",
      "actagent.model": "gpt-5.4",
      "actagent.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("actagent.tokens")?.add).toHaveBeenCalledWith(2, {
      "actagent.channel": "unknown",
      "actagent.agent": "unknown",
      "actagent.provider": "openai",
      "actagent.model": "gpt-5.4",
      "actagent.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("actagent.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("actagent.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "actagent.lane": "session",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("actagent.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("keeps only the bounded prefix from scoped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("actagent.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "actagent.lane": "dreaming-narrative",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("actagent.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("actagent.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "actagent.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "actagent.agent": "main",
      "actagent.skill.activation": "read",
      "actagent.skill.name": "tiny-llm-brainstorm",
      "actagent.skill.source": "workspace",
      "actagent.toolName": "read",
    };
    expect(telemetryState.counters.get("actagent.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "actagent.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("actagent.run");
    expect(spanNames).toContain("actagent.model.call");
    expect(spanNames).toContain("actagent.harness.run");
    expect(spanNames).toContain("actagent.tool.execution");

    const runOptions = startedSpanOptions("actagent.run");
    expect(runOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["actagent.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["actagent.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["actagent.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "actagent.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "actagent.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("actagent.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("actagent.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["actagent.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["actagent.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["actagent.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["actagent.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["actagent.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["actagent.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["actagent.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["actagent.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["actagent.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["actagent.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "actagent.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "actagent.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "actagent.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("actagent.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["actagent.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["actagent.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["actagent.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["actagent.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["actagent.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "actagent.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "actagent.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("actagent.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["actagent.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["actagent.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("actagent.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["actagent.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["actagent.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("actagent.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["actagent.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["actagent.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("actagent.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["actagent.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["actagent.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("actagent.model.call");
    expect(modelSpanAttributes["actagent.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["actagent.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["actagent.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("actagent.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "actagent.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("actagent.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["actagent.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["actagent.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["actagent.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "actagent.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "actagent.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("actagent.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["actagent.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "actagent.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "actagent.runId")).toBe(false);

    const toolSpan = spanByName("actagent.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("actagent.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("actagent.model.failover");
    expect(failoverOptions?.attributes?.["actagent.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["actagent.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["actagent.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["actagent.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["actagent.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["actagent.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["actagent.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["actagent.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "actagent.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("actagent.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("actagent.model.failover")).toStrictEqual([
      1,
      {
        "actagent.failover.reason": "overloaded",
        "actagent.failover.suspended": "true",
        "actagent.lane": "main",
        "actagent.model": "claude-opus-4-6",
        "actagent.provider": "anthropic",
        "actagent.failover.to_model": "gpt-5.4",
        "actagent.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("actagent.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "actagent.toolName": "browser",
        "actagent.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "actagent.tool.owner": "browser-tools",
        "actagent.tool.params.kind": "object",
        "actagent.deniedReason": "tools.deny",
      },
    ]);
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalledWith(
      "actagent.tool.execution",
      expect.anything(),
      expect.anything(),
    );
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("actagent.model.failover");
    expect(failoverOptions?.attributes?.["actagent.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "actagent.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("actagent.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("actagent.model.call");
    expect(modelCallOptions?.attributes?.["actagent.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "actagent.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "actagent.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("actagent.provider.request", {
      "actagent.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("actagent.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["actagent.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "actagent.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("actagent.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "actagent.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["actagent.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["actagent.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["actagent.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["actagent.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["actagent.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["actagent.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["actagent.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["actagent.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["actagent.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["actagent.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["actagent.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["actagent.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["actagent.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("actagent.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "actagent.toolName": "process",
      "actagent.loop.level": "critical",
      "actagent.loop.action": "block",
      "actagent.loop.detector": "known_poll_no_progress",
      "actagent.loop.count": 20,
      "actagent.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("actagent.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["actagent.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["actagent.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["actagent.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["actagent.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["actagent.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["actagent.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "actagent.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("actagent.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("actagent.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "actagent.memory.level": "critical",
        "actagent.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("actagent.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "actagent.memory.level": "critical",
      "actagent.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("actagent.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["actagent.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["actagent.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["actagent.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["actagent.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["actagent.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["actagent.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["actagent.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["actagent.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["actagent.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["actagent.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "actagent.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("actagent.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "actagent.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "actagent.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "actagent.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["actagent.run"]).toBeUndefined();
    expect(parentBySpanName["actagent.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["actagent.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("actagent.message.processed");
    const harnessSpan = spanByName("actagent.harness.run");
    const runSpan = spanByName("actagent.run");
    const usageSpan = spanByName("actagent.model.usage");
    const modelCallSpan = spanByName("actagent.model.call");
    ...
    expect(parentBySpanName["actagent.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["actagent.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["actagent.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["actagent.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["actagent.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("actagent.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("actagent.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("actagent.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("actagent.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("actagent.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("actagent.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("actagent.message.processed")["actagent.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("actagent.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("actagent.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("actagent.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("actagent.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("actagent.run").spanContext();
    const modelParentContext = startedSpanParentContexts("actagent.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("actagent.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("actagent.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["actagent.run"]).toBeUndefined();
    expect(parentBySpanName["actagent.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["actagent.run"]).toBeUndefined();
    expect(parentBySpanName["actagent.model.call"]).toBeUndefined();
    expect(parentBySpanName["actagent.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "actagent.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "actagent.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "actagent.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("actagent.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["actagent.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["actagent.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["actagent.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["actagent.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("actagent.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["actagent.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["actagent.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["actagent.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["actagent.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["actagent.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["actagent.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["actagent.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "actagent.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "actagent.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "actagent.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("actagent.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("actagent.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("actagent.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "actagent.channel": "matrix",
      "actagent.delivery.kind": "text",
    });
    ...
    expect(deliveryDurationRecords[0]?.[1]["actagent.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["actagent.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["actagent.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["actagent.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["actagent.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["actagent.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["actagent.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "actagent.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["actagent.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["actagent.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["actagent.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["actagent.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["actagent.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["actagent.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["actagent.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "actagent.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "actagent.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("actagent.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["actagent.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["actagent.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["actagent.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("actagent.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["actagent.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["actagent.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["actagent.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["actagent.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("actagent.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["actagent.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["actagent.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["actagent.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("actagent.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["actagent.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["actagent.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["actagent.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("actagent.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["actagent.status"]).toBe("released");
    expect(telemetryState.counters.get("actagent.talk.event")?.add).toHaveBeenCalledWith(1, {
      "actagent.talk.brain": "agent-consult",
      "actagent.talk.event_type": "input.audio.delta",
      "actagent.talk.mode": "realtime",
      "actagent.talk.provider": "openai",
      "actagent.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("actagent.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "actagent.talk.brain": "agent-consult",
        "actagent.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("actagent.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "actagent.talk.brain": "agent-consult",
      "actagent.talk.event_type": "latency.metrics",
      "actagent.talk.mode": "realtime",
      "actagent.talk.provider": "openai",
      "actagent.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("actagent.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("actagent.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "actagent.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("actagent.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "actagent.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "actagent.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["actagent.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["actagent.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["actagent.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["actagent.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["actagent.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["actagent.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["actagent.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "actagent.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "actagent.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "actagent.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "actagent.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["actagent.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("actagent.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["actagent.reason"])).toContain("…");
    expect(typeof attrs?.["actagent.reason"]).toBe("string");
    expect(String(attrs?.["actagent.reason"])).not.toContain(

...
export function shouldShowTelegramDmAccessWarning(cfg: ACTAgentConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`actagent config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`actagent config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),

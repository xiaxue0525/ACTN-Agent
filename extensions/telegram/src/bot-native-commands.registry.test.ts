...
import { clearPluginCommands, registerPluginCommand } from "actagent/plugin-sdk/plugin-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it } from "vitest";

let registerTelegramNativeCommands: typeof import("./bot-native-commands.js").registerTelegramNativeCommands;
let setActivePluginRegistry: typeof import("actagent/plugin-sdk/plugin-test-runtime").setActivePluginRegistry;
...
describe("registerTelegramNativeCommands real plugin registry", () => {
  beforeAll(async () => {
    ({ setActivePluginRegistry } = await import("actagent/plugin-sdk/plugin-test-runtime"));
  ...
  it("allows requireAuth:false plugin commands for unauthorized senders through the real registry", async () => {
    ...
    registerTelegramNativeCommands({
      ...createNativeCommandTestParams({
        commands: { allowFrom: { telegram: ["999"] } } as ACTAgentConfig["commands"],
      }),
      bot,
      allowFrom: ["999"],
      nativeEnabled: false,

...
function formatTelegramAccountConfigPath(cfg: ACTAgentConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: ACTAgentConfig): {
...
async function repairTelegramConfig(params: { cfg: ACTAgentConfig }): Promise<{
  config: ACTAgentConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: ACTAgentConfig): Promise<{
  config: ACTAgentConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("actagent/plugin-sdk/runtime");

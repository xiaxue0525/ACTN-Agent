...
} from "actagent/plugin-sdk/allowlist-config-edit";
import type { ChannelMessageActionAdapter } from "actagent/plugin-sdk/channel-contract";
import {
...
import { createAccountStatusSink } from "actagent/plugin-sdk/channel-outbound";
...
import { createChannelDirectoryAdapter } from "actagent/plugin-sdk/directory-runtime";
import { formatErrorMessage } from "actagent/plugin-sdk/error-runtime";
import type { RoutePeer } from "actagent/plugin-sdk/routing";
import {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<ACTAgentConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside ACTAgent state; translate it back
  ...
  base: {
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as ACTAgentConfig;

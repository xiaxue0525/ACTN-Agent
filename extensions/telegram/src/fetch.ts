...
import { resolveRequestUrl } from "actagent/plugin-sdk/request-url";
...
// fix for the leak observed in actagent#68128 is the transport lifecycle that
...
function resolveACTAgentProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.ACTAGENT_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveACTAgentProxyUrlForTelegram() : undefined;

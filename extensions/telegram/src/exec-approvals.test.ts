...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<ACTAgentConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<ACTAgentConfig["channels"]>["telegram"]>>,
): ACTAgentConfig {
  return {
    channels: {
      telegram: {

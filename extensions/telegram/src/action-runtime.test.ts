...
import { resolveStorePath } from "actagent/plugin-sdk/session-store-runtime";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ACTAgentConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ACTAgentConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ACTAgentConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: ACTAgentConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ACTAGENT_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ACTAgentConfig;
    await expect(
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("actagent/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "actagent-telegram-action-sessions.json") },
    } as ACTAgentConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ACTAgentConfig, accountId = "media") {
    await handleTelegramAction(

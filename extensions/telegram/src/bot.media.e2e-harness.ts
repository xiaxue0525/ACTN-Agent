...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("actagent/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("actagent/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "actagent-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as ACTAgentConfig) as TelegramBotDeps["getRuntimeConfig"];

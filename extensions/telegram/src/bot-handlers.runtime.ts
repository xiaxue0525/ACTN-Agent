...
import { parseExecApprovalCommandText } from "actagent/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "actagent/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "actagent/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "actagent/plugin-sdk/command-auth-native";
import { hasControlCommand } from "actagent/plugin-sdk/command-detection";
import { isAbortRequestText } from "actagent/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "actagent/plugin-sdk/command-status";
import type { DmPolicy, ACTAgentConfig } from "actagent/plugin-sdk/config-contracts";
import type {
  TelegramGroupConfig,
  TelegramTopicConfig,
...
import { applyModelOverrideToSessionEntry } from "actagent/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "actagent/plugin-sdk/models-provider-runtime";
...
import { resolveThreadSessionKeys } from "actagent/plugin-sdk/routing";
import { danger, logVerbose, warn } from "actagent/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in actagent.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,

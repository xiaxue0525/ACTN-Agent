...
import { resolveChannelStreamingBlockEnabled } from "actagent/plugin-sdk/channel-outbound";
import { resolveNativeCommandSessionTargets } from "actagent/plugin-sdk/command-auth-native";
import {
...
import type { ChannelGroupPolicy } from "actagent/plugin-sdk/config-contracts";
import type {
  ReplyToMode,
...
import { getRuntimeConfigSnapshot } from "actagent/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "actagent/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("actagent/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("actagent/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): ACTAgentConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: ACTAgentConfig): TelegramAccountConfig => {
    try {

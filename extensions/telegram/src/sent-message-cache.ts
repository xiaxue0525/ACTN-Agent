...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("actagent.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "actagent.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<ACTAgentConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<ACTAgentConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<ACTAgentConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<ACTAgentConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<ACTAgentConfig, "session">,
): void {

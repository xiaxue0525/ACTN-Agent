...
vi.mock("actagent/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/config-mutation")>(
    "actagent/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/cron-store-runtime")>(
    "actagent/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/actagent.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);

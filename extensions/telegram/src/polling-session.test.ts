...
import {
  ...
  type ACTAgentStateKyselyDatabaseForTests,
...
vi.mock("actagent/plugin-sdk/delivery-queue-runtime", () => ({
...
type TelegramPollingTestDatabase = Pick<
  ACTAgentStateKyselyDatabaseForTests,
...
function openTelegramSpoolTestKysely(spoolDir: string) {
  const database = openACTAgentStateDatabase({
    env: { ...process.env, ACTAGENT_STATE_DIR: spoolDir },
  });
  return {
    database,
...
async function withTempSpool<T>(fn: (spoolDir: string) => Promise<T>): Promise<T> {
  const spoolDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-telegram-spool-"));
  try {
...
describe("TelegramPollingSession", () => {
  ...
  beforeEach(() => {
    ...
    installTelegramIngressQueueRuntime(() =>
      path.join(os.tmpdir(), "actagent-telegram-test-state"),
    );
  });
  ...
  it("does not call getUpdates for offset confirmation (avoiding 409 conflicts)", async () => {
    ...
    // ACTAgent middleware still skips duplicates using the persisted update offset.
  ...
  it("initializes the main-thread bot before draining isolated ingress spool", async () => {
    const abort = new AbortController();
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-telegram-spool-"));
  ...
  it("logs an actionable duplicate-poller hint for getUpdates conflicts", async () => {
    ...
    expectLogIncludes(log, "Another ACTAgent gateway, script, or Telegram poller");
  });

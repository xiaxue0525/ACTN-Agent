/**
 * ACTAgent ACPX runtime adapter. It wraps the upstream acpx runtime with
 * ACTAgent session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { redactSensitiveText } from "actagent/plugin-sdk/security-runtime";
...
type ACTAgentAcpxRuntimeOptions = AcpRuntimeOptions & {
  actagentWrapperRoot?: string;
  actagentGatewayInstanceId?: string;
  actagentProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  actagentProcessCleanup?: AcpxProcessCleanupDeps;
};
type ACTAgentRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type ACTAgentRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type ACTAgentLeaseSessionMetadata = {
  actagentLeaseId: string;
  actagentGatewayInstanceId: string;
};

function withACTAgentManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // ACTAgent owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withACTAgentLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: ACTAgentLeaseSessionMetadata,
): T & ACTAgentLeaseSessionMetadata {
  return {
    ...record,
    actagentLeaseId: metadata.actagentLeaseId,
    actagentGatewayInstanceId: metadata.actagentGatewayInstanceId,
  };
}
...
function readACTAgentLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { actagentLeaseId } = record as { actagentLeaseId?: unknown };
  return typeof actagentLeaseId === "string" ? actagentLeaseId.trim() || undefined : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withACTAgentLeaseSessionMetadata(record, {
        actagentLeaseId: lease.leaseId,
        actagentGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withACTAgentLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            actagentLeaseId: launch.leaseId,
            actagentGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const ACTAGENT_BRIDGE_EXECUTABLE = "actagent";
const ACTAGENT_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_ACTAGENT_PREFIX = "openai/";
...
function isACTAgentBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === ACTAGENT_BRIDGE_EXECUTABLE) {
    return parts[1] === ACTAGENT_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^actagent(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === ACTAGENT_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_ACTAGENT_PREFIX)) {
    value = value.slice(CODEX_ACP_ACTAGENT_PREFIX.length);
  }
...
function withAcpxSessionOptions(input: ACTAgentRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isACTAgentBridgeCommand(command);
}
...
/** ACTAgent-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: ACTAgentAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { actagentProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = actagentProcessCleanup;
    this.wrapperRoot = options.actagentWrapperRoot;
    this.gatewayInstanceId = options.actagentGatewayInstanceId;
    this.processLeaseStore = options.actagentProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readACTAgentLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readACTAgentLeaseIdFromRecord(record);
    ...
    await cleanupACTAgentOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
      wrapperRoot: this.wrapperRoot,
      deps: this.processCleanupDeps,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withACTAgentManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: ACTAgentRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withACTAgentManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;

...
const { cleanupACTAgentOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupACTAgentOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleACTAgentOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleACTAgentOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupACTAgentOwnedAcpxProcessTree: cleanupACTAgentOwnedAcpxProcessTreeMock,
  reapStaleACTAgentOwnedAcpxOrphans: reapStaleACTAgentOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { ACTAgentPluginServiceContext } from "../runtime-api.js";
import { createAcpxRuntimeService, resolveAcpxTimerTimeoutMs } from "./service.js";

const tempDirs: string[] = [];
const previousEnv = {
  ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE: process.env.ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE,
  ACTAGENT_SKIP_ACPX_RUNTIME: process.env.ACTAGENT_SKIP_ACPX_RUNTIME,
  ACTAGENT_SKIP_ACPX_RUNTIME_PROBE: process.env.ACTAGENT_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupACTAgentOwnedAcpxProcessTreeMock.mockClear();
  reapStaleACTAgentOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("ACTAGENT_SKIP_ACPX_RUNTIME");
  restoreEnv("ACTAGENT_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): ACTAgentPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".actagent-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.ACTAGENT_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupACTAgentOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupACTAgentOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale ACTAgent-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    reapStaleACTAgentOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupACTAgentOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleACTAgentOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale ACTAgent-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.ACTAGENT_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({

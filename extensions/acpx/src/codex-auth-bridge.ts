...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--actagent-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @actagent/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep ACTAgent's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const actAgentWrapperArgs = new Set([
  ${quoteCommandPart(ACTAGENT_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(ACTAGENT_GATEWAY_INSTANCE_ID_ARG)},
]);

function readACTAgentWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(ACTAGENT_ACPX_LEASE_ID_ENV)}] ||
    readACTAgentWrapperArg(args, ${quoteCommandPart(ACTAGENT_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripACTAgentWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (actAgentWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripACTAgentWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[actagent] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[actagent] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in ACTAgent; fallback must not float to an unpatched newer release.

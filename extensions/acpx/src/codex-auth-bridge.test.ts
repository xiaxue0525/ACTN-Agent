...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-acpx-codex-auth-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("prepareAcpxCodexAuthConfig", () => {
  it("installs an isolated Codex ACP wrapper without synthesizing auth from canonical ACTAgent OAuth", async () => {
  ...
  it("launches the locally installed Codex ACP bin with isolated CODEX_HOME", async () => {
    ...
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        "--actagent-acpx-lease-id",
        "lease-1",
        "--actagent-gateway-instance-id",
        "gateway-1",
      ],
      {
        cwd: root,

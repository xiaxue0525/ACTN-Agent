...
import { spawn as startACTAgentCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `actagent ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaACTAgentCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startACTAgentCliProcess>,
...
export function startMatrixQaACTAgentCli(params: {
  ...
  const distEntryPath = resolveMatrixQaACTAgentCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startACTAgentCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaACTAgentCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaACTAgentCli(params).wait();
}
...
export async function createMatrixQaACTAgentCliRuntime(params: {
  accountId: string;
  accessToken: string;

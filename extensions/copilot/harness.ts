...
export function createCopilotAgentHarness(
  ...
  // Maps ACTAgent session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const actagentSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same ACTAgent session so that the GitHub Copilot agent runtime's prompt
        ...
        const tracked = actagentSessionId ? trackedSessions.get(actagentSessionId) : undefined;
        const stored = actagentSessionId
          ? resetBlockedStoredSessions.has(actagentSessionId)
            ? undefined
            : lookupStoredBinding(options?.sessionStore, actagentSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: actagentSessionId
            ? ({
                sdkSessionId,
                pooledClient,
                sessionConfig,
                ...
                trackedSessions.set(actagentSessionId, {
                  sdkSessionId,
                ...
                const persisted = registerStoredBinding(options?.sessionStore, actagentSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                if (persisted) {
                  resetBlockedStoredSessions.delete(actagentSessionId);
                }
              }
            : undefined,
        });
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const actagentSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!actagentSessionId) {
        return;
      }
      const tracked = trackedSessions.get(actagentSessionId);
      if (deleteStoredBinding(options?.sessionStore, actagentSessionId)) {
        resetBlockedStoredSessions.delete(actagentSessionId);
      } else {
        resetBlockedStoredSessions.add(actagentSessionId);
      }
      if (!tracked) {
        // Session was created by a different harness, or already reset.
        return;
      }
      trackedSessions.delete(actagentSessionId);
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. ACTAgent only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // ACTAgent session/transcript state stays in SQLite, with no marker
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with ACTAgent hooks/tools, so preserve the
      ...
      try {
        ...
        if (isStaleSdkSessionError(err)) {
          trackedSessions.delete(actagentSessionId);
          deleteStoredBinding(options?.sessionStore, actagentSessionId);
          return {
            ok: false,
            compacted: false,

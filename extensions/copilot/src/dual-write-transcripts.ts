 ...
 * ACTAgent audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The ACTAgent shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the ACTAgent transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  const existing = record["__actagent"];
  const baseMeta =
  ...
  return {
    ...record,
    __actagent: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the ACTAgent audit trail just misses the intermediate messages for
 * this turn.
 */

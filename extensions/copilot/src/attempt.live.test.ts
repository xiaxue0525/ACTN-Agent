...
import type { AgentHarnessAttemptParams } from "actagent/plugin-sdk/agent-harness-runtime";
...
vi.mock("actagent/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("actagent/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createACTAgentCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["ACTAGENT_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.ACTAGENT_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "actagent-copilot-live-"));

...
// actagent/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project actagent-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the ACTAgent audit transcript so
...
export interface CopilotAttemptDeps {
   ...
   * `actagent/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the actagentSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildACTAgentCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load ACTAgent workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // ACTAgent bootstrap block (SDK still loads AGENTS.md natively).
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: ACTAgent may
  ...
  //
  // Without this synthesis the ACTAgent audit transcript never sees
  ...
  // (user/assistant/toolResult) into the ACTAgent audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // ACTAgent shell (attempt-execution.ts) writes only the user prompt
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      // authoritative; only the ACTAgent audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through ACTAgent's
    ...
    // requires routing the request to the ACTAgent channel/TUI prompt
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // ACTAgent workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + ACTAgent context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no ACTAgent-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __actagent?: unknown };

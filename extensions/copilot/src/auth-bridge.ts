 ...
 *     use cases.
 *   - Falls back to ACTAGENT_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<actAgentHome>/.actagent/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `ACTAGENT_HOME` for the home directory root.
 ...
 *   4. ACTAGENT_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with ACTAgent's own files
  ...
  const actAgentHome = readString(args.env.ACTAGENT_HOME);
  const rootHome = actAgentHome ? resolve(actAgentHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <actAgentHome>/.actagent/agents/<agentId>/copilot
  return resolve(join(rootHome, ".actagent", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // ACTAGENT_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "ACTAGENT_GITHUB_TOKEN", value: readString(env.ACTAGENT_GITHUB_TOKEN) },

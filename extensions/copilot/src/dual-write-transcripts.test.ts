...
import type { AgentMessage } from "actagent/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-copilot-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the ACTAgent transcript", async () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("actagent-copilot-mirror-missing-dir-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    await mirrorCopilotTranscript({
      sessionFile,
      messages: [tagged],
      idempotencyScope: "copilot:actagent-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot:actagent-session-1:sdk-session-1:assistant:0"',
    );

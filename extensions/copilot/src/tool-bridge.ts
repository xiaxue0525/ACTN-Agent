...
type CreateACTAgentCodingTools =
  (typeof import("actagent/plugin-sdk/agent-harness"))["createACTAgentCodingTools"];
type ACTAgentCodingToolsOptions = NonNullable<Parameters<CreateACTAgentCodingTools>[0]>;
 ...
 * authoritative `createACTAgentCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `actagent/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createACTAgentCodingTools` so the
  ...
  createACTAgentCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createACTAgentCodingTools =
    input.createACTAgentCodingTools ??
    (await import("actagent/plugin-sdk/agent-harness")).createACTAgentCodingTools;

  const toolOptions = buildACTAgentCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createACTAgentCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createACTAgentCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createACTAgentCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertACTAgentToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createACTAgentCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged ACTAgent tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertACTAgentToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
...
function buildACTAgentCodingToolsOptions(
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as ACTAgentCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertACTAgentToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // ACTAgent owns its bridged tools by design (the harness docs:
    // "ACTAgent still owns ... ACTAgent dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with ACTAgent's coding-tool set. Mark every bridged tool as
    ...
    // same name." ACTAgent's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged ACTAgent tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit ACTAgent's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged ACTAgent tools are wrapped with

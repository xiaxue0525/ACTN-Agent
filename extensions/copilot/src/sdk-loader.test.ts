...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { actagentDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { actagentDefaultImporterSentinel?: boolean };

      expect(sdk.actagentDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("actagent plugins install @actagent/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("actagent plugins install @actagent/copilot");
  });
  ...
  it("default fallback dir points at ~/.actagent/npm-runtime/copilot", () => {
    expect(COPILOT_SDK_FALLBACK_DIR).toMatch(/\.actagent[\\/]+npm-runtime[\\/]+copilot$/);
  });

  it("resolves the fallback dir from ACTAGENT_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        ACTAGENT_STATE_DIR: "/tmp/actagent-state",
      }),
    ).toBe(path.join("/tmp/actagent-state", "npm-runtime", "copilot"));
  });

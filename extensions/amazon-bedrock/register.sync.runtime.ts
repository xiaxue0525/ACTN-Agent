...
import { registerApiProvider, streamSimple } from "actagent/plugin-sdk/llm";
 ...
 * IDs that may not contain the model name). When ACTAgent's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if ACTAgent identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher ACTAgent uses for direct model IDs.
 */
 ...
 * the ACTAgent config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: ACTAgentPluginApi): void {

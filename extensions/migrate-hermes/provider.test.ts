...
import { createCapturedPluginRegistration } from "actagent/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when ACTAGENT_HOME is set", () => {
    const previous = process.env.ACTAGENT_HOME;
    process.env.ACTAGENT_HOME = path.join(path.sep, "tmp", "actagent-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ACTAGENT_HOME = previous;
      }
    }
  });

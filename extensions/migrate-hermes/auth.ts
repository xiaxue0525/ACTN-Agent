  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes auth.json contains legacy OAuth credentials. ACTAgent no longer imports those into live auth during Hermes migration.",
        recommendation:
          "Run actagent models auth login --provider openai after migration, or run actagent doctor --fix for existing ACTAgent legacy auth state.",
      }),
    );
  }

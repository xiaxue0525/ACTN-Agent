...
import { resolveAuthStorePathForDisplay } from "actagent/plugin-sdk/agent-runtime";
...
vi.mock("actagent/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("actagent/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}

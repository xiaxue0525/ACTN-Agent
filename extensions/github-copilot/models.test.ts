...
import { createProviderUsageFetch, makeResponse } from "actagent/plugin-sdk/test-env";
...
vi.mock("actagent/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("actagent/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/actagent-state",
}));

import type { ProviderResolveDynamicModelContext } from "actagent/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/actagent-state/credentials/github-copilot.token.json";

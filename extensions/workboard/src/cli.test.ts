...
vi.mock("actagent/plugin-sdk/gateway-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/gateway-runtime")>(
    "actagent/plugin-sdk/gateway-runtime",
  );
  return {
    ...actual,
...
describe("registerWorkboardCli", () => {
  ...
  it("does not fall back to local dispatch for explicit gateway targets", async () => {
    ...
    gatewayRuntime.callGatewayFromCli.mockRejectedValueOnce(
      new Error("connect ECONNREFUSED 127.0.0.1:19199"),
    );

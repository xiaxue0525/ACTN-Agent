...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/actagent/",
      }),
    ).toBe("https://example.com/actagent");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/actagent?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/actagent#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/actagent/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/actagent");
...
describe("diffs viewer URL helpers", () => {
  it("defaults to loopback for lan/tailnet bind modes", () => {
    expect(
      buildViewerUrl({
        config: { gateway: { bind: "lan", port: 19199 } },
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("http://127.0.0.1:19199/plugins/diffs/view/id/token");
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/actagent",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/actagent/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("actagentDiffsReady");
...
describe("resolveDiffsLanguagePackAvailability", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      const root = fs.mkdtempSync(join(os.tmpdir(), "actagent-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "actagent.plugin.json"),

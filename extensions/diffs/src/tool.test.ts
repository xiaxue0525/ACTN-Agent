...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("actagent-diffs-tool-"));
  });
  ...
  it("returns a viewer URL in view mode", async () => {
    ...
    expect(text).toContain("http://127.0.0.1:19199/plugins/diffs/view/");
    expect(String(readDetails(result).viewerUrl)).toContain(
      "http://127.0.0.1:19199/plugins/diffs/view/",
    );
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/actagent/plugins/diffs/view/",
    );
...
function createApi(pluginConfig?: Record<string, unknown>): ACTAgentPluginApi {
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: ACTAgentPluginToolContext = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",

...
import type { ACTAgentPluginApi, ACTAgentPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("actagent-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<ACTAgentPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: ACTAgentPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: ACTAgentConfig = {
      gateway: {
        port: 19199,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/actagent",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<ACTAgentPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as ACTAgentPluginApi);

...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "actagent/plugin-sdk/realtime-voice";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "ACTAgent agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the ACTAgent agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call actagent_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as ACTAgentConfig,
      agentId,

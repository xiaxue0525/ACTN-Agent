...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured ACTAgent agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as ACTAgentConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as ACTAgentConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {

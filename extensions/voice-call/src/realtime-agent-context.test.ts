...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "actagent-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity and workspace context", async () => {
    ...
    await writeFile(path.join(workspaceDir, "IDENTITY.md"), "Name: actagent Voice\nVibe: snappy\n");
    ...
    expect(instructions).toContain("ACTAgent agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call actagent_agent_consult before answering requests");
    expect(instructions).toContain("- Agent id: voice");
    expect(instructions).toContain("- Name: actagent Voice");

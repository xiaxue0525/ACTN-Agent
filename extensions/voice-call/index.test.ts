...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<ACTAgentPluginApi["registerService"]>[0];
};
type MockCallSource = {
  mock: {
...
function setup(config: Record<string, unknown>): Registered {
  ...
  const api = createTestPluginApi({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: vi.fn() } } as unknown as ACTAgentPluginApi["runtime"],
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  beforeEach(() => {
    ...
    callGatewayFromCliMock.mockRejectedValue(new Error("connect ECONNREFUSED 127.0.0.1:19199"));
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("actagent.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("actagent.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("actagent.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("ACTAGENT_CLI", "1");
    process.argv = ["node", "actagent", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "actagent doctor --fix"');
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager["initiateCall"]).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "ACTAgent voice call smoke test.",
        mode: "notify",
      });

...
import { expectExplicitVideoGenerationCapabilities } from "actagent/plugin-sdk/provider-test-contracts";
...
describe("fal video generation provider", () => {
  ...
  it("submits HeyGen video-agent requests without unsupported fal controls", async () => {
    ...
    const result = await provider.generateVideo({
      ...
      prompt: "A founder explains ACTAgent in a concise studio video",
      durationSeconds: 8,
      aspectRatio: "16:9",
      resolution: "720P",
      audio: true,

  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking ACTAgent to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the ACTAgent Google Meet microphone before asking ACTAgent to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };

...
import { asRecord, normalizeOptionalString } from "actagent/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the ACTAgent browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the ACTAgent browser profile; configure browser.defaultProfile to choose another profile",
  });

...
import { runPluginCommandWithTimeout } from "actagent/plugin-sdk/run-command";
import type { RuntimeEnv } from "actagent/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "actagent/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredACTAgentTmpDir(), "actagent-signal-"));

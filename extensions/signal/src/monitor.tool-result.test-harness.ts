...
import type { MockFn } from "actagent/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/actagent-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("actagent/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("actagent/plugin-sdk/runtime-config-snapshot")
  >("actagent/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/reply-runtime")>(
...
vi.mock("actagent/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/system-event-runtime")>(
    "actagent/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("actagent/plugin-sdk/reply-runtime"),
      import("actagent/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {

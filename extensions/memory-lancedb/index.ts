/**
 * ACTAgent Memory (LanceDB) Plugin
 *
...
import type { AgentToolResult } from "actagent/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "actagent/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "actagent/plugin-sdk/chat-channel-ids";
import type { ACTAgentConfig } from "actagent/plugin-sdk/config-contracts";
import type { MemoryEmbeddingProvider } from "actagent/plugin-sdk/memory-core-host-engine-embeddings";
import {
  parseStrictPositiveInteger,
  resolveTimerTimeoutMs,
} from "actagent/plugin-sdk/number-runtime";
import { readFiniteNumberParam, readPositiveIntegerParam } from "actagent/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "actagent/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "actagent/plugin-sdk/runtime-env";
import {
...
import { definePluginEntry, type ACTAgentPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("actagent/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("actagent/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("actagent/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("actagent/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("actagent/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("actagent/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as ACTAgentConfig;
...
function createEmbeddings(api: ACTAgentPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Matches JSON blobs that look like ACTAgent transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains ACTAgent-injected envelope or
...
/**
 * Strips ACTAgent-injected envelope metadata from a user message so that only

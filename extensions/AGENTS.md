...
- Extension production code should import from `actagent/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `actagent.plugin.json` and the package
  `actagent` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `actagent/plugin-sdk/*`.

...
description: Maintain the ACTAgent memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `actagent wiki ingest`, `actagent wiki compile`, and `actagent wiki lint` as the default maintenance loop.
- In `bridge` mode, run `actagent wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `actagent wiki unsafe-local import` only when the user explicitly opted into private local path access.

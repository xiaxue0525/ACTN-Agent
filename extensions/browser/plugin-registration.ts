...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "ACTAGENT_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage ACTAgent's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via ACTAgent's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated ACTAgent-managed browser (`actagent`).",
...
function createBrowserToolOptions(ctx: ACTAgentPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: ACTAgentPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: ACTAgentPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): ACTAgentPluginService {
  let service: ACTAgentPluginService | null = null;
...
export function registerBrowserPlugin(api: ACTAgentPluginApi) {
  api.registerTool(((ctx: ACTAgentPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as ACTAgentPluginToolFactory);
  api.registerCli(
    async ({ program }) => {

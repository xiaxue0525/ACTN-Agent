...
import { DEFAULT_ACTAGENT_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
const LEGACY_actagentd_BROWSER_PROFILE_NAME = "actagentd";
...
/** Legacy managed actagentd profile paths that can be archived by doctor --fix. */
...
function collectChromeMcpProfiles(cfg: ACTAgentConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: ACTAgentConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "actagent";
    if (driver !== "existing-session") {
...
function isLegacyactagentdProfileConfigured(cfg: ACTAgentConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
  if (normalizeOptionalString(browser.defaultProfile) === LEGACY_actagentd_BROWSER_PROFILE_NAME) {
    return true;
  }
  ...
  if (Object.hasOwn(configuredProfiles, LEGACY_actagentd_BROWSER_PROFILE_NAME)) {
    return true;
  }
...
/** Detects unmanaged legacy actagentd browser profile residue on disk. */
export function detectLegacyactagentdBrowserProfileResidue(
  cfg: ACTAgentConfig,
  ...
  const legacyProfileDir = resolveManagedBrowserProfileDir(
    configDir,
    LEGACY_actagentd_BROWSER_PROFILE_NAME,
  );
  ...
  if (isLegacyactagentdProfileConfigured(cfg, legacyProfileDir)) {
    return null;
  }
  ...
  if (
    resolved.defaultProfile !== DEFAULT_ACTAGENT_BROWSER_PROFILE_NAME ||
  ...
  return {
    ...
    canonicalUserDataDir: resolveManagedBrowserUserDataDir(
      configDir,
      DEFAULT_ACTAGENT_BROWSER_PROFILE_NAME,
    ),
  };
}

function formatLegacyactagentdBrowserProfileResidueNote(
  residue: LegacyactagentdBrowserProfileResidue,
): string {
  return [
    `- Legacy managed browser profile residue was found at ${residue.legacyProfileDir}.`,
    `- The canonical ACTAgent-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("actagent doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  const resolved = resolveBrowserConfig(cfg.browser, cfg);
  const legacyactagentdResidue = detectLegacyactagentdBrowserProfileResidue(cfg, {
    configDir: deps?.configDir,
    pathExists: deps?.pathExists,
  });
  if (legacyactagentdResidue) {
    noteFn(formatLegacyactagentdBrowserProfileResidueNote(legacyactagentdResidue), "Browser");
  }
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- ACTAgent-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for ACTAgent-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- ACTAgent-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time ACTAgent connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. ACTAgent does not bundle Chrome.`,
...
/** Archives legacy actagentd browser profile residue when doctor --fix is requested. */
export async function maybeArchiveLegacyactagentdBrowserProfileResidue(
  cfg: ACTAgentConfig,
  deps?: BrowserDoctorFilesystemDeps,
): Promise<{ changes: string[]; warnings: string[] }> {
  const residue = detectLegacyactagentdBrowserProfileResidue(cfg, deps);
  if (!residue) {
  ...
  try {
    ...
    return {
      changes: [
        [
          "Archived legacy actagentd managed browser profile residue.",
    ...
    return {
      changes: [],
      warnings: [`Legacy actagentd browser profile residue could not be archived: ${message}`],
    };
  }
}

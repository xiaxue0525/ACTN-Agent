...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        actagent: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ACTAGENT_GATEWAY_PORT;
    process.env.ACTAGENT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ACTAGENT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ACTAGENT_GATEWAY_PASSWORD;

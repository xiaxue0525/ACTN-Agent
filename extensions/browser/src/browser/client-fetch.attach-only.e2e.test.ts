...
import { clearRuntimeConfigSnapshot } from "actagent/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("actagent-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".actagent", "actagent.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to ACTAgent");
      expect(message).toContain("Restarting the ACTAgent gateway will not launch it");
      expect(message).not.toContain("Restart the ACTAgent gateway");

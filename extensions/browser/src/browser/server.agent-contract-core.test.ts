...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "actagent" }),
    });
    ...
    const createactagentd = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "legacyactagentd", driver: "actagentd" }),
    });
    expect(createactagentd.status).toBe(200);
    const createactagentdBody = (await createactagentd.json()) as {
      profile?: string;
      transport?: string;
      cdpPort?: number | null;
      userDataDir?: string | null;
    };
    expect(createactagentdBody.profile).toBe("legacyactagentd");
    expect(createactagentdBody.transport).toBe("cdp");
    expect(createactagentdBody.cdpPort).toBeTypeOf("number");
    expect(createactagentdBody.userDataDir).toBeNull();

    const explicitUserDataDir = "/tmp/actagent-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/actagent`, {
      method: "DELETE",
    });

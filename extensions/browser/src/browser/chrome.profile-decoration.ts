/**
 * ACTAgent-managed Chrome profile decoration.
 *
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".actagent-profile-decorated");
}
 ...
 */
export function decorateACTAgentProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ACTAGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ACTAGENT_BROWSER_COLOR).toUpperCase();

...
function localACTAgentProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "actagent",
...
function createLocalACTAgentResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localACTAgentProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveACTAgentUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-reset-"));
    const profileDir = path.join(tempRoot, "actagent");
    ...
    const ops = createLocalACTAgentResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveACTAgentUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-reset-no-own-"));
    const profileDir = path.join(tempRoot, "actagent");
    fs.mkdirSync(profileDir, { recursive: true });

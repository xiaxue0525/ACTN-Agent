 ...
 */
import { resolveNonNegativeIntegerOption } from "actagent/plugin-sdk/number-runtime";
  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-actagent-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-actagent-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-actagent-labels", "1");
          ...
          tag.setAttribute("data-actagent-labels", "1");

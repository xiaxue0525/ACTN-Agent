...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredACTAgentTmpDir: vi.fn(() => "/tmp/actagent"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-actagent-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-actagent-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredACTAgentTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredACTAgentTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredACTAgentTmpDir.mockReturnValue("/tmp/actagent");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredACTAgentTmpDir.mockReturnValue("/tmp/actagent-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "actagent-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "actagent-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredACTAgentTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "actagent-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "actagent-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredACTAgentTmpDir.mockReturnValue(tempDir);

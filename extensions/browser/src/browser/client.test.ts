...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("actagent");
    ...
    expect(deepDoctorResult.profile).toBe("actagent");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=actagent&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=actagent&deep=true"));

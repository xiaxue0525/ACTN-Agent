...
const resolvePreferredACTAgentTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/actagent"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "actagent-browser-")));
    ...
    resolvePreferredACTAgentTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "actagent-demo");

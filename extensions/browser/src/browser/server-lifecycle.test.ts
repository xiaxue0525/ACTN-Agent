...
const { stopACTAgentChromeMock } = vi.hoisted(() => ({
  stopACTAgentChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopACTAgentChrome: stopACTAgentChromeMock,
}));
...
beforeEach(() => {
  ...
  stopACTAgentChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["actagent", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      actagent: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.actagent).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopACTAgentChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("actagent browser stop failed: Error: oops");
  });
});

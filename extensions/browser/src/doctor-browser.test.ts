...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- ACTAgent-managed browser profile(s) are configured: actagent.",
  ...
  it("warns about legacy actagentd managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/actagent-home";
    ...
    await noteChromeMcpBrowserReadiness(
      ...
      },
      {
        ...
        getUid: () => 1000,
        configDir,
        pathExists: (targetPath) => targetPath.endsWith("/browser/actagentd/user-data"),
        resolveManagedExecutable: () => ({ kind: "chrome", path: "/usr/bin/google-chrome" }),
      },
    );
    ...
    expect(note).toContain("/tmp/actagent-home/browser/actagentd");
    expect(note).toContain("/tmp/actagent-home/browser/actagent/user-data");
    expect(note).toContain("actagent doctor --fix");
  });

  it("does not warn when actagentd is still configured as a browser profile", async () => {
    const noteFn = vi.fn();
    ...
    await noteChromeMcpBrowserReadiness(
      {
        browser: {
          profiles: {
            actagentd: { color: "#FF4500" },
            actagent: { color: "#00AA00" },
          },
        },
      },
      {
...
describe("legacy actagentd browser profile cleanup", () => {
  it("archives stale actagentd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/actagent-home/browser/.trash/actagentd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/actagent-home/browser/actagentd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy actagentd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/actagent-home/browser/actagent/user-data");
  });

  it("does not archive a configured actagentd browser profile", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/unused");

    const result = await maybeArchiveLegacyactagentdBrowserProfileResidue(
      {
        browser: {
          defaultProfile: "actagentd",
          profiles: {
            actagentd: { color: "#FF4500" },
          },
        },

...
type BrowserAutoEnableProbe = Parameters<ACTAgentPluginApi["registerAutoEnableProbe"]>[0];
...
function createApi() {
  ...
  const api = createTestPluginApi({
    ...
    source: "test",
    config: {},
    runtime: {} as ACTAgentPluginApi["runtime"],
    registerCli,
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "actagent.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/actagent", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/actagent", logger: { warn: vi.fn() } });
  ...
  it("eager-loads the browser control service when explicitly requested", async () => {
    vi.stubEnv("ACTAGENT_EAGER_BROWSER_CONTROL_SERVER", "1");
    const { api, registerService } = createApi();
    registerBrowserPlugin(api);
  ...
  for (const value of ["false", "", "disabled"]) {
    it(`keeps browser control service env value ${JSON.stringify(value)} lazy`, async () => {
      vi.stubEnv("ACTAGENT_EAGER_BROWSER_CONTROL_SERVER", value);
      const { api, registerService } = createApi();
      registerBrowserPlugin(api);
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "actagent" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "actagent", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});

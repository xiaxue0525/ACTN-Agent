// Phone Control plugin entrypoint registers its ACTAgent integration.
import {
  asDateTimestampMs,
...
function openArmStateStore(api: ACTAgentPluginApi) {
...
async function readArmState(api: ACTAgentPluginApi): Promise<ArmStateFile | null> {
  return (await openArmStateStore(api).lookup(ARM_STATE_KEY)) ?? null;
}

async function writeArmState(api: ACTAgentPluginApi, state: ArmStateFile | null): Promise<void> {
...
export default definePluginEntry({
  ...
  register(api: ACTAgentPluginApi) {
    ...
    const timerService: ACTAgentPluginService = {

...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: ACTAgentConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): ACTAgentConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("actagent/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/acp-runtime")>(
    "actagent/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("actagent/plugin-sdk/native-command-config-runtime")
  >("actagent/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/error-runtime")>(
    "actagent/plugin-sdk/error-runtime",
  );
  return {
    ...actual,

...
import { createUnionActionGate } from "actagent/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "actagent/plugin-sdk/channel-contract";
import type { DiscordActionConfig, ACTAgentConfig } from "actagent/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: ACTAgentConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: ACTAgentConfig) {

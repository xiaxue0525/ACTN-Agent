...
describe("discord config schema", () => {
  ...
  it("loads guild map and dm group settings", () => {
    ...
    expect(cfg.dm?.groupChannels).toEqual(["actagent-dm"]);
    ...
    expect(cfg.guilds?.["123"]?.slug).toBe("friends-of-actagent");
  ...
  it("rejects invalid Discord realtime voice modes", () => {
    for (const voice of [
      ...
      { mode: "agent-proxy", realtime: { wakeNames: [""] } },
      { mode: "agent-proxy", realtime: { wakeNames: ["actagent Bot Helper"] } },
  ...
  it.each([
    ...
    },
    {
      ...
      config: { activity: "Live", activityUrl: "https://twitch.tv/actagent" },
    },
    {

...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "actagent/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "actagent/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "actagent/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "actagent/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "actagent/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run actagent definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/actagent-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/actagent-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("actagent/plugin-sdk/reply-runtime", () => ({
...
vi.mock("actagent/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("actagent/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/actagent-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/actagent-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/actagent-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/actagent-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
...
describe("processDiscordMessage draft streaming", () => {
  ...
  it("keeps Discord progress lines below the configured label", async () => {
    ...
    expect(draftStream.update).toHaveBeenCalledWith("ACTAgenting...\n\n🧩 First\n🧩 Second\n🧩 Third");
  });

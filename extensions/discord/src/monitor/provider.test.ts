...
let runtimeEnvModule: typeof import("actagent/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): ACTAgentConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("actagent/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("actagent/plugin-sdk/runtime-env");
  ...
  it("passes ACTAgent event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });

...
vi.mock("actagent/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("actagent/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<ACTAgentConfig["channels"]>["discord"]>["execApprovals"],
): ACTAgentConfig {
  return {
    channels: {
      discord: {

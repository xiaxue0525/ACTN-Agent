...
import { buildPluginBindingApprovalCustomId } from "actagent/plugin-sdk/conversation-runtime";
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): ACTAgentConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/actagent-sessions-test.json");
  ...
  it("does not fall through to actagent when a plugin Discord interaction already replied", async () => {
    registerDiscordComponentEntries({
      entries: [createButtonEntry({ callbackData: "codex:approve" })],
      modals: [],
    });
  ...
  it("resolves plugin binding approvals without falling through to actagent", async () => {
    registerDiscordComponentEntries({
      entries: [
        createButtonEntry({

...
import { clearSessionStoreCacheForTest } from "actagent/plugin-sdk/session-store-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { ChannelType, type AutocompleteInteraction } from "../internal/discord.js";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("actagent/plugin-sdk/conversation-binding-runtime");
...
vi.mock("actagent/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("actagent/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("actagent/plugin-sdk/conversation-binding-runtime")>(
        "actagent/plugin-sdk/conversation-binding-runtime",
      ),
  );
});
...
vi.mock("actagent/plugin-sdk/agent-runtime", () => ({
  ...
  resolveDefaultModelForAgent: (params: { cfg: ACTAgentConfig }) => {
...
vi.mock("actagent/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `actagent-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("actagent/plugin-sdk/command-auth-native").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("actagent/plugin-sdk/command-auth-native").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("actagent/plugin-sdk/command-auth-native");

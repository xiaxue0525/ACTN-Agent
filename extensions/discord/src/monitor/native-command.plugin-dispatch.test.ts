...
import type { NativeCommandSpec } from "actagent/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "actagent/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "actagent/plugin-sdk/reply-dispatch-runtime";
import { getSessionEntry } from "actagent/plugin-sdk/session-store-runtime";
...
async function createNativeCommand(cfg: ACTAgentConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: ACTAgentConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: ACTAgentConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("actagent/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("actagent/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("actagent/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:actagent-codex-app-server:dm";

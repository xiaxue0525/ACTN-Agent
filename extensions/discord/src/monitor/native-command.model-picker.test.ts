...
import * as commandRegistryModule from "actagent/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "actagent/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "actagent/plugin-sdk/command-auth-native";
import type { ACTAgentConfig } from "actagent/plugin-sdk/config-contracts";
import * as globalsModule from "actagent/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "actagent/plugin-sdk/session-store-runtime";
import * as commandTextModule from "actagent/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "actagent", label: "ACTAgent Default", description: "Use ACTAgent." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();

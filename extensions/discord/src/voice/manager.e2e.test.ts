...
import type { RealtimeVoiceAgentControlResult } from "actagent/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active ACTAgent run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("actagent/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/routing")>(
    "actagent/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/agent-runtime")>(
    "actagent/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/actagent-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/actagent-tts.json"),
  };
});

vi.mock("actagent/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("actagent/plugin-sdk/realtime-bootstrap-context")
  >("actagent/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/runtime-env")>(
    "actagent/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/realtime-voice")>(
    "actagent/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same ACTAgent agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("actagent_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("actagent_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact ACTAgent answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    controlRealtimeVoiceAgentRunMock.mockResolvedValueOnce({
      ...
      active: true,
      aborted: true,
      message: "Cancelled the active ACTAgent run.",
      speak: true,
      show: true,
      suppress: false,
    });
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active ACTAgent run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active ACTAgent run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active ACTAgent run.", true);
  ...
  it("accepts ACTAgent as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "actagent wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "ACTAgent, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("ACTAgent");
    expectUserMessageIncludes("actagent wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    const manager = createManager(
      ...
      {
        agents: {
          list: [{ id: "agent-1", identity: { name: "actagent Bot Helper" } }],
        },
      },
    );
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "actagent Bot Helper, should not wake");
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "ACTAgent, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const actAgentTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    actAgentTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "Open actagent can you still hear me?");

    expect(agentCommandArgsAt(4).message).toContain("can you still hear me?");
    expect(agentCommandArgsAt(4).message).not.toContain("Open actagent");
  ...
  it("uses configured wake names before realtime agent-proxy consults", async () => {
    ...
    const manager = createManager({
      ...
      voice: {
        ...
        realtime: {
          ...
          wakeNames: ["actagent", "actagent Bot", "Okay Google"],
        },
      },
    });
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "actagent Bot, ship it");
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "actagent Bot Helper, ship it");
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "ACTAgent, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("actagent_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "ACTAgent already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call actagent_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("actagent_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "ACTAgent realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("ACTAgent realtime voice profile context");

...
import { sleepWithAbort } from "actagent/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "actagent/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: ACTAgentConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). ACTAgent adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
      ...
      status: createComputedAccountStatusAdapter<ResolvedDiscordAccount, DiscordProbe>({
        ...
        buildCapabilitiesDiagnostics: async ({ account, target }) => {
          ...
          const statusCfg: ACTAgentConfig = {
            channels: {
              discord: {
                accounts: {
                  [account.accountId]: {

...
describe("discord doctor", () => {
  ...
  it("removes unsupported Discord realtime wake names", () => {
      ...
      cfg: {
        channels: {
          discord: {
            voice: {
              realtime: {
                wakeNames: ["actagent", "actagent Bot Helper", "Open actagent"],
              },
            },
            accounts: {
              work: {
    ...
    expect(result.changes).toEqual([
      ...
      "Removed empty channels.discord.accounts.empty.voice.realtime.wakeNames; unset wake names use the default agent/ACTAgent fallback.",
  ...
  it("repairs safe numeric ids into strings and warns for unsafe lists", () => {
    ...
    const result = maybeRepairDiscordNumericIds(cfg, "actagent doctor --fix");
  ...
  it("formats repair guidance for unsafe numeric ids", () => {
    ...
    expect(warnings[1]).toContain("actagent doctor --fix");
  });

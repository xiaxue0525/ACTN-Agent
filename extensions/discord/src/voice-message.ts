...
import { unlinkIfExists } from "actagent/plugin-sdk/media-runtime";
import { parseStrictFiniteNumber } from "actagent/plugin-sdk/number-runtime";
import type { RetryRunner } from "actagent/plugin-sdk/retry-runtime";
import { writeExternalFileWithinRoot } from "actagent/plugin-sdk/security-runtime";
import { fetchWithSsrFGuard, type SsrFPolicy } from "actagent/plugin-sdk/ssrf-runtime";
...
async function generateWaveformFromPcm(filePath: string): Promise<string> {
  const tempDir = resolvePreferredACTAgentTmpDir();

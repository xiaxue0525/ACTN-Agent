...
vi.mock("actagent/plugin-sdk/provider-transport-runtime", async (importOriginal) => ({
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "actagent.modelProviderRequestTransport",
);
...
describe("google transport stream", () => {
  ...
  afterAll(() => {
    vi.doUnmock("actagent/plugin-sdk/provider-transport-runtime");
  ...
  it("rejects non-integer Gemini 3 first-response retry env values", () => {
    const envName = "ACTAGENT_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS";
  ...
  it("retries Gemini 3 requests with lean thinking when the first attempt has no first response", async () => {
    vi.stubEnv("ACTAGENT_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS", "10");
    guardedFetchMock
  ...
  it("detects supported Vertex ADC sources synchronously", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-adc-detect-"));
  ...
  it("resolves non-file Vertex ADC through google-auth-library without OAuth refresh fetch", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-authlib-"));
  ...
  it("does not cache google-auth ADC tokens when fallback expiry would exceed Date range", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-authlib-expiry-"));
    vi.useFakeTimers();
  ...
  it("uses google-auth-library bearer auth for Google Vertex credential marker requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-authlib-stream-"));
    vi.stubEnv("GOOGLE_APPLICATION_CREDENTIALS", "");
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
  ...
  it("refreshes authorized_user ADC before Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-adc-"));
  ...
  it("does not reuse authorized_user ADC tokens with unsafe expiry lifetimes", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-unsafe-adc-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("refreshes authorized_user ADC from the Windows APPDATA fallback for Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-appdata-adc-"));
  ...
  it("treats the Google transport alias as the same route for signature replay", () => {
    const model = {
      ...
      api: "actagent-google-generative-ai-transport",
    } as Model<"actagent-google-generative-ai-transport">;
  ...
  it("adds skip-validator fallback to unsigned sibling Gemini 3 tool calls", () => {
      ...
      messages: [
        {
          ...
          content: [
            ...
            },
            {
              ...
              arguments: { text: "actagent", mode: "reverse" },
            },
          ],
        },
      ],
    ...
    expect(parts[2]).toMatchObject({
      thoughtSignature: "skip_thought_signature_validator",
      functionCall: { name: "string_transform", args: { text: "actagent", mode: "reverse" } },
    });
  });

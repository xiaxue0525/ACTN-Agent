...
import { transcodeAudioBufferToOpus } from "actagent/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "actagent/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: ACTAgentConfig): string | undefined {
...
function isACTAgentGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isACTAgentGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {

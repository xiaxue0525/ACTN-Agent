...
import { CUSTOM_LOCAL_AUTH_MARKER } from "actagent/plugin-sdk/provider-auth";
...
import { resolveAgentModelPrimaryValue } from "actagent/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "actagent/plugin-sdk/provider-setup";
import type { WizardPrompter } from "actagent/plugin-sdk/setup";
...
vi.mock("actagent/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("actagent/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("actagent/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("actagent/plugin-sdk/provider-auth");
  vi.doUnmock("actagent/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): ACTAgentConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.ACTAGENT_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("ACTAGENT_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([

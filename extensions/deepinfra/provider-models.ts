...
import { fetchWithTimeout } from "actagent/plugin-sdk/provider-http";
...
import { asPositiveSafeInteger } from "actagent/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=actagent&filter=with_meta`;
...
// Static fallback. Chat rows live in actagent.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=actagent&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `ACTAGENT_LIVE_*`
// indirection too).

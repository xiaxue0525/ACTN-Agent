...
describe("transcribeSenseAudioAudio", () => {
  ...
  it("can transcribe generated speech in live mode", async () => {
    if (process.env.ACTAGENT_LIVE_TEST !== "1" || !process.env.SENSEAUDIO_API_KEY) {
      return;
    }
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "actagent-senseaudio-live-"));
    try {
      const aiffPath = path.join(tempDir, "speech.aiff");
      const mp3Path = path.join(tempDir, "speech.mp3");
      const sayResult = spawnSync("say", ["-o", aiffPath, "open actagent live transcription test"], {
        encoding: "utf8",
      });

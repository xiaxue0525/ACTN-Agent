...
describe("mattermost monitor slash", () => {
  ...
  it("registers deduped default and native skill commands across teams", async () => {
    vi.stubEnv("ACTAGENT_GATEWAY_PORT", "18888");
    ...
    resolveCallbackUrl.mockReturnValue("https://actagent.test/slash");
    ...
    await registerMattermostMonitorSlashCommands({
      client,
      cfg: { gateway: { port: 19199 } } as never,
    ...
    expect(firstRegistration).toEqual({
      ...
      callbackUrl: "https://actagent.test/slash",
      commands: [
        { trigger: "ping", description: "ping" },
        {
          trigger: "oc_skill",
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "mattermost: slash commands registered (2 commands across 2 teams, callback=https://actagent.test/slash)",
    );
  });

  it("falls back to the configured gateway port when the env port is out of range", async () => {
    vi.stubEnv("ACTAGENT_GATEWAY_PORT", "65536");
    ...
    expect(resolveCallbackUrl).toHaveBeenCalledWith(
      expect.objectContaining({ gatewayPort: 19199 }),
    );
  });
  ...
  it("warns on loopback callback urls and reports partial team failures", async () => {
    ...
    parseTcpPort.mockReturnValue(null);
    fetchMattermostUserTeams.mockResolvedValue([{ id: "team-1" }, { id: "team-2" }]);
    resolveCallbackUrl.mockReturnValue("http://127.0.0.1:19199/slash");
    registerSlashCommands
      .mockResolvedValueOnce([{ token: "token-1", trigger: "ping" }])
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "mattermost: slash commands callbackUrl resolved to http://127.0.0.1:19199/slash (loopback) while baseUrl is https://chat.example.com. This MAY be unreachable depending on your deployment. If native slash commands don't work, set channels.mattermost.commands.callbackUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).",
    );
    expect(runtime.error).toHaveBeenCalledWith(

...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__actagent_channel_id";
...
type InteractionCallbackConfig = Pick<ACTAgentConfig, "gateway" | "channels"> & {
  interactions?: {
...
export function computeInteractionCallbackUrl(
  ...
  const port = typeof cfg?.gateway?.port === "number" ? cfg.gateway.port : 19199;
  let host =
    cfg?.gateway?.customBindHost && !isWildcardBindHost(cfg.gateway.customBindHost)
      ? cfg.gateway.customBindHost.trim()
      : "localhost";

  // Bracket IPv6 literals so the URL is valid: http://[::1]:19199/...
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "actagent-mattermost-interactions").update(botToken).digest("hex");
}

...
describe("callback URL registry", () => {
  it("stores and retrieves callback URLs", () => {
    setInteractionCallbackUrl("acct1", "http://localhost:19199/mattermost/interactions/acct1");
    expect(getInteractionCallbackUrl("acct1")).toBe(
      "http://localhost:19199/mattermost/interactions/acct1",
    );
  });
...
describe("resolveInteractionCallbackUrl", () => {
  ...
  it("uses interactions.callbackBaseUrl when configured", () => {
    const url = resolveInteractionCallbackUrl("default", {
      channels: {
        mattermost: {
          interactions: {
            callbackBaseUrl: "https://gateway.example.com/actagent",
          },
        },
      },
    });
    expect(url).toBe("https://gateway.example.com/actagent/mattermost/interactions/default");
  });
  ...
  it("uses default port 19199 when no config provided", () => {
    const url = resolveInteractionCallbackUrl("myaccount");
    expect(url).toBe("http://localhost:19199/mattermost/interactions/myaccount");
  });
});
...
describe("buildButtonAttachments", () => {
  ...
  it("returns an array with one attachment containing all buttons", () => {
    const result = buildButtonAttachments({
      callbackUrl: "http://localhost:19199/mattermost/interactions/default",
      buttons: [
  ...
  it("sets type to 'button' on every action", () => {
    const result = buildButtonAttachments({
      callbackUrl: "http://localhost:19199/cb",
      buttons: [{ id: "a", name: "A" }],
    });
  ...
  it("passes callback URL to each button integration", () => {
    const url = "http://localhost:19199/mattermost/interactions/default";
    const result = buildButtonAttachments({
      callbackUrl: url,
      buttons: [
        { id: "a", name: "A" },
...
describe("createMattermostInteractionHandler", () => {
  ...
  function createActionContext(actionId = "approve", channelId = "chan-1") {
    const context = { action_id: actionId, __actagent_channel_id: channelId };

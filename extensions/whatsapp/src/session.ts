...
import { VERSION } from "actagent/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "actagent/plugin-sdk/fetch-runtime";
import { danger, success } from "actagent/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "actagent/plugin-sdk/runtime-env";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("actagent channels login")}`,
              ),
            );
          }
        }

...
import { resolveAccountEntry } from "actagent/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "actagent/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "actagent/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "actagent/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "actagent/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "actagent/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "actagent/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "actagent/plugin-sdk/reply-history";
import { resolveAgentRoute } from "actagent/plugin-sdk/routing";
...
import { getChildLogger } from "actagent/plugin-sdk/runtime-env";
import {
  defaultRuntime,
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("actagent channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("actagent channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              ...
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("actagent channels login --channel whatsapp")}\` to relink.`,
              );
              ...
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("actagent channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("actagent channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("actagent channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("actagent channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {

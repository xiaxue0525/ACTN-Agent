...
vi.mock("actagent/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/channel-pairing")>(
...
vi.mock("actagent/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/media-store")>(
    "actagent/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("actagent/plugin-sdk/plugin-state-test-runtime")>(
      "actagent/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/actagent-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `actagent-inbound-media-${crypto.randomUUID()}`);

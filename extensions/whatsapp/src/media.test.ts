...
import { captureEnv } from "actagent/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "actagent/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "actagent/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ACTAGENT_STATE_DIR.
    // Also keep it outside the ACTAgent temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ACTAGENT_STATE_DIR"]);
    process.env.ACTAGENT_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "actagent-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredACTAgentTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/actagent` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredACTAgentTmpDir();
  ...
  it("allows default ACTAgent state workspace and sandbox roots", async () => {
  ...
  it("rejects default ACTAgent state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expectLocalMediaAccessCode(
      loadWebMedia(path.join(stateDir, "workspace-actagentdy", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
      "path-not-allowed",
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-actagentdy");

...
import { formatEnvelopeTimestamp } from "actagent/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("actagent:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("actagent:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[actagent]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/actagent-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("actagent:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("actagent:lastSocket")];
  if (typeof getter === "function") {

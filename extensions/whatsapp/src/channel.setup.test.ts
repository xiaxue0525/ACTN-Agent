...
import { createQueuedWizardPrompter } from "actagent/plugin-sdk/plugin-test-runtime";
...
vi.mock("actagent/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: ACTAgentConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<ACTAgentConfig["channels"]>] as object),
          enabled,
        },
      },
    }),

...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "actagent-test-web-logout-"));
    previousOAuthDir = process.env.ACTAGENT_OAUTH_DIR;
    process.env.ACTAGENT_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.ACTAGENT_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the ACTAgent auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the ACTAgent auth root", async () => {

...
vi.mock("actagent/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/logging-core")>(
    "actagent/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("ACTAgent WhatsApp listener close");

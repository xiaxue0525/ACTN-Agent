...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "actagent/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("actagent/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("actagent/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: ACTAgentConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("actagent/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `actagent-outbound-${crypto.randomUUID()}.log`);

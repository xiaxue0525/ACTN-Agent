...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ACTAGENT_STATE_DIR", "ACTAGENT_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-wa-profile-"));
  ...
  it("resolves the default web auth dir from ACTAGENT_STATE_DIR at call time", async () => {

...
import { generateSecureUuid } from "actagent/plugin-sdk/core";
import { redactIdentifier } from "actagent/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "actagent/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "actagent/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "actagent/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "actagent/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: ACTAgentConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`actagent channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: ACTAgentConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();

...
import type { ChannelSetupWizard } from "actagent/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "actagent/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<ACTAgentConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: ACTAgentConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: ACTAgentConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: ACTAgentConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: ACTAgentConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("actagent channels login"),
      }),
      "WhatsApp",
    );
  }

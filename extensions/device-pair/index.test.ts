...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredACTAgentTmpDir: vi.fn(() => path.join(os.tmpdir(), "actagent-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredACTAgentTmpDir: pluginApiMocks.resolvePreferredACTAgentTmpDir,
...
function createApi(params?: {
  ...
  runtime?: ACTAgentPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as ACTAgentPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: ACTAgentPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredACTAgentTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredACTAgentTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the ACTAgent iOS app:");
    ...
    expect(text).not.toContain("![ACTAgent pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the ACTAgent iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("normalizes secure bare publicUrl host ports before issuing setup codes", async () => {
    const command = registerPairCommand({
      ...
      pluginConfig: {
        publicUrl: "gateway.example.test:19199/setup",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(text).toContain("Gateway: wss://gateway.example.test:19199");
  });

  it("allows loopback cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://127.0.0.1:19199",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(text).toContain("Gateway: ws://127.0.0.1:19199");
  });

  it("allows private LAN cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://192.168.1.20:19199",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://192.168.1.20:19199");
  });

  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://actagent.local:19199",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://actagent.local:19199");
  });

  it("rejects public cleartext setup urls before issuing setup codes", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://gateway.example.test:19199",
      },
    });
    const result = await command.handler(
      createCommandContext({
  ...
  it.each([
    ...
    "ws://user:pass@gateway.example.test:19199",

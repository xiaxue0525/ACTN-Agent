...
import type { CreateSandboxBackendParams } from "actagent/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from ACTAgent config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "actagent-session-1234",
        backendId: "openshell",
        runtimeLabel: "actagent-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "actagent-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "actagent-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "actagent-session-5678",
        backendId: "openshell",
        runtimeLabel: "actagent-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "actagent",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "actagent-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "actagent-session-5678"],
    });
  });
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("actagent-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("actagent-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("actagent-openshell-fs-");
    const outsideDir = await makeTempDir("actagent-openshell-outside-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("actagent-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("actagent-openshell-agent-");

...
import { createSandboxTestContext } from "actagent/plugin-sdk/test-fixtures";
import {
...
const ACTAGENT_OPENSHELL_E2E = process.env.ACTAGENT_E2E_OPENSHELL === "1";
const ACTAGENT_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ACTAGENT_OPENSHELL_COMMAND =
  process.env.ACTAGENT_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "actagent-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ACTAGENT_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ACTAGENT_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ACTAGENT_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(ACTAGENT_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-openshell-e2e-"));
      ...
      const gatewayName = `actagent-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `actagent-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/actagent-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ACTAGENT_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("actagent-openshell-e2e");

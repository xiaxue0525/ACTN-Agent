...
import type { ProviderAuthContext, ProviderAuthMethod } from "actagent/plugin-sdk/plugin-entry";
import {
...
import { waitForLocalOAuthCallback } from "actagent/plugin-sdk/provider-auth-runtime";
...
export function buildXaiOAuthAuthorizeUrl(params: {
  ...
  url.searchParams.set("referrer", "actagent");
  return url.toString();
}
  ...
  try {
    ...
    return buildOauthProviderAuthResult({
      ...
      notes: [
        ...
        "xAI may label the consent app as Grok Build because ACTAgent uses xAI's shared OAuth client.",
      ],
    });
  } catch (err) {

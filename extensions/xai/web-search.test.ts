...
import { createTestWizardPrompter } from "actagent/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "actagent/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "actagent/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync, withFetchPreconnect } from "actagent/plugin-sdk/test-env";
...
vi.mock("actagent/plugin-sdk/provider-auth", async (importOriginal) => {
  const original = await importOriginal<typeof import("actagent/plugin-sdk/provider-auth")>();
  return {
    ...original,
...
vi.mock("actagent/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("actagent/plugin-sdk/provider-auth-runtime")>();
  return {
    ...original,
...
vi.mock("actagent/plugin-sdk/provider-web-search", async (importOriginal) => {
  const original = await importOriginal<typeof import("actagent/plugin-sdk/provider-web-search")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      const result = await maybeTool.execute({ query: "ACTAgent" });
  ...
  it("uses xAI OAuth auth before API-key fallback for web search", async () => {
    ...
    const tool = provider.createTool({
      config: {
        agents: {
          list: [{ id: "main", default: true, agentDir: "/tmp/actagent-xai-main-agent" }],
        },
        plugins: {
          entries: {
            xai: {
    ...
    await tool.execute({ query: "ACTAgent Grok OAuth web search" });

    expect(providerAuthRuntimeMocks.resolveApiKeyForProvider).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: "xai",
        agentDir: "/tmp/actagent-xai-main-agent",
      }),
    );
  ...
  it("uses the active agentDir for xAI OAuth web search auth", async () => {
    ...
    const provider = createXaiWebSearchProvider();
    const tool = provider.createTool({
      agentDir: "/tmp/actagent-xai-active-agent",
      config: {
        agents: {
          list: [
            { id: "main", default: true, agentDir: "/tmp/actagent-xai-main-agent" },
            { id: "side", agentDir: "/tmp/actagent-xai-active-agent" },
          ],
        },
      },
    });
    ...
    await tool.execute({ query: "ACTAgent Grok active agent OAuth web search" });
  ...
  it("refreshes xAI OAuth auth and retries web search after a 401", async () => {
    ...
    const result = await tool.execute({ query: "ACTAgent Grok OAuth refresh test" });
  ...
  it("falls back to xAI API-key auth when OAuth refresh cannot recover", async () => {
    ...
    const result = await tool.execute({ query: "ACTAgent Grok API fallback test" });
  ...
  it("falls back to an xAI API-key auth profile when stale OAuth remains first", async () => {
    ...
    const result = await tool.execute({ query: "ACTAgent Grok profile fallback test" });
  ...
  it("falls back to env auth after a stale xAI API-key auth profile returns unauthorized", async () => {
    ...
    const result = await tool.execute({ query: "ACTAgent Grok API-key fallback test" });
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool.execute({ query: "ACTAgent Grok proxy test" });
  ...
  it("reports malformed xAI web search JSON as a provider error", async () => {
    ...
    await expect(tool.execute({ query: "ACTAgent" })).rejects.toThrow(
...
describe("xai provider models", () => {
  ...
  it("publishes the remaining Grok 3 family in the ACTAgent catalog", () => {

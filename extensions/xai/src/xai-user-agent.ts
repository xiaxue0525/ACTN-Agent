// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatACTAgentUserAgent`.

import { ACTAGENT_VERSION as PACKAGE_VERSION } from "actagent/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "actagent";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.ACTAGENT_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.ACTAGENT_SERVICE_VERSION) ??
...
// an empty record so the actagent identity is not forwarded to the proxy.

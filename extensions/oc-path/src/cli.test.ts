/**
 * Smoke tests for the `actagent path` CLI handlers.
 *
...
describe("actagent path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "actagent.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://actagent.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://actagent.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );

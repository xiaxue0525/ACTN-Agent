...
import type { PreparedInboundReply } from "actagent/plugin-sdk/channel-inbound";
...
function buildConfig(overrides?: Partial<ACTAgentBotConfig>): ACTAgentBotConfig {
  return {
    channels: {
      feishu: {
        enabled: true,
  ...
  it("issues a pairing challenge in the comment thread when dmPolicy=pairing", async () => {
    ...
    expect(pairingReply).toEqual({
      ...
      content: [
        ...
        "```",
        "actagent pairing approve feishu TESTCODE",
        "```",
      ].join("\n"),

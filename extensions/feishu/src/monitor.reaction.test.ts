...
import { hasControlCommand, isControlCommandMessage } from "actagent/plugin-sdk/command-detection";
import { createNonExitingRuntimeEnv } from "actagent/plugin-sdk/plugin-test-runtime";
...
const cfg = {} as ACTAgentBotConfig;
...
async function resolveNonBotReaction(params?: { cfg?: ACTAgentBotConfig; uuid?: () => string }) {
  return await resolveReactionSyntheticEvent({
    cfg: params?.cfg ?? cfg,
    accountId: "default",
...
function buildDebounceConfig(): ACTAgentBotConfig {
  return {
    messages: {
      inbound: {
        debounceMs: 0,
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    vi.spyOn(dedup, "recordProcessedFeishuMessage").mockResolvedValue(true);
    vi.spyOn(dedup, "hasProcessedFeishuMessage").mockResolvedValue(false);
    const onMessage = await setupDebounceMonitor({ botName: "ACTAgent Bot" });
    ...
    const firstParams = mockCallAt(handleFeishuMessageMock, 0, "Feishu message dispatch")[0] as
      | { botName?: string }
      | undefined;
    expect(firstParams?.botName).toBe("ACTAgent Bot");
  });

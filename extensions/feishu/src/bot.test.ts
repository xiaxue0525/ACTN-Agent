...
import type * as ConversationRuntime from "actagent/plugin-sdk/conversation-runtime";
...
import { resolveGroupSessionKey } from "actagent/plugin-sdk/session-store-runtime";
...
async function dispatchMessage(params: { cfg: ACTAgentBotConfig; event: FeishuMessageEvent }) {
...
describe("createFeishuMessageReceiveHandler media dedupe", () => {
  it("keeps same-id media variants distinct at receive time", async () => {
    ...
    const handler = createFeishuMessageReceiveHandler({
      cfg: { channels: { feishu: { dmPolicy: "open" } } } as ACTAgentBotConfig,

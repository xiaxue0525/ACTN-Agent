...
import { formatAllowFromLowercase } from "actagent/plugin-sdk/allow-from";
import {
...
import { createPairingPrefixStripper } from "actagent/plugin-sdk/channel-pairing";
import {
...
import { createComputedAccountStatusAdapter } from "actagent/plugin-sdk/status-helpers";
...
function areAnyFeishuReactionActionsEnabled(cfg: ACTAgentBotConfig): boolean {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      config: {
        ...
        deleteAccount: ({ cfg, accountId }) => {
          ...
          if (isDefault) {
            ...
            const next = { ...cfg } as ACTAgentBotConfig;
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("actagent/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("actagent/plugin-sdk/config-mutation");

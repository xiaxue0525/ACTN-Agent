...
const emptyConfig: ACTAgentBotConfig = {};
...
vi.mock("actagent/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("actagent/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
function expectPathIsolatedToTmpRoot(pathValue: string, key: string): void {
  ...
  const tmpRoot = realpathSync(resolvePreferredACTAgentTmpDir());
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("actagent/plugin-sdk/media-runtime");
    vi.resetModules();
  });
  ...
  it("passes mediaLocalRoots as localRoots to loadWebMedia for local paths (#27884)", async () => {
    ...
    const roots = ["/allowed/workspace", "/tmp/actagent"];
    await sendMediaFeishu({
      cfg: emptyConfig,
      to: "user:ou_target",
...
describe("downloadMessageResourceFeishu", () => {
  ...
  it("saves message resource streams directly to the media store", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-feishu-media-"));
    try {
      ...
      expect(result.saved.path).toContain(`${path.sep}.actagent${path.sep}media${path.sep}inbound`);

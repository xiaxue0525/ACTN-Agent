...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("actagent/plugin-sdk/memory-core-host-runtime-cli"),
    import("actagent/plugin-sdk/memory-core-host-runtime-core"),
    import("actagent/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("actagent/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("actagent/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("actagent/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("actagent/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("prints index identity mismatch reasons", async () => {
    ...
    expectLogged(log, "Fix: Run: actagent memory status --index --agent main");
    expect(close).toHaveBeenCalled();
  });
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const log = spyRuntimeLogs(defaultRuntime);
      await runMemoryCli(["status"]);
      expectLogged(log, "Fix: actagent memory status --fix --agent main");
      ...
      await runMemoryCli(["status", "--fix"]);
      expectNotLogged(log, "Fix: actagent memory status --fix --agent main");
    });
  });
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("actagent:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## ACTAgent / runtime / workflow preferences and corrections",
          ...
          "- jpactagenthq updated and running.",
          "",
      ...
      expect(rendered).not.toContain("jpactagenthq updated and running");
      expect(close).toHaveBeenCalled();
    });
  });
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- actagent:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- actagent:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      await writeDailyMemoryNote(workspaceDir, "2026-04-01", [
        ...
        "line 8",
        "line 9",
        "Gateway host uses local mode and binds loopback port 19199",
        ...
        results: [
          {
            ...
            endLine: 14,
            score: 0.91,
            snippet: "Gateway host uses local mode and binds loopback port 19199",
            source: "memory",
          },
        ],
      });
      ...
      expect(memoryText).toContain("actagent-memory-promotion:");

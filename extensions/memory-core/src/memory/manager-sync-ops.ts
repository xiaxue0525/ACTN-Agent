...
import { classifyMemoryMultimodalPath } from "actagent/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
const TEST_MEMORY_WATCH_FACTORY_KEY = Symbol.for("actagent.test.memoryWatchFactory");
const TEST_MEMORY_NATIVE_WATCH_FACTORY_KEY = Symbol.for("actagent.test.memoryNativeWatchFactory");
...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: ACTAgentConfig;
  ...
  private warnIfMemoryWatchPressure(count: number, unit: MemoryWatchPressureUnit): void {
    warnIfMemoryWatchPressureHigh(
      ...
      count,
      unit,
      "Large memory folders or extraPaths can make ACTAgent run out of file watchers or open files.",
    ...
    try {
      if (needsFullReindex) {
        if (
          ...
          process.env.ACTAGENT_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {

...
import type { EmbeddingProviderAdapter } from "actagent/plugin-sdk/embedding-providers";
import type { MemoryEmbeddingProviderAdapter } from "actagent/plugin-sdk/memory-core-host-engine-embeddings";
...
const mockEmbeddingRegistry = vi.hoisted(() => ({
  ...
  genericLookupConfigs: [] as Array<ACTAgentConfig | undefined>,
}));

vi.mock("actagent/plugin-sdk/embedding-providers", () => ({
  getEmbeddingProvider: (id: string, config?: ACTAgentConfig) => {
...
vi.mock("actagent/plugin-sdk/memory-core-host-engine-embeddings", () => ({

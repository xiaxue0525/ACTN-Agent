...
import type {
  MemorySearchConfig,
  ACTAgentConfig,
} from "actagent/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("actagent.test.memoryWatchFactory");
  const nativeKey = Symbol.for("actagent.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("actagent.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("actagent.test.memoryNativeWatchFactory");

vi.mock("actagent/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("actagent/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "actagent/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): ACTAgentConfig {
    const defaults: NonNullable<NonNullable<ACTAgentConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: ACTAgentConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. ACTAgent watches directories only so

...
import { withMockedWindowsPlatform } from "actagent/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("actagent.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("actagent.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("actagent.qmdEmbedQueueTail");
...
vi.mock("actagent/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("actagent/plugin-sdk/memory-core-host-engine-foundation")
  >("actagent/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("actagent/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/file-lock")>(
    "actagent/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { ACTAgentConfig } from "actagent/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,

...
} from "actagent/plugin-sdk/memory-core-host-embedding-registry";
import { getProviderEnvVars } from "actagent/plugin-sdk/provider-env-vars";
...
function formatLocalSetupError(err: unknown): string {
  ...
  return [
    ...
    missing
      ? `2) Install ${NODE_LLAMA_CPP_RUNTIME_PACKAGE} next to the ACTAgent package or source checkout`
      : null,
...
const localAdapter: MemoryEmbeddingProviderAdapter = {
  ...
  create: async (options) => {
    const { createLocalEmbeddingProvider } =
      await import("actagent/plugin-sdk/memory-core-host-engine-embeddings");

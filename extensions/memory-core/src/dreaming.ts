...
import { peekSystemEventEntries } from "actagent/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: ACTAgentPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: ACTAgentPluginApi): void {
  ...
  const resolveCurrentConfig = (): ACTAgentConfig =>
    (api.runtime.config?.current?.() ?? api.config) as ACTAgentConfig;

...
function resolveStoredDreaming(config: ACTAgentConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: ACTAgentConfig = {}) {
  const registered: { command?: ACTAgentPluginCommandDefinition } = {};
  let runtimeConfig: ACTAgentConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: ACTAgentConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: ACTAgentConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: ACTAgentPluginCommandDefinition) => {

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent memory status", "Show index and provider status."],
          [
            "actagent memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["actagent memory status --deep", "Probe embedding provider readiness."],
          ["actagent memory index --force", "Force a full reindex."],
          ['actagent memory search "meeting notes"', "Quick search using positional query."],
          [
            'actagent memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `actagent memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "actagent memory rem-backfill --path ./memory",
          ...
          ["actagent memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.actagent.ai/cli/memory")}\n`,
    );

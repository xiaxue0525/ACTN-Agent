...
import type { MemoryEmbeddingProbeResult } from "actagent/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "actagent/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "actagent/plugin-sdk/routing";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: actagent memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: ACTAgentConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: ACTAgentConfig): string {
...
function resolveAgent(cfg: ACTAgentConfig, agent?: string) {
...
function resolveAgentIds(cfg: ACTAgentConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: actagent memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredACTAgentTmpDir(), "actagent-rem-backfill-"),
      );
      try {

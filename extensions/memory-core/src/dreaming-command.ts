...
import { resolveMemoryDreamingConfig } from "actagent/plugin-sdk/memory-core-host-status";
import type { ACTAgentPluginApi, PluginCommandContext } from "actagent/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "actagent/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: ACTAgentConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: ACTAgentConfig, enabled: boolean): ACTAgentConfig {
...
function formatStatus(cfg: ACTAgentConfig): string {
...
export async function handleDreamingCommand(api: ACTAgentPluginApi, ctx: PluginCommandContext) {
...
export function registerDreamingCommand(api: ACTAgentPluginApi): void {
  api.registerCommand({
    name: "dreaming",

...
} from "actagent/plugin-sdk/system-event-runtime";
...
function getGatewayStartHandler(
  ...
  event: { port: number },
  ctx: { config?: ACTAgentConfig; workspaceDir?: string; getCron?: () => unknown },
) => Promise<unknown> {
...
async function triggerGatewayStart(
  ...
  await getGatewayStartHandler(onMock)({ port: 19199 }, ctx);
}
...
describe("short-term dreaming cron reconciliation", () => {
  ...
  it("migrates legacy light/rem dreaming cron jobs during reconciliation", async () => {
    ...
    const legacyLightJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__actagent_memory_core_light_sleep__" },
      createdAtMs: 8,
    };
    const legacyRemJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__actagent_memory_core_rem_sleep__" },
      createdAtMs: 9,
    };
...
describe("short-term dreaming trigger", () => {
  ...
  it("applies promotions when the managed dreaming token is embedded in a reminder body", async () => {
    ...
    const result = await runShortTermDreamingPromotionIfTriggered({
      cleanedBody: [
        "System: rotate logs",
        "System: __actagent_memory_core_short_term_promotion_dream__",
        "",
        "A scheduled reminder has been triggered. The reminder content is:",
        "",
        "rotate logs",
        "__actagent_memory_core_short_term_promotion_dream__",
        "",
  ...
  it("applies promotions when the managed dreaming token is wrapped by the cron label", async () => {
    ...
    const result = await runShortTermDreamingPromotionIfTriggered({
      cleanedBody: [
        "[cron:e795558c-a273-4124-ba88-d4916688d977 Memory Dreaming Promotion] __actagent_memory_core_short_term_promotion_dream__",

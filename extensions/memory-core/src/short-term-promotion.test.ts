...
vi.mock("actagent/plugin-sdk/memory-host-events", () => ({
  appendMemoryHostEvent: vi.fn(async () => {}),
}));

vi.mock("actagent/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("actagent/plugin-sdk/security-runtime")>(
    "actagent/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
import { privateFileStore } from "actagent/plugin-sdk/security-runtime";
import {
...
describe("short-term promotion", () => {
  ...
  it("reconciles existing promotion markers instead of appending duplicates", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      await writeDailyMemoryNote(workspaceDir, "2026-04-01", [
        "line 1",
        "line 2",
        "The gateway should stay loopback-only on port 19199.",
      ]);
      await recordShortTermRecalls({
        workspaceDir,
        ...
        results: [
          {
            ...
            endLine: 3,
            score: 0.95,
            snippet: "The gateway should stay loopback-only on port 19199.",
            source: "memory",
          },
        ],
      });
      ...
      expect(memoryText.match(/actagent-memory-promotion:/g)?.length).toBe(1);
      expect(
        memoryText.match(/The gateway should stay loopback-only on port 19199\./g)?.length,
      ).toBe(1);
    });
  });
  ...
  it("does not re-append promoted candidates whose marker key path contains spaces", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      await writeDailyMemoryNoteInSubdir(workspaceDir, "project alpha", "2026-04-01", [
        "alpha",
        "The project alpha gateway should stay loopback-only on port 19199.",
      ]);
      await recordShortTermRecalls({
        workspaceDir,
        ...
        results: [
          {
            ...
            endLine: 2,
            score: 0.95,
            snippet: "The project alpha gateway should stay loopback-only on port 19199.",
            source: "memory",
          },
        ],
      });
      ...
      expect(memoryText).toContain(
        "<!-- actagent-memory-promotion:memory:memory/project alpha/2026-04-01.md:2:2 -->",
      );
      expect(memoryText.match(/actagent-memory-promotion:/g)?.length).toBe(1);
      expect(
        memoryText.match(/The project alpha gateway should stay loopback-only on port 19199\./g)
          ?.length,
      ).toBe(1);
    });
  });
  ...
  describe("lineRangeOverlapsDreamingFence", () => {
    ...
    it("returns false when the range sits entirely outside any dreaming fence", () => {
      const lines = [
        "# Daily note",
        "Real durable content.",
        "<!-- actagent:dreaming:rem:start -->",
        "staged dream content",
        "<!-- actagent:dreaming:rem:end -->",
        "More real content.",
      ];
  ...
  it("applies promotion candidates to MEMORY.md and marks them promoted", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      await writeDailyMemoryNote(workspaceDir, "2026-04-01", [
        ...
        "theta",
        "iota",
        "Gateway binds loopback and port 19199",
        ...
        results: [
          {
            ...
            endLine: 12,
            score: 0.92,
            snippet: "Gateway binds loopback and port 19199",
            source: "memory",
          },
        ],
      });
  ...
  it("does not double-prefix promoted snippets that are already markdown bullets", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(applied.applied).toBe(1);
      const memoryText = await fs.readFile(path.join(workspaceDir, "MEMORY.md"), "utf-8");
      expect(memoryText).toContain("- Gateway binds loopback and port 19199");
      expect(memoryText).not.toContain("- - Gateway binds loopback and port 19199");
    });
  });
  ...
  it("keeps promoted MEMORY.md entries compact while preserving provenance", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toMatch(/<!-- actagent-memory-promotion:[^\n]+ -->/);
    });
  });
    ...
    it("drops the oldest promoted section before write when memoryFileMaxChars would be exceeded", async () => {
      await withTempWorkspace(async (workspaceDir) => {
        ...
        const seeded = [
          ...
          "<!-- actagent-memory-promotion:legacy-old -->",
          `- ${filler}`,
          "",
          "## Promoted From Short-Term Memory (2026-04-20)",
          "<!-- actagent-memory-promotion:legacy-newer -->",
          `- ${filler}`,
          "",
        ].join("\n");

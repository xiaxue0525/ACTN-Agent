...
describe("duckduckgo web search provider", () => {
  ...
  it("rejects fractional and out-of-range counts before searching", async () => {
    ...
    await expect(tool.execute({ query: "actagent docs", count: 4.5 })).rejects.toThrow(
      "count must be an integer from 1 to 10.",
    );
    await expect(tool.execute({ query: "actagent docs", count: 11 })).rejects.toThrow(
      "count must be an integer from 1 to 10.",
    );
  ...
  it("decodes direct and redirect urls plus common html entities", () => {
    expect(
      ddgClientTesting.decodeDuckDuckGoUrl(
        "https://duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com%2Fsearch%3Fq%3dactagent",
      ),
    ).toBe("https://example.com/search?q=actagent");

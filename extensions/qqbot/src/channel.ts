...
import { getExecApprovalReplyMetadata } from "actagent/plugin-sdk/approval-runtime";
import {
...
import { sanitizeAssistantVisibleText } from "actagent/plugin-sdk/text-chunking";
...
import { toGatewayAccount, writeACTAgentConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped actagent.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // actagent.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeACTAgentConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe actagent.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeACTAgentConfigThroughRuntime(getQQBotRuntime(), nextCfg as ACTAgentConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as ACTAgentConfig, accountId);

...
let actagentVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; ACTAgent/${actagentVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; actagentVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.actagentVersion) {
    actagentVersion = options.actagentVersion;
  }
}

/** Update the ACTAgent framework version in the User-Agent (called after runtime injection). */
export function setACTAgentVersion(version: string): void {
  if (version) {

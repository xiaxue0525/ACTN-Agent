 ...
 * silk-wasm availability checks — all without importing `actagent/plugin-sdk`.
 ...
 * `ACTAGENT_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective ACTAgent home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of ACTAgent treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `actagent` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `ACTAGENT_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveACTAgentHome(): string {
  const raw = process.env.ACTAGENT_HOME?.trim();
...
/**
 * Return a legacy path under `~/.actagent/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".actagent", "qqbot", ...subPaths);
}

/** Return a path under `~/.actagent/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<actagent-home>/.actagent/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under ACTAgent's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `ACTAGENT_HOME` (when set) so files written by agents into
 * the ACTAgent-managed media tree are reachable by this plugin even when
 * `HOME` and `ACTAGENT_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveACTAgentHome(), ".actagent", "media", "qqbot", ...subPaths);
}

/** Return a path under `<actagent-home>/.actagent/media/qqbot`, creating it on demand. */
...
/**
 * Return `<actagent-home>/.actagent/media`, ACTAgent's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `actagent/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `ACTAGENT_HOME`.
 */
function getACTAgentMediaDir(): string {
  return path.join(resolveACTAgentHome(), ".actagent", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const actagentHomeDir = resolveACTAgentHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When ACTAGENT_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the ACTAgent-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".actagent", "workspace", "qqbot"),
      path.join(actagentHomeDir, ".actagent", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and ACTAgent's shared `~/.actagent/media`
  ...
  const allowedRoots = [getACTAgentMediaDir(), getQQBotMediaPath()];

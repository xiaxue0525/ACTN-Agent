...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.actagent/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".actagent", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".actagent", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "actagent.sqlite");

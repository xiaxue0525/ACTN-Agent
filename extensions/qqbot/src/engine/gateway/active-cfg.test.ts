...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => ACTAgentConfig>());

vi.mock("actagent/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): ACTAgentConfig {
  return shape as unknown as ACTAgentConfig;
}

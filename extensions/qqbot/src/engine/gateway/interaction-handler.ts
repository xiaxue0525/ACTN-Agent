 ...
 *
 * 1. **Config query**  (type=2001) — reads config, ACKs with `actagent_cfg`.
...
// ============ actagent_cfg snapshot ============

/**
 * Build the canonical `actagent_cfg` snapshot returned in interaction ACKs.
 *
 * Pure function — all resolution helpers live in engine/config/.
 */
function buildactagentCfgSnapshot(
  ...
  return {
    ...
    actagent_type: "actagent",
    actagent_ver: getFrameworkVersion(),
...
/** Apply a config-update interaction and return the updated actagent_cfg. */
async function applyConfigUpdate(
  ...
  const actagentCfgUpdate = resolved?.actagent_cfg as Record<string, unknown> | undefined;
  const groupOpenid = event.group_openid ?? "";

  const currentCfg = structuredClone(configApi.current());
  let changed = false;

  if (actagentCfgUpdate?.require_mention !== undefined && groupOpenid) {
    applyRequireMentionUpdate(currentCfg, accountId, groupOpenid, actagentCfgUpdate);
    changed = true;
  }
  ...
  if (changed) {
    ...
    log?.info(
      `Config updated via interaction ${event.id}: require_mention=${String(actagentCfgUpdate?.require_mention)}, group=${groupOpenid}`,
    );
  }

  const latestCfg = changed ? configApi.current() : currentCfg;
  return buildactagentCfgSnapshot(latestCfg, accountId, groupOpenid, runtime);
}
  ...
  return (event) => {
    ...
    if (type === InteractionType.CONFIG_QUERY && runtime.config) {
      void handleWithAck(creds, event, log, "CONFIG_QUERY", () => {
        ...
        return buildactagentCfgSnapshot(cfg, account.accountId, event.group_openid ?? "", runtime);
      });
      return;
    }
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => ACTAgentConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  let cfg: ACTAgentConfig;
  try {
    cfg = params.getActiveCfg() as ACTAgentConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
...
async function handleWithAck(
  ...
  try {
    const actagentCfg = await handler();
    await acknowledgeInteraction(creds, event.id, 0, { actagent_cfg: actagentCfg });

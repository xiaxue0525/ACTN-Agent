...
import { coerceSecretRef, normalizeSecretInputString } from "actagent/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: ACTAgentConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: ACTAgentConfig): string {
...
/** Apply account config updates back into the ACTAgent config object. */

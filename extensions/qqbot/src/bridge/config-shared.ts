...
export const qqbotConfigAdapter = {
  listAccountIds: (cfg: ACTAgentConfig) => listQQBotAccountIds(cfg),
  resolveAccount: (cfg: ACTAgentConfig, accountId?: string | null) =>
    resolveQQBotAccount(cfg, accountId, { allowUnresolvedSecretRef: true }),
  defaultAccountId: (cfg: ACTAgentConfig) => resolveDefaultQQBotAccountId(cfg),
  setAccountEnabled: ({
    cfg,
    accountId,
    enabled,
  ...
  deleteAccount: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId: string }) =>
    deleteAccountFromConfigSection({
      cfg,
      sectionKey: "qqbot",
      accountId,
...
export const qqbotSetupAdapterShared = {
  resolveAccountId: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId?: string | null }) =>

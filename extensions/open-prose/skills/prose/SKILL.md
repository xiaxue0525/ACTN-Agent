...
metadata: { "actagent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ACTAgent `sessions_spawn`
- **File I/O** == ACTAgent `read`/`write`
- **Remote fetch** == ACTAgent `web_fetch` (or `exec` with curl when POST is required)

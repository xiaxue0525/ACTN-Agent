...
import { resolveMergedAccountConfig } from "actagent/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "actagent";
  const botDisplayName = merged.botDisplayName?.trim() || "ACTAgent QA";
  return {
    accountId,
    enabled,

...
import { canResolveEnvSecretRefInReadOnlyPath } from "actagent/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "actagent/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "actagent/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<ACTAgentConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<ACTAgentConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ACTAgentConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: ACTAgentConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: ACTAgentConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: ACTAgentConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: ACTAgentConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: ACTAgentConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: ACTAgentConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: ACTAgentConfig, override?: number): number {

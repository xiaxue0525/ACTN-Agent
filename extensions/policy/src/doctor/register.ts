...
const policySecretsUnmanagedProviderCheck: HealthCheck = {
  ...
  kind: "plugin",
  description:
    "ACTAgent config SecretRefs use configured secret providers when policy requires managed providers.",
  source: "policy",
  async detect(ctx) {
...
const policySecretsDeniedProviderSourceCheck: HealthCheck = {
  ...
  kind: "plugin",
  description:
    "ACTAgent config secret providers and SecretRefs do not use sources denied by policy.",
  source: "policy",
  async detect(ctx) {
...
const policyAuthProfileInvalidMetadataCheck: HealthCheck = {
  ...
  kind: "plugin",
  description: "ACTAgent config auth profiles declare required provider and mode metadata.",
  source: "policy",
  async detect(ctx) {
...
const policyAuthProfileUnapprovedModeCheck: HealthCheck = {
  ...
  kind: "plugin",
  description: "ACTAgent config auth profile modes stay within the policy allowlist.",
  source: "policy",
  async detect(ctx) {
...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://actagent.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://actagent.config/plugins/entries/policy/config/expectedAttestationHash",
...
function isGroupIngressDisabled(
  ...
  const channelDefaultsParent = "oc://actagent.config/channels/defaults";
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or actagent.",
        ),
      );
    }
  }
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/actagent\.config\/channels\/(.+)$/)?.[1])

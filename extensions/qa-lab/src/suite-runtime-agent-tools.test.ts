...
const repoRoot = "/repo/actagent";
const gatewayTempRoot = "/tmp/actagent-qa-runtime";
  ...
  it("keeps plugin-tools MCP stderr on startup failures", async () => {
    connectMock.mockImplementationOnce(async () => {
      const stderrListener = stderrOnMock.mock.calls[0]?.[1] as
        ...
        | undefined;
      stderrListener?.(
        Buffer.from("Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@actagent/example'\n"),
      );
    ...
    expect(message).toContain("MCP stderr tail:");
    expect(message).toContain("Cannot find package '@actagent/example'");
    expect(closeMock).toHaveBeenCalled();
  });
});

...
export type TokenEfficiencyRow = {
  ...
  actagent: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    actagent: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  actagent: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(actagentTotalTokens: number, codexTotalTokens: number): number {
  if (actagentTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - actagentTotalTokens) / actagentTotalTokens) * 100;
}
...
function toolNamesForCells(actagent: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...actagent.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const actagent = runtimeUsage(params.result.cells.actagent);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(actagent.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.actagent, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const actagentTotals = rows.map((row) => row.actagent.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const actagentTotalTokens = actagentTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    actagent: {
      totalTokens: actagentTotalTokens,
      p50PerScenario: percentile(actagentTotals, 50),
      p90PerScenario: percentile(actagentTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(actagentTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.actagent.totalTokens <= 0) {
    failures.push(`${row.scenarioId} actagent live usage totalTokens=${row.actagent.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-ACTAgent live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# ACTAgent Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| actagent | ${report.aggregate.actagent.totalTokens} | ${report.aggregate.actagent.p50PerScenario} | ${report.aggregate.actagent.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | ACTAgent in/out/total/tools | Codex in/out/total/tools | Token delta | Classification | Flagged | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.actagent.inputTokens}/${row.actagent.outputTokens}/${row.actagent.totalTokens}/${row.actagent.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }

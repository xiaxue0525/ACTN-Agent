...
export type RuntimeId = "actagent" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    runtimeParityCellStatus(result.cells.actagent) === "pass" &&
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[ACTAgent heartbeat poll]";
...
function classifyRuntimeParityCells(params: {
  actagent: RuntimeParityCell;
  codex: RuntimeParityCell;
  actagentScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.actagent.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.actagent.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.actagent.transportErrorClass || params.codex.transportErrorClass
  ...
  const actagentTranscriptLines = params.actagent.transcriptBytes.trim().length
    ? params.actagent.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    actagentTranscriptLines !== codexTranscriptLines ||
    (!params.actagent.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.actagent.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${actagentTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    params.actagentScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    params.actagent.runtimeErrorClass ||
    params.codex.runtimeErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.actagentScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.actagentScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }

  if (
    normalizeTextForParity(params.actagent.finalText) ===
...
export async function runRuntimeParityScenario(params: {
  ...
  const actagent = await params.runCell("actagent");
  const codex = await params.runCell("codex");
  const drift = classifyRuntimeParityCells({
    actagent: actagent.cell,
    codex: codex.cell,
    actagentScenarioStatus: actagent.scenarioStatus,

...
type QaRuntimeParityScenarioReport = {
  ...
  driftDetails?: string;
  actagentStatus: "pass" | "fail" | "missing";
  codexStatus: "pass" | "fail" | "missing";
  actagentTokens: number;
  codexTokens: number;
...
function describeLiveUsageFailure(scenarioName: string, scenario: QaRuntimeParityScenarioReport) {
  const missing = [
    scenario.actagentTokens > 0
      ? undefined
      : `${scenario.actagentStatus === "pass" ? "actagent" : "actagent failed"}=0`,
...
export function renderQaAgenticParityMarkdownReport(comparison: QaAgenticParityComparison): string {
  ...
  const lines = [
    `# ACTAgent Agentic Parity Report — ${comparison.candidateLabel} vs ${comparison.baselineLabel}`,
    "",
...
export function buildQaRuntimeParityReport(params: {
  ...
  const scenarios: QaRuntimeParityScenarioReport[] = params.summary.scenarios.map((scenario) => {
    ...
    const actagentCell = parity.cells.actagent;
    const codexCell = parity.cells.codex;
    const actagentStatus = runtimeParityCellStatus(actagentCell);
    ...
    const reportScenario = {
      ...
      actagentStatus,
      codexStatus,
      actagentTokens: actagentCell.usage.totalTokens,
      codexTokens: codexCell.usage.totalTokens,
      actagentToolCalls: actagentCell.toolCalls.length,
...
export function renderQaRuntimeParityMarkdownReport(report: QaRuntimeParityReport): string {
  const lines = [
    `# ACTAgent Runtime Parity Report — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  for (const scenario of report.scenarios) {
    ...
    lines.push(
      `- actagent: ${scenario.actagentStatus} (${scenario.actagentToolCalls} tool calls, ${scenario.actagentTokens} tokens)`,
    );
    lines.push(

...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { actagent: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { actagent: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      actagent: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.actagent.push(parity.cells.actagent);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# ACTAgent JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",

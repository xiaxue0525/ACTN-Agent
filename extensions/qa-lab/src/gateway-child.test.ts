...
vi.mock("actagent/plugin-sdk/temp-path", () => ({
  resolvePreferredACTAgentTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/actagent-qa/actagent.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/actagent-qa/home",
    stateDir: "/tmp/actagent-qa/state",
    tempRoot: "/tmp/actagent-qa",
    xdgConfigHome: "/tmp/actagent-qa/xdg-config",
    xdgDataHome: "/tmp/actagent-qa/xdg-data",
    xdgCacheHome: "/tmp/actagent-qa/xdg-cache",
    bundledPluginsDir: "/tmp/actagent-qa/bundled-plugins",
    stagedBundledPluginsRoot: "/repo/.artifacts/qa-runtime/actagent-qa-suite-test",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.ACTAGENT_TEST_FAST).toBe("1");
    expect(env.ACTAGENT_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS).toBe("2000");
    expect(env.ACTAGENT_QA_PARENT_PID).toBe(String(process.pid));
    expect(env.ACTAGENT_QA_TEMP_ROOT).toBe("/tmp/actagent-qa");
    expect(env.ACTAGENT_QA_STAGED_RUNTIME_ROOT).toBe(
      "/repo/.artifacts/qa-runtime/actagent-qa-suite-test",
    );
    expect(env.ACTAGENT_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.ACTAGENT_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.ACTAGENT_BUNDLED_PLUGINS_DIR).toBe("/tmp/actagent-qa/bundled-plugins");
    expect(env.ACTAGENT_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ACTAGENT_LIVE_OPENAI_KEY: "openai-live",
        ACTAGENT_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ACTAGENT_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing ACTAgent home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/actagent-qa/home");
    expect(env.ACTAGENT_HOME).toBe("/tmp/actagent-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping ACTAgent home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.ACTAGENT_HOME).toBe("/tmp/actagent-qa/home");
    expect(env.ACTAGENT_STATE_DIR).toBe("/tmp/actagent-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping ACTAgent home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        ACTAGENT_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.ACTAGENT_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.ACTAGENT_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ACTAGENT_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        ACTAGENT_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.ACTAGENT_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.ACTAGENT_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ACTAGENT_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        ACTAGENT_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.ACTAGENT_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.ACTAGENT_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          ACTAGENT_LIVE_ANTHROPIC_KEY: "anthropic-live",
          ACTAGENT_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          ACTAGENT_LIVE_CODEX_API_KEY: "codex-live",
          ACTAGENT_LIVE_GEMINI_KEY: "gemini-live",
          ACTAGENT_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.ACTAGENT_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.ACTAGENT_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.ACTAGENT_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.ACTAGENT_LIVE_CODEX_API_KEY).toBeUndefined();
      expect(env.ACTAGENT_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        ACTAGENT_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("stages the OpenAI API-key fallback for live OpenAI QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        ACTAGENT_LIVE_OPENAI_KEY: "qa-live-codex-fallback-key",
      },
    });
  ...
  it("stages direct live OpenAI API-key aliases for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        ACTAGENT_LIVE_CODEX_API_KEY: "qa-live-direct-codex-key",
      },
    });
  ...
  it("fails fast when live OpenAI runs have no portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        providerIds: ["openai"],
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-actagent-codex-home"),
        },
        readCodexCredentials: () => null,
      }),
  ...
  it("fails fast when forced Codex runtime uses OpenAI model refs without portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-actagent-codex-home"),
          ACTAGENT_QA_FORCE_RUNTIME: "codex",
        },
        readCodexCredentials: () => null,
      }),
    ).toThrow("QA live-frontier cannot run Codex-backed OpenAI models");
  ...
  it("stages configured OpenAI env secret refs for default OpenAI live QA runs", async () => {
    ...
    const env = {
      ACTAGENT_LIVE_CODEX_API_KEY: "qa-configured-env-ref-not-a-real-key",
    };
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      cfg: {
        models: {
          providers: {
            openai: {
              ...
              apiKey: {
                source: "env",
                provider: "default",
                id: "ACTAGENT_LIVE_CODEX_API_KEY",
              },
            },
          },
        },
  ...
  it("stages configured OpenAI env markers for live QA runs", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
        ...
        models: {
          providers: {
            openai: {
              baseUrl: "",
              models: [],
              apiKey: "ACTAGENT_LIVE_CODEX_API_KEY",
            },
          },
        },
      },
      ...
      providerIds: ["openai"],
      env: {
        ACTAGENT_LIVE_CODEX_API_KEY: "qa-configured-marker-not-a-real-key",
      },
    });
  ...
  it("allows loopback gateway health probes through the SSRF guard", async () => {
    ...
    expect(request.url).toBe("http://127.0.0.1:19199/readyz");
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "ACTAGENT_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "ACTAGENT_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "ACTAGENT_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
        "ACTAGENT_LIVE_CODEX_API_KEY=codex-live-secret",
        "botToken=12345:AbCdEfGhIjKl",
        '"driverToken":"12345:driver-secr3t"',
        "sutToken='12345:sut-secr3t'",
        "leaseToken=lease-12345",
        "url=http://127.0.0.1:19199/#token=abc123",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "ACTAGENT_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "ACTAGENT_QA_CONVEX_SECRET_CI=<redacted>",
        "ACTAGENT_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "ACTAGENT_LIVE_CODEX_API_KEY=<redacted>",
        "botToken=<redacted>",
        '"driverToken":"<redacted>"',
        "sutToken=<redacted>",
        "leaseToken=<redacted>",
        "url=http://127.0.0.1:19199/#token=<redacted>",
      ].join("\n"),
    );
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      testing.assertQaArtifactDirWithinRepo("/tmp/actagent-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "actagent.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "actagent.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "actagent.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "actagent.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@actagent/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@actagent/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "actagent.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    const overrides = await testing.readQaLiveProviderConfigOverrides({
      providerIds: ["custom-openai"],
      env: { ACTAGENT_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
    });
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ actagent: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ actagent: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ actagent: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );

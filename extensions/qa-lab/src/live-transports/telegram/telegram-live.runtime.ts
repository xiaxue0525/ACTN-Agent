...
import { isRecord, uniqueStrings } from "actagent/plugin-sdk/string-coerce-runtime";
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    regressionRefs: ["actagent/actagent#74698"],
    timeoutMs: 45_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        expectReply: true,
        input: `/status@${sutUsername}`,
        expectedTextIncludes: ["ACTAgent", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["ACTAgent", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@ACTAgentQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["actagent/actagent#87392"],
    timeoutMs: 90_000,
    buildRun: (sutUsername) => {
  ...
  },
  {
    ...
    regressionRefs: ["actagent/actagent#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        allowAnySutReply: true,
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "ACTAGENT_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "ACTAGENT_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "ACTAGENT_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "ACTAGENT_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("ACTAGENT_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledACTAgentTelegramOnboardingPreflight(params: {
  actAgentCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".actagent");
  ...
  const env = {
    ...process.env,
    HOME: homeDir,
    ACTAGENT_HOME: stateDir,
    ACTAGENT_CONFIG_PATH: path.join(stateDir, "actagent.json"),
    ACTAGENT_STATE_DIR: stateDir,
    ACTAGENT_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-actagent-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
      params.actAgentCommand,
      [
        "onboard",
        "--non-interactive",
        "--accept-risk",
    ...
    await execFileAsync(params.actAgentCommand, ["doctor", "--non-interactive"], { env });
  } finally {
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutACTAgentCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledACTAgentTelegramOnboardingPreflight({
        actAgentCommand: params.sutACTAgentCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutACTAgentCommand
        ? {
            executablePath: params.sutACTAgentCommand,

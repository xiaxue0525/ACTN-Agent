...
describe("live transport artifacts", () => {
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (ACTAGENT_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (ACTAGENT_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
});

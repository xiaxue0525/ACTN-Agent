...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        ACTAGENT_QA_SLACK_CHANNEL_ID: "C123456789",
        ACTAGENT_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ACTAGENT_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ACTAGENT_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        ACTAGENT_QA_SLACK_CHANNEL_ID: "qa-channel",
        ACTAGENT_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ACTAGENT_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ACTAGENT_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("ACTAGENT_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        ACTAGENT_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        ACTAGENT_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("resolves Slack readiness timeout from the shared transport env", () => {
    ...
    expect(
      testing.resolveSlackQaReadyTimeoutMs({
        ACTAGENT_QA_TRANSPORT_READY_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveSlackQaReadyTimeoutMs({
        ACTAGENT_QA_TRANSPORT_READY_TIMEOUT_MS: "bad",
      }),
    ).toBe(45_000);
  });
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "actagent-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing ACTAGENT_QA_CONVEX_SITE_URL");

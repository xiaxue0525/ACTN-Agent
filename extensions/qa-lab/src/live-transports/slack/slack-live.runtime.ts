...
import { createSlackWebClient, createSlackWriteClient } from "@actagent/slack/api.js";
...
const SLACK_QA_CAPTURE_CONTENT_ENV = "ACTAGENT_QA_SLACK_CAPTURE_CONTENT";
const SLACK_QA_APPROVAL_CHECKPOINT_DIR_ENV = "ACTAGENT_QA_SLACK_APPROVAL_CHECKPOINT_DIR";
const SLACK_QA_APPROVAL_CHECKPOINT_TIMEOUT_MS_ENV =
  "ACTAGENT_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS";
...
const SLACK_QA_SCENARIOS: SlackQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    timeoutMs: 8_000,
    configOverrides: {
      allowFrom: ["U_ACTAGENT_QA_NEVER_ALLOWED"],
      users: ["U_ACTAGENT_QA_NEVER_ALLOWED"],
    },
    buildRun: (sutUserId) => {
...
function resolveSlackQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): SlackQaRuntimeEnv {
  const runtimeEnv = {
    channelId: resolveEnvValue(env, "ACTAGENT_QA_SLACK_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "ACTAGENT_QA_SLACK_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "ACTAGENT_QA_SLACK_SUT_BOT_TOKEN"),
    sutAppToken: resolveEnvValue(env, "ACTAGENT_QA_SLACK_SUT_APP_TOKEN"),
  };
  return validateSlackQaRuntimeEnv(runtimeEnv, "ACTAGENT_QA_SLACK");
}
...
function resolveSlackQaReadyTimeoutMs(env: NodeJS.ProcessEnv = process.env) {
  const raw = env.ACTAGENT_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {

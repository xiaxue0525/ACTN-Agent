...
import { startWhatsAppQaDriverSession } from "@actagent/whatsapp/api.js";
import { normalizeE164 } from "actagent/plugin-sdk/account-resolution";
...
import { normalizeStringEntries, uniqueStrings } from "actagent/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "ACTAGENT_QA_WHATSAPP_CAPTURE_CONTENT";
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /ACTAgent: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `actagentqa reply with only this exact marker: ${replyToken}`,
...
function inferWhatsAppCredentialSource(
  ...
  const normalized =
    value?.trim().toLowerCase() || env.ACTAGENT_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "ACTAGENT_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "ACTAGENT_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "ACTAGENT_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "ACTAGENT_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.ACTAGENT_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "ACTAGENT_QA_WHATSAPP",
  );
}
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredACTAgentTmpDir(), "actagent-whatsapp-qa-"),
    );

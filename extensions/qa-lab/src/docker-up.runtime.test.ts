...
function createHealthyDockerDeps(calls: string[]): QaDockerUpDeps {
  return {
    async runCommand(command, args, cwd) {
      ...
      if (args.join(" ").includes("ps --format json actagent-qa-gateway")) {
...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    const repoRoot = path.resolve("/repo/actagent");
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${composeFile} ps --format json actagent-qa-gateway @${repoRoot}`,
      ]);
  ...
  it("uses a repo-root-relative default output dir when none is provided", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${path.join(repoRoot, ".artifacts/qa-docker/docker-compose.qa.yml")} ps --format json actagent-qa-gateway @${repoRoot}`,
      ]);
    } finally {
  ...
  it("falls back to free host ports when defaults are already occupied", async () => {
    ...
    const gatewayPortReservation = await occupyPortOrAcceptExisting(19199);
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json actagent-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q actagent-qa-gateway")) {
          ...
          fetchImpl: vi.fn(async (input: string) => {
            ...
            return {
              ok:
                ...
                input === "http://192.168.165.4:19199/healthz",
            };
          }),
          sleepImpl: vi.fn(async () => {}),
        },
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${composeFile} up -d @${repoRoot}`,
        `docker compose -f ${composeFile} ps --format json actagent-qa-gateway @${repoRoot}`,
        `docker compose -f ${composeFile} ps -q actagent-qa-gateway @${repoRoot}`,
        `docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @${repoRoot}`,
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
        "http://192.168.165.4:19199/healthz",
      ]);
      expect(result.gatewayUrl).toBe("http://192.168.165.4:19199/");
    } finally {
      await rm(outputDir, { recursive: true, force: true });
    }
  });

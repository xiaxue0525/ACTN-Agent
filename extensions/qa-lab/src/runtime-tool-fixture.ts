...
function formatCodexNativeWorkspaceDetails(params: {
  ...
  return [
    `codex-native-workspace ${params.toolName}: ACTAgent dynamic exposure is intentionally omitted because Codex owns this workspace operation natively`,
    params.reason ? `reason: ${params.reason}` : undefined,
    `available ACTAgent dynamic tools: ${[...params.tools].toSorted().join(", ")}`,
    params.happyRequest
...
export async function runRuntimeToolFixture(
  ...
  const dynamicExposureIntentionallyExcluded =
    env.gateway.runtimeEnv.ACTAGENT_QA_FORCE_RUNTIME === "codex" &&

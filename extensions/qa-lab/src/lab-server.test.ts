...
vi.mock("@actagent/qa-channel/api.js", async () => await import("../../qa-channel/api.js"));
...
vi.mock("actagent/plugin-sdk/proxy-capture", () => ({
  ...
  resolveDebugProxySettings: () => ({
    dbPath: process.env.ACTAGENT_DEBUG_PROXY_DB_PATH ?? "",
    blobDir: process.env.ACTAGENT_DEBUG_PROXY_BLOB_DIR ?? "",
    proxyUrl: process.env.ACTAGENT_DEBUG_PROXY_URL ?? "",
...
describe("qa-lab server", () => {
  it("serves bootstrap state and message state", async () => {
    ...
    const lab = await startQaLabServerForTest({
      ...
      outputPath,
      repoRoot,
      controlUiUrl: "http://127.0.0.1:19199/?token=qa-token&panel=chat#token=fragment-token",
      embeddedGateway: "disabled",
    });
    cleanups.push(async () => {
      await lab.stop();
    ...
    expect(bootstrap.controlUiUrl).toBe("http://127.0.0.1:19199/?panel=chat");
    expect(bootstrap.controlUiEmbeddedUrl).toBe("http://127.0.0.1:19199/?panel=chat");
    ...
    expect(startupStatus.status.gateway.url).toBe("http://127.0.0.1:19199/?panel=chat");
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "  process.exit(0);",
        "});",
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.ACTAGENT_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "setInterval(() => {}, 1000);",
      ].join("\n"),
      "utf8",
    );
  ...
  it("exposes structured outcomes and can attach control-ui after startup", async () => {
    ...
    lab.setControlUi({
      controlUiUrl: "http://127.0.0.1:19199/?password=late-password#token=late-token",
    });

    const bootstrap = (await (await fetchWithRetry(`${lab.baseUrl}/api/bootstrap`)).json()) as {
      controlUiEmbeddedUrl: string | null;
    };
    expect(bootstrap.controlUiEmbeddedUrl).toBe("http://127.0.0.1:19199/");
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    process.env.ACTAGENT_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.ACTAGENT_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    const store = captureMock.store;
    store.upsertSession({
      id: "qa-capture-session",
      startedAt: Date.now(),
      mode: "proxy-run",
      sourceScope: "actagent",
    ...
    cleanups.push(async () => {
      delete process.env.ACTAGENT_DEBUG_PROXY_DB_PATH;
      delete process.env.ACTAGENT_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });

...
describe("codex plugin lifecycle: pinned-old codex plugin with new ACTAgent", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than ACTAgent 2026.5.21. Run "actagent plugins update codex" or unpin codex, then rerun "actagent doctor --fix".',
    );
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old ACTAgent", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer ACTAgent host than 2026.5.21. Upgrade ACTAgent or install a codex plugin version pinned to 2026.5.21.",
    );
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults ACTAgent pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "actagent" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=actagent"],
    },
    {
      name: "mixed profile with main-agent ACTAgent pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "actagent" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=actagent"],
    },
  ])(
    "keeps codex auth and strips stale ACTAgent runtime pins for $name",

...
} from "actagent/plugin-sdk/provider-auth";
...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "ACTAGENT_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "ACTAGENT_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "ACTAGENT_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["ACTAGENT_LIVE_ANTHROPIC_KEY"],
  gemini: ["ACTAGENT_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "ACTAGENT_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: ACTAgentConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: ACTAgentConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: ACTAgentConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.ACTAGENT_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "actagent") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or ACTAGENT_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host ACTAgent OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}

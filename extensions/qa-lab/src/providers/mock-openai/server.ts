...
import { escapeRegExp } from "actagent/plugin-sdk/text-utility-runtime";
import { readRequestBodyWithLimit } from "actagent/plugin-sdk/webhook-ingress";
...
function buildQaToolSearchArgs(targetTool: string, failureMode: boolean): Record<string, unknown> {
  ...
  if (targetTool === "web_search") {
    return { query: "ACTAgent runtime parity fixed query", count: 1 };
  }
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this ACTAgent repo from source + docs before acting.`;
  }
  ...
  if (
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await actagent.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          `return await actagent.tools.call(match.id, ${JSON.stringify(plannedArgs)});`,
        ].join("\n"),
      });
    }
    ...
    if (
      ...
      return buildToolCallEventsWithArgs("write", {
        ...
        content: [
          ...
          "Trial prompt: Prepare a release readiness reply for a local ACTAgent QA change.",
    ...
    if (
      ...
      return buildToolCallEventsWithArgs("write", {
        ...
        content: [
          ...
          "Next step: ask maintainer whether manually landed commits can count for author credit",
        ].join("\n"),
      });
    }
  ...
  if (/personal task followthrough check/i.test(allInputText)) {
    ...
    if (
      ...
      (!taskEvidenceText.includes("# Personal task ledger") &&
        !taskEvidenceText.includes("Task: prepare a local ACTAgent PR readiness note."))
    ) {

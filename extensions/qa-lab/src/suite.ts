...
import { disposeRegisteredAgentHarnesses } from "actagent/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.ACTAGENT_QA_SUITE_PROGRESS);
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.ACTAGENT_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const actagentCell = params.result.cells.actagent;
  return {
    ...
    steps: [
      {
        name: actagentCell.runtime,
        status:
          actagentCell.runtimeErrorClass || actagentCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(actagentCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.ACTAGENT_BUILD_PRIVATE_QA = "1";
    patch.ACTAGENT_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.ACTAGENT_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.ACTAGENT_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
async function writeQaSuiteArtifacts(params: {
  ...
  const report = renderQaMarkdownReport({
    title: "ACTAgent QA Scenario Suite",
    startedAt: params.startedAt,
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "actagent"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as ACTAgentConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.ACTAGENT_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,

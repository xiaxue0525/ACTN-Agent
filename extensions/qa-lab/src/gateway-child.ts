...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    ACTAGENT_HOME: params.homeDir,
    ACTAGENT_CONFIG_PATH: params.configPath,
    ACTAGENT_STATE_DIR: params.stateDir,
    ACTAGENT_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    ACTAGENT_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    ACTAGENT_QA_PARENT_PID: String(process.pid),
    ACTAGENT_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { ACTAGENT_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    ACTAGENT_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    ACTAGENT_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { ACTAGENT_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { ACTAGENT_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: ACTAgentConfig) => ACTAgentConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-qa-suite-"),
  );
  ...
  const keepTemp = process.env.ACTAGENT_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";

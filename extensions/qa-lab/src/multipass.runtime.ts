...
import { sleep } from "actagent/plugin-sdk/runtime-env";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/actagent-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/actagent";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/actagent-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `actagent-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "ACTAGENT_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `ACTAGENT_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `ACTAGENT_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# ACTAgent QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm actagent qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), `${plan.vmName}-qa-suite-`),
  );

...
function renderCaptureStartupInstructions(status: CaptureStartupStatus | null): string {
  ...
  const gatewayStart = `ACTAGENT_DEBUG_PROXY_ENABLED=1 \\
ACTAGENT_DEBUG_PROXY_REQUIRE=1 \\
ACTAGENT_DEBUG_PROXY_URL=http://127.0.0.1:7799 \\
pnpm actagent gateway --port 19199 --bind loopback`;
  const qaStart = "pnpm qa:lab:ui --port 43124 --control-ui-url http://127.0.0.1:19199/";
  const caInstall = "pnpm proxy:install-ca";
  const caTrust =
    "sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain /Users/thoffman/.actagent/debug-proxy/certs/root-ca.pem";
...
function messageAvatar(m: Message): { emoji: string; bg: string; role: string } {
  if (m.direction === "outbound") {
    return { emoji: "\uD83E\uDD80", bg: "#7c6cff", role: "actagent" }; // 🦀
  }
  return { emoji: "\uD83E\uDD9E", bg: "#d97706", role: "actagentfather" }; // 🐲
}

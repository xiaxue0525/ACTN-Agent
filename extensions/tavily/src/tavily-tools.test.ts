...
function fakeApi(): ACTAgentPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.actagent.ai", "", "actagent.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.actagent.ai", "actagent.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<ACTAgentPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<ACTAgentPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "actagent" });
    ...
    expect(searchParams.query).toBe("actagent");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as ACTAgentConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);

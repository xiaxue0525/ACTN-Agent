...
vi.mock("actagent/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: actagent", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "actagent" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: actagent", async () => {

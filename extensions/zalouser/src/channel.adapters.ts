...
import { createScopedDmSecurityResolver } from "actagent/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: ACTAgentConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: ACTAgentConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: ACTAgentConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();

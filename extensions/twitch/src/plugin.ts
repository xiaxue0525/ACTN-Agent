/**
 * Twitch channel plugin for ACTAgent.
 *
...
} from "actagent/plugin-sdk/channel-pairing";
 ...
 * for ACTAgent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: ACTAgentConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: ACTAgentConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: ACTAgentConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: ACTAgentConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("actagent/plugin-sdk/runtime-env").RuntimeEnv;

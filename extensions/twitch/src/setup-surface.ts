...
import { getChatChannelMeta, type ChannelPlugin } from "actagent/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: ACTAgentConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ACTAGENT_TWITCH_ACCESS_TOKEN?.trim();

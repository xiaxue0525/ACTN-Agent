...
import type { LookupFn } from "actagent/plugin-sdk/ssrf-runtime";
...
vi.mock("actagent/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("actagent/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=actagent&format=json&categories=general%2Cnews&language=en",
    );
  });

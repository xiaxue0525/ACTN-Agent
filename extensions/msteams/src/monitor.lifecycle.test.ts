...
function createConfig(port: number): ACTAgentConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: ACTAgentConfig,
  patch: NonNullable<NonNullable<ACTAgentConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): ACTAgentConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { actagentPollId: "poll-1", choices: "0" } } },
      },
    });

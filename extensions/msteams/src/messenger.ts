...
import {
  ...
  SILENT_REPLY_TOKEN,
  type ChunkMode,
} from "actagent/plugin-sdk/reply-chunking";
import {
...
import type { MarkdownTableMode, MSTeamsReplyStyle, ACTAgentConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as ACTAgentConfig,
      channel: "msteams",
    });

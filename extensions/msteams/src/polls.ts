...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["actagentPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["actagent", "pollId"]) ??
    readNestedString(value, ["actagent", "poll", "id"]) ??
    readNestedString(value, ["data", "actagentPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "actagent", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "actagentPollId"]) ??
    ...
    actions: [
      {
        type: "Action.Execute",
        title: "Vote",
        verb: "actagent.poll.vote",
        data: {
          actagentPollId: pollId,
          pollId,
        },
      },
    ],

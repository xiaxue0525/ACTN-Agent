...
function resolveQianfanPreset(cfg: ACTAgentConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: ACTAgentConfig) => {
...
export function applyQianfanProviderConfig(cfg: ACTAgentConfig): ACTAgentConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: ACTAgentConfig): ACTAgentConfig {

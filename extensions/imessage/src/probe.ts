...
import type { BaseProbeResult } from "actagent/plugin-sdk/channel-contract";
import {
  asDateTimestampMs,
  resolveExpiresAtMsFromDurationMs,
} from "actagent/plugin-sdk/number-runtime";
import { runCommandWithTimeout } from "actagent/plugin-sdk/process-runtime";
...
import { detectBinary } from "actagent/plugin-sdk/setup";
import {
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run ACTAgent on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by actagent/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // actagent/imsg#114. We do this even when the bridge is unavailable

...
function cfg(actions?: Record<string, boolean | undefined>): ACTAgentConfig {
  return {
    channels: {
      imessage: {
        cliPath: "imsg",
...
describe("imessage message actions", () => {
  ...
  describe("reply with attachment (actagent/imsg#114 plumbing)", () => {
    ...
    it("rejects reply + attachment when imsg does not advertise send-rich --file", async () => {
      ...
      // drop the attachment (the original actagent/actagent#79822 symptom).
      probeMock.getCachedIMessagePrivateApiStatus.mockReturnValue({
        available: true,
        v2Ready: true,
        selectors: {},

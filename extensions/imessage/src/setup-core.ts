...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: ACTAgentConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: ACTAgentConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: ACTAgentConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run ACTAgent on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `actagent channels status --probe` to verify private API actions.",
    "Ensure ACTAgent has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: ACTAgentConfig; accountId?: string }) =>

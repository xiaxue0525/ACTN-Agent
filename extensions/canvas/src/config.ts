...
export function isCanvasPluginEnabled(config?: ACTAgentConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: ACTAgentConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the ACTAgent state canvas directory.",
      advanced: true,
    },
    "host.port": {

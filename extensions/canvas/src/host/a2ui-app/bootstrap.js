...
import { themeContext } from "@actagent/a2ui-theme-context";
...
class ACTAgentA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.actagentA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["actagent:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.actagentCanvasA2UIAction ??
      globalThis.actagentCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.actagentCanvasA2UIAction) {
...
if (!customElements.get("actagent-a2ui-host")) {
  customElements.define("actagent-a2ui-host", ACTAgentA2UIHost);
}

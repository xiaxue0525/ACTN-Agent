...
import { isTruthyEnvValue, type RuntimeEnv } from "actagent/plugin-sdk/runtime-env";
import { resolveStateDir } from "actagent/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "actagent/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "actagent/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.actagentCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.actagentCanvasA2UIAction &&
        typeof window.actagentCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.actagentSendUserAction === "function";
  ...
  window.addEventListener("actagent:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ACTAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.actagentSendUserAction === "function"
        ? window.actagentSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ACTAGENT_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ACTAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

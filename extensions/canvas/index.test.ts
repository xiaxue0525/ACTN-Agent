...
import type { AnyAgentTool, ACTAgentPluginApi } from "actagent/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<ACTAgentPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<ACTAgentPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<ACTAgentPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<ACTAgentPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<ACTAgentPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<ACTAgentPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__actagent__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__actagent__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );

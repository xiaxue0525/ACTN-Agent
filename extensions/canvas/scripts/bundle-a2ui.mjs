...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.ACTAGENT_SPARSE_PROFILE || process.env.ACTAGENT_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because ACTAGENT_A2UI_SKIP_MISSING=1 or ACTAGENT_SPARSE_PROFILE is set.",
      );
      return;
    }

...
function resolveKimiBaseUrl(kimi?: KimiConfig, actAgentConfig?: ACTAgentConfig): string {
  ...
  const moonshotBaseUrl = actAgentConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: ACTAgentConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.actagent.ai/tools/kimi-search",

...
async function importLobsterAjvConstructor(): Promise<AjvConstructor> {
  const lobsterEntry = requireForTest.resolve("@actagentdbot/lobster");
...
describe("createEmbeddedLobsterRunner", () => {
  ...
  it("detects workflow files and parses argsJson", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lobster-runner-"));
  ...
  it("fails closed when the embedded runtime requests unsupported input", async () => {
    ...
    ).rejects.toThrow("Lobster input requests are not supported by the ACTAgent Lobster tool yet");
  });
  ...
  it("falls back to the installed package core file when the core export is unavailable", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lobster-package-"));
    const packageRoot = path.join(tempDir, "node_modules", "@actagentdbot", "lobster");
    ...
    try {
      ...
      const runtime = await loadEmbeddedToolRuntimeFromPackage({
        importModule: async (specifier) => {
          if (specifier === "@actagentdbot/lobster/core") {
  ...
  it("deduplicates content-identical schema compilation in the installed Lobster runtime", async () => {
    ...
    const corePath = requireForTest.resolve("@actagentdbot/lobster/core");

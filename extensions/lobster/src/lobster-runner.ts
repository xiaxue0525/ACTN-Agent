...
function findLobsterPackageRoot(resolvedEntryPath: string): string {
  ...
  while (true) {
    ...
    try {
      ...
      if (parsed.name === "@actagentdbot/lobster") {
        return dir;
      }
    } catch {
    ...
    if (parent === dir) {
      throw new Error(`Could not locate @actagentdbot/lobster package root from ${resolvedEntryPath}`);
    }
    dir = parent;
  }
}
    ...
    if (envelope.status === "needs_input") {
      return {
        ...
        error: {
          ...
          message: "Lobster input requests are not supported by the ACTAgent Lobster tool yet",
        },
      };
    }
    return {
...
export async function loadEmbeddedToolRuntimeFromPackage(
  ...
  const packageEntryPath = resolvePackageEntry("@actagentdbot/lobster");
  await installLobsterAjvCompileCache(packageEntryPath);

  let coreLoadError: unknown;
  try {
    const coreSpecifier = ["@actagentdbot", "lobster", "core"].join("/");
    return toEmbeddedToolRuntime(await importModule(coreSpecifier), "@actagentdbot/lobster/core");
  } catch (error) {
    coreLoadError = error;
  }

...
import type { ACTAgentPluginApi, ACTAgentPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<ACTAgentPluginApi> = {}): ACTAgentPluginApi {
...
function fakeCtx(overrides: Partial<ACTAgentPluginToolContext> = {}): ACTAgentPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: ACTAgentPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }

 ...
 */
import { formatCliCommand, parseDurationMs } from "actagent/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "actagent/plugin-sdk/number-runtime";
import type {
  ACTAgentPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type ACTAgentConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "actagent/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("actagent models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("actagent models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("actagent doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: ACTAgentPluginApi): void {

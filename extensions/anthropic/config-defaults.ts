...
function usesClaudeCliModelSelection(config: ACTAgentConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: ACTAgentConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: ACTAgentConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an ACTAgent config object. */

 ...
 * thinking levels, and keeps ACTAgent-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer ACTAgent-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching ACTAgent-managed Claude CLI runs. */
...
function isACTAgentRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from ACTAgent exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isACTAgentRequestedYolo(context)
...
/** Map ACTAgent thinking levels to Claude CLI effort flags for a model id. */

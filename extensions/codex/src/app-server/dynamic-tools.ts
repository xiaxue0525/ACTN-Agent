/**
 * Bridges ACTAgent runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import { emitTrustedDiagnosticEvent } from "actagent/plugin-sdk/diagnostic-runtime";
import type { ImageContent, TextContent } from "actagent/plugin-sdk/llm";
...
/** Namespace attached to ACTAgent-owned dynamic tools exposed to Codex. */
export const CODEX_ACTAGENT_DYNAMIC_TOOL_NAMESPACE = "actagent";

// Keep ACTAgent session spawning searchable in Codex mode so Codex's native
...
/**
 * Creates dynamic tool specs and a call handler that executes ACTAgent tools,
  ...
  return {
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!tool) {
        if (registeredToolNames.has(call.tool)) {
          return {
            contentItems: [
              {
                type: "inputText",
                text: `ACTAgent tool is not available for this turn: ${call.tool}`,
              },
            ],
            success: false,
          };
        ...
        return {
          contentItems: [{ type: "inputText", text: `Unknown ACTAgent tool: ${call.tool}` }],
          success: false,
        };
      }
...
function createCodexDynamicToolSpec(params: {
  ...
  return {
    ...base,
    namespace: CODEX_ACTAGENT_DYNAMIC_TOOL_NAMESPACE,
    deferLoading: true,
  };
}
...
function convertToolContents(
  ...
  const noticeText = `...(ACTAgent truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;

...
import { resetDiagnosticEventsForTest } from "actagent/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "actagent/plugin-sdk/hook-runtime";
import { clearPluginCommands } from "actagent/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("ACTAGENT_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredACTAgentTmpDir(), "actagent-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetACTAgentCodingToolsFactoryForTests();

 ...
 * access through the active ACTAgent sandbox backend.
 */
...
import type { HttpHeader, ACTAgentExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  if (!command) {
    throw new Error("ACTAgent sandbox HTTP exec spec did not provide a command.");
  }
...
tmp=$(mktemp "$TMPDIR/actagent-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/actagent-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json

...
import { buildCodexUserMcpServersThreadConfigPatch } from "actagent/plugin-sdk/codex-mcp-projection";
import { listRegisteredPluginAgentPromptGuidance } from "actagent/plugin-sdk/plugin-runtime";
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. ACTAgent adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an ACTAgent cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
...
export function buildDeveloperInstructions(
  ...
  const sections = [
    "You are a personal agent running inside ACTAgent. ACTAgent has dynamic tools for ACTAgent-owned messaging, cron, sessions, media, gateway, and nodes.",
    buildDeferredDynamicToolManifest(options.dynamicTools),
    buildSkillWorkshopInstruction(options.dynamicTools),
    "Use Codex native `spawn_agent` for Codex subagents. Use ACTAgent `sessions_spawn` only for ACTAgent or ACP delegation.",
...
function buildDeferredDynamicToolManifest(
  ...
  return `Deferred searchable ACTAgent dynamic tools available: ${deferredToolNames.join(", ")}. Use \`tool_search\` to load exact callable specs before use.`;
}
...
function buildVisibleReplyInstruction(
  ...
  if (messageToolAvailable) {
    return "For the current source conversation, reply normally in your final assistant message; ACTAgent will deliver it through the active source conversation. Use `message` only for explicit out-of-band sends, media/file sends, or sends to a different target.";
  }
  return "For the current source conversation, reply normally in your final assistant message; ACTAgent will deliver it through the active source conversation.";
}
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is ACTAgent's virtual provider; let Codex app-server keep its
  ...
  if (isCodexAppServerNativeAuthProfile(params) && normalizedLower === "openai") {
    // When ACTAgent is forwarding ChatGPT/Codex OAuth, `openai` is Codex's

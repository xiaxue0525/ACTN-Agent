...
import type { EmbeddedRunAttemptParams } from "actagent/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
  type HarnessContextEngine as ContextEngine,
} from "actagent/plugin-sdk/agent-harness-runtime";
import { SessionManager } from "actagent/plugin-sdk/agent-sessions";
import { registerSandboxBackend } from "actagent/plugin-sdk/sandbox";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/actagent-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-codex-context-engine-"));
  });
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    expectRequestInputTextContains(harness, "ACTAgent assembled context for this turn:");
  ...
  it("projects thread-bootstrap context only once for a matching context-engine epoch", async () => {
    ...
    expectRequestInputTextContains(firstHarness, "ACTAgent assembled context for this turn:");
  ...
  it("resumes a matching thread-bootstrap binding even when the bootstrap turn exceeded the opt-in native byte guard", async () => {
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      contextEngine: {
        ...
        engineId: "lossless-actagent",
        policyFingerprint:
          '{"schemaVersion":1,"engineId":"lossless-actagent","ownsCompaction":true,"projectionMaxChars":24000}',
        projection: {
          schemaVersion: 1,
          mode: "thread_bootstrap",
          epoch: "epoch-1",
  ...
  it("retries a resumed context-engine thread on a fresh Codex thread without plugin compaction", async () => {
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      contextEngine: {
        ...
        engineId: "lossless-actagent",
        policyFingerprint:
          '{"schemaVersion":1,"engineId":"lossless-actagent","ownsCompaction":true,"contextTokenBudget":400000,"projectionMaxChars":1000000}',
        projection: {
          schemaVersion: 1,
          mode: "thread_bootstrap",
          epoch: "epoch-before",
    ...
    expect(savedBinding?.contextEngine?.engineId).toBe("lossless-actagent");
    expect(savedBinding?.contextEngine?.projection).toBeUndefined();
  });
  ...
  it("fails first-turn Codex context overflow instead of falling back to ACTAgent compaction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const compact = vi.fn<ContextEngine["compact"]>(async () => ({
      ok: true,
  ...
  it("keeps current inbound context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentInboundContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] ACTAgent: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 ACTAgent: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("ACTAgent assembled context for this turn:");
    expect(inputText).toContain("Current user request:\nhello");
    expect(inputText).toContain("[reply target] ACTAgent: anchor REPLYCTX");

...
describe("maybeCompactCodexAppServerSession", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-codex-compact-"));
  });
  ...
  it("blocks native app-server compaction when the current ACTAgent session is sandboxed", async () => {
    const fake = createFakeCodexClient();
    setCodexAppServerClientFactoryForTest(async () => fake.client);
    const sessionFile = await writeTestBinding();
    ...
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because ACTAgent sandboxing is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("blocks native app-server compaction when exec host=node is active", async () => {
    ...
    expect(result.compacted).toBe(false);
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because ACTAgent exec host=node is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("does not impose an ACTAgent timeout after Codex accepts native compaction", async () => {
    const fake = createFakeCodexClient();
  ...
  it("warns when stale ACTAgent compaction overrides are ignored", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "ignoring ACTAgent compaction overrides for Codex app-server compaction; Codex uses native server-side compaction",
      {
        sessionId: "session-1",
        sessionKey: "agent:main:session-1",

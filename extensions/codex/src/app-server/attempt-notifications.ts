 ...
 */
import { asBoolean } from "actagent/plugin-sdk/string-coerce-runtime";
import {
...
/** Detects completion for an ACTAgent dynamic tool result still awaited by Codex. */
export function isPendingACTAgentDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingACTAgentDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingACTAgentDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;

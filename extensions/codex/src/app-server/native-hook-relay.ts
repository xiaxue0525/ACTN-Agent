/**
 * Bridges Codex native hook callbacks into ACTAgent's native hook relay so
 * app-server tool events can still run ACTAgent policy and diagnostics.
 */
...
/** Codex hook events that can be registered through ACTAgent's native relay. */
...
/** Registers an ACTAgent native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian ACTAgent plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("actagent:codex:native-hook-relay:v1");
  hash.update("\0");

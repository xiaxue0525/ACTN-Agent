 ...
 * ACTAgent-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "actagent/plugin-sdk/agent-runtime";
import { isToolAllowed } from "actagent/plugin-sdk/sandbox";
...
type ACTAgentCodingToolsOptions = NonNullable<
  Parameters<(typeof import("actagent/plugin-sdk/agent-harness"))["createACTAgentCodingTools"]>[0]
>;

/** Factory seam for constructing ACTAgent runtime tools without eagerly loading agent-harness. */
export type ACTAgentCodingToolsFactory =
  (typeof import("actagent/plugin-sdk/agent-harness"))["createACTAgentCodingTools"];
type ACTAgentDynamicTool = ReturnType<ACTAgentCodingToolsFactory>[number];
type ACTAgentSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export type DynamicToolBuildParams = {
  ...
  sandbox: ACTAgentSandboxContext;
...
let actAgentCodingToolsFactoryForTests: ACTAgentCodingToolsFactory | undefined;

/** Overrides the runtime tool factory for tests that need deterministic tool catalogs. */
export function setACTAgentCodingToolsFactoryForTests(factory: ACTAgentCodingToolsFactory): void {
  actAgentCodingToolsFactoryForTests = factory;
}

/** Clears the test-only runtime tool factory override. */
export function resetACTAgentCodingToolsFactoryForTests(): void {
  actAgentCodingToolsFactoryForTests = undefined;
}

/** Splits sandbox and run session keys so tool calls can bind to both scopes when needed. */
export function resolveACTAgentCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<ACTAgentCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const createACTAgentCodingTools =
    actAgentCodingToolsFactoryForTests ??
    (await import("actagent/plugin-sdk/agent-harness")).createACTAgentCodingTools;
  toolBuildStages.mark("load-agent-harness-tools");
  const sessionKeys = resolveACTAgentCodingToolsSessionKeys(params, input.sandboxSessionKey);
  const allTools = createACTAgentCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as ACTAgentCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-actagent-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow ACTAgent allowlists must fail closed rather than
...
/** Returns true when ACTAgent policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: ACTAgentSandboxContext | undefined,
  ...
  // active ACTAgent sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside ACTAgent sandboxing. */
...
/** Converts ACTAgent sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForACTAgentSandbox(
  sandbox: ACTAgentSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForACTAgentSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForACTAgentSandbox(
  sandbox: ACTAgentSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: ACTAgentDynamicTool[],
  allTools: ACTAgentDynamicTool[],
  ...
  const sandboxExecTool: ACTAgentDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through ACTAgent's configured sandbox backend for this session. Use when ACTAgent sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no ACTAgent sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: ACTAgentDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through ACTAgent's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling only when no ACTAgent sandbox is active and native Code Mode is available.",
  };

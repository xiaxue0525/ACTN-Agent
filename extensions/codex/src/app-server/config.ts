...
} from "actagent/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "actagent/plugin-sdk/number-runtime";
import { normalizeAgentId } from "actagent/plugin-sdk/routing";
import { normalizeTrimmedStringList } from "actagent/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "actagent/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("actagent.codexAppServerStartOptionsKeySecret");
...
type ACTAgentExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type ACTAgentExecSecurity = "deny" | "allowlist" | "full";
type ACTAgentExecAsk = "off" | "on-miss" | "always";
type ACTAgentExecApprovalFloorsForCodexAppServer = {
  security?: ACTAgentExecSecurity;
  ask?: ACTAgentExecAsk;
};
export type ACTAgentExecPolicyForCodexAppServer = {
  mode?: ACTAgentExecMode;
  security: ACTAgentExecSecurity;
  ask: ACTAgentExecAsk;
  touched: boolean;
};
type ACTAgentExecPolicy = ACTAgentExecPolicyForCodexAppServer;
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.ACTAGENT_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.ACTAGENT_CODEX_APP_SERVER_ARGS);
  const headers = normalizeHeaders(config.headers);
  const clearEnv = normalizeStringList(config.clearEnv);
  const authToken = readNonEmptyString(config.authToken);
  const url = readNonEmptyString(config.url);
  const execMode = resolveEffectiveACTAgentExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForACTAgentExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.ACTAGENT_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.ACTAGENT_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForACTAgentExecMode(execMode);
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                actAgentSandboxActive: Boolean(params.actAgentSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.ACTAGENT_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.ACTAGENT_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.ACTAGENT_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.ACTAGENT_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.ACTAGENT_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.ACTAGENT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.ACTAGENT_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.ACTAGENT_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.ACTAGENT_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<ACTAgentExecMode, "auto" | "ask">;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<ACTAgentExecMode, "auto">,
...
function selectForcedDangerFullAccessSandbox(params: {
  ...
  if (params.defaultPolicy?.dangerFullAccessAllowed === false) {
    if (params.actAgentSandboxActive) {
...
export function resolveACTAgentExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): ACTAgentExecMode | undefined {
  const policy = resolveACTAgentExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveACTAgentExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): ACTAgentExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyACTAgentExecPolicyLayer(createDefaultACTAgentExecPolicy(), globalExec);
  ...
  return applyACTAgentExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveACTAgentExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveACTAgentExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveACTAgentExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const basePolicy = resolveACTAgentExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyACTAgentExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveACTAgentExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyACTAgentExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveACTAgentExecModeForCodexAppServer(params: {
  execMode?: ACTAgentExecMode;
  execPolicy?: ACTAgentExecPolicyForCodexAppServer;
}): ACTAgentExecMode | undefined {
...
function resolveCodexPolicyModeForACTAgentExecMode(
  mode: ACTAgentExecMode | undefined,
...
function assertCodexAppServerAllowedForACTAgentExecMode(mode: ACTAgentExecMode | undefined): void {
...
function createDefaultACTAgentExecPolicy(): ACTAgentExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyACTAgentExecPolicyLayer(
  base: ACTAgentExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveACTAgentExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveACTAgentExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveACTAgentExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: ACTAgentExecPolicy;
}): ACTAgentExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyACTAgentExecApprovalFloors(
  base: ACTAgentExecPolicy,
  approvalFloors?: ACTAgentExecApprovalFloorsForCodexAppServer,
): ACTAgentExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minACTAgentExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxACTAgentExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveACTAgentExecPolicyForMode(
  mode: ACTAgentExecMode,
): Omit<ACTAgentExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveACTAgentExecModeFromPolicy(params: {
  security: ACTAgentExecSecurity;
  ask: ACTAgentExecAsk;
}): ACTAgentExecMode {
...
function minACTAgentExecSecurity(
  left: ACTAgentExecSecurity,
  right: ACTAgentExecSecurity,
): ACTAgentExecSecurity {
  const order: Record<ACTAgentExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxACTAgentExecAsk(left: ACTAgentExecAsk, right: ACTAgentExecAsk): ACTAgentExecAsk {
  const order: Record<ACTAgentExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): ACTAgentExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): ACTAgentExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): ACTAgentExecAsk | undefined {

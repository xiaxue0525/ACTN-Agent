...
import { resolveUserPath } from "actagent/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.ACTAGENT_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.ACTAGENT_TRAJECTORY_DIR?.trim();
  if (dirOverride) {

...
import { nativeHookRelayTesting } from "actagent/plugin-sdk/agent-harness-runtime";
import {
...
const createACTAgentCodingToolsMock = vi.fn();
...
vi.mock("actagent/plugin-sdk/agent-harness", () => ({
  createACTAgentCodingTools: (...args: unknown[]) => createACTAgentCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createACTAgentCodingToolsMock.mockReset();
    ...
    createACTAgentCodingToolsMock.mockReturnValue([
      {
        name: "wiki_status",
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createACTAgentCodingToolsMock);
  ...
  it("rejects /btw before forking when the current ACTAgent session is sandboxed", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because ACTAgent sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because ACTAgent exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to ACTAgent tools", async () => {
    const client = createFakeClient();
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createACTAgentCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createACTAgentCodingToolsMock.mockImplementation(() => {

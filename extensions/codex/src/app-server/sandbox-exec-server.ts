/**
 * Hosts the local ACTAgent sandbox exec-server that Codex app-server native
...
import { embeddedAgentLog } from "actagent/plugin-sdk/agent-harness-runtime";
...
const SANDBOX_EXEC_SERVERS = new Map<string, Promise<ACTAgentExecServer>>();
  ...
  await Promise.all(
    servers.map(async (entry) => {
      if (entry.status === "fulfilled") {
        ...
        await closeACTAgentExecServer(entry.value);
      }
    }),
  );
}
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  if (!canExposeLocalExecServerToAppServer(params.appServerStartOptions)) {
    throw new Error(
      "ACTAgent Codex exec-server uses a local loopback URL and cannot be registered with a remote Codex app-server.",
    );
  }
  assertCodexSandboxExecServerSupported(params.client);
  const execServer = await acquireACTAgentExecServer(params.sandbox);
  try {
    ...
    await releaseACTAgentExecServer(execServer);
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    await releaseACTAgentExecServer(server);
  }
}
...
function assertCodexSandboxExecServerSupported(client: CodexAppServerClient): void {
  ...
  if (
    ...
    throw new Error(
      `Codex app-server ${MIN_CODEX_SANDBOX_EXEC_SERVER_APP_SERVER_VERSION} or newer is required for ACTAgent sandbox exec-server environments, but detected ${
...
async function acquireACTAgentExecServer(sandbox: SandboxContext): Promise<ACTAgentExecServer> {
  const key = sandbox.runtimeId;
  while (true) {
    const existing = SANDBOX_EXEC_SERVERS.get(key);
    const promise = existing ?? startAndRememberACTAgentExecServer(sandbox);
...
function startAndRememberACTAgentExecServer(sandbox: SandboxContext): Promise<ACTAgentExecServer> {
  const created = startACTAgentExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startACTAgentExecServer(sandbox: SandboxContext): Promise<ACTAgentExecServer> {
  const server = new WebSocketServer({ host: "127.0.0.1", port: 0 });
  await once(server, "listening");
  const address = server.address();
  if (!address || typeof address === "string") {
    throw new Error("ACTAgent Codex exec-server did not bind to a TCP port.");
  }
  const environmentId = buildEnvironmentId(sandbox);
  const authPath = `/actagent-${randomUUID()}`;
  const url = `ws://127.0.0.1:${(address as AddressInfo).port}${authPath}`;
  const execServer: ACTAgentExecServer = {
    authPath,
    closed: false,
    environmentId,
    refCount: 0,
...
async function releaseACTAgentExecServer(execServer: ACTAgentExecServer): Promise<void> {
  ...
  await closeACTAgentExecServer(execServer);
}

async function closeACTAgentExecServer(execServer: ACTAgentExecServer): Promise<void> {
  if (execServer.closed) {
    return;
  }
...
function handleConnection(execServer: ACTAgentExecServer, socket: WebSocket): void {
...
async function dispatchRequest(
  ...
  switch (request.method) {
    ...
    default:
      throw new Error(`Unsupported ACTAgent sandbox exec-server method: ${request.method}`);
  }
}

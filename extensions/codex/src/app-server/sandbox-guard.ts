/**
 * Blocks direct Codex app-server requests that would bypass ACTAgent sandbox or
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-actagent-environment"],
...
/** Returns a block message when a direct app-server method would bypass ACTAgent execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-actagent-environment" &&
    hasACTAgentSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasACTAgentSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("actagent-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because ACTAgent sandboxing is active for this session.`,
    "This mode cannot route execution through the ACTAgent sandbox backend.",

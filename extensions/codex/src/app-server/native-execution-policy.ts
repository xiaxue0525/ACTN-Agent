 ...
 * or whether ACTAgent must keep exec/process on a configured node host.
 */
import type { ACTAgentConfig } from "actagent/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "actagent/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "actagent/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<ACTAgentConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "ACTAgent exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected ACTAgent node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because ACTAgent exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected ACTAgent node.",
    "Use a normal Codex harness turn so ACTAgent exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: ACTAgentConfig): AgentEntry[] {

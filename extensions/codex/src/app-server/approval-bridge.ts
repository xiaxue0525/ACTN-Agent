/**
 * Bridges Codex app-server approval requests into ACTAgent policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runACTAgentToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    // Native hook policy did not decide; fall back to the ACTAgent approval
...
/** Converts an ACTAgent approval outcome into the app-server method response. */
...
async function runACTAgentToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  ...
  const policyRequest = buildACTAgentToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `ACTAgent native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
    };
  }
...
function readNativeRelayPreToolUseDecision(
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "ACTAgent native hook relay failed for Codex app-server approval.",
    };
  }
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "ACTAgent native hook policy denied Codex app-server approval.",
    };
  }
  ...
  return {
    blocked: true,
    reason: output
      ? "ACTAgent native hook relay returned a non-deny Codex app-server approval decision."
      : "ACTAgent native hook relay returned an unreadable Codex app-server approval result.",
  };
}
...
function buildACTAgentToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
...
function unsupportedApprovalResponse(): JsonValue {
  return {
    decision: "decline",
    reason: "ACTAgent codex app-server bridge does not grant native approvals yet.",
  };
}

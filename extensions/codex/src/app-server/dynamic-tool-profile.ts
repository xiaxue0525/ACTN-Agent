...
/** Tool names owned by Codex app-server and normally excluded from ACTAgent dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  ACTAGENT_BUILD_PRIVATE_QA?: string;
  ACTAGENT_QA_FORCE_RUNTIME?: string;
};

/** Normalizes ACTAgent/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.ACTAGENT_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters ACTAgent tools that Codex owns natively or config explicitly excludes. */

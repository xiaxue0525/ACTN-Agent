...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through thread start unchanged", async () => {
    ...
    expect(startPayload?.serviceName).toBe("ACTAgent");
    ...
    expect(startPayload?.developerInstructions).toContain("ACTAgent");
  });

...
import { loadExecApprovals } from "actagent/plugin-sdk/exec-approvals-runtime";
import { pathExists } from "actagent/plugin-sdk/security-runtime";
...
import {
  ...
  resolveACTAgentCodingToolsSessionKeys,
  resetACTAgentCodingToolsFactoryForTests,
  setACTAgentCodingToolsFactoryForTests,
...
export async function runCodexAppServerAttempt(
  ...
  const configuredAppServer = resolveCodexAppServerRuntimeOptions({
    pluginConfig,
    execPolicy,
    actAgentSandboxActive: sandbox?.enabled === true,
  });
  ...
  const appServer = resolveCodexAppServerForACTAgentToolPolicy({
  ...
  if (configuredAppServer.approvalPolicy === "never" && appServer.approvalPolicy === "untrusted") {
    embeddedAgentLog.info("codex app-server approval policy promoted for ACTAgent tool policy", {
      from: "never",
      to: "untrusted",
  ...
  // ACTAgent transcript is persistence/search state. Context-engine output is
  ...
  const decorateCodexTurnPromptText = (prompt: string) =>
    prependCodexACTAgentPromptContext(prompt, actAgentPromptContext, {
  ...
  const pendingACTAgentDynamicToolCompletionIds = new Set<string>();
  ...
  const releaseTurnAfterTerminalDynamicTool = (paramsValue: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingACTAgentDynamicToolCompletionIdsCount: pendingACTAgentDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const notificationState = applyCodexTurnNotificationState({
      ...
      activeTurnItemIds,
      activeAppServerTurnRequests,
      pendingACTAgentDynamicToolCompletionIds,
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      pendingACTAgentDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        ...
        pendingACTAgentDynamicToolCompletionIds.delete(call.callId);
  ...
  try {
    ...
    if (
      ...
      // Do not try to pre-compact or summarize through ACTAgent here. Codex owns
      // automatic compaction; ACTAgent may only discard a stale projection thread

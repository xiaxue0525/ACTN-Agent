...
import {
  ...
  isPendingACTAgentDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingACTAgentDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingACTAgentDynamicToolCompletionNotification(
    notification,
    params.pendingACTAgentDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after ACTAgent hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served ACTAgent
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingACTAgentDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }

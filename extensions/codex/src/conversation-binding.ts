...
import { resolveSessionAgentIds } from "actagent/plugin-sdk/agent-runtime";
...
import {
  ...
  type ACTAgentExecPolicyForCodexAppServer,
...
const NATIVE_CONVERSATION_INTERACTIVE_APPROVALS_UNAVAILABLE =
  "ACTAgent native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.";
...
async function resolveConversationAppServerRuntime(params: {
  ...
  const runtime = resolveCodexAppServerRuntimeOptions({
    ...
    execPolicy,
    actAgentSandboxActive: Boolean(sandboxForPolicy?.enabled),
  });
  assertNativeConversationApprovalPolicySupported({ execPolicy, runtime });
  return { execPolicy, runtime };
}

const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("actagent.codex.conversationBinding");
    ...
    const response: CodexThreadStartResponse = await resolved.client.request(
      "thread/start",
      {
        ...
        developerInstructions:
          "This Codex thread is bound to an ACTAgent conversation. Answer normally; ACTAgent will deliver your final response back to the conversation.",
      ...
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "ACTAgent native Codex conversation binding does not expose dynamic ACTAgent tools yet.",
            },
          ],
          success: false,
        };
...
function resolveConversationExecPolicy(params: {
  ...
  const agentId =
    ...
    (params.config
      ...
      : undefined);
  return resolveACTAgentExecPolicyForCodexAppServer({
    config: params.config,
    agentId,

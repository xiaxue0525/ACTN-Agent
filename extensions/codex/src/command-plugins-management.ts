...
/**
 * Lightweight read/write surface over the ACTAgent config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to actagent.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in actagent.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
export function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.actagent/actagent.json, never to ~/.codex/config.toml):",
...
export function formatPluginList(
  ...
  return [
    "Codex sub-plugins in ACTAgent config (~/.actagent/actagent.json):",
    "",
    ...rows.map(
      (r) =>

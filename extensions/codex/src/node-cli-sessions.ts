...
export function createCodexCliSessionNodeHostCommands(): ACTAgentPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): ACTAgentPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredACTAgentTmpDir(), "actagent-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [

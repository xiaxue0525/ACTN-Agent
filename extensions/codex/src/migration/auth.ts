...
import { loadAuthProfileStoreWithoutExternalProfiles } from "actagent/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
  markMigrationItemConflict,
  markMigrationItemError,
  markMigrationItemSkipped,
} from "actagent/plugin-sdk/migration";
import type { MigrationItem, MigrationProviderContext } from "actagent/plugin-sdk/plugin-entry";
import {
...
type AgentDefaultModelConfigs = NonNullable<
  NonNullable<NonNullable<ACTAgentConfig["agents"]>["defaults"]>["models"]
>;
...
async function buildCodexOAuthCredential(source: CodexSource): Promise<CodexAuthCredential | null> {
  ...
  } satisfies Partial<ACTAgentConfig>;
...
function replaceConfigDraft(draft: ACTAgentConfig, next: ACTAgentConfig): void {
  for (const key of Object.keys(draft) as Array<keyof ACTAgentConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<ACTAgentConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config = (ctx.runtime?.config?.current?.() as ACTAgentConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: ACTAgentConfig): ACTAgentConfig {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: ACTAgentConfig) => ACTAgentConfig,

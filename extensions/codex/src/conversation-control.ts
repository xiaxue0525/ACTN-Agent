...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("actagent.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string, lookup: CodexAppServerBindingLookup = {}) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this ACTAgent session yet.");
  }
  return binding;
}

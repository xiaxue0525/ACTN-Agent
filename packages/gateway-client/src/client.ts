...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@actagent/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@actagent/gateway-protocol/client-info";
import {
...
} from "@actagent/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@actagent/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@actagent/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for ACTAgent-owned
...
const DEFAULT_GATEWAY_CLIENT_URL = "ws://127.0.0.1:19199";
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside ACTAgent; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).ACTAGENT_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          "(ssh -N -L 19199:127.0.0.1:19199 user@gateway-host), or use Tailscale Serve/Funnel. " +
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `actagent doctor --fix` for guidance.",
      );
  ...
  private isTrustedDeviceRetryEndpoint(): boolean {
    const rawUrl = this.opts.url ?? "ws://127.0.0.1:19199";
    try {

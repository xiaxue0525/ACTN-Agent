// ACTAgent SDK tests cover index behavior.
...
import { GatewayClientTransport, ACTAgent } from "./index.js";
...
describe("ACTAgent SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("runs an agent and streams normalized events over a Gateway websocket", async () => {
    ...
    const oc = new ACTAgent({ transport });
    try {
...
describe("ACTAgent SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.ACTAGENT_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.ACTAGENT_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("ACTAgent SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.ACTAGENT_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: ACTAGENT_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("ACTAGENT_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);

// ACTAgent SDK module implements client behavior.
...
// High-level ACTAgent SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the ACTAgent SDK client. */
export type ACTAgentOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: ACTAgentTransport;
};

function resolveGatewayUrl(options: ACTAgentOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `ACTAgent Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current ACTAgent Gateway yet`);
}
...
function readChatProjection(event: ACTAgentEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: ACTAgentEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: ACTAgentEvent): boolean {
  return (
...
export class ACTAgent {
  ...
  private readonly transport: ACTAgentTransport;
  private readonly normalizedEvents = new EventHub<ACTAgentEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, ACTAgentEvent[]>();
  private connected = false;
  private eventPumpPromise: Promise<void> | null = null;
  private eventPumpReady: Promise<void> | null = null;

  constructor(options: ACTAgentOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: ACTAgentEvent) => boolean): AsyncIterable<ACTAgentEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: ACTAgentEvent) => boolean,
  ): AsyncIterable<ACTAgentEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: ACTAgentEvent): ACTAgentEvent | undefined => {
    ...
    const matches = (event: ACTAgentEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: ACTAgentEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): ACTAgentEvent[] {
...
export class Agent {
  constructor(
    private readonly client: ACTAgent,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: ACTAgent) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<ACTAgentEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: ACTAgent,

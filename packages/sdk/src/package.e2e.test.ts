// ACTAgent SDK tests cover package behavior.
...
const WORKSPACE_PACKAGE_NAMES = [
  "@actagent/gateway-protocol",
  "@actagent/gateway-client",
  "@actagent/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@actagent/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startACTAgentRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("ACTAgent SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@actagent/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startACTAgentRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@actagent:registry=${registry.registryUrl}`);
    try {
      await runCommand(
        "npm",
    ...
    const importScript = `
      import { GatewayClientTransport, ACTAgent, normalizeGatewayEvent } from "@actagent/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof ACTAgent !== "function") throw new Error("missing client export");

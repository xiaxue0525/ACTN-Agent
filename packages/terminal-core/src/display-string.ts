...
/** Resolve the underlying OS home before applying ACTAgent overrides. */
...
/** Resolve raw home with ACTAGENT_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.ACTAGENT_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$ACTAGENT_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$ACTAGENT_HOME" for terminal display. */

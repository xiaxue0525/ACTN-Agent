...
import { resolveChannelTtsVoiceDelivery } from "actagent/plugin-sdk/channel-targets";
import type {
  ACTAgentConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "actagent/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "actagent/plugin-sdk/media-runtime";
import { clampTimerTimeoutMs } from "actagent/plugin-sdk/number-runtime";
import {
...
import { isVerbose, logVerbose } from "actagent/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredACTAgentTmpDir } from "actagent/plugin-sdk/sandbox";
import { privateFileStoreSync } from "actagent/plugin-sdk/security-runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "actagent/plugin-sdk/string-coerce-runtime";
import { stripMarkdown } from "actagent/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "actagent/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.ACTAGENT_TTS_PREFS?.trim();
  if (envPath) {
...
function resolveConfiguredSpeechVoiceModelRefs(cfg: ACTAgentConfig | undefined): VoiceModelRef[] {
...
function sortSpeechProvidersForAutoSelection(cfg?: ACTAgentConfig) {
...
function resolveTtsRuntimeConfig(cfg: ACTAgentConfig): ACTAgentConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: ACTAgentConfig): TtsProvider[] {
  const effectiveCfg = cfg ? resolveTtsRuntimeConfig(cfg) : undefined;

...
export { splitShellArgs } from "./actagent-runtime-io.js";

// Shared ACTAgent config helpers used by memory host, QMD, and agent context code.
...
/** Narrow ACTAgent config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying ACTAgent-specific overrides. */
...
/** Resolve ACTAGENT_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.ACTAGENT_HOME);
...
/** Resolve absolute user paths, including "~" against the effective ACTAgent home. */
...
export function resolveStateDir(
  ...
  if (env.ACTAGENT_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .actagent.
...
/** Resolve the default agent workspace, partitioned by ACTAGENT_PROFILE when set. */
...
function listAgentEntries(cfg: ACTAgentConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: ACTAgentConfig): string {
...
function resolveAgentConfig(cfg: ACTAgentConfig, agentId: string): AgentConfig | undefined {

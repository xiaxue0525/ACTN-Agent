...
import { requireApiKey, resolveApiKeyForProvider } from "./actagent-runtime-auth.js";
...
function resolveACTAgentAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.ACTAGENT_VERSION?.trim() : undefined;
  return {
    originator: "actagent",
    ...(version ? { version } : {}),
    "User-Agent": version ? `actagent/${version}` : "actagent",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveACTAgentAttributionHeaders());
  }

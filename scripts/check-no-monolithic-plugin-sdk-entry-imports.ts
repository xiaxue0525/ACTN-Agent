// Check No Monolithic Plugin Sdk Entry Imports script supports ACTAgent repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverACTAgentPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']actagent\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']actagent\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']actagent\/plugin-sdk\/channel-runtime["']/,
    label: "actagent/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']actagent\/plugin-sdk\/config-runtime["']/,
    label: "actagent/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']actagent\/plugin-sdk\/infra-runtime["']/,
    label: "actagent/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic actagent/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy actagent/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused actagent/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }

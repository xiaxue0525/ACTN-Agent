...
elif [[ -n "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(actagent_live_collect_auth_dirs_from_csv "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(actagent_live_collect_auth_files_from_csv "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=()
actagent_live_init_docker_run_args DOCKER_RUN_ARGS "${ACTAGENT_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ACTAGENT_LIVE_REQUIRE_PROFILE_KEYS="${ACTAGENT_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_MODELS="${ACTAGENT_LIVE_GATEWAY_MODELS:-modern}" \
  -e ACTAGENT_LIVE_GATEWAY_PROVIDERS="${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_THINKING="${ACTAGENT_LIVE_GATEWAY_THINKING:-}" \
  -e ACTAGENT_LIVE_GATEWAY_SMOKE="${ACTAGENT_LIVE_GATEWAY_SMOKE:-1}" \
  -e ACTAGENT_LIVE_GATEWAY_MAX_MODELS="${ACTAGENT_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e ACTAGENT_LIVE_GATEWAY_HEARTBEAT_MS="${ACTAGENT_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS="${ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \

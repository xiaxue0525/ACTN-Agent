// Ios Version script supports ACTAgent repository automation.
...
if (options.format === "shell") {
  process.stdout.write(
    [
      `ACTAGENT_IOS_VERSION=${version.canonicalVersion}`,
      `ACTAGENT_MARKETING_VERSION=${version.marketingVersion}`,
      `ACTAGENT_BUILD_VERSION=${version.buildVersion}`,
    ].join("\n") + "\n",
  );
} else {

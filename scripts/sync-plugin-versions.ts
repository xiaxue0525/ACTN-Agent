// Sync Plugin Versions script supports ACTAgent repository automation.
...
const ACTAGENT_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncACTAgentDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.actagent;
  if (!current || current === "workspace:*" || !ACTAGENT_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.actagent?.compat;
  const current = compat?.pluginApi;
  if (!current || !ACTAGENT_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildACTAgentVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.actagent?.build;
  const current = build?.actagentVersion;
  if (!current) {
    return false;
  }
  ...
  build.actagentVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncACTAgentDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncACTAgentDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildACTAgentVersionChanged = syncBuildACTAgentVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildACTAgentVersionChanged;
    if (!packageChanged) {

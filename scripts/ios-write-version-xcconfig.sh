...
- ACTAGENT_IOS_VERSION = exact canonical iOS version
- ACTAGENT_MARKETING_VERSION = short iOS/App Store version
- ACTAGENT_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    ACTAGENT_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    ACTAGENT_MARKETING_VERSION)
...
ACTAGENT_IOS_VERSION = ${IOS_VERSION}
ACTAGENT_MARKETING_VERSION = ${MARKETING_VERSION}
ACTAGENT_BUILD_VERSION = ${BUILD_NUMBER}
EOF

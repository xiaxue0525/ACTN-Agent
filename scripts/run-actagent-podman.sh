#!/usr/bin/env bash
# Rootless ACTAgent in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-actagent-podman.sh launch        # Start gateway
#   ./scripts/run-actagent-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   actagent --container actagent dashboard --no-open
#   actagent --container actagent channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ACTAGENT_GATEWAY_TOKEN|ACTAGENT_PODMAN_CONTAINER|ACTAGENT_PODMAN_IMAGE|ACTAGENT_IMAGE|ACTAGENT_PODMAN_PULL|ACTAGENT_PODMAN_RUN_TIMEOUT|ACTAGENT_PODMAN_GATEWAY_HOST_PORT|ACTAGENT_GATEWAY_PORT|ACTAGENT_PODMAN_BRIDGE_HOST_PORT|ACTAGENT_BRIDGE_PORT|ACTAGENT_GATEWAY_BIND|ACTAGENT_PODMAN_USERNS|ACTAGENT_BIND_MOUNT_OPTIONS|ACTAGENT_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-actagent-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ACTAGENT_CONFIG_DIR:-$EFFECTIVE_HOME/.actagent}"
ENV_FILE="${ACTAGENT_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ACTAGENT_CONFIG_DIR:-$EFFECTIVE_HOME/.actagent}"
ENV_FILE="${ACTAGENT_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ACTAGENT_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ACTAGENT_PODMAN_CONTAINER:-actagent}"
ACTAGENT_IMAGE="${ACTAGENT_PODMAN_IMAGE:-${ACTAGENT_IMAGE:-actagent:local}}"
PODMAN_PULL="${ACTAGENT_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${ACTAGENT_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${ACTAGENT_PODMAN_GATEWAY_HOST_PORT:-${ACTAGENT_GATEWAY_PORT:-19199}}"
HOST_BRIDGE_PORT="${ACTAGENT_PODMAN_BRIDGE_HOST_PORT:-${ACTAGENT_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ACTAGENT_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ACTAGENT_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v actagent >/dev/null 2>&1; then
    return 0
  fi
  ACTAGENT_CONTAINER="" ACTAGENT_CONFIG_DIR="$config_dir" \
    actagent config get gateway.bind 2>/dev/null || true
}
...
# ACTAGENT_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ACTAGENT_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'ACTAGENT_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v actagent >/dev/null 2>&1; then
    echo "Warning: actagent not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ACTAGENT_CONTAINER="" ACTAGENT_CONFIG_DIR="$config_dir" \
      actagent config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ACTAGENT_CONTAINER="" ACTAGENT_CONFIG_DIR="$config_dir" \
      actagent config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ACTAGENT_CONTAINER="" ACTAGENT_CONFIG_DIR="$config_dir" \
    actagent config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ACTAGENT_GATEWAY_TOKEN:-}" ]]; then
  export ACTAGENT_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ACTAGENT_GATEWAY_TOKEN" "$ACTAGENT_GATEWAY_TOKEN"
  echo "Generated ACTAGENT_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/actagent.json"
...
PODMAN_USERNS="${ACTAGENT_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ACTAGENT_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ACTAGENT_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ACTAGENT_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ACTAGENT_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ACTAGENT_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.actagent/.npm \
    -e ACTAGENT_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.actagent:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.actagent/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ACTAGENT_IMAGE" \
...
run_podman_detached --pull="$PODMAN_PULL" -d --replace \
  ...
  -v "$CONFIG_DIR:/home/node/.actagent:rw${SELINUX_MOUNT_OPTS}" \
  -v "$WORKSPACE_DIR:/home/node/.actagent/workspace:rw${SELINUX_MOUNT_OPTS}" \
  -p "${PUBLISH_HOST}:${HOST_GATEWAY_PORT}:19199" \
  -p "${PUBLISH_HOST}:${HOST_BRIDGE_PORT}:18790" \
  "$ACTAGENT_IMAGE" \
  node dist/index.js gateway --bind "$GATEWAY_BIND" --port 19199 >/dev/null

echo "Container $CONTAINER_NAME started: http://127.0.0.1:${HOST_GATEWAY_PORT}/"
echo "podman exec -it $CONTAINER_NAME actagent dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME actagent devices approve --latest  # if pairing required"

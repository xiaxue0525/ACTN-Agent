// Debug Claude Usage script supports ACTAgent repository automation.
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.ACTAGENT_STATE_DIR?.trim() || path.join(os.homedir(), ".actagent");
...
const resolveFetchTimeoutMs = (raw = process.env.ACTAGENT_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "ACTAGENT_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};

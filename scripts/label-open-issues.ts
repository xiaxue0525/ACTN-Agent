// Label Open Issues script supports ACTAgent repository automation.
...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "actagent", STATE_FILE_NAME);
...
function resolveOpenAITimeoutMs(raw = process.env.ACTAGENT_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_TIMEOUT_MS,
    label: "ACTAGENT_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
          ...
          input: [
            {
              role: "system",
              content:
                "You classify GitHub issues and pull requests for ACTAgent. Respond with JSON only, no extra text.",
            },
            {
              role: "user",
              content: [
...
async function main() {
  ...
  logHeader("ACTAgent Issue Label Audit");

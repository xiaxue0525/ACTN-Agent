...
if [[ "${ACTAGENT_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${ACTAGENT_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
if [[ "${ACTAGENT_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
if [[ "${mode}" == "--pack-dry-run" ]]; then
  ACTAGENT_PLUGIN_NPM_BUNDLE_DEPENDENCIES=1 \

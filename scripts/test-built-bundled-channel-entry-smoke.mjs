...
process.env.ACTAGENT_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "ACTAGENT_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "ACTAGENT_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/actagent");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "actagent");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.actagent?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.actagent.extensions) && packageJson.actagent.extensions.length > 0
    ...
    const setupEntry = packageJson.actagent.setupEntry;

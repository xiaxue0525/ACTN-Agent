...
function resolveACTAgentLaunchArgs(repoRoot, sourceEntryExists = existsSync) {
  const sourceEntry = path.join(repoRoot, "src", "entry.ts");
  if (sourceEntryExists(sourceEntry)) {
    return ["--import", "tsx", sourceEntry];
  }
  return [path.join(repoRoot, "actagent.mjs")];
}
...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveACTAgentLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          ACTAGENT_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.ACTAGENT_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromACTAgent = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromACTAgent("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "actagent-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `actagent-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "actagent-rtt-rpc",
        role: "operator",

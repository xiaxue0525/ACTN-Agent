...
ZIP=${1:?"Usage: $0 ACTAgent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/actagent/actagent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^ACTAgent-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/actagent/actagent/releases/download/v${VERSION}/"}

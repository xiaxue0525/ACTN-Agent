 ...
 * `actagent update`. Use it to verify the completion installation flow
...
\`actagent update\`, \`actagent doctor\`, and \`actagent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ACTAgent Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`actagent completion --install\` later to enable.`));
    return;
  }

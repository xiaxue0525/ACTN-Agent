#!/usr/bin/env node
// Builds the ACTAgent package artifact used by Docker E2E.
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building ACTAgent package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      env: {
        ...
        ACTAGENT_RUN_NODE_SKIP_DTS_BUILD: "1",
      },
      timeoutMs: resolveTimeoutMs(
        "ACTAGENT_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestACTAgentTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed ACTAgent tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

export async function packACTAgentPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing ACTAgent package");
  ...
  try {
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "ACTAGENT_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestACTAgentTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing ACTAgent package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "ACTAGENT_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packACTAgentPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking ACTAgent package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-actagent-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "ACTAGENT_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",

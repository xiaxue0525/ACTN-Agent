#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports ACTAgent repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const actagentMeta =
    ...
    "actagent" in source.packageJson
      ? (source.packageJson.actagent as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    actagentMeta &&
    typeof actagentMeta.channel === "object" &&
    actagentMeta.channel &&
    !Array.isArray(actagentMeta.channel)
      ? (actagentMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}

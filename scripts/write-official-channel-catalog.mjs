...
function toCatalogInstall(value, packageName) {
  ...
  const actagenthubSpec = trimString(install.actagenthubSpec);
  ...
  return {
    ...
    ...(npmSpec ? { npmSpec } : {}),
    ...(defaultChoice === "actagenthub" || defaultChoice === "npm" || defaultChoice === "local"
      ? { defaultChoice }
      : {}),
...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.actagent) ? packageJson.actagent : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.actagent?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.actagent?.channel?.id) || left.name;
    const rightId = trimString(right.actagent?.channel?.id) || right.name;

...
const TMUX_CWD_ENV_KEY = "ACTAGENT_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@actagent.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "ACTAGENT_CONFIG_PATH",
  "ACTAGENT_DIAGNOSTICS",
  "ACTAGENT_DIAGNOSTICS_EVENT_LOOP",
  "ACTAGENT_DIAGNOSTICS_TIMELINE_PATH",
  "ACTAGENT_GATEWAY_PORT",
  "ACTAGENT_GATEWAY_RESTART_TRACE",
  "ACTAGENT_GATEWAY_STARTUP_TRACE",
  "ACTAGENT_HOME",
  "ACTAGENT_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.ACTAGENT_TRACE_SYNC_IO ??= "0";
    if (nextEnv.ACTAGENT_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.ACTAGENT_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.ACTAGENT_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "actagent",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `ACTAGENT_GATEWAY_WATCH_TMUX_CHILD=1`,
    `ACTAGENT_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.ACTAGENT_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.ACTAGENT_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.ACTAGENT_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,

...
const (
	...
	envDocsI18nPromptTimeout    = "ACTAGENT_DOCS_I18N_PROMPT_TIMEOUT"
	envDocsI18nCommandWaitDelay = "ACTAGENT_DOCS_I18N_COMMAND_WAIT_DELAY"
	envDocsI18nCodexExecutable  = "ACTAGENT_DOCS_I18N_CODEX_EXECUTABLE"
)
...
func runCodexExecPrompt(ctx context.Context, req codexPromptRequest) (string, error) {
	outputFile, err := os.CreateTemp("", "actagent-docs-i18n-codex-*.txt")
	if err != nil {
		return "", err
	}
...
func isolatedCodexHomeBase() (string, error) {
	...
	base := filepath.Join(cacheDir, "actagent-docs-i18n")

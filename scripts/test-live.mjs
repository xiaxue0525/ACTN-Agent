...
export function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with ACTAGENT_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    ACTAGENT_LIVE_TEST: baseEnv.ACTAGENT_LIVE_TEST || "1",
    ACTAGENT_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.ACTAGENT_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { ACTAGENT_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }

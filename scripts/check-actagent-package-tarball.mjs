...
function usage() {
  return "Usage: node scripts/check-actagent-package-tarball.mjs <actagent.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`ACTAgent package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.ACTAGENT_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-actagent-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "actagent") {
      errors.push("npm-shrinkwrap.json root name must be actagent");
    }
    ...
    if (!rootPackage || rootPackage.name !== "actagent") {
      errors.push("npm-shrinkwrap.json packages root must name actagent");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`ACTAgent package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`ACTAgent package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("ACTAgent package tarball integrity passed.");

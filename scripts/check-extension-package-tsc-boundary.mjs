...
export function resolveCompileConcurrency(
  ...
  const raw = env.ACTAGENT_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "ACTAGENT_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.ACTAGENT_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";

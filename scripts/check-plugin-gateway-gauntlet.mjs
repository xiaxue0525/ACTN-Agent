...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("ACTAGENT_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("ACTAGENT_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    keepRunRoot: process.env.ACTAGENT_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.ACTAGENT_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function actagentCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceActagentCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    ACTAGENT_LOG_DIR: path.join(runRoot, "logs"),
    ACTAGENT_QA_SUITE_PROGRESS: process.env.ACTAGENT_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...actagentCommand(params.repoRoot, [command, "--help"]),
    ...
    for (const { phase, args, alias } of commands) {
      ...
      params.rows.push(
        await runMeasuredCommand({
          ...
          ...actagentCommand(params.repoRoot, ["plugins", ...args]),
          label: `${plugin.id}-${phase}`,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceActagentCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-plugin-gauntlet-"));

// Write Cli Startup Metadata script supports ACTAgent repository automation.
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.actagent?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.actagent?.channel?.order;
      const labelRaw = parsed.actagent?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".actagent-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "actagent-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "actagent-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "actagent", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["actagent.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      ACTAGENT_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",

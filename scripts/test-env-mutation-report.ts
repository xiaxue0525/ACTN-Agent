#!/usr/bin/env node
// Test Env Mutation Report script supports ACTAgent repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/actagent-test-state.ts", "canonical ACTAgent test state helper"],
  ...
  ["test/setup-actagent-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "ACTAgent test env mutation report",

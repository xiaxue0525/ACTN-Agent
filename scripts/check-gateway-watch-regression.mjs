...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ACTAGENT_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[actagent] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".actagent");
  const isolatedConfigPath = path.join(isolatedStateDir, "actagent.json");
  const shellSource = [
    'echo "$$" > "$ACTAGENT_WATCH_PID_FILE"',
    'mkdir -p "$ACTAGENT_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ACTAGENT_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ACTAGENT_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ACTAGENT_HOME: isolatedHomeDir,
    ACTAGENT_CONFIG_PATH: isolatedConfigPath,
    ACTAGENT_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-gateway-watch-"));

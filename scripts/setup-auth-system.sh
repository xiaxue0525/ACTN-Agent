#!/bin/bash
# Setup ACTAgent Auth Management System
...
echo "=== ACTAgent Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'actagent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ACTAgent message: Send warning via ACTAgent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/actagent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/actagent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/actagent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now actagent-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/actagentdAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/actagentdAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/actagentd*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (actagentdAuth) shows status and opens auth URL if needed."
echo "The full widget (actagentdAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status actagent-auth-monitor.timer"
echo ""
echo "Quick commands:"

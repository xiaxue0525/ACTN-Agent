...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.ACTAGENT_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[actagent] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[actagent] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[actagent] This usually means a file in node_modules is empty or truncated.");
  console.error("[actagent] Recommended recovery:");
  console.error("[actagent]   rm -rf node_modules");
  console.error("[actagent]   pnpm store prune");
  console.error("[actagent]   pnpm install");
  console.error("");
  console.error("[actagent] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ACTAGENT_WATCH_MODE = "1";
  childEnv.ACTAGENT_WATCH_SESSION = watchSession;
  ...
  childEnv.ACTAGENT_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.ACTAGENT_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `actagent doctor --fix --non-interactive` once.",
        deps,
      );

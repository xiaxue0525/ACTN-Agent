#!/usr/bin/env bash
# Reset ACTAgent like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${ACTAGENT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ACTAgent.app/Contents/MacOS/ACTAgent"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ACTAgent"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ACTAgent"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ACTAgent"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.actagent.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/actagent-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ACTAGENT_RESTART_LOG:-${TMPDIR:-/tmp}/actagent-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ACTAGENT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.actagent/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ACTAGENT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node actagent.mjs daemon install --force --runtime node"
      log "  node actagent.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.actagent/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.actagent.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ACTAgent instances"
kill_all_actagent
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ACTAgent"
...
choose_app_bundle() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    ...
    fail "ACTAGENT_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi

  if [[ -d "${ROOT_DIR}/dist/ACTAgent.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ACTAgent.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ACTAgent.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/ACTAgent.app" ]]; then
    APP_BUNDLE="/Applications/ACTAgent.app"
    return 0
  fi

  fail "App bundle not found. Set ACTAGENT_APP_BUNDLE to your installed ACTAgent.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node actagent.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node actagent.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".actagent", "actagent.json"), "utf8");
        const cfg = JSON.parse(raw);
        const port = cfg && cfg.gateway && typeof cfg.gateway.port === "number" ? cfg.gateway.port : 19199;
        process.stdout.write(String(port));
      } catch {
        process.stdout.write("19199");
      }
    '
  )"
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: ACTAgent is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.actagent.gateway.plist' | head -n 40 || true"
fi

...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "actagentlog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
actagentlog - ACTAgent Logging Utility

USAGE:
    actagentlog [OPTIONS]

DESCRIPTION:
    View ACTAgent logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    ACTAgent logs flow through the macOS unified log (subsystem: ai.actagent).
...
QUICK START:
    actagentlog -n 100             Show last 100 lines from all components
    actagentlog -f                 Follow logs in real-time
    actagentlog -e                 Show only errors
    actagentlog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    actagentlog                   Show last 50 lines from past 5 minutes (default)
    actagentlog -f                Stream logs continuously
    actagentlog -n 100            Show last 100 lines
    actagentlog -e                Show only recent errors
    actagentlog -l 30m -n 200     Show last 200 lines from past 30 minutes
    actagentlog -c ServerManager  Show recent ServerManager logs
    actagentlog -s "fail"         Search for "fail" in recent logs
    actagentlog --server -e       Show recent server errors
    actagentlog -f -d             Stream debug logs continuously

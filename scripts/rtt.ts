...
// Rtt script supports ACTAgent repository automation.
...
function usage() {
  return [
    "Usage: pnpm rtt <actagent@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--credential-source env|convex] [--credential-role maintainer|ci] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt actagent@main --package-tgz .artifacts/package/actagent.tgz",
    "  pnpm rtt actagent@beta",
    "  pnpm rtt actagent@2026.4.30",
    "  pnpm rtt actagent@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  let harnessRoot = "~/Developer/actagentdbot";
  let output = "runs";
  ...
  return {
    spec: validateACTAgentPackageSpec(spec),
    options: {
      packageTgz,
      credentialRole,
      credentialSource,
...
async function main() {
  ...
  if (spec === "actagent@main" && !options.packageTgz) {
    throw new Error("actagent@main requires --package-tgz.");
  }
  const version =
    spec === "actagent@main"

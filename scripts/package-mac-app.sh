...
# Build and bundle ACTAgent into a minimal .app we can open.
# Outputs to dist/ACTAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/lib/plistbuddy.sh"
APP_ROOT="$ROOT_DIR/dist/ACTAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ACTAgent"
MLX_TTS_HELPER_PRODUCT="actagent-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.actagent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/actagent/actagent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ACTAgent/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" ACTAgentBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" ACTAgentGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ACTAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ACTAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ACTAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ACTAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ACTAgent/Resources/ACTAgent.icns" "$APP_ROOT/Contents/Resources/ACTAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ACTAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ACTAgentKit resources"
ACTAGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ACTAgentKit_ACTAgentKit.bundle"
if [ -d "$ACTAGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ACTAgentKit_ACTAgentKit.bundle"
  cp -R "$ACTAGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ACTAgentKit_ACTAgentKit.bundle"
else
  echo "ERROR: ACTAgentKit resource bundle not found at $ACTAGENTKIT_BUNDLE" >&2
  exit 1
fi
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/ACTAgent"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged ACTAgent bundle (${pids[*]})"

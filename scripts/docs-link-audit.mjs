...
import { resolveACTAgentHubRepoPath, syncACTAgentHubDocsTree } from "./docs-sync-publish.mjs";
...
function buildAuditIndex(docsDir = DOCS_DIR, options = {}) {
  ...
  if (options.allowExternalACTAgentHubRoutes === true) {
    for (const page of collectNavPageEntries(docsConfig.navigation || [])) {
      ...
      if (route === "/actagenthub" || route.startsWith("/actagenthub/")) {
...
/**
 * Prepares a docs directory, mirroring ACTAgentHub docs when available.
 *
 * @param {string} [sourceDir]
 * @param {{
 *   resolveACTAgentHubRepoPathImpl?: typeof resolveACTAgentHubRepoPath;
 *   syncACTAgentHubDocsTreeImpl?: typeof syncACTAgentHubDocsTree;
 * }} [options]
 */
export function prepareMirroredDocsDir(sourceDir = DOCS_DIR, options = {}) {
  const sourceRoot = path.resolve(sourceDir);
  if (sourceRoot !== path.resolve(DOCS_DIR)) {
    return { dir: sourceRoot, mirroredACTAgentHub: false, cleanup: () => {} };
  }

  const resolveACTAgentHubRepoPathImpl = options.resolveACTAgentHubRepoPathImpl ?? resolveACTAgentHubRepoPath;
  const syncACTAgentHubDocsTreeImpl = options.syncACTAgentHubDocsTreeImpl ?? syncACTAgentHubDocsTree;
  const actagenthubRepo = resolveACTAgentHubRepoPathImpl("", { required: false });
  if (!actagenthubRepo) {
    return { dir: sourceRoot, mirroredACTAgentHub: false, cleanup: () => {} };
  }

  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-docs-link-audit-"));
  try {
    fs.cpSync(sourceRoot, tempDir, { recursive: true });
    syncACTAgentHubDocsTreeImpl(tempDir, { repoPath: actagenthubRepo, required: false });
    return {
      dir: tempDir,
      mirroredACTAgentHub: true,
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-docs-anchor-audit-"));
  try {
...
export function auditDocsLinks(options = {}) {
  ...
  const index = buildAuditIndex(docsDir, {
    allowExternalACTAgentHubRoutes: options.allowExternalACTAgentHubRoutes === true,
  });
...
export function runDocsLinkAuditCli(options = {}) {
  ...
  try {
    const { checked, broken } = auditDocsLinks({
      ...
      allowExternalACTAgentHubRoutes: !mirroredDocsDir.mirroredACTAgentHub,
    });

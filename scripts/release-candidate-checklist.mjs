...
ACTAgent Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful ACTAgent NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish ACTAgent with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.ACTAGENT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error("ACTAGENT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive number");
  }
...
export function buildPublishCommand(options) {
  const fields = [
    ...
    ["publish_actagent_npm", "true"],
    ["release_profile", "from-validation"],
    ["wait_for_actagenthub", "false"],
  ];
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `actagent@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "actagent-npm-release.yml";
    const before = await beforeRunIds(options.repo, workflowFile);
    const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
      tag: options.tag,
      preflight_only: "true",
    ...
    options.npmPreflightRunId =
      dispatchedRunId ||
      (await findNewRunId(options.repo, workflowFile, "ACTAgent NPM Release", before));
  }
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "ACTAgent NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    options.repo,
    options.npmPreflightRunId,
    `actagent-npm-preflight-${options.tag}`,
    "actagent-npm-preflight-",
    npmDir,
  );
  ...
  const pluginACTAgentHubPlan = await collectPluginPlanWithRetry(
  ...
  writeFileSync(
    ...
    [
      ...
      `- ACTAgentHub plan: ${pluginACTAgentHubPlan.packages?.length ?? 0} packages`,

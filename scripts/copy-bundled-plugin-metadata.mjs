...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.ACTAGENT_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "actagent.plugin.json");
    ...
    if (packageJson.actagent && "extensions" in packageJson.actagent) {
      packageJson.actagent = {
        ...packageJson.actagent,
        extensions: rewritePackageExtensions(packageJson.actagent.extensions),
        ...(typeof packageJson.actagent.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.actagent.setupEntry) }
          : {}),
      };
    }

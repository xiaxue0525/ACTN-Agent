...
DEFAULT_PROVIDER="${ACTAGENT_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${ACTAGENT_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${ACTAGENT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${ACTAGENT_LIVE_CLI_BACKEND_AUTH:-auto}"
CLI_SETUP_TIMEOUT_SECONDS="${ACTAGENT_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}"
TEMP_DIRS=()
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: ACTAGENT_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: ACTAGENT_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
export ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export ACTAGENT_LIVE_CLI_BACKEND_RESUME_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ -n "${ACTAGENT_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(actagent_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(actagent_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
...
run_setup_command() {
  local timeout_value="${ACTAGENT_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s"
  local timeout_bin=""
...
provider="${ACTAGENT_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${ACTAGENT_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${ACTAGENT_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${ACTAGENT_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export ACTAGENT_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ -n "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${ACTAGENT_LIVE_CLI_BACKEND_COMMAND}"
  "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND}" -V || "${ACTAGENT_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${ACTAGENT_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="ACTAGENT-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
DOCKER_AUTH_ENV=(
  -e ACTAGENT_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV="$ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e ACTAGENT_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV="${ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi

DOCKER_RUN_ARGS=()
actagent_live_init_docker_run_args DOCKER_RUN_ARGS "${ACTAGENT_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ACTAGENT_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e ACTAGENT_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e ACTAGENT_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e ACTAGENT_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e ACTAGENT_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e ACTAGENT_LIVE_TEST=1 \
  -e ACTAGENT_LIVE_CLI_BACKEND=1 \
  -e ACTAGENT_LIVE_CLI_BACKEND_DEBUG="${ACTAGENT_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e ACTAGENT_CLI_BACKEND_LOG_OUTPUT="${ACTAGENT_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e ACTAGENT_TEST_CONSOLE="${ACTAGENT_TEST_CONSOLE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e ACTAGENT_LIVE_CLI_BACKEND_COMMAND="${ACTAGENT_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_ARGS="${ACTAGENT_LIVE_CLI_BACKEND_ARGS:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS="${ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_CLEAR_ENV="${ACTAGENT_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e ACTAGENT_LIVE_CLI_BACKEND_RESUME_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${ACTAGENT_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_IMAGE_ARG="${ACTAGENT_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e ACTAGENT_LIVE_CLI_BACKEND_IMAGE_MODE="${ACTAGENT_LIVE_CLI_BACKEND_IMAGE_MODE:-}")

...
const PLUGIN_SPEC =
  process.env.ACTAGENT_KITCHEN_SINK_NPM_SPEC || "npm:@actagent/kitchen-sink@latest";
const PLUGIN_ID = process.env.ACTAGENT_KITCHEN_SINK_PLUGIN_ID || "actagent-kitchen-sink-fixture";
...
const READY_TIMEOUT_MS = readPositiveInt(process.env.ACTAGENT_KITCHEN_SINK_RPC_READY_MS, 240000);
const COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.ACTAGENT_KITCHEN_SINK_RPC_COMMAND_MS,
  180000,
);
const INSTALL_TIMEOUT_MS = readPositiveInt(
  process.env.ACTAGENT_KITCHEN_SINK_RPC_INSTALL_MS,
  Math.max(COMMAND_TIMEOUT_MS, 600000),
);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.ACTAGENT_KITCHEN_SINK_RPC_CALL_MS, 60000);
const FETCH_TIMEOUT_MS = readPositiveInt(process.env.ACTAGENT_KITCHEN_SINK_RPC_FETCH_MS, 10000);
const FETCH_BODY_MAX_BYTES = readPositiveInt(
  process.env.ACTAGENT_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
  1024 * 1024,
);
const MAX_RSS_MIB = readPositiveInt(process.env.ACTAGENT_KITCHEN_SINK_MAX_RSS_MIB, 2048);
const MAX_COMMAND_RSS_MIB = readPositiveInt(
  process.env.ACTAGENT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
  8192,
);
const GATEWAY_TEARDOWN_GRACE_MS = 10000;
const GATEWAY_TEARDOWN_KILL_GRACE_MS = 2000;
const OUTPUT_CAPTURE_CHARS = readPositiveInt(
  process.env.ACTAGENT_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
  1024 * 1024,
);
...
Runs the external Kitchen Sink plugin RPC walk against a built ACTAgent entry.

Environment:
  ACTAGENT_ENTRY                         Built ACTAgent entrypoint. Defaults to dist/index.mjs or dist/index.js.
  ACTAGENT_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@actagent/kitchen-sink@latest.
  ACTAGENT_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: actagent-kitchen-sink-fixture.
  ACTAGENT_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  ACTAGENT_KITCHEN_SINK_RPC_COMMAND_MS   ACTAgent command timeout.
  ACTAGENT_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  ACTAGENT_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  ACTAGENT_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  ACTAGENT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  ACTAGENT_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      ACTAGENT_KITCHEN_SINK_PERSONALITY:
        process.env.ACTAGENT_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runACTAgent(runner, args, env, options = {}) {
  const command = await resolveACTAgentCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveACTAgentCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.ACTAGENT_CONFIG_PATH),
        configPath: options.env.ACTAGENT_CONFIG_PATH,
...
async function loadCallGatewayModule(runner) {
  if (!usesBuiltACTAgentEntry(runner)) {
    return null;
  }
...
async function rpcCallViaCli(method, params, options) {
  const { stdout } = await runACTAgent(
    options.runner,
    [
      "gateway",
      "call",
...
export function usesBuiltACTAgentEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.ACTAGENT_ENTRY && entry === env.ACTAGENT_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.ACTAGENT_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.ACTAGENT_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("actagent-gateway"),
    );
...
export async function main() {
  let runner = resolveACTAgentRunner();
  const port = readPositiveInt(process.env.ACTAGENT_KITCHEN_SINK_RPC_PORT, DEFAULT_PORT);
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.ACTAGENT_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runACTAgent(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,

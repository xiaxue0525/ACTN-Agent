...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${ACTAGENT_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${ACTAGENT_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${ACTAGENT_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${ACTAGENT_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "ACTAGENT_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (ACTAGENT_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set ACTAGENT_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/actagent-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack ACTAgent tarball"
...
' "$pack_json_file" "$PACK_DIR"
  ...
  if [ -z "$PACKAGE_TGZ" ] || [ ! -f "$PACKAGE_TGZ" ]; then
    echo "missing packed ACTAgent tarball" >&2
    exit 1
  fi
}
...
main() {
  ...
  echo "==> Bun global install packed ACTAgent"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  actagent_bin="$BUN_INSTALL/bin/actagent"
  if [ ! -x "$actagent_bin" ]; then
    actagent_bin="$(command -v actagent || true)"
  fi
  if [ -z "$actagent_bin" ] || [ ! -x "$actagent_bin" ]; then
    echo "Bun global install did not create an executable actagent binary" >&2
    exit 1
  fi

  echo "==> ACTAgent version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$actagent_bin" --version

  echo "==> ACTAgent image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$actagent_bin" infer image providers --json)"
  ACTAGENT_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}

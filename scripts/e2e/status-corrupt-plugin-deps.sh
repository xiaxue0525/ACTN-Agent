...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/actagent-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.ACTAGENT_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".actagent-rename-tmp");
...
run_actagent() {
  HOME="$HOME_DIR" \
  ACTAGENT_HOME="$STATE_DIR" \
  ACTAGENT_STATE_DIR="$STATE_DIR" \
  ACTAGENT_CONFIG_PATH="$CONFIG_PATH" \
  ACTAGENT_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_actagent status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run actagent doctor --fix" "$BEFORE" >/dev/null

run_actagent doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_actagent status --all --timeout 1 > "$AFTER"

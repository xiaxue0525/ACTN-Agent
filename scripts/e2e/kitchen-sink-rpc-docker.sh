...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-kitchen-sink-rpc-e2e" ACTAGENT_KITCHEN_SINK_RPC_E2E_IMAGE)"
MAX_MEMORY_MIB="${ACTAGENT_KITCHEN_SINK_MAX_MEMORY_MIB:-2048}"
MAX_CPU_PERCENT="${ACTAGENT_KITCHEN_SINK_MAX_CPU_PERCENT:-1200}"
DOCKER_RUN_TIMEOUT="${ACTAGENT_KITCHEN_SINK_RPC_DOCKER_RUN_TIMEOUT:-900s}"
CONTAINER_NAME="actagent-kitchen-sink-rpc-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/actagent-kitchen-sink-rpc.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/actagent-kitchen-sink-rpc-stats.XXXXXX")"
...
DOCKER_ENV_ARGS=(
  ...
  -e ACTAGENT_ENTRY=/app/actagent.mjs
)

for env_name in \
  ACTAGENT_KITCHEN_SINK_NPM_SPEC \
  ACTAGENT_KITCHEN_SINK_PLUGIN_ID \
  ACTAGENT_KITCHEN_SINK_PERSONALITY \
  ACTAGENT_KITCHEN_SINK_RPC_READY_MS \
  ACTAGENT_KITCHEN_SINK_RPC_COMMAND_MS \
  ACTAGENT_KITCHEN_SINK_RPC_INSTALL_MS \
  ACTAGENT_KITCHEN_SINK_RPC_CALL_MS \
  ACTAGENT_KITCHEN_SINK_MAX_RSS_MIB \
  ACTAGENT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB; do
  env_value="${!env_name:-}"

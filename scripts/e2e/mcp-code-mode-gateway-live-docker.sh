...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-mcp-code-mode-gateway-live-e2e" ACTAGENT_IMAGE)"
PORT="${ACTAGENT_MCP_CODE_MODE_LIVE_GATEWAY_PORT:-19199}"
TOKEN="mcp-code-mode-live-e2e-$(date +%s)-$$"
CONTAINER_NAME="actagent-mcp-code-mode-live-e2e-$$"
CLIENT_LOG="$(mktemp -t actagent-mcp-code-mode-live-log.XXXXXX)"
PROFILE_FILE="${ACTAGENT_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${ACTAGENT_TESTBOX_PROFILE_FILE:-$HOME/.actagent-testbox-live.profile}}"
...
# The profile is only a credential source. Keep this lane's ACTAgent runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset ACTAGENT_TESTBOX

docker_e2e_build_or_reuse "$IMAGE_NAME" mcp-code-mode-gateway-live
ACTAGENT_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "ACTAGENT_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "ACTAGENT_SKIP_ACPX_RUNTIME=1" \
  -e "ACTAGENT_SKIP_ACPX_RUNTIME_PROBE=1" \
  ...
  -e "ACTAGENT_MCP_CODE_MODE_MODEL=${ACTAGENT_MCP_CODE_MODE_LIVE_MODEL:-actagent/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(actagent_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    actagent_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480

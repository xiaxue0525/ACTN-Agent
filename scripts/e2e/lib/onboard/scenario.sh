...
ACTAGENT_ONBOARD_SCENARIO_SOURCE_ONLY="${ACTAGENT_ONBOARD_SCENARIO_SOURCE_ONLY:-0}"
if [ "$ACTAGENT_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
...
if [ -z "${ACTAGENT_ENTRY:-}" ] && [ "$ACTAGENT_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
  ACTAGENT_ENTRY="$(actagent_e2e_resolve_entrypoint)"
fi
export ACTAGENT_ENTRY
ONBOARD_TMP_ROOT="${ACTAGENT_ONBOARD_E2E_TMPDIR:-${TMPDIR:-/tmp}}"
ONBOARD_TMP_ROOT="${ONBOARD_TMP_ROOT%/}"
[ -n "$ONBOARD_TMP_ROOT" ] || ONBOARD_TMP_ROOT="/tmp"
mkdir -p "$ONBOARD_TMP_ROOT"
ONBOARD_TMP_DIR="$(mktemp -d "$ONBOARD_TMP_ROOT/actagent-onboard.XXXXXX")"
ACTAGENT_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"
GATEWAY_LOG_PATH="$ONBOARD_TMP_DIR/gateway-e2e.log"
export ACTAGENT_E2E_LOG_DIR
export GATEWAY_LOG_PATH
mkdir -p "$ACTAGENT_E2E_LOG_DIR"
...
start_gateway() {
  GATEWAY_PID="$(actagent_e2e_start_gateway "$ACTAGENT_ENTRY" 19199 "$GATEWAY_LOG_PATH")"
}

wait_for_gateway() {
  for _ in $(seq 1 20); do
    if actagent_e2e_probe_tcp 127.0.0.1 19199 500 >/dev/null 2>&1; then
      return 0
    fi
    if [ -f "$GATEWAY_LOG_PATH" ] && grep -E -q "listening on ws://[^ ]+:19199" "$GATEWAY_LOG_PATH"; then
...
cleanup_wizard_case() {
  ...
  actagent_e2e_stop_process "${wizard_pid:-}"
  stop_gateway "${gw_pid:-}"
...
run_wizard_cmd() {
  ...
  set_isolated_actagent_env "$state_ref"
  ...
  local log_path="$ACTAGENT_E2E_LOG_DIR/${case_name}.log"
  WIZARD_LOG_PATH="$log_path"
  export WIZARD_LOG_PATH
  # Run under script to keep an interactive TTY for clack prompts.
  actagent_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
...
run_wizard() {
  ...
  run_wizard_cmd "$case_name" "$state_ref" "node \"$ACTAGENT_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
}

assert_onboard_config() {
  local scenario="$1"
  shift
  actagent_e2e_assert_file "$ACTAGENT_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$ACTAGENT_CONFIG_PATH" "$@"
}

set_isolated_actagent_env() {
  local state_ref="$1"
  actagent_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_actagent_env local-basic
  actagent_e2e_run_logged local-basic node "$ACTAGENT_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  validate_local_basic_log "$ACTAGENT_E2E_LAST_LOG_PATH"

  # Assert config + workspace scaffolding.
  workspace_dir="$ACTAGENT_STATE_DIR/workspace"
  sessions_dir="$ACTAGENT_STATE_DIR/agents/main/sessions"

  actagent_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    actagent_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_actagent_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  actagent_e2e_run_logged remote-non-interactive node "$ACTAGENT_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
    --remote-url ws://gateway.local:19199 \
...
run_case_reset() {
  set_isolated_actagent_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$ACTAGENT_CONFIG_PATH"

  actagent_e2e_run_logged reset-config node "$ACTAGENT_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$ACTAGENT_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_actagent_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$ACTAGENT_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$ACTAGENT_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  actagent_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}

...
const pluginRecordSnapshot = () => {
  const config = readJson(actagentPath("actagent.json"));
  const records = readPluginInstallRecords({ fallbackRecords: config.plugins?.installs ?? {} });
  const record = records["lossless-actagent"] ?? records["@example/lossless-actagent"];
  if (!record) {
...
function seedInstallState() {
  writeJson(actagentPath("extensions", "lossless-actagent", "package.json"), {
    name: "@example/lossless-actagent",
    version: "0.9.0",
  });
  writeJson(process.env.ACTAGENT_CONFIG_PATH, { plugins: {} });
  writePluginInstallIndexForE2E({
    version: 1,
    warning: "DO NOT EDIT. This file is generated by ACTAgent plugin registry commands.",
    ...
    installRecords: {
      "lossless-actagent": {
        source: "npm",
        spec: "@example/lossless-actagent@0.9.0",
        installPath: "~/.actagent/extensions/lossless-actagent",
        resolvedName: "@example/lossless-actagent",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@example/lossless-actagent@0.9.0",
...
function registryHealthy() {
  return new Promise((resolve) => {
    const req = http.get("http://127.0.0.1:4873/@example%2flossless-actagent", (res) => {
...
function assertOutput(logPath) {
  ...
  const failure = output.includes("Downloading @example/lossless-actagent")
    ? "Unexpected npm download/reinstall path"
    : !output.includes("lossless-actagent is up to date (0.9.0).")
...
function isCorruptPluginDisabledAfterUpdate(evidence, pluginId) {
  ...
  return (
    ...
    message.includes("ACTAgent will continue without it")
  );
}
...
function assertCorruptPluginDetails(plugins, pluginId) {
  ...
  const expectedFragments = disabledAfterFailure
    ? [
        ...
        "ACTAgent will continue without it",

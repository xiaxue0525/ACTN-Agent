...
export OPENAI_API_KEY="sk-actagent-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="19199"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="ACTAGENT_E2E_OK_RELEASE_UPGRADE"
...
BASELINE_SPEC="${ACTAGENT_RELEASE_UPGRADE_BASELINE_SPEC:-actagent@latest}"
export SUCCESS_MARKER MOCK_REQUEST_LOG CLICKCLACK_STATE

candidate_version="$(
  tar -xOf "${ACTAGENT_CURRENT_PACKAGE_TGZ:?missing ACTAGENT_CURRENT_PACKAGE_TGZ}" package/package.json |
...
cleanup() {
  ...
  actagent_e2e_stop_process "${clickclack_pid:-}"
...
if ! actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v actagent >/dev/null
baseline_root="$(actagent_e2e_package_root)"
baseline_entry="$(actagent_e2e_package_entrypoint "$baseline_root")"
actagent_e2e_enable_actagent_cli_timeout

mock_pid="$(actagent_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if actagent_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
actagent_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
actagent plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
actagent release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
actagent_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate ACTAgent package"
...
actagent release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1

...
function expectFailure() {
  ...
  const expected =
    source === "actagenthub"
      ? /Version not found on ACTAgentHub|ACTAgentHub .* failed \(404\)|version.*not found/iu
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".actagent")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /actagent-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertACTAgentHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(actagentPeerPath)) {
    throw new Error(`missing kitchen-sink actagent peer symlink: ${actagentPeerPath}`);
  }
  if (!fs.lstatSync(actagentPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink actagent peer is not a symlink: ${actagentPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink actagent peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertACTAgentHubArtifactMetadata(record) {
  ...
  if (record.artifactKind !== "npm-pack" || record.artifactFormat !== "tgz") {
    throw new Error(`missing kitchen-sink ACTAgentHub artifact metadata: ${JSON.stringify(record)}`);
  }
  if (!record.actagentpackSha256 || typeof record.actagentpackSize !== "number") {
    throw new Error(`missing kitchen-sink actagentpack metadata: ${JSON.stringify(record)}`);
  }
...
function assertInstalled() {
  ...
  } else if (source === "actagenthub") {
    const value = spec.slice("actagenthub:".length).trim();
    const slashIndex = value.lastIndexOf("/");
    const atIndex = value.lastIndexOf("@");
    const packageName = atIndex > 0 && atIndex > slashIndex ? value.slice(0, atIndex) : value;
    if (record.spec !== spec) {
      throw new Error(`expected kitchen-sink ACTAgentHub spec ${spec}, got ${record.spec}`);
    }
    if (record.actagenthubPackage !== packageName) {
      throw new Error(`expected ACTAgentHub package ${packageName}, got ${record.actagenthubPackage}`);
    }
    if (record.actagenthubFamily !== "code-plugin" && record.actagenthubFamily !== "bundle-plugin") {
      throw new Error(`unexpected ACTAgentHub family: ${record.actagenthubFamily}`);
    }
    if (!record.version || !record.integrity || !record.resolvedAt) {
      throw new Error(`missing ACTAgentHub resolution metadata: ${JSON.stringify(record)}`);
    }
    assertACTAgentHubArtifactMetadata(record);
  }
  ...
  if (source === "actagenthub" && record.artifactKind === "npm-pack") {

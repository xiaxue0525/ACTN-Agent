...
if [[ -z "${ACTAGENT_ENTRY:-}" && "$KITCHEN_SINK_SWEEP_SOURCE_ONLY" != "1" ]]; then
...
if [[ -z "${KITCHEN_SINK_TMP_DIR:-}" ]]; then
  KITCHEN_SINK_TMP_DIR="$(mktemp -d "/tmp/actagent-kitchen-sink.XXXXXX")"
...
KITCHEN_SINK_ACTAGENTHUB_FIXTURE_DIR=""
KITCHEN_SINK_ACTAGENTHUB_PID_FILE=""

cleanup_kitchen_sink_sweep() {
  if [[ -n "${KITCHEN_SINK_ACTAGENTHUB_PID_FILE:-}" && -f "$KITCHEN_SINK_ACTAGENTHUB_PID_FILE" ]]; then
    actagent_e2e_stop_process "$(cat "$KITCHEN_SINK_ACTAGENTHUB_PID_FILE" 2>/dev/null || true)"
  fi
  if [[ -n "${KITCHEN_SINK_ACTAGENTHUB_FIXTURE_DIR:-}" ]]; then
    rm -rf "$KITCHEN_SINK_ACTAGENTHUB_FIXTURE_DIR"
  fi
...
run_kitchen_sink_actagent_logged() {
  local label="$1"
  shift
  run_logged_print "$label" actagent_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" "$@"
}

run_kitchen_sink_actagent_capture() {
  local output_file="$1"
  shift
  actagent_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" "$@" >"$output_file"
}
...
start_kitchen_sink_actagenthub_fixture_server() {
  local fixture_dir="$1"
  ...
  node scripts/e2e/lib/actagenthub-fixture-server.cjs kitchen-sink-plugin "$server_port_file" >"$server_log" 2>&1 &
  local server_pid="$!"
  echo "$server_pid" >"$server_pid_file"
  KITCHEN_SINK_ACTAGENTHUB_FIXTURE_DIR="$fixture_dir"
  KITCHEN_SINK_ACTAGENTHUB_PID_FILE="$server_pid_file"

  local wait_attempts="${ACTAGENT_ACTAGENTHUB_FIXTURE_WAIT_ATTEMPTS:-600}"
  ...
  echo "Timed out waiting for kitchen-sink ACTAgentHub fixture server." >&2
  return 1
}
...
run_success_scenario() {
  ...
  if [ -n "${KITCHEN_SINK_PREINSTALL_SPEC:-}" ]; then
    run_kitchen_sink_actagent_logged "kitchen-sink-preinstall-${KITCHEN_SINK_LABEL}" plugins install "$KITCHEN_SINK_PREINSTALL_SPEC"
    assert_kitchen_sink_cutover_preinstalled
    install_args+=("--force")
  fi
  run_kitchen_sink_actagent_logged "kitchen-sink-install-${KITCHEN_SINK_LABEL}" plugins install "${install_args[@]}"
  configure_kitchen_sink_runtime
  run_kitchen_sink_actagent_logged "kitchen-sink-enable-${KITCHEN_SINK_LABEL}" plugins enable "$KITCHEN_SINK_ID"
  run_kitchen_sink_actagent_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-plugins.json" plugins list --json
  run_kitchen_sink_actagent_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect.json" plugins inspect "$KITCHEN_SINK_ID" --runtime --json
  run_kitchen_sink_actagent_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect-all.json" plugins inspect --all --runtime --json
  assert_kitchen_sink_installed
  if [ "$KITCHEN_SINK_SOURCE" = "actagenthub" ]; then
    run_kitchen_sink_actagent_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_SPEC" --force
  else
    run_kitchen_sink_actagent_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_ID" --force
  fi
  remove_kitchen_sink_channel_config
  run_kitchen_sink_actagent_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}

run_failure_scenario() {
  echo "Testing expected ${KITCHEN_SINK_LABEL} install failure from ${KITCHEN_SINK_SPEC}..."
  run_expect_failure "install-${KITCHEN_SINK_LABEL}" actagent_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" plugins install "$KITCHEN_SINK_SPEC"
  remove_kitchen_sink_channel_config
  run_kitchen_sink_actagent_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}
...
run_kitchen_sink_sweep_main() {
  if [[ "$KITCHEN_SINK_SCENARIOS" == *"actagenthub:"* ]]; then
    if [[ "${ACTAGENT_KITCHEN_SINK_LIVE_ACTAGENTHUB:-0}" = "1" ]]; then
      ...
      if [[ -n "${ACTAGENT_ACTAGENTHUB_URL:-}" || -n "${ACTAGENTHUB_URL:-}" ]]; then
        echo "Ignoring ambient ACTAgentHub URL for fixture-mode kitchen-sink E2E; set ACTAGENT_KITCHEN_SINK_LIVE_ACTAGENTHUB=1 for live ACTAgentHub."
      fi
      unset ACTAGENT_ACTAGENTHUB_URL ACTAGENTHUB_URL
      actagenthub_fixture_dir="$(mktemp -d "${KITCHEN_SINK_TMP_DIR}/actagenthub.XXXXXX")"
      start_kitchen_sink_actagenthub_fixture_server "$actagenthub_fixture_dir"
    fi
  fi
  ...
  while IFS='|' read -r label spec plugin_id source expectation surface_mode personality preinstall_spec; do
    ...
    export ACTAGENT_KITCHEN_SINK_PERSONALITY="${personality:-}"

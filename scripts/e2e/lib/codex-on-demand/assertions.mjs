...
const inspect = readJson("/tmp/actagent-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@actagent/codex")) {
  throw new Error(`expected @actagent/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @actagent/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@actagent/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@actagent/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "actagent-agent.sqlite");
...
if (authRaw.includes("sk-actagent-codex-on-demand-e2e")) {

...
ARTIFACT_ROOT="$(dirname "${ACTAGENT_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/actagent-upgrade-survivor-artifacts/summary.json}")"
...
RUNTIME_ROOT="$ACTAGENT_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${ACTAGENT_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${ACTAGENT_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export ACTAGENT_TEST_STATE_TMPDIR="${ACTAGENT_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$ACTAGENT_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${ACTAGENT_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${ACTAGENT_UPGRADE_SURVIVOR_BASELINE:?missing ACTAGENT_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${ACTAGENT_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${ACTAGENT_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${ACTAGENT_CURRENT_PACKAGE_TGZ:-}}"
...
export ACTAGENT_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
rm -f "$SUMMARY_JSON" "$CONFIG_COVERAGE_JSON"
: >"$PHASE_LOG"

validate_baseline_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^actagent@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "ACTAGENT_UPGRADE_SURVIVOR_BASELINE must be actagent@latest, actagent@beta, actagent@alpha, an exact ACTAgent release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "ACTAGENT_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    actagent@*)
      baseline_spec="$raw"
      baseline_version="${raw#actagent@}"
      ;;
    *@*)
      echo "ACTAGENT_UPGRADE_SURVIVOR_BASELINE must be actagent@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="actagent@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "ACTAGENT_UPGRADE_SURVIVOR_BASELINE must be actagent@latest, actagent@beta, actagent@alpha, actagent@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "ACTAGENT_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.ACTAGENT_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.ACTAGENT_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.ACTAGENT_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      actagent_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/actagent\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${ACTAGENT_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "ACTAGENT_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@actagent-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${ACTAGENT_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$ACTAGENT_STATE_DIR/extensions/opik-actagent"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-actagent",
...
JSON
  cat >"$shadow_root/actagent.plugin.json" <<'JSON'
{
  "id": "opik-actagent",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.actagent-runtime-deps.json" \
      "$plugin_dir/.actagent-runtime-deps-stamp.json" \
      "$plugin_dir/.actagent-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.actagent-install-stage-upgrade-survivor" \
      "$plugin_dir/.actagent-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ACTAGENT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ACTAGENT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/actagent-upgrade-survivor-dep" \
      "$plugin_dir/.actagent-runtime-deps-copy-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep" \
      "$plugin_dir/.actagent-install-stage-upgrade-survivor" \
      "$plugin_dir/.actagent-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep" \
      "$ACTAGENT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep" \
      "$ACTAGENT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep"
    printf '{"name":"actagent-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/actagent-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.actagent-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.actagent-runtime-deps-stamp.json"
    printf '{"name":"actagent-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.actagent-runtime-deps-copy-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep/package.json"
    printf '{"name":"actagent-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep/package.json"
    printf '{"name":"actagent-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ACTAGENT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep/package.json"
    printf '{"name":"actagent-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ACTAGENT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/actagent-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"actagent-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$ACTAGENT_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$ACTAGENT_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    cat "$BASELINE_INSTALL_LOG" >&2 || true
    return 1
  fi
  if ! command -v actagent >/dev/null; then
    echo "baseline install did not expose actagent on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent --version 2>&1)"; then
    echo "baseline actagent --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline actagent --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  actagent_e2e_eval_test_state_from_b64 "${ACTAGENT_TEST_STATE_FUNCTION_B64:?missing ACTAGENT_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.actagent /root/workspace
    actagent_test_state_create /root minimal
  else
    actagent_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export ACTAGENT_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
...
log_file="${ACTAGENT_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/actagent-systemctl-shim.log}"
pid_file="${ACTAGENT_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/actagent-systemctl-shim.pid}"
daemon_log="${ACTAGENT_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/actagent-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/actagent-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u ACTAGENT_GATEWAY_TOKEN -u ACTAGENT_GATEWAY_PASSWORD actagent gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
...
const stateDir = process.env.ACTAGENT_STATE_DIR;
if (!stateDir) {
  throw new Error("missing ACTAGENT_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$ACTAGENT_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$ACTAGENT_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  ACTAGENT_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing ACTAGENT_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u ACTAGENT_GATEWAY_TOKEN
    -u ACTAGENT_GATEWAY_PASSWORD
    -u ACTAGENT_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(ACTAGENT_ALLOW_ROOT=1)
  fi
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" actagent "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
...
assert_root_managed_vps_cli_usable() {
  ...
  actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" actagent config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" actagent plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
...
validate_post_doctor_config() {
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${ACTAGENT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$ACTAGENT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  args+=(--out "$out_file")
...
start_gateway() {
  local port=19199
  local budget="${ACTAGENT_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u ACTAGENT_GATEWAY_TOKEN -u ACTAGENT_GATEWAY_PASSWORD actagent gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_probes() {
  ...
  export ACTAGENT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING="discord,telegram,whatsapp,feishu,matrix"
  readyz_seconds="$(probe_gateway_endpoint /readyz ready "$READYZ_JSON")"
  unset ACTAGENT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING
}

check_gateway_status() {
  local port=19199
  local budget="${ACTAGENT_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then

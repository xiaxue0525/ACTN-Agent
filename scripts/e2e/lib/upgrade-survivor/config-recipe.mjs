    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/actagent-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.ACTAGENT_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
export function resolveUpgradeSurvivorACTAgentCommand(argv, params = {}) {
  const platform = params.platform ?? process.platform;
  if (platform === "win32") {
    const comSpec = params.comSpec ?? process.env.ComSpec ?? "cmd.exe";
    return {
      command: comSpec,
      args: ["/d", "/s", "/c", buildCmdExeCommandLine("actagent.cmd", argv)],
      commandLabel: ["actagent", ...argv].join(" "),
      shell: false,
...
export function runUpgradeSurvivorACTAgentStep(step, params = {}) {
  const invocation = resolveUpgradeSurvivorACTAgentCommand(step.argv);
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runUpgradeSurvivorACTAgentStep(adaptedStep);

...
function getScenario() {
  const scenario = process.env.ACTAGENT_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("ACTAGENT_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.ACTAGENT_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("ACTAGENT_STATE_DIR");
  const workspace = requireEnv("ACTAGENT_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".actagent", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".actagent-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
    write(
      path.join(
        runtimeRoot,
        plugin,
        ".actagent-runtime-deps-copy-stale",
        "node_modules",
        "stale-sentinel",
        "package.json",
      ),
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.ACTAGENT_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `actagent-${version}-${plugin}`,
          ".actagent-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/actagent-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const stage = process.env.ACTAGENT_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-actagent");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`actagent-${version}-`))
      : [];
    assert(
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const installedFromOfficialACTAgentHubNpmPack =
    record.source === "actagenthub" &&
    record.actagenthubChannel === "official" &&
    record.artifactKind === "npm-pack";
  assert(
    installedFromNpm || installedFromOfficialACTAgentHubNpmPack,
    `configured external ${pluginId} plugin must be installed from npm or official ACTAgentHub npm-pack, got: ${record.source}`,
  );
  ...
  assert(
    record.actagenthubPackage === packageName,
    `configured external ${pluginId} ACTAgentHub package changed: ${record.actagenthubPackage}`,
  );
  const extensionsRoot = path.join(requireEnv("ACTAGENT_STATE_DIR"), "extensions");
  assert(
    isPathInside(extensionsRoot, installPath),
    `configured external ${pluginId} ACTAgentHub install path outside managed extensions root: ${installPath}`,
  );
}
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@actagent/brave-plugin",
    pluginId: "brave",
  });

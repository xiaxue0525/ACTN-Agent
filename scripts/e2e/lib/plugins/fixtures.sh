ACTAGENT_PLUGINS_FIXTURE_PID_FILES=()
ACTAGENT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
ACTAGENT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

actagent_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${ACTAGENT_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
...
actagent_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  ACTAGENT_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  actagent_plugins_install_fixture_cleanup_trap
}

actagent_plugins_install_fixture_cleanup_trap() {
  if [[ "${ACTAGENT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *actagent_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "ACTAGENT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  ACTAGENT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap actagent_plugins_fixture_exit_trap EXIT
}

actagent_plugins_fixture_exit_trap() {
  local status="$?"
  actagent_plugins_cleanup_fixture_servers
  if [[ -n "${ACTAGENT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$ACTAGENT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.actagent.extensions = ["./index.js", " "];

...
const scratchRoot = process.env.ACTAGENT_PLUGINS_TMP_DIR || os.tmpdir();
const readJson = (file) => JSON.parse(fs.readFileSync(file, "utf8"));
const scratchFile = (name) => path.join(scratchRoot, name);

function readACTAgentHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = path.join(process.env.HOME, ".actagent", "actagent.json");
  const config = fs.existsSync(configPath) ? readJson(configPath) : {};
  const allowLegacyCompat = process.env.ACTAGENT_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readACTAgentConfig();
...
function parseACTAgentHubPackageName(rawSpec) {
  const value = rawSpec.slice("actagenthub:".length).trim();
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".actagent", "git");
...
function assertACTAgentHubExternalInstallContract(installPath) {
  const actagentPeerPath = path.join(installPath, "node_modules", "actagent");
  if (!fs.existsSync(actagentPeerPath)) {
    throw new Error(`missing ACTAgentHub actagent peer symlink: ${actagentPeerPath}`);
  }
  if (!fs.lstatSync(actagentPeerPath).isSymbolicLink()) {
    throw new Error(`ACTAgentHub actagent peer is not a symlink: ${actagentPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(actagentPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ACTAgentHub actagent peer ${linkedHostRoot} to target ${hostRoot}`);
  }

  const dependencyPackagePath = path.join(installPath, "node_modules", "is-number", "package.json");
  if (fs.existsSync(dependencyPackagePath)) {
    assertRealPathInside(installPath, dependencyPackagePath, "ACTAgentHub isolated dependency");
  }
}

function assertACTAgentHubArtifactMetadata(record, pluginId) {
  if (record.artifactKind === "legacy-zip") {
    if (record.artifactFormat !== "zip") {
      throw new Error(
        `missing ACTAgentHub legacy ZIP artifact metadata for ${pluginId}: ${JSON.stringify(record)}`,
      );
    }
    return;
  }
  ...
  if (record.artifactKind !== "npm-pack" || record.artifactFormat !== "tgz") {
    throw new Error(`missing ACTAgentHub artifact metadata for ${pluginId}: ${JSON.stringify(record)}`);
  }
  if (!record.actagentpackSha256 || typeof record.actagentpackSize !== "number") {
    throw new Error(`missing ACTAgentHub actagentpack metadata for ${pluginId}: ${JSON.stringify(record)}`);
  }
  if (!record.npmIntegrity || !record.npmShasum || !record.npmTarballName) {
    throw new Error(
      `missing ACTAgentHub npm artifact metadata for ${pluginId}: ${JSON.stringify(record)}`,
    );
  }
}
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@actagent/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@actagent/demo-plugin-npm") {
...
function assertInvalidACTAgentExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  const output = fs.readFileSync(scratchFile("plugins-invalid-actagent-extensions.log"), "utf8");
  for (const expected of ["actagent.extensions[1]", "non-empty string"]) {
  ...
  const list = readJson(scratchFile("plugins-invalid-actagent-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".actagent", "extensions", pluginId);
...
async function assertACTAgentHubPreflight() {
  const spec = process.env.ACTAGENTHUB_PLUGIN_SPEC;
  if (!spec?.startsWith("actagenthub:")) {
    throw new Error(`expected actagenthub: spec, got ${spec}`);
  }

  const limits = readACTAgentHubPreflightLimits();
  const packageName = parseACTAgentHubPackageName(spec);
  const baseUrl = (
    process.env.ACTAGENT_ACTAGENTHUB_URL ||
    process.env.ACTAGENTHUB_URL ||
    "https://actagenthub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.ACTAGENT_ACTAGENTHUB_TOKEN ||
    process.env.ACTAGENTHUB_TOKEN ||
    process.env.ACTAGENTHUB_AUTH_TOKEN ||
    "";
  const preflightUrl = `${baseUrl}/api/v1/packages/${encodeURIComponent(packageName)}`;
  const response = await withTimeout(
    `ACTAgentHub package preflight for ${packageName}`,
    limits.timeoutMs,
    (signal) =>
      fetch(preflightUrl, {
  ...
  if (!response.ok) {
    const body = await withTimeout(
      `ACTAgentHub package preflight response for ${packageName}`,
      limits.timeoutMs,
      () =>
    ...
    throw new Error(
      `ACTAgentHub package preflight failed for ${packageName}: ${response.status} ${body}`,
    );
  }
  ...
  const detail = await withTimeout(
    `ACTAgentHub package preflight JSON for ${packageName}`,
    limits.timeoutMs,
    () => JSON.parse(rawDetail),
  );
  const family = detail.package?.family;
  if (family !== "code-plugin" && family !== "bundle-plugin") {
    throw new Error(`ACTAgentHub package ${packageName} is not installable as a plugin: ${family}`);
  }
  if (detail.package?.runtimeId && detail.package.runtimeId !== process.env.ACTAGENTHUB_PLUGIN_ID) {
    throw new Error(
      `ACTAgentHub package ${packageName} runtimeId ${detail.package.runtimeId} does not match expected ${process.env.ACTAGENTHUB_PLUGIN_ID}`,
    );
  }
  console.log(`Using ACTAgentHub package ${packageName} (${family}).`);
}

function assertACTAgentHubInstalled() {
  const pluginId = process.env.ACTAGENTHUB_PLUGIN_ID;
  const spec = process.env.ACTAGENTHUB_PLUGIN_SPEC;
  const packageName = parseACTAgentHubPackageName(spec);
  const list = readJson(scratchFile("plugins-actagenthub-installed.json"));
  const inspect = readJson(scratchFile("plugins-actagenthub-inspect.json"));
  const plugin = (list.plugins || []).find((entry) => entry.id === pluginId);
  if (!plugin) {
    throw new Error(`ACTAgentHub plugin not found after install: ${pluginId}`);
  }
  if (plugin.status !== "loaded") {
    throw new Error(`unexpected ACTAgentHub plugin status for ${pluginId}: ${plugin.status}`);
  }
  if (inspect.plugin?.id !== pluginId) {
    throw new Error(`unexpected ACTAgentHub inspect plugin id: ${inspect.plugin?.id}`);
  }
  ...
  if (!record) {
    throw new Error(`missing ACTAgentHub install record for ${pluginId}`);
  }
  if (record.source !== "actagenthub") {
    throw new Error(`unexpected ACTAgentHub install source for ${pluginId}: ${record.source}`);
  }
  if (record.actagenthubPackage !== packageName) {
    throw new Error(
      `unexpected ACTAgentHub package for ${pluginId}: ${record.actagenthubPackage}, expected ${packageName}`,
    );
  }
  if (record.actagenthubFamily !== "code-plugin" && record.actagenthubFamily !== "bundle-plugin") {
    throw new Error(`unexpected ACTAgentHub family for ${pluginId}: ${record.actagenthubFamily}`);
  }
  if (typeof record.installPath !== "string" || record.installPath.length === 0) {
    throw new Error(`missing ACTAgentHub install path for ${pluginId}`);
  }
  assertACTAgentHubArtifactMetadata(record, pluginId);

  const installPath = resolveHomePath(record.installPath);
  const extensionsRoot = path.join(process.env.HOME, ".actagent", "extensions");
  if (!installPath.startsWith(`${extensionsRoot}${path.sep}`)) {
    throw new Error(`ACTAgentHub install path is outside managed extensions root: ${installPath}`);
  }
  if (!fs.existsSync(installPath)) {
    throw new Error(`ACTAgentHub install path missing on disk: ${installPath}`);
  }
  if (record.artifactKind === "npm-pack") {
    assertACTAgentHubExternalInstallContract(installPath);
  }
  fs.writeFileSync(scratchFile("plugins-actagenthub-install-path.txt"), installPath, "utf8");
}

function assertACTAgentHubRemoved() {
  const pluginId = process.env.ACTAGENTHUB_PLUGIN_ID;
  const installPath = fs
    .readFileSync(scratchFile("plugins-actagenthub-install-path.txt"), "utf8")
    .trim();
  const list = readJson(scratchFile("plugins-actagenthub-uninstalled.json"));
  if ((list.plugins || []).some((entry) => entry.id === pluginId)) {
    throw new Error(`ACTAgentHub plugin still listed after uninstall: ${pluginId}`);
  }
  ...
  if (installRecords[pluginId]) {
    throw new Error(`ACTAgentHub install record still present after uninstall: ${pluginId}`);
  }

  const configAfterUninstallPath = path.join(process.env.HOME, ".actagent", "actagent.json");
  ...
  if (configAfterUninstall.plugins?.entries?.[pluginId]) {
    throw new Error(`ACTAgentHub config entry still present after uninstall: ${pluginId}`);
  }
  if ((configAfterUninstall.plugins?.allow || []).includes(pluginId)) {
    throw new Error(`ACTAgentHub allowlist entry still present after uninstall: ${pluginId}`);
  }
  if ((configAfterUninstall.plugins?.deny || []).includes(pluginId)) {
    throw new Error(`ACTAgentHub denylist entry still present after uninstall: ${pluginId}`);
  }
  if (fs.existsSync(installPath)) {
    throw new Error(
      `ACTAgentHub managed install directory still exists after uninstall: ${installPath}`,
    );
  }
}

function assertACTAgentHubUpdated() {
  const output = fs.readFileSync(scratchFile("plugins-actagenthub-update.log"), "utf8");
  if (!output.includes(`${process.env.ACTAGENTHUB_PLUGIN_ID} already at `)) {
    throw new Error(`expected ACTAgentHub update to report already-at version:\n${output}`);
  }
  assertACTAgentHubInstalled();
}
...
const commands = {
  ...
  "invalid-actagent-extensions": assertInvalidACTAgentExtensionsRejected,
  ...
  "actagenthub-preflight": assertACTAgentHubPreflight,
  "actagenthub-installed": assertACTAgentHubInstalled,
  "actagenthub-updated": assertACTAgentHubUpdated,
  "actagenthub-removed": assertACTAgentHubRemoved,
};

...
function actagentPath(...parts) {
  return path.join(home, ".actagent", ...parts);
}
...
function config() {
  return readJson(process.env.ACTAGENT_CONFIG_PATH ?? actagentPath("actagent.json"));
}
...
function assertNpmProjectRoot(pluginId, packageName) {
  const record = recordFor(pluginId);
  assert(record?.installPath, `install path missing for ${pluginId}`);
  const relative = path.relative(actagentPath("npm", "projects"), record.installPath);
  assert(
  ...
  assert(
    !fs.existsSync(actagentPath("npm", "node_modules", ...packageSegments)),

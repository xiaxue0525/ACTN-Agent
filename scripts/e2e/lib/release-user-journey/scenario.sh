...
export OPENAI_API_KEY="sk-actagent-release-user-journey"
export ACTAGENT_GATEWAY_TOKEN="release-user-journey-token"
export CLICKCLACK_BOT_TOKEN="clickclack-release-token"

PORT="19199"
MOCK_PORT="44180"
CLICKCLACK_PORT="44181"
SUCCESS_MARKER="ACTAGENT_E2E_OK_RELEASE_USER_JOURNEY"
...
actagent plugins install "$plugin_a_dir" >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
actagent journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
actagent plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
actagent plugins install "$plugin_b_dir" >"$PLUGIN_B_INSTALL_LOG" 2>&1
actagent journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
actagent plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
actagent channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
actagent doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1

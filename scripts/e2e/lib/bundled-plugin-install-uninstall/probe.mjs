...
function resolveStateDir() {
  if (process.env.ACTAGENT_STATE_DIR) {
    return process.env.ACTAGENT_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".actagent");
}
...
function resolveACTAgentEntry() {
  if (process.env.ACTAGENT_ENTRY) {
    return process.env.ACTAGENT_ENTRY;
  }
  ...
  throw new Error("Missing ACTAGENT_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveACTAgentEntry();
  const timeoutMs = readPositiveIntEnv(
    "ACTAGENT_BUNDLED_PLUGIN_LIST_TIMEOUT_MS",
  ...
  const result = spawnSync(process.execPath, [entry, "plugins", "list", "--json"], {
    ...
    maxBuffer: readPositiveIntEnv(
      "ACTAGENT_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES",
...
async function loadManifestEntries() {
  const explicit = (process.env.ACTAGENT_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `ACTAGENT_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("ACTAGENT_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("ACTAGENT_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `ACTAGENT_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.ACTAGENT_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }

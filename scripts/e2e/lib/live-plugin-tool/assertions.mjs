...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
function stateDir() {
  return process.env.ACTAGENT_STATE_DIR || path.join(process.env.HOME, ".actagent");
}

function configPath() {
  return process.env.ACTAGENT_CONFIG_PATH || path.join(stateDir(), "actagent.json");
}

function agentOutputPath() {
  return process.env.ACTAGENT_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/actagent-agent.json";
}

function agentErrorPath() {
  return process.env.ACTAGENT_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/actagent-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/actagent-plugin-inspect.json")
    ? readJson("/tmp/actagent-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "actagent.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/actagent-plugins-list.json");
  ...
  const inspect = readJson("/tmp/actagent-plugin-inspect.json");

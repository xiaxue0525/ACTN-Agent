...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.ACTAGENT_WORKSPACE_DIR || path.join(process.env.HOME, ".actagent", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.ACTAGENT_STATE_DIR, "ACTAGENT_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.ACTAGENT_GATEWAY_TOKEN?.trim();
  fs.mkdirSync(sharedWorkspace, { recursive: true });
  writeJson(path.join(stateDir, "actagent.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.ACTAGENT_STATE_DIR, "actagent.json"))?.agents?.list ?? [];

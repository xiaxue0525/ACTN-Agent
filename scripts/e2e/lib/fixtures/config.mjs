...
function writeConfig(kind) {
  const configPath = requireArg(process.env.ACTAGENT_CONFIG_PATH, "ACTAGENT_CONFIG_PATH");
  const port = readPositiveIntEnv("PORT", 19199);
  const config =
    kind === "config-reload"
      ? {
          gateway: {
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.ACTAGENT_GATEWAY_TOKEN, "ACTAGENT_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.ACTAGENT_STATE_DIR, "actagent.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.ACTAGENT_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.ACTAGENT_CONFIG_BATCH_PATH,
    "ACTAGENT_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: readPositiveIntEnv("ACTAGENT_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", 900),
    },
    { path: "models.providers.openai.agentRuntime", value: { id: "actagent" } },
    { path: "gateway.controlUi.enabled", value: false },
    { path: "gateway.mode", value: "local" },
    { path: "gateway.bind", value: "lan" },
    { path: "gateway.auth.mode", value: "token" },
    { path: "gateway.auth.token", value: process.env.ACTAGENT_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.ACTAGENT_OPENWEBUI_MODEL },
  ]);
}

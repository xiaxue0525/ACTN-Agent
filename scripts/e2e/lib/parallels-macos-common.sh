...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/actagent-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/actagent-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ACTAGENT_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ACTAGENT_SECRET_ENV_NAME:-}" ] && [ -n "${ACTAGENT_SECRET_FILE:-}" ] && [ -f "$ACTAGENT_SECRET_FILE" ]; then
  secret_value="$(cat "$ACTAGENT_SECRET_FILE")"
  rm -f "$ACTAGENT_SECRET_FILE"
  export "${ACTAGENT_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ACTAGENT_SECRET_ENV_NAME=$api_key_env" \
      "ACTAGENT_SECRET_FILE=$secret_path" \
      "ACTAGENT_WRAPPER_FILE=$wrapper_path" \

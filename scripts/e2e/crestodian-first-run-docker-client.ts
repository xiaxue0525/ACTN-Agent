...
async function main() {
  ...
  const tempState = await createE2eStateDir("actagent-crestodian-first-run-");
  ...
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "actagent"]),
    "fresh bare ACTAgent invocation did not route to onboarding",
  );
  assert(
    shouldStartCrestodianForModernOnboard(["node", "actagent", "onboard", "--modern"]),
    "modern onboard invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "actagent", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(

// Mcp Code Mode Gateway Client script supports ACTAgent repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ACTAGENT_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("ACTAGENT_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.ACTAGENT_MCP_CODE_MODE_MODEL?.trim() || "actagent/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing ACTAGENT_STATE_DIR");

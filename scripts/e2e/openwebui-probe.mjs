...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.ACTAGENT_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "actagent/default") ?? modelIds.find((id) => id === "actagent");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing actagent model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `actagent model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {

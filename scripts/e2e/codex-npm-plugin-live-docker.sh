#!/usr/bin/env bash
# Installs ACTAgent from a prepared package tarball, installs @actagent/codex
...
CANDIDATE_ROOT="${ACTAGENT_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-codex-npm-plugin-live-e2e" ACTAGENT_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${ACTAGENT_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${ACTAGENT_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${ACTAGENT_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${ACTAGENT_CODEX_NPM_PLUGIN_PROFILE_FILE:-${ACTAGENT_TESTBOX_PROFILE_FILE:-$HOME/.actagent-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${ACTAGENT_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ACTAGENT_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ACTAGENT_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires ACTAGENT_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/actagent-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/actagent-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      cat /tmp/actagent-codex-plugin-pack.log >&2 || true
      exit 1
    fi
...
ACTAGENT_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e ACTAGENT_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${ACTAGENT_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e ACTAGENT_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${ACTAGENT_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e ACTAGENT_CODEX_NPM_PLUGIN_MODEL="${ACTAGENT_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e ACTAGENT_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${ACTAGENT_CODEX_NPM_PLUGIN_SPEC:?missing ACTAGENT_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${ACTAGENT_CODEX_NPM_PLUGIN_MODEL:?missing ACTAGENT_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="ACTAGENT-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${ACTAGENT_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  actagent_e2e_dump_logs \
    ...
    /tmp/actagent-codex-plugin-install.log \
    /tmp/actagent-codex-plugin-enable.log \
    /tmp/actagent-codex-plugins-list.json \
    /tmp/actagent-codex-plugin-inspect.json \
    /tmp/actagent-codex-preflight.log \
    /tmp/actagent-codex-agent.json \
    /tmp/actagent-codex-agent.err \
    /tmp/actagent-codex-agent-turn1.json \
    /tmp/actagent-codex-agent-turn1.err \
    /tmp/actagent-codex-agent-turn2.json \
    /tmp/actagent-codex-agent-turn2.err \
    /tmp/actagent-codex-plugin-uninstall.log \
    /tmp/actagent-codex-plugins-list-after-uninstall.json \
    /tmp/actagent-codex-agent-after-uninstall.json \
    /tmp/actagent-codex-agent-after-uninstall.err
}
...
actagent plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/actagent-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
actagent plugins enable codex >/tmp/actagent-codex-plugin-enable.log 2>&1

actagent plugins list --json >/tmp/actagent-codex-plugins-list.json
actagent plugins inspect codex --runtime --json >/tmp/actagent-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/actagent-codex-preflight.log 2>&1 </dev/null
...
echo "Running ACTAgent local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the ACTAgent Codex plugin." \
  /tmp/actagent-codex-agent-turn1.json \
  /tmp/actagent-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
actagent plugins uninstall codex --force >/tmp/actagent-codex-plugin-uninstall.log 2>&1
actagent plugins list --json >/tmp/actagent-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if actagent agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/actagent-codex-agent-after-uninstall.json 2>/tmp/actagent-codex-agent-after-uninstall.err; then
  echo "Expected ACTAgent agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/actagent-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/actagent-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  cat /tmp/actagent-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi

#!/usr/bin/env bash
# Installs an ACTAgent package candidate in Docker, performs Telegram
...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-npm-telegram-live-e2e" ACTAGENT_NPM_TELEGRAM_LIVE_E2E_IMAGE)"
...
OUTPUT_DIR="${ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live/$RUN_ID}"
...
validate_actagent_package_spec() {
  ...
  echo "ACTAGENT_NPM_TELEGRAM_PACKAGE_SPEC must be actagent@alpha, actagent@beta, actagent@latest, or an exact ACTAgent release version; got: $spec" >&2
  exit 1
}
...
validate_credential_preflight() {
  if [ "${ACTAGENT_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT:-0}" = "1" ]; then
    return 0
  fi
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    {
      ...
      echo "Use one of:"
      echo "  direct Telegram env: ACTAGENT_QA_TELEGRAM_GROUP_ID, ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN, ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN"
      echo "  Convex env: ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex plus ACTAGENT_QA_CONVEX_SITE_URL and a role secret"
    } >&2
    exit 1
  fi
}
...
run_log="$(mktemp "${TMPDIR:-/tmp}/actagent-npm-telegram-live.XXXXXX")"
...
docker_env=(
  ...
  -e ACTAGENT_E2E_COMMAND_TIMEOUT="${ACTAGENT_E2E_COMMAND_TIMEOUT:-300s}"
  -e ACTAGENT_NPM_TELEGRAM_PACKAGE_SPEC="$PACKAGE_SPEC"
  -e ACTAGENT_NPM_TELEGRAM_PACKAGE_LABEL="$PACKAGE_LABEL"
  -e ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"
  -e ACTAGENT_NPM_TELEGRAM_FAST="${ACTAGENT_NPM_TELEGRAM_FAST:-1}"
)
...
if [ -n "$credential_source" ]; then
  docker_env+=(-e ACTAGENT_QA_CREDENTIAL_SOURCE="$credential_source")
fi
if [ -n "$credential_role" ]; then
  docker_env+=(-e ACTAGENT_QA_CREDENTIAL_ROLE="$credential_role")
fi
...
for key in \
  ...
  ACTAGENT_QA_ALLOW_INSECURE_HTTP \
  ACTAGENT_QA_REDACT_PUBLIC_METADATA \
  ACTAGENT_QA_TELEGRAM_CAPTURE_CONTENT \
  ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS \
  ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS \
  ACTAGENT_QA_SUITE_PROGRESS \
  ACTAGENT_NPM_TELEGRAM_PROVIDER_MODE \
  ACTAGENT_NPM_TELEGRAM_MODEL \
  ACTAGENT_NPM_TELEGRAM_ALT_MODEL \
  ACTAGENT_NPM_TELEGRAM_SCENARIOS \
  ACTAGENT_NPM_TELEGRAM_SKIP_HOTPATH \
  ACTAGENT_NPM_TELEGRAM_SUT_ACCOUNT \
  ACTAGENT_NPM_TELEGRAM_ALLOW_FAILURES; do
...
export HOME="$(mktemp -d "/tmp/actagent-npm-telegram-install.XXXXXX")"
...
export HOME="$(mktemp -d "/tmp/actagent-npm-telegram-runtime.XXXXXX")"
export NPM_CONFIG_PREFIX="/npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export ACTAGENT_NPM_TELEGRAM_REPO_ROOT="/app"

dump_hotpath_logs() {
  local status="$1"
  echo "installed-package onboarding recovery hot path failed with exit code $status" >&2
  for file in \
    /tmp/actagent-npm-telegram-onboard.json \
    /tmp/actagent-npm-telegram-channel-add.log \
    /tmp/actagent-npm-telegram-doctor-fix.log \
    /tmp/actagent-npm-telegram-doctor-check.log; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
command -v actagent
actagent_e2e_run_command actagent --version
mkdir -p /app/node_modules
actagent_package_dir="/npm-global/lib/node_modules/actagent"
# The mounted QA harness imports actagent/plugin-sdk and package dependencies;
# point those imports at the installed package without copying source plugins into the test image.
rm -rf /app/node_modules/actagent
ln -sfnT "$actagent_package_dir" /app/node_modules/actagent
rm -rf /app/dist
ln -sfnT "$actagent_package_dir/dist" /app/dist
cp "$actagent_package_dir/package.json" /app/package.json
node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs \
  /app/package.json \
  /app/node_modules/actagent/package.json
for deps_dir in "$actagent_package_dir/node_modules" /npm-global/lib/node_modules; do
  [ -d "$deps_dir" ] || continue
...
link_installed_package_dependency() {
  local name="$1"
  local source="/npm-global/lib/node_modules/actagent/node_modules/$name"
...
if [ "${ACTAGENT_NPM_TELEGRAM_SKIP_HOTPATH:-0}" != "1" ]; then
  echo "Running installed-package onboarding recovery hot path..."
  hotpath_openai_api_key="${OPENAI_API_KEY:-sk-actagent-npm-telegram-hotpath}"
  OPENAI_API_KEY="$hotpath_openai_api_key" actagent_e2e_run_command actagent onboard \
    ...
    --skip-skills \
    --skip-health \
    --json >/tmp/actagent-npm-telegram-onboard.json </dev/null

  actagent_e2e_run_command actagent channels add --channel telegram --token "123456:actagent-npm-telegram-hotpath" >/tmp/actagent-npm-telegram-channel-add.log 2>&1 </dev/null
  actagent_e2e_run_command actagent doctor --fix --non-interactive >/tmp/actagent-npm-telegram-doctor-fix.log 2>&1 </dev/null
  actagent_e2e_run_command actagent doctor --non-interactive >/tmp/actagent-npm-telegram-doctor-check.log 2>&1 </dev/null
fi

export ACTAGENT_NPM_TELEGRAM_SUT_COMMAND="$(command -v actagent)"
trap - ERR

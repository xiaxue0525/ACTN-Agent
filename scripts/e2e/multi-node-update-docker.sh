...
# ACTAgent under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `actagent update`. Verifies that:
#
...
#
# Requires: Docker, a built actagent-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="actagent-multi-node-update-e2e"
DOCKER_RUN_TIMEOUT="${ACTAGENT_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
docker_e2e_build_or_reuse "$IMAGE_NAME" multi-node-update "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR" "bare" "${ACTAGENT_SKIP_DOCKER_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${ACTAGENT_CURRENT_PACKAGE_TGZ:-}")"
...
echo ""
echo "── Step 2: Install ACTAgent under node-A ──"

# Use node-A to install actagent with npm prefix A.
...
echo "Installing ACTAgent package under node-A prefix: $NPM_PREFIX_A"
actagent_e2e_install_package "$ARTIFACTS/install-a.log" "ACTAgent package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking actagent location..."

ACTAGENT_A="$(command -v actagent)"
echo "actagent binary: $ACTAGENT_A"
echo "actagent version: $(actagent --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/actagent"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/actagent-gateway.service"
...
if ! actagent gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but actagent is NOT installed there.
...
echo "which actagent: $(command -v actagent)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run actagent update (this is the bug) ──"
...
echo "Running actagent update --yes --json..."
UPDATE_EXIT=0
actagent update --yes --json \
  --tag /tmp/actagent-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: actagent update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but ACTAgent package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the ACTAgent package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/actagent/package.json" ]; then
  echo "WARNING: ACTAgent was ALSO installed under node-B prefix (split install)"
else
  echo "OK: ACTAgent is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell ACTAgent root differs" "$ARTIFACTS/update.err" 2>/dev/null; then

...
import type { ACTAgentConfig } from "../../dist/config/types.actagent.js";
...
function makeParams(commandBody: string, cfg: ACTAgentConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: ACTAgentConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("actagent-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.ACTAGENT_CONFIG_PATH ?? path.join(stateDir, "actagent.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env ACTAGENT_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/actagent-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/actagent-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as ACTAgentConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "ACTAGENT_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/actagent-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/actagent-work",
    ),

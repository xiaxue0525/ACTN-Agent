...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-release-upgrade-user-journey-e2e" ACTAGENT_RELEASE_UPGRADE_USER_JOURNEY_E2E_IMAGE)"
SKIP_BUILD="${ACTAGENT_RELEASE_UPGRADE_USER_JOURNEY_E2E_SKIP_BUILD:-0}"
run_log=""
cleanup() {
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz release-upgrade-user-journey "${ACTAGENT_CURRENT_PACKAGE_TGZ:-}")"
...
ACTAGENT_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 release-upgrade-user-journey empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e "ACTAGENT_RELEASE_UPGRADE_BASELINE_SPEC=${ACTAGENT_RELEASE_UPGRADE_BASELINE_SPEC:-actagent@latest}" \

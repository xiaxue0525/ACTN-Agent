// ACTAgent bundle MCP tools Docker harness.
...
server.tool("docker_probe", "Docker ACTAgent MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("actagent-agent-bundle-mcp-");

...
// Telegram User Credential script supports ACTAgent repository automation.
...
const COMMAND_TIMEOUT_MS = optionalPositiveInteger(
  process.env.ACTAGENT_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS?.trim(),
  120_000,
);
const BROKER_TIMEOUT_MS = optionalPositiveInteger(
  process.env.ACTAGENT_TELEGRAM_USER_CREDENTIAL_BROKER_TIMEOUT_MS?.trim(),
  30_000,
);
...
async function resolveConvexLeaseConfig(opts: Map<string, string>) {
  ...
  const siteUrl =
    opts.get("site-url") ||
    process.env.ACTAGENT_QA_CONVEX_SITE_URL?.trim() ||
    fileEnv.ACTAGENT_QA_CONVEX_SITE_URL;
  const token =
    opts.get("ci-secret") ||
    process.env.ACTAGENT_QA_CONVEX_SECRET_CI?.trim() ||
    fileEnv.ACTAGENT_QA_CONVEX_SECRET_CI;
  if (!siteUrl) {
    throw new Error("Missing ACTAGENT_QA_CONVEX_SITE_URL.");
  }
  if (!token) {
    throw new Error("Missing ACTAGENT_QA_CONVEX_SECRET_CI.");
  }
  return {
    siteUrl,
    token,
    leaseTtlMs: optionalPositiveInteger(
      opts.get("lease-ttl-ms") ||
        process.env.ACTAGENT_QA_CREDENTIAL_LEASE_TTL_MS?.trim() ||
        fileEnv.ACTAGENT_QA_CREDENTIAL_LEASE_TTL_MS,
      20 * 60 * 1_000,
    ),
    heartbeatIntervalMs: optionalPositiveInteger(
      opts.get("heartbeat-interval-ms") ||
        process.env.ACTAGENT_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS?.trim() ||
        fileEnv.ACTAGENT_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS,
      30_000,
    ),
    ownerId:
      opts.get("owner-id") ||
...
async function createTelegramUserPayload(opts: Map<string, string>) {
  ...
  const sutToken =
    process.env.ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN?.trim() ||
  ...
  const groupId =
    process.env.ACTAGENT_QA_TELEGRAM_GROUP_ID?.trim() ||
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-telegram-user-credential-"));
...
async function restoreTelegramUserPayload(params: {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-telegram-user-restore-"));
  const tdlibArchive = path.join(tempRoot, "tdlib.tgz");
  const desktopArchive = path.join(tempRoot, "desktop-tdata.tgz");

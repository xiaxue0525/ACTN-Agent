...
const groupId = process.env.ACTAGENT_QA_TELEGRAM_GROUP_ID;
const driverToken = process.env.ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN;
const sutToken = process.env.ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN;
const outputDir = process.env.ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR ?? ".artifacts/rtt/raw";
const telegramApiBaseUrl = (
  process.env.ACTAGENT_QA_TELEGRAM_API_BASE_URL ?? "https://api.telegram.org"
).replace(/\/+$/u, "");
const timeoutMs = readPositiveIntEnv("ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", 180000);
const canaryTimeoutMs = readPositiveIntEnv("ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS", timeoutMs);
const warmSampleCount = readPositiveIntEnv("ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES", 20);
const sampleTimeoutMs = readPositiveIntEnv("ACTAGENT_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS", 30000);
const botApiTimeoutMs = readPositiveIntEnv("ACTAGENT_NPM_TELEGRAM_BOT_API_TIMEOUT_MS", 30000);
const botApiBodyMaxBytes = readPositiveIntEnv(
  "ACTAGENT_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES",
  1024 * 1024,
);
const maxWarmFailures = readPositiveIntEnv("ACTAGENT_NPM_TELEGRAM_MAX_FAILURES", warmSampleCount);
const successMarker = process.env.ACTAGENT_NPM_TELEGRAM_SUCCESS_MARKER ?? "ACTAGENT_E2E_OK";
const scenarioIds = new Set(
  (process.env.ACTAGENT_NPM_TELEGRAM_SCENARIOS ?? "telegram-mentioned-message-reply")
    .split(",")
...
if (!groupId || !driverToken || !sutToken) {
  throw new Error(
    "missing Telegram env: ACTAGENT_QA_TELEGRAM_GROUP_ID, ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN, ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN",
  );
}

...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".actagent", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "actagent.runtime-context",
  ...
  assert(
    !userText.includes("ACTAGENT_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-session-runtime-context-"));
  process.env.HOME = root;
  process.env.ACTAGENT_STATE_DIR = path.join(root, ".actagent");
  process.env.ACTAGENT_CONFIG_PATH = path.join(process.env.ACTAGENT_STATE_DIR, "actagent.json");
  try {
    ...
    if (process.env.ACTAGENT_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {

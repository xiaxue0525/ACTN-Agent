...
IMAGE_NAME="$(docker_e2e_resolve_image "actagent-live-plugin-tool-e2e" ACTAGENT_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${ACTAGENT_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${ACTAGENT_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${ACTAGENT_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${ACTAGENT_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${ACTAGENT_TESTBOX_PROFILE_FILE:-$HOME/.actagent-testbox-live.profile}}"
run_log=""
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ACTAGENT_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ACTAGENT_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires ACTAGENT_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
ACTAGENT_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e ACTAGENT_LIVE_PLUGIN_TOOL_MODEL="${ACTAGENT_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${ACTAGENT_LIVE_PLUGIN_TOOL_MODEL:?missing ACTAGENT_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@actagent/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="ACTAgent E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  actagent_e2e_dump_logs \
    ...
    /tmp/actagent-plugin-install.log \
    /tmp/actagent-plugin-enable.log \
    /tmp/actagent-plugins-list.json \
    /tmp/actagent-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/actagent-live-plugin-tool.XXXXXX)"
...
actagent plugins install "npm-pack:$plugin_tgz" --force >/tmp/actagent-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
actagent plugins enable "$PLUGIN_ID" >/tmp/actagent-plugin-enable.log 2>&1
actagent plugins list --json >/tmp/actagent-plugins-list.json
actagent plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/actagent-plugin-inspect.json
...
actagent agent --local \
  ...
  --thinking off \
  --timeout "${ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/actagent-agent.json 2>/tmp/actagent-agent.err

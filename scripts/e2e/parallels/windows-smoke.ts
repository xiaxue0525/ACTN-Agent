...
// Windows Smoke script supports ACTAgent repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://actagent.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-actagent-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'ACTAgent\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-actagent-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://actagent.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run actagent update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("ACTAGENT_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("actagent-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("actagent-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("actagent-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsACTAgentResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanACTAgent: boolean): void {
    const cleanScript = cleanACTAgent
      ? "npm.cmd uninstall -g actagent --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "actagent --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-ACTAgent --version");
...
Invoke-ACTAgent onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 19199 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "actagent onboard failed with exit code $LASTEXITCODE" }
${this.windowsPluginIsolationScript()}`,
  ...
  private async guestPowerShellBackground(
    ...
    await runWindowsBackgroundPowerShell({
      ...
      label,
      onLaunchRetry: warn,
      script: `${windowsACTAgentResolver}\n${script}`,
      timeoutMs,
...
${windowsScopedEnvFunction}
$script:ACTAgentUpdateExit = 0
Invoke-WithScopedEnv @{ ACTAGENT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; ACTAGENT_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-ACTAgent update --channel dev --yes --json
  $script:ACTAgentUpdateExit = $LASTEXITCODE
}
if ($script:ACTAgentUpdateExit -ne 0) { throw "actagent update failed with exit code $script:ACTAgentUpdateExit" }
Invoke-ACTAgent --version
Invoke-ACTAgent update status --json`,
...
Invoke-ACTAgent update status --json`,
  ...
  private gatewayAction(action: "restart" | "stop"): Promise<void> {
    return this.guestPowerShellBackground(
      ...
      `$ErrorActionPreference = 'Continue'
$PSNativeCommandUseErrorActionPreference = $false
Invoke-ACTAgent gateway ${action}
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-ACTAgent gateway probe --url ws://127.0.0.1:19199 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-ACTAgent gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-ACTAgent gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-ACTAgent gateway status ${suffix}`);
  }
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $output = Invoke-ACTAgent @args 2>&1
...
if (-not $agentOk) { throw 'actagent agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastACTAgentVersion(this.runDir, phaseName, /ACTAgent\s+([0-9][^\s]*)/gi);
  }

// Package Artifact script supports ACTAgent repository automation.
...
export function resolveACTAgentRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("actagent@") ? rawValue.slice("actagent@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`actagent@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "actagent", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no actagent registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packACTAgent(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "actagent-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `actagent-main-${shortHead}.tgz`);
...
async function acquirePackageLock(lockDir: string, ownerToken: string): Promise<void> {
  const timeoutMs = readPositiveIntEnv("ACTAGENT_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("ACTAGENT_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);

// Macos Discord script supports ACTAgent repository automation.
...
export class MacosDiscordSmoke {
  constructor(
    private input: {
      ...
      guest: MacosGuest;
      guestNode: string;
      guestACTAgent: string;
      guestACTAgentEntry: string;
      runDir: string;
      vmName: string;
    },
  ) {}
...
${this.input.guestNode} ${this.input.guestACTAgentEntry} config set channels.discord.token ${shellQuote(this.input.config.token)}
${this.input.guestNode} ${this.input.guestACTAgentEntry} config set channels.discord.enabled true
${this.input.guestNode} ${this.input.guestACTAgentEntry} config set channels.discord.groupPolicy allowlist
${this.input.guestNode} ${this.input.guestACTAgentEntry} config set channels.discord.guilds ${shellQuote(guilds)} --strict-json
${this.input.guestNode} ${this.input.guestACTAgentEntry} doctor --fix --yes --non-interactive
...
JS
${this.input.guestNode} ${this.input.guestACTAgentEntry} plugins enable discord
${this.input.guestNode} ${this.input.guestACTAgentEntry} gateway restart
${this.input.guestNode} ${this.input.guestACTAgentEntry} channels status --probe --json`);
  }

...
// Macos Smoke script supports ACTAgent repository automation.
...
const guestACTAgent = "actagent";
const guestACTAgentEntry = '"$(npm root -g)/actagent/actagent.mjs"';
const guestACTAgentEntryRunner = `node ${guestACTAgentEntry}`;
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://actagent.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://actagent.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("actagent-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("actagent-parallels-macos-tgz.");
    try {
      ...
      if (await this.needsHostTgz()) {
        this.artifact = await packACTAgent({
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("actagent-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("actagent-main-upgrade.tgz"),
      );
  ...
  private guestACTAgentEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/actagent/actagent.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'actagent.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'actagent-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'actagent.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g actagent >/dev/null 2>&1 || true
rm -rf "$HOME/.actagent"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/actagent-install.sh
bash /tmp/actagent-install.sh --version ${shellQuote(this.installVersion)}
${guestACTAgent} --version`,
    );
  }
...
${guestACTAgent} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestACTAgent, "--version"]);
...
check_path "$root/actagent"
check_path "$root/actagent/extensions"
if [ -d "$root/actagent/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/actagent/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/actagent-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/actagent`)}
export PATH=${shellQuote(`/tmp/actagent-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".actagent", "actagent.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 ACTAGENT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ACTAGENT_DISABLE_BUNDLED_PLUGINS=1 ${guestACTAgentEntryRunner} update --channel dev --yes --json
${guestACTAgentEntryRunner} --version
${guestACTAgentEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestACTAgentEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'actagent.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'actagent-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'actagent.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} ACTAGENT_HOME=${shellQuote(home)} ACTAGENT_STATE_DIR=${shellQuote(`${home}/.actagent`)} ACTAGENT_CONFIG_PATH=${shellQuote(
        `${home}/.actagent/actagent.json`,
      )} ${guestACTAgentEntryRunner} gateway run --bind loopback --port 19199 --force </dev/null >/tmp/actagent-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestACTAgent(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestACTAgent, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestACTAgent(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestACTAgent(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`actagent ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:19199/ >/tmp/actagent-dashboard-smoke.html 2>/dev/null; then
    grep -F '<title>ACTAgent Control</title>' /tmp/actagent-dashboard-smoke.html >/dev/null &&
      grep -F '<actagent-app></actagent-app>' /tmp/actagent-dashboard-smoke.html >/dev/null &&
      exit 0
  fi
  sleep 1
done
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestACTAgentEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestACTAgentEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestACTAgentEntryExec([
      "config",
      "set",
    ...
    this.guestACTAgentEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestACTAgentEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestACTAgentEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    ...
    const matches = [...log.matchAll(/ACTAgent\s+([0-9][^\s]*)/gi)];

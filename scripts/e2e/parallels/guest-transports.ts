// Guest Transports script supports ACTAgent repository automation.
...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `actagent-parallels-${nonce}`;
...
${pathsScript}
function Add-ACTAgentBackgroundLog {
...
try {
  & {
${options.script}
  } *>&1 | Add-ACTAgentBackgroundLog
  Set-Content -Path $exitPath -Value '0' -Encoding UTF8
} catch {
  $_ | Add-ACTAgentBackgroundLog
...
if (Test-Path $logPath) {
  ...
  try {
    ...
    "__ACTAGENT_LOG_LENGTH__:$length"
    if ($length -gt $offset) {
      ...
      if ($read -gt 0) {
        ...
        "__ACTAGENT_LOG_OFFSET__:$nextOffset"
...
if (Test-Path $donePath) {
  ...
  "__ACTAGENT_BACKGROUND_EXIT__:$backgroundExit"
  ...
  exit 0
      ...
      const offsetMatch = poll.stdout.match(/__ACTAGENT_LOG_OFFSET__:(\d+)/);
      if (offsetMatch) {
        lastLogOffset = Number(offsetMatch[1]);
      }
      const lengthMatch = poll.stdout.match(/__ACTAGENT_LOG_LENGTH__:(\d+)/);
      const logLength = lengthMatch ? Number(lengthMatch[1]) : lastLogOffset;
      if (poll.stdout.includes("__ACTAGENT_BACKGROUND_DONE__")) {
        doneSeen = true;
        ...
        const exitMatch = poll.stdout.match(/__ACTAGENT_BACKGROUND_EXIT__:(\S+)/);
...
function cleanupWindowsBackground(
  ...
  const stopProcessTree = options.stopProcessTree
    ? `function Stop-ACTAgentBackgroundProcessTree([int]$ProcessId) {
  Get-CimInstance Win32_Process -Filter "ParentProcessId=$ProcessId" -ErrorAction SilentlyContinue | ForEach-Object {
    Stop-ACTAgentBackgroundProcessTree ([int]$_.ProcessId)
  }
...
if (Test-Path $pidPath) {
  ...
  if ($backgroundPid) {
    Stop-ACTAgentBackgroundProcessTree ([int]$backgroundPid)
  }
}
`
    : "";
...
export class LinuxGuest {
  ...
  exec(args: string[], options: GuestExecOptions = {}): string {
    const result = run(
      "prlctl",
      ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "ACTAGENT_ALLOW_ROOT=1", ...args],
      {
        check: false,
        input: options.input,
        quiet: true,
  ...
  bash(script: string): string {
    const scriptPath = `/tmp/actagent-parallels-${process.pid}-${Date.now()}.sh`;
    const write = run(
      "prlctl",
      [
        "exec",
...
export class WindowsGuest {
  ...
  powershell(script: string, options: GuestExecOptions = {}): string {
    const scriptName = `actagent-parallels-${process.pid}-${Date.now()}.ps1`;

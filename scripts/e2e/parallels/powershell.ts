// Powershell script supports ACTAgent repository automation.
import {
  configPathMapKey,
...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'actagent-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-ACTAgent config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:ACTAGENT_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.actagent\\actagent.json' }
$agentTurnVersionText = Invoke-ACTAgent --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'ACTAgent\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:ACTAGENT_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:ACTAGENT_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:ACTAGENT_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'actagent-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.ACTAGENT_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.ACTAGENT_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.ACTAGENT_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@actagent", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "actagent" };
  } else {
...
Remove-Item Env:ACTAGENT_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:ACTAGENT_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:ACTAGENT_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsACTAgentResolver = String.raw`function Resolve-ACTAgentCommand {
  if ($script:ACTAgentResolvedCommand) { return $script:ACTAgentResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\actagent.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\actagent.ps1'
  }
  foreach ($name in @('actagent.cmd', 'actagent.ps1', 'actagent')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'actagent.cmd'
    $shimCandidates += Join-Path $npmPrefix 'actagent.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:ACTAgentResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:ACTAgentResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\actagent\actagent.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:ACTAgentResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:ACTAgentResolvedCommand
    }
  }
  throw 'actagent command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-ACTAgent {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $ACTAgentArgs)
  $command = Resolve-ACTAgentCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @ACTAgentArgs
    } else {
      & $command.Path @ACTAgentArgs
    }
  } finally {

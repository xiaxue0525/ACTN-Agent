// Plugin Isolation script supports ACTAgent repository automation.
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `ACTAGENT_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:ACTAGENT_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'actagent-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:ACTAGENT_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.ACTAGENT_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.ACTAGENT_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.ACTAGENT_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@actagent", "codex"), {
  recursive: true,
  force: true,
});

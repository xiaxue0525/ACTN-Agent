...
#
# Suggested cron: */30 * * * * /home/admin/actagent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ACTAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., actagent-alerts)
...
STATE_FILE="$HOME/.actagent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ACTAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use actagent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ACTAgent to $NOTIFY_PHONE..."
            actagent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ACTAgent is down. Run: ssh l36 '~/actagent/scripts/mobile-reauth.sh'" "urgent"
    exit 1

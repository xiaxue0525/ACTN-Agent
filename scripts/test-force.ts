...
// Test Force script supports ACTAgent repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured ACTAgent gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.ACTAGENT_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `actagent-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      ACTAGENT_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {

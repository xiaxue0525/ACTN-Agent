// Bench Cli Startup script supports ACTAgent repository automation.
...
const DEFAULT_ENTRY = "actagent.mjs";
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "ACTAGENT_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "ACTAGENT_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.ACTAGENT_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "actagent-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".actagent");
...
function printUsage(): void {
  console.log(`ACTAgent CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: actagent.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "actagent-cli-bench-"));

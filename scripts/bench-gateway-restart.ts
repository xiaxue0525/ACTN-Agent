// Bench Gateway Restart script supports ACTAgent repository automation.
...
function printUsage(): void {
  console.log(`ACTAgent Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "actagent.json");
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "actagent-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "actagent-bench",
    npm_config_update_notifier: "false",
    ACTAGENT_CONFIG: configPath,
    ...
    ACTAGENT_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "actagent-gateway-restart-bench-"));

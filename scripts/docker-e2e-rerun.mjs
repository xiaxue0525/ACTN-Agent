...
const DEFAULT_WORKFLOW = "actagent-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef = process.env.ACTAGENT_DOCKER_E2E_WORKFLOW_REF || process.env.GITHUB_REF_NAME;
...
function main() {
  ...
  if (isLocalJson) {
    ...
    const outputDir =
      options.dir || path.join(os.tmpdir(), `actagent-docker-e2e-rerun-${options.input}`);

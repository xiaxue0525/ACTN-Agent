...
# ACTAgent Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one ACTAgent."
...
resolve_actagent_effective_home() {
    local actagent_home="${ACTAGENT_HOME:-}"
    if [[ -z "$actagent_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$actagent_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$actagent_home" == \~/* ]]; then
        echo "${HOME}${actagent_home:1}"
        return
    fi
    echo "$actagent_home"
}
...
GUM_VERSION="${ACTAGENT_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🐲 ACTAgent Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://actagent.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ACTAGENT_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.actagent.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_actagent_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.actagent-* "$npm_root"/actagent 2>/dev/null || true
}

extract_actagent_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_actagent_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/actagent" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/actagent"|"/usr/local/bin/actagent")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/actagent/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale actagent symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing actagent binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing ACTAgent package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing ACTAgent package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_actagent_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "ACTAgent npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*actagent" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_actagent_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_actagent_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "ACTAgent npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an actagent binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES=()
TAGLINES+=("Your terminal just grew actagents—type something and let the bot pinch the busywork.")
...
TAGLINES+=("Automation with actagents: minimal fuss, maximal pinch.")
...
TAGLINES+=("actagents out, commit in—let's ship something mildly responsible.")
...
HOLIDAY_CHRISTMAS="Christmas: Ho ho ho—ACTAgent Assistant is here to ship joy, roll back chaos, and stash the keys safely."
...
pick_tagline() {
    ...
    if [[ -n "${ACTAGENT_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ACTAGENT_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ACTAGENT_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ACTAGENT_NO_ONBOARD:-0}
NO_PROMPT=${ACTAGENT_NO_PROMPT:-0}
DRY_RUN=${ACTAGENT_DRY_RUN:-0}
INSTALL_METHOD=${ACTAGENT_INSTALL_METHOD:-}
ACTAGENT_VERSION=${ACTAGENT_VERSION:-latest}
USE_BETA=${ACTAGENT_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_actagent_effective_home)/actagent"
GIT_DIR=${ACTAGENT_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ACTAGENT_GIT_UPDATE:-1}
NPM_LOGLEVEL="${ACTAGENT_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ACTAGENT_VERBOSE:-0}"
VERIFY_INSTALL="${ACTAGENT_VERIFY_INSTALL:-0}"
ACTAGENT_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
ACTAgent installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/actagent)
...
Environment variables:
  ACTAGENT_INSTALL_METHOD=git|npm
  ACTAGENT_VERSION=latest|next|<semver>|<spec>
  ACTAGENT_BETA=0|1
  ACTAGENT_GIT_DIR=...
  ACTAGENT_GIT_UPDATE=0|1
  ACTAGENT_NO_PROMPT=1
  ACTAGENT_VERIFY_INSTALL=1
  ACTAGENT_DRY_RUN=1
  ACTAGENT_NO_ONBOARD=1
  ACTAGENT_VERBOSE=1
  ACTAGENT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected ACTAgent checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a ACTAgent source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_actagent_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"actagent"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://actagent.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future ACTAgent updates; use npm i -g actagent@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_actagent_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/actagent" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/actagent" ]]; then
        ln -sf "$npm_root/actagent/dist/entry.js" "${npm_bin}/actagent"
        ui_info "Created actagent bin link at ${npm_bin}/actagent"
    fi
    return 0
}

# Check for existing ACTAgent installation
check_existing_actagent() {
    if [[ -n "$(type -P actagent 2>/dev/null || true)" ]]; then
        ui_info "Existing ACTAgent installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_actagent_ref() {
    local requested="${ACTAGENT_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "actagent" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_actagent_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${ACTAGENT_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_actagent_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/actagent"
        ...
        local version=""
        version="$(actagent_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_actagent_npm_root_candidates)
}

warn_duplicate_actagent_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_actagent_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple ACTAgent global installs detected"
    echo "  Different Node/npm environments can run different ACTAgent versions."

    local active_node active_npm active_actagent
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_actagent="${ACTAGENT_BIN:-}"
    if [[ -z "$active_actagent" ]]; then
        active_actagent="$(type -P actagent 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active actagent: ${INFO}${active_actagent:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make actagent show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

actagent_command_for_user() {
    local actagent="${1:-}"
    if [[ -z "$actagent" ]]; then
        echo "actagent"
        return 0
    ...
    local actagent_dir="${actagent%/*}"
    if [[ "$actagent_dir" != "$actagent" ]] && path_has_dir "$ORIGINAL_PATH" "$actagent_dir"; then
        echo "actagent"
        return 0
    fi

    local quoted_actagent=""
    printf -v quoted_actagent '%q' "$actagent"
    echo "$quoted_actagent"
}
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${ACTAGENT_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_actagent_not_found() {
    ui_warn "Installed, but actagent is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t actagent 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named actagent; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_actagent_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P actagent 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/actagent" ]]; then
        echo "${npm_bin}/actagent"
        return 0
    fi
...
install_actagent_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/actagent/actagent.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing ACTAgent from git checkout: ${repo_dir}"
    else
        ui_info "Installing ACTAgent from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning ACTAgent" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_actagent_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_actagent_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building ACTAgent" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/actagent" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/actagent"
    ui_success "ACTAgent wrapper installed to \$HOME/.local/bin/actagent"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view actagent dist-tags.beta 2>/dev/null || true)"
...
is_actagent_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#actagent@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:actagent/actagent($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/actagent/actagent(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]actagent/actagent(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/actagent/actagent(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:actagent/actagent(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:actagent/actagent#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_actagent() {
    local package_name="actagent"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ACTAGENT_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="actagent"
        else
            ACTAGENT_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ACTAGENT_VERSION}" ]]; then
        ACTAGENT_VERSION="latest"
    fi

    if is_actagent_source_package_install_spec "${ACTAGENT_VERSION}"; then
        ui_error "npm installs do not support ACTAgent GitHub source targets like '${ACTAGENT_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${ACTAGENT_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${ACTAGENT_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing ACTAgent v${resolved_version}"
    else
        ui_info "Installing ACTAgent (${ACTAGENT_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${ACTAGENT_VERSION}")"

    if ! install_actagent_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_actagent_paths
        install_actagent_npm "${install_spec}"
    fi

    if [[ "${ACTAGENT_VERSION}" == "latest" && "${package_name}" == "actagent" ]]; then
        if ! resolve_actagent_bin &> /dev/null; then
            ui_warn "npm install actagent@latest failed; retrying actagent@next"
            cleanup_npm_actagent_paths
            install_actagent_npm "actagent@next"
        fi
    fi

    ensure_actagent_bin_link || true

    ui_success "ACTAgent installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local actagent="${ACTAGENT_BIN:-}"
    if [[ -z "$actagent" ]]; then
        actagent="$(resolve_actagent_bin || true)"
    fi
    if [[ -z "$actagent" ]]; then
        ui_info "Skipping doctor (actagent not on PATH yet)"
        warn_actagent_not_found
        return 0
    fi
    run_quiet_step "Running doctor" "$actagent" doctor --non-interactive || true
...
maybe_open_dashboard() {
    ...
    if ! "$actagent" dashboard --help >/dev/null 2>&1; then
        return 0
    fi
    "$actagent" dashboard || true
}

resolve_workspace_dir() {
    local profile="${ACTAGENT_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_actagent_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.actagent/workspace-${profile}"
    else
        echo "${effective_home}/.actagent/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_actagent_effective_home)"
    local config_path="${ACTAGENT_CONFIG_PATH:-$effective_home/.actagent/actagent.json}"
    local legacy_config_path="${HOME}/.actagent/actagent.json"
    local legacy_actagentdbot_path="${HOME}/.actagentdbot/actagentdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.actagentdbot/actagentdbot.json" ]]; then
        return
    fi
    if [[ -z "${ACTAGENT_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
        if [[ -f "$legacy_config_path" || -f "$legacy_actagentdbot_path" ]]; then
            return
        fi
    fi
    ...
    if ! is_promptable; then
        local user_actagent
        user_actagent="$(actagent_command_for_user "${ACTAGENT_BIN:-}")"
        ui_info "BOOTSTRAP.md found but no TTY; run ${user_actagent} onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$actagent" ]]; then
        ui_info "BOOTSTRAP.md found but actagent not on PATH; skipping onboarding"
        warn_actagent_not_found
        return
    fi
    ...
    "$actagent" onboard || {
        local user_actagent
        user_actagent="$(actagent_command_for_user "$actagent")"
        ui_error "Onboarding failed; run ${user_actagent} onboard to retry"
        return
    }
}
...
if ! declare -F extract_actagent_semver >/dev/null 2>&1; then
...
resolve_actagent_version() {
    ...
    local actagent="${ACTAGENT_BIN:-}"
    if [[ -z "$actagent" ]] && command -v actagent &> /dev/null; then
        actagent="$(command -v actagent)"
    fi
    if [[ -n "$actagent" ]]; then
        raw_version_output=$("$actagent" --version 2>/dev/null || true)
        raw_version_output="${raw_version_output%%$'\n'*}"
        raw_version_output="${raw_version_output//$'\r'/}"
        version="$(extract_actagent_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/actagent/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/actagent/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "actagent daemon status --json" "$actagent" daemon status --json || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if ! is_gateway_daemon_loaded "$actagent"; then
        return 0
    fi

    ui_info "Refreshing loaded gateway service"
    if run_quiet_step "Refreshing gateway service" "$actagent" gateway install --force; then
    ...
    if run_quiet_step "Restarting gateway service" "$actagent" gateway restart; then
    ...
    run_quiet_step "Probing gateway service" "$actagent" gateway status --deep || true
}
...
verify_installation() {
    ...
    if [[ -z "$actagent" ]]; then
        ui_error "Install verify failed: actagent not on PATH yet"
        warn_actagent_not_found
        return 1
    fi

    run_quiet_step "Checking ACTAgent version" "$actagent" --version || return 1

    if is_gateway_daemon_loaded "$actagent"; then
        run_quiet_step "Checking gateway service" "$actagent" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: actagent gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_actagent_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found ACTAgent checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ACTAGENT_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_actagent; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing ACTAgent"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g actagent &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g actagent 2>/dev/null || true
        ...
        install_actagent_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/actagent" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/actagent"
    ...
    ACTAGENT_BIN="$(resolve_actagent_bin || true)"
    warn_duplicate_actagent_global_installs || true
    ...
    installed_version=$(resolve_actagent_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐲 ACTAgent installed successfully (${installed_version})!"
    else
        ui_celebrate "🐲 ACTAgent installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "The lobster has molted. Harder shell, sharper actagents."
        ...
        local completion_messages=(
            ...
            "cracks actagents Alright, what are we building?"
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/actagent"
        ui_kv "Update command" "actagent update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running actagent doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ACTAGENT_UPDATE_IN_PROGRESS=1 "$actagent" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                ACTAGENT_UPDATE_IN_PROGRESS=1 "$actagent" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ACTAGENT_UPDATE_IN_PROGRESS=1 "$actagent" plugins update --all || true
            else
            ...
            local user_actagent
            user_actagent="$(actagent_command_for_user "${ACTAGENT_BIN:-}")"
            ui_info "No TTY; run ${user_actagent} doctor and ${user_actagent} plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            local user_actagent
            user_actagent="$(actagent_command_for_user "${ACTAGENT_BIN:-}")"
            ui_info "Skipping onboard (requested); run ${user_actagent} onboard later"
        else
            local effective_home
            effective_home="$(resolve_actagent_effective_home)"
            local config_path="${ACTAGENT_CONFIG_PATH:-$effective_home/.actagent/actagent.json}"
            ...
            if is_promptable; then
                ...
                if [[ -z "$actagent" ]]; then
                    ui_info "Skipping onboarding (actagent not on PATH yet)"
                    warn_actagent_not_found
                    return 0
                fi
                exec </dev/tty
                exec "$actagent" onboard
            fi
            local user_actagent
            user_actagent="$(actagent_command_for_user "${ACTAGENT_BIN:-}")"
            ui_info "No TTY; run ${user_actagent} onboard to finish setup"
            return 0
        fi
    fi

    if command -v actagent &> /dev/null; then
        local actagent="${ACTAGENT_BIN:-}"
        if [[ -z "$actagent" ]]; then
            actagent="$(resolve_actagent_bin || true)"
        fi
        if [[ -n "$actagent" ]] && is_gateway_daemon_loaded "$actagent"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (actagent daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ACTAGENT_UPDATE_IN_PROGRESS=1 "$actagent" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: actagent daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ACTAGENT_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main

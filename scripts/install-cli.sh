...
# ACTAgent CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://actagent.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
resolve_actagent_effective_home() {
  ...
  case "$actagent_home" in
    \~)
      echo "$HOME"
      ;;
    \~/*)
      echo "${HOME}/${actagent_home#~/}"
      ;;
    *)
      echo "$actagent_home"
      ;;
...
ACTAGENT_EFFECTIVE_HOME="$(resolve_actagent_effective_home)"
PREFIX="${ACTAGENT_PREFIX:-${HOME}/.actagent}"
ACTAGENT_VERSION="${ACTAGENT_VERSION:-latest}"
NODE_VERSION="${ACTAGENT_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${ACTAGENT_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${ACTAGENT_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${ACTAGENT_INSTALL_METHOD:-npm}"
GIT_DIR="${ACTAGENT_GIT_DIR:-${ACTAGENT_EFFECTIVE_HOME}/actagent}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.actagent; use \$ACTAGENT_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/actagent, or \$ACTAGENT_HOME/actagent)
  --version <ver>                     ACTAgent version (default: latest)
...
Environment variables:
  ...
  ACTAGENT_HOME=...
  ACTAGENT_PREFIX=...
  ACTAGENT_VERSION=latest|next|<semver>
  ACTAGENT_GIT_DIR=...
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${ACTAGENT_GIT_DIR:-${ACTAGENT_EFFECTIVE_HOME}/actagent}}"
...
resolve_git_actagent_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "actagent" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_actagent() {
  local requested="${ACTAGENT_VERSION:-latest}"
  if is_actagent_source_package_install_spec "$requested"; then
    fail "npm installs do not support ACTAgent GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"actagent\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing ACTAgent (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "actagent@latest"; then
      log "npm install actagent@latest failed; retrying actagent@next"
      emit_json "{\"event\":\"step\",\"name\":\"actagent\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "actagent@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "actagent@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/actagent"
  cat > "${PREFIX}/bin/actagent" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/actagent/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/actagent"
  emit_json "{\"event\":\"step\",\"name\":\"actagent\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_actagent_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"actagent\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing ACTAgent from git checkout: ${repo_dir}"
  else
    log "Installing ACTAgent from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/actagent"
  emit_json "{\"event\":\"step\",\"name\":\"actagent\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_actagent_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/actagent" ]]; then
    version="$("${PREFIX}/bin/actagent" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
is_gateway_daemon_loaded() {
  local actagent="$1"
  if [[ -z "$actagent" || ! -x "$actagent" ]]; then
    return 1
  fi

  local status_json=""
  status_json="$("$actagent" daemon status --json 2>/dev/null || true)"
...
refresh_gateway_service_if_loaded() {
  local actagent="${PREFIX}/bin/actagent"
  if [[ ! -x "$actagent" ]]; then
    return 0
  fi
  ...
  if ! "$actagent" gateway install --force >/dev/null 2>&1; then
  ...
  if ! "$actagent" gateway restart >/dev/null 2>&1; then
  ...
  "$actagent" gateway status --probe --json >/dev/null 2>&1 || true
...
main() {
  ...
  if [[ "${ACTAGENT_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_actagent_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "ACTAgent installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "ACTAgent installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/actagent" onboard
  fi
}

if [[ "${ACTAGENT_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi

...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[actagent] warning: detected ${packageManager} for install lifecycle.`,
    "[actagent] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[actagent] prefer: corepack pnpm install",
  ].join("\n");
}

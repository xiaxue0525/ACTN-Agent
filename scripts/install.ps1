# ACTAgent Installer for Windows
# Usage: powershell -c "irm https://actagent.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://actagent.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "ACTAgent installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  ACTAgent Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ACTAGENT_INSTALL_METHOD)) {
        $InstallMethod = $env:ACTAGENT_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ACTAGENT_GIT_DIR)) {
        $GitDir = $env:ACTAGENT_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:ACTAGENT_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:ACTAGENT_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:ACTAGENT_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "actagent")
}
...
function Get-ACTAgentDepsRoot {
    ...
    return (Join-Path $localAppData "ACTAgent\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-ACTAgentDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or actagent is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingACTAgent {
    if (Get-ACTAgentCommandPath) {
        Write-Host "[*] Existing ACTAgent installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-ACTAgentDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("actagent-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install ACTAgent." -ForegroundColor Red
...
function Get-ACTAgentCommandPath {
    $actagentCmd = Get-Command actagent.cmd -ErrorAction SilentlyContinue
    if ($actagentCmd -and $actagentCmd.Source) {
        return $actagentCmd.Source
    }

    $actagent = Get-Command actagent -ErrorAction SilentlyContinue
    if ($actagent -and $actagent.Source) {
        return $actagent.Source
    }

    return $null
}

function Invoke-ACTAgentCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-ACTAgentCommandPath
    if (-not $commandPath) {
        throw "actagent command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveACTAgentCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "actagent $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-ACTAgentOnPath {
    if (Get-ACTAgentCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "actagent.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] actagent is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmACTAgentInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-ACTAgentSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("actagent@")) {
        $normalizedTag = $normalizedTag.Substring("actagent@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:actagent/actagent($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/actagent/actagent(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]actagent/actagent(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/actagent/actagent(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:actagent/actagent(\.git)?($|[?#])'
    )
}
...
function Install-ACTAgent {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-ACTAgentSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support ACTAgent GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use actagent package for beta, actagent for stable
    $packageName = "actagent"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "actagent"
    }
    $installSpec = Resolve-NpmACTAgentInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing ACTAgent ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://actagent.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] ACTAgent installed" -ForegroundColor Green
    return $true
}

# Install ACTAgent from GitHub
function Install-ACTAgentFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/actagent/actagent.git"
    Write-Host "[*] Installing ACTAgent from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] ACTAgent build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "actagent.cmd"
    ...
    Write-Host "[OK] ACTAgent wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-ACTAgentCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-ACTAgentCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-ACTAgentCommandPath)) {
        return
    }
    ...
    try {
        Invoke-ACTAgentCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-ACTAgentCommand gateway restart | Out-Null
        Invoke-ACTAgentCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:ACTAGENT_GIT_DIR)) {
        return $env:ACTAGENT_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "actagent")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingACTAgent
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "actagent") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-ACTAgentFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "actagent.cmd"
        ...
        $npmInstallResults = @(Install-ACTAgent)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-ACTAgentOnPath)) {
        Write-Host "Install completed, but ACTAgent is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: actagent doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-ACTAgentCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.actagent -and $npmList.dependencies.actagent.version) {
                $installedVersion = $npmList.dependencies.actagent.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "ACTAgent installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "ACTAgent installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\actagent.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "actagent doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "actagent onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveACTAgentCommand onboard
        }
    }

// Plugin Npm Release script supports ACTAgent repository automation.
...
import { parseReleaseVersion } from "../actagent-npm-release-check.ts";
...
export const ACTAGENT_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/actagent/actagent";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.actagent?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.actagent?.extensions ?? [];

  if (!packageName.startsWith("@actagent/")) {
    errors.push(
      `package name must start with "@actagent/"; found "${packageName || "<missing>"}".`,
    );
  }
  if (packageJson.private === true) {
    errors.push("package.json private must not be true.");
  }
  if (repositoryUrl !== ACTAGENT_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${ACTAGENT_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("actagent.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("actagent.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("actagent.install.npmSpec must be a non-empty string for publishable plugins.");
  }
  errors.push(
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.actagent?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "actagent-plugin-npm-view-"));

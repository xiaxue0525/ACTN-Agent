// Release Beta Verifier script supports ACTAgent repository automation.
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { readBoundedResponseText } from "./bounded-response.ts";
import { collectACTAgentHubPublishablePluginPackages } from "./plugin-actagenthub-release.ts";
import {
...
export type ReleaseVerifyBetaArgs = {
  ...
  skipACTAgentHub: boolean;
  rerunFailedACTAgentHub: boolean;
  workflowRuns: {
    fullReleaseValidation?: string;
    actagentNpm?: string;
    pluginNpm?: string;
    pluginACTAgentHub?: string;
    npmTelegram?: string;
  };
};
...
const DEFAULT_REPO = "actagent/actagent";
const DEFAULT_ACTAGENTHUB_REGISTRY = "https://actagenthub.ai";
const ACTAGENTHUB_REQUEST_TIMEOUT_MS = 20_000;
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--full-release-validation-run ID] [--actagent-npm-run ID] [--plugin-npm-run ID] [--plugin-actagenthub-run ID] [--npm-telegram-run ID] [--skip-actagenthub]",
    );
  }
  ...
  const parsed: ReleaseVerifyBetaArgs = {
    ...
    distTag: "beta",
    repo: DEFAULT_REPO,
    registry: DEFAULT_ACTAGENTHUB_REGISTRY,
    workflowRef: undefined,
    pluginSelection: [],
    evidenceOut: undefined,
    skipPostpublish: false,
    skipACTAgentHub: false,
    rerunFailedACTAgentHub: false,
    workflowRuns: {},
  };
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--actagent-npm-run":
        parsed.workflowRuns.actagentNpm = next();
        break;
      case "--plugin-npm-run":
        parsed.workflowRuns.pluginNpm = next();
        break;
      case "--plugin-actagenthub-run":
        parsed.workflowRuns.pluginACTAgentHub = next();
        break;
      ...
      case "--skip-postpublish":
        ...
        break;
      case "--skip-actagenthub":
        parsed.skipACTAgentHub = true;
        break;
      case "--rerun-failed-actagenthub":
        parsed.rerunFailedACTAgentHub = true;
        break;
      default:
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetch(url, {
        ...options,
        signal: AbortSignal.timeout(ACTAGENTHUB_REQUEST_TIMEOUT_MS),
      });
...
export async function readBoundedJsonResponse(
  response: Response,
  label: string,
  maxBytes = ACTAGENTHUB_RESPONSE_BODY_MAX_BYTES,
): Promise<unknown> {
...
function readACTAgentHubTags(detail: unknown): Record<string, string> {
  if (!isRecord(detail)) {
    return {};
  }
...
async function verifyACTAgentHubPackage(params: {
  registry: string;
  packageName: string;
  version: string;
  distTag: string;
  ...
  const tags = readACTAgentHubTags(detail);
  if (tags[params.distTag] !== params.version) {
    throw new Error(
      `${params.packageName}: ACTAgentHub tag ${params.distTag} points to ${tags[params.distTag] ?? "<missing>"}, expected ${params.version}.`,
    );
  }

  const versionStatus = await fetchStatusWithRetry(versionUrl, "GET");
  if (versionStatus < 200 || versionStatus >= 300) {
    throw new Error(`${params.packageName}: ACTAgentHub exact version returned HTTP ${versionStatus}.`);
  }

  const artifactStatus = await fetchStatusWithRetry(artifactUrl, "HEAD");
  if (artifactStatus < 200 || artifactStatus >= 400) {
    throw new Error(`${params.packageName}: ACTAgentHub artifact returned HTTP ${artifactStatus}.`);
  }
}
...
export async function verifyBetaRelease(
  ...
  const actagentNpm = verifyNpmPackage("actagent", args.version, args.distTag);
  lines.push(`actagent npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("actagent postpublish verifier OK");
  }
  ...
  const actAgentHubPlugins = args.skipACTAgentHub
    ? []
    : collectACTAgentHubPublishablePluginPackages(rootDir, {
        packageNames: args.pluginSelection.length > 0 ? args.pluginSelection : undefined,
      });
  if (args.skipACTAgentHub) {
    lines.push("ACTAgentHub skipped");
  } else {
    assertSelectedPackagesResolved({
      label: "ACTAgentHub plugin",
      selection: args.pluginSelection,
      packages: actAgentHubPlugins,
    });
    for (const plugin of actAgentHubPlugins) {
      await verifyACTAgentHubPackage({
    ...
    lines.push(`ACTAgentHub OK: ${actAgentHubPlugins.length}`);
  }
  ...
  if (args.workflowRuns.pluginACTAgentHub !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.pluginACTAgentHub,
        label: "Plugin ACTAgentHub Release",
        repo: args.repo,
        expectedWorkflowName: "Plugin ACTAgentHub Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: args.rerunFailedACTAgentHub,
      }),
    );
  }
  if (args.workflowRuns.actagentNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.actagentNpm,
        label: "ACTAgent NPM Release",
        repo: args.repo,
        expectedWorkflowName: "ACTAgent NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          actagentNpmIntegrity: actagentNpm.integrity,
          githubReleaseUrl: releaseUrl,
          pluginNpmPackageCount: npmPlugins.length,
          actAgentHubPackageCount: actAgentHubPlugins.length,
          workflowRuns,
        },
        null,
        2,

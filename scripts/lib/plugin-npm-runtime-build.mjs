...
export function isPublishablePluginPackage(packageJson) {
  return packageJson.actagent?.release?.publishToNpm === true;
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "actagent" || id.startsWith("actagent/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "actagent.plugin.json")) {
    merged.add("actagent.plugin.json");
  }
...
function normalizeACTAgentPeerRange(value) {
...
function resolveACTAgentPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeACTAgentPeerRange(packageJson.actagent?.compat?.pluginApi) ||
    normalizeACTAgentPeerRange(packageJson.peerDependencies?.actagent) ||
    normalizeACTAgentPeerRange(packageJson.actagent?.build?.actagentVersion) ||
    normalizeACTAgentPeerRange(rootPackageJson?.version) ||
    normalizeACTAgentPeerRange(packageJson.version)
  );
}

/** Resolve package peer dependency metadata for the ACTAgent plugin API. */
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const actagentPeerRange = resolveACTAgentPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!actagentPeerRange) {
    throw new Error(
      `cannot infer actagent peerDependency range for ${plan.pluginDir}; set actagent.compat.pluginApi or package version`,
    );
  }
  ...
  const existingACTAgentMeta = getRecord(existingPeerDependenciesMeta.actagent);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      actagent: actagentPeerRange,
    },
    peerDependenciesMeta: {
      ...existingPeerDependenciesMeta,
      actagent: {
        ...existingACTAgentMeta,
        optional: true,
      },
    },
  };
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.actagent?.extensions)
      ? packageJson.actagent.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.actagent?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.actagent.setupEntry)
      : undefined,
  };
  return {
    ...plan,

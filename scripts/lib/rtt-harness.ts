// Rtt Harness script supports ACTAgent repository automation.
...
const ACTAGENT_PACKAGE_SPEC_RE =
  /^actagent@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
function resolveRttCredentialSource(
  ...
  const rawSource =
    env.ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.ACTAGENT_QA_CREDENTIAL_SOURCE;
  if (rawSource?.trim()) {
    return parseRttCredentialSource(rawSource);
  }
  if (
    env.CI &&
    env.ACTAGENT_QA_CONVEX_SITE_URL?.trim() &&
    (env.ACTAGENT_QA_CONVEX_SECRET_CI?.trim() || env.ACTAGENT_QA_CONVEX_SECRET_MAINTAINER?.trim())
  ) {
    return "convex";
  }
  return "env";
...
function resolveRttCredentialRole(
  ...
  const rawRole = env.ACTAGENT_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.ACTAGENT_QA_CREDENTIAL_ROLE;
  if (rawRole?.trim()) {
...
export function validateACTAgentPackageSpec(spec: string) {
  if (!ACTAGENT_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be actagent@main, actagent@alpha, actagent@beta, actagent@latest, or an exact ACTAgent release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    ACTAGENT_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { ACTAGENT_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    ACTAGENT_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    ACTAGENT_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    ...(params.credentialSource
      ? { ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE: params.credentialSource }
      : {}),
    ...(params.credentialRole
      ? { ACTAGENT_NPM_TELEGRAM_CREDENTIAL_ROLE: params.credentialRole }
      : {}),
    ACTAGENT_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    ACTAGENT_NPM_TELEGRAM_FAST: params.baseEnv.ACTAGENT_NPM_TELEGRAM_FAST ?? "1",
    ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    ACTAGENT_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export function assertRequiredEnv(
  ...
  if (credentialSource === "convex") {
    ...
    if (!env.ACTAGENT_QA_CONVEX_SITE_URL?.trim()) {
      missing.push("ACTAGENT_QA_CONVEX_SITE_URL");
    }
    if (credentialRole === "ci" && !env.ACTAGENT_QA_CONVEX_SECRET_CI?.trim()) {
      missing.push("ACTAGENT_QA_CONVEX_SECRET_CI");
    }
    if (credentialRole === "maintainer" && !env.ACTAGENT_QA_CONVEX_SECRET_MAINTAINER?.trim()) {
      missing.push("ACTAGENT_QA_CONVEX_SECRET_MAINTAINER");
    }
    if (missing.length > 0) {
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing ACTAgent Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("ACTAgent package.json must contain a non-empty version.");
  }

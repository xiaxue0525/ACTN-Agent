// Verifies installed packages can bootstrap the default ACTAgent workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "actagent-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "actagent.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".actagent", "workspace");

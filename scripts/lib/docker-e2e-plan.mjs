...
export const DEFAULT_E2E_BARE_IMAGE = "actagent-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "actagent-docker-e2e-functional:local";
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("actagent@") ? value : `actagent@${value}`;
  if (
    !/^actagent@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected actagent@latest, actagent@beta, actagent@alpha, or actagent@YYYY.M.D.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^actagent@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
    ...
    return matrixBaselines.flatMap((baselineSpec) =>
      matrixScenarios
        ...
        .map((scenario) => {
          ...
          const commandPrefix = [
            `ACTAGENT_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
            baselineSpec
              ? `ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
              : "",
            scenario ? `ACTAGENT_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
          ]
            .filter(Boolean)
            .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `ACTAGENT_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `ACTAGENT_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedACTAgentPackage(poolLanes) {
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS,
      process.env.ACTAGENT_UPGRADE_SURVIVOR_SCENARIOS,
    ),
...
function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedACTAgentPackage(scheduledLanes),
    },
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(selectableLanes, [selectedName], "ACTAGENT_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;

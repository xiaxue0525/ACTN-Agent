...
const MANUAL_SECTION_START = "<!-- actagent-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- actagent-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving ACTAgent messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to ACTAgent.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an ACTAgent plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.actagent?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in ACTAgent";
  }
  const install = packageJson.actagent?.install;
  const release = packageJson.actagent?.release;
  const actagenthubSpec =
    typeof install?.actagenthubSpec === "string" ? `: \`${install.actagenthubSpec}\`` : "";
  const npmSpec =
    typeof install?.npmSpec === "string" && install.npmSpec !== packageJson.name
      ? `: \`${install.npmSpec}\``
      : "";
  if (release?.publishToACTAgentHub === true && release?.publishToNpm === true) {
    if (install?.defaultChoice === "actagenthub") {
      return actagenthubSpec ? `ACTAgentHub${actagenthubSpec}; npm${npmSpec}` : `ACTAgentHub + npm${npmSpec}`;
    }
    return actagenthubSpec ? `npm${npmSpec}; ACTAgentHub${actagenthubSpec}` : `npm${npmSpec}; ACTAgentHub`;
  }
  if (release?.publishToACTAgentHub === true) {
    return `ACTAgentHub${actagenthubSpec || npmSpec}`;
  }
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.actagent?.install?.actagenthubSpec === "string" ||
    typeof packageJson.actagent?.install?.npmSpec === "string";
  if (!excludedDirs.has(dirName)) {
    return "core";
  }
  if (release?.publishToACTAgentHub === true || release?.publishToNpm === true || hasInstallSpec) {
    return "external";
  }
  return "source";
}
...
function renderReferenceIndex(records) {
  return `---
summary: "Generated index of ACTAgent plugin reference pages"
read_when:
  - You need a reference page for a specific ACTAgent plugin
...
\`actagent.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "actagent.plugin.json");

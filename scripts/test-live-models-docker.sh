...
if actagent_live_truthy "${ACTAGENT_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  ACTAGENT_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${ACTAGENT_LIVE_PROVIDERS:-}" || -n "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      actagent_live_collect_auth_dirs_from_csv "${ACTAGENT_LIVE_PROVIDERS:-}"
      actagent_live_collect_auth_dirs_from_csv "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      actagent_live_collect_auth_files_from_csv "${ACTAGENT_LIVE_PROVIDERS:-}"
      actagent_live_collect_auth_files_from_csv "${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${ACTAGENT_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
actagent_live_init_docker_run_args DOCKER_RUN_ARGS "${ACTAGENT_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ACTAGENT_LIVE_MODELS="${ACTAGENT_LIVE_MODELS:-modern}" \
  -e ACTAGENT_LIVE_PROVIDERS="${ACTAGENT_LIVE_PROVIDERS:-}" \
  -e ACTAGENT_LIVE_MAX_MODELS="${ACTAGENT_LIVE_MAX_MODELS:-}" \
  -e ACTAGENT_LIVE_MODEL_TIMEOUT_MS="${ACTAGENT_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e ACTAGENT_LIVE_REQUIRE_PROFILE_KEYS="${ACTAGENT_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_MODELS="${ACTAGENT_LIVE_GATEWAY_MODELS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_PROVIDERS="${ACTAGENT_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ACTAGENT_LIVE_GATEWAY_MAX_MODELS="${ACTAGENT_LIVE_GATEWAY_MAX_MODELS:-}" \

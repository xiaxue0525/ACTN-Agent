// Issue 78851 Model Resolution script supports ACTAgent repository automation.
...
function printUsage(): void {
  process.stdout.write(`ACTAgent issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): ACTAgentConfig {
  const providers: NonNullable<NonNullable<ACTAgentConfig["models"]>["providers"]> = {};
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureACTAgentModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-issue-78851-"));

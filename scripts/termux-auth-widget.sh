...
# ACTAgent Auth Widget for Termux
...
SERVER="${ACTAGENT_SERVER:-l36}"

# Check auth status
termux-toast "Checking ACTAgent auth..."

STATUS=$(ssh "$SERVER" '$HOME/actagent/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/actagent/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ACTAGENT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/actagent/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ACTAGENT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ACTAgent won't work until re-auth"
                ;;
        esac
        ;;

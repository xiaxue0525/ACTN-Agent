...
BASE_IMAGE="${BASE_IMAGE:-actagent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-actagent-sandbox-common:bookworm-slim}"
...
ACTAGENT_DOCKER_BUILD_USE_BUILDX="${ACTAGENT_DOCKER_BUILD_USE_BUILDX:-0}"
ACTAGENT_DOCKER_BUILD_CACHE_FROM="${ACTAGENT_DOCKER_BUILD_CACHE_FROM:-}"
ACTAGENT_DOCKER_BUILD_CACHE_TO="${ACTAGENT_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=actagent.sandbox=1)
NOTE

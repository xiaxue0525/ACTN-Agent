...
Optional custom relay:
  ACTAGENT_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-beta-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/ACTAgent.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.actagent.ai"
PUSH_RELAY_BASE_URL="${ACTAGENT_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ACTAGENT_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ACTAGENT_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ACTAGENT_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ACTAGENT_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ACTAGENT_URL_SLASH)$(ACTAGENT_URL_SLASH)#g'
)"
...
ACTAGENT_CODE_SIGN_STYLE = Automatic
ACTAGENT_DEVELOPMENT_TEAM = ${TEAM_ID}
ACTAGENT_IOS_SELECTED_TEAM = ${TEAM_ID}
ACTAGENT_APP_BUNDLE_ID = ai.actagent.client
ACTAGENT_SHARE_BUNDLE_ID = ai.actagent.client.share
ACTAGENT_ACTIVITY_WIDGET_BUNDLE_ID = ai.actagent.client.activitywidget
ACTAGENT_WATCH_APP_BUNDLE_ID = ai.actagent.client.watchkitapp
ACTAGENT_WATCH_EXTENSION_BUNDLE_ID = ai.actagent.client.watchkitapp.extension
ACTAGENT_APP_PROFILE =
ACTAGENT_SHARE_PROFILE =
ACTAGENT_PUSH_TRANSPORT = relay
ACTAGENT_PUSH_DISTRIBUTION = official
ACTAGENT_URL_SLASH = /
ACTAGENT_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ACTAGENT_PUSH_APNS_ENVIRONMENT = production
EOF

...
const SKIP_GATEWAY_ENV = {
  ...
  ACTAGENT_SKIP_ACPX_RUNTIME: "1",
  ACTAGENT_SKIP_ACPX_RUNTIME_PROBE: "1",
...
Options:
  ...
  --keep                         Keep the synthetic ACTAGENT_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.ACTAGENT_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.ACTAGENT_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.ACTAGENT_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("ACTAGENT_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "ACTAGENT_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readPositiveNumberEnv("ACTAGENT_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readNumberEnv("ACTAGENT_MEMORY_FD_REPRO_SAMPLE_DELAY_MS", 1_000);
  options.settleDelayMs ??= readNumberEnv("ACTAGENT_MEMORY_FD_REPRO_SETTLE_DELAY_MS", 5_000);
...
/**
 * Writes isolated ACTAgent config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".actagent");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    ACTAGENT_STATE_DIR: path.join(homeDir, ".actagent"),

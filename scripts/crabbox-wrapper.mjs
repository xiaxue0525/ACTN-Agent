...
const ignoreRepoBinary = process.env.ACTAGENT_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.ACTAGENT_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for ACTAgent proof.",
      "[crabbox] run `crabbox login --url https://crabbox.actagent.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set ACTAGENT_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    ...
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/actagent/actagent.git 2>/dev/null || git remote set-url origin https://github.com/actagent/actagent.git;",
    `git fetch -q --depth=1 origin ${quotedBase}:refs/remotes/origin/main;`,
    "git reset --mixed --quiet refs/remotes/origin/main;",
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=ACTAgent -c user.email=ci@actagent.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$actagentModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($actagentModulesDir) {",
    'if (-not (Test-Path $actagentModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $actagentModulesDir" }',
    '$actagentWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$actagentSelfModules = Join-Path $actagentModulesDir "node_modules"',
    'if (-not (Test-Path $actagentSelfModules)) { cmd /c mklink /J "$actagentSelfModules" "$actagentModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $actagentWorkspaceModules)) { cmd /c mklink /J "$actagentWorkspaceModules" "$actagentModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remoteAwsMacosJsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.ACTAGENT_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "actagent_crabbox_bootstrap_macos_js() {",
    'tool_root="${ACTAGENT_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.actagent-crabbox-toolchain}";',
    ...
    'macos_locale="${ACTAGENT_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.actagent-crabbox-node-ready";',
    ...
    "actagent_crabbox_env() {",
    "actagent_env_args=();",
    "actagent_env_ignore=0;",
    "actagent_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) actagent_env_ignore=1; actagent_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${actagent_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) actagent_env_ignore=1; actagent_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) actagent_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then actagent_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) actagent_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$actagent_env_ignore" = "1" ]; then actagent_env_args+=("PATH=$PATH:${1#PATH=}"); else actagent_env_args+=("$1"); fi; actagent_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) actagent_env_args+=("$1"); shift ;;',
    '--) actagent_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$actagent_env_ignore" = "1" ] && [ "$actagent_env_path_seen" = "0" ]; then actagent_env_args+=("PATH=$PATH"); fi;',
    'command env "${actagent_env_args[@]}" "$@";',
    "};",
  ];
  if (packageManager) {
    bootstrap.push(
  ...
  bootstrap.push("};", "actagent_crabbox_bootstrap_macos_js");
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    shellCommand: `actagent_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "actagent-crabbox-macos-script-"));
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/actagent-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_actagent_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_actagent_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `ACTAGENT_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "actagent", "crabbox-sync");
  }
  return resolve(tmpdir(), "actagent-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.ACTAGENT_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "ACTAGENT_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set ACTAGENT_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower ACTAGENT_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "actagent-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "ACTAGENT_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/actagent/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for ACTAgent CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for ACTAgent Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/actagent-crabbox-docker-work";
  console.error(

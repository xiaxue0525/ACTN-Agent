...
if [[ "${mode}" != "--dry-run" && "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/plugin-actagenthub-publish.sh [--dry-run|--publish] <package-dir>" >&2
  exit 2
fi
...
if ! command -v actagenthub >/dev/null 2>&1; then
  echo "actagenthub CLI is required on PATH" >&2
  exit 1
fi
...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-actagent/actagent}}"
...
actagenthub_workdir="${actagentdHUB_WORKDIR:-${ACTAGENTHUB_WORKDIR:-${invocation_root}}}"

pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/actagent-actagenthub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${ACTAGENT_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${ACTAGENT_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
echo "Resolved ACTAgentHub workdir: ${actagenthub_workdir}"
echo "Publish auth: GitHub Actions OIDC via ACTAgentHub short-lived token"

printf 'Pack command: ACTAGENTHUB_WORKDIR=%q' "${actagenthub_workdir}"
printf ' %q' "${pack_cmd[@]}"
printf '\n'
...
ACTAGENTHUB_WORKDIR="${actagenthub_workdir}" \
...
try {
  ...
  console.error(`actagenthub package pack did not return JSON: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}
if (!parsed || typeof parsed.path !== "string" || parsed.path.trim() === "") {
  console.error("actagenthub package pack output did not include a tarball path.");
  process.exit(1);
}
...
if [[ ! -f "${pack_path}" ]]; then
  echo "actagentpack tarball not found: ${pack_path}" >&2
  exit 1
fi
...
echo "Resolved actagentpack: ${pack_path}"

printf 'Publish command: ACTAGENTHUB_WORKDIR=%q' "${actagenthub_workdir}"
printf ' %q' "${publish_cmd[@]}"
printf '\n'

if [[ "${mode}" == "--dry-run" ]]; then
  ACTAGENTHUB_WORKDIR="${actagenthub_workdir}" "${publish_cmd[@]}" --dry-run
  exit 0
fi

publish_log="${pack_dir}/publish.log"
for attempt in $(seq 1 "${ACTAGENT_ACTAGENTHUB_PUBLISH_ATTEMPTS:-8}"); do
  if ACTAGENTHUB_WORKDIR="${actagenthub_workdir}" "${publish_cmd[@]}" > >(tee "${publish_log}") 2>&1; then
    exit 0
  fi
  if ! grep -Eqi "rate limit|too many requests|\\b429\\b" "${publish_log}"; then
    exit 1
  fi
  echo "ACTAgentHub publish hit a rate limit; retrying (${attempt}/${ACTAGENT_ACTAGENTHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${ACTAGENT_ACTAGENTHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ACTAgentHub publish failed after ${ACTAGENT_ACTAGENTHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1

...
if [[ -n "${OPENAI_API_KEY:-}" || -n "${OPENAI_BASE_URL:-}" || -n "${GEMINI_API_KEY:-}" || -n "${GOOGLE_API_KEY:-}" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/actagent-subagent-live-env.XXXXXX")"
  TEMP_DIRS+=("$docker_env_dir")
  docker_env_file="$docker_env_dir/provider.env"
  {
    if [[ -n "${OPENAI_API_KEY:-}" ]]; then
      printf 'ACTAGENT_DOCKER_LIVE_OPENAI_API_KEY=%s\n' "${OPENAI_API_KEY}"
    fi
    if [[ -n "${OPENAI_BASE_URL:-}" ]]; then
      printf 'ACTAGENT_DOCKER_LIVE_OPENAI_BASE_URL=%s\n' "${OPENAI_BASE_URL}"
    fi
    if [[ -n "${GEMINI_API_KEY:-}" ]]; then
      printf 'ACTAGENT_DOCKER_LIVE_GEMINI_API_KEY=%s\n' "${GEMINI_API_KEY}"
    fi
    if [[ -n "${GOOGLE_API_KEY:-}" ]]; then
      printf 'ACTAGENT_DOCKER_LIVE_GOOGLE_API_KEY=%s\n' "${GOOGLE_API_KEY}"
    fi
  } >"$docker_env_file"
  DOCKER_EXTRA_ENV_FILES+=(--env-file "$docker_env_file")
fi

CONTAINER_NODE_OPTIONS="$(actagent_live_container_node_options)"

read -r -d '' LIVE_TEST_CMD <<'EOF' || true
set -euo pipefail
[ -f "$HOME/.profile" ] && [ -r "$HOME/.profile" ] && source "$HOME/.profile" || true
if [ -n "${ACTAGENT_DOCKER_LIVE_OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$ACTAGENT_DOCKER_LIVE_OPENAI_API_KEY"
  unset ACTAGENT_DOCKER_LIVE_OPENAI_API_KEY
fi
if [ -n "${ACTAGENT_DOCKER_LIVE_OPENAI_BASE_URL:-}" ]; then
  export OPENAI_BASE_URL="$ACTAGENT_DOCKER_LIVE_OPENAI_BASE_URL"
  unset ACTAGENT_DOCKER_LIVE_OPENAI_BASE_URL
fi
if [ -n "${ACTAGENT_DOCKER_LIVE_GEMINI_API_KEY:-}" ]; then
  export GEMINI_API_KEY="$ACTAGENT_DOCKER_LIVE_GEMINI_API_KEY"
  unset ACTAGENT_DOCKER_LIVE_GEMINI_API_KEY
fi
if [ -n "${ACTAGENT_DOCKER_LIVE_GOOGLE_API_KEY:-}" ]; then
  export GOOGLE_API_KEY="$ACTAGENT_DOCKER_LIVE_GOOGLE_API_KEY"
  unset ACTAGENT_DOCKER_LIVE_GOOGLE_API_KEY
fi
...
ACTAGENT_LIVE_SUBAGENT_E2E=1 \
ACTAGENT_VITEST_MAX_WORKERS="${ACTAGENT_VITEST_MAX_WORKERS:-1}" \
...
echo "==> Model: ${ACTAGENT_LIVE_SUBAGENT_E2E_MODEL:-openai/gpt-5.5}"
echo "==> Profile file: $PROFILE_STATUS"
DOCKER_RUN_ARGS=()
actagent_live_init_docker_run_args DOCKER_RUN_ARGS "${ACTAGENT_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ACTAGENT_LIVE_TEST_QUIET="${ACTAGENT_LIVE_TEST_QUIET:-}" \
  -e ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS="${ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e ACTAGENT_LIVE_SUBAGENT_E2E=1 \
  -e ACTAGENT_LIVE_SUBAGENT_E2E_MODEL="${ACTAGENT_LIVE_SUBAGENT_E2E_MODEL:-}" \
  -e ACTAGENT_VITEST_FS_MODULE_CACHE=0 \
  -e ACTAGENT_VITEST_MAX_WORKERS="${ACTAGENT_VITEST_MAX_WORKERS:-1}")

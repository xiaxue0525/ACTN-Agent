...
const DEFAULT_OUTPUT_NAME = "actagent-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "ACTAGENT_TRUSTED_PACKAGE_TOKEN";
...
export const ACTAGENT_PACKAGE_SPEC_RE =
  /^actagent@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-actagent-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateACTAgentPackageSpec(spec) {
  if (!ACTAGENT_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be actagent@alpha, actagent@beta, actagent@latest, or an exact ACTAgent release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateACTAgentPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an ACTAgent branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `actagent-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (/^actagent-.*\.tgz$/u.test(trimmed)) {
        filename = trimmed;
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^actagent-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no ACTAgent tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking ACTAgent package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-actagent-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `ACTAgent package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "actagent") {
    throw new Error(`package candidate must be named "actagent"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {

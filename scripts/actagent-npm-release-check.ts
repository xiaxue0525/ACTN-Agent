...
// ACTAgent Npm Release Check script supports ACTAgent repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/actagent/actagent";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@actagent/fs-safe";
...
const skipPackValidationEnv = "ACTAGENT_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "ACTAGENT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "actagent") {
    errors.push(`package.json name must be "actagent"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.actagent !== "actagent.mjs") {
    errors.push(
      `package.json bin.actagent must be "actagent.mjs"; found "${pkg.bin?.actagent ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`actagent-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `actagent-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}

#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports ACTAgent repository automation.
...
type DriverMode = "token" | "webhook" | "actagent";
...
const DEFAULT_ACTAGENT_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.ACTAGENT_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.ACTAGENT_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".actagent");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "actagent") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or actagent.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver actagent] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) ACTAgent spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|actagent> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override ACTAgent state dir for plugin-state polling\n" +
    "  --actagent-bin <path>        ACTAgent CLI binary for driver=actagent (default: actagent)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ACTAGENT_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ACTAGENT_DISCORD_SMOKE_DRIVER\n" +
    "  ACTAGENT_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ACTAGENT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ACTAGENT_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ACTAGENT_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ACTAGENT_DISCORD_SMOKE_AGENT\n" +
    "  ACTAGENT_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ACTAGENT_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ACTAGENT_DISCORD_SMOKE_POLL_MS\n" +
    "  ACTAGENT_STATE_DIR\n" +
    "  ACTAGENT_DISCORD_SMOKE_ACTAGENT_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ACTAGENT_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.ACTAGENT_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token") || process.env.ACTAGENT_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ACTAGENT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ACTAGENT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ACTAGENT_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ACTAGENT_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ACTAGENT_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ACTAGENT_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ACTAGENT_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ACTAGENT_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir") || resolveStateDir());
  const actagentBin =
    resolveArg("--actagent-bin") || process.env.ACTAGENT_DISCORD_SMOKE_ACTAGENT_BIN || "actagent";
...
async function actagentCliJson<T>(params: {
  actagentBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.actagentBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_ACTAGENT_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`actagent ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithActagent(params: {
  actagentBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await actagentCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    actagentBin: params.actagentBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "actagent") {
    return await readMessagesWithActagent({
      actagentBin: params.args.actagentBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `actagent-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await actagentCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("actagent message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "actagent"
            ? await readMessagesWithActagent({
                actagentBin: args.actagentBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from ACTAgent.`,
        diagnostics: {
          bindingCandidates: [
            {

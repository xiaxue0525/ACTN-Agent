...
function resolveCliShardSpec(args, env) {
  ...
  return env.ACTAGENT_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrencyRaw =
    process.env.ACTAGENT_ADDITIONAL_BOUNDARY_CONCURRENCY ??
    process.env.ACTAGENT_EXTENSION_BOUNDARY_CONCURRENCY;
  const concurrencyLabel =
    process.env.ACTAGENT_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
      ? "ACTAGENT_EXTENSION_BOUNDARY_CONCURRENCY"
      : "ACTAGENT_ADDITIONAL_BOUNDARY_CONCURRENCY";
  const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
  const checkTimeoutMs = resolvePositiveInteger(
    process.env.ACTAGENT_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
    DEFAULT_CHECK_TIMEOUT_MS,
    "ACTAGENT_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
  );
  const outputMaxBytes = resolvePositiveInteger(
    process.env.ACTAGENT_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
    DEFAULT_OUTPUT_MAX_BYTES,
    "ACTAGENT_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
  );

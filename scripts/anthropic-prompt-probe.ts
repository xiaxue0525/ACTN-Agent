// Anthropic Prompt Probe script supports ACTAgent repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// ACTAGENT_PROMPT_TRANSPORT=direct|gateway
// ACTAGENT_PROMPT_MODE=extra
// ACTAGENT_PROMPT_TEXT='...'
// ACTAGENT_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.ACTAGENT_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.ACTAGENT_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.ACTAGENT_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.ACTAGENT_USER_PROMPT?.trim() || "is actagentd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "ACTAGENT_PROMPT_CAPTURE",
  raw: process.env.ACTAGENT_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "ACTAGENT_PROMPT_INCLUDE_RAW",
  raw: process.env.ACTAGENT_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "ACTAGENT_PROMPT_KEEP_TMP",
  raw: process.env.ACTAGENT_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.ACTAGENT_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "ACTAGENT_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.ACTAGENT_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "ACTAGENT_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.ACTAGENT_PROMPT_GATEWAY_TIMEOUT_MS,
});
const SETUP_TOKEN_RAW = process.env.ACTAGENT_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.ACTAGENT_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.ACTAGENT_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set ACTAGENT_LIVE_SETUP_TOKEN_VALUE or ACTAGENT_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["actagent.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        ACTAGENT_AGENT_DIR: params.agentDir,
        ACTAGENT_GATEWAY_TOKEN: params.gatewayToken,
        ACTAGENT_SKIP_CHANNELS: "1",
        ACTAGENT_SKIP_GMAIL_WATCHER: "1",
        ACTAGENT_SKIP_CANVAS_HOST: "1",
        ACTAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
        ACTAGENT_DISABLE_BONJOUR: "1",
        ACTAGENT_SKIP_CRON: "1",
        ACTAGENT_TEST_MINIMAL_GATEWAY: "1",
        ACTAGENT_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing ACTAGENT_PROMPT_TEXT or ACTAGENT_PROMPT_LIST_JSON");
  }

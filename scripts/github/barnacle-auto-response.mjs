...
import {
  ...
  hasACTAgentSweeperExactHeadProof,
...
const thirdPartyExtensionMessage =
  "Please publish this as a third-party plugin on [ACTAgentHub](https://actagenthub.ai) instead of adding it to the core repo. Docs: https://docs.actagent.ai/plugin and https://docs.actagent.ai/actagenthub";
...
const rules = [
  {
    ...
    close: true,
    message:
      "Thanks for the contribution! New skills should be published on [ACTAgentHub](https://actagenthub.ai) for everyone to use. We’re keeping the core lean on skills, so I’m closing this out.",
  },
  {
    label: "r: support",
    close: true,
    message:
      "Please use [our support server](https://discord.gg/actagentd) and ask in #help or #users-helping-users to resolve this, or follow the stuck FAQ at https://docs.actagent.ai/help/faq#im-stuck-whats-the-fastest-way-to-get-unstuck.",
  },
  {
    label: "r: false-positive",
    close: true,
    message:
      "Closing this because it looks like a false positive or reclassification-only report rather than an actionable ACTAgent bug. If this is still a real issue, please open a fresh report with concrete reproduction steps and current-version details.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    message:
      "BlueBubbles is deprecated and no longer ships as a bundled ACTAgent channel. Use iMessage via `imsg` instead: https://docs.actagent.ai/channels/imessage. If this needs to stay BlueBubbles-backed, publish it as a third-party plugin on ACTAgentHub instead of adding it back to core.",
  },
  {
    label: "r: moltbook",
    close: true,
    ...
    message:
      "ACTAgent is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  "r: skill": {
    color: "5319E7",
    description: "Auto-close: skills should be published on ACTAgentHub, not added to core.",
  },
  "r: support": {
    color: "0E8A16",
  ...
  "r: third-party-extension": {
    color: "5319E7",
    description: "Auto-close: third-party plugins/capabilities belong on ACTAgentHub.",
  },
  "r: bluebubbles": {
    color: "D93F0B",
    description: "Auto-close: BlueBubbles is deprecated; use iMessage via imsg or ACTAgentHub.",
  },
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for ACTAgent.",
  },
  "r: spam": {
    color: "B60205",
  ...
  [PROOF_SUFFICIENT_LABEL]: {
    color: "0E8A16",
    description: "ACTAgentSweeper judged the real behavior proof convincing.",
  },
  [PROOF_OVERRIDE_LABEL]: {
    color: "C2E0C6",
  ...
  "triage: external-plugin-candidate": {
    color: "C5DEF5",
    description: "Candidate: plugin/capability may belong on ACTAgentHub.",
  },
};
...
const pingWarningMessage =
  "Please don’t spam-ping multiple maintainers at once. Be patient, or join our community Discord for help: https://discord.gg/actagentd";
...
const candidateActionRules = [
  {
    ...
    close: true,
    message:
      "Closing this PR because it does not include real behavior proof. Please reopen or resubmit with after-fix evidence from a real ACTAgent setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count. Unit tests, mocks, snapshots, lint, typechecks, and CI are supplemental only.",
  },
  {
    label: candidateLabels.mockOnlyProof,
    close: true,
    message:
      "Closing this PR because the proof only shows tests, mocks, snapshots, lint, typechecks, or CI. Please reopen or resubmit with after-fix evidence from a real ACTAgent setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because docs discoverability and community-plugin listing changes should go through ACTAgentHub or a maintainer-owned docs plan, not drive-by core churn.",
  },
  {
    label: candidateLabels.lowSignalDocs,
    close: true,
    message:
      "Closing this PR because the docs-only change is too low-signal for the core repo. Please reopen or resubmit with a concrete ACTAgent docs gap and linked context.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the template is mostly blank and does not describe a concrete ACTAgent problem, fix, or test plan. Please reopen or resubmit with the missing context filled in.",
  },
];

const normalizeLogin = (login) => login.toLowerCase();
const automationPrHeadPrefixes = ["actagentsweeper/", "clownfish/"];
...
function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/actagent\/actagent\/(?:issues|pull)\/\d+)/i.test(text);
}
...
export function classifyPullRequestCandidateLabels(pullRequest, files) {
  ...
  if (
    ...
    !linkedReference &&
    (discoverabilityDocs ||
      /\b(community plugin|plugin listing|discoverability|showcase|actagenthub)\b/i.test(text))
  ) {
  ...
  const addsPluginManifest = files.some(
    (file) =>
      file.status === "added" && /^extensions\/[^/]+\/actagent\.plugin\.json$/i.test(file.filename),
  );
  if (
    !clearDesignContext &&
    (addsPluginManifest ||
      /\b(third[- ]party|external plugin|community plugin|actagenthub)\b/i.test(lowerText))
  ) {
...
function isACTAgentSweeperOwnedLabel(label) {
  return label === "actagentsweeper" || label.startsWith("actagentsweeper:");
}

function isActiveACTAgentSweeperWork(pullRequest, labelSet) {
  const authorLogin = pullRequest.user?.login ?? "";
  const headRef = pullRequest.head?.ref ?? "";
  return (
    /actagentsweeper/i.test(authorLogin) ||
    headRef.startsWith("actagentsweeper/") ||
    [...labelSet].some(isACTAgentSweeperOwnedLabel)
  );
}
...
function shouldRemoveProofSufficientLabel(
  ...
  labelSet,
  proofEvaluation,
  hasExactHeadACTAgentSweeperProof,
) {
  if (hasExactHeadACTAgentSweeperProof) {
    return false;
  }
  if (proofEvaluation.status === "override") {
    return false;
  }
  if (isActiveACTAgentSweeperWork(pullRequest, labelSet)) {
    return false;
  }
...
function shouldPreserveACTAgentSweeperProofJudgment(context, labelSet) {
  return (
...
async function applyPullRequestCandidateLabels(github, context, core, pullRequest, labelSet) {
  ...
  const hasExactHeadACTAgentSweeperProof =
    labelSet.has(PROOF_SUFFICIENT_LABEL) &&
    hasACTAgentSweeperExactHeadProof({
      pullRequest,
  ...
  const candidateLabelsToApply = shouldPreserveACTAgentSweeperProofJudgment(context, labelSet)
...
function isACTAgentSweeperProofSufficientLabelEvent(context) {
  ...
  return (
    ...
    /actagentsweeper/i.test(senderLogin)
  );
}
...
async function removeLabels(github, context, issueNumber, labels, labelSet) {
  for (const label of labels) {
    ...
    if (isACTAgentSweeperOwnedLabel(label)) {
      continue;
    }
    try {
      await github.rest.issues.removeLabel({
...
export async function runBarnacleAutoResponse({ github, context, core = console }) {
  ...
  if (pullRequest) {
    ...
    if (isACTAgentSweeperProofSufficientLabelEvent(context)) {
      core.info(
        `Skipping PR auto-response checks for #${pullRequest.number} because ACTAgentSweeper owns ${PROOF_SUFFICIENT_LABEL}.`,
      );
      return;
    }

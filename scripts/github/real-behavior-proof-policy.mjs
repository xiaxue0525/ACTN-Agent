...
export const ACTAGENTSWEEPER_PROOF_VERDICT_STATUS = "actagentsweeper_exact_head_pass";
const ACTAGENTSWEEPER_BOT_LOGINS = new Set(["actagentsweeper[bot]", "actagent-actagentsweeper[bot]"]);
...
const liveCommandRegex =
  /\b(?:actagent|node|docker|curl|gh|ssh|adb|xcrun|xcodebuild|open|npm\s+run|pnpm\s+actagent)\b/i;
...
function result(status, reason, details = {}) {
  return {
    ...
    passed: ["passed", "skipped", "override", ACTAGENTSWEEPER_PROOF_VERDICT_STATUS].includes(status),
    ...details,
  };
}
...
function isTrustedACTAgentSweeperComment(comment) {
  const appSlug = String(
    comment?.performed_via_github_app?.slug ?? comment?.performedViaGithubApp?.slug ?? "",
  ).toLowerCase();
  if (appSlug === "actagentsweeper") {
    return true;
  }
  // GitHub can omit performed_via_github_app on issue comments while still
  // returning a reserved ACTAgentSweeper App bot identity.
  const login = String(comment?.user?.login ?? "").toLowerCase();
  const userType = String(comment?.user?.type ?? "");
  return ACTAGENTSWEEPER_BOT_LOGINS.has(login) && userType === "Bot";
}

export function hasACTAgentSweeperExactHeadProof({ pullRequest, comments = [] } = {}) {
  ...
  for (const comment of comments) {
    if (!isTrustedACTAgentSweeperComment(comment)) {
      continue;
    }
    const body = String(comment?.body ?? "");
    const markers = body.match(/<!--\s*actagentsweeper-verdict:pass\b[\s\S]*?-->/gi) ?? [];
...
export function evaluateACTAgentSweeperExactHeadProof({ pullRequest, comments = [] } = {}) {
  if (hasACTAgentSweeperExactHeadProof({ pullRequest, comments })) {
    return result(
      ACTAGENTSWEEPER_PROOF_VERDICT_STATUS,
      "ACTAgentSweeper accepted real behavior proof for the exact PR head.",
    );
  }
  return result("insufficient", "No exact-head ACTAgentSweeper proof verdict was found.");
}

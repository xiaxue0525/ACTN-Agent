...
export const dependencyChangeMarker = "<!-- actagent:dependency-guard -->";
export const dependencyGraphGuardMarker = "<!-- actagent:dependency-graph-guard -->";
...
const securityTeamSlug = process.env.ACTAGENT_SECURITY_TEAM_SLUG ?? "actagent-secops";
...
export function renderAuthorizedDependencyComment(override) {
  const lines = [
    ...
    "",
    "This PR includes dependency graph changes. A repository admin or member of `@actagent/actagent-secops` authorized this exact head SHA with `/allow-dependencies-change`.",
    "",
...
export function renderTrustedDependencyComment({ actor, headSha }) {
  return [
    ...
    "",
    "This PR includes dependency graph changes. The dependency guard is informational because the PR author is a repository admin or a member of `@actagent/actagent-secops`.",
    "",
...
ACTAgent does not accept package lockfile changes through PRs. This PR did not change dependency graph fields in package manifests, so the workflow restored the lockfile residue from the target branch automatically.
...
export function renderBlockedDependencyComment({
  ...
  return [
    ...
    "",
    "ACTAgent does not accept dependency graph changes through PRs unless a repository admin or security explicitly authorizes the current head SHA. Dependency updates are generated internally by maintainers so external PRs cannot change the resolved graph.",
    "",
    ...
    ...removalSteps,
    "",
    "If this PR intentionally needs a dependency graph change, ask a repository admin or member of `@actagent/actagent-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  ...
  const baseHeaders = {
    ...
    "user-agent": "actagent-dependency-guard",
...
async function main() {
  ...
  const autoscrubToken = process.env.ACTAGENT_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN;
  const autoscrubApi = autoscrubToken ? githubApi(autoscrubToken) : null;
  const explicitSecurityApprovers = securityApproverSet(process.env.ACTAGENT_SECURITY_APPROVERS);
  const trustedCommentAuthors = dependencyGuardCommentAuthors(
    process.env.ACTAGENT_DEPENDENCY_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.ACTAGENT_DEPENDENCY_GUARD_MODE ?? "enforce";

...
REMOTE_URL="${ACTAGENT_REF_REMOTE:-https://github.com/actagent/actagent.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-actagent-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves ACTAgent branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve ACTAgent ref: ${REF}" >&2
exit 1

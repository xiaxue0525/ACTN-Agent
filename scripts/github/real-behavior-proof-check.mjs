...
async function main(env = process.env) {
  ...
  if ((appToken || env.GITHUB_TOKEN) && repository && pullRequest.number) {
    ...
    try {
      ...
      const actagentsweeperEvaluation = evaluateACTAgentSweeperExactHeadProof({
        pullRequest,
        comments,
      });
      if (actagentsweeperEvaluation.passed) {
        console.log(actagentsweeperEvaluation.reason);
        process.exit(0);
      }
    } catch (error) {
      console.warn(
  ...
  const message = `${evaluation.reason} Add after-fix evidence from a real ACTAgent setup in the PR body. Screenshots, recordings, terminal screenshots, console output, redacted runtime logs, linked artifacts, or copied live output count. Unit tests, mocks, snapshots, lint, typechecks, and CI are supplemental only. A maintainer can apply proof: override when appropriate.`;

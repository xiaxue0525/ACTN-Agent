...
APP_BUNDLE="${1:-dist/ACTAgent.app}"
...
ENT_TMP_DIR=$(mktemp -d -t actagent-entitlements.XXXXXX)
trap cleanup EXIT
...
MLX_TTS_HELPER="$APP_BUNDLE/Contents/MacOS/actagent-mlx-tts"
if [ -f "$MLX_TTS_HELPER" ]; then
  echo "Signing MLX TTS helper"; sign_item "$MLX_TTS_HELPER" "$APP_ENTITLEMENTS"
fi

# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/ACTAgent" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/ACTAgent" "$APP_ENTITLEMENTS"
fi

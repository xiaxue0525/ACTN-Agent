# Medium game plan ACTAgent harness

```yaml qa-scenario
id: medium-game-plan-actagent-harness
title: Medium game plan ACTAgent harness
surface: workspace
coverage:
  primary:
    - workspace.planning
  secondary:
    - agents.actagent-harness
objective: Verify GPT-5.5 can use the ACTAgent harness to plan and build a medium-complex self-contained browser game.
successCriteria:
  - A live-frontier run fails fast unless the selected primary model is openai/gpt-5.5.
  - The scenario forces the embedded ACTAgent harness before the build turn.
...
codeRefs:
  ...
  - src/agents/harness/builtin-actagent.ts
  - extensions/qa-lab/src/suite.ts
execution:
  kind: flow
  summary: Run with `pnpm actagent qa suite --provider-mode live-frontier --model openai/gpt-5.5 --alt-model openai/gpt-5.5 --fast --thinking medium --scenario medium-game-plan-actagent-harness`.
  config:
    requiredProvider: openai
    requiredModel: gpt-5.5
    harnessRuntime: actagent
    artifactFile: star-garden-defenders-actagent.html
    gameTitle: Star Garden Defenders
    minBytes: 5000
    buildPrompt: |-
      Enter plan mode first and write a short implementation plan before editing.

      Then build a medium-complex, self-contained browser game at ./star-garden-defenders-actagent.html.
...
```yaml qa-flow
steps:
  - name: confirms GPT-5.5 ACTAgent harness target
    actions:
      - set: selected
        value:
      ...
      - if:
          ...
          then:
            ...
            - call: runAgentPrompt
              args:
                - ref: env
                - sessionKey: agent:qa:medium-game-actagent
                  message:

...
coverage:
  ...
  secondary:
    - tools.invocation
objective: Verify ACTAgent can expose plugin tools over MCP and a real MCP client can call one successfully.
successCriteria:
...
execution:
  kind: flow
  summary: Verify ACTAgent can expose plugin tools over MCP and a real MCP client can call one successfully.
  config:

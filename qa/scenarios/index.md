...
- use `pnpm actagent qa coverage` to render the current inventory
...
- `standard`: required Codex-vs-ACTAgent mock gate coverage for first-hour depth and
  default runtime-tool fixtures. ACTAgent dynamic integration tools in this
  tier are hard-gated by `actagent qa coverage --tools --summary`; Codex-native
  workspace rows remain separately tracked until native/live behavior is the
  asserted surface. Rows that explicitly target searchable/deferred ACTAgent
  dynamic loading stay report-only unless a fixture promotes them to required. Selected with
  `actagent qa suite --runtime-pair actagent,codex --runtime-parity-tier standard`
...
agent:
  identityMarkdown: |-
    ...
    You are the ACTAgent QA operator agent.
...
kickoffTask: |-
  QA mission:
  Understand this ACTAgent repo from source + docs before acting.

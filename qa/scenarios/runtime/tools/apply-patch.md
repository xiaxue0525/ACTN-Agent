...
coverage:
  primary:
    - tools.apply-patch
objective: Verify apply_patch behavior is tracked across ACTAgent and Codex while Codex owns patching natively.
successCriteria:
  - ACTAgent may expose ACTAgent apply_patch while Codex app-server mode may omit duplicate ACTAgent dynamic apply_patch.
...
execution:
  ...
  config:
    ...
    knownHarnessGap:
      issue: "#80320"
      reason: Codex-native apply_patch is intentionally not an ACTAgent dynamic tool; QA fault injection still uses synthetic failure-path inputs.

...
coverage:
  primary:
    - tools.bash
objective: Verify shell command behavior is tracked across ACTAgent and Codex while Codex owns exec/process natively.
successCriteria:
...
execution:
  kind: flow
  summary: Exercise the bash family through the ACTAgent exec tool.
  config:
    toolName: exec
    toolCoverage:
      family: bash

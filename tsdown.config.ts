...
const RUN_NODE_SKIP_DTS_BUILD = process.env.ACTAGENT_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.ACTAGENT_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.ACTAGENT_BUILD_PRIVATE_QA === "1";
...
function shouldAlwaysBundleDependency(id: string): boolean {
  return (
    id === "@actagent/fs-safe" ||
    id.startsWith("@actagent/fs-safe/") ||
    id === "@actagent/normalization-core" ||
    id.startsWith("@actagent/normalization-core/") ||
    id === "@actagent/media-core" ||
    id.startsWith("@actagent/media-core/") ||
    id === "@actagent/acp-core" ||
    id.startsWith("@actagent/acp-core/") ||
    id === "zod" ||
    id.startsWith("zod/")
  );
}
...
function shouldExternalizeAgentCoreDependency(id: string): boolean {
  return (
    id === "@actagent/llm-core" ||
    id.startsWith("@actagent/llm-core/") ||
    id === "ignore" ||
    id === "actagent" ||
    id.startsWith("actagent/") ||
    id === "typebox" ||
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return (
    id === "ws" ||
    id.startsWith("ws/") ||
    id === "@actagent/gateway-protocol" ||
    id.startsWith("@actagent/gateway-protocol/")
  );
}
...
function shouldExternalizeSpeechCoreDependency(id: string): boolean {
  return id === "actagent" || id.startsWith("actagent/");
}
...
function shouldExternalizeLlmRuntimeDependency(id: string): boolean {
  return id === "@actagent/llm-core" || id.startsWith("@actagent/llm-core/");
}

...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "actagent.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "actagent.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    if (task.runtime !== "cli") {
      ...
      if (!childSessionKey) {
        ...
        // not have ACTAgent child sessions to terminate. Cancellation clears

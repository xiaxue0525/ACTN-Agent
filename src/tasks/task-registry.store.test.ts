...
const ORIGINAL_ENV = captureEnv(["ACTAGENT_STATE_DIR"]);
...
describe("task-registry store runtime", () => {
  ...
  it("rejects corrupt persisted task rows during sqlite restore", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-store-corrupt-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("drops invalid requester origins during sqlite restore", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-store-invalid-origin-" },
      async () => {
        resetTaskRegistryForTests();
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("persists requester origin atomically when creating sqlite tasks", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-create-origin-" },
      async () => {
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("prunes stale sqlite delivery state while retaining current rows", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-delivery-prune-" },
      async () => {
  ...
  it("prunes large sqlite snapshots without binding every task id at once", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-large-prune-" },
      async () => {
  ...
  it("reopens after the shared state database is closed", async () => {
    await withACTAgentTestState(
      { layout: "state-only", prefix: "actagent-task-store-" },
      async () => {
  ...
  it("hardens the sqlite task store directory and file modes", async () => {
    ...
    await withACTAgentTestState(
      ...
      async () => {
        ...
        expect(databasePath.endsWith(path.join("state", "actagent.sqlite"))).toBe(true);

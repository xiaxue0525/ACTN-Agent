...
import { normalizeModelCatalog } from "@actagent/model-catalog-core/model-catalog-normalize";
...
const ACTAGENT_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): ACTAgentProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
  const actagenthubSpec = normalizeOptionalString(value.actagenthubSpec);
  const parsedACTAgentHub = actagenthubSpec ? parseACTAgentHubPluginSpec(actagenthubSpec) : null;
  ...
  if (!parsedACTAgentHub && !parsedNpm) {
    return undefined;
  }
  const defaultChoice =
    value.defaultChoice === "actagenthub" && parsedACTAgentHub
      ? "actagenthub"
  ...
  return {
    ...(parsedACTAgentHub ? { actagenthubSpec } : {}),
...
function normalizePlugin(value: unknown): ACTAgentProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): ACTAgentProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): ACTAgentProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): ACTAgentProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly ACTAgentProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is ACTAgentProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): ACTAgentProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeACTAgentProviderIndex(value: unknown): ACTAgentProviderIndex | undefined {
  if (!isRecord(value) || value.version !== ACTAGENT_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, ACTAgentProviderIndexProvider> = {};
  ...
  return {
    version: ACTAGENT_PROVIDER_INDEX_VERSION,

...
import {
  buildACTAgentCompileCacheRespawnPlan,
  isSourceCheckoutInstallRoot,
  resolveACTAgentCompileCacheDirectory,
  resolveEntryInstallRoot,
  runACTAgentCompileCacheRespawnPlan,
  shouldEnableACTAgentCompileCache,
...
describe("entry compile cache", () => {
  ...
  it("resolves install roots from source and dist entry paths", () => {
    expect(resolveEntryInstallRoot("/repo/actagent/src/entry.ts")).toBe("/repo/actagent");
    expect(resolveEntryInstallRoot("/repo/actagent/dist/entry.js")).toBe("/repo/actagent");
    expect(resolveEntryInstallRoot("/pkg/actagent/entry.js")).toBe("/pkg/actagent");
  });

  it("treats git and source entry markers as source checkouts", async () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-source-");
    await fs.writeFile(path.join(root, ".git"), "gitdir: .git/worktrees/actagent\n", "utf8");

    expect(isSourceCheckoutInstallRoot(root)).toBe(true);
  });

  it("disables compile cache for source-checkout installs", async () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-src-entry-");
  ...
  it("keeps compile cache enabled for packaged installs unless disabled by env", () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-package-");

    expect(shouldEnableACTAgentCompileCache({ env: {}, installRoot: root })).toBe(true);
    expect(
      shouldEnableACTAgentCompileCache({
  ...
  it("scopes packaged compile cache by package install metadata", async () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-package-key-");
    const packageJsonPath = path.join(root, "package.json");
    await fs.writeFile(packageJsonPath, '{"version":"2026.4.29"}\n', "utf8");

    const directory = resolveACTAgentCompileCacheDirectory({
      env: { NODE_COMPILE_CACHE: path.join(root, ".node-cache") },
      installRoot: root,
    });

    expect(directory).toContain(path.join(".node-cache", "actagent"));
  ...
  it("builds a one-shot no-cache respawn plan when source checkout inherits NODE_COMPILE_CACHE", async () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-respawn-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");

    const plan = buildACTAgentCompileCacheRespawnPlan({
      currentFile: path.join(root, "dist", "entry.js"),
      env: { NODE_COMPILE_CACHE: "/tmp/actagent-cache" },
    ...
    expect(plan).toEqual({
      ...
      env: {
        ...
        ACTAGENT_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
      },
    });
  });

  it("does not respawn packaged installs when NODE_COMPILE_CACHE is configured", () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-package-respawn-");
  ...
  it("does not respawn source checkouts twice", async () => {
    const root = makeTempDir(tempDirs, "actagent-compile-cache-respawn-once-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");
    ...
    expect(
      buildACTAgentCompileCacheRespawnPlan({
        currentFile: path.join(root, "dist", "entry.js"),
        env: {
          NODE_COMPILE_CACHE: "/tmp/actagent-cache",
          ACTAGENT_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
        },
        installRoot: root,
      }),
  ...
  it("runs compile-cache respawn plans with the child-process bridge", () => {
    ...
    runACTAgentCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/actagent/dist/entry.js", "status"],
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
      {
    ...
    expect(spawn).toHaveBeenCalledWith(
      "/usr/bin/node",
      ["/repo/actagent/dist/entry.js", "status"],
      {
        stdio: "inherit",
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
  ...
  it("marks signal-terminated compile-cache respawn children as failed without forcing another exit", () => {
    ...
    runACTAgentCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/actagent/dist/entry.js"],
        env: {},
      },
      {
        spawn: spawn as unknown as typeof import("node:child_process").spawn,

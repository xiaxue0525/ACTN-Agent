...
/** Resolves `~` and ACTAgent home-relative paths with injectable env/home sources. */
...
/** Resolves the ACTAgent config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".actagent");
  try {
...
/** Resolves the effective ACTAgent home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ACTAGENT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ACTAGENT_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$ACTAGENT_HOME`. */
...
// Configuration root; can be overridden via ACTAGENT_STATE_DIR.

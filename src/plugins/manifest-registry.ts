...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: ACTAgentPackageManifest["channel"];
}): Record<string, PluginManifestChannelConfig> | undefined {
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "actagent",
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    pluginId: sanitizeForLog(params.record.id),
    source: sanitizeForLog(params.record.manifestPath),
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add actagent.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
...
function isTrustedOfficialPluginInstall(params: {
  ...
  if (
    installRecord.source === "actagenthub" &&
    officialInstall?.actagenthubSpec &&
    installRecord.actagenthubChannel === "official" &&
    (installRecord.actagenthubPackage === packageName ||
      installRecord.spec === officialInstall.actagenthubSpec ||
      installRecord.resolvedSpec === officialInstall.actagenthubSpec)
  ) {
    return true;
  }
  return false;
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "actagent") === "bundle";
    const manifestRes:
          ...
          message:
            minHostVersionCheck.kind === "invalid"
              ...
              : minHostVersionCheck.kind === "unknown_host_version"
                ? `plugin requires ACTAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
                : `plugin requires ACTAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
        });
        continue;
      }

...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as ACTAgentConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as ACTAgentConfig;
    const load = vi.fn(async (config?: ACTAgentConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: ACTAgentConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );

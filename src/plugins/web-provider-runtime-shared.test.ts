...
vi.mock("./loader.js", () => ({
  ...
  loadACTAgentPlugins: mocks.loadACTAgentPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadACTAgentPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadACTAgentPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadACTAgentPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadACTAgentPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadACTAgentPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadACTAgentPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});

...
import { sortUniqueStrings } from "@actagent/normalization-core/string-normalization";
...
type LoadACTAgentPlugins = typeof import("./loader.js").loadACTAgentPlugins;
...
const loadACTAgentPluginsMock = vi.fn<LoadACTAgentPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/actagent.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadACTAgentPluginsMock, "ACTAgent plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadACTAgentPluginsMock, "ACTAgent plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: ACTAgentConfig = {
    plugins: {},
  };
  const autoEnabledConfig: ACTAgentConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadACTAgentPlugins: (...args: Parameters<LoadACTAgentPlugins>) =>
        loadACTAgentPluginsMock(...args),

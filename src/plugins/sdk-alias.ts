...
import { resolveACTAgentDevSourceRoot } from "./dev-source-root.js";
...
function hasTrustedACTAgentRootIndicator(params: {
  packageRoot: string;
  ...
  const hasACTAgentBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("actagent")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.actagent === "string");
  const hasACTAgentEntrypoint = fs.existsSync(path.join(params.packageRoot, "actagent.mjs"));
  return hasCliEntryExport || hasACTAgentBin || hasACTAgentEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedACTAgentRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedACTAgentRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedACTAgentRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveACTAgentDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveACTAgentPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedACTAgentRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["actagent/plugin-sdk", "@actagent/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@actagent/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@actagent/gateway-client",
  ...
  {
    packageName: "@actagent/gateway-protocol",
  ...
  {
    packageName: "@actagent/markdown-core",
  ...
  {
    packageName: "@actagent/media-generation-core",
  ...
  {
    packageName: "@actagent/media-core",
  ...
  {
    packageName: "@actagent/normalization-core",
  ...
  {
    packageName: "@actagent/terminal-core",
  ...
  {
    packageName: "@actagent/net-policy",
  ...
  {
    packageName: "@actagent/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveACTAgentPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@actagent/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.ACTAGENT_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `actagent/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "actagent/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  actAgentPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.actAgentPackageRoot, "extensions"),
    path.join(params.actAgentPackageRoot, "dist", "extensions"),
    path.join(params.actAgentPackageRoot, "dist-runtime", "extensions"),

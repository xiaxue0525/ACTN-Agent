...
import { uniqueValues } from "@actagent/normalization-core/string-normalization";
import {
...
import type {
  ...
  ACTAgentPluginApi,
  ACTAgentPluginChannelRegistration,
  ACTAgentPluginCliCommandDescriptor,
  ACTAgentPluginCliRegistrar,
  ACTAgentPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  ACTAgentPluginGatewayRuntimeScopeSurface,
  ACTAgentGatewayDiscoveryService,
  ACTAgentPluginHostedMediaResolver,
  ACTAgentPluginHttpRouteParams,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@actagent/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@actagent/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("actagent.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<ACTAgentPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<ACTAgentPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<ACTAgentPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: ACTAgentPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: ACTAgentPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: ACTAgentPluginHookOptions | undefined,
    config: ACTAgentPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: ACTAgentPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: ACTAgentPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: ACTAgentPluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as ACTAgentPluginChannelRegistration).plugin === "object"
        ? (registration as ACTAgentPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is ACTAgentPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: ACTAgentPluginReloadRegistration) => {
    const normalized: ACTAgentPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: ACTAgentPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: ACTAgentPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: ACTAgentPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: ACTAgentPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: ACTAgentGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: ACTAgentPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<ACTAgentPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<ACTAgentPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as ACTAgentConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as ACTAgentConfig | undefined) ??
                    params.config;

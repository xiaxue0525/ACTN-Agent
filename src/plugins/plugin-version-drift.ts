...
import {
  resolveTrustedSourceLinkedOfficialACTAgentHubInstall,
...
function isPluginEnabled(config: ACTAgentConfig | undefined, pluginId: string): boolean {
...
function shouldCompareOfficialInstallToGateway(params: {
  ...
  const officialACTAgentHubInstall = resolveTrustedSourceLinkedOfficialACTAgentHubInstall(params);
  if (officialACTAgentHubInstall) {
    if (officialACTAgentHubInstall.actagenthubSpec) {
      return !parseACTAgentHubPluginSpec(officialACTAgentHubInstall.actagenthubSpec)?.version;
    }
    return (
      parseRegistryNpmSpec(officialACTAgentHubInstall.npmSpec ?? "")?.selectorKind !== "exact-version"
    );
  }
  return false;
}
 ...
 *   `version` field of the installed actagent package.json).
 ...
 * @param params.config The merged daemon-side ACTAgentConfig (optional).

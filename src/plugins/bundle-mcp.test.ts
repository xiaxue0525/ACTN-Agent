...
function createEnabledBundleConfig(pluginIds: string[]): ACTAgentConfig {
  return {
    plugins: {
...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("actagent-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("actagent-bundle-mcp-workspace-");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "actagent-bundle-inline-placeholder",
      async ({ homeDir, workspaceDir }) => {

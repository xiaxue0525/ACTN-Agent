...
vi.mock("../loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadACTAgentPlugins: (...args: Parameters<typeof loaderMocks.loadACTAgentPlugins>) =>
      loaderMocks.loadACTAgentPlugins(...args),
  };
});
...
beforeEach(() => {
  loaderMocks.loadACTAgentPlugins.mockReset();
});
...
describe("ensureStandaloneRuntimePluginRegistryLoaded", () => {
  it("reuses a compatible gateway startup registry for gateway-bindable dispatch load options", () => {
    ...
    expect(loaderMocks.loadACTAgentPlugins).not.toHaveBeenCalled();
  });
  ...
  it("loads a fresh registry when dispatch config is not startup-compatible", () => {
    ...
    loaderMocks.loadACTAgentPlugins.mockReturnValue(loadedRegistry);
    ...
    expect(loaderMocks.loadACTAgentPlugins).toHaveBeenCalledOnce();
  });
});

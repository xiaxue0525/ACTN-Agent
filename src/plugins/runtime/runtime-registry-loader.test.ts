...
const mocks = vi.hoisted(() => ({
  loadACTAgentPlugins: vi.fn<typeof import("../loader.js").loadACTAgentPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadACTAgentPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadACTAgentPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadACTAgentPlugins>) => mocks.loadACTAgentPlugins(...args),
    );
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadACTAgentPlugins.mockClear();

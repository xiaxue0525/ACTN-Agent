/** Updates installed plugins across npm, ACTAgentHub, marketplace, Git, and bundled bridge sources. */
...
import { satisfiesPluginApiRange } from "../infra/actagenthub.js";
import type { NpmSpecResolution } from "../infra/install-source-utils.js";
import { createNpmMetadataEnv, resolveNpmSpecMetadata } from "../infra/install-source-utils.js";
import {
  compareACTAgentReleaseVersions,
  isExactSemverVersion,
  isACTAgentOrgNpmSpec,
...
import {
  ...
  installedPackageNeedsACTAgentPeerLinkRepair,
...
import { buildACTAgentHubPluginInstallRecordFields } from "./actagenthub-install-records.js";
import { ACTAGENTHUB_INSTALL_ERROR_CODE, installPluginFromACTAgentHub } from "./actagenthub.js";
import { normalizePluginsConfig, resolveEffectiveEnableState } from "./config-state.js";
import {
  getExternalizedBundledPluginLegacyPathSuffix,
  getExternalizedBundledPluginACTAgentHubSpec,
...
import {
  resolveTrustedSourceLinkedOfficialACTAgentHubSpec,
...
import { linkACTAgentPeerDependencies } from "./plugin-peer-link.js";
...
function formatACTAgentHubInstallFailure(params: {
  pluginId: string;
  spec: string;
  phase: "check" | "update";
  error: string;
}): string {
  return `Failed to ${params.phase} ${params.pluginId}: ${params.error} (ACTAgentHub ${params.spec}).`;
}
...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareACTAgentReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
> {
  ...
  if (
    !parsedSpec ||
    !parsedSpec.name.startsWith("@actagent/") ||
...
function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageACTAgent?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageACTAgent);
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareACTAgentReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackACTAgentHubBridgeToNpm(params: {
  result: { ok: false; code?: string };
  npmSpec?: string;
}): boolean {
  if (!isACTAgentOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
    params.result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    params.result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND ||
    params.result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.ARTIFACT_DOWNLOAD_UNAVAILABLE ||
    params.result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.ARTIFACT_UNAVAILABLE
  );
}

function shouldFallbackACTAgentHubToDefault(result: { ok: false; code?: string }): boolean {
  return (
    result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    result.code === ACTAGENTHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND
  );
}

function shouldFallbackBetaACTAgentHubUpdate(result: { ok: false; code?: string }): boolean {
  return shouldFallbackACTAgentHubToDefault(result);
}
...
function resolveTrustedSourceLinkedOfficialNpmFallbackForACTAgentHubUpdate(params: {
  pluginId: string;
  record: PluginInstallRecord;
  effectiveACTAgentHubSpec?: string;
  recordACTAgentHubSpec?: string;
  ...
  if (!isOfficialACTAgentHubInstallRecord(params.record)) {
    return null;
  }
  ...
  const recordedPackageNames = [
    params.record.actagenthubPackage,
    resolveACTAgentHubSpecPackageName(params.record.spec),
    resolveACTAgentHubSpecPackageName(params.effectiveACTAgentHubSpec),
  ].filter((value): value is string => Boolean(value));
  if (!recordedPackageNames.includes(officialPackageName)) {
    return null;
  }

  const effectiveACTAgentHubVersion = params.effectiveACTAgentHubSpec
    ? parseACTAgentHubPluginSpec(params.effectiveACTAgentHubSpec)?.version
    : undefined;
  const recordACTAgentHubVersion = params.recordACTAgentHubSpec
    ? parseACTAgentHubPluginSpec(params.recordACTAgentHubSpec)?.version
    : undefined;
  if (effectiveACTAgentHubVersion && effectiveACTAgentHubVersion.toLowerCase() !== "latest") {
    return {
      installSpec: `${officialPackageName}@${effectiveACTAgentHubVersion}`,
      recordSpec:
        recordACTAgentHubVersion && recordACTAgentHubVersion.toLowerCase() !== "latest"
          ? `${officialPackageName}@${recordACTAgentHubVersion}`
          : officialSpec,
      ...(params.updateChannel === "beta" && effectiveACTAgentHubVersion.toLowerCase() === "beta"
...
function isBridgeACTAgentHubInstall(params: {
  bridge: ExternalizedBundledPluginBridge;
  record: PluginInstallRecord;
}): boolean {
  ...
  const actagenthubSpec = getExternalizedBundledPluginACTAgentHubSpec(params.bridge);
  const bridgeACTAgentHubPackage = actagenthubSpec ? parseACTAgentHubPluginSpec(actagenthubSpec)?.name : undefined;
  const recordACTAgentHubPackage =
    params.record.actagenthubPackage ?? parseACTAgentHubPluginSpec(params.record.spec ?? "")?.name;
  return Boolean(bridgeACTAgentHubPackage && recordACTAgentHubPackage === bridgeACTAgentHubPackage);
}
...
function resolveACTAgentHubUpdateSpecs(params: {
  record: PluginInstallRecord;
  officialSpecOverride?: string;
  updateChannel?: UpdateChannel;
}): {
  ...
  if (!params.officialSpecOverride && !params.record.actagenthubPackage) {
    return {};
  }
  const recordSpec =
    params.officialSpecOverride ?? params.record.spec ?? `actagenthub:${params.record.actagenthubPackage}`;
...
function isBridgeAlreadyInstalledFromPreferredSource(params: {
  ...
  return preferredSource === "actagenthub"
    ? isBridgeACTAgentHubInstall(params)
...
function migratePluginConfigId(cfg: ACTAgentConfig, fromId: string, toId: string): ACTAgentConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function withoutPluginInstallRecord(cfg: ACTAgentConfig, pluginId: string): ACTAgentConfig {
  const installs = cfg.plugins?.installs;
...
function resetDisabledPluginSlots(
  slots: NonNullable<ACTAgentConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<ACTAgentConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: ACTAgentConfig, pluginId: string): ACTAgentConfig {
...
async function repairACTAgentPeerLinksForNpmInstalls(params: {
  config: ACTAgentConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair actagent peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsACTAgentPeerLinkRepair(installPath)) {
      continue;
    }
    ...
    try {
      ...
      const peerLinkRepair = await linkACTAgentPeerDependencies({
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair actagent peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsACTAgentPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair actagent peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (
    ...
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; ACTAgent will continue without it. ` +
        message;
  ...
  for (const pluginId of targets) {
    ...
    const officialNpmSpec = params.syncOfficialPluginInstalls
      ...
      : undefined;
    const officialACTAgentHubSpec = params.syncOfficialPluginInstalls
      ? resolveTrustedSourceLinkedOfficialACTAgentHubSpec({ pluginId, record })
      : undefined;
    ...
    if (normalizedPluginConfig) {
      ...
      if (!enableState.enabled && !officialNpmSpec && !officialACTAgentHubSpec) {
        outcomes.push({
          pluginId,
          status: "skipped",
    ...
    const npmSpecs =
      record.source === "npm"
        ...
        : undefined;
    const actagenthubSpecs =
      record.source === "actagenthub"
        ? resolveACTAgentHubUpdateSpecs({
            record,
            officialSpecOverride: officialACTAgentHubSpec,
            updateChannel: params.updateChannel,
          })
        : undefined;
    const effectiveSpec =
      record.source === "npm"
        ...
        : record.source === "actagenthub"
          ? actagenthubSpecs?.installSpec
          : record.spec;
    const recordSpec =
      record.source === "npm"
        ? npmSpecs?.recordSpec
        : record.source === "actagenthub"
          ? actagenthubSpecs?.recordSpec
          : record.spec;
    const officialNpmFallbackSpecs =
      record.source === "actagenthub"
        ? resolveTrustedSourceLinkedOfficialNpmFallbackForACTAgentHubUpdate({
            pluginId,
            record,
            effectiveACTAgentHubSpec: effectiveSpec,
            recordACTAgentHubSpec: recordSpec,
            updateChannel: params.updateChannel,
          })
        : null;
    ...
    let activeACTAgentHubInstallSpec = effectiveSpec;
    ...
    if (record.source === "actagenthub" && !record.actagenthubPackage && !officialACTAgentHubSpec) {
      outcomes.push({
        pluginId,
        status: "skipped",
        message: `Skipping "${pluginId}" (missing ACTAgentHub package metadata).`,
      });
      continue;
    }

    if (record.source === "actagenthub" || record.source === "marketplace") {
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !installedPackageNeedsACTAgentPeerLinkRepair(installPath) &&
    ...
    if (params.dryRun) {
      let probe:
        ...
        | Awaited<ReturnType<typeof installPluginFromACTAgentHub>>
          ...
          record.source === "npm"
            ...
            : record.source === "actagenthub"
              ? await installPluginFromACTAgentHub({
                  spec: effectiveSpec ?? `actagenthub:${record.actagenthubPackage!}`,
                  config: params.config,
                  baseUrl: record.actagenthubUrl,
                  mode: "update",
                  extensionsDir,
                  timeoutMs: params.timeoutMs,
                  dryRun: true,
      ...
      if (
        !probe.ok &&
        record.source === "actagenthub" &&
        actagenthubSpecs?.fallbackSpec &&
        shouldFallbackBetaACTAgentHubUpdate(probe)
      ) {
        channelFallbackSuffix = formatBetaChannelFallbackOutcomeSuffix({
          fallbackLabel: actagenthubSpecs.fallbackLabel ?? effectiveSpec,
          fallbackSpec: actagenthubSpecs.fallbackSpec,
          verb: "would use",
        });
        logger.warn?.(
          `Plugin "${pluginId}" has no beta ACTAgentHub release for ${actagenthubSpecs.fallbackLabel ?? effectiveSpec}; using ${actagenthubSpecs.fallbackSpec} instead. Core update can still complete.`,
        );
        probe = await installPluginFromACTAgentHub({
          spec: actagenthubSpecs.fallbackSpec,
          config: params.config,
          baseUrl: record.actagenthubUrl,
          mode: "update",
          extensionsDir,
        ...
        activeACTAgentHubInstallSpec = actagenthubSpecs.fallbackSpec;
      ...
      if (
        ...
        shouldFallbackACTAgentHubBridgeToNpm({
          result: probe,
          npmSpec: officialNpmFallbackInstallSpec,
        })
      ) {
        channelFallbackSuffix = ` (warning: official ACTAgentHub artifact fallback would use ${officialNpmFallbackInstallSpec}).`;
        logger.warn?.(
          `Plugin "${pluginId}" could not download official ACTAgentHub artifact for ${activeACTAgentHubInstallSpec ?? `actagenthub:${record.actagenthubPackage!}`}; using npm ${officialNpmFallbackInstallSpec} instead. Core update can still complete.`,
        );
        usedNpmFallback = true;
      ...
      if (!probe.ok) {
        recordFailure(
          ...
          record.source === "npm" || usedOfficialNpmFallback
            ...
            : record.source === "actagenthub"
              ? formatACTAgentHubInstallFailure({
                  pluginId,
                  spec: activeACTAgentHubInstallSpec ?? `actagenthub:${record.actagenthubPackage!}`,
                  phase: "check",
                  error: probe.error,
                })
              : record.source === "git"
    ...
    activeACTAgentHubInstallSpec = effectiveSpec;
    let npmChannelFallback: PluginUpdateChannelFallback | undefined;
    ...
    if (
      ...
      record.source === "actagenthub" &&
      actagenthubSpecs?.fallbackSpec &&
      shouldFallbackBetaACTAgentHubUpdate(result)
    ) {
      channelFallbackSuffix = formatBetaChannelFallbackOutcomeSuffix({
        fallbackLabel: actagenthubSpecs.fallbackLabel ?? effectiveSpec,
        fallbackSpec: actagenthubSpecs.fallbackSpec,
      ...
      result = await installPluginFromACTAgentHub({
        spec: actagenthubSpecs.fallbackSpec,
        config: params.config,
        baseUrl: record.actagenthubUrl,
        mode: "update",
    ...
    if (
      ...
      usedOfficialNpmFallback = true;
      resultSource = "npm";
      channelFallbackSuffix = ` (warning: official ACTAgentHub artifact fallback used ${officialNpmFallbackInstallSpec}).`;
      result = await installNpmSpecForUpdate({
        spec: officialNpmFallbackInstallSpec,
        config: params.config,
        mode: "update",
    ...
    } else if (resultSource === "actagenthub") {
      const actagenthubResult = result as Extract<
        Awaited<ReturnType<typeof installPluginFromACTAgentHub>>,
        { ok: true }
      >;
      next = recordPluginInstall(next, {
        pluginId: resolvedPluginId,
        ...buildACTAgentHubPluginInstallRecordFields(actagenthubResult.actagenthub),
        spec: recordSpec ?? record.spec ?? `actagenthub:${record.actagenthubPackage!}`,
        installPath: result.targetDir,
        version: nextVersion,
      });
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairACTAgentPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
...
export async function syncPluginsForUpdateChannel(params: {
  ...
  if (params.channel === "dev") {
    ...
    for (const bridge of bridges) {
      ...
      const actagenthubSpec = getExternalizedBundledPluginACTAgentHubSpec(bridge);
      const trustedSourceLinkedOfficialInstall = isTrustedSourceLinkedOfficialBridgeNpmInstall({
        targetPluginId,
        npmSpec,
      });
      let installSource = preferredSource;
      let installSpec = preferredSource === "actagenthub" ? actagenthubSpec : npmSpec;
      let result:
        | Awaited<ReturnType<typeof installPluginFromNpmSpec>>
        | Awaited<ReturnType<typeof installPluginFromACTAgentHub>>;
      ...
      if (preferredSource === "actagenthub") {
        result = await installPluginFromACTAgentHub({
          spec: actagenthubSpec,
          config: params.config,
          ...(bridge.actagenthubUrl ? { baseUrl: bridge.actagenthubUrl } : {}),
          mode: "update",
          expectedPluginId: targetPluginId,
          logger,
        });
        if (!result.ok && npmSpec && shouldFallbackACTAgentHubBridgeToNpm({ result, npmSpec })) {
          const warning = `ACTAgentHub ${actagenthubSpec} unavailable for ${targetPluginId}; falling back to npm ${npmSpec}.`;
      ...
      removeBridgeBundledLoadPaths({ bridge, loadPaths: loadHelpers, env });
      if (installSource === "actagenthub") {
        summary.switchedToACTAgentHub.push(resolvedPluginId);
      } else {

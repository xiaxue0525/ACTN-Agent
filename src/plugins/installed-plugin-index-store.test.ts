...
function makeTempDir() {
  return makeTrackedTempDir("actagent-installed-plugin-index-store", tempDirs);
}
...
function createIndex(overrides: Partial<InstalledPluginIndex> = {}): InstalledPluginIndex {
  return {
    ...
    plugins: [
      {
        pluginId: "demo",
        manifestPath: "/plugins/demo/actagent.plugin.json",
...
describe("installed plugin index persistence", () => {
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/actagent.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/actagent.plugin.json",
  ...
  it("preserves ACTAgentHub actagentpack source facts when refreshing the manifest cache", async () => {
    const stateDir = makeTempDir();
    const installPath = path.join(stateDir, "plugins", "actagentpack-demo");
    await writePersistedInstalledPluginIndex(
      createIndex({
        installRecords: {
          "actagentpack-demo": {
    ...
    expectInstallRecord(index, "actagentpack-demo", expectedRecord);
    expectPluginIds(index, []);
    await expectPersistedIndex(stateDir, {
      pluginIds: [],
      installRecords: {
        "actagentpack-demo": expectedRecord,
      },
    });
  });
});

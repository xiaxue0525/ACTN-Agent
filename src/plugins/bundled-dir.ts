...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "actagent-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "ACTAGENT_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.ACTAGENT_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "actagent.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveACTAgentPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveACTAgentPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveACTAgentPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.ACTAGENT_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.ACTAGENT_BUNDLED_PLUGINS_DIR ?? "",
    ...
    actAgentHome: env.ACTAGENT_HOME ?? "",
    home: env.HOME ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.ACTAGENT_BUNDLED_PLUGINS_DIR?.trim();

  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        actagent?: ACTAgentPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.actagent?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "actagent.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )

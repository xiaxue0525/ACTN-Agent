...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@actagent/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@actagent/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@actagent/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@actagent/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@actagent/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@actagent/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@actagent/discord:dangerous-exec:src/voice/audio.ts",
  "@actagent/google-meet:dangerous-exec:src/node-host.ts",
  "@actagent/google-meet:dangerous-exec:src/realtime.ts",
  "@actagent/matrix:dangerous-exec:src/matrix/deps.ts",
  "@actagent/voice-call:dangerous-exec:src/tunnel.ts",
  "@actagent/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@actagent/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@actagent/acpx:dangerous-exec:dist/service-<hash>.js",
  "@actagent/codex:dangerous-exec:dist/client-<hash>.js",
  "@actagent/google-meet:dangerous-exec:dist/index.js",
  "@actagent/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@actagent/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "actagent-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        actagent?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.actagent?.release?.publishToNpm !== true) {
        return [];
      }

...
import {
  ...
  readACTAgentManagedNpmRootOverrides,
  repairManagedNpmRootACTAgentPeer,
...
import { installedPackageNeedsACTAgentPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
function formatUnresolvedACTAgentPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares actagent as a peer dependency, but ACTAgent could not create a plugin-local node_modules/actagent link. Run from a packaged ACTAgent install or reinstall ACTAgent, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing actagent.extensions; update the plugin package to include actagent.extensions (for example ["./dist/index.js"]). See https://docs.actagent.ai/help/troubleshooting#plugin-install-fails-with-missing-actagent-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "actagent.plugin.json",
  ".codex-plugin/plugin.json",
  ".claude-plugin/plugin.json",
  ".cursor-plugin/plugin.json",
];
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_actagent-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_actagent-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_ACTAGENT_EXTENSIONS: "missing_actagent_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_ACTAGENT_EXTENSIONS: "empty_actagent_extensions",
  INVALID_ACTAGENT_EXTENSIONS: "invalid_actagent_extensions",
...
function validateACTAgentPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: ACTAgentPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json actagent.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this ACTAgent runtime exposes ${params.currentHostVersion}. Upgrade ACTAgent or install a compatible plugin version and retry.`,
...
function validateACTAgentPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json actagent.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires ACTAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set ACTAGENT_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires ACTAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade ACTAgent and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateACTAgentPackageCompatibility({
...
function ensureACTAgentExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ACTAGENT_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json actagent.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ACTAGENT_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_ACTAGENT_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareACTAgentReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@actagent/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official ACTAgent package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official ACTAgent package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official ACTAgent install.`,
  );
...
function validateNpmResolutionCompatibility(params: {
  ...
  return validateACTAgentPackageInstallCompatibility({
    runtime: params.runtime,
    pluginId: params.expectedPluginId ?? params.resolution.name ?? params.parsedSpec.name,
    packageMetadata: params.resolution.packageACTAgent as ACTAgentPackageManifest | undefined,
  });
}
...
async function resolveLatestCompatibleNpmResolution(params: {
  ...
  for (const version of candidates) {
    ...
    if (!compatibilityError) {
      params.logger.warn?.(
        `Resolved ${params.parsedSpec.raw} to ${params.currentResolution.resolvedSpec ?? currentVersion}, but that version is incompatible with this ACTAgent runtime; using newest compatible ${metadataResult.metadata.resolvedSpec ?? spec}.`,
      );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.snapshot) {
    try {
      ...
      await relinkACTAgentPeerDependenciesInManagedNpmRoot({
        npmRoot: params.npmRoot,
  ...
  if (params.packageName !== "actagent") {
    try {
      await repairManagedNpmRootACTAgentPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
        logger: params.logger,
      });
      ...
      params.logger.warn?.(
        `Failed to repair managed npm actagent peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "actagent") {
      continue;
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "actagent") {
      const repairedACTAgentPeer = await repairManagedNpmRootACTAgentPeer({
        npmRoot,
        timeoutMs,
        logger,
      });
      if (repairedACTAgentPeer) {
        logger.info?.(`Repaired stale actagent peer dependency in ${npmRoot}`);
      }
    }
    let preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
    const managedOverrides = await readACTAgentManagedNpmRootOverrides();
    ...
    if (install.code !== 0 && isManagedNpmProjectCorruptionInstallFailure(install)) {
      ...
      try {
        ...
        return await rollbackFailedManagedNpmInstall({
          ok: false,
          error: `npm install failed with a managed npm project corruption signature, but ACTAgent could not quarantine ${npmRoot} for rebuild: ${String(error)}. Original npm error: ${originalError}`,
        });
      }
      logger.warn?.(
    ...
    if (params.packageName !== "actagent") {
      ...
      if (repairedACTAgentPeer) {
        logger.info?.(`Repaired stale actagent peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkACTAgentPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair actagent peer links after npm install: ${String(error)}`,
      });
    }
    if (installedPackageNeedsACTAgentPeerLinkRepair(installRoot)) {
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: formatUnresolvedACTAgentPeerLinkError(params.packageName),
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "actagent security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installBundleFromSourceDir(
  ...
  const packageMetadata = packageManifestResult.manifest
    ...
    : undefined;
  const compatibilityError = validateACTAgentPackageInstallCompatibility({
    runtime,
    pluginId,
    packageMetadata,
  });
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureACTAgentExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid actagent.plugin.json: ${ocManifestResult.error}`,
  ...
  const extensionsResult = ensureACTAgentExtensions({
    manifest,
  });
...
async function scanAndLinkInstalledPackage(params: {
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedACTAgentPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains actagent.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}

...
function isPackageRootACTAgentPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "actagent") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootACTAgentPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedACTAgentPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootACTAgentPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostACTAgentRootRealPath(): Promise<string | null> {
...
function isTrustedHostACTAgentPath(params: {
  resolvedTargetPath: string;
  trustedHostACTAgentRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostACTAgentRootRealPath !== null &&
    isPathInside(params.trustedHostACTAgentRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      ...
      isTrustedHostACTAgentPath({
        resolvedTargetPath,
        trustedHostACTAgentRootRealPath: params.trustedHostACTAgentRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "actagent" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostACTAgentRootRealPath = await resolveTrustedHostACTAgentRootRealPath();
      ...
      if (entry.isSymbolicLink()) {
        ...
        if (pathContainsNodeModulesSegment(relativeNextPath)) {
          const symlinkTargetInspection = await inspectNodeModulesSymlinkTarget({
            ...
            trustedHostACTAgentRootRealPath,
          });
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "actagenthub":
        return { kind: "actagenthub", authority: "actagent", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "actagent-bundled":
        return { kind: "bundled", authority: "actagent", mutable: false, network: false };
      case "actagent-managed":
      case "actagent-extra":
        return { kind: "managed", authority: "actagent", mutable: false, network: false };
      default:

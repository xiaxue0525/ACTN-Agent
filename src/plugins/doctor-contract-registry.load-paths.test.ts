...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "actagent-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): ACTAgentConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: ACTAgentConfig, pluginId: string): Record<string, unknown> {

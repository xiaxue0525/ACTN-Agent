...
import type { ACTAgentStateDatabaseOptions } from "../state/actagent-state-db.js";
...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const actagent = manifest.actagent;
  if (!isRecord(actagent)) {
    return false;
  }
  const extensions = actagent.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "actagent.plugin.json"));
  ...
  if (options.stateDir) {
    return {
      env: {
        ...
        ACTAGENT_STATE_DIR: options.stateDir,
      },
    };
  }
...
function readPersistedInstalledPluginIndexForRecords(
  ...
  try {
    const database = openACTAgentStateDatabase(resolveStateDatabaseOptions(options));

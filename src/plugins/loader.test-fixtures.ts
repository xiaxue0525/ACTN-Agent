...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadACTAgentPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadACTAgentPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "actagent-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ACTAGENT_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".actagent", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ACTAGENT_STATE_DIR: stateDir, ...params.env }, () =>
    loadACTAgentPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}

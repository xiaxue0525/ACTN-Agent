...
function collectConfiguredPluginEntryIds(config: ACTAgentConfig): string[] {
...
function resolveRegister(mod: ACTAgentPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: ACTAgentPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as ACTAgentPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: ACTAgentPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so ACTAgent ignored the plugin setup runtime entry. Remove setup-api/actagent.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}

...
const fixtureRoot = makeTrackedTempDir("actagent-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadACTAgentPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "actagent.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("actagent plugins inspect <id> --runtime --json");
    expect(summary).toContain("actagent plugins list");
  });
});

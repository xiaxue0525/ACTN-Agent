...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".actagent", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "ACTAGENT_HOME", "ACTAGENT_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("actagent-bundle-commands-home-");
      const workspaceDir = await createTempDir("actagent-bundle-commands-workspace-");

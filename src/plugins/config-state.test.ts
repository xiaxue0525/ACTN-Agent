...
describe("normalizePluginsConfig", () => {
  ...
  it.each([
    [{}, undefined],
    [{ slots: { contextEngine: "lossless-actagent" } }, "lossless-actagent"],
  ...
  it("normalizes unknown plugin ids without consulting discovery", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverACTAgentPlugins");
  ...
  it("does not consult discovery or manifests for alias lookup", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverACTAgentPlugins").mockReturnValue({
      candidates: [
        {
          idHint: "anthropic",
          source: "/tmp/actagent-bundled-anthropic/index.js",
          rootDir: "/tmp/actagent-bundled-anthropic",
          origin: "bundled",
          bundledManifest: {
            id: "anthropic",
            configSchema: {},
        ...
        },
        {
          ...
          source: "/tmp/actagent-global-anthropic/index.js",
          rootDir: "/tmp/actagent-global-anthropic",
          origin: "global",
        },
      ],
      diagnostics: [],
    ...
    const loadManifest = vi.spyOn(manifest, "loadPluginManifest").mockReturnValue({
      ok: true,
      manifestPath: "/tmp/actagent-global-anthropic/actagent.plugin.json",
      manifest: {
...
describe("resolveEnableState", () => {
  ...
  it("keeps an explicitly selected workspace context engine enabled when omitted from plugins.allow", () => {
    expectNormalizedEnableState({
      ...
      config: {
        allow: ["telegram"],
        slots: { contextEngine: "lossless-actagent" },
      },
      expected: {
        enabled: true,
      },

...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/actagent-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "actagent.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): ACTAgentConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    ACTAGENT_HOME: path.join(homeDir, "home"),
    ACTAGENT_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    ACTAGENT_DISABLE_PERSISTED_PLUGIN_REGISTRY:

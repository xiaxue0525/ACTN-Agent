...
function resolveMemoryRuntimePluginIds(config: ACTAgentConfig): string[] {
...
function resolveMemoryRuntimeWorkspaceDir(cfg: ACTAgentConfig): string | undefined {
...
function ensureMemoryRuntime(cfg?: ACTAgentConfig) {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: ACTAgentConfig; agentId: string }) {
  return ensureMemoryRuntime(params.cfg)?.resolveMemoryBackendConfig(params) ?? null;
}

/** Closes all active plugin-backed memory search managers. */
export async function closeActiveMemorySearchManagers(cfg?: ACTAgentConfig): Promise<void> {
  void cfg;

...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before ACTAgent consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether ACTAgent should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume ACTAgent's bundle
   ...
   *
   * Use this when the backend should consume a canonical ACTAgent auth profile
   ...
   * `profile-only` treats the selected ACTAgent auth profile as the sole auth
   ...
   * as static config, such as mapping ACTAgent thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside ACTAgent's tool

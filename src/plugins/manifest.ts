/** Loads and normalizes ACTAgent plugin manifests, including contracts and config schemas. */
...
export const PLUGIN_MANIFEST_FILENAME = "actagent.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `actagent qa`, for example `matrix`. */
  commandName: string;
...
// package.json "actagent" metadata (used for setup/catalog)
...
export type PluginPackageInstall = {
  ...
  npmSpec?: string;
  localPath?: string;
  defaultChoice?: "actagenthub" | "npm" | "local";
  minHostVersion?: string;
  expectedIntegrity?: string;
  allowInvalidConfigRecovery?: boolean;
};

export type ACTAgentPackageStartup = {
  /**
...
export type ACTAgentPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type ACTAgentPackageCompat = {
  pluginApi?: string;
};

export type ACTAgentPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: ACTAgentPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  channel?: PluginPackageChannel;
  compat?: ACTAgentPackageCompat;
  install?: PluginPackageInstall;
  startup?: ACTAgentPackageStartup;
};
...
} & Partial<Record<ManifestKey, ACTAgentPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): ACTAgentPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawACTAgent = manifest?.[MANIFEST_KEY] as unknown;
  if (rawACTAgent === undefined || rawACTAgent === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawACTAgent)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json actagent must be an object",
    };
  }
  const raw = rawACTAgent.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json actagent.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json actagent.extensions[${index}] must be a non-empty string`,
      };
    }

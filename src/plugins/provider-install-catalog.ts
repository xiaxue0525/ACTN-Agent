...
import {
  ...
  type ACTAgentProviderIndexProvider,
...
type ProviderInstallCatalogParams = {
  config?: import("../config/types.actagent.js").ACTAgentConfig;
  workspaceDir?: string;
...
function resolveInstallInfoFromInstallRecord(
  ...
  if (record.source === "actagenthub" && record.spec?.trim()) {
    return {
      actagenthubSpec: record.spec.trim(),
...
function resolveInstallInfoFromPackageSource(params: {
  ...
  const actagenthub = isRecord(source?.actagenthub) ? source.actagenthub : undefined;
  ...
  const actagenthubSpec =
    params.origin === "bundled" || params.origin === "config"
      ? normalizeOptionalString(actagenthub?.spec)
      : undefined;
  const localPath = normalizeOptionalString(local?.path);
  if (!actagenthubSpec && !npmSpec && !localPath) {
    return null;
  }
  ...
  return {
    ...
    ...(defaultChoice
      ? { defaultChoice }
      : actagenthubSpec
        ? { defaultChoice: "actagenthub" as const }
        : npmSpec
...
function resolveInstallInfoFromProviderIndex(
  provider: ACTAgentProviderIndexProvider,
): PluginPackageInstall | null {
  ...
  if (!actagenthubSpec && !npmSpec) {
    return null;
  }
  const defaultChoice =
    normalizeDefaultChoice(install.defaultChoice) ?? (actagenthubSpec ? "actagenthub" : "npm");
  return {
    ...(actagenthubSpec ? { actagenthubSpec } : {}),
    ...(npmSpec ? { npmSpec } : {}),
    defaultChoice,

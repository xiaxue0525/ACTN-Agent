...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin index install records store", () => {
  it("writes machine-managed install records outside config", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@actagent/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@actagent/plugin-twitch",
        },
      },
      {
        stateDir,
    ...
    expect(indexPath).toBe(path.join(stateDir, "state", "actagent.sqlite"));
  ...
  it("recovers managed npm metadata when the persisted record points at an older package version", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        codex: {
          source: "npm",
          spec: "@actagent/codex@2026.5.16-beta.1",
          installPath: codexDir,
          version: "2026.5.16-beta.1",
          resolvedName: "@actagent/codex",
          resolvedVersion: "2026.5.16-beta.1",
          resolvedSpec: "@actagent/codex@2026.5.16-beta.1",
    ...
    const record = expectRecordFields(loaded.codex, {
      source: "npm",
      spec: "@actagent/codex@2026.5.18-beta.1",
      installPath: codexDir,
      version: "2026.5.18-beta.1",
      resolvedName: "@actagent/codex",
      resolvedVersion: "2026.5.18-beta.1",
      resolvedSpec: "@actagent/codex@2026.5.18-beta.1",
    });
  ...
  it("keeps recovered managed npm records cached until cache clear after package changes", () => {
    ...
    expectRecordFields(loadInstalledPluginIndexInstallRecordsSync({ stateDir }).codex, {
      ...
      resolvedSpec: "@actagent/codex@2026.5.19-beta.1",
    });
  });
  ...
  it("preserves ACTAgentHub actagentpack install metadata in persisted records", async () => {
    const stateDir = makeStateDir();
    const candidate = createPluginCandidate(stateDir, "actagentpack-demo");
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        "actagentpack-demo": {
          source: "actagenthub",
          spec: "actagenthub:actagentpack-demo",
          installPath: path.join(stateDir, "plugins", "actagentpack-demo"),
    ...
    const loaded = await loadInstalledPluginIndexInstallRecords({ stateDir });
    expectRecordFields(loaded["actagentpack-demo"], {
      source: "actagenthub",
      spec: "actagenthub:actagentpack-demo",
      artifactKind: "npm-pack",
      artifactFormat: "tgz",

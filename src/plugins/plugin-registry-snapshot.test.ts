...
function makeTempDir() {
  return makeTrackedTempDir("actagent-plugin-registry-snapshot", tempDirs);
}

function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    ACTAGENT_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
    ACTAGENT_STATE_DIR: path.join(rootDir, "state"),
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("recovers managed npm plugins missing from a stale persisted registry", () => {
    ...
    expect(result.snapshot.installRecords.whatsapp).toEqual({
      source: "npm",
      spec: "@actagent/whatsapp@2026.5.2",
      installPath: whatsappDir,
      version: "2026.5.2",
      resolvedName: "@actagent/whatsapp",
      resolvedVersion: "2026.5.2",
      resolvedSpec: "@actagent/whatsapp@2026.5.2",
    });
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), ACTAGENT_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
  ...
  it("refreshes a memoized derived snapshot when workspace plugins are installed", () => {
    ...
    writePackagePlugin(path.join(workspaceDir, ".actagent", "extensions", "demo"));
  ...
  it("keeps persisted package plugins when file hashes match", () => {
    ...
    expect(record.manifestFile.size).toBe(
      fs.statSync(path.join(rootDir, "actagent.plugin.json")).size,
    );
  ...
  it("treats persisted registry as stale when a plugin diagnostic source path no longer exists", () => {
    ...
    const config = {};
    const ghostDir = path.join(tempRoot, "extensions", "lossless-actagent");
    const npmPluginDir = writeManagedNpmPlugin({
      stateDir,
      packageName: "@martian-engineering/lossless-actagent",
    ...
    const losslessPlugin = requirePluginRecord(result.snapshot.plugins, "lossless-actagent");

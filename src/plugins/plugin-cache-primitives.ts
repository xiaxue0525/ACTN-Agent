...
export type ConfigScopedRuntimeCache<T> = WeakMap<ACTAgentConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: ACTAgentConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: ACTAgentConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<ACTAgentConfig, Promise<T>>();

  const createPromise = (config?: ACTAgentConfig): Promise<T> => {
  ...
  return {
    async load(config?: ACTAgentConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<ACTAgentConfig, Promise<T>>();
    },
  };
}

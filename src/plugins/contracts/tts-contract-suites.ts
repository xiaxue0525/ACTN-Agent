...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "actagent/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "actagent/plugin-sdk/test-env";
import { beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { AssistantMessage } from "../../llm/types.js";
import { resolveWorkspacePackagePublicModuleUrl } from "../../plugin-sdk/test-helpers/public-surface-loader.js";

type TtsRuntimeModule = typeof import("actagent/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("actagent/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@actagent/speech-core",
...
let completeSimple: typeof import("actagent/plugin-sdk/llm").completeSimple;
...
vi.mock("actagent/plugin-sdk/llm", () => {
...
function asLegacyTtsConfig(value: unknown): ACTAgentConfig {
  return value as ACTAgentConfig;
}

function asLegacyACTAgentConfig(value: Record<string, unknown>): ACTAgentConfig {
  return value as unknown as ACTAgentConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): ACTAgentConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("actagent/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: ACTAgentConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("actagent/plugin-sdk/llm"));
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyACTAgentConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: ACTAgentConfig = asLegacyACTAgentConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ACTAGENT_TTS_PREFS;
      process.env.ACTAGENT_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.ACTAGENT_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: ACTAgentConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,

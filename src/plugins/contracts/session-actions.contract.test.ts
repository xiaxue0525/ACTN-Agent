...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: ACTAgentPluginApi | undefined;
    let workspaceApi: ACTAgentPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: ACTAgentPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: ACTAgentPluginApi | undefined;
      registerTestPlugin({

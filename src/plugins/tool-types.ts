...
export type ACTAgentPluginActiveModelContext = {
  provider?: string;
  modelId?: string;
  modelRef?: string;
};
...
export type ACTAgentPluginToolContext = {
   ...
   * the local operator, installed plugin code, or a modified ACTAgent runtime.
   */
  activeModel?: ACTAgentPluginActiveModelContext;
  browser?: {
...
export type ACTAgentPluginToolFactory = (
  ctx: ACTAgentPluginToolContext,
...
export type ACTAgentPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;

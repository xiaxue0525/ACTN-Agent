...
import { normalizeOptionalString as normalizeString } from "@actagent/normalization-core/string-coerce";
...
function getConfigPath(config: ACTAgentConfig | undefined, path: string): unknown {
...
function setConfigPath(target: ACTAgentConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: ACTAgentConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: ACTAgentConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: ACTAgentConfig) =>

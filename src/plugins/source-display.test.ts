...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "actagent", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".actagent", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".actagent", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "actagent-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".actagent", "extensions"),
      },
    });
  });
});

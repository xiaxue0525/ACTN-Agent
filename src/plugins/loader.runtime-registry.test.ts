...
function makeACTAgentDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-loader-dev-source-"));
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadACTAgentPlugins(loadOptions);

      loadACTAgentPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadACTAgentPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadACTAgentPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadACTAgentPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadACTAgentPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadACTAgentPlugins(loadOptions)).not.toBe(registry);
  });
});

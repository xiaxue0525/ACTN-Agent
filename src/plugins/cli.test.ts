...
const mocks = vi.hoisted(() => ({
  ...
  loadACTAgentPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadACTAgentPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadACTAgentPluginCliRegistry(...args),
  loadACTAgentPlugins: (...args: unknown[]) => mocks.loadACTAgentPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: ACTAgentConfig;
  autoEnabledConfig: ACTAgentConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadACTAgentPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadACTAgentPluginCliRegistry.mockReset();
    mocks.loadACTAgentPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadACTAgentPlugins.mockReset();
    mocks.loadACTAgentPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as ACTAgentConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as ACTAgentConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as ACTAgentConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as ACTAgentConfig);

    const loadOptions = getMockCallObject(mocks.loadACTAgentPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadACTAgentPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as ACTAgentConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as ACTAgentConfig);

    expect(mocks.loadACTAgentPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as ACTAgentConfig;
    const configB = { plugins: {} } as ACTAgentConfig;
    const envA = { ACTAGENT_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { ACTAGENT_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadACTAgentPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadACTAgentPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadACTAgentPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadACTAgentPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as ACTAgentConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadACTAgentPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as ACTAgentConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadACTAgentPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadACTAgentPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadACTAgentPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadACTAgentPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as ACTAgentConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });

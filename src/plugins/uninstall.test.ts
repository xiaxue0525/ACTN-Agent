...
type PluginConfig = NonNullable<ACTAgentConfig["plugins"]>;
...
function createSinglePluginWithEmptySlotsConfig(): ACTAgentConfig {
...
function createSingleNpmInstallConfig(installPath: string): ACTAgentConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "actagent-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as ACTAgentConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/actagent/plugin",
            "/missing/actagent/plugin",
          ),
        },
        loadPaths: ["/missing/actagent/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@actagent", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("actagent-kitchen-sink-fixture"),
        installs: {
          "actagent-kitchen-sink-fixture": {
            source: "npm",
            spec: "@actagent/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
      pluginId: "actagent-kitchen-sink-fixture",
      deleteFiles: true,
      extensionsDir,
    });
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@actagent/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@actagent/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin actagent peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "actagent");
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "actagent"), "missing");
  ...
  it("deletes managed ACTAgentHub install directories", async () => {
    const stateDir = path.join(tempDir, "state");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = resolvePluginInstallDir("actagentpack-demo", extensionsDir);
    await fs.mkdir(installPath, { recursive: true });
    await fs.writeFile(path.join(installPath, "index.js"), "// actagenthub plugin");

    const result = await uninstallPlugin({
      config: createPluginConfig({
        entries: createSinglePluginEntries("actagentpack-demo"),
        installs: {
          "actagentpack-demo": {
            source: "actagenthub",
            spec: "actagenthub:actagentpack-demo@2026.5.1-beta.2",
            installPath,
            actagenthubUrl: "https://actagenthub.ai",
            actagenthubPackage: "actagentpack-demo",
            ...
            npmTarballName: "actagentpack-demo-2026.5.1-beta.2.tgz",
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "actagent-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-actagent-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "actagent-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@actagent", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@actagent/kitchen-sink@latest",
          installPath,
        },
        extensionsDir,
      }),
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@actagent", "kitchen-sink");
  ...
  it("uses configured installPath when ACTAgentHub installed it under the managed extensions root", () => {
    const stateDir = path.join(os.tmpdir(), "actagent-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = resolvePluginInstallDir("actagentpack-demo", extensionsDir);
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@actagent/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@actagent", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("actagent-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "actagent-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "actagent-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);

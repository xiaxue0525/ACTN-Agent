...
import type {
  ACTAgentPluginHookOptions,
  ACTAgentPluginToolFactory,
  ACTAgentPluginToolOptions,
} from "./tool-types.js";
...
export type {
  ACTAgentPluginActiveModelContext,
  ACTAgentPluginHookOptions,
 ...
 */
export type ACTAgentPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<ACTAgentConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). ACTAgent uses this to expose
 ...
 *
 * Runs after ACTAgent resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common ACTAgent auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and ACTAgent wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before ACTAgent builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. ACTAgent no
...
/**
 * Provider-owned stream wrapper hook after ACTAgent applies its generic
 ...
 *
 * Return `true` or `false` to override ACTAgent's built-in provider cache TTL
 ...
 *
 * Runs only after ACTAgent exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that ACTAgent should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after ACTAgent loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, ACTAgent falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * ACTAgent calls this only from async model resolution paths. After it
   ...
   *
   * Runs after ACTAgent performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before ACTAgent registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic ACTAgent wrappers.
   *
   ...
   *
   * Called after ACTAgent resolves the raw configured credential but before the
   ...
   * hint than ACTAgent's generic auth-store guidance.
   */
   ...
   * ACTAgent's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when ACTAgent should surface a provider-specific hint for
   ...
   * merging. ACTAgent deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. ACTAgent stores the
   ...
   * without replacing the full ACTAgent-owned system prompt.
   */
   ...
   *
   * Runs after ACTAgent's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep ACTAgent's default fallback policy.
   */
   ...
   * rewrites after ACTAgent has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before ACTAgent handles
   ...
   *
   * ACTAgent uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `actagent doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `actagent doctor` should offer.
   *
   ...
   *
   * ACTAgent calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep ACTAgent's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * ACTAgent falls back to that stored profile.
   */
...
export type ACTAgentPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral ACTAgent session id when available. */
  sessionId?: string;
  /** Transcript file for this ACTAgent session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active ACTAgent session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current ACTAgent configuration */
  config: ACTAgentConfig;
...
export type ACTAgentPluginCommandDefinition = {
...
export type ACTAgentPluginHttpRouteAuth = "gateway" | "plugin";
export type ACTAgentPluginHttpRouteMatch = "exact" | "prefix";
export type ACTAgentPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type ACTAgentPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type ACTAgentPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type ACTAgentPluginHttpRouteParams = {
  path: string;
  handler: ACTAgentPluginHttpRouteHandler;
  handleUpgrade?: ACTAgentPluginHttpRouteUpgradeHandler;
  auth: ACTAgentPluginHttpRouteAuth;
  match?: ACTAgentPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: ACTAgentPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type ACTAgentPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type ACTAgentPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `actagent` program. For nested
...
export type ACTAgentPluginCliRegistrar = (ctx: ACTAgentPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want ACTAgent to keep a plugin command lazy-loaded while still
 ...
 */
export type ACTAgentPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type ACTAgentPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `actagent nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `actagent nodes`, so a descriptor named
   * `"camera"` exposes `actagent nodes camera`.
   */
  descriptors?: ACTAgentPluginCliCommandDescriptor[];
};

export type ACTAgentPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type ACTAgentPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type ACTAgentPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type ACTAgentPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type ACTAgentPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: ACTAgentPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type ACTAgentPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: ACTAgentPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<ACTAgentPluginNodeInvokeTransportResult>;
};

export type ACTAgentPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type ACTAgentPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  ) => Promise<ACTAgentPluginNodeInvokePolicyResult> | ACTAgentPluginNodeInvokePolicyResult;
};

export type ACTAgentPluginSecurityAuditContext = {
  config: ACTAgentConfig;
  sourceConfig: ACTAgentConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type ACTAgentPluginSecurityAuditCollector = (
  ctx: ACTAgentPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type ACTAgentGatewayDiscoveryAdvertiseContext = {
...
export type ACTAgentGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: ACTAgentGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type ACTAgentPluginServiceContext = {
  config: ACTAgentConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type ACTAgentPluginService = {
  id: string;
  start: (ctx: ACTAgentPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: ACTAgentPluginServiceContext) => void | Promise<void>;
};

export type ACTAgentPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type ACTAgentPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: ACTAgentPluginReloadRegistration;
  nodeHostCommands?: ACTAgentPluginNodeHostCommand[];
  securityAuditCollectors?: ACTAgentPluginSecurityAuditCollector[];
  register?: (api: ACTAgentPluginApi) => void;
  activate?: (api: ACTAgentPluginApi) => void;
};

export type ACTAgentPluginModule = ACTAgentPluginDefinition | ((api: ACTAgentPluginApi) => void);
...
export type PluginConfigMigration = (config: ACTAgentConfig) =>
  | {
      config: ACTAgentConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `actagent migrate`. */
...
export type ACTAgentPluginSessionStateApi = {
...
export type ACTAgentPluginSessionWorkflowApi = {
...
export type ACTAgentPluginSessionControlsApi = {
...
export type ACTAgentPluginSessionApi = {
  state: ACTAgentPluginSessionStateApi;
  workflow: ACTAgentPluginSessionWorkflowApi;
  controls: ACTAgentPluginSessionControlsApi;
};

export type ACTAgentPluginAgentEventsApi = {
...
export type ACTAgentPluginAgentApi = {
  events: ACTAgentPluginAgentEventsApi;
};

export type ACTAgentPluginRunContextApi = {
...
export type ACTAgentPluginLifecycleApi = {
...
export type ACTAgentPluginApi = {
   ...
   */
  session: ACTAgentPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: ACTAgentPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: ACTAgentPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: ACTAgentPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | ACTAgentPluginToolFactory,
    opts?: ACTAgentPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: ACTAgentPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: ACTAgentPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: ACTAgentPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: ACTAgentPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: ACTAgentPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, ACTAgent
  ...
  /**
   * Register a plugin-owned node feature command group under `actagent nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: ACTAgentPluginCliRegistrar,
    opts?: ACTAgentPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: ACTAgentPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: ACTAgentPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: ACTAgentPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: ACTAgentPluginSecurityAuditCollector) => void;
  registerService: (service: ACTAgentPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: ACTAgentGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `actagent migrate` (migration capability). */
   ...
   */
  registerCommand: (command: ACTAgentPluginCommandDefinition) => void;

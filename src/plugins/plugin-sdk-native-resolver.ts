/** Installs native Node resolution aliases so plugins can import the ACTAgent SDK in dev and tests. */
...
export type InstallACTAgentPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["actagent/plugin-sdk", "@actagent/plugin-sdk"] as const;
...
function resolveLoaderModulePath(options: InstallACTAgentPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "actagent" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { actagent?: unknown }).actagent === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(
  options: InstallACTAgentPluginSdkNativeResolverOptions,
): string[] {
...
export function installACTAgentPluginSdkNativeResolver(
  options: InstallACTAgentPluginSdkNativeResolverOptions = {},
): string[] {
...
export function installACTAgentInternalCorePackageNativeResolver(
  options: Pick<InstallACTAgentPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  for (const alias of listInternalCorePackageNativeAliases(options)) {
    registerNativeAlias(alias);
  }
...
export function resetACTAgentPluginSdkNativeResolverForTest(): void {

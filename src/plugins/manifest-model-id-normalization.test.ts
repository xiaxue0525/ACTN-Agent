...
import { listACTAgentPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  ACTAGENT_BUNDLED_PLUGINS_DIR: process.env.ACTAGENT_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function restoreACTAgentStateDirEnv(): void {
  const value = ORIGINAL_ENV.ACTAGENT_STATE_DIR;
  if (value === undefined) {
    delete process.env.ACTAGENT_STATE_DIR;
  } else {
    process.env.ACTAGENT_STATE_DIR = value;
  }
}

function restoreACTAgentHomeEnv(): void {
  const value = ORIGINAL_ENV.ACTAGENT_HOME;
  if (value === undefined) {
    delete process.env.ACTAGENT_HOME;
  } else {
    process.env.ACTAGENT_HOME = value;
  }
}

function restoreACTAgentDisableBundledPluginsEnv(): void {
  const value = ORIGINAL_ENV.ACTAGENT_DISABLE_BUNDLED_PLUGINS;
  if (value === undefined) {
    delete process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS = value;
  }
}

function restoreACTAgentBundledPluginsDirEnv(): void {
  const value = ORIGINAL_ENV.ACTAGENT_BUNDLED_PLUGINS_DIR;
  if (value === undefined) {
    delete process.env.ACTAGENT_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = value;
  }
}

function restoreEnv(): void {
  restoreACTAgentStateDirEnv();
  restoreACTAgentHomeEnv();
  restoreACTAgentDisableBundledPluginsEnv();
  restoreACTAgentBundledPluginsDirEnv();
}

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "actagent.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/actagent.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: ACTAgentConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    process.env.ACTAGENT_STATE_DIR = stateDirA;
    process.env.ACTAGENT_HOME = undefined;
    process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.ACTAGENT_STATE_DIR = stateDirB;
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listACTAgentPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listACTAgentPluginManifestMetadata(process.env)).toHaveLength(1);

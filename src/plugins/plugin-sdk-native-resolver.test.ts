...
import {
  installACTAgentPluginSdkNativeResolver,
  resetACTAgentPluginSdkNativeResolverForTest,
...
function writeFakeACTAgentPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "actagent.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installACTAgentPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(stableRoot);
    writeFakeACTAgentPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.ACTAGENT_DEV_SOURCE_ROOT;
    process.env.ACTAGENT_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installACTAgentPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("actagent/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("actagent/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.ACTAGENT_DEV_SOURCE_ROOT;
      } else {
        process.env.ACTAGENT_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("actagent/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("actagent/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(root);
    ...
    expect(installedAliases).toContain("actagent/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("actagent/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve ACTAgent SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeACTAgentPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("actagent/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("actagent/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("actagent/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installACTAgentPluginSdkNativeResolver, resetACTAgentPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"actagent/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"actagent/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installACTAgentPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("actagent/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("actagent/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for ACTAgent-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(root);
    ...
    expect(installedAliases).toContain("@actagent/normalization-core/string-coerce");
    expect(installedAliases).toContain("@actagent/media-core/mime");
    expect(installedAliases).toContain("@actagent/acp-core/runtime/types");
    expect(installedAliases).toContain("@actagent/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@actagent/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@actagent/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@actagent/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@actagent/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@actagent/normalization-core/string-coerce")).toThrow();
    expect(() => requireFromPlugin.resolve("@actagent/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@actagent/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@actagent/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(root);
    ...
    expect(installedAliases).toContain("actagent/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("actagent/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("actagent/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeACTAgentPackage(root);
    ...
    expect(installedAliases).toContain("actagent/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("actagent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("actagent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("actagent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("actagent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("actagent/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});

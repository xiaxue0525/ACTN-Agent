...
describe("resolveInitialTuiAgentId", () => {
  const cfg: ACTAgentConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/actagent" },
        { id: "ops", workspace: "/tmp/actagent/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/actagent/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run actagent devices list");
    expect(state.pairingHint).toContain("actagent devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("actagent tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/actagent.mjs", "models", "auth", "login"],
    });
  });
});

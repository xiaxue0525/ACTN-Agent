...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.ACTAGENT_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.ACTAGENT_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      ...
      let fastMode = process.env.ACTAGENT_TUI_PTY_FAST_MODE === "true";
      ...
      async function main() {
        await runTui({
          ...
          deliver: false,
          historyLimit: 5,
          title: "actagent tui pty fixture",
        });
      }
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "actagent-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ACTAGENT_THEME: "dark",
      ACTAGENT_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },

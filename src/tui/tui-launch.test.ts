...
describe("launchTuiCli", () => {
  beforeEach(() => {
    ...
    process.argv[1] = "/repo/actagent.mjs";
  ...
  it("passes local mode through to the relaunched TUI", async () => {
    ...
    const options = expectSpawned(["/repo/actagent.mjs", "tui", "--local"]);
    expect(options.stdio).toBe("inherit");
  });
  ...
  it("pins the child gateway URL and config auth source through env without adding url argv", async () => {
    ...
    await launchTuiCli(
      { deliver: false },
      { authSource: "config", gatewayUrl: "ws://127.0.0.1:19199" },
    );

    const options = expectSpawned(["/repo/actagent.mjs", "tui"]);
    expect(options.env?.ACTAGENT_GATEWAY_URL).toBe("ws://127.0.0.1:19199");
    expect(options.env?.ACTAGENT_TUI_SETUP_AUTH_SOURCE).toBe("config");
  });
});

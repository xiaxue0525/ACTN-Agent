...
function hasProviderWildcardModelAllowlist(cfg: ACTAgentConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: ACTAgentConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: ACTAgentConfig) {
  return cfg.models?.mode === "replace" && hasProviderWildcardModelAllowlist(cfg);
}

async function loadEmbeddedTuiModelCatalog(cfg: ACTAgentConfig) {

  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: {
          "tui-pty-mock/gpt-5.5": { agentRuntime: { id: "actagent" } },
        },
        skills: [],
        skipBootstrap: true,
      },
...
async function startLocalModeTui() {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "actagent-tui-pty-local-"));
  ...
  const script = [
    ...
    `program.parseAsync([process.execPath, "actagent", "tui", "--local"], { from: "node" }).catch((error) => {`,
    `  console.error(error);`,
    `  process.exit(1);`,
    `});`,
  ].join("\n");
  ...
  const run = startPty(process.execPath, ["--import", "tsx", "--eval", script], {
    ...
    env: {
      ...
      ACTAGENT_THEME: "dark",
      ACTAGENT_CODEX_DISCOVERY_LIVE: "0",
      NO_COLOR: undefined,
    },

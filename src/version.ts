...
declare const __ACTAGENT_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "actagent";
...
function readInjectedVersion(): string | undefined {
  return typeof __ACTAGENT_VERSION__ === "string" ? __ACTAGENT_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["ACTAGENT_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["ACTAGENT_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["ACTAGENT_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.ACTAGENT_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current ACTAgent version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.ACTAGENT_BUNDLED_VERSION,
});

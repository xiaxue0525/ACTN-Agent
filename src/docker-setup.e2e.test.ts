...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "actagent-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ACTAGENT_GATEWAY_TOKEN: "test-token",
    ACTAGENT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ACTAGENT_WORKSPACE_DIR: join(sandbox.rootDir, "actagent"),
    ACTAGENT_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("actagent-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    ACTAGENT_CONFIG_DIR: configDir,
    ACTAGENT_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_DOCKER_APT_PACKAGES: "curl wget",
      ACTAGENT_EXTRA_MOUNTS: undefined,
      ACTAGENT_HOME_VOLUME: "actagent-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("ACTAGENT_EXTRA_MOUNTS=");
    expect(envFile).toContain("ACTAGENT_HOME_VOLUME=actagent-home"); // pragma: allowlist secret
    expect(envFile).toContain("ACTAGENT_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `ACTAGENT_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("actagent-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/actagent`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("actagent-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ACTAGENT_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node actagent-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env ACTAGENT_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node actagent-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:19199","http://127.0.0.1:19199"]}]`,
    );
    expect(log).not.toContain("run --rm actagent-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy ACTAGENT_DOCKER_APT_PACKAGES into ACTAGENT_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("ACTAGENT_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ACTAGENT_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg ACTAGENT_DOCKER_APT_PACKAGES");
  });

  it("prefers ACTAGENT_IMAGE_APT_PACKAGES over legacy ACTAGENT_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_IMAGE_APT_PACKAGES: "curl wget httpie",
      ACTAGENT_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("ACTAGENT_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ACTAGENT_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg ACTAGENT_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty ACTAGENT_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg ACTAGENT_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network actagent-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\bactagent-cli\b/.test(line),
    );
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_HOME: "/mnt/c/Users/Trevor",
      ACTAGENT_STATE_DIR: "/mnt/c/Users/Trevor/.actagent",
      ACTAGENT_CONFIG_PATH: "/mnt/c/Users/Trevor/.actagent/actagent.json",
      ACTAGENT_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("writes ACTAGENT_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh actagent-gateway -c");
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_CONFIG_DIR: configDir,
      ACTAGENT_WORKSPACE_DIR: workspaceDir,
      ACTAGENT_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/actagent -xdev");
  });

  it("reuses existing config token when ACTAGENT_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ACTAGENT_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when ACTAGENT_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "ACTAGENT_GATEWAY_TOKEN=dotenv-token-123\nACTAGENT_GATEWAY_PORT=19199\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ACTAGENT_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "ACTAGENT_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ACTAGENT_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("ACTAGENT_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats ACTAGENT_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ACTAGENT_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg ACTAGENT_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker actagent-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        ACTAGENT_SANDBOX: "1",
        ACTAGENT_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps actagent-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate actagent-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --force-recreate actagent-gateway`,
      );
  ...
  it("rejects injected multiline ACTAGENT_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ACTAGENT_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ACTAGENT_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ACTAGENT_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ACTAGENT_HOME_VOLUME must match");
  });

  it("rejects ACTAGENT_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ACTAGENT_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when ACTAGENT_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ACTAGENT_SKIP_ONBOARDING=1");
  });

  it("treats ACTAGENT_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ACTAGENT_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env ACTAGENT_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/ACTAGENT_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/ACTAGENT_DISABLE_BONJOUR: \$\{ACTAGENT_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:actagent-gateway"');
    expect(compose).toContain("depends_on:\n      - actagent-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/ACTAGENT_GATEWAY_TOKEN: \$\{ACTAGENT_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        "${ACTAGENT_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.actagent-auth-profile-secrets}:/home/node/.config/actagent",
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{ACTAGENT_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/ACTAGENT_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/ACTAGENT_STATE_DIR: \/home\/node\/\.actagent$/gm)).toHaveLength(2);
    expect(
      compose.match(/ACTAGENT_CONFIG_PATH: \/home\/node\/\.actagent\/actagent\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/ACTAGENT_CONFIG_DIR: \/home\/node\/\.actagent$/gm)).toHaveLength(2);
    expect(
      compose.match(/ACTAGENT_WORKSPACE_DIR: \/home\/node\/\.actagent\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG ACTAGENT_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG ACTAGENT_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${ACTAGENT_IMAGE_APT_PACKAGES-$ACTAGENT_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg ACTAGENT_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG ACTAGENT_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG ACTAGENT_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG ACTAGENT_IMAGE_APT_PACKAGES");
  });
});

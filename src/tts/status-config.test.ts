...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-tts-status-"));
});
  ...
  await withEnvAsync(
    {
      ...
      ACTAGENT_STATE_DIR: path.join(home, ".actagent"),
    },
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".actagent", "settings", "tts.json");
  ...
  it("derives the default prefs path from ACTAGENT_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".actagent-dev");

#!/usr/bin/env node
// Boots the ACTAgent CLI entry point under Node.
// CLI process entrypoint for ACTAgent command execution.
...
import {
  enableACTAgentCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutACTAgentCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureACTAgentExecMarkerOnProcess } from "./infra/actagent-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "actagent.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "actagent.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutACTAgentCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "actagent";
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.ACTAGENT_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[actagent] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[actagent] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[actagent] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[actagent] Failed to display help:",
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.ACTAGENT_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }

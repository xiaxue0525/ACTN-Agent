// Plugin state SQLite helpers persist plugin state in the ACTAgent state database.
...
import { resolveACTAgentStateSqlitePath } from "../state/actagent-state-db.paths.js";
import {
...
type PluginStateEntriesTable = ACTAgentStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<ACTAgentStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveACTAgentStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveACTAgentStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveACTAgentStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): ACTAgentStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runACTAgentStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveACTAgentStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const wasOpen = cachedDatabase !== null;
  const stateWasOpen = isACTAgentStateDatabaseOpen();
  ...
  try {
    ...
    openACTAgentStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {

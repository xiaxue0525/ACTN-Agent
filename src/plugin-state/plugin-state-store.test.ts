...
beforeAll(async () => {
  testState = await createACTAgentTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveACTAgentStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withACTAgentTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveACTAgentStateSqlitePath(stateA.env)).not.toBe(
              resolveACTAgentStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createACTAgentTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openACTAgentStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveACTAgentStateSqlitePath();

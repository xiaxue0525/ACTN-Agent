...
import { clampTimerTimeoutMs } from "@actagent/normalization-core/number-coercion";
...
type CapabilityProviderCandidate = {
  ...
  isConfigured?: (ctx: { cfg?: ACTAgentConfig; agentDir?: string }) => boolean;
};
...
function resolveCurrentDefaultProviderId(cfg?: ACTAgentConfig): string {
...
function resolveAutoCapabilityFallbackRefs(params: {
  cfg: ACTAgentConfig;
  agentDir?: string;
  listProviders: (cfg?: ACTAgentConfig) => CapabilityProviderCandidate[];
}): string[] {

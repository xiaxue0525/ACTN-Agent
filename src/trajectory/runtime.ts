...
async function replaceTrajectoryWindow(params: {
  ...
  await writeSiblingTempFile({
    ...
    chmodDir: false,
    mode: 0o600,
    tempPrefix: ".actagent-trajectory-",
...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with ACTAGENT_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.ACTAGENT_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }

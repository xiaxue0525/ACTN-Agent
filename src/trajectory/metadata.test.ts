...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/actagent.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.actagent/actagent.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          ACTAGENT_STATE_DIR: "/Users/tester/.actagent",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$ACTAGENT_STATE_DIR/actagent.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/actagent.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.actagent" };

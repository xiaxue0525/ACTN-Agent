...
let compiledSecretTargetRegistryState: {
  ...
  actAgentCompiledSecretTargets: CompiledTargetRegistryEntry[];
  actAgentTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreACTAgentTargetState: {
  knownTargetIds: Set<string>;
  actAgentCompiledSecretTargets: CompiledTargetRegistryEntry[];
  actAgentTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelACTAgentTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const actAgentCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "actagent.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    actAgentCompiledSecretTargets,
    actAgentTargetsById: buildConfigTargetIdIndex(actAgentCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreACTAgentTargetState() {
  if (compiledCoreACTAgentTargetState) {
    return compiledCoreACTAgentTargetState;
  }
  const actAgentCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "actagent.json")
    .map(compileTargetRegistryEntry);
  compiledCoreACTAgentTargetState = {
    knownTargetIds: new Set(actAgentCompiledSecretTargets.map((entry) => entry.id)),
    actAgentCompiledSecretTargets,
    actAgentTargetsById: buildConfigTargetIdIndex(actAgentCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(actAgentCompiledSecretTargets),
  };
  return compiledCoreACTAgentTargetState;
}

function getCompiledChannelACTAgentTargets(
  channelId: string,
  ...
  if (compiledChannelACTAgentTargets.has(normalizedChannelId)) {
    return compiledChannelACTAgentTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "actagent.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelACTAgentTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreACTAgentTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
/**
 * Resolves an actagent.json config path to the matching plan-capable secrets target.
 */
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreACTAgentTargetState().actAgentCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelACTAgentTargets(explicitChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().actAgentCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
/**
 * Discovers configured secret-bearing values in actagent.json using the full registry.
 */
...
/**
 * Discovers configured actagent.json targets, optionally limited to selected registry ids.
 */
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreACTAgentTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreACTAgentTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.actAgentCompiledSecretTargets,
    entriesById: registryState.actAgentTargetsById,
  });

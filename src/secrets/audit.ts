...
import { resolveStateDir, type ACTAgentConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `actagent secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as ACTAgentConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,

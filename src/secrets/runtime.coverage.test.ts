...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: ACTAgentConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "actagent.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.ACTAGENT_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_ACTAGENT_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.ACTAGENT_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.ACTAGENT_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.ACTAGENT_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.ACTAGENT_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectACTAgentCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "actagent.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_ACTAGENT_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForACTAgentTarget(
...
async function expectACTAgentCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = `ACTAGENT_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForACTAgentTarget(config, entry, envId, wildcardToken);
  }
...
const ACTAGENT_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectACTAgentCoverageEntries({ includePluginEntries: false }),
);
const ACTAGENT_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectACTAgentCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = ACTAGENT_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectACTAgentCoverageBatchResolved("actagent.json core", googleChatBatch);
    }
  });

  describe("actagent.json core and channel registry targets", () => {
    test.each(ACTAGENT_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectACTAgentCoverageBatchResolved("actagent.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("actagent.json plugin registry targets", () => {
    test.each(ACTAGENT_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectACTAgentCoverageBatchResolved("actagent.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `ACTAGENT_AUTH_SECRET_TARGET_${entry.id}`;

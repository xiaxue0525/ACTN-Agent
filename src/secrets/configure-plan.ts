/** Builds the interactive `actagent secrets configure` target list and apply plan. */
...
/** Credential target shown by `actagent secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: ACTAgentConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current ACTAgent config only. */
export function buildConfigureCandidates(config: ACTAgentConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `actagent:${candidate.path}`;
}
...
/** Builds configure candidates for ACTAgent config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: ACTAgentConfig;
  authoredACTAgentConfig?: ACTAgentConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredACTAgentConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const actagentCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `actagent.json` as const,
  ...
  return [...actagentCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "actagent secrets configure",

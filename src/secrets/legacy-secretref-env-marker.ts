...
function toCandidate(
  ...
  defaults: NonNullable<ACTAgentConfig["secrets"]>["defaults"] | undefined,
 ...
 */
export function migrateLegacySecretRefEnvMarkers(config: ACTAgentConfig): {
  config: ACTAgentConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as ACTAgentConfig & Record<string, unknown>;

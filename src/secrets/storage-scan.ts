...
import { isRecord as isJsonObject } from "@actagent/normalization-core/record-coerce";
...
export function listAuthProfileStorePaths(config: ACTAgentConfig, stateDir: string): string[] {
...
export function listAuthProfileStoreAgentDirs(config: ACTAgentConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.ACTAGENT_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {

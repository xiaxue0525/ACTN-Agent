...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        ACTAGENT_BASE_ENV: "base",
        ACTAGENT_TO_REMOVE: undefined,
      },
      env: {
        ACTAGENT_TEST_ENV: "ok",
      },
    });

    expect(resolved.ACTAGENT_BASE_ENV).toBe("base");
    expect(resolved.ACTAGENT_TEST_ENV).toBe("ok");
    expect(resolved.ACTAGENT_TO_REMOVE).toBeUndefined();
    expect(resolved.ACTAGENT_CLI).toBe(ACTAGENT_CLI_ENV_VALUE);
  });

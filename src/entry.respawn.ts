...
export const ACTAGENT_NODE_OPTIONS_READY = "ACTAGENT_NODE_OPTIONS_READY";
export const ACTAGENT_NODE_EXTRA_CA_CERTS_READY = "ACTAGENT_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.ACTAGENT_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[ACTAGENT_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[ACTAGENT_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv) &&
    !isTruthyEnvValue(env[ACTAGENT_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[ACTAGENT_NODE_OPTIONS_READY] = "1";
...
export function runCliRespawnPlan(
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        "[actagent] Failed to respawn CLI:",

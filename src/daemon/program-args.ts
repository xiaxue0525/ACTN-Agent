...
export const ACTAGENT_WRAPPER_ENV_KEY = "ACTAGENT_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/actagent -> .pnpm/actagent@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // actagent from node_modules/.bin points at the package root sibling.
...
export async function resolveACTAgentWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${ACTAGENT_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveACTAgentWrapperPath(params.wrapperPath);
  if (wrapperPath) {

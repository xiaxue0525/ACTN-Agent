/** Inspects installed platform services for extra ACTAgent or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["actagent", "actagentdbot"] as const;
...
export function renderGatewayServiceCleanupHints(
  ...
  const profile = env.ACTAGENT_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["actagent_service_marker"];
  const kindKeys = ["actagent_service_kind"];
...
function isACTAgentGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "actagent") {
    return false;
  }
  return label.startsWith("ai.actagent.");
}

function isACTAgentGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("actagent-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isACTAgentGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  const defaultName = normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName());
  return normalized === defaultName || normalized.startsWith("actagent gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("actagentdbot");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "actagent"
        ? "actagent"
        : executionMarker === "actagentdbot" || legacyLabel
          ? "actagentdbot"
          : null;
    if (!marker) {
      continue;
    ...
    if (marker === "actagent" && isACTAgentGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "actagent" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedACTAgent?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedACTAgent ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedACTAgent &&
      marker === "actagent" &&
      isACTAgentGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isACTAgentGatewayTaskName(name)) {
        continue;
      }

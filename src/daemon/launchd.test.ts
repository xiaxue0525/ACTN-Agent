...
import {
  ...
  disableACTAgentUpdateLaunchdJob,
  findStaleACTAgentUpdateLaunchdJobs,
  isLaunchAgentListed,
  isACTAgentUpdateLaunchdLabel,
  parseLaunchctlPrint,
  parseLaunchctlListACTAgentUpdateJobs,
  readLaunchAgentProgramArguments,
  readLaunchAgentRuntime,
  repairLaunchAgentBootstrap,
  removeACTAgentUpdateLaunchdJob,
  restartLaunchAgent,
...
const inspectPortUsage = vi.hoisted(() =>
  vi.fn(async () => ({ port: 19199, status: "free", listeners: [], hints: [] })),
);
const formatPortDiagnostics = vi.hoisted(() => vi.fn(() => ["Port 19199 is already in use."]));
...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.actagent.gateway";
...
beforeEach(() => {
  ...
  inspectPortUsage.mockResolvedValue({ port: 19199, status: "free", listeners: [], hints: [] });
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.actagent.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", ACTAGENT_PROFILE: "default" },
    });
  ...
  it("parses stale ACTAgent updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListACTAgentUpdateJobs(
      [
        "123 0 ai.actagent.gateway",
        "- 127 ai.actagent.update.2026.5.12",
        "- 0 ai.actagent.manual-update.1717168800",
        "8142 0 ai.actagent.update.2026.5.13-beta.1",
        "- 0 ai.actagent.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      {
        label: "ai.actagent.manual-update.1717168800",
        lastExitStatus: 0,
      },
      {
        label: "ai.actagent.update.2026.5.12",
        lastExitStatus: 127,
      },
      {
        label: "ai.actagent.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale ACTAgent updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.actagent.update.2026.5.12\n";

      const jobs = await findStaleACTAgentUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.actagent.manual-update.1717168800",
        "812 0 ai.actagent.manual-update.profile",
        "913 0 ai.actagent.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleACTAgentUpdateLaunchdJobs({
        ACTAGENT_PROFILE: "manual-update.profile",
        ACTAGENT_LAUNCHD_LABEL: "ai.actagent.manual-update.custom-label",
        ACTAGENT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        ACTAGENT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it("recognizes only ACTAgent updater launchd labels", () => {
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.update.2026.5.12")).toBe(true);
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.manual-update.1717168800")).toBe(true);
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.gateway")).toBe(false);
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.manual-update.gateway")).toBe(false);
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.manual-update.profile")).toBe(false);
    expect(isACTAgentUpdateLaunchdLabel("ai.actagent.manual-updater.1717168800")).toBe(false);
    expect(isACTAgentUpdateLaunchdLabel("com.example.update")).toBe(false);
  });

  it.runIf(process.platform === "darwin")("removes legacy updater launchd jobs", async () => {
    await expect(removeACTAgentUpdateLaunchdJob(" ai.actagent.update.2026.5.12 ")).resolves.toBe(
      true,
    );

    expect(state.launchctlCalls).toContainEqual(["remove", "ai.actagent.update.2026.5.12"]);
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.actagent.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.actagent.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.actagent.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.actagent.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from ACTAgent label env",
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          ACTAGENT_LAUNCHD_LABEL: "ai.actagent.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the ACTAgent update label",
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.actagent.update.2026.5.12",
          ACTAGENT_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.actagent.manual-update.1717168800",
          ACTAGENT_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentACTAgentUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.actagent.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableACTAgentUpdateLaunchdJob("ai.actagent.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableACTAgentUpdateLaunchdJob("ai.actagent.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.actagent/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.actagent/service-env/ai.actagent.gateway.env";
    const wrapperPath = "/Users/test/.actagent/service-env/ai.actagent.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.actagent/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      ACTAGENT_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        ACTAGENT_STATE_DIR: serviceEnv.ACTAGENT_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.actagent.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.actagent.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.actagent/service-env/ai.actagent.gateway.env";
    const callerWrapperPath =
      "/Users/test/.actagent/service-env/ai.actagent.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.actagent.gateway.env](http:/ai.actagent.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.actagent.gateway-env-wrapper.sh](http:/ai.actagent.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.ACTAGENT_GATEWAY_PORT).toBe("19199");
    expect(command?.environment?.ACTAGENT_STATE_DIR).toBe(serviceEnv.ACTAGENT_STATE_DIR);
    expect(command?.environmentValueSources?.ACTAGENT_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export ACTAGENT_GATEWAY_PORT='19199'");
    expect(rewrittenEnv).toContain(
      "export ACTAGENT_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/actagent/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.actagent.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.actagent.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.actagent.gateway from inside the same launchd service",
        );
      },
    );
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      ACTAGENT_LAUNCHD_LABEL: "com.example.actagent.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    await withProcessEnv(
      {
        ...
        ACTAGENT_LAUNCHD_LABEL: undefined,
        ACTAGENT_SERVICE_MARKER: undefined,
        ACTAGENT_SERVICE_KIND: undefined,
      },
      async () => {
        await stopLaunchAgent({ env, stdout });
      },
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.actagent.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ACTAGENT_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("restarts LaunchAgent with kickstart and no bootout", async () => {
    ...
    expect(cleanStaleGatewayProcessesSync).toHaveBeenCalledWith(19199);
    expect(state.launchctlCalls).toEqual([
      ["enable", serviceId],
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.actagent-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ACTAGENT_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ACTAGENT_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.actagent.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/actagent/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.actagent.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.actagent.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when ACTAGENT_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.actagent.gateway.plist",
    },
    {
      name: "uses profile-specific label when ACTAGENT_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", ACTAGENT_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.actagent.jbphoenix.plist",
    },
    {
      name: "prefers ACTAGENT_LAUNCHD_LABEL over ACTAGENT_PROFILE",
      env: {
        HOME: "/Users/test",
        ACTAGENT_PROFILE: "jbphoenix",
        ACTAGENT_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from ACTAGENT_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        ACTAGENT_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty ACTAGENT_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        ACTAGENT_PROFILE: "myprofile",
        ACTAGENT_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.actagent.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        ACTAGENT_LAUNCHD_LABEL: "../evil/label",
      }),

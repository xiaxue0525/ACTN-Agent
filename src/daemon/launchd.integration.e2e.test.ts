...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `actagent-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ACTAGENT_LAUNCHD_LABEL: `ai.actagent.launchd-int-${testId}`,
      ACTAGENT_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

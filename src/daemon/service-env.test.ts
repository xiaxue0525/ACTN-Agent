...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ACTAGENT_GATEWAY_PORT).toBe("19199");
    expect(env.ACTAGENT_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ACTAGENT_SERVICE_MARKER).toBe("actagent");
    expect(env.ACTAGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ACTAGENT_SERVICE_VERSION).toBe("string");
    expect(env.ACTAGENT_SYSTEMD_UNIT).toBe("actagent-gateway.service");
    expect(env.ACTAGENT_WINDOWS_TASK_NAME).toBe("ACTAgent Gateway");
    if (process.platform === "darwin") {
      expect(env.ACTAGENT_LAUNCHD_LABEL).toBe("ai.actagent.gateway");
    }
  });

  it("sets the ACTAgent-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 19199,
      platform: "darwin",
  ...
  it("passes through ACTAGENT_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.ACTAGENT_WRAPPER).toBe("/usr/local/bin/actagent-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".actagent", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ACTAGENT_PROFILE: "work" },
      port: 19199,
    });
    expect(env.ACTAGENT_SYSTEMD_UNIT).toBe("actagent-gateway-work.service");
    expect(env.ACTAGENT_WINDOWS_TASK_NAME).toBe("ACTAgent Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ACTAGENT_LAUNCHD_LABEL).toBe("ai.actagent.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        ACTAGENT_SYSTEMD_UNIT: "actagent-gateway-maintenance",
      },
      port: 19199,
      platform: "linux",
    });

    expect(env.ACTAGENT_SYSTEMD_UNIT).toBe("actagent-gateway-maintenance.service");
  });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", ACTAGENT_PROFILE: "work" },
      port: 19199,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.ACTAGENT_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the ACTAgent-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.ACTAGENT_LAUNCHD_LABEL).toBe("ai.actagent.node");
  });

  it("passes through ACTAGENT_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ACTAGENT_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ACTAGENT_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through ACTAGENT_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ACTAGENT_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits ACTAGENT_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.ACTAGENT_WINDOWS_TASK_NAME).toBe("ACTAgent Node");
    expect(env.ACTAGENT_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.ACTAGENT_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
  ...
  const builders = [
    {
      ...
      build: (env: Record<string, string | undefined>, platform?: NodeJS.Platform) =>
        buildServiceEnvironment({ env, port: 19199, platform }),
    },
    {
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".actagent"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ACTAGENT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".actagent-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ACTAGENT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".actagent"));
  });

  it("uses ACTAGENT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ACTAGENT_STATE_DIR: "/var/lib/actagent" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/actagent"));
  });

  it("expands ~ in ACTAGENT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ACTAGENT_STATE_DIR: "~/actagent-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/actagent-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ACTAGENT_STATE_DIR: "C:\\State\\actagent" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\actagent");
  });
});

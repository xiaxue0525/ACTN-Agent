...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.ACTAGENT_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.ACTAGENT_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.ACTAGENT_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    ACTAGENT_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });

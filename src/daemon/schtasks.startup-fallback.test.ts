...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.ACTAGENT_WINDOWS_TASK_NAME ?? "ACTAgent Gateway";
  return path.join(
    env.APPDATA,
    "Microsoft",
    "Windows",
...
async function writeNodeScript(env: Record<string, string>, port = "19199") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "ACTAGENT_SERVICE_KIND=node"`,
      `set "ACTAGENT_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\actagent.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    ACTAGENT_WINDOWS_TASK_NAME: "ACTAgent Node",
  };
}
    ...
    if (command === "powershell" && Array.isArray(args) && args.includes(NODE_PROCESS_QUERY)) {
      return makeSpawnSyncResult({
        stdout: JSON.stringify([
          {
            ProcessId: processId,
            CommandLine: "C:\\bin\\actagent.cmd node run --host 127.0.0.1 --port 19199",
          },
        ]),
      });
    }
...
function expectStartupFallbackSpawn() {
  ...
  expect(args).toContain("19199");
  expect(options.detached).toBe(true);
  expect((options.env as Record<string, string> | undefined)?.ACTAGENT_GATEWAY_PORT).toBe("19199");
...
function installGatewayScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    env,
    stdout,
    programArguments: ["node", "gateway.js", "--port", "19199"],
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "actagent", "node", "run", "--host", "127.0.0.1", "--port", "19199"],
    environment: {
      ACTAGENT_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("falls back to a Startup-folder launcher when schtasks create is denied", async () => {
    await withWindowsEnv("actagent-win-startup-", async ({ env }) => {
      addStartupFallbackMissingResponses([
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node actagent node run --host 127.0.0.1 --port 19199",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("actagent-win-startup-", async ({ env }) => {
      env.ACTAGENT_SERVICE_KIND = "node";
      env.ACTAGENT_WINDOWS_TASK_NAME = "ACTAgent Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\actagent.cmd node run --host 127.0.0.1 --port 19199",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\actagent.cmd node run --host 127.0.0.1 --port 19199",
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    await withWindowsEnv("actagent-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.ACTAGENT_GATEWAY_PORT;

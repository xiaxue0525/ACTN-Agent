...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical ACTAgent proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.ACTAGENT_PROXY_URL);
  return proxyUrl ? { ACTAGENT_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const wrapperPath = normalizeOptionalString(env.ACTAGENT_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = resolveGatewaySystemdUnitEnv(env);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ACTAGENT_PROFILE: profile,
    ACTAGENT_WRAPPER: wrapperPath,
    ACTAGENT_GATEWAY_PORT: String(port),
    ACTAGENT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ACTAGENT_SYSTEMD_UNIT: systemdUnit,
    ACTAGENT_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ACTAGENT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ACTAGENT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ACTAGENT_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.ACTAGENT_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ACTAGENT_GATEWAY_TOKEN: gatewayToken,
    ACTAGENT_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ACTAGENT_STATE_DIR: sharedEnv.stateDir,
    ACTAGENT_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ACTAGENT_STATE_DIR;

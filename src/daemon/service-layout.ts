/** Summarizes installed service command paths and ACTAgent package layout. */
...
async function resolveACTAgentPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveACTAgentPackageRoot(entrypointReal) : undefined;

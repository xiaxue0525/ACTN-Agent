...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ACTAGENT_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `actagent gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ACTAGENT_GATEWAY_TOKEN does not match gateway.auth.token in actagent.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.ACTAGENT_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ACTAGENT_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.ACTAGENT_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by ACTAgent ${serviceVersion}; current CLI is ${VERSION}.`,

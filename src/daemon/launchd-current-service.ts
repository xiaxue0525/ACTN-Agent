...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.ACTAGENT_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.ACTAGENT_SERVICE_MARKER) === "actagent" &&
    Boolean(normalizeOptionalString(env.ACTAGENT_SERVICE_KIND))
  ) {

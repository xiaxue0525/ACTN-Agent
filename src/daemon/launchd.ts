...
const ACTAGENT_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.actagent.update.";
const ACTAGENT_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.actagent\.manual-update\.\d+$/;

export type StaleACTAgentUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeACTAgentUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(ACTAGENT_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.actagent labels.
  return ACTAGENT_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.ACTAGENT_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.ACTAGENT_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.ACTAGENT_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.ACTAGENT_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isACTAgentUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeACTAgentUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentACTAgentUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.ACTAGENT_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeACTAgentUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.ACTAGENT_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.ACTAGENT_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by ACTAgent. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListACTAgentUpdateJobs(
  output: string,
): StaleACTAgentUpdateLaunchdJob[] {
  const jobs: StaleACTAgentUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeACTAgentUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleACTAgentUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleACTAgentUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListACTAgentUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function removeACTAgentUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeACTAgentUpdateLaunchdLabel(label);
...
export async function disableACTAgentUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeACTAgentUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentACTAgentUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentACTAgentUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableACTAgentUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.ACTAGENT_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.ACTAGENT_GATEWAY_PORT ?? "");
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.actagent.ai/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `actagent gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.ACTAGENT_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "actagent gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "actagent gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `actagent gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "actagent gateway restart",
    });

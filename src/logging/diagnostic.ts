...
function resolveDiagnosticSessionStorePaths(config?: ACTAgentConfig): string[] | undefined {
  if (!config) {
    return undefined;
  }
  try {
...
function shouldWriteCriticalMemoryPressureBundle(config?: ACTAgentConfig): boolean {
...
export function resolveStuckSessionWarnMs(config?: ACTAgentConfig): number {

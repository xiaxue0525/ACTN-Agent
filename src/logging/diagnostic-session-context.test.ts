...
describe("diagnostic session context", () => {
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-diagnostic-session-"));
  ...
  it("parses cron run session keys", () => {
    expect(parseCronRunSessionKey("agent:actagentblocker:cron:job-123:run:run-456")).toEqual({
      agentId: "actagentblocker",
      cronJobId: "job-123",
      cronRunId: "run-456",
    });
  });
  ...
  it("formats cron job and last assistant context for stalled session logs", async () => {
    ...
    writeJsonl(path.join(stateDir, "agents", "actagentblocker", "sessions", "run-456.jsonl"), [

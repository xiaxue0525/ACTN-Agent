...
describe("stuck session recovery", () => {
  ...
  it("does not release a sibling-key lane while the same session file has an active run", async () => {
    ...
    const outcome = await recoverStuckDiagnosticSession({
      ...
      sessionFile: "/tmp/actagent-shared-session.jsonl",
      ageMs: 180_000,
      queueDepth: 1,
    });
  ...
  it("logs stopped cron context when aborting an active embedded run", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-recovery-context-"));
    try {
      ...
      fs.mkdirSync(path.join(tempDir, "agents", "actagentblocker", "sessions"), {
        recursive: true,
      });
      fs.writeFileSync(
        path.join(tempDir, "agents", "actagentblocker", "sessions", "run-456.jsonl"),
        JSON.stringify({
      ...
      await recoverStuckDiagnosticSession({
        sessionId: "run-456",
        sessionKey: "agent:actagentblocker:cron:job-123:run:run-456",
        ageMs: 629_000,
        allowActiveAbort: true,
      });
    } finally {
    ...
    expect(warnLogMessages()).toEqual([
      'stuck session recovery: sessionId=run-456 sessionKey=agent:actagentblocker:cron:job-123:run:run-456 age=629s action=abort_embedded_run aborted=true drained=true released=0 stopped="Twitter Mention Moderation Agent" cronJobId=job-123 cronRunId=run-456 lastAssistant="There are 40 cached mentions."',
      "stuck session recovery outcome: status=aborted action=abort_embedded_run sessionId=run-456 sessionKey=agent:actagentblocker:cron:job-123:run:run-456 activeSessionId=run-456 activeWorkKind=embedded_run lane=session:agent:actagentblocker:cron:job-123:run:run-456 aborted=true drained=true forceCleared=false released=0",
    ]);
  });

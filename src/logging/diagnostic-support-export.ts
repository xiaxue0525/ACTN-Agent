...
const SUPPORT_EXPORT_PREFIX = "actagent-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.ACTAGENT_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$ACTAGENT_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/actagent-sanitized.jsonl`: sanitized log summaries and metadata",

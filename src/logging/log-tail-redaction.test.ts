...
const originalEnv = captureEnv(["ACTAGENT_CONFIG_PATH"]);
let tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "actagent.log");
    const configFile = path.join(dir, "actagent.json");
    const basicSecret = "c2VjcmV0OnBhc3M=";
    const actAgentToken = "supersecretgatewaytoken1234567890";
    ...
    await fs.writeFile(
      ...
      [
        ...
        `X-ACTAgent-Token: ${actAgentToken}`,
        `x-pomerium-jwt-assertion: ${pomeriumJwt}`,
        "normal diagnostic line",
      ].join("\n"),
      "utf8",
    );
    process.env.ACTAGENT_CONFIG_PATH = configFile;
    ...
    expect(text).toContain("X-ACTAgent-Token: supers…7890");
    ...
    expect(text).not.toContain(actAgentToken);

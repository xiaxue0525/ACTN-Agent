...
function resolveDefaultRollingLogFile(date = new Date()): string {
  const logDir = canUseNodeFs() ? resolvePreferredACTAgentTmpDir() : POSIX_ACTAGENT_TMP_DIR;
...
export function resolveConfiguredLogFilePath(config?: ACTAgentConfig | null): string {

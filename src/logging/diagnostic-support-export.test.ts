...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "actagent.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["actagent", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              ACTAGENT_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$ACTAGENT_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/actagent-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.ACTAGENT_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts numeric private fields in support snapshots and config", () => {
    ...
    expect(sanitizeSupportConfigValue(19199, redaction, "port")).toBe(19199);
  });
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\actagent`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$ACTAGENT_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\actagent\\dist\\index.js`,
              "--config",
              `${stateDir}\\actagent.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\actagent\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\actagent\\\\dist\\\\index.js");
    expect(serialized).toContain("$ACTAGENT_STATE_DIR\\\\actagent.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\actagent\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/actagent.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/actagent-sanitized.jsonl");

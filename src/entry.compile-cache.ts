...
export function shouldEnableACTAgentCompileCache(params: {
...
export function resolveACTAgentCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
export type ACTAgentCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

type ACTAgentCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

export function buildACTAgentCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): ACTAgentCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.ACTAGENT_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutACTAgentCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runACTAgentCompileCacheRespawnPlan(plan);
  return true;
}

export function runACTAgentCompileCacheRespawnPlan(
  plan: ACTAgentCompileCacheRespawnPlan,
  runtime: ACTAgentCompileCacheRespawnRuntime = {
    spawn,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[actagent] Failed to respawn CLI without compile cache: ${
...
export function enableACTAgentCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableACTAgentCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveACTAgentCompileCacheDirectory(params));
  } catch {

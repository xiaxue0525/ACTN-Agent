...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[actagent proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[actagent proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`actagent proxy coverage\` for details.`,
  );
}

...
export function hasConfiguredInternalHooks(config: ACTAgentConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: ACTAgentConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }

...
import type { ACTAgentConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "actagent-bundled": {
    ...
    canOverride: ["actagent-bundled"],
    canBeOverriddenBy: ["actagent-managed", "actagent-plugin"],
  },
  "actagent-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["actagent-bundled", "actagent-plugin"],
    canBeOverriddenBy: ["actagent-managed"],
  },
  "actagent-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["actagent-bundled", "actagent-managed", "actagent-plugin"],
    canBeOverriddenBy: ["actagent-managed"],
  },
  "actagent-workspace": {
    precedence: 40,
    ...
    canOverride: ["actagent-workspace"],
    canBeOverriddenBy: ["actagent-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "actagent-plugin") {
    return { enabled: true };
  }

...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/actagent-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/actagent-agent/.actagent-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: ACTAgentConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded ACTAgent agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded ACTAgent agent run options");
  }

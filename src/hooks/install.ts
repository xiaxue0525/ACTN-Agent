...
async function ensureACTAgentHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing actagent.hooks");
  }
  const list = normalizeTrimmedStringList(hooks);
  if (list.length === 0) {
    throw new Error("package.json actagent.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureACTAgentHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `actagent.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `actagent.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "actagent-hook-pack-",
    spec,
    timeoutMs,

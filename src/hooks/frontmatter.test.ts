...
import {
  parseFrontmatter,
  resolveACTAgentMetadata,
  resolveHookInvocationPolicy,
} from "./frontmatter.js";
import type { ACTAgentHookMetadata } from "./types.js";
...
function requireACTAgentMetadata(metadata: ACTAgentHookMetadata | undefined): ACTAgentHookMetadata {
  if (!metadata) {
    throw new Error("expected actagent metadata");
  }
  return metadata;
}
...
`;
    ...
    expect(parsed.actagent.emoji).toBe("💾");
    expect(parsed.actagent.events).toEqual(["command:new"]);
  });
...
`;
    ...
    expect(parsed.actagent.emoji).toBe("📝");
    expect(parsed.actagent.events).toEqual(["command"]);
    expect(parsed.actagent.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.actagent.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"actagent": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"actagent": {"events": ["test"]}}');
  });
...
describe("resolveACTAgentMetadata", () => {
  it("extracts actagent metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveACTAgentMetadata(frontmatter);
    const actagent = requireACTAgentMetadata(result);
    expect(actagent.emoji).toBe("🔥");
    expect(actagent.events).toEqual(["command:new", "command:reset"]);
    expect(actagent.requires?.config).toEqual(["workspace.dir"]);
    expect(actagent.requires?.bins).toEqual(["git"]);
  });
  ...
  it("returns undefined when actagent key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        actagent: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ACTAgent" },
            { id: "npm", kind: "npm", package: "@actagent/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@actagent/hook");
  });
...
`;
    ...
    const actagent = requireACTAgentMetadata(resolveACTAgentMetadata(frontmatter));
    expect(actagent.emoji).toBe("💾");
    expect(actagent.events).toEqual(["command:new", "command:reset"]);
    expect(actagent.requires?.config).toEqual(["workspace.dir"]);
    expect(actagent.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const actagent = resolveACTAgentMetadata(frontmatter);
    expect(actagent?.emoji).toBe("disk");
    expect(actagent?.events).toEqual(["command:new"]);
  });
});

...
This directory contains hooks that ship with ACTAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.actagent/workspace`)
...
homepage: https://docs.actagent.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.actagent/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.actagent/actagent.json`:
...
2. Restart gateway: `pkill -9 -f 'actagent.*gateway' && pnpm actagent gateway`
3. Enable the hook: `actagent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.actagent.ai/automation/hooks

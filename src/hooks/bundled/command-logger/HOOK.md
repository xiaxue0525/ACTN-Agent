...
homepage: https://docs.actagent.ai/automation/hooks#command-logger
metadata:
  {
    "actagent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.actagent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.actagent/logs/commands.log ~/.actagent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/actagent`:
   ```
   /home/username/.actagent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

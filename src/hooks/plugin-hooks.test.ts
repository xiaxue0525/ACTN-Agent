...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "actagent-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ACTAGENT_BUNDLED_HOOKS_DIR;
    process.env.ACTAGENT_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ACTAGENT_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ACTAGENT_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".actagent", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): ACTAgentConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("actagent-plugin");
  ...
  it("does not treat Claude hooks.json bundles as ACTAgent hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".actagent", "extensions", "claude-bundle");

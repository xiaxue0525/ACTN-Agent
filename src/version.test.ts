...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "actagent-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __ACTAGENT_VERSION__");
    expect(source).toContain("? __ACTAGENT_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "actagent", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "actagent", version: "2.3.4" });
  ...
  it("ignores non-actagent package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers ACTAGENT_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ACTAGENT_VERSION: "9.9.9",
        ACTAGENT_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale ACTAGENT_VERSION for compatibility checks", () => {
    const previous = process.env.ACTAGENT_VERSION;
    const previousService = process.env.ACTAGENT_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ACTAGENT_VERSION = "2026.3.25";
      process.env.ACTAGENT_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("ACTAGENT_VERSION", previous);
      restoreEnvValue("ACTAGENT_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ACTAGENT_VERSION: "2026.3.99",
        ACTAGENT_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ACTAGENT_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ACTAGENT_VERSION: "2026.3.99",
        ACTAGENT_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores unusable env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ACTAGENT_VERSION: "   ",
        ACTAGENT_SERVICE_VERSION: "  2.0.0  ",

...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: ACTAgentConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: ACTAgentConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: ACTAgentConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): ACTAgentConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    ...
    process.env.ACTAGENT_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
    ...
    expect(probeParams.url).toBe("ws://127.0.0.1:19199");
  ...
  it("suppresses token-bearing onboarding output when requested", async () => {
    ...
    const output = vi
      ...
      .join("\n");
    expect(output).toContain("http://127.0.0.1:19199");
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("ACTAGENT_GATEWAY_PASSWORD", "env-password");
    ...
    expect(waitArgs.url).toBe("ws://127.0.0.1:19199");

...
function hasMeaningfulConfig(config: ACTAgentConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.ACTAGENT_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh ACTAgent setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: ACTAgentConfig) => Promise<ACTAgentConfig>;
}): Promise<void> {
  const [
  ...
  // can reference a concrete ACTAgent config target.

...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 ACTAgent 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `actagent doctor`。",
      previewTitle: "遷移預覽",
      source: "遷移來源",
    ...
    plugins: {
      ...
      downloadFromACTAgentHub: "從 ACTAgentHub 下載（{spec}）",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1。",
      directDefaultsTls: "直接遠端存取預設使用 TLS。",
      loopbackSshHint: "如果 Gateway 僅監聽 loopback，請選擇 SSH 通道並保持 ws://127.0.0.1:19199。",
      noGatewaysFound: "沒有找到 Gateway",
      passwordPrompt: "Gateway 密碼",
      plaintextPasswordHint: "直接把密碼存入 ACTAgent 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 ACTAgent 設定",
    ...
    security: {
      ...
      beta: "ACTAgent 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
      confirm: "我理解 ACTAgent 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 ACTAgent。",
      learnMore: "了解更多",
      notMultitenant: "ACTAgent 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，ACTAgent 是個人 agent：一個可信操作者邊界。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.actagent.ai/skills",
      enterEnv: "輸入 {env}",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "ACTAgent" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 ACTAgent 將你加入允許清單。",
      ...
      separatePhoneLabel: "ACTAgent 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 ACTAgent 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "ACTAgent"',
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 ACTAGENT_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：ACTAGENT_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 ACTAgent，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 actagent configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.actagent.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 ACTAgent。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 ACTAgent。",
      ...
      gatewayTokenStored:
        "儲存位置：$ACTAGENT_CONFIG_PATH（預設 ~/.actagent/actagent.json）中的 gateway.auth.token，或 ACTAGENT_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 ACTAgent。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 ACTAgent。",
      ...
      restart: "重新啟動",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.actagent.ai/security",
      ...
      webDocs: "文件：https://docs.actagent.ai/tools/web",
      ...
      workspaceDocs: "文件：https://docs.actagent.ai/concepts/agent-workspace",

...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.ACTAGENT_GATEWAY_PORT ?? process.env.ACTAGENT_GATEWAY_PORT;
    const port = raw ? Number.parseInt(raw, 10) : Number.NaN;
    return Number.isFinite(port) && port > 0 ? port : 19199;
  }),
);
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.actagent/actagent.json",
    exists: false,
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-onboard-suite-"));
  });
  ...
  it("allows size-drop writes for pending plugin install record migration", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValueOnce({
      ...
      config: {
        plugins: {
          installs: {
            demo: { source: "npm", spec: "@actagent/demo-plugin" },
          },
        },
      },
      issues: [],
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    ...
    process.env.ACTAGENT_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.ACTAGENT_GATEWAY_PORT;
    process.env.ACTAGENT_GATEWAY_PORT = "18791";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.ACTAGENT_GATEWAY_PORT = previousPort;
      }
    }

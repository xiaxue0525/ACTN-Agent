...
describe("configureGatewayForSetup", () => {
  ...
  async function runGatewayConfig(params?: {
    ...
    const prompter = createPrompter({
      ...
      textQueue: params?.textQueue ?? ["19199", undefined],
    });
  ...
  it("prefers ACTAGENT_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.ACTAGENT_GATEWAY_TOKEN;
    process.env.ACTAGENT_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps ACTAGENT_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.ACTAGENT_GATEWAY_TOKEN;
    process.env.ACTAGENT_GATEWAY_TOKEN = "advanced-env-token";
    ...
    try {
      ...
      const text = vi.fn(async () => "19199") as unknown as WizardPrompter["text"];
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.ACTAGENT_GATEWAY_PASSWORD;
    process.env.ACTAGENT_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["19199", "ACTAGENT_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ACTAGENT_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.ACTAGENT_GATEWAY_TOKEN;
    process.env.ACTAGENT_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["19199", "ACTAGENT_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ACTAGENT_GATEWAY_TOKEN = previous;
      }
    }
  });

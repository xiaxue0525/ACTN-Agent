// Setup wizard orchestrates onboarding prompts and generated ACTAgent config.
...
async function writeWizardConfigFile(
  configInput: ACTAgentConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: ACTAgentConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: ACTAgentConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: ACTAgentConfig | undefined = baseConfig;
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("actagent doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (compatibilityNotices.length > 0) {
    await prompter.note(
      [
        ...
        `Inspect: ${formatCliCommand("actagent plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("actagent configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: actagent onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.ACTAGENT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.ACTAGENT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: ACTAgentConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }

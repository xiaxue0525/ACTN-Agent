...
function isInstalledOrConfigured(config: ACTAgentConfig, pluginId: string): boolean {
...
function formatInstallHint(install: PluginPackageInstall): string {
  if (install.actagenthubSpec && install.npmSpec) {
    return install.defaultChoice === "actagenthub"
      ? "ACTAgentHub, with npm fallback"
      : "npm, with ACTAgentHub fallback";
  }
  if (install.actagenthubSpec) {
    return "ACTAgentHub";
  }
  if (install.npmSpec) {
    return "npm";

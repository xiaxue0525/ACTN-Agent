...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("ACTAgent");
    ...
    expect(normalized).toContain("Runtime: ACTAgent Default");
  ...
  it("shows the default ACTAgent harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "actagent-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: ACTAgent Default");
    expect(normalized).not.toContain("· actagent");
  });

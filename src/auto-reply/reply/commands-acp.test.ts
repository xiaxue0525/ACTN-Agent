...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: ACTAgentConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: ACTAgentConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("spawns an ACP session and binds a Discord thread", async () => {
    ...
    const result = await runDiscordAcpCommand("/acp spawn codex --cwd /home/bob/actagentd");
    ...
    expect(introText).toContain("cwd: /home/bob/actagentd");
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("accepts unicode dash option prefixes in /acp spawn args", async () => {
    const result = await runThreadAcpCommand(
      "/acp spawn codex \u2014mode oneshot \u2014thread here \u2014cwd /home/bob/actagentd \u2014label jeerreview",
    );
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    hoisted.readAcpSessionEntryMock.mockReturnValue({
      ...
      acp: {
        ...
        lastError: [
          "ACTAgent runtime context (internal):",
    ...
    expect(result?.reply?.text).not.toContain("ACTAgent runtime context (internal):");

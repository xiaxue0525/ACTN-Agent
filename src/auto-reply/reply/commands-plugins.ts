  ...
  const lines = [
    ...
    ...report.plugins.map((plugin) => {
      const format = plugin.bundleFormat
        ? `${plugin.format ?? "actagent"}/${plugin.bundleFormat}`
        : (plugin.format ?? "actagent");
...
async function installPluginFromPluginsCommand(params: {
  ...
  const actagenthubSpec = parseACTAgentHubPluginSpec(params.raw);
  if (actagenthubSpec) {
    ...
    await persistPluginInstall({
      ...
      install: {
        ...
        installPath: result.targetDir,
        version: result.version,
        integrity: result.actagenthub.integrity,
        resolvedAt: result.actagenthub.resolvedAt,
        actagenthubUrl: result.actagenthub.actagenthubUrl,
        actagenthubPackage: result.actagenthub.actagenthubPackage,
        actagenthubFamily: result.actagenthub.actagenthubFamily,
        actagenthubChannel: result.actagenthub.actagenthubChannel,
      },
    });
    return { ok: true, pluginId: result.pluginId };
  }
...
export const handlePluginsCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  let committedConfig: ACTAgentConfig;
  try {

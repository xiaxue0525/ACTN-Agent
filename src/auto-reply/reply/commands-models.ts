...
import { parseStrictPositiveInteger } from "@actagent/normalization-core/number-coercion";
import {
...
function buildRuntimeChoice(params: {
  ...
  return {
    ...
    description:
      id === "actagent"
        ? "Use the built-in ACTAgent runtime."
        : params.cli
...
export async function buildModelsProviderData(
  ...
  for (const binding of runtimeBindings) {
    ...
    addRuntimeChoice(choices, buildRuntimeChoice({ cfg, provider, runtime: "actagent" }));
    addRuntimeChoice(
      choices,
      buildRuntimeChoice({
        cfg,

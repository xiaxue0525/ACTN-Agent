...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(
    () =>
      "Recommended follow-up: run actagent doctor --non-interactive in a terminal or approvals-capable ACTAgent surface.",
  ),
  ...
  triggerACTAgentRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerACTAgentRestart: mocks.triggerACTAgentRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerACTAgentRestart.mockReset();
    mocks.triggerACTAgentRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerACTAgentRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerACTAgentRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerACTAgentRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });

...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside runAgentTurnWithFallback", async () => {
    // Regression test for actagent/actagent#68056.
  ...
  it("passes current inbound media paths as native ACTAgent images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "actagent-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native ACTAgent images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "actagent-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "actagent-native-agent-partial-"));
    cleanupPaths.push(tmpDir);

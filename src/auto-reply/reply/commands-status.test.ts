...
vi.mock("../../agents/harness/builtin-actagent.js", () => ({
  createACTAgentAgentHarness: () => ({
    id: "actagent",
    label: "ACTAgent Default",
...
describe("buildStatusReply subagent summary", () => {
  ...
  it("does not leak internal runtime context through the task status line", async () => {
    ...
    expect(reply?.text).not.toContain("ACTAgent runtime context (internal):");
  ...
  it("shows the effective non-ACTAgent embedded harness in /status", async () => {
  ...
  it("uses Codex OAuth auth labels for openai models running on the Codex harness", async () => {
    ...
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".actagent", "agents", "main", "agent");
  ...
  it("uses Codex OAuth auth labels for explicit OpenAI ACTAgent auth order", async () => {
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".actagent", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("uses workspace-scoped auth evidence in /status auth labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".actagent", "extensions", "workspace-auth-label");
  ...
  it("keeps /status on a session-pinned ACTAgent harness after config changes", async () => {
    registerStatusCodexHarness();

...
import { buildCurrentACTAgentCliCommand } from "./commands-actagent-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.actagent.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "actagent gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentACTAgentCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this ACTAgent session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an ACTAgent session file.",
    ) === true
  );
}

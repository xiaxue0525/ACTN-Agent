...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop actagent!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@actagent_bot", { botUsername: "actagent_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@actagent_bot", { botUsername: "actagent_bot" })).toBe(true);

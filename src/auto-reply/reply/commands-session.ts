...
import { scheduleGatewaySigusr1Restart, triggerACTAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ACTAgent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelPath: string | null = null;
  ...
  const restartMethod = triggerACTAgentRestart();
  if (!restartMethod.ok) {
    await removeRestartSentinelFile(sentinelPath);
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ACTAgent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

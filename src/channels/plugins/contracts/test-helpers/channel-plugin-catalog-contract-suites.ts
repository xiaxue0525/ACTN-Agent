    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@actagent/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-state-"),
          );
          ...
          return {
            ...
            env: {
              ...process.env,
              ACTAGENT_STATE_DIR: stateDir,
              ACTAGENTBOT_STATE_DIR: undefined,
              ACTAGENT_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
            },
            expected: {
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that ACTAgent can discover during onboarding and install on demand via `actagent plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-actagent-plugin",
                  description:
                    "ACTAgent WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  actagent: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-actagent-plugin@1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@actagent/prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@actagent/prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@actagent/prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@actagent/prerelease-demo-channel",
      ...
      {
        name: "accepts external manifest entries with ACTAgentHub-only install metadata",
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-actagenthub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  actagent: {
                    channel: {
                      ...
                      detailLabel: "ACTAgentHub",
                      docsPath: "/channels/actagenthub-chat",
                      docsLabel: "actagenthub chat",
                      blurb: "ACTAgentHub-backed chat channel.",
                      aliases: ["chchat"],
                      order: 47,
                    },
                    install: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "actagenthub",
                actagenthub: {
                  spec: "actagenthub:actagent/actagenthub-chat@2026.5.2",
                  packageName: "actagent/actagenthub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "ACTAgent Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  actagent: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "actagent-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "actagent-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "actagent-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredACTAgentTmpDir(), "actagent-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@actagent/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              ACTAGENT_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              ACTAGENT_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@actagent/default-env-demo",

...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): ACTAgentConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as ACTAgentConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: ACTAgentConfig) => string[];
  input: ACTAgentConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: ACTAgentConfig, expected: string) {

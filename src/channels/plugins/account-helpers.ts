...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: ACTAgentConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: ACTAgentConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: ACTAgentConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: ACTAgentConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: ACTAgentConfig): string {

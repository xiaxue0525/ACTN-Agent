...
import type { ACTAgentPackageManifest } from "../../plugins/manifest.js";
...
export type ChannelPluginCatalogInstall = PluginPackageInstall &
  ({ actagenthubSpec: string } | { npmSpec: string });
...
const ENV_CATALOG_PATHS = ["ACTAGENT_PLUGIN_CATALOG_PATHS", "ACTAGENT_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveACTAgentPackageRootSync({ cwd: process.cwd() }),
      resolveACTAgentPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<ACTAgentPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  ...
  const actagenthubSpec = normalizeOptionalString(params.install?.actagenthubSpec);
  let npmSpec =
  ...
  const defaultChoice: NonNullable<PluginPackageInstall["defaultChoice"]> =
    requestedDefaultChoice === "actagenthub" && actagenthubSpec
      ? "actagenthub"

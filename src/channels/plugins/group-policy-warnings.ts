...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: ACTAgentConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: ACTAgentConfig }>(
  collector: WarningCollector<{ cfg: ACTAgentConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: ACTAgentConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: ACTAgentConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: ACTAgentConfig },
>(
  projectCfg: (cfg: ACTAgentConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: ACTAgentConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: ACTAgentConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: ACTAgentConfig },
>(params: {
  providerConfigPresent: (cfg: ACTAgentConfig) => boolean;

...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://actagent.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("actagent-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["actagent-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("actagent-weixin");
    expect(getRegisteredChannelPluginMeta("ACTAGENT-WEIXIN")?.aliases).toEqual(["weixin"]);
  });

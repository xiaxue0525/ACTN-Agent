 ...
 *
 * Stores, claims, completes, and tombstones inbound channel events in ACTAgent state.
 */
...
import type {
  ...
  DB as ACTAgentStateKyselyDatabase,
} from "../../state/actagent-state-db.generated.js";
import {
...
type ChannelIngressDatabase = Pick<ACTAgentStateKyselyDatabase, "channel_ingress_events">;
...
function openStateDatabase(stateDir?: string) {
  return openACTAgentStateDatabase({
    env: stateDir ? { ...process.env, ACTAGENT_STATE_DIR: stateDir } : process.env,
  });
}
...
/** Creates a durable channel/account-scoped ingress queue backed by the ACTAgent state database. */

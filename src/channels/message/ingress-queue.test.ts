...
type ChannelIngressTestDatabase = Pick<ACTAgentStateKyselyDatabase, "channel_ingress_events">;

async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-ingress-queue-"));
  try {

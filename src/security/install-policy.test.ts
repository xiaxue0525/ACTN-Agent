...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-install-policy-"));
  tempDirs.push(dir);
  return dir;
}
...
function baseRequest(sourcePath: string): InstallPolicyRequest {
  return {
    ...
    origin: { type: "actagenthub", slug: "weather", version: "1.0.0" },
    request: {
      kind: "skill-install",
      mode: "install",
...
function configWithPolicy(scriptPath: string, env: Record<string, string>): ACTAgentConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
describe("runInstallPolicy", () => {
  ...
  it("executes policy for skills when targets are omitted", async () => {
    ...
    expect(captured.actagentVersion).toEqual(expect.any(String));
    ...
    expect(captured.origin).toMatchObject({ type: "actagenthub", slug: "weather" });
  });
  ...
  it("prefixes operator blocks", async () => {
    ...
    expect(warnings.join("\n")).toContain("source=actagenthub/actagent");

...
type CollectDangerousConfigFlags = (cfg: ACTAgentConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.ACTAGENT_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.ACTAGENT_GATEWAY_PASSWORD);

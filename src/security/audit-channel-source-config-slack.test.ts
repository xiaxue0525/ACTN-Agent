...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: ACTAgentConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: ACTAgentConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: ACTAgentConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): ACTAgentConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: ACTAgentConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {

...
describe("security audit exec surface findings", () => {
  // Redirect the ACTAgent home (ACTAGENT_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.actagent/exec-approvals.json` on the host running
  // the suite.
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("ACTAgent exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when ACTAgent exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({

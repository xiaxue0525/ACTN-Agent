...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("actagent status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("actagent security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ACTAGENT_GATEWAY_TOKEN/ACTAGENT_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("actagent security audit --deep")}".`,
    });
  }
  return findings;
}

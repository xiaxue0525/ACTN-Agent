...
import type { ACTAgentConfig, SecurityConfig } from "../config/types.actagent.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "actagent" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load ACTAgent config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    actagentVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });

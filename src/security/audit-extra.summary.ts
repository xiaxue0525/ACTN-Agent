...
function summarizeGroupPolicy(cfg: ACTAgentConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ACTAgentConfig): boolean {
...
function isBrowserEnabled(cfg: ACTAgentConfig): boolean {
  return cfg.browser?.enabled !== false;
}

/** Produce a concise inventory of major security-relevant surfaces. */
export function collectAttackSurfaceSummaryFindings(cfg: ACTAgentConfig): SecurityAuditFinding[] {

  ...
  it("applies gateway auth precedence across local and remote modes", async () => {
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.ACTAGENT_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.ACTAGENT_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
    ...
    const cases: Array<{
      ...
      },
      {
        ...
        cfg: {
          gateway: {
            ...
            remote: { url: "wss://remote.example.com:19199", token: "remote-token-xyz789" },
          },
        },
      ...
      },
      {
        ...
        cfg: {
          gateway: {
            mode: "remote",
            auth: { token: "local-token-should-not-use" },
            remote: { url: "wss://remote.example.com:19199", token: "remote-token" },
          },
        },
        env: { token: "env-token" },
      ...
      },
      {
        ...
        cfg: {
          gateway: {
            mode: "remote",
            remote: { url: "wss://remote.example.com:19199", password: "remote-pass" },
          },
        },

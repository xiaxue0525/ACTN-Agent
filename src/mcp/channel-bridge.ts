...
/**
 * Runtime bridge between MCP tools and the ACTAgent Gateway channel APIs.
 *
...
export class ACTAgentChannelBridge {
  ...
  constructor(
    private readonly cfg: ACTAgentConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "ACTAgent MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`actagent mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `actagent mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }

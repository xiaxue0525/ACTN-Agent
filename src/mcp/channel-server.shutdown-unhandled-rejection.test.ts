...
vi.mock("./channel-bridge.js", () => ({
  ACTAgentChannelBridge: class MockACTAgentChannelBridge {
...
describe("serveACTAgentChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveACTAgentChannelMcp } = await import("./channel-server.js");

    const servePromise = serveACTAgentChannelMcp({ verbose: false });

...
import { ACTAgentChannelBridge } from "./channel-bridge.js";
import { ClaudePermissionRequestSchema, type ClaudeChannelMode } from "./channel-shared.js";
import { getChannelMcpCapabilities, registerChannelMcpTools } from "./channel-tools.js";

/**
 * MCP stdio server assembly for ACTAgent channel conversations.
 *
 ...
 */
export { ACTAgentChannelBridge } from "./channel-bridge.js";

/** Options accepted by the channel MCP server factory and stdio entry point. */
export type ACTAgentMcpServeOptions = {
  gatewayUrl?: string;
  gatewayToken?: string;
...
async function resolveMcpConfig(config: ACTAgentConfig | undefined): Promise<ACTAgentConfig> {
  if (config) {
    return config;
  }
...
export async function createACTAgentChannelMcpServer(opts: ACTAgentMcpServeOptions = {}): Promise<{
  server: McpServer;
  bridge: ACTAgentChannelBridge;
  start: () => Promise<void>;
  close: () => Promise<void>;
}> {
  ...
  const server = new McpServer(
    { name: "actagent", version: VERSION },
    capabilities ? { capabilities } : undefined,
  );
  const bridge = new ACTAgentChannelBridge(cfg, {
...
export async function serveACTAgentChannelMcp(opts: ACTAgentMcpServeOptions = {}): Promise<void> {
  const { server, start, close } = await createACTAgentChannelMcpServer(opts);

/**
 * Standalone MCP server that exposes ACTAgent plugin-registered tools
...
function resolvePluginToolPolicy(config: ACTAgentConfig): {
...
function resolveTools(config: ACTAgentConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "actagent-plugin-tools", tools });
}

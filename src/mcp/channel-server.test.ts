...
import { createACTAgentChannelMcpServer, ACTAgentChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createACTAgentChannelMcpServer({
...
describe("actagent channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new ACTAgentChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__actagent"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(

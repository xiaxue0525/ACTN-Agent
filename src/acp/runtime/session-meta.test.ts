...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTempDir({ prefix: "actagent-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "actagent.sqlite");
  ...
  it("honors ACTAGENT_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTempDir({ prefix: "actagent-acp-meta-" }, async (dir) => {
      const env = { ...process.env, ACTAGENT_STATE_DIR: dir } as NodeJS.ProcessEnv;

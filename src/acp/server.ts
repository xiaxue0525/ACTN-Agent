#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the ACTAgent Gateway. */
...
function printHelp(): void {
  console.log(`Usage: actagent acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or ACTAGENT_GATEWAY_TOKEN.",
    );
  }

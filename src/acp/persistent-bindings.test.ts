...
type ConfiguredBinding = NonNullable<ACTAgentConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: ACTAgentConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/actagent" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);

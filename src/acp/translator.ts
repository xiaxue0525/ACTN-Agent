...
import { readBool, readNonNegativeInteger, readString } from "@actagent/acp-core/meta";
import { defaultAcpSessionStore, type AcpSessionStore } from "@actagent/acp-core/session";
import { toAcpSessionLineageMeta } from "@actagent/acp-core/session-lineage-meta";
...
/** ACP Agent implementation backed by the ACTAgent Gateway and replay ledger. */
export class AcpGatewayAgent implements Agent {
  ...
  private assertSupportedSessionSetup(mcpServers: ReadonlyArray<unknown>): void {
    ...
    throw new Error(
      "ACP bridge mode does not support per-session MCP servers. Configure MCP on the ACTAgent gateway or agent instead.",
    );
  }

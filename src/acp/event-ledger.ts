...
import { resolveIntegerOption } from "@actagent/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the ACTAgent state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & ACTAgentStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: ACTAgentStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  const normalized = normalizeLedgerOptions(params);
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runACTAgentStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openACTAgentStateDatabase(dbOptions).db);

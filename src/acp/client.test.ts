...
const createTempDir = () => tempDirs.make("actagent-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ACTAGENT_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "actagent",
    });

    expect(env.ACTAGENT_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("actagent");
  });

  it("overrides pre-existing ACTAGENT_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ACTAGENT_SHELL: "wrong",
    });
    expect(env.ACTAGENT_SHELL).toBe("acp-client");
  });
  ...
  it("preserves ACTAGENT_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ACTAGENT_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ACTAGENT_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default ACTAgent bridge", () => {
    ...
    expect(env.ACTAGENT_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ACTAGENT_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "actagent",
        serverArgs: ["acp"],
        defaultServerCommand: "actagent",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ACTAGENT_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "actagent", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "actagent", "dist", "entry.js");
    const shimPath = path.join(dir, "actagent.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\actagent\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/actagent-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/actagent-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/actagent-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);

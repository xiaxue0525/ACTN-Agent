/** Interactive stdio ACP client used to connect a terminal session to an ACTAgent ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "actagent";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "actagent-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("ACTAgent ACP client");

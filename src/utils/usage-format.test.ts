...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["ACTAGENT_AGENT_DIR", "ACTAGENT_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-usage-format-"));
  ...
  it("prefers models.json pricing over actagent config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to actagent config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {

...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current ACTAgent nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentACTAgentClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("actagent-");
}

function isLegacyactagentdbotClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("actagentdbot-") || normalized.startsWith("moldbot-");
}

function pickPreferredLegacyMigrationMatch(
  matches: NodeMatchCandidate[],
): NodeMatchCandidate | undefined {
  const current = matches.filter((match) => isCurrentACTAgentClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  const legacyCount = matches.filter((match) => isLegacyactagentdbotClient(match.clientId)).length;
  if (legacyCount === 0 || current.length + legacyCount !== matches.length) {
    return undefined;
  }
  // During actagentdbot -> ACTAgent migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentACTAgentClient(node.clientId)) {
    score += 10;
  } else if (isLegacyactagentdbotClient(node.clientId)) {
    score -= 10;
  }
  return score;
}

...
import { normalizeCsvOrLooseStringList } from "@actagent/normalization-core/string-normalization";
...
/** Parses the JSON5 ACTAgent manifest block embedded inside a string frontmatter field. */
export function resolveACTAgentManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type ACTAgentManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an ACTAgent manifest block. */
export function resolveACTAgentManifestRequires(
  metadataObj: Record<string, unknown>,
): ACTAgentManifestRequires | undefined {
  const requiresRaw =
...
export function resolveACTAgentManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an ACTAgent manifest block. */
export function resolveACTAgentManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedACTAgentManifestInstallBase = {
...
export function parseACTAgentManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedACTAgentManifestInstallBase | undefined {
  ...
  const spec: ParsedACTAgentManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyACTAgentManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedACTAgentManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {

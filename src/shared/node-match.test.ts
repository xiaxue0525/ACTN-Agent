...
describe("shared/node-match", () => {
  ...
  it("matches candidates by node id, remote ip, normalized name, and long prefix", () => {
    ...
    expect(resolveNodeMatches(nodes, "actagent")).toStrictEqual([]);
  ...
  it("prefers a unique current ACTAgent client over a legacy actagentdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=actagent-macos.*node=third-mac.*client=actagent-macos/,
    );
  });

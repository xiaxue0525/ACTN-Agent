// Generates setup codes used to pair external channels with ACTAgent.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.ACTAGENT_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.ACTAGENT_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.ACTAGENT_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.ACTAGENT_GATEWAY_PASSWORD)),
  });

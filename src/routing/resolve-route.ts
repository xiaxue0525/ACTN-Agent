...
function listAgents(cfg: ACTAgentConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ACTAgentConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ACTAgentConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ACTAgentConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ACTAgentConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ACTAgentConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ACTAgentConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ACTAgentConfig,
  {
    bindingsRef: ACTAgentConfig["bindings"];
    agentsRef: ACTAgentConfig["agents"];
    sessionRef: ACTAgentConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ACTAgentConfig): Map<string, ResolvedAgentRoute> {

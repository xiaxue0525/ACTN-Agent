...
import { createAssistantMessageEventStream } from "actagent/plugin-sdk/llm";
...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.actagentCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.actagentCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the ACTAgent transport so attribution survives ACTAgent defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("actagent");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^actagent\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("actagent");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^actagent\//);
  });
});

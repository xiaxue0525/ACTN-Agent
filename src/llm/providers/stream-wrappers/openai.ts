...
type ACTAgentSimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: ACTAgentConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  baseStreamFn: StreamFn | undefined,
  params: { config?: ACTAgentConfig; agentDir?: string; codeModeToolSurfaceEnabled?: boolean },
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    if (
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: ACTAgentSimpleStreamOptions = {
        ...options,

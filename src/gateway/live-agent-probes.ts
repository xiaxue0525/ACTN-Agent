...
} from "@actagent/normalization-core/number-coercion";
...
const ACTAGENT_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const ACTAGENT_CLI_CHILD_TIMEOUT_MS = ACTAGENT_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the ACTAgent MCP tool `actagent-tools/cron` (server `actagent-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__actagent-tools__cron` or `mcp__actagent_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the ACTAgent cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the ACTAgent MCP tool `actagent-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__actagent-tools__cron` or `mcp__actagent_tools__cron`. " +
      ...
      "I will verify externally with the ACTAgent cron CLI."
    );
  }
  return (
    "Your previous ACTAgent cron MCP tool call was cancelled before the job was created. " +
    "Retry the ACTAgent MCP tool `actagent-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__actagent-tools__cron` or `mcp__actagent_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runACTAgentCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(ACTAGENT_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["actagent.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: ACTAGENT_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `actagent ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `actagent ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runACTAgentCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",

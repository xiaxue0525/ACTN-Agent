...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.ACTAGENT_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ACTAGENT_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ACTAGENT_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writePluginBindingApproval(params: {
  ...
  const actagentDir = path.join(params.homeDir, ".actagent");
  await fs.mkdir(actagentDir, { recursive: true });
  await fs.writeFile(
    path.join(actagentDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.ACTAGENT_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.ACTAGENT_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.ACTAGENT_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-live-codex-bind-"));
      ...
      const bindModel =
        process.env.ACTAGENT_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("ACTAGENT_CONFIG_PATH", previous.configPath);
        restoreEnvVar("ACTAGENT_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("ACTAGENT_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("ACTAGENT_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("ACTAGENT_SKIP_CRON", previous.skipCron);
        restoreEnvVar("ACTAGENT_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("ACTAGENT_STATE_DIR", previous.stateDir);
      }
    },

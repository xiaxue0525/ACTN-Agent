...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current ACTAgent session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsACTAgentStatus =
    normalized.includes("actagent is running on") ||
    /actagent\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("actagent status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsACTAgentStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from actagent status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live actagent config shows") ||

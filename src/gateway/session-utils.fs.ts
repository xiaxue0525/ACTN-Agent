...
/** Attach ACTAgent metadata to a transcript message without dropping existing metadata. */
export function attachACTAgentTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__actagent"] &&
    typeof record["__actagent"] === "object" &&
    !Array.isArray(record["__actagent"])
      ? (record["__actagent"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __actagent: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __actagent: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachACTAgentTranscriptMeta(message, { seq: firstSeq + index }),
  );
    ...
    const recordTimestampMs =
      typeof entry.timestamp === "string"
        ...
        : typeof entry.timestamp === "number"
          ? entry.timestamp
          : Number.NaN;
    return attachACTAgentTranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "actagent" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "actagent" && model === "delivery-mirror";

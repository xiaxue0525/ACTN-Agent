...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.ACTAGENT_GATEWAY_STARTUP_TRACE);
  let timelineConfig: ACTAgentConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.ACTAGENT_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: ACTAgentConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`actagent config set gateway.auth.mode token\` and \`actagent config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed ACTAgent config and rebuild.",
    "For the first-party Nix flow, see https://github.com/actagent/nix-actagent#quick-start and https://docs.actagent.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing actagent.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.ACTAGENT_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.ACTAGENT_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "ACTAGENT_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "ACTAGENT_RAW_STREAM_PATH",
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: ACTAgentConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: ACTAgentConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({

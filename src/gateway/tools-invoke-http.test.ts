...
let lastCreateACTAgentToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/actagent-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateACTAgentToolsContext?.agentTo,
          agentThreadId: lastCreateACTAgentToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createACTAgentTools: (ctx: Record<string, unknown>) => {
      lastCreateACTAgentToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateACTAgentToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-actagent-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-actagent-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateACTAgentToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateACTAgentToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateACTAgentToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateACTAgentToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateACTAgentToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateACTAgentToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateACTAgentToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-actagent-message-to": "channel:24514",
        "x-actagent-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as full operator access on /tools/invoke", async () => {
    ...
    expect(lastCreateACTAgentToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateACTAgentToolsContext?.senderIsOwner).toBe(false);

...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__actagent_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.ACTAGENT_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.ACTAGENT_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {

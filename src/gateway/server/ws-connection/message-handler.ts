...
/**
 * Lazily resolve the local node host's nodeId from ~/.actagent/node.json.
 * Process-stable: only changes on `actagent node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-actagent-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.actagent/node.json nodeId). SSH-tunneled remote

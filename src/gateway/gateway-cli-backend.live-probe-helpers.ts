...
async function callLoopbackJsonRpc(params: {
  ...
  if (params.messageProvider) {
    headers["x-actagent-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-actagent-account-id"] = params.accountId;
  }
  const timeoutMs = parsePositiveInt(
    params.env?.ACTAGENT_MCP_LOOPBACK_PROBE_TIMEOUT_MS,
    CLI_CRON_MCP_LOOPBACK_REQUEST_TIMEOUT_MS,
    "ACTAGENT_MCP_LOOPBACK_PROBE_TIMEOUT_MS",
  );
  const maxBodyBytes = parsePositiveInt(
    params.env?.ACTAGENT_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES,
    CLI_CRON_MCP_LOOPBACK_MAX_BODY_BYTES,
    "ACTAGENT_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES",
  );

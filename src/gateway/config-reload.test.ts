...
describe("buildGatewayReloadPlan", () => {
  ...
  it("treats plugin install timestamp-only changes as no-op", () => {
    const plan = buildGatewayReloadPlan([
      "plugins.installs.lossless-actagent.resolvedAt",
      "plugins.installs.lossless-actagent.installedAt",
    ]);
    expect(plan.restartGateway).toBe(false);
    ...
    expect(resolveConfigReloadMetadata("plugins.installs.lossless-actagent.resolvedAt").kind).toBe(
      "none",
    );
    expect(resolveConfigReloadMetadata("plugins.installs.lossless-actagent.installedAt").kind).toBe(
      "none",
    );
  });
  ...
  it("hot-reloads plugin entry config changes", () => {
    const plan = buildGatewayReloadPlan(["plugins.entries.lossless-actagent.config.mode"]);
    expect(plan.restartGateway).toBe(false);
    expect(plan.reloadPlugins).toBe(true);
    expect(plan.disposeMcpRuntimes).toBe(true);
    expect(plan.hotReasons).toContain("plugins.entries.lossless-actagent.config.mode");
  });
...
function createReloaderHarness(
  ...
  options: {
    initialCompareConfig?: ACTAgentConfig;
  ...
  const onHotReload = vi.fn(async (_plan: GatewayReloadPlan, _nextConfig: ACTAgentConfig) => {});
  const onRestart = vi.fn((_plan: GatewayReloadPlan, _nextConfig: ACTAgentConfig) => {});
...
function getOnlyRestartCall(harness: ReloaderHarness): [GatewayReloadPlan, ACTAgentConfig] {
...
function getOnlyHotReloadCall(harness: ReloaderHarness): [GatewayReloadPlan, ACTAgentConfig] {
...
describe("startGatewayConfigReloader", () => {
  ...
  it("skips plugin-local invalid reloads without degraded mode", async () => {
    const activeConfig: ACTAgentConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const invalidSnapshot = makeSnapshot({
      ...
      issues: [
        {
          path: "plugins.entries.lossless-actagent.config.cacheAwareCompaction",
    ...
    const promoteSnapshot = vi.fn(async (_snapshot: ConfigFileSnapshot, _reason: string) => true);
    const previousConfig: ACTAgentConfig = {
      ...activeConfig,
      plugins: {
        entries: {
          "lossless-actagent": {
    ...
    expect(
      log.warn.mock.calls.some(([message]) =>
        message.includes(
          "config reload skipped (invalid config): plugins.entries.lossless-actagent.config.cacheAwareCompaction:",
        ),
      ),
    ).toBe(true);
  ...
  it("hot-reloads direct diagnostics memory pressure snapshot edits", async () => {
    const nextConfig: ACTAgentConfig = {
      gateway: { reload: { debounceMs: 0 } },
  ...
  it("does not suppress functional install changes that collide with timestamp paths", async () => {
    ...
    const nextSourceConfig: ACTAgentConfig = {
      gateway: { reload: { debounceMs: 0 } },
      plugins: {
        installs: {
          lossless: {
  ...
  it("queues restart when an external plugin source write only changes the managed index", async () => {
    ...
    const readPluginInstallRecords = vi.fn().mockResolvedValueOnce({
      "lossless-actagent": {
        source: "npm",
        spec: "@martian-engineering/lossless-actagent",
        installPath: "/tmp/actagent/plugins/lossless-actagent",
        installedAt: "2026-04-22T00:00:00.000Z",
      },
    ...
    expect(harness.onHotReload).not.toHaveBeenCalled();
    const [plan, nextConfig] = getOnlyRestartCall(harness);
    expect(plan.changedPaths).toEqual(["plugins.installs.lossless-actagent"]);
    expect(plan.restartGateway).toBe(true);
    expect(plan.restartReasons).toEqual(["plugins.installs.lossless-actagent"]);
  ...
  it("keeps external plugin policy-only writes on the hot reload path", async () => {
    ...
    const installRecords = {
      telegram: {
        source: "npm",
        spec: "@actagent/telegram",
        installPath: "/tmp/actagent/plugins/telegram",
      },
    } satisfies Record<string, PluginInstallRecord>;
    const readSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>().mockResolvedValueOnce(
      makeSnapshot({
  ...
  it("queues restart when an external plugin source write also changes plugin config", async () => {
    ...
    expect(plan.changedPaths).toEqual(["plugins.entries", "plugins.installs.lossless-actagent"]);
    expect(plan.restartGateway).toBe(true);
    expect(plan.restartReasons).toEqual(["plugins.installs.lossless-actagent"]);

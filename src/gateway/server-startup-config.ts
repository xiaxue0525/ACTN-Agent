...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: ACTAgentConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: ACTAgentConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: ACTAgentConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: ACTAgentConfig): ACTAgentConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: ACTAgentConfig): void {

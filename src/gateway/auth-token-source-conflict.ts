...
const GATEWAY_ENV_TOKEN = "ACTAGENT_GATEWAY_TOKEN";
const GATEWAY_SERVICE_KIND = "gateway";

// Doctor/startup warning shape for shells where ACTAGENT_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.ACTAGENT_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.ACTAGENT_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.actagent/.env, or launchctl env if gateway.auth.token is intended, ` +

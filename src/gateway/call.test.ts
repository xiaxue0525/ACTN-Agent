...
function resetGatewayCallMocks() {
  ...
  const loadConfigForTests = getRuntimeConfig as unknown as () => ACTAgentConfig;
...
function setGatewayNetworkDefaults(port = 19199) {
  resolveGatewayPort.mockReturnValue(port);
  pickPrimaryTailnetIPv4.mockReturnValue(undefined);
}

function setLocalLoopbackGatewayConfig(port = 19199) {
...
function makeRemotePasswordGatewayConfig(remotePassword: string, localPassword = "from-config") {
  return {
    gateway: {
      mode: "remote",
      remote: { url: "wss://remote.example:19199", password: remotePassword },
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS;
    delete process.env.ACTAGENT_CONFIG_PATH;
    delete process.env.ACTAGENT_GATEWAY_PORT;
    delete process.env.ACTAGENT_GATEWAY_URL;
  ...
  it("uses url override in remote mode even when remote url is missing", async () => {
    ...
    resolveGatewayPort.mockReturnValue(19199);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
  ...
  it("keeps direct-local backend shared-token auth independent of paired device state", async () => {
    ...
    expect(lastClientOptions?.url).toBe("ws://127.0.0.1:19199");
    expect(lastClientOptions?.token).toBe("explicit-token");
  ...
  it("allows Tailscale-authenticated backend calls without client-side credentials", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        remote: { url: "wss://actagent.example.test" },
    ...
    expect(lastClientOptions?.url).toBe("wss://actagent.example.test");
    expect(lastClientOptions?.token).toBeUndefined();
  ...
  it("keeps backend device identity enabled for remote shared-token auth", async () => {
    ...
    expect(lastClientOptions?.url).toBe("wss://remote.example:19199");
    expect(lastClientOptions?.token).toBe("explicit-token");
    expect(lastClientOptions?.clientName).toBe(GATEWAY_CLIENT_NAMES.GATEWAY_CLIENT);
    expect(lastClientOptions?.mode).toBe(GATEWAY_CLIENT_MODES.BACKEND);
    expect(lastClientOptions?.deviceIdentity).toEqual(deviceIdentityState.value);
  ...
  it("uses ACTAGENT_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(19199);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.ACTAGENT_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token";
  ...
  it("uses remote tlsFingerprint with env URL override", async () => {
    ...
    setGatewayNetworkDefaults(19199);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.ACTAGENT_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("emits a remote fallback note when remote url is missing", () => {
    ...
    expect(details.url).toBe("ws://127.0.0.1:19199");
    ...
    expect(details.message).toContain("Gateway target: ws://127.0.0.1:19199");
  });
  ...
  it("uses env ACTAGENT_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.ACTAGENT_GATEWAY_URL;
    try {
      process.env.ACTAGENT_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env ACTAGENT_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.ACTAGENT_GATEWAY_URL;
      } else {
        process.env.ACTAGENT_GATEWAY_URL = prevUrl;
      }
    }
  });

  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-gateway-call-"));
    process.env.ACTAGENT_STATE_DIR = tempStateDir;
    process.env.ACTAGENT_CONFIG_PATH = path.join(tempStateDir, "missing-config.json");
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      testing.setDepsForTests({
        getRuntimeConfig: {} as never,
        resolveGatewayPort: () => 19199,
      });
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://remote.example.com:19199" },
      },
    });
    resolveGatewayPort.mockReturnValue(19199);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    ...
    expect((thrown as Error).message).toContain("actagent doctor --fix");
  });
  ...
  it("redacts credential-bearing target URLs from insecure ws:// errors", () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://user:pass@remote.example.com:19199/ws?token=secret-token" },
      },
    });
    resolveGatewayPort.mockReturnValue(19199);

    expect(() => buildGatewayConnectionDetails()).toThrow(
      'Gateway URL "ws://***:***@remote.example.com:19199/ws?token=***" uses plaintext',
    );
    try {
      buildGatewayConnectionDetails();
    } catch (error) {
  ...
  it("allows ws:// private remote URLs for trusted LAN and Tailnet configs", () => {
    ...
    expect(details.url).toBe("ws://10.0.0.8:19199");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });

  it("allows ws:// hostname remote URLs when ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://actagent-gateway.ai:19199" },
      },
    });
    resolveGatewayPort.mockReturnValue(19199);
    ...
    expect(details.url).toBe("ws://actagent-gateway.ai:19199");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway error details", () => {
  ...
  it("includes connection details when the gateway closes", async () => {
    ...
    expect(err?.message).toContain("Gateway target: ws://127.0.0.1:19199");
  ...
  it("includes connection details on timeout", async () => {
    ...
    expect(errMessage).toContain("Gateway target: ws://127.0.0.1:19199");
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["ACTAGENT_HANDSHAKE_TIMEOUT_MS"]);
    try {
  ...
  it("runs the signal abort hook on the active gateway connection before teardown", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => ACTAgentConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token";
    process.env.ACTAGENT_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.ACTAGENT_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "ACTAGENT_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ACTAGENT_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.ACTAGENT_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",

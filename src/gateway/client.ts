// ACTAgent Gateway client facade.
// Wraps the shared gateway-client package with ACTAgent host dependencies.
import {
...
function createACTAgentGatewayClientHostDeps(
  overrides?: GatewayClientHostDeps,
): GatewayClientHostDeps {
  return {
    // This wrapper is the only place the package reaches into ACTAgent runtime
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    ...
    // ACTAgent device identity, token storage, proxy routing, and logging.
    this.#client = new BaseGatewayClient({
      ...opts,
      clientVersion: opts.clientVersion ?? VERSION,
      hostDeps: createACTAgentGatewayClientHostDeps(opts.hostDeps),
    });
  }

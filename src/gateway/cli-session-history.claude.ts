...
// Converts Claude project JSONL into ACTAgent transcript-compatible messages.
...
import { attachACTAgentTranscriptMeta } from "./session-utils.fs.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; ACTAgent history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    return attachACTAgentTranscriptMeta(
      {
        role: "user",
        content,

...
import type { loadACTAgentPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "ACTAGENT_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: ACTAgentConfig): GatewayMemoryStartupPolicy {
...
async function resolveRestartSentinelPathFast(
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.ACTAGENT_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.ACTAGENT_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".actagent");
  if (env.ACTAGENT_TEST_FAST === "1" || (await pathExists(newStateDir))) {
    return path.join(newStateDir, RESTART_SENTINEL_FILENAME);
  }
  const legacyStateDir = path.join(home, ".actagentdbot");
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadACTAgentPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@actagent/model-catalog-core/provider-id");
  if (
  ...
  const { ensureACTAgentModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureACTAgentModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: ACTAgentConfig;
  pluginRegistry: ReturnType<typeof loadACTAgentPlugins>;
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@actagent/acp-core/runtime/session-identifiers"),
          ]);
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: ACTAgentConfig;
    pluginRegistry: ReturnType<typeof loadACTAgentPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;

...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.ACTAGENT_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.ACTAGENT_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: ACTAgentConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-actagent-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-actagent-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-actagent-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-actagent-current-message-id")),
    currentInboundAudio: normalizeMcpCurrentInboundAudio(
      getHeader(req, "x-actagent-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-actagent-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-actagent-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-actagent-source-reply-delivery-mode"),
    ),

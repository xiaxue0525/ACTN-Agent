...
const WRITE_SCOPE_HEADER = { "x-actagent-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-actagent-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("actagent/default");
  ...
  it("rejects disallowed x-actagent-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "actagent/default",
        input: "hello",
      },
      { "x-actagent-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,

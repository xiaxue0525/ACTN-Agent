// OpenAI-compatible `/v1/models` HTTP route backed by configured ACTAgent agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([ACTAGENT_MODEL_ID, ACTAGENT_DEFAULT_MODEL_ID]);
  ids.add(`actagent/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`actagent/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== ACTAGENT_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {

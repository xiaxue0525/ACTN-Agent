...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `ACTAGENT_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "ACTAGENT_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("ACTAGENT_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.ACTAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.ACTAGENT_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "ACTAGENT_LIVE_CLI_BACKEND_ARGS",
      process.env.ACTAGENT_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `ACTAGENT_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.ACTAGENT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.ACTAGENT_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.ACTAGENT_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.ACTAGENT_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.ACTAGENT_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.ACTAGENT_SKIP_PROVIDERS = "1";
  process.env.ACTAGENT_SKIP_GMAIL_WATCHER = "1";
  process.env.ACTAGENT_SKIP_CRON = "1";
  process.env.ACTAGENT_SKIP_CANVAS_HOST = "1";
  process.env.ACTAGENT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ACTAGENT_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("ACTAGENT_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("ACTAGENT_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("ACTAGENT_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("ACTAGENT_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("ACTAGENT_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("ACTAGENT_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("ACTAGENT_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("ACTAGENT_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("ACTAGENT_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("ACTAGENT_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("ACTAGENT_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);

...
function createSnapshot(config: ACTAgentConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: ACTAgentConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: ACTAgentConfig) {
...
// set ACTAGENT_SKIP_CHANNELS / ACTAGENT_SKIP_PROVIDERS at module load. When a

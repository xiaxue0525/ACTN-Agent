...
function configWithDefaultEnvProvider(gateway: NonNullable<ACTAgentConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: ACTAgentConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: ACTAgentConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("ACTAGENT_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        ACTAGENT_GATEWAY_TOKEN: "test-token-from-env",

...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("actagent doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.ACTAGENT_PROXY_ACTIVE;
    delete process.env.ACTAGENT_PROXY_LOOPBACK_MODE;
  ...
  it("blocks ws:// to non-loopback addresses (CWE-319)", () => {
    ...
    const client = new GatewayClient({
      url: "ws://remote.example.com:19199",
      onConnectError,
    });
  ...
  it("keeps gateway-only loopback bypass active only during WebSocket construction", () => {
    ...
    expect(proxylineRegisterBypassMock).toHaveBeenCalledWith({ url: "ws://127.0.0.1:19199" });
  ...
  it("allows wss:// to any address", () => {
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "wss://remote.example.com:19199",
      onConnectError,
    });
  ...
  it("allows ws:// hostnames with ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://actagent-gateway.ai:19199",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { ACTAGENT_HOME: "/tmp/custom-actagent-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      ACTAGENT_STATE_DIR: "/tmp/actagent-client-service-state",
  ...
  it("does not auto-reconnect on token mismatch when retry is not trusted", async () => {
    loadDeviceAuthTokenMock.mockReturnValue({ token: "stored-device-token" });
    const client = new GatewayClient({
      url: "wss://gateway.example.com:19199",
      token: "shared-token",
    });

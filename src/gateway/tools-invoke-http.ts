...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-actagent-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-actagent-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-actagent-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-actagent-thread-id"));

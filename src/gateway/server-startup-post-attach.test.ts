...
const hoisted = vi.hoisted(() => {
  ...
  const ensureACTAgentModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/actagent-state",
    resolveConfigPath: vi.fn(() => "/tmp/actagent-state/actagent.json"),
    resolveGatewayPort: vi.fn(() => 19199),
    resolveStateDir: vi.fn(() => "/tmp/actagent-state"),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureACTAgentModelsJson: hoisted.ensureACTAgentModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("ACTAGENT_SKIP_CHANNELS", "0");
    vi.stubEnv("ACTAGENT_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureACTAgentModelsJson.mockReset();
    hoisted.ensureACTAgentModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-home-"));
    try {
      const actagentHome = path.join(osHome, "actagent-home");
      const stateDirFromHome = path.join(actagentHome, ".actagent");
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          ACTAGENT_HOME: "~/actagent-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\actagent-state`);
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          ACTAGENT_STATE_DIR: "~\\actagent-state",
        } as NodeJS.ProcessEnv),
      ).toBe(true);
    } finally {
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/actagent-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/actagent-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        ACTAGENT_SKIP_CHANNELS: undefined,
        ACTAGENT_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { ACTAGENT_SKIP_CHANNELS: undefined, ACTAGENT_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { ACTAGENT_SKIP_CHANNELS: "1", ACTAGENT_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (ACTAGENT_SKIP_CHANNELS=1 or ACTAGENT_SKIP_PROVIDERS=1)",
    );
  });
  ...
  it("passes typed gateway_start context with config, workspace dir, and a live cron getter", async () => {
    ...
    expect(event).toEqual({ port: 19199 });
    expect(ctx.port).toBe(19199);

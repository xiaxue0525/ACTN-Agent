...
function asPluginInstallConfig(records: PluginInstallRecords): ACTAgentConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ACTAgentConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ACTAgentConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ACTAgentConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: ACTAgentConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {

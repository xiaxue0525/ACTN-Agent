...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.send creates a configured agent main session before sending", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-send-agent-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-"));
  ...
  test("chat.history mirrors current-session message tool sends before NO_REPLY", async () => {
    ...
    expect(
      historyMessages.some((message) => {
        ...
        const entry = message as { role?: unknown; actagentMessageToolMirror?: unknown };
        return entry.role === "assistant" && Boolean(entry.actagentMessageToolMirror);
      }),
    ).toBe(true);
  });
    ...
    expect(
      historyMessages.some(
        (message) =>
          ...
          Boolean((message as { actagentMessageToolMirror?: unknown }).actagentMessageToolMirror),
      ),
    ).toBe(true);
  });

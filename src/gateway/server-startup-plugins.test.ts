...
const resolveACTAgentPackageRootSync = vi.hoisted(() => vi.fn((_params: unknown) => "/package"));
...
vi.mock("../infra/actagent-root.js", () => ({
  resolveACTAgentPackageRootSync: (params: unknown) => resolveACTAgentPackageRootSync(params),
}));
...
function slackConfig(): ACTAgentConfig {
  return {
    channels: {
...
describe("prepareGatewayPluginBootstrap startup plugins", () => {
  beforeEach(() => {
    ...
    resolveACTAgentPackageRootSync.mockClear().mockReturnValue("/package");

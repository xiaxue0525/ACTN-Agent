...
type GatewaySecretDefaults = NonNullable<ACTAgentConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${ACTAGENT_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ACTAGENT_GATEWAY_TOKEN));
}
...
export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ACTAGENT_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.ACTAGENT_GATEWAY_PASSWORD);

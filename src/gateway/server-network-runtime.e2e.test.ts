...
describe("gateway network runtime", () => {
  ...
  it("bootstraps env proxy dispatching when the gateway starts directly", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-proxy-home-"));
    ...
    try {
      ...
      process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = path.join(tempHome, "empty-bundled-plugins");
      await fs.mkdir(process.env.ACTAGENT_BUNDLED_PLUGINS_DIR, { recursive: true });

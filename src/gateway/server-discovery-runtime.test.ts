...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "actagent.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/actagent.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (ACTAgent)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/actagent"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.ACTAGENT_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.ACTAGENT_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.ACTAGENT_SSH_PORT = "2222";
    ...
    expect(bonjour.service.advertise).toHaveBeenCalledWith({
      ...
      cliPath: "/usr/local/bin/actagent",
      minimal: false,
    });
  ...
  it("skips local discovery services for truthy ACTAGENT_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.ACTAGENT_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "actagent.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("actagent.internal.");
    expect(zoneParams.gatewayPort).toBe(19199);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (ACTAgent)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (actagent.internal. → /tmp/actagent.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });

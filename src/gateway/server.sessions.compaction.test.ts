...
test("sessions.compact without maxLines runs embedded manual compaction for checkpoint-capable flows", async () => {
  ...
  expect(compactionCall.workspaceDir).toBe(path.join(os.tmpdir(), "actagent-gateway-test"));
  ...
  expect(callConfig.agents?.defaults?.workspace).toBe(
    path.join(os.tmpdir(), "actagent-gateway-test"),
  );
...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gw-sessions-nested-"));
  ...
  await withEnvAsync({ ACTAGENT_CONFIG_PATH: undefined }, async () => {

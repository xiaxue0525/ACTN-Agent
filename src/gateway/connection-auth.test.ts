...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: ACTAgentConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<ACTAgentConfig>): ACTAgentConfig {
  return input as ACTAgentConfig;
}
...
const DEFAULT_ENV = {
  ...
  ACTAGENT_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when ACTAGENT env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when ACTAGENT env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when ACTAGENT env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {

...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ACTAGENT_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: ACTAgentConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: ACTAgentConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: ACTAgentConfig, requestedAgentId: string) => {

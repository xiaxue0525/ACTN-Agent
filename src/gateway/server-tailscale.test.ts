...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  it("calls enableTailscaleServe in serve mode when preserveFunnel is unset", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(19199);
  ...
  it("skips enableTailscaleServe when preserveFunnel is true and a Funnel route covers the port", async () => {
    ...
    expect(mocks.hasTailscaleFunnelRouteForPort).toHaveBeenCalledWith(19199);
    expect(mocks.enableTailscaleServe).not.toHaveBeenCalled();
    expect(logTailscale.info.mock.calls).toEqual([
      ["serve skipped: preserving externally configured Tailscale Funnel for port 19199"],
    ]);
  });
  ...
  it("notes resetOnExit is a no-op when preserveFunnel skips Serve", async () => {
    ...
    expect(logTailscale.info.mock.calls).toEqual([
      [
        "serve skipped: preserving externally configured Tailscale Funnel for port 19199; resetOnExit is a no-op because no Serve route was applied this run",
      ],
    ]);
  });
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(19199, undefined, "svc:actagent");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:actagent: https://actagent.tailnet.ts.net/ (WS via wss://actagent.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:actagent");
  });
  ...
  it("does not use serviceName in funnel mode", async () => {
    ...
    expect(mocks.enableTailscaleFunnel).toHaveBeenCalledWith(19199);
    expect(mocks.enableTailscaleServe).not.toHaveBeenCalled();
    expect(logTailscale.info).toHaveBeenCalledWith(

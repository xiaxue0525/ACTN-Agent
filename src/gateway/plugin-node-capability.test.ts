...
describe("plugin node capability helpers", () => {
  test("builds scoped host urls from clean base urls", () => {
    expect(
      buildPluginNodeCapabilityScopedHostUrl(
        "http://127.0.0.1:19199/root/?debug=1#hash",
        "token value",
      ),
    ).toBe("http://127.0.0.1:19199/root/__actagent__/cap/token%20value");
    expect(buildPluginNodeCapabilityScopedHostUrl("not a url", "token")).toBeUndefined();
    expect(buildPluginNodeCapabilityScopedHostUrl("http://127.0.0.1:19199", " ")).toBeUndefined();
  });
  ...
  test("normalizes scoped urls and moves capability into the query string", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl(
      "/__actagent__/cap/token%20value/__actagent__/canvas/file.txt?download=1",
    );
    expect(normalized).toEqual({
      pathname: "/__actagent__/canvas/file.txt",
      capability: "token value",
      rewrittenUrl: "/__actagent__/canvas/file.txt?download=1&oc_cap=token+value",
      scopedPath: true,
  ...
  test("replaces scoped capability tokens without nesting capability prefixes", () => {
    expect(
      replacePluginNodeCapabilityInScopedHostUrl(
        "http://127.0.0.1:19199/__actagent__/cap/old-token/__actagent__/a2ui/",
        "new token",
      ),
    ).toBe("http://127.0.0.1:19199/__actagent__/cap/new%20token/__actagent__/a2ui");
  });

  test("marks malformed scoped urls without authorizing a path capability", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl("/__actagent__/cap/broken");
  ...
  test("refreshes client plugin surface url and stored capability", () => {
    const client = makeClient({
      pluginSurfaceUrls: {
        canvas: "http://127.0.0.1:19199/__actagent__/cap/old-token",
      },
    ...
    expect(refreshed?.scopedUrl).toContain("/__actagent__/cap/");
    expect(refreshed?.scopedUrl).not.toContain("old-token/__actagent__/cap/");
  ...
  test("does not refresh client plugin capabilities when the clock is invalid", () => {
    ...
    expect(client.pluginSurfaceUrls?.canvas).toBe(
      "http://127.0.0.1:19199/__actagent__/cap/old-token",
    );

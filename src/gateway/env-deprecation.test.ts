...
import {
  resetLegacyACTAgentEnvWarningForTest,
  warnLegacyACTAgentEnvVars,
} from "./env-deprecation.js";

describe("warnLegacyACTAgentEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("Legacy ACTAGENTBOT_*, MOLTBOT_* environment variables");
    expect(message).toContain("3 total");
    expect(message).toContain("replacing the legacy prefix with ACTAGENT_");
    ...
    expect(options).toEqual({
      code: "ACTAGENT_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current ACTAGENT names", () => {
    warnLegacyACTAgentEnvVars({ ACTAGENT_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyACTAgentEnvVars({ ACTAGENTBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyACTAgentEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyACTAgentEnvVars({ ACTAGENTBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyACTAgentEnvVars({ ACTAGENTBOT_GATEWAY_TOKEN: "old-token" });

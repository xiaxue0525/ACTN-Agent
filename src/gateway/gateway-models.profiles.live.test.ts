...
import { ensureACTAgentModelsJson } from "../agents/models-config.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, ACTAgentConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ACTAGENT_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.ACTAGENT_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.ACTAGENT_LIVE_GATEWAY_SMOKE);
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.ACTAGENT_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.ACTAGENT_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.ACTAGENT_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["ACTAGENT_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveSetupTimeoutMs(
  raw = process.env.ACTAGENT_LIVE_GATEWAY_SETUP_TIMEOUT_MS,
): number {
  return Math.max(1_000, toInt(raw, 180_000));
}

function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.ACTAGENT_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.ACTAGENT_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.ACTAGENT_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.ACTAGENT_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.ACTAGENT_TEST_FAST,
  };
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.ACTAGENT_TEST_FAST = previous.testFast;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.ACTAGENT_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.ACTAGENT_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.ACTAGENT_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("ACTAGENT_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("ACTAGENT_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("ACTAGENT_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.ACTAGENT_LIVE_GATEWAY_MODELS;
    delete process.env.ACTAGENT_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ACTAGENT_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.ACTAGENT_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.ACTAGENT_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ACTAGENT_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.ACTAGENT_LIVE_GATEWAY_MAX_MODELS = "2";
...
describe("buildLiveGatewayConfig", () => {
  it("pins selected live gateway models to the ACTAgent runtime", () => {
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      expect(process.env.ACTAGENT_TEST_FAST).toBeUndefined();
    } finally {
      ...
      restoreOptionalEnv("ACTAGENT_TEST_FAST", previous.testFast);
    }
  });
});
      ...
      defaults: {
        ...
        models: Object.fromEntries(
          params.candidates.map((m) => [
            `${m.provider}/${m.id}`,
            { agentRuntime: { id: "actagent" as const } },
          ]),
        ),
      },
    },
...
async function sanitizeAuthConfig(params: {
  cfg: ACTAgentConfig;
  agentDir: string;
}): Promise<ACTAgentConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<ACTAgentConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const runtimeEnv = enterProductionEnvForLiveRun();
  const previous = {
    configPath: process.env.ACTAGENT_CONFIG_PATH,
    token: process.env.ACTAGENT_GATEWAY_TOKEN,
    skipChannels: process.env.ACTAGENT_SKIP_CHANNELS,
    skipGmail: process.env.ACTAGENT_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ACTAGENT_SKIP_CRON,
    skipCanvas: process.env.ACTAGENT_SKIP_CANVAS_HOST,
    disableBonjour: process.env.ACTAGENT_DISABLE_BONJOUR,
    logLevel: process.env.ACTAGENT_LOG_LEVEL,
    agentDir: process.env.ACTAGENT_AGENT_DIR,
    stateDir: process.env.ACTAGENT_STATE_DIR,
  };

  process.env.ACTAGENT_SKIP_CHANNELS = "1";
  process.env.ACTAGENT_SKIP_GMAIL_WATCHER = "1";
  process.env.ACTAGENT_SKIP_CRON = "1";
  process.env.ACTAGENT_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.ACTAGENT_DISABLE_BONJOUR = "1";
    process.env.ACTAGENT_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.ACTAGENT_GATEWAY_TOKEN = token;
  const agentId = "dev";
  ...
  const tempStateDir: string | undefined = await fs.mkdtemp(
    path.join(os.tmpdir(), "actagent-live-state-"),
  );
  process.env.ACTAGENT_STATE_DIR = tempStateDir;
  ...
  process.env.ACTAGENT_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".actagent"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".actagent", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.actagent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: ACTAgentConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-live-"));
  const tempConfigPath = path.join(tempDir, "actagent.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ACTAGENT_CONFIG_PATH = tempConfigPath;
  ...
  try {
              ...
              for (
                ...
                try {
                  toolText = await requestGatewayAgentText({
                    ...
                    modelKey,
                    message: strictReply
                      ? "ACTAgent live tool probe (local, safe): " +
    ...
    process.env.ACTAGENT_CONFIG_PATH = previous.configPath;
    process.env.ACTAGENT_GATEWAY_TOKEN = previous.token;
    process.env.ACTAGENT_SKIP_CHANNELS = previous.skipChannels;
    process.env.ACTAGENT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ACTAGENT_SKIP_CRON = previous.skipCron;
    process.env.ACTAGENT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ACTAGENT_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.ACTAGENT_LOG_LEVEL = previous.logLevel;
    process.env.ACTAGENT_AGENT_DIR = previous.agentDir;
    process.env.ACTAGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const modelsJsonResult = await withGatewayLiveSetupTimeout(
          ensureACTAgentModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ACTAGENT_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    await ensureACTAgentModelsJson(cfg);
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.actagent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

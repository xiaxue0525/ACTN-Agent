...
import { asFiniteNumber } from "@actagent/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@actagent/normalization-core/record-coerce";
import { normalizeOptionalString } from "@actagent/normalization-core/string-coerce";
import { ACTAGENT_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("actagentReasoningReplay" in entry) {
    delete entry.actagentReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__actagent"]);
  if (transcriptMeta) {
    mirror["__actagent"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readTtsSupplementMarker(
  ...
  const marker = message.actagentTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__actagent"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === ACTAGENT_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function actagentAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "actagent" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    actagentAssistantModel(previousVisible) !== "acp-runtime" ||
    actagentAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }

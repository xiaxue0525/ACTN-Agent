...
const { createACTAgentTools } = await import("../agents/actagent-tools.js");
...
type SessionSendTool = ReturnType<typeof createACTAgentTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createACTAgentTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ACTAGENT_GATEWAY_PORT", "ACTAGENT_GATEWAY_TOKEN"]);
  ...
  process.env.ACTAGENT_GATEWAY_PORT = String(gatewayPort);
  process.env.ACTAGENT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-send-route-"));

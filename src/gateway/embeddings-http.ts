...
// Bridges /v1/embeddings requests to configured ACTAgent memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps ACTAgent agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== ACTAGENT_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `actagent` or `actagent/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-actagent-model")) ||

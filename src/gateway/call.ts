...
import { isLoopbackIpAddress } from "@actagent/net-policy/ip";
...
type GatewayRuntimeConfigLoader = () => ACTAgentConfig | Promise<ACTAgentConfig>;
const defaultGetRuntimeConfig = async (): Promise<ACTAgentConfig> =>
...
async function loadGatewayConfig(): Promise<ACTAgentConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: ACTAgentConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: ACTAgentConfig) {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.ACTAGENT_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.ACTAGENT_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ACTAGENT_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as ACTAgentConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `actagent doctor` for diagnostics.";
  }
  return message;
}

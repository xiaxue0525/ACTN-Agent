...
import { CURRENT_SESSION_VERSION, SessionManager } from "actagent/plugin-sdk/agent-sessions";
import type { AssistantMessage } from "actagent/plugin-sdk/llm";
...
function checkpointConfig(storePath: string): ACTAgentConfig {
  return {
...
describe("session-compaction-checkpoints", () => {
  test("async capture stores pre-compaction identity without copying the transcript", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-async-"));
    tempDirs.push(dir);
  ...
  test("async capture derives session metadata without synchronous SessionManager.open", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-async-metadata-"));
    tempDirs.push(dir);
  ...
  test("async capture scans bounded metadata without copying oversized transcripts", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-async-oversized-"));
    tempDirs.push(dir);
  ...
  test("async fork creates a checkpoint branch transcript without SessionManager sync reads", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork truncates mutable checkpoint sources at the stored leaf", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-fork-leaf-"));
    tempDirs.push(dir);
  ...
  test("async fork migrates legacy checkpoint snapshots before writing a current header", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-legacy-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork skips JSON-valid garbage transcript entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-checkpoint-garbage-fork-"));
    tempDirs.push(dir);

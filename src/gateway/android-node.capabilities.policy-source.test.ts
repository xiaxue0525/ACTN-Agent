  ...
  it("returns true for config-driven remote urls even if loopback-tunneled", () => {
    expect(
      shouldFetchRemotePolicyConfig(
        details({ url: "ws://127.0.0.1:19199", urlSource: "config gateway.remote.url" }),
      ),
    ).toBe(true);
  });

  it("returns true for env and cli overrides", () => {
    expect(shouldFetchRemotePolicyConfig(details({ urlSource: "env ACTAGENT_GATEWAY_URL" }))).toBe(
      true,
    );

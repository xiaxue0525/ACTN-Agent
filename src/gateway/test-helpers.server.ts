...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ACTAGENT_CONFIG_PATH) {
    configPaths.add(process.env.ACTAGENT_CONFIG_PATH);
  }
  if (process.env.ACTAGENT_STATE_DIR) {
    configPaths.add(path.join(process.env.ACTAGENT_STATE_DIR, "actagent.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "actagent-test-no-bundled-extensions")
    : "actagent-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".actagent-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".actagent-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".actagent-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>actagent-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__actagentTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__actagentTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["ACTAGENT_GATEWAY_TOKEN"]);
  const prev = process.env.ACTAGENT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ACTAGENT_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ACTAGENT_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

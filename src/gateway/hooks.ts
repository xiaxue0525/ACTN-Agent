...
export function resolveHooksConfig(cfg: ACTAgentConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ACTAgentConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-actagent-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-actagent-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-actagent-idempotency-key"]) ||

...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listACTAgentPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listACTAgentPluginManifestMetadata: (
      params?: Parameters<typeof actual.listACTAgentPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listACTAgentPluginManifestMetadata(params);
      return actual.listACTAgentPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listACTAgentPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as ACTAgentConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listACTAgentPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/actagent.plugin.json",
    ...overrides,
  };
}

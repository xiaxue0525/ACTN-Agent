  ...
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-actagent-account-id": "work",
        "x-actagent-message-channel": "telegram",
        "x-actagent-current-channel-id": "telegram:chat123",
        "x-actagent-current-thread-ts": "42",
        "x-actagent-current-message-id": "reply-message-1",
        "x-actagent-current-inbound-audio": "true",
        "x-actagent-inbound-event-kind": "room_event",
        "x-actagent-source-reply-delivery-mode": "message_tool_only",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-actagent-message-channel": "telegram",
          "x-actagent-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-actagent-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-actagent-current-inbound-audio": "true" } : {}),
        },
      });
    ...
    const sendToolsList = async (token?: string) =>
      await sendLoopbackToolsList({
        ...
        headers: {
          ...
          "x-actagent-message-channel": "matrix",
        },
      });
  ...
  it("ignores spoofed owner headers on loopback requests", async () => {
    ...
    const response = await sendLoopbackToolsList({
      ...
      headers: {
        "x-session-key": "agent:main:matrix:dm:test",
        "x-actagent-message-channel": "matrix",
        "x-actagent-sender-is-owner": "true",
      },
    });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.actagent?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.actagent?.headers?.Authorization).toBe(
      "Bearer ${ACTAGENT_MCP_TOKEN}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-message-channel"]).toBe(
      "${ACTAGENT_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-current-channel-id"]).toBe(
      "${ACTAGENT_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-current-thread-ts"]).toBe(
      "${ACTAGENT_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-current-message-id"]).toBe(
      "${ACTAGENT_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-current-inbound-audio"]).toBe(
      "${ACTAGENT_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.actagent?.headers?.["x-actagent-source-reply-delivery-mode"]).toBe(
      "${ACTAGENT_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(config.mcpServers?.actagent?.headers).not.toHaveProperty("x-actagent-sender-is-owner");
  });
});

...
import { isAudioFileName } from "@actagent/media-core/mime";
...
function resolveWebchatPromptCacheKey(params: {
  ...
  const digest = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  return `actagent-webchat-${digest}`;
}
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__actagent"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "actagent" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as ACTAgentConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__actagent"]);
...
async function handleChatHistoryRequest({
  ...
  const requestedAgentId = resolveRequestedChatAgentId({
    cfg: (context as { getRuntimeConfig?: () => ACTAgentConfig }).getRuntimeConfig?.(),
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent ACTAgent session model is text-only.
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through ACTAgent runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { actagentTtsSupplement: ttsSupplementMarker }
                          : {}),

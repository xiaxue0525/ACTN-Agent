...
const TEST_AGENT_WORKSPACE = "/tmp/actagent-test-workspace";
...
describe("gateway send mirroring", () => {
  ...
  it("uses the resolved runtime config for message.action when the source snapshot matches", async () => {
          ...
          accounts: {
            dractagent: {
              token: {
                source: "env",
                provider: "default",
                id: "DISCORD_BOT_TOKEN_dractagent",
              },
            },
          },
        },
    ...
    await sendHandlers["message.action"]({
      params: {
        channel: "discord",
        action: "channel-info",
        params: { channelId: "123", accountId: "dractagent" },
  ...
  it("keeps the post-auto-enable request config for message.action when the runtime source snapshot does not match", async () => {
    ...
    mocks.getRuntimeConfigSnapshot.mockReturnValue({
      channels: {
        discord: {
          accounts: {
            dractagent: { token: "stale-runtime-token" },
          },
        },
      },
    });
  ...
  it("maps gateway asVoice sends onto outbound audioAsVoice payloads", async () => {
    ...
    const { respond } = await runSend({
      to: "channel:C1",
      message: "voice note",
      mediaUrl: "file:///tmp/actagent-voice.ogg",
      asVoice: true,
      channel: "slack",
    ...
    expect(deliveryCall()?.payloads?.[0]?.mediaUrl).toBe("file:///tmp/actagent-voice.ogg");

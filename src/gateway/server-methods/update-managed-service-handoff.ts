...
const SERVICE_IDENTITY_ENV_VARS = new Set<string>([
  ...
  "ACTAGENT_SYSTEMD_UNIT",
  "ACTAGENT_WINDOWS_TASK_NAME",
] as const);
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `actagent-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside actagent-gateway.service",
    );
  }

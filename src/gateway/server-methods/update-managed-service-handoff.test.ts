...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-handoff-helper-test-"));
  tempDirs.add(tmpDir);
  ...
  await startManagedServiceUpdateHandoff({
    ...
    argv1: "/opt/actagent/actagent.mjs",
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      ACTAGENT_LAUNCHD_LABEL: "com.example.actagent.test",
      ACTAGENT_SYSTEMD_UNIT: "actagent-test.service",
      ACTAGENT_WINDOWS_TASK_NAME: "ACTAgent Test Gateway",
    ...
    expect(result.command).toBe("actagent update --yes --timeout 1800");
    ...
    expect(options.env.ACTAGENT_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=actagent-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.ACTAGENT_SYSTEMD_UNIT).toBe("actagent-gateway.service");
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "actagent-other-temp");

...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "actagent update --yes --timeout 1800",
  logPath: "/tmp/actagent-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/actagent-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/actagent-root.js")>(
    "../../infra/actagent-root.js",
  );
  return {
    ...actual,
    resolveACTAgentPackageRoot: async () => "/tmp/actagent",
  };
});
...
vi.mock("./update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (timeoutMs?: number) =>
    timeoutMs
      ? `actagent update --yes --timeout ${Math.ceil(timeoutMs / 1000)}`
      : "actagent update --yes",
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/actagent-global",
    packageRoot: "/tmp/actagent-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("keeps git/dev updates on the in-process gateway update path", async () => {
    ...
    mockGitInstallSurface("/tmp/actagent-git");
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      command: "actagent update --yes --timeout 1800",
      message:
        "Run `actagent update --yes --timeout 1800` from a shell outside the gateway service.",
    });
  });

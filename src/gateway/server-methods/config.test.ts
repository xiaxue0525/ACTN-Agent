...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/actagent.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/actagent.json"],
    });
  });

  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/actagent.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/actagent.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.ACTAGENT_CONFIG_PATH = "/tmp/config $(touch pwned).json";
    execFileMock.mockImplementation((...args: unknown[]) => {
  ...
  it("returns a detailed error and logs details when the opener fails", async () => {
    process.env.ACTAGENT_CONFIG_PATH = "/tmp/config.json";

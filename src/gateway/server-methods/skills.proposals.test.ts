...
vi.mock("../../skills/lifecycle/actagenthub.js", () => ({
  ...
  searchSkillsFromACTAgentHub: vi.fn(),
...
vi.mock("../../skills/security/actagenthub-verdicts.js", () => ({
  collectACTAgentHubVerdictTargets: vi.fn(() => []),
  fetchACTAgentSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    envSnapshot = captureEnv(["ACTAGENT_STATE_DIR"]);
    ...
    mocks.workspaceDir = await tempDirs.make("actagent-skills-proposals-gateway-");
    stateDir = await tempDirs.make("actagent-skills-proposals-gateway-state-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("actagent-skills-proposals-gateway-second-");

...
const getRuntimeConfig = vi.hoisted(() =>
  vi.fn<() => ACTAgentConfig>(() => ({}) as ACTAgentConfig),
);
...
function setRuntimeConfig(config: ACTAgentConfig): void {
  getRuntimeConfig.mockReturnValue(config);
}

function pluginEntries(...ids: string[]): ACTAgentConfig["plugins"] {
  return {
    entries: Object.fromEntries(ids.map((id) => [id, { enabled: true }])),
  };
}

function telegramConfig(): ACTAgentConfig {
  return {
    channels: {
      telegram: {
...
function telegramSlackConfig(params: { includeMainSession?: boolean } = {}): ACTAgentConfig {
  return {
...
function msteamsConfig(): ACTAgentConfig {
  return {
    channels: {
      msteams: {
...
function slackSynologyConfig(): ACTAgentConfig {
  return {
    channels: {
      slack: {
        botToken: "xoxb-slack-token",
...
function slackConfig(params: { includeMainSession?: boolean } = {}): ACTAgentConfig {
  return {
    ...(params.includeMainSession ? { session: { mainKey: "main" } } : {}),
    channels: {
      slack: {
...
describe("cron method validation", () => {
  beforeEach(() => {
    getRuntimeConfig.mockReset().mockReturnValue({} as ACTAgentConfig);

...
describe("node plugin surface refresh", () => {
  it("refreshes generic plugin surface capability urls", async () => {
    ...
    expect(parsedCanvasUrl.origin).toBe("http://127.0.0.1:19199");
    expect(parsedCanvasUrl.pathname.startsWith("/__actagent__/cap/")).toBe(true);
    const capabilityToken = parsedCanvasUrl.pathname.slice("/__actagent__/cap/".length);

...
import { fetchACTAgentHubSkillDetail } from "../../infra/actagenthub.js";
...
import {
  collectACTAgentHubVerdictTargets,
  fetchACTAgentSkillSecurityVerdicts,
} from "../../skills/security/actagenthub-verdicts.js";
import {
  applySkillProposal,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      const report = buildRemoteAwareWorkspaceSkillStatus(resolved);
      const targets = collectACTAgentHubVerdictTargets(report);
      if (targets.length === 0) {
        respond(true, { schema: "actagent.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchACTAgentSkillSecurityVerdicts(targets);
      respond(true, { schema: "actagent.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
  ...
  "skills.install": async ({ params, respond, context }) => {
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "actagenthub") {
      const p = params as {
        source: "actagenthub";
        slug: string;
        version?: string;
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "actagenthub") {
      ...
      if (!p.slug && !p.all) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, 'actagenthub skills.update requires "slug" or "all"'),
        );
        return;
      }
      if (p.slug && p.all) {
        respond(
          ...
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            'actagenthub skills.update accepts either "slug" or "all", not both',
          ),
        );
        return;
      }

...
const persistedConfigResultMock = vi.fn((config: ACTAgentConfig) => config);
...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/actagent.json" }),
    writeConfigFile: writeConfigFileMock,
    replaceConfigFile: async (params: { nextConfig: ACTAgentConfig; writeOptions?: unknown }) => {
...
function tokenAuthConfig(token: string): ACTAgentConfig {
  return {
    gateway: {
      auth: {
        mode: "token",
...
function hotReloadConfig(): ACTAgentConfig {
  return {
    gateway: {
      reload: {
        mode: "hot",
...
function mockPreviousConfig(config: ACTAgentConfig): void {
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: ACTAgentConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: ACTAgentConfig }) => ({
      config,
    }),
  );
  restartSentinelMocks.writeRestartSentinel.mockClear();
  persistedConfigResultMock.mockImplementation((config: ACTAgentConfig) => config);
});

describe("config shared auth disconnects", () => {
  it("returns the persisted config from config.set write results", async () => {
    const prevConfig: ACTAgentConfig = {
      gateway: {
        port: 19000,
      },
    };
    const submittedConfig: ACTAgentConfig = {
      gateway: {
        port: 19001,
      },
    };
    const persistedConfig: ACTAgentConfig = {
      gateway: {
        port: 19001,
      },
      meta: {

...
type GatewayInstallAuthMode = NonNullable<NonNullable<ACTAgentConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: ACTAgentConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.ACTAGENT_GATEWAY_PASSWORD) ||
    normalizeOptionalString(durableServiceEnv.ACTAGENTBOT_GATEWAY_PASSWORD),
  );
}

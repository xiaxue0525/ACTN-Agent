...
const CODEX_HARNESS_LIVE = process.env.ACTAGENT_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.ACTAGENT_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["ACTAGENT_TRAJECTORY", "ACTAGENT_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-actagent-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.ACTAGENT_TRAJECTORY = "1";
      process.env.ACTAGENT_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".actagent", "trajectory-exports", "bundle");

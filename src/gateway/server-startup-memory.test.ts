...
async function startMemoryBackendForTest(cfg: ACTAgentConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: ACTAgentConfig) {
...
function expectQmdManagerRequests(cfg: ACTAgentConfig, agentIds: string[]) {

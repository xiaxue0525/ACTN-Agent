...
const {
  ...
  prepareMirroredDocsDir: (
    sourceDir?: string,
    options?: {
      resolveACTAgentHubRepoPathImpl?: (value?: string, options?: { required?: boolean }) => string;
      syncACTAgentHubDocsTreeImpl?: (
...
describe("docs-link-audit", () => {
  ...
  it("cleans anchor audit docs copies when docs.json is invalid", () => {
    ...
    const before = tempEntries("actagent-docs-anchor-audit-");
    try {
      expect(() => prepareAnchorAuditDocsDir(docsRoot)).toThrow();
      const after = tempEntries("actagent-docs-anchor-audit-");
  ...
  it("cleans mirrored docs copies when ACTAgentHub sync fails", () => {
    const before = tempEntries("actagent-docs-link-audit-");

    expect(() =>
      prepareMirroredDocsDir(undefined, {
        resolveACTAgentHubRepoPathImpl() {
          return path.join(os.tmpdir(), "actagenthub-docs");
        },
        syncACTAgentHubDocsTreeImpl() {
    ...
    expect(() =>
      runDocsLinkAuditCli({
        ...
        prepareMirroredDocsDirImpl: () => ({
          ...
          dir: path.join(os.tmpdir(), "actagent-docs-mirrored"),
  ...
  it("uses Mintlify through pnpm dlx for anchor validation", () => {
    ...
    const exitCode = runDocsLinkAuditCli({
      args: ["--anchors"],
      env: { ...process.env, ACTAGENT_DOCS_LINK_SENTINEL: "1" },
    ...
    expect(invocation).toEqual({
      ...
      options: expect.objectContaining({
        cwd: anchorDocsDir,
        env: expect.objectContaining({ ACTAGENT_DOCS_LINK_SENTINEL: "1" }),
        shell: false,
        stdio: "inherit",
      }),
    });

...
function expectACTAgentSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = ACTAgentSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectACTAgentSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = ACTAgentSchema.safeParse(config);

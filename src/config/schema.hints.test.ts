...
import { isSensitiveUrlConfigPath } from "@actagent/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = ACTAgentSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ACTAgentConfig";
    const hints = mapSensitivePaths(ACTAgentSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(ACTAgentSchema, "", isSensitiveUrlConfigPath);

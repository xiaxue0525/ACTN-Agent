// Loads, validates, migrates, snapshots, and writes ACTAgent config files.
import crypto from "node:crypto";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { collectManifestModelIdNormalizationPolicies } from "@actagent/model-catalog-core/provider-model-id-normalization";
...
import type { ACTAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: ACTAgentConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: ACTAgentConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: ACTAgentConfig) => Promise<unknown>;
};
...
function coerceConfig(value: unknown): ACTAgentConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ACTAgentConfig;
}
...
function stampConfigVersion(cfg: ACTAgentConfig, version?: string): ACTAgentConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: ACTAgentConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your ACTAgent config was written by version ${touched}, but this command is running ${VERSION}.`,
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.ACTAGENT_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.ACTAGENT_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files ACTAgent writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ACTAgentConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: ACTAgentConfig): ACTAgentConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: ACTAgentConfig): ACTAgentConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run actagent plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function validateSuspiciousRecoveryBackup(parsed: unknown): boolean {
    try {
      ...
      const loadValidationPluginMetadataSnapshot = (config: ACTAgentConfig) => {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): ACTAgentConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as ACTAgentConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: ACTAgentConfig = {};
  ...
  async function readBestEffortConfigLocal(): Promise<ACTAgentConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffortLocal(): Promise<ACTAgentConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as ACTAgentConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as ACTAgentConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ACTAgentConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ACTAGENT_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.ACTAGENT_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ACTAGENT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.ACTAGENT_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: ACTAgentConfig) => {
...
// module scope. `ACTAGENT_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: ACTAgentConfig;
  candidate: ACTAgentConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: ACTAgentConfig): ACTAgentConfig {
...
export function loadConfig(options?: {
  ...
  // reparsing actagent.json on hot code paths.
...
export async function readSourceConfigBestEffort(): Promise<ACTAgentConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: ACTAgentConfig = nextCfg;

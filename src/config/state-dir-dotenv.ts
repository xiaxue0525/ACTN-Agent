...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, ACTAgent env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still ACTAgent-managed keys:
 ...
 * literal references for keys ACTAgent previously managed.
 */
...
/**
 * Read and parse `~/.actagent/.env` (or `$ACTAGENT_STATE_DIR/.env`), returning

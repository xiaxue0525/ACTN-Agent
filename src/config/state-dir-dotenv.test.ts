...
describe("readStateDirDotEnvVarsFromStateDir", () => {
  async function withDotEnv<T>(content: string, run: (dir: string) => T | Promise<T>): Promise<T> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-dotenv-test-"));
  ...
  it("skips values that are unresolved shell variable references", async () => {
    const content = [
      'SUPERMEMORY_ACTAGENT_API_KEY="${SUPERMEMORY_ACTAGENT_KEY}"',
      "QUOTED_SUPERMEMORY_ACTAGENT_API_KEY='\"$SUPERMEMORY_ACTAGENT_KEY\"'",
    ...
    await withDotEnv(content, async (dir) => {
      const result = readStateDirDotEnvVarsFromStateDir(dir);
      expect(Object.keys(result)).not.toContain("SUPERMEMORY_ACTAGENT_API_KEY");
      expect(Object.keys(result)).not.toContain("QUOTED_SUPERMEMORY_ACTAGENT_API_KEY");
  ...
  it("returns empty object when .env is missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-dotenv-missing-"));
    try {

...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.actagent) || !isRecord(entry.actagent.channel)) {
      continue;
    }
    const channel = entry.actagent.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: ACTAgentConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: ACTAgentConfig, pluginId: string) => boolean;

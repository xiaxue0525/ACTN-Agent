// Defines the top-level ACTAgent configuration type.
...
/** Top-level ACTAgent config as read from user/project config files. */
export type ACTAgentConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last ACTAgent version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** ACTAgent version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for ACTAgent UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type ACTAgentConfigInput = Omit<ACTAgentConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const actAgentConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = ACTAgentConfig & {
  readonly [actAgentConfigStateBrand]?: TState;
};

...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ACTAGENT_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-actagent",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-actagent#general");
  });
  ...
  it("derives session transcripts dir from ACTAGENT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ACTAGENT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ACTAGENT_STATE_DIR differs from stored paths", () => {

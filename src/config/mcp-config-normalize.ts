...
type ACTAgentMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_ACTAGENT_TRANSPORT: Record<string, ACTAgentMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to ACTAgent HTTP transport names. */
export function resolveACTAgentMcpTransportAlias(
  value: unknown,
): ACTAgentMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_ACTAGENT_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias ACTAgent can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_ACTAGENT_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveACTAgentMcpTransportAlias(next.type);
  // `transport` is ACTAgent's canonical field; legacy `type` only fills a gap.

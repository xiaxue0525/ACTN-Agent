...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: ACTAgentConfig): boolean {
...
function isProviderConfigured(cfg: ACTAgentConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: ACTAgentConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: ACTAgentConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: ACTAgentConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: ACTAgentConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: ACTAgentConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: ACTAgentConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: ACTAgentConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: ACTAgentConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: ACTAgentConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: ACTAgentConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: ACTAgentConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: ACTAgentConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: ACTAgentConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: ACTAgentConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: ACTAgentConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: ACTAgentConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: ACTAgentConfig): boolean {
  return (
...
function hasPluginEntries(cfg: ACTAgentConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: ACTAgentConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: ACTAgentConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: ACTAgentConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ACTAgentConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: ACTAgentConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: ACTAgentConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;

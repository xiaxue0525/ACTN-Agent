...
export type WideAreaDiscoveryConfig = {
  ...
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "actagent.internal"). */
  domain?: string;
};
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/actagent"). */
  basePath?: string;
   ...
   * Default off; prefer hosted /__actagent__/canvas or /__actagent__/a2ui content.
   */
 ...
 * Used when actagentdbot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayTailscaleConfig = {
  ...
  resetOnExit?: boolean;
  /** Optional Tailscale Service name, such as `svc:actagent`, for Serve mode. */
  serviceName?: string;
  /**
   ...
   * startup. Lets operators manage Funnel exposure outside ACTAgent without
...
export type GatewayRemoteConfig = {
  ...
  /** Gateway port on the remote SSH host. Defaults to 19199. */
  remotePort?: number;
...
export type GatewayReloadConfig = {
   ...
   * @see https://github.com/actagent/actagent/issues/65485
   */
...
export type GatewayConfig = {
  /** Single multiplexed port for Gateway WS + HTTP (default: 19199). */
  port?: number;
  /**
   ...
   * Env var ACTAGENT_HANDSHAKE_TIMEOUT_MS takes precedence. Default: 15000.
   */

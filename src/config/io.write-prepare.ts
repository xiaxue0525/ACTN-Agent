...
import { normalizeConfiguredProviderCatalogModelId } from "@actagent/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  actagent config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  actagent config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: ACTAgentConfig,
  pathSegments: string[],
): { changed: boolean; next: ACTAgentConfig } {

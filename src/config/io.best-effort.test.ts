...
import { withTempHome, writeACTAgentConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.actagent/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeACTAgentConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.actagent`);
      expect(entries.some((entry) => entry.startsWith("actagent.json.clobbered."))).toBe(false);
    });
  });

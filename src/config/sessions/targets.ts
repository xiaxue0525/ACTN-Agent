...
export function listConfiguredSessionStoreAgentIds(cfg: ACTAgentConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "actagent agents list" to see configured agents.`,
      );
    }
    return [
      {

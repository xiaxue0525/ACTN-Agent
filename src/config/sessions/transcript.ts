...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyACTAgentAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyACTAgentAssistant &&
    isTranscriptOnlyACTAgentAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlyACTAgentAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return (
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyACTAgentAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `actagent.cache-ttl` custom events) so
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "actagent" && message.model === "delivery-mirror";
}

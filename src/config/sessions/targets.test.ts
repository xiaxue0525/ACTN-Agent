...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): ACTAgentConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: ACTAgentConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        ACTAGENT_STATE_DIR: envStateDir,
      };
      const cfg: ACTAgentConfig = {};

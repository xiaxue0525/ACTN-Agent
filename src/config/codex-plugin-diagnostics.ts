...
function isOpenAiCodexDefaultRuntimeSelection(params: {
  ...
  // Custom OpenAI-compatible base URLs stay on the ACTAgent runtime path.
...
function codexPluginEntryEnabled(cfg: ACTAgentConfig): boolean | undefined {
...
function openAiProviderRuntimePolicy(cfg: ACTAgentConfig): AgentRuntimePolicyConfig | undefined {
...
function listConfiguredAgentIds(cfg: ACTAgentConfig): Array<string | undefined> {
...
function openAiHasCodexDefaultRuntimePolicy(cfg: ACTAgentConfig): boolean {
  for (const [providerId, providerConfig] of Object.entries(cfg.models?.providers ?? {})) {
    if (normalizeProviderId(providerId) !== OPENAI_PROVIDER_ID) {
      continue;
    }
    ...
    // A model-scoped explicit "auto"/"default" overrides provider-wide PI/ACTAgent
...
function openAiDefaultRouteKeepsCodexUnavailable(cfg: ACTAgentConfig): boolean {
  const policy = openAiDefaultRouteRuntimePolicy(cfg);
  if (!policy?.id?.trim()) {
    // With no explicit runtime policy, the OpenAI route only needs Codex on the
    // official OpenAI endpoint. OpenAI-compatible proxies stay on ACTAgent.
    return !openAIProviderUsesCodexRuntimeByDefault({
      provider: OPENAI_PROVIDER_ID,
      config: cfg,
    });
 ...
 */
export function configExplicitlyKeepsCodexUnavailableForOpenAi(cfg: ACTAgentConfig): boolean {
 ...
 * all default routes would otherwise stay on the ACTAgent runtime.
 */
export function shouldSuppressMissingCodexPluginDiagnostics(cfg: ACTAgentConfig): boolean {

// Validates normalized ACTAgent config and reports user-facing errors.
import path from "node:path";
import { collectConfiguredModelRefs } from "@actagent/model-catalog-core/configured-model-refs";
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@actagent/net-policy/ip";
...
import type { ACTAgentConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: ACTAgentConfig): ACTAgentConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.actagent.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  const installSpec =
    install?.defaultChoice === "actagenthub" ? (actagenthubSpec ?? npmSpec) : (npmSpec ?? actagenthubSpec);
  if (!installSpec) {
    return null;
  }
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: actagent plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: actagent plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: ACTAgentConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: ACTAgentConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: ACTAgentConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: ACTAgentConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: ACTAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ACTAgentSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as ACTAgentConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: ACTAgentConfig | null | undefined;
  ...
  const ensureCompatConfig = (): ACTAgentConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path: pathEntry,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run actagent doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or actagent doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run actagent doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run actagent doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native ACTAgent config schema.

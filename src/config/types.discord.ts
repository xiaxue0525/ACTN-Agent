...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the ACTAgent agent brain for every substantive turn. */
  ...
  /** Debounce window before buffered transcripts are sent to the ACTAgent agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which ACTAgent conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing ACTAgent Discord conversation. */

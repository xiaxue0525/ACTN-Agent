...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.ACTAGENT_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.ACTAGENT_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.ACTAGENT_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `actagent doctor --fix` once the

...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ACTAGENT_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveACTAgentPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ACTAGENT_STATE_DIR: path.join(os.tmpdir(), "actagent-config-doc-baseline-state"),

...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "actagent-web-search": {
                source: "npm",
                spec: "@ollama/actagent-web-search",
                installPath: "/tmp/actagent-web-search",
                resolvedName: "@ollama/actagent-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "actagent-web-search": {
                source: "npm",
                spec: "@ollama/actagent-web-search@0.2.2",
                installPath: "/tmp/actagent-web-search",
                resolvedName: "@ollama/actagent-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("actagent config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('actagent config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/actagent.json",
        expectedConfigPath: "/tmp/actagent.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: ACTAgentConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://actagent.ai/config-from-include.json",
      gateway: { mode: "local" },
    };

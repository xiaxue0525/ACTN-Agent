...
import type { ConfigFileSnapshot, ACTAgentConfig } from "./types.js";

type MockValidationIssue = { path: string; message: string };
type MockValidationResult =
  | { ok: true; config: ACTAgentConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: ACTAgentConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/actagent.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "actagent-config-mutate-" });
  const originalNixMode = process.env.ACTAGENT_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.ACTAGENT_NIX_MODE = originalNixMode;
    }
  ...
  it("mutates source config with optimistic hash protection", async () => {
    const snapshot = createSnapshot({
      hash: "source-hash",
      sourceConfig: { gateway: { port: 19199 } },
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("ACTAgent Nix overview: https://docs.actagent.ai/install/nix");
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".actagent", "config", "plugins.json5");
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "ACTAGENT_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const initialConfig = { gateway: { mode: "local" } } satisfies ACTAgentConfig;
    ...
    const injectedWrite = vi.fn(async (config: ACTAgentConfig, options?: ConfigWriteOptions) => {

...
const TEMP_MEDIA_PREFIX = "actagent-media-";
...
function createGroqAudioConfig(): ACTAgentConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ACTAgentConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ACTAgentConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      ACTAGENT_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ACTAgentConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredACTAgentTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: ACTAgentConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("actagent-antigravity-probe-"),
    });

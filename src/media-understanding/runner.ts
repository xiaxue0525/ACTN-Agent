...
import { mergeInboundPathRoots } from "@actagent/media-core/inbound-path-policy";
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.ACTAGENT_ANTIGRAVITY_CLI?.trim();

...
describe("runCapability image skip", () => {
  ...
  it("uses explicit media image models instead of native vision skip", async () => {
    await withMediaFixture(
      {
        filePrefix: "actagent-image-explicit-vision",
        extension: "png",
        mediaType: "image/png",
  ...
  it("lets per-request image prompts override entry prompts", async () => {
    await withMediaFixture(
      {
        filePrefix: "actagent-image-request-prompt",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },
  ...
  it("runs providerless configured imageModel fallbacks on the unique configured provider", async () => {
    await withMediaFixture(
      {
        filePrefix: "actagent-image-providerless-fallbacks",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },
  ...
  it("does not native-skip MiniMax chat models that claim image input", async () => {
    ...
    try {
      await withMediaFixture(
        {
          filePrefix: "actagent-minimax-vlm-no-native-skip",
          extension: "png",
          mediaType: "image/png",
          fileContents: Buffer.from("image"),
        },
  ...
  it("preserves MiniMax CN aliases from configured provider routing", async () => {
    ...
    try {
      await withMediaFixture(
        {
          filePrefix: "actagent-minimax-cn-provider",
          extension: "png",
          mediaType: "image/png",
          fileContents: Buffer.from("image"),
        },
  ...
  it("keeps MiniMax auto routing on VLM when registry lacks a default model", async () => {
    ...
    await withMediaFixture(
      {
        filePrefix: "actagent-minimax-vlm-default",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },
  ...
  it("keeps non-MiniMax media aliases canonical for image execution", async () => {
    ...
    await withMediaFixture(
      {
        filePrefix: "actagent-gemini-media-alias",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },
  ...
  it("passes workspaceDir to auto image provider auth checks", async () => {
    ...
    hasAvailableAuthForProvider.mockImplementation(
      async (params) => params.workspaceDir === "/tmp/actagent-workspace",
    );

    try {
      await withMediaFixture(
        {
          filePrefix: "actagent-image-workspace-auth",
          extension: "png",
          mediaType: "image/png",
          fileContents: Buffer.from("image"),
        },
  ...
  it("auto-selects configured OpenRouter image providers with a resolved model", async () => {
    ...
    await withMediaFixture(
      {
        filePrefix: "actagent-image-openrouter",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },
  ...
  it("skips configured image providers without an auto-resolvable model", async () => {
    await withMediaFixture(
      {
        filePrefix: "actagent-image-custom-skip",
        extension: "png",
        mediaType: "image/png",
        fileContents: Buffer.from("image"),
      },

// Creates isolated ACTAgent state directories for integration-style tests.
...
type ACTAgentTestStateLayout = "home" | "state-only" | "split";

type ACTAgentTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type ACTAgentTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: ACTAgentTestStateLayout;
  scenario?: ACTAgentTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "actagent-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "ACTAGENT_AGENT_DIR",
  "ACTAGENT_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "actagent.json"),
...
function scenarioConfig(options: ACTAgentTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        port: options.gateway?.port ?? 19199,
        bind: "loopback",
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "actagent-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: ACTAgentTestStateOptions): Record<string, string | undefined> {
...
export async function createACTAgentTestState(
  options: ACTAgentTestStateOptions = {},
): Promise<ACTAgentTestState> {
  ...
  const state: ACTAgentTestState = {
    ...
    applyEnv: () => {
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed ACTAgent env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withACTAgentTestState<T>(
  options: ACTAgentTestStateOptions,
  fn: (state: ACTAgentTestState) => Promise<T>,
): Promise<T> {
  const state = await createACTAgentTestState(options);
  try {

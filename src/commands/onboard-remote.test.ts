...
const INSECURE_WS_URL_MESSAGE =
  ...
  "Break-glass: ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks.";
...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["ACTAGENT_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("defaults discovered direct remote URLs to wss://", async () => {
    ...
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.initialValue).toBe("wss://gateway.tailnet.ts.net:19199");
    ...
    expect(next.gateway?.remote?.url).toBe("wss://gateway.tailnet.ts.net:19199");
    ...
    expect(prompter.note).toHaveBeenCalledWith(
      [
        ...
        "Using: wss://gateway.tailnet.ts.net:19199",
        "TLS pin: sha256:abc123",
        "If your gateway is loopback-only, choose SSH tunnel and keep ws://127.0.0.1:19199.",
      ].join("\n"),
      "Direct remote",
    );
  });
  ...
  it("falls back to manual URL entry when discovery trust is declined", async () => {
    ...
    const manualUrl = "wss://manual.example.com:19199";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.initialValue).toBe("ws://127.0.0.1:19199");
        return manualUrl;
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    const next = await promptRemoteGatewayConfig({} as ACTAgentConfig, prompter);
  ...
  it("trusts discovery endpoint without fingerprint and omits tlsFingerprint", async () => {
    ...
    expect(next.gateway?.remote?.url).toBe("wss://gw.example:19199");
    expect(next.gateway?.remote?.tlsFingerprint).toBeUndefined();
  });
  ...
  it("does not route from TXT-only discovery metadata", async () => {
    ...
    expect(next.gateway?.remote?.url).toBe("ws://127.0.0.1:19199");
  ...
  it("validates insecure ws:// remote URLs and allows trusted private ws:// by default", async () => {
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        ...
        expect(params.validate?.("ws://203.0.113.10:19199")).toBe(INSECURE_WS_URL_MESSAGE);
        // ws:// to trusted LAN/Tailnet endpoints is accepted.
        expect(params.validate?.("ws://10.0.0.8:19199")).toBeUndefined();
        expect(params.validate?.("ws://127.0.0.1:19199")).toBeUndefined();
        expect(params.validate?.("wss://remote.example.com:19199")).toBeUndefined();
        return "wss://remote.example.com:19199";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("wss://remote.example.com:19199");
    expect(next.gateway?.remote?.token).toBeUndefined();
  });

  it("allows ws:// hostname remote URLs when ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://actagent-gateway.ai:19199")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:19199")).toBe(INSECURE_WS_URL_MESSAGE);
        return "ws://actagent-gateway.ai:19199";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://actagent-gateway.ai:19199");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:19199";
      }
      if (params.message === "Environment variable name") {
        return "ACTAGENT_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];

...
function mockSnapshot(token: unknown = "abc") {
  ...
  resolveControlUiLinksMock.mockReturnValue({
    httpUrl: "http://127.0.0.1:19199/",
    wsUrl: "ws://127.0.0.1:19199",
  });
...
describe("dashboardCommand", () => {
  ...
  it("opens and copies the dashboard link by default", async () => {
    ...
    expect(copyToClipboardMock).toHaveBeenCalledWith("http://127.0.0.1:19199/#token=abc123");
    expect(openUrlMock).toHaveBeenCalledWith("http://127.0.0.1:19199/#token=abc123");
    expect(runtime.log).toHaveBeenCalledWith(
      "Opened in your browser. Keep that tab to control ACTAgent.",
    );
  });
  ...
  it("never logs the gateway token in the dashboard URL (CVE regression)", async () => {
    ...
    expect(copyToClipboardMock).toHaveBeenCalledWith(
      `http://127.0.0.1:19199/#token=${secretToken}`,
    );
    expect(openUrlMock).toHaveBeenCalledWith(`http://127.0.0.1:19199/#token=${secretToken}`);
    ...
    expect(runtime.log).toHaveBeenCalledWith("Dashboard URL: http://127.0.0.1:19199/");
  ...
  it("never passes token to SSH hint (CVE regression — SSH path)", async () => {
    ...
    expect(formatControlUiSshHintMock).toHaveBeenCalledWith({ port: 19199, basePath: undefined });
  ...
  it("guides user to manual auth when delivery channels both fail (CVE-safe)", async () => {
    ...
    expect(allLogs).toMatch(/ACTAGENT_GATEWAY_TOKEN/);
  ...
  it("respects --no-open and falls through to manual-auth hint when clipboard fails (token configured)", async () => {
    ...
    expectLogWith("ACTAGENT_GATEWAY_TOKEN");
  });
  ...
  it("prints non-tokenized URL with guidance when token SecretRef is unresolved", async () => {
    ...
    expect(copyToClipboardMock).toHaveBeenCalledWith("http://127.0.0.1:19199/");
  ...
  it("keeps URL non-tokenized when token SecretRef is unresolved but env fallback exists", async () => {
    ...
    expect(copyToClipboardMock).toHaveBeenCalledWith("http://127.0.0.1:19199/");
    expect(openUrlMock).toHaveBeenCalledWith("http://127.0.0.1:19199/");

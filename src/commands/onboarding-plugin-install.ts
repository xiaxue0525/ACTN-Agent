 ...
 *
 * It selects local, ACTAgentHub, npm, or override install sources; records durable
...
import { isACTAgentOrgNpmSpec, parseRegistryNpmSpec } from "../infra/npm-registry-spec.js";
import { normalizeUpdateChannel, resolveRegistryUpdateChannel } from "../infra/update-channels.js";
import {
  findBundledPluginSourceInMap,
  resolveBundledPluginSources,
} from "../plugins/bundled-sources.js";
import { ACTAGENTHUB_INSTALL_ERROR_CODE } from "../plugins/actagenthub-error-codes.js";
...
type InstallChoice = "actagenthub" | "npm" | "local" | "skip";
type InstallPluginFromACTAgentHubResult = Awaited<
  ReturnType<(typeof import("../plugins/actagenthub.js"))["installPluginFromACTAgentHub"]>
>;
...
function shouldFallbackACTAgentHubToNpm(params: {
  ...
  // Only official ACTAgent npm packages are safe fallback targets for ACTAgentHub
...
function addPluginLoadPath(cfg: ACTAgentConfig, pluginPath: string): ACTAgentConfig {
...
function resolveACTAgentHubSpecForOnboarding(install: PluginPackageInstall): string | null {
  const actagenthubSpec = install.actagenthubSpec?.trim();
  if (!actagenthubSpec) {
    return null;
  }
  const parsed = parseACTAgentHubPluginSpec(actagenthubSpec);
  return parsed ? actagenthubSpec : null;
}
...
function resolveInstallDefaultChoice(params: {
  ...
  hasACTAgentHubSpec: boolean;
  hasNpmSpec: boolean;
}): InstallChoice {
  const { cfg, entry, localPath, bundledLocalPath, hasACTAgentHubSpec, hasNpmSpec } = params;
  const hasRemoteSpec = hasACTAgentHubSpec || hasNpmSpec;
  const entryDefault = entry.install.defaultChoice;
  const remoteDefault = (): InstallChoice => {
    if (entryDefault === "actagenthub" && hasACTAgentHubSpec) {
      return "actagenthub";
    }
    if (entryDefault === "npm" && hasNpmSpec) {
      return "npm";
    }
    return hasNpmSpec ? "npm" : "actagenthub";
  };
  if (!hasRemoteSpec) {
...
async function promptInstallChoice(params: {
  ...
  effectiveACTAgentHubSpec?: string | null;
}): Promise<InstallChoice> {
  const rawACTAgentHubSpec = resolveACTAgentHubSpecForOnboarding(params.entry.install);
  ...
  const actagenthubSpec = params.bundledLocalPath
    ? null
    : (params.effectiveACTAgentHubSpec ?? rawACTAgentHubSpec);
  const npmSpec = params.bundledLocalPath ? null : (params.effectiveNpmSpec ?? rawNpmSpec);
  const safeLabel = sanitizeTerminalText(params.entry.label);
  const safeACTAgentHubSpec = actagenthubSpec ? sanitizeTerminalText(actagenthubSpec) : null;
  ...
  if (safeACTAgentHubSpec) {
    options.push({
      value: "actagenthub",
      label: t("wizard.plugins.downloadFromACTAgentHub", { spec: safeACTAgentHubSpec }),
    });
  }
  if (safeNpmSpec) {
    options.push({
    ...
    params.defaultChoice === "local" && !params.localPath
      ? actagenthubSpec
        ...
        : npmSpec
          ? "npm"
          : "skip"
      : params.defaultChoice === "actagenthub" && !actagenthubSpec
        ? npmSpec
          ? "npm"
          : params.localPath
            ? "local"
 ...
 * ACTAgentHub…`, `Extracting /tmp/…/wecom-…-2026.4.23.tgz…`, `Installing to
...
async function installPluginFromACTAgentHubSpecWithProgress(params: {
  entry: OnboardingPluginInstallEntry;
  ...
  | {
      status: "completed";
      result: InstallPluginFromACTAgentHubResult;
    }
> {
  const safeLabel = sanitizeTerminalText(params.entry.label);
  const progress = params.prompter.progress(formatPluginInstallProgress(safeLabel));
  ...
  try {
    const { installPluginFromACTAgentHub } = await import("../plugins/actagenthub.js");
    const result = await withTimeout(
      installPluginFromACTAgentHub({
        spec: params.actagenthubSpec,
        timeoutMs: ONBOARDING_PLUGIN_INSTALL_TIMEOUT_MS,
        extensionsDir: resolveDefaultPluginExtensionsDir(),
        expectedPluginId: params.entry.pluginId,
        mode: "install",
...
export async function ensureOnboardingPluginInstalled(params: {
  ...
  const actagenthubSpec = resolveACTAgentHubSpecForOnboarding(entry.install);
  ...
  const actagenthubSpecs = actagenthubSpec
    ? resolveACTAgentHubInstallSpecsForUpdateChannel({
  ...
  const npmSpecs = npmSpec
    ...
    : null;
  const actagenthubInstallSpec = actagenthubSpecs?.installSpec ?? actagenthubSpec;
  ...
  const defaultChoice = resolveInstallDefaultChoice({
    ...
    localPath,
    bundledLocalPath,
    hasACTAgentHubSpec: Boolean(actagenthubSpec),
  ...
  const choice =
    params.promptInstall === false
      ...
      : await promptInstallChoice({
          ...
          effectiveACTAgentHubSpec: actagenthubInstallSpec,
  ...
  if (choice === "actagenthub" && actagenthubInstallSpec) {
    const installOutcome = await installPluginFromACTAgentHubSpecWithProgress({
      entry,
      actagenthubSpec: actagenthubInstallSpec,
      prompter,
      runtime,
    });
    ...
    if (installOutcome.status === "timed_out") {
      await prompter.note(
        formatPluginInstallTimedOutNote(sanitizeTerminalText(actagenthubInstallSpec)),
        t("wizard.plugins.installTitle"),
      );
      runtime.error?.(
        `Plugin install timed out after ${ONBOARDING_PLUGIN_INSTALL_TIMEOUT_MS}ms: ${sanitizeTerminalText(actagenthubInstallSpec)}`,
      );
      return {
        cfg: next,
        installed: false,
    ...
    if (result.ok) {
      ...
      next = recordPluginInstall(next, {
        pluginId: result.pluginId,
        ...buildACTAgentHubPluginInstallRecordFields(result.actagenthub),
    ...
    await prompter.note(
      [
        t("wizard.plugins.installFailed", {
          spec: sanitizeTerminalText(actagenthubInstallSpec),
          error: summarizeInstallError(result.error),
        }),
        t("wizard.plugins.returningToSelection"),
      ].join("\n"),
    ...
    if (!npmInstallSpec || !shouldFallbackACTAgentHubToNpm({ result, npmSpec: npmInstallSpec })) {
      runtime.error?.(`Plugin install failed: ${sanitizeTerminalText(result.error)}`);
      return {
        cfg: next,
        installed: false,
    ...
    // ACTAgentHub package/version misses for official packages can recover through

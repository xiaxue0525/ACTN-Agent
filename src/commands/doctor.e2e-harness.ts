...
export const resolveACTAgentPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
export const buildGatewayInstallPlan = vi.mocked(
  vi.fn().mockResolvedValue({
    programArguments: ["node", "cli", "gateway", "--port", "19199"],
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadACTAgentPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("actagent/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("actagent/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("actagent/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/actagent-root.js", () => ({
  ...
  resolveACTAgentPackageRootSync: vi.fn(() => "/tmp/actagent"),
}));
...
beforeEach(() => {
  ...
  resolveACTAgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ACTAGENT_STATE_DIR;
  originalUpdateInProgress = process.env.ACTAGENT_UPDATE_IN_PROGRESS;
  process.env.ACTAGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-doctor-state-"));

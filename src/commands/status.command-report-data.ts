// Builds the data model for the standard `actagent status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("actagent security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("actagent status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("actagent tasks maintenance --apply")}`,

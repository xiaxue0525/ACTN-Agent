...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
      return "Fix: run `actagent doctor --fix`.";
    case "module-missing":
      return "Fix: run `actagent doctor --fix`.";
    case "service-missing":
      return "Fix: run `actagent gateway install --force`.";
    case "service-stopped":
      return "Fix: run `actagent gateway restart`.";
    case "startup-blocked":
      return "Fix: run `actagent gateway status --deep`.";
    case "not-listening":
      return "Fix: start `actagent gateway run`, or run `actagent gateway restart` for a managed gateway.";
    default:
      return undefined;
  }
}

...
export async function loadValidConfigOrThrow(): Promise<ACTAgentConfig> {
...
export async function updateConfig(
  mutator: (cfg: ACTAgentConfig, context: UpdateConfigContext) => ACTAgentConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ACTAgentConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ACTAgentConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("actagent agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

...
const TEST_GATEWAY_URL = "ws://127.0.0.1:19199";
...
describe("checkGatewayHealth", () => {
  ...
  it("uses a lightweight status RPC for the restart liveness gate", async () => {
    ...
    expect(note.mock.calls.map(([, title]) => title)).not.toContain("ACTAgent version mismatch");
  });
  ...
  it("notes CLI and gateway version mismatch when the gateway reports another runtime version", async () => {
    ...
    const mismatchNotes = note.mock.calls
      .filter(([, title]) => title === "ACTAgent version mismatch")
      .map(([message]) => String(message));
    const mismatchOutput = mismatchNotes.join("\n");
    expect(mismatchOutput).toContain("the running Gateway is ACTAgent 2026.4.23");
    expect(mismatchOutput).not.toContain("That usually means");
    expect(mismatchOutput).toContain("Check `actagent --version`, `which actagent`");
  ...
  it("propagates checked: false and skipped: true when gateway skipped the embedding probe", async () => {
    ...
    callGateway.mockResolvedValue({
      embedding: {
        ...
        checked: false,
        error:
          "memory embedding readiness not checked; run `actagent memory status --deep` to probe",
      },
    });

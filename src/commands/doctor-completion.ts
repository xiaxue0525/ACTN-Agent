...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(actagent completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `actagent doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`actagent completion --write-state\` or \`actagent doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
...
export async function ensureCompletionCacheExists(binName = "actagent"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);

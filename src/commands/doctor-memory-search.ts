...
import {
  ...
  normalizeProviderId,
} from "@actagent/model-catalog-core/provider-id";
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: ACTAgentConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: ACTAgentConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("actagent doctor --fix")} or ${formatCliCommand("actagent memory status --fix")}`
    : `Verify: ${formatCliCommand("actagent memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: ACTAgentConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: ACTAgentConfig): boolean {
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("actagent config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("actagent config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("actagent memory status --deep")}`,
        ]
          .filter(Boolean)
    ...
    note(
      [
        ...
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`actagent config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("actagent config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("actagent memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("actagent config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("actagent memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `actagent doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("actagent configure --section model")}`,
      `- To disable: ${formatCliCommand("actagent config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("actagent memory status --deep")}`,
    ].join("\n"),
    "Memory search",

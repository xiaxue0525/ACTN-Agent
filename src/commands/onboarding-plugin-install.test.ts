...
const installPluginFromACTAgentHub = vi.hoisted(() => vi.fn());
...
const enablePluginInConfig = vi.hoisted(() =>
  vi.fn<(cfg: ACTAgentConfig, pluginId: string) => PluginEnableResult>((cfg, pluginId) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })),
...
const recordPluginInstall = vi.hoisted(() =>
  vi.fn((cfg: ACTAgentConfig, update: { pluginId: string }) => ({
    ...cfg,
    plugins: {
      ...cfg.plugins,
      installs: {
...
describe("ensureOnboardingPluginInstalled", () => {
  ...
  it("localizes plugin install choices", async () => {
    ...
    let captured:
      | {
          message: string;
          options: Array<{
            value: "actagenthub" | "npm" | "local" | "skip";
            label: string;
            hint?: string;
          }>;
        }
    ...
    try {
      ...
      expect(captured?.options).toEqual([
        { value: "npm", label: "从 npm 下载（@actagent/qqbot@beta）" },
  ...
  it("refuses non-skipped installs in Nix mode before package work", async () => {
    const previous = process.env.ACTAGENT_NIX_MODE;
    ...
    try {
      await expect(
        ensureOnboardingPluginInstalled({
          ...
          entry: {
            ...
            label: "Demo Provider",
            install: {
              npmSpec: "@actagent/demo-plugin@1.2.3",
            },
          },
          promptInstall: false,
          prompter: {
      ...
      ).rejects.toThrow("ACTAGENT_NIX_MODE=1");
    } finally {
      if (previous === undefined) {
        delete process.env.ACTAGENT_NIX_MODE;
      } else {
        process.env.ACTAGENT_NIX_MODE = previous;
      }
    }
  ...
  it("uses a guarded npm-pack install override for the matching plugin id", async () => {
    ...
    process.env.ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES = "1";
    process.env.ACTAGENT_PLUGIN_INSTALL_OVERRIDES = JSON.stringify({
    ...
    installPluginFromNpmPackArchive.mockResolvedValue({
      ok: true,
      pluginId: "demo-plugin",
      targetDir: "/tmp/actagent/extensions/demo-plugin",
      version: "1.2.3",
    ...
    expect(recordUpdate).toEqual({
      ...
      installPath: "/tmp/actagent/extensions/demo-plugin",
      version: "1.2.3",
  ...
  it("uses a guarded npm install override without official-trust flags", async () => {
    process.env.ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES = "1";
    process.env.ACTAGENT_PLUGIN_INSTALL_OVERRIDES = JSON.stringify({
      codex: "npm:@actagent/codex@2026.5.8",
      "other-plugin": "npm-pack:/tmp/other.tgz",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/actagent/extensions/codex",
      version: "2026.5.8",
      npmResolution: {
        name: "@actagent/codex",
        version: "2026.5.8",
        resolvedSpec: "@actagent/codex@2026.5.8",
      },
    });
    ...
    expect(npmCall.spec).toBe("@actagent/codex@2026.5.8");
    expect(npmCall.expectedPluginId).toBe("codex");
  });

  it("installs and records ACTAgentHub provider plugins with source facts", async () => {
    installPluginFromACTAgentHub.mockImplementation(async (params) => {
      params.logger?.info?.("Downloading demo-plugin from ACTAgentHub…");
      return {
        ok: true,
        pluginId: "demo-plugin",
        targetDir: "/tmp/demo-plugin",
        ...
        actagenthub: {
          ...
          actagenthubPackage: "demo-plugin",
    ...
    const result = await ensureOnboardingPluginInstalled({
      ...
      entry: {
        ...
        label: "Demo Provider",
        install: {
          actagenthubSpec: "actagenthub:demo-plugin@2026.5.2",
          npmSpec: "@actagent/demo-plugin@2026.5.2",
          defaultChoice: "actagenthub",
        },
      },
      prompter: {
        select: vi.fn(async () => "actagenthub"),
        progress: vi.fn(() => ({ update, stop })),
      } as never,
      runtime: {} as never,
    });

    const [actAgentHubCall] = readFirstMockCall(
      installPluginFromACTAgentHub,
      "installPluginFromACTAgentHub",
    ) as [ACTAgentHubInstallCall];
    expect(actAgentHubCall.spec).toBe("actagenthub:demo-plugin@2026.5.2");
    expect(actAgentHubCall.expectedPluginId).toBe("demo-plugin");
    expect(actAgentHubCall.mode).toBe("install");
    expect(actAgentHubCall.timeoutMs).toBe(300_000);
    ...
    expect(recordUpdate.source).toBe("actagenthub");
    expect(recordUpdate.spec).toBe("actagenthub:demo-plugin@2026.5.2");
    expect(recordUpdate.installPath).toBe("/tmp/demo-plugin");
    expect(recordUpdate.version).toBe("2026.5.2");
    expect(recordUpdate.integrity).toBe("sha256-actagentpack");
    expect(recordUpdate.actagenthubPackage).toBe("demo-plugin");
    expect(recordUpdate.actagentpackSize).toBe(4096);
    ...
    expect(installed?.source).toBe("actagenthub");
    expect(installed?.spec).toBe("actagenthub:demo-plugin@2026.5.2");
  });
  ...
  it("passes npm specs and optional expected integrity to npm installs with progress", async () => {
    const npmResolution = {
      ...
      version: "1.2.3",
      resolvedSpec: "@wecom/wecom-actagent-plugin@1.2.3",
    ...
    expect(npmCall.spec).toBe("@wecom/wecom-actagent-plugin@1.2.3");
    ...
    expect(recordUpdate.spec).toBe("@wecom/wecom-actagent-plugin@1.2.3");
    expect(recordUpdate.installPath).toBe("/tmp/demo-plugin");
    ...
    expect(installed?.spec).toBe("@wecom/wecom-actagent-plugin@1.2.3");
  ...
  it("offers registry npm specs without requiring an exact version or integrity pin", async () => {
    let captured:
      | {
          ...
          initialValue: "actagenthub" | "npm" | "local" | "skip";
        }
      | undefined;
  ...
  it("defaults dual-source remote installs to npm unless ACTAgentHub is explicit", async () => {
    let captured:
      | {
          options: Array<{
            value: "actagenthub" | "npm" | "local" | "skip";
    ...
    expect(captured?.options).toEqual([
      { value: "actagenthub", label: "Download from ACTAgentHub (actagenthub:demo-plugin@2026.5.2)" },
      { value: "npm", label: "Download from npm (@actagent/demo-plugin@2026.5.2)" },
      { value: "skip", label: "Skip for now" },
    ]);
    expect(captured?.initialValue).toBe("npm");
    expect(installPluginFromACTAgentHub).not.toHaveBeenCalled();
  ...
  it("honors explicit ACTAgentHub defaults for dual-source remote installs", async () => {
    let captured:
      | {
          initialValue: "actagenthub" | "npm" | "local" | "skip";
        }
    ...
    expect(captured?.initialValue).toBe("actagenthub");
  });

  it("falls back from ACTAgentHub to npm when the ACTAgentHub artifact is unavailable", async () => {
    installPluginFromACTAgentHub.mockResolvedValueOnce({
      ok: false,
    ...
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.2",
        resolvedSpec: "@actagent/demo-plugin@2026.5.2",
    ...
    expect(npmCall.spec).toBe("@actagent/demo-plugin@2026.5.2");
    expect(npmCall.expectedPluginId).toBe("demo-plugin");
    expect(result.installed).toBe(true);
  });
  ...
  it("does not fall back from ACTAgentHub to non-ACTAgent npm packages", async () => {
    ...
    expect(runtimeError).toHaveBeenCalledWith(
      "Plugin install failed: ACTAgentHub actagentpack artifact is unavailable.",
    );
  ...
  it("does not fall back from ACTAgentHub to npm when ACTAgentHub verification fails", async () => {
    const confirm = vi.fn(async () => true);
    const runtimeError = vi.fn();
    installPluginFromACTAgentHub.mockResolvedValueOnce({
      ok: false,
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeError).toHaveBeenCalledWith(
      "Plugin install failed: ACTAgentHub actagentpack integrity mismatch.",
    );
    expect(result).toEqual({
      cfg: {},
      installed: false,
  ...
  it("does not offer local installs when the workspace only has a spoofed .git marker", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-spoofed-git-" }, async (temp) => {
  ...
  it("allows local installs for real gitdir checkouts and sanitizes prompt text", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-gitdir-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("does not add local plugin paths when enablement is blocked by policy", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-blocked-enable-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("allows local installs for linked git worktrees", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-worktree-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
  ...
  it("records local install source metadata when a local path is selected", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-local-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
      ...
      const [recordCfg, recordUpdate] = readFirstMockCall(
        recordPluginInstall,
        "recordPluginInstall",
      ) as [ACTAgentConfig, PluginInstallRecord];
  ...
  it("hides the npm download option for bundled plugins so the menu matches non-npm channels", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-bundled-prompt-" }, async (temp) => {
      ...
      const prompt = requireCapturedPrompt(captured);
      // "Download from npm (@actagent/tlon)" must NOT appear: the bundled
  ...
  it("enables bundled plugins without adding their bundled directory as a local install", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-bundled-record-" }, async (temp) => {
  ...
  it("records local install source metadata when npm install falls back to local", async () => {
    await withTempDir(
      { prefix: "actagent-onboarding-install-npm-fallback-record-" },
      async (temp) => {
        const workspaceDir = path.join(temp, "workspace");
        const pluginDir = path.join(workspaceDir, "plugins", "demo");
        await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("records absolute local catalog paths as workspace-relative source metadata", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-portable-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("keeps local installs available when cwd is a git repo but workspaceDir is not", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-cwd-git-" }, async (temp) => {
  ...
  it("rejects local install paths outside the trusted workspace roots", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-outside-root-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
  ...
  it("rejects local install paths when relative resolution looks cross-drive", async () => {
    await withTempDir({ prefix: "actagent-onboarding-install-cross-drive-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });

...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): ACTAgentConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): ACTAgentConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/actagent/sandbox/containers.json",
        shardedDir: "/tmp/actagent/sandbox/containers",
        exists: true,
        valid: true,
        entries: 2,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers: /tmp/actagent/sandbox/containers.json (2 entries)",
        "Run actagent doctor --fix to migrate them to sharded registry files.",
      ].join("\n"),
      "Sandbox",
    );
  });
  ...
  it("migrates legacy registry files during doctor --fix", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      "- Migrated containers registry from /tmp/actagent/sandbox/containers.json into 2 shards.",
      "Doctor changes",
    );
  });
});

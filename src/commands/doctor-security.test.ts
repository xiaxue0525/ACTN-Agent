...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.ACTAGENT_GATEWAY_TOKEN;
    prevPassword = process.env.ACTAGENT_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.ACTAGENT_SERVICE_KIND;
    delete process.env.ACTAGENT_GATEWAY_TOKEN;
    delete process.env.ACTAGENT_GATEWAY_PASSWORD;
    delete process.env.ACTAGENT_SERVICE_KIND;
  });
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.ACTAGENT_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.ACTAGENT_GATEWAY_PASSWORD;
    } else {
      process.env.ACTAGENT_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.ACTAGENT_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".actagent"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".actagent", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("CRITICAL");
    expect(message).toContain("without authentication");
    expect(message).toContain("Safer remote access");
    expect(message).toContain("ssh -N -L 19199:127.0.0.1:19199");
  });

  it("uses env token to avoid critical warning", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when ACTAGENT_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("ACTAGENT_GATEWAY_TOKEN conflicts with gateway.auth.token");
    expect(message).toContain("Direct local Gateway clients commonly prefer the env token");
    expect(message).toContain("~/.actagent/.env");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("ACTAGENT_GATEWAY_TOKEN overrides");
  });
  ...
  it("does not warn inside the managed gateway service credential context", async () => {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("ACTAGENT_GATEWAY_TOKEN conflicts");
  });

  it("does not warn when config token uses ACTAGENT_GATEWAY_TOKEN SecretRef", async () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as ACTAgentConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("actagent approvals get --gateway");
  });
  ...
  it("warns when model provider API keys are stored as plaintext in config", async () => {
    ...
    expect(message).toContain("actagent secrets audit --check");
  });
  ...
  it("warns when normalized tools.exec mode is broader than host exec defaults", async () => {
    ...
    expect(message).toContain('defaults.security="allowlist"');
    expect(message).not.toContain("ACTAgent default");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as ACTAgentConfig);
    ...
    expect(message).toContain("Run: actagent security audit --deep");
  });

...
import { bundledPluginRoot, bundledPluginRootAt } from "actagent/plugin-sdk/test-fixtures";
...
vi.mock("../../plugins/loader.js", () => ({
  loadACTAgentPlugins: vi.fn(),
}));

const discoverACTAgentPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverACTAgentPlugins: (args: unknown) => discoverACTAgentPlugins(args),
}));

import fs from "node:fs";
import type { ChannelPluginCatalogEntry } from "../../channels/plugins/catalog.js";
import type { ACTAgentConfig } from "../../config/config.js";
import { loadACTAgentPlugins } from "../../plugins/loader.js";
...
const bundledChatNpmSpec = "@actagent/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/actagent", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/actagent-test/${id}`,
    source: `/tmp/actagent-test/${id}/index.ts`,
    manifestPath: `/tmp/actagent-test/${id}/actagent.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadACTAgentPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadACTAgentPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverACTAgentPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: ACTAgentConfig = { update: { channel } };
...
function expectLoadACTAgentPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadACTAgentPlugins), callIndex),
    "loadACTAgentPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: ACTAgentConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/actagent-ledger-channel";
    process.env.ACTAGENT_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: ACTAgentConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      ...
      npmResolution: {
        name: "@actagent/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@actagent/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(requireMockCallArg(installPluginFromNpmSpec, 0), "npm install args", {
      spec: "@actagent/wecom@beta",
      expectedPluginId: "wecom-actagent-plugin",
    });
    expect(result.cfg.plugins?.installs?.["wecom-actagent-plugin"]?.spec).toBe("@actagent/wecom");
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/actagent", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ACTAgentHub as the first-class install source for channel catalog entries", async () => {
    const runtime = makeRuntime();
    const { prompter, select } = makeSkipInstallPrompter();
    const cfg: ACTAgentConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        id: "actagenthub-chat",
        pluginId: "actagenthub-chat",
        meta: {
          id: "actagenthub-chat",
          label: "ACTAgentHub Chat",
          selectionLabel: "ACTAgentHub Chat",
          docsPath: "/channels/actagenthub-chat",
          blurb: "Test",
        },
        install: {
          actagenthubSpec: "actagenthub:actagent/actagenthub-chat@2026.5.2",
    ...
    const selectArgs = requireMockCallArg(select, 0);
    expect(selectArgs.initialValue).toBe("actagenthub");
    const options = requireSelectOptions(select);
    expect(options).toHaveLength(2);
    expectRecordFields(options[0], "actagenthub option", {
      value: "actagenthub",
      label: "Download from ACTAgentHub (actagenthub:actagent/actagenthub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@actagent/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-actagent-plugin");
  });
  ...
  it("reloads the setup plugin registry without using plugin registry cache", () => {
    ...
    expectLoadACTAgentPluginFields({
      config: cfg,
  ...
  it("scopes snapshots by activation-declared channel ownership when direct channel lists are empty", () => {
    ...
    loadPluginManifestRegistry.mockImplementation((args: unknown) => {
      if (
        isRecord(args) &&
        args.config === cfg &&
        args.workspaceDir === "/tmp/actagent-workspace" &&

...
const DEFAULT_GATEWAY_URL = "ws://127.0.0.1:19199";
...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.ACTAGENT_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.actagent.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            `ssh -N -L 19199:127.0.0.1:19199 <user>@${host}${target.sshPort ? ` -p ${target.sshPort}` : ""}`,

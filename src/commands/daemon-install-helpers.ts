...
import {
  ACTAGENT_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveACTAgentWrapperPath,
...
// they share the ACTAGENT_ prefix that is otherwise stripped from preserved
...
const PRESERVED_ACTAGENT_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "ACTAGENT_CLI_CONTAINER_BYPASS",
  "ACTAGENT_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("ACTAGENT_") && !PRESERVED_ACTAGENT_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[ACTAGENT_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${ACTAGENT_WRAPPER_ENV_KEY} because it points to the Windows task script; using the ACTAgent gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveACTAgentWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [ACTAGENT_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, ACTAGENT_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.ACTAGENT_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("actagent gateway install")}\` after fixing the error.`;
}

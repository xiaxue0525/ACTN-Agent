...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.actagent.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.actagent.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        ACTAGENT_STATE_DIR: "/tmp/actagent-service",
        ACTAGENT_CONFIG_PATH: "/tmp/actagent-service/actagent.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { ACTAGENT_STATE_DIR?: string; ACTAGENT_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.ACTAGENT_STATE_DIR).toBe("/tmp/actagent-service");
    expect(handoffEnv?.ACTAGENT_CONFIG_PATH).toBe("/tmp/actagent-service/actagent.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-actagent/bin/actagent logs --follow",
          address: "TCP 127.0.0.1:50123->127.0.0.1:19199 (ESTABLISHED)",
          direction: "client",
        },
      ],
    });
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "actagent-gateway", address: "0.0.0.0:19199" }];
    inspectPortUsage.mockResolvedValue({
      port: 19199,
      status: "busy",
      listeners,
    ...
    expect(isExpectedGatewayListeners).toHaveBeenCalledWith(listeners, 19199);
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "actagent-gateway", address: "0.0.0.0:19199" },
        { pid: 5002, commandLine: "actagent-gateway", address: "127.0.0.1:19199" },
      ],
    ...
    expect(note).toHaveBeenCalledWith("Port 19199 is already in use.", "Gateway port");
  });
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("actagent gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system ACTAgent gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "actagent-gateway.service",
        detail: "unit: /etc/systemd/system/actagent-gateway.service",
        scope: "system",
        marker: "actagent",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level ACTAgent gateway service detected while the user gateway service is not installed.",
        "- actagent-gateway.service (unit: /etc/systemd/system/actagent-gateway.service)",
        "ACTAgent will not install a second user-level gateway service automatically.",
        "Run `actagent gateway status --deep` or `actagent doctor --deep` to inspect duplicate services.",

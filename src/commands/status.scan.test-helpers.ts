...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/actagent-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = ACTAgentConfig>(
  overrides: T = {} as T,
): ACTAgentConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): ACTAgentConfig {

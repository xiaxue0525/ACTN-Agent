// Implements `actagent dashboard` URL resolution, readiness check, clipboard, and browser launch.
...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set ACTAGENT_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `actagent dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control ACTAgent.");
  ...
  if (fallbackToManualAuth) {
    runtime.log(
      "Token auto-auth not delivered. Append your gateway token (from ACTAGENT_GATEWAY_TOKEN or gateway.auth.token) as a URL fragment with key `token` to authenticate.",
    );
  }
}

...
function isACTAgentCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "actagent") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "actagent.cmd" || basename === "actagent.ps1" || basename === "actagent.exe"
    );
  }
  return false;
}
...
/** Resolve the ACTAgent CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonACTAgentBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isACTAgentCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "actagent.cmd" : "actagent");
...
/** Merge Node and ACTAgent binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonACTAgentBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}

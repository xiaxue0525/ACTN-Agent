...
describe("buildCleanupPlan", () => {
  test("resolves inside-state flags and workspace dirs", () => {
    ...
    const defaultWorkspace = path.join(tmpRoot, "actagent-workspace-default");
    const opsWorkspace = path.join(tmpRoot, "actagent-workspace-ops");
    const cfg = {
      agents: {
    ...
    const plan = buildCleanupPlan({
      cfg: cfg as unknown as ACTAgentConfig,
      stateDir: path.join(tmpRoot, "actagent-state"),
      configPath: path.join(tmpRoot, "actagent-state", "actagent.json"),
      oauthDir: path.join(tmpRoot, "actagent-oauth"),
    });
  ...
  test("includes implicit per-agent workspaces under the state dir", () => {
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "actagent-cleanup-plan");
    ...
    return withEnvAsync(
      {
        ...
        ACTAGENT_WORKSPACE_DIR: undefined,
      },
      async () => {
        const plan = buildCleanupPlan({
          cfg: cfg as unknown as ACTAgentConfig,
          stateDir,
          configPath: path.join(stateDir, "actagent.json"),
...
describe("applyAgentDefaultPrimaryModel", () => {
  it("does not mutate when already set", () => {
    const cfg = { agents: { defaults: { model: { primary: "a/b" } } } } as ACTAgentConfig;
  ...
  it("normalizes legacy models", () => {
    const cfg = { agents: { defaults: { model: { primary: "legacy" } } } } as ACTAgentConfig;
  ...
  it("normalizes retired Google Gemini primary models before writing config", () => {
    const cfg = { agents: { defaults: {} } } as ACTAgentConfig;
    const result = applyAgentDefaultPrimaryModel({
      cfg,
...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "actagent-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "actagent.json"),
    ...
    expect(runtime.log.mock.calls.map(([line]) => line.replaceAll("\\", "/"))).toEqual([
      "[dry-run] remove /tmp/actagent-cleanup/state",
      "[dry-run] remove /tmp/actagent-cleanup/oauth",
    ]);
  });

  it("preserves nested workspace paths during state-only removal", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cleanup-"));
    const stateDir = path.join(tmpRoot, ".actagent");
  ...
  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/actagent-workspace-1", "/tmp/actagent-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toEqual([
      "[dry-run] remove /tmp/actagent-workspace-1",
      "[dry-run] remove /tmp/actagent-workspace-2",
    ]);
  });

  it("removes owned legacy workspace attestations", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cleanup-attest-"));

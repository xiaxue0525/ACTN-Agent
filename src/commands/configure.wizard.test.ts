...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: ACTAgentConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: ACTAgentConfig }) =>
      Boolean(config.auth?.profiles?.["openai:default"]),
    ),
    setupChannels: vi.fn(async (cfg: ACTAgentConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.actagent/actagent.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.actagent/workspace",
  applyWizardMetadata: (cfg: ACTAgentConfig) => cfg,
...
function setupBaseWizardState(config: ACTAgentConfig = {}) {
  ...
  mocks.resolveControlUiLinks.mockReturnValue({ wsUrl: "ws://127.0.0.1:19199" });
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: ACTAgentConfig) => cfg);
    mocks.promptAuthConfig.mockReset();
    mocks.promptAuthConfig.mockImplementation(async (cfg: ACTAgentConfig) => cfg);
    mocks.promptGatewayConfig.mockReset();
    mocks.promptGatewayConfig.mockImplementation(async (cfg: ACTAgentConfig) => ({
      config: cfg,
      port: 19199,
    }));
  });
  ...
  it("keeps startup gateway hint probes bounded", async () => {
    ...
    const localProbe = probeRequests.find((request) => request.url === "ws://127.0.0.1:19199");
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: ACTAgentConfig) =>
  ...
  it("notes unavailable web search providers under plugin policy", async () => {
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      [
        ...
        "Docs: https://docs.actagent.ai/tools/web",
      ].join("\n"),
      "Web search",
    );
  ...
  it("still supports keyless web search providers through the shared setup flow", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.actagent.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.setupSearch.mockImplementation(async (cfg: ACTAgentConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(String(defaults.workspace)).toContain("/.actagent/workspace");

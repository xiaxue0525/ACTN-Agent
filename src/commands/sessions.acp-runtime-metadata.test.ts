 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "actagent"`, `source: "implicit"`.
...
/**
 * Build a minimal `ACTAgentConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "actagent"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): ACTAgentConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="actagent", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "actagent". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "actagent" and reflects the actual runtime driving the session.

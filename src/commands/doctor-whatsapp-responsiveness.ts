...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "actagent-tui") {
    return true;
  }
  return executable === "actagent" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local ACTAgent TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: ACTAgentConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("actagent doctor --fix")}.`,
      );
    }
  }

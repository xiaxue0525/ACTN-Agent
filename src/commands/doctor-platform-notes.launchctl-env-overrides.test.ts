...
import {
  ...
  collectMacStaleACTAgentUpdateLaunchdJobsWarning,
  noteMacLaunchctlGatewayEnvOverrides,
  noteMacStaleACTAgentUpdateLaunchdJobs,
...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("collects clear unsetenv instructions for token override", async () => {
    const getenv = vi.fn(async (name: string) =>
      name === "ACTAGENT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(warning).toContain("ACTAGENT_GATEWAY_TOKEN");
    expect(warning).toContain("launchctl unsetenv ACTAGENT_GATEWAY_TOKEN");
    expect(warning).not.toContain("ACTAGENT_GATEWAY_PASSWORD");
  });
  ...
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    expect(message).toContain("ACTAGENT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ACTAGENT_GATEWAY_TOKEN");
    expect(message).not.toContain("ACTAGENT_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "ACTAGENT_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
          password: { source: "env", provider: "default", id: "ACTAGENT_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        providers: {
    ...
    expect(message).toContain("ACTAGENT_GATEWAY_PASSWORD");
  });
...
describe("noteMacStaleACTAgentUpdateLaunchdJobs", () => {
  it("collects stale updater job cleanup guidance on macOS", async () => {
    ...
    const warning = await collectMacStaleACTAgentUpdateLaunchdJobsWarning({
      platform: "darwin",
      findJobs,
      env,
    });
    ...
    expect(warning).toContain("Stale ACTAgent updater launchd job(s) detected");
    expect(warning).toContain("ai.actagent.update.2026.5.12");
    expect(warning).toContain("ai.actagent.manual-update.1717168800");
    expect(warning).toContain("launchctl remove <label>");
    expect(warning).toContain("actagent gateway restart");
  });
  ...
  it("uses service env for gateway platform stale updater warnings", async () => {
    ...
    await collectMacGatewayPlatformWarnings({} as ACTAgentConfig, {
      platform: "darwin",
      service,
      findJobs,
    });
  ...
  it("uses service env for doctor stale updater notes", async () => {
    ...
    await noteMacStaleACTAgentUpdateLaunchdJobs({
      platform: "darwin",
      service,
      findJobs,
    });
  ...
  it("prints stale updater job cleanup guidance on macOS", async () => {
    ...
    const [message, title] = requireNoteCall(noteFn);
    expect(title).toBe("Gateway (macOS)");
    expect(message).toContain("Stale ACTAgent updater launchd job(s) detected");
    expect(message).toContain("ai.actagent.update.2026.5.12");
    expect(message).toContain("ai.actagent.manual-update.1717168800");
    expect(message).toContain("launchctl remove <label>");
    expect(message).toContain("actagent gateway restart");
  });

...
import { resolvePrimaryStringValue } from "@actagent/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.ACTAGENT_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: ACTAgentConfig): {
  ...
  const parsed = ACTAgentSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: ACTAgentConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: ACTAgentConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: ACTAgentConfig): void {

...
describe("readServiceStatusSummary", () => {
  it("marks ACTAgent-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["actagent", "gateway", "run"] })),
    ...
    expect(summary.managedByACTAgent).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByACTAgent).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.ACTAGENT_GATEWAY_PORT === "19199";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ACTAGENT_GATEWAY_PORT === "19199" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["actagent", "gateway", "run", "--port", "19199"],
    ...
    expect(loadedArgs?.env?.ACTAGENT_GATEWAY_PORT).toBe("19199");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.ACTAGENT_GATEWAY_PORT).toBe("19199");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "actagent-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "actagent-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "actagent-gateway.service"));

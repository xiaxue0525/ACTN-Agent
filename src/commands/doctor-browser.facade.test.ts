...
describe("doctor browser facade", () => {
  ...
  it("delegates legacy actagentd browser profile detection to the browser facade surface", async () => {
    const residue = {
    ...
    loadBundledPluginPublicSurfaceModuleSync.mockReturnValue({
      ...
      detectLegacyactagentdBrowserProfileResidue: detect,
    });
    const cfg: ACTAgentConfig = {
      browser: {
        profiles: {
    ...
    const deps = {
      ...
      pathExists: (targetPath: string) => targetPath === "/tmp/actagent-home/browser/actagentd",
    };

    await expect(detectLegacyactagentdBrowserProfileResidue(cfg, deps)).resolves.toEqual(residue);
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
      dirName: "browser",
      artifactBasename: "browser-doctor.js",
    });
  ...
  it("delegates legacy actagentd browser profile cleanup to the browser facade surface", async () => {
    const cleanup = vi.fn().mockResolvedValue({ changes: ["archived"], warnings: [] });
    loadBundledPluginPublicSurfaceModuleSync.mockReturnValue({
      noteChromeMcpBrowserReadiness: vi.fn(),
      maybeArchiveLegacyactagentdBrowserProfileResidue: cleanup,
    });
    ...
    await expect(maybeArchiveLegacyactagentdBrowserProfileResidue(cfg, deps)).resolves.toEqual({
      changes: ["archived"],
      warnings: [],
    });
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({

...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".actagent-work");
    const defaultStateDir = path.join(homeDir, ".actagent");
    const profileConfigPath = path.join(profileStateDir, "actagent.json");
    ...
    fs.writeFileSync(
      ...
      `actagent-workspace-attestation:v1\n${new Date().toISOString()}\n`,
    );

    vi.stubEnv("HOME", homeDir);
    vi.stubEnv("ACTAGENT_HOME", homeDir);
    vi.stubEnv("ACTAGENT_PROFILE", "work");
    vi.stubEnv("ACTAGENT_STATE_DIR", profileStateDir);
    vi.stubEnv("ACTAGENT_CONFIG_PATH", profileConfigPath);
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "actagent-trash-symlink-target");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "actagent.json");
    const sourcePath = path.join(realParent, "actagent.json");
...
describe("formatControlUiSshHint", () => {
  it("includes the IPv4-only BYOH note and workaround", () => {
    const hint = formatControlUiSshHint({ port: 19199 });
...
describe("summarizeExistingConfig", () => {
    ...
    expect(
      summarizeExistingConfig({
        ...
        gateway: {
          ...
          port: 19199,
          bind: "lan",
          remote: { url: "ws://192.168.0.202:19199" },
        },
      }),
    ).toBe("Model: openai/gpt-5.4\nGateway: remote via LAN at ws://192.168.0.202:19199");
  });
  ...
  it("uses the port when no remote gateway URL is configured", () => {
    ...
    ).toBe("Gateway: local via loopback on :19199");
  });
...
describe("resolveControlUiLinks", () => {
  it("uses customBindHost for custom bind", () => {
    ...
    expect(links.httpUrl).toBe("http://192.168.1.100:19199/");
    expect(links.wsUrl).toBe("ws://192.168.1.100:19199");
  });
  ...
  it("uses secure schemes when gateway TLS is enabled", () => {
    ...
    expect(links.httpUrl).toBe("https://192.168.1.100:19199/");
    expect(links.wsUrl).toBe("wss://192.168.1.100:19199");
  });
  ...
  it("falls back to loopback for invalid customBindHost", () => {
    ...
    expect(links.httpUrl).toBe("http://127.0.0.1:19199/");
    expect(links.wsUrl).toBe("ws://127.0.0.1:19199");
  });
  ...
  it("uses tailnet IP for tailnet bind", () => {
    ...
    expect(links.httpUrl).toBe("http://100.64.0.9:19199/");
    expect(links.wsUrl).toBe("ws://100.64.0.9:19199");
  });

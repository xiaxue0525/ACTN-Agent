// Builds overview table rows for `actagent status` and `actagent status --all`.
...
/** Builds the default `actagent status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("actagent security audit --deep")}` },
    ],

...
describe("formatContextEngineHealthLine", () => {
  it("summarizes quarantined context engines", () => {
      ...
      quarantined: [
        {
          ...
          owner: "plugin:lossless-actagent",
          operation: "assemble",
          reason: "db corrupt",
          failedAt: 123,
        },
    ...
    expect(formatContextEngineHealthLine(summary)).toBe(
      "Context engine: warning (1 quarantined; downgraded to legacy: lossless-actagent)",
    );
  });
});
  ...
  it("formats gateway connection details as indented key/value lines", () => {
    const err = new Error(
      [
        ...
        "Config: /Users/steipete/.actagent-dev/actagent.json",
        "Bind: loopback",
      ].join("\n"),
    );

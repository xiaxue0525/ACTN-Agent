...
function createBaseParams(
  ...
  return {
    ...
    connectionDetailsForReport: "ws://127.0.0.1:19199",
    snap: null,
...
describe("status-all diagnosis port checks", () => {
  ...
  it("labels ACTAgent Tailscale exposure separately from daemon state", async () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "actagent-gateway", address: "127.0.0.1:19199" },
      { pid: 5001, commandLine: "actagent-gateway", address: "[::1]:19199" },
    ]);

    await appendStatusAllDiagnosis(params);

    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 19199");
    expect(output).toContain("Detected dual-stack loopback listeners");
    expect(output).not.toContain("Port 19199 is already in use.");
  });
  ...
  it("treats a single wildcard Gateway listener as healthy", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 19199");
    expect(output).toContain("Detected ACTAgent Gateway listener on the configured port.");
    expect(output).not.toContain("Port 19199 is already in use.");
  });
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "actagent-gateway", address: "127.0.0.1:19199" },
      { pid: 5002, commandLine: "actagent-gateway", address: "[::1]:19199" },
    ]);
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("! Port 19199");
    expect(output).toContain("2 ACTAgent gateway processes appear to be listening on port 19199");
    expect(output).toContain("Port 19199 is already in use.");
  });
  ...
  it("adds direct update restart guidance for failed update sentinels", async () => {
    ...
    expect(output).toContain(
      "Update restart: failed · managed-service-handoff-failed · run actagent gateway status --deep",
    );
    expect(output).toContain("Update restart failed; run actagent gateway status --deep.");
    expect(output).toContain(
      "If the service is down, run actagent gateway restart or actagent gateway install --force.",
    );
  });
  ...
  it("adds direct update restart guidance for pending update sentinels", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain(
      "Update restart: restart pending health verification · run actagent gateway status --deep",
    );
    expect(output).toContain(
      "Update restart is still pending; run actagent update status --json for handoff state.",
    );
  });
  ...
  it("does not read or display stale stderr tails on Darwin", async () => {
    ...
    try {
      ...
      restartLogMocks.resolveGatewayRestartLogPath.mockReturnValue(
        "/tmp/actagent/logs/gateway-restart.log",
      );
      ...
      expect(gatewayMocks.readFileTailLines).not.toHaveBeenCalledWith(
        "/Users/test/Library/Logs/actagent/gateway.err.log",
        40,
      );
      expect(output).toContain("# stdout: /Users/test/Library/Logs/actagent/gateway.log");

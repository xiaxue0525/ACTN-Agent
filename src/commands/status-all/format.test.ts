...
describe("status-all format", () => {
  ...
  it("builds gateway summary parts for fallback remote targets", () => {
    expect(
      ...
      targetText: "fallback ws://127.0.0.1:19199",
      targetTextWithSource:
        "fallback ws://127.0.0.1:19199 (missing gateway.remote.url (fallback local))",
  ...
  it("formats dashboard values consistently", () => {
    expect(formatStatusDashboardValue("https://actagent.local")).toBe("https://actagent.local");
  ...
  it("builds shared gateway surface values for node and gateway views", () => {
    expect(
      ...
      dashboardUrl: "http://127.0.0.1:19199/",
      gatewayValue:
  ...
  it("prefers node-only gateway values when present", () => {
    expect(
      buildStatusGatewaySurfaceValues({
        ...
        nodeOnlyGateway: {
          gatewayValue: "node → remote.example:19199 · no local gateway",
        },
      }),
    ).toEqual({
      dashboardUrl: null,
  ...
  it("builds overview rows with shared ordering", () => {
    expect(
      buildStatusOverviewRows({
        ...
        dashboardValue: "https://actagent.local",
        ...
        middleRows: [{ Item: "Security", Value: "Run: actagent security audit --deep" }],
      ...
      { Item: "Dashboard", Value: "https://actagent.local" },
      ...
      { Item: "Security", Value: "Run: actagent security audit --deep" },
  ...
  it("builds overview surface rows from shared gateway and update inputs", () => {
    expect(
      ...
      { Item: "Version", Value: "1.0.0" },
      { Item: "Dashboard", Value: "http://127.0.0.1:19199/" },

...
type EnvSnapshot = {
  ...
  ACTAGENT_OAUTH_DIR?: string;
  ACTAGENT_AGENT_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    ACTAGENT_OAUTH_DIR: process.env.ACTAGENT_OAUTH_DIR,
    ACTAGENT_AGENT_DIR: process.env.ACTAGENT_AGENT_DIR,
  };
}
...
function setupSessionState(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ACTAgentConfig) {
...
async function runStateIntegrityText(cfg: ACTAgentConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-doctor-state-integrity-"));
  ...
  it("prompts for oauth dir when ACTAGENT_OAUTH_DIR is explicitly configured", async () => {
    process.env.ACTAGENT_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ACTAgentConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when ACTAGENT_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.ACTAGENT_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("actagent tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.ACTAGENT_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.ACTAGENT_HOME = symlinkHome;
        process.env.ACTAGENT_STATE_DIR = path.join(symlinkHome, ".actagent");
  ...
  it("prints actagent-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ACTAgentConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/actagent sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/actagent sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /actagent sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-tui-pointer-clear-"));
    try {

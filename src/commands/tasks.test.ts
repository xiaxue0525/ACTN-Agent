...
import type { ACTAgentTestState } from "../test-utils/actagent-test-state.js";
...
async function withTaskCommandStateDir(
  run: (state: ACTAgentTestState) => Promise<void>,
): Promise<void> {
  await withACTAgentTestState(
    { layout: "state-only", prefix: "actagent-tasks-command-" },
    async (state) => {
      resetTaskRegistryDeliveryRuntimeForTests();

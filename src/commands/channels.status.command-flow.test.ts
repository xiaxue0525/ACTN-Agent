...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/actagent.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@actagent/feishu",
          installCommand: "actagent plugins install @actagent/feishu",
          doctorFixCommand: "actagent doctor --fix",
          repairHint:
            "Install the official external plugin with: actagent plugins install @actagent/feishu, or run: actagent doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: actagent plugins install @actagent/feishu, or run: actagent doctor --fix.",
    );
  });
    ...
    mocks.callGateway.mockRejectedValue(
      new Error(
        [
          ...
          "Source: env ACTAGENT_GATEWAY_URL",
        ].join("\n"),
      ),
    );
    mocks.requireValidConfigSnapshot.mockResolvedValue({ secretResolved: false, channels: {} });

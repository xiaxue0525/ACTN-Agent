...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.ACTAGENT_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.ACTAGENT_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.actagent.ai/gateway/health",
        "https://docs.actagent.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from actagent.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<ACTAgentConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "ACTAgent update wizard" : "ACTAgent configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: ACTAgentConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.actagent.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("actagent doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      const localUrl = "ws://127.0.0.1:19199";
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.ACTAGENT_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.ACTAGENT_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.actagent.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.actagent.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );

...
const states: ACTAgentTestState[] = [];
...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<ACTAgentTestState> {
  const state = await createACTAgentTestState({
    layout: "state-only",
    prefix: "actagent-doctor-oauth-sidecar-",
    env: {
      ACTAGENT_AGENT_DIR: undefined,
      ACTAGENT_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "actagent-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans ACTAGENT_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.ACTAGENT_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.ACTAGENT_AGENT_DIR = previousAgentDir;
      }
    }
  });

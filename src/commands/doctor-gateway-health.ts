...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is ACTAgent ${VERSION}; the running Gateway is ACTAgent ${gatewayVersion}.`,
      "Check `actagent --version`, `which actagent`, and `actagent gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `actagent` points to the version you want, or reinstall the Gateway service from that same ACTAgent install.",
    ].join("\n"),
    "ACTAgent version mismatch",
  );
}

// Implements guided and non-interactive `actagent channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: ACTAgentConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as ACTAgentConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run actagent doctor --fix, then retry actagent channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("actagent channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }

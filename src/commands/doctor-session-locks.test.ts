...
describe("noteSessionLockHealth", () => {
  let state: ACTAgentTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createACTAgentTestState({
      layout: "state-only",
      prefix: "actagent-doctor-locks-",
    });
  });
  ...
  it("uses configured stale threshold without removing live ACTAgent lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an ACTAgent process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-actagent-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");

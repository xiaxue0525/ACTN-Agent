...
import { resolveTimerTimeoutMs } from "@actagent/normalization-core/number-coercion";
...
export function guardCancel<T>(value: T | symbol, runtime: RuntimeEnv): T {
  if (isCancel(value)) {
    cancel(
      stylePromptTitle("Setup cancelled. / 设置已取消。") ?? "Setup cancelled. / 设置已取消。",
    );
    runtime.exit(0);
...
export function summarizeExistingConfig(config: ACTAgentConfig): string {
  const rows: string[] = [];
  ...
  return rows.length ? rows.join("\n") : "No key settings detected. / 未检测到密钥设置。";
}

function summarizeGatewayConfig(config: ACTAgentConfig): string | null {
...
export function validateGatewayPasswordInput(value: unknown): string | undefined {
  ...
  if (trimmed === "undefined" || trimmed === "null") {
    return 'Cannot be the literal string "undefined" or "null" / 不能是字面量 "undefined" 或 "null"';
  }
  return undefined;
}
...
export function printWizardHeader(runtime: RuntimeEnv) {
  ...
  const title = supportsDecorativeEmoji() && icon ? `${icon} ACTAGENT ${icon}` : "ACTAGENT";
...
export function formatControlUiSshHint(params: {
  ...
  return [
    "No GUI detected. Open from your computer: / 未检测到 GUI，请从你的电脑打开：",
    `ssh -N -L ${params.port}:127.0.0.1:${params.port} ${sshTarget}`,
    "Then open: / 然后打开：",
    localUrl,
    authedUrl,
    "BYOH note: lan, tailnet, and custom bind are currently IPv4-only. / BYOH 提示：lan、tailnet 和自定义绑定目前仅支持 IPv4。",
    "If your host is IPv6-only, use an IPv4 sidecar or proxy in front of the Gateway. / 如果你的主机仅支持 IPv6，请在网关前使用 IPv4 sidecar 或代理。",
    "Docs: / 文档：",
    "https://docs.actagent.ai/gateway/remote",
    "https://docs.actagent.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export async function moveToTrash(pathname: string, runtime: RuntimeEnv): Promise<void> {
  ...
  try {
    ...
    runtime.log(
      `Moved to Trash: ${shortenHomePath(pathname)} / 已移至废纸篓：${shortenHomePath(pathname)}`,
    );
  } catch {
    runtime.log(
      `Failed to move to Trash (manual delete): ${shortenHomePath(pathname)} / 移至废纸篓失败（需手动删除）：${shortenHomePath(pathname)}`,
    );
  }
}

...
function isTempBackedACTAgentRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const actagentIndex = lower.lastIndexOf("actagent");
  if (actagentIndex < 1) {
    return false;
  }
  return lower[actagentIndex - 1] === "tmp" || lower[actagentIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("actagent@"),
    ) || isTempBackedACTAgentRoot(beforeSkillRoot)
  );
}

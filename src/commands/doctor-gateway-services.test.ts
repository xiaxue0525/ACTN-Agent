...
const originalUpdateInProgress = process.env.ACTAGENT_UPDATE_IN_PROGRESS;
...
async function runRepair(cfg: ACTAgentConfig, options: { allowExecSecretRefs?: boolean } = {}) {
...
function createGatewayCommand(entrypoint: string) {
  return {
    programArguments: ["/usr/bin/node", entrypoint, "gateway", "--port", "19199"],
    environment: {},
  };
}
    ...
    issues: [
      {
        ...
        message: "Gateway service ACTAGENT_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fsMocks.realpath.mockImplementation(async (value: string) => value);
    mocks.resolveGatewayPort.mockReturnValue(19199);
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: ACTAgentConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.ACTAGENT_GATEWAY_TOKEN?.trim() || undefined;
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.ACTAGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
  });
  ...
  it("does not duplicate gateway runtime warnings already emitted by the node install plan", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: [nvmNode, "/usr/local/bin/actagent", "gateway", "--port", "19199"],
      environment: {},
    });
  ...
  it("passes planned managed env keys into service audit for legacy inline secret detection", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        ACTAGENT_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
  ...
  it("repairs gateway services whose pinned port differs from current config", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: ["/usr/bin/node", "/usr/local/bin/actagent", "gateway", "--port", "18888"],
      workingDirectory: "/tmp",
      environment: {},
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
  ...
  it("uses ACTAGENT_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ ACTAGENT_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/actagent/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/actagent@2026.3.12/node_modules/actagent/dist/index.js",
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/actagent/")) {
          return value.replace(
            "/global/5/node_modules/actagent/",
            "/global/5/node_modules/.pnpm/actagent@2026.3.12/node_modules/actagent/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/actagent/../actagent/dist/index.js",
      installEntrypoint: "/opt/actagent/dist/index.js",
  ...
  it("keeps wrapper-managed gateway services aligned during entrypoint drift checks", async () => {
    const wrapperPath = "/usr/local/bin/actagent-doppler";
    mocks.readCommand.mockResolvedValue({
      programArguments: [wrapperPath, "gateway", "--port", "19199"],
      environment: {
        ACTAGENT_WRAPPER: wrapperPath,
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: true,
    ...
    mocks.buildGatewayInstallPlan.mockImplementation(async ({ env }) => ({
      programArguments: [env.ACTAGENT_WRAPPER, "gateway", "--port", "19199"],
      environment: {
        ACTAGENT_WRAPPER: env.ACTAGENT_WRAPPER,
      },
    }));
    ...
    expect(requireRecord(installPlanOptions.env, "install env").ACTAGENT_WRAPPER).toBe(wrapperPath);
    expect(
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      "Gateway service invokes ACTAGENT_WRAPPER: /usr/local/bin/actagent-doppler",
      "Gateway",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/actagent/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/actagent/dist/index.js",
    });
  ...
  it("skips entrypoint rewrites for an active systemd unit", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      ...createGatewayCommand("/opt/old-actagent/dist/index.js"),
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...createGatewayCommand("/opt/new-actagent/dist/index.js"),
      workingDirectory: "/tmp",
    });
  ...
  it("leaves all service metadata unchanged when an active unit has command drift plus other issues", async () => {
    mockProcessPlatform("linux");
    mocks.readCommand.mockResolvedValue({
      programArguments: ["/usr/bin/actagent", "run"],
      environment: {},
      sourcePath: "/home/test/.config/systemd/user/actagent-gateway.service",
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
  ...
  it("skips entrypoint rewrite in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/actagent/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/actagent/dist/index.js",
    ...
    expectNoteContaining("actagent gateway install --force", "Gateway service config");
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            ACTAGENT_GATEWAY_TOKEN: "env-file-token",
          },
  ...
  it("reports service config drift but skips service rewrite when service repair policy is external", async () => {
    await withEnvAsync({ ACTAGENT_SERVICE_REPAIR_POLICY: "external" }, async () => {
      setupGatewayEntrypointRepairScenario({
        currentEntrypoint: "/Users/test/Library/npm/node_modules/actagent/dist/entry.js",
        installEntrypoint: "/Users/test/Library/npm/node_modules/actagent/dist/index.js",
        installWorkingDirectory: "/tmp",
  ...
  it("warns when the gateway service entrypoint resolves to a source checkout", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-doctor-service-layout-"));
      try {
  ...
  it("does not duplicate Gateway service config panels for a source-checkout entrypoint with audit findings", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "actagent-doctor-service-config-dedup-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        const installEntrypoint = "/usr/local/lib/node_modules/actagent/dist/index.js";
        setupGatewayEntrypointRepairScenario({
        ...
        const forceMatches = consolidated.match(/actagent gateway install --force/g) ?? [];
  ...
  it("keeps the gateway install force hint when a source-checkout warning is suppressed and repair is declined", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "actagent-doctor-service-config-force-hint-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        expect(gatewayServiceConfigNotes[1]?.[0]).toContain("actagent gateway install --force");
      } finally {
        await fs.rm(root, { recursive: true, force: true });
      }
    });
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    mocks.findExtraGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /home/test/.config/systemd/user/actagentdbot-gateway.service",
        scope: "user",
        legacy: true,
      },
    ]);
    mocks.uninstallLegacySystemdUnits.mockResolvedValue([
      {
        name: "actagentdbot-gateway",
        unitPath: "/home/test/.config/systemd/user/actagentdbot-gateway.service",
        enabled: true,
        exists: true,
      },
    ]);
    ...
    expectNoteContaining("actagentdbot-gateway.service", "Legacy gateway removed");
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing ACTAgent gateway next.",
    );
  });
  ...
  it("reports legacy services but skips cleanup when service repair policy is external", async () => {
    await withEnvAsync({ ACTAGENT_SERVICE_REPAIR_POLICY: "external" }, async () => {
      ...
      expectNoteContaining("actagentdbot-gateway.service", "Other gateway-like services detected");
      expect(mocks.note).toHaveBeenCalledWith(
        EXTERNAL_SERVICE_REPAIR_NOTE,

...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: ACTAgentConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => ACTAgentConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: ACTAgentConfig, sourceConfig: ACTAgentConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      ...
      effectiveConfig: ACTAgentConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "actagent-agent-" });
}
...
function mockConfig(home: string, storePath: string): ACTAgentConfig {
  const cfg = {
    agents: {
      defaults: {

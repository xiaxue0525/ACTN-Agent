...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("actagent/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 actagent-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/actagent/dist/index.js gateway --port 19199",
        " 103 actagent channels",
        " 104 actagent tui --local",
        " 105 /usr/bin/actagent chat",
        " 106 helper --note 'actagent tui'",
        " 107 actagent-helper actagent terminal",
        " 108 actagent --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "actagent-tui" },
      { pid: 104, command: "actagent tui --local" },
      { pid: 105, command: "/usr/bin/actagent chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "actagent-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as ACTAgentConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "actagent-tui" }],

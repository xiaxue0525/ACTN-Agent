...
function resolveBundledWorkspaceDir(cfg: ACTAgentConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isACTAgentNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "actagent" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by ACTAgent. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed ACTAgent package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isACTAgentNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; ACTAgent loads the packaged bundled plugin from "${hit.toPath}".`,
  );

...
function isCodexRuntimeConfigured(cfg: ACTAgentConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: ACTAgentConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode ACTAgent agents use isolated per-agent Codex homes.",
      ...
      "- If the Codex plugin is not installed, run `actagent plugins install npm:@actagent/codex` first. Then run `actagent migrate plan codex` to inventory them. Applying that migration copies skills into the current ACTAgent agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}

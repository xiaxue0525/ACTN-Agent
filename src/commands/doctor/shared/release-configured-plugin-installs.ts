...
import { normalizeNullableString as normalizeId } from "@actagent/normalization-core/string-coerce";
...
import { compareACTAgentVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: ACTAgentConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: ACTAgentConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: ACTAgentConfig): string[] {
...
function isPluginEntryDisabled(cfg: ACTAgentConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: ACTAgentConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: ACTAgentConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: ACTAgentConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: ACTAgentConfig): string[] {
...
function collectConfiguredChannelIds(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectConfiguredProviderIds(cfg: ACTAgentConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: ACTAgentConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: ACTAgentConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: ACTAgentConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: ACTAgentConfig): string[] {
...
function addEligiblePluginId(cfg: ACTAgentConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareACTAgentVersions(
  ...
  const touchedComparedToRelease = compareACTAgentVersions(params.touchedVersion, releaseVersion);

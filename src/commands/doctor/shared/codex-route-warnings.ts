...
import { AGENT_MODEL_CONFIG_KEYS } from "@actagent/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@actagent/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@actagent/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@actagent/normalization-core/string-coerce";
...
const LOSSLESS_CONTEXT_ENGINE_ID = "lossless-actagent";
...
function resolveImplicitDefaultAgentModelRef(cfg: ACTAgentConfig): string {
...
function collectConfigModelRefs(cfg: ACTAgentConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: ACTAgentConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: ACTAgentConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: ACTAgentConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: ACTAgentConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: ACTAgentConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: ACTAgentConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: ACTAgentConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: ACTAgentConfig }): ConfigRouteRepairResult {
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `actagent doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an ACTAgent runtime policy."
...
function collectCodexAppServerCommandWarnings(cfg: ACTAgentConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
...
}): { cfg: ACTAgentConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "actagent") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "actagent") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `actagent doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],

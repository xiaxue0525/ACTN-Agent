...
import { isRecord as hasRecord } from "@actagent/normalization-core/record-coerce";
...
function listAgentRecords(cfg: ACTAgentConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: ACTAgentConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: ACTAgentConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: ACTAgentConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: ACTAgentConfig): boolean {
...
function hasExplicitChannelPluginBlockerConfig(cfg: ACTAgentConfig): boolean {
...
function hasConfiguredSafeBins(cfg: ACTAgentConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: ACTAgentConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: ACTAgentConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: ACTAgentConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; ACTAgent falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; ACTAgent falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: ACTAgentConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: ACTAgentConfig): string[] {
  const warnings: string[] = [];

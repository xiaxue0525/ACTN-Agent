...
describe("collectCodexRouteWarnings", () => {
  ...
  it("warns when openai-codex primary models still use the legacy route", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        "- Run `actagent doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    ]);
  });
  ...
  it("ignores ACTAGENT_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
  ...
  it("warns when Codex app-server command includes inline arguments", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.actagent\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.actagent\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
  ...
  it("warns when Codex runtime routes are configured while the Codex plugin is disabled", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        "- Run `actagent doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an ACTAgent runtime policy.",
      ].join("\n"),
    ]);
  });

  it("warns when Codex runtime has ACTAgent compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
    ...
    expect(warnings).toStrictEqual([
      [
        "- Codex runtime uses native server-side compaction and ignores ACTAgent compaction summarizer overrides.",
        ...
        "- Run `actagent doctor --fix`: it removes unsupported Codex compaction overrides.",
      ].join("\n"),
    ]);
  });
  ...
  it("migrates legacy Lossless compaction config to the context-engine slot", () => {
    ...
    expect(result.warnings).toStrictEqual([]);
    expect(result.cfg.plugins?.slots?.contextEngine).toBe("lossless-actagent");
    expect(result.cfg.plugins?.entries?.["lossless-actagent"]).toEqual({
      enabled: true,
      config: {
    ...
    expect(result.changes).toContain(
      'Set plugins.slots.contextEngine to "lossless-actagent" for legacy Lossless compaction config.',
    );
    expect(result.changes).toContain(
  ...
  it("does not migrate mixed Lossless provider-only and summary-model consumers", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        ...
        "- agents.defaults.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.list.fast.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-actagent.config.summaryModel.",
  ...
  it("preserves Codex runtime policy for migrated Lossless summary models", () => {
    ...
    expect(result.warnings).toStrictEqual([]);
    expect(result.cfg.plugins?.entries?.["lossless-actagent"]?.config).toEqual({
      summaryModel: "openai/gpt-5.4-mini",
    });
  ...
  it("does not collapse conflicting per-agent Lossless summary models", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.list.fast.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.list.fast.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-actagent.config.summaryModel.",
        "- agents.list.deep.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.list.deep.compaction.model: openai/gpt-5.5 should become plugins.entries.lossless-actagent.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("does not overwrite a non-Lossless context-engine slot", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.defaults.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.defaults.compaction.model: openai/gpt-5.4 should become plugins.entries.lossless-actagent.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("preserves Codex runtime policy for blocked Lossless summary rewrites", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.defaults.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.defaults.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-actagent.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("does not migrate shared Lossless summary models inherited by non-Codex agents", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.list.codex.compaction.provider: lossless-actagent should become plugins.slots.contextEngine: lossless-actagent.",
        "- agents.defaults.compaction.model: openai/gpt-5.4 should become plugins.entries.lossless-actagent.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
          ...
          list: [
            {
              ...
              model: "anthropic/claude-sonnet-4-6",
              models: {
                "anthropic/claude-sonnet-4-6": { agentRuntime: { id: "actagent" } },
              },
            },
          ],
        },
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the ACTAgent runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the ACTAgent runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise ACTAgent/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        models: {
          providers: {
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "actagent"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "actagent"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves explicit ACTAgent runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("clears stale Codex overrides while preserving explicit ACTAgent session pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to ACTAgent", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
        ...
        agentHarnessId: "actagent",
        agentRuntimeOverride: "actagent",
        authProfileOverride: "openai:work",
      },
    };
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(store.main.updatedAt).toBe(1);
    expect(store.main.agentHarnessId).toBe("actagent");
    expect(store.main.agentRuntimeOverride).toBe("actagent");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "actagent" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });

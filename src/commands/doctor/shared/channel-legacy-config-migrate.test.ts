...
describe("bundled channel legacy config migrations", () => {
  ...
  it("applies plugin doctor normalizers for configured non-channel plugin entries", () => {
    collectRelevantDoctorPluginIds.mockReturnValueOnce(["lossless-actagent"]);
    applyPluginDoctorCompatibilityMigrations.mockReturnValueOnce({
      config: {
        plugins: {
          entries: {
      ...
      changes: ["Configured plugins.entries.lossless-actagent.llm.allowedModels."],
    });
    ...
    expect(result.changes).toEqual(["Configured plugins.entries.lossless-actagent.llm.allowedModels."]);
  });
});

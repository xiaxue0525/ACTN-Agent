...
function migrateLegacyConfigForTest(raw: unknown): {
  ...
  return changes.length === 0
    ? { config: null, changes }
    : { config: next as ACTAgentConfig, changes };
}
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        ...
        list: [
          {
            id: "reviewer",
            agentRuntime: { fallback: "actagent" },
            embeddedHarness: {
              runtime: "codex",
              fallback: "none",
            },
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        defaults: {
          ...
          models: {
            "anthropic/claude-opus-4-7": { agentRuntime: { id: "actagent" } },
          },
        },
      },
    });
...
describe("legacy migrate MCP server type aliases", () => {
  it("moves CLI-native http type to ACTAgent streamable HTTP transport", () => {
    const res = migrateLegacyConfigForTest({
      mcp: {
        servers: {
          silo: {
...
describe("legacy migrate controlUi.allowedOrigins seed (issue #29385)", () => {
  it("seeds allowedOrigins for bind=lan with no existing controlUi config", () => {
    ...
    expect(res.changes).toStrictEqual([
      'Seeded gateway.controlUi.allowedOrigins ["http://localhost:19199","http://127.0.0.1:19199"] for bind=lan. Required since v2026.2.26. Add other machine origins to gateway.controlUi.allowedOrigins if needed.',
    ]);
  });
...
describe("legacy model compat migrate", () => {
  it("upgrades retired model refs", () => {
    ...
    expect(
      (res.config?.plugins?.entries?.["lossless-actagent"] as { config?: { summaryModel?: string } })
        ?.config?.summaryModel,
    ).toBe("anthropic/claude-sonnet-4-6");
    expect(
      (res.config?.plugins?.entries?.["lossless-actagent"] as { config?: { dataPath?: string } })
        ?.config?.dataPath,
    ).toBe("/tmp/claude-opus-4-5");
    expect(
      (
        res.config?.plugins?.entries?.["lossless-actagent"] as {
    ...
    expectMigrationChangesToIncludeFragments(res.changes, [
      ...
      'config.plugins.entries.lossless-actagent.config.summaryModel from "anthropic/claude-3-5-sonnet" to "anthropic/claude-sonnet-4-6"',

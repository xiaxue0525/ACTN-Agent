...
import { buildACTAgentHubPluginInstallRecordFields } from "../../../plugins/actagenthub-install-records.js";
import { ACTAGENTHUB_INSTALL_ERROR_CODE, installPluginFromACTAgentHub } from "../../../plugins/actagenthub.js";
import {
...
const ACTAGENT_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const ACTAGENT_STABLE_OR_BETA_COMPANION_VERSION_RE =
  /^(\d{4}\.[1-9]\d?\.[1-9]\d?)(?:-beta\.[1-9]\d*)?$/;

function shouldFallbackACTAgentHubToNpm(params: {
...
function resolveCandidateACTAgentHubSpec(install: PluginPackageInstall): string | undefined {
  const explicit = install.actagenthubSpec?.trim();
  if (explicit) {
    return explicit;
  }
  return undefined;
...
function addConfiguredAgentRuntimePluginIds(ids: Set<string>, cfg: ACTAgentConfig): void {
  for (const runtime of collectConfiguredRuntimePluginIds(cfg)) {
    addConfiguredPluginId(ids, runtime);
  }
}

function collectConfiguredPluginIds(cfg: ACTAgentConfig): Set<string> {
...
function collectBlockedPluginIds(cfg: ACTAgentConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: ACTAgentConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
function collectDownloadableInstallCandidates(params: {
  ...
  for (const entry of listRawChannelPluginCatalogEntries({
    ...
    const actagenthubSpec = resolveCandidateACTAgentHubSpec(entry.install);
    if (!npmSpec && !actagenthubSpec) {
      continue;
    }
    candidates.set(pluginId, {
      ...
      ...(npmSpec ? { npmSpec } : {}),
      ...(actagenthubSpec ? { actagenthubSpec } : {}),
  ...
  for (const entry of listOfficialExternalPluginCatalogEntries()) {
    ...
    const actagenthubSpec = resolveCandidateACTAgentHubSpec(install);
    if (!npmSpec && !actagenthubSpec) {
      continue;
    }
    candidates.set(pluginId, {
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareACTAgentReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = ACTAGENT_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = ACTAGENT_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
...
function collectCandidateOfficialPackageNames(
  ...
  const actagenthubName = candidate.actagenthubSpec
    ? parseACTAgentHubPluginSpec(candidate.actagenthubSpec)?.name
    : undefined;
  if (npmName) {
    names.add(npmName);
  }
  if (actagenthubName) {
    names.add(actagenthubName);
  }
  return names;
}
...
function collectInstalledRecordPackageNames(record: PluginInstallRecord): Set<string> {
  ...
  if (record.source === "actagenthub") {
    const specName = record.spec ? parseACTAgentHubPluginSpec(record.spec)?.name : undefined;
    for (const value of [record.actagenthubPackage, specName]) {
      if (value) {
        names.add(value);
      }
    }
...
function isTrustedOfficialInstallRecordForCandidate(params: {
  ...
  if (record.source !== "npm" && record.source !== "actagenthub") {
    return false;
  }
...
function recordMatchesBundledPackage(
  ...
  if (record.source === "actagenthub") {
    return [record.actagenthubPackage, record.spec].some(
      (value) => recordACTAgentHubPackageName(value) === packageName,
    );
  }
  return false;
}
...
function recordACTAgentHubPackageName(value: string | undefined): string | undefined {
  const trimmed = value?.trim();
  if (!trimmed) {
    return undefined;
  }
  return parseACTAgentHubPluginSpec(trimmed)?.name ?? trimmed;
}
...
async function installCandidate(params: {
  ...
  const actagenthubSpecs = candidate.actagenthubSpec
    ? resolveACTAgentHubInstallSpecsForUpdateChannel({
        spec: candidate.actagenthubSpec,
        updateChannel: params.updateChannel,
      })
    : null;
  const npmSpecs = candidate.npmSpec
    ...
    : null;
  const actagenthubInstallSpec = actagenthubSpecs?.installSpec ?? candidate.actagenthubSpec;
  const npmInstallSpec = npmSpecs?.installSpec ?? candidate.npmSpec;
  const npmDir = resolveDefaultPluginNpmDir(params.env);
  const existingACTAgentHubPackagePath = actagenthubInstallSpec
    ? resolveExistingCandidateACTAgentHubPackagePath({
        candidate,
        extensionsDir,
      })
    : null;
  ...
  const shouldTryACTAgentHub =
    actagenthubInstallSpec &&
    !existingNpmPackagePath &&
    !(params.preferNpm && npmInstallSpec) &&
    candidate.defaultChoice !== "npm";
  if (shouldTryACTAgentHub) {
    const actagenthubResult = await installPluginFromACTAgentHub({
      spec: actagenthubInstallSpec,
      extensionsDir,
      env: params.env,
      expectedPluginId: candidate.pluginId,
      mode: params.mode === "update" || existingACTAgentHubPackagePath ? "update" : "install",
    });
    if (actagenthubResult.ok) {
      const pluginId = actagenthubResult.pluginId;
      return {
        records: {
          ...params.records,
          [pluginId]: {
            ...buildACTAgentHubPluginInstallRecordFields(actagenthubResult.actagenthub),
            spec: actagenthubSpecs?.recordSpec ?? actagenthubInstallSpec,
            installPath: actagenthubResult.targetDir,
        ...
        changes: [
          formatInstalledConfiguredPluginChange({
            pluginId,
            installSpec: actagenthubInstallSpec,
    ...
    if (
      !npmInstallSpec ||
      !shouldFallbackACTAgentHubToNpm({ result: actagenthubResult, npmSpec: npmInstallSpec })
    ) {
      return {
        records: params.records,
        changes: [],
        warnings: [
          `Failed to install missing configured plugin "${candidate.pluginId}" from ${actagenthubInstallSpec}: ${actagenthubResult.error}`,
        ],
        failedPluginId: candidate.pluginId,
      };
    }
    changes.push(
      `ACTAgentHub ${actagenthubInstallSpec} unavailable for "${candidate.pluginId}"; falling back to npm ${npmInstallSpec}.`,
    );
  }
  if (!npmInstallSpec) {
    return {
...
function resolveExistingCandidateACTAgentHubPackagePath(params: {
  candidate: DownloadableInstallCandidate;
...
/** Repair missing installs inferred from the current ACTAgent config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      changes.push(
        `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "actagent doctor --fix" after the update completes.`,
      );
    }
  }

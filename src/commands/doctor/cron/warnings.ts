...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("actagent cron list")} and ${formatCliCommand("actagent cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.actagent/bin/ensure-whatsapp.sh` is not maintained by current ACTAgent and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("actagent channels status --probe")}, ${formatCliCommand("actagent doctor")}, and ${formatCliCommand("actagent gateway status")} for current health checks.`,

...
const testConfigStore = new Map<string, ACTAgentConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.ACTAGENT_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.ACTAGENT_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("ACTAGENT_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "actagent.json");
}
...
function readTestConfig<T = ACTAgentConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: ACTAgentConfig) => cfg.gateway?.port ?? 19199,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/actagent-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "actagent"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("actagent-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (actagent#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "actagent");
      const seededAgents = [
      ...
      testConfigStore.set(resolveTestConfigPath(), {
        ...
        gateway: { mode: "local", port: 19199, auth: { mode: "token", token: "seed_tok" } },
      } as ACTAgentConfig);
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (actagent#84692)", async () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `actagent gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      waitForGatewayReachableMock = vi.fn(async () => ({
        ok: false,
        detail: "connect ECONNREFUSED 127.0.0.1:19199",
      }));
      ...
      expect(parsed.hints).toContain("Fix: run `actagent gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.ACTAGENT_STATE_DIR = stateDir;
      process.env.ACTAGENT_CONFIG_PATH = path.join(stateDir, "actagent.json");

...
const mocks = vi.hoisted(() => {
  ...
  return {
    readBestEffortConfig: vi.fn(async () => ({
      gateway: {
        mode: "remote",
        remote: { url: "wss://remote.example:19199", token: "rtok" },
        auth: { token: "ltok" },
      },
    })),
    resolveGatewayPort: vi.fn((_cfg?: unknown) => 19199),
...
describe("gateway-status command", () => {
  ...
  it("includes diagnostic next steps when no gateway is reachable or discoverable", async () => {
    ...
    expect(warning?.message).toContain("actagent gateway status --deep --require-rpc");
  ...
  it("suppresses unresolved SecretRef auth warnings when probe is reachable", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      { MISSING_GATEWAY_TOKEN: undefined, ACTAGENT_GATEWAY_TOKEN: undefined },
      async () => {
  ...
  it("does not resolve local token SecretRef when ACTAGENT_GATEWAY_TOKEN is set", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      {
    ...
    expect(runtimeErrors).toHaveLength(0);
    const localProbeCall = requireProbeCall("ws://127.0.0.1:19199");
    ...
    expect(configRemoteTarget?.config).toMatchInlineSnapshot(`
      {
        ...
        "gateway": {
          ...
          "remoteUrl": "wss://remote.example:19199",
          "tailscaleMode": null,
        },
        "issues": [],
        "legacyIssues": [],
  ...
  it("uses local TLS target strategy and fingerprint for local loopback probes", async () => {
    ...
    const localProbeCall = requireProbeCall("wss://127.0.0.1:19199");
  ...
  it("passes the full caller timeout through to local loopback probes", async () => {
    ...
    expect(requireProbeCall("ws://127.0.0.1:19199").timeoutMs).toBe(15_000);
  });
  ...
  it("keeps inactive local loopback probes on the short timeout in remote mode", async () => {
    ...
    expect(requireProbeCall("ws://127.0.0.1:19199").timeoutMs).toBe(800);
  });
  ...
  it("infers ssh-auto targets from resolved discovery hosts", async () => {
    ...
    await withEnvAsync({ USER: "steipete" }, async () => {
      ...
      discoverGatewayBeacons.mockResolvedValueOnce([
        { instanceName: "bad", tailnetDns: "-V" },
        { host: "goodhost", sshPort: 2222, port: 19199, instanceName: "Gateway" },
      ]);
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    await withEnvAsync({ USER: "steipete" }, async () => {
      readBestEffortConfig.mockResolvedValueOnce(
        makeRemoteGatewayConfig("ws://peters-mac-studio-1.sheep-coho.ts.net:19199"),
      );
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    await withEnvAsync({ USER: "" }, async () => {
      readBestEffortConfig.mockResolvedValueOnce(
        makeRemoteGatewayConfig("wss://studio.example:19199"),
      );

...
import type { ACTAgentConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ACTAgentConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: ACTAgentConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "ACTAgent default (full)";
  const defaultRequestedAskSource = "ACTAgent default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("actagent approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: ACTAgentConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: ACTAgentConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: ACTAgentConfig): string[] {
  ...
  return [
    "- WARNING: actagent.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("actagent secrets configure")} or ${formatCliCommand("actagent secrets apply")}, then verify with ${formatCliCommand("actagent secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.actagent/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("actagent approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Example tunnel: ssh -N -L 19199:127.0.0.1:19199 user@gateway-host",
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("actagent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("actagent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("actagent doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "actagent config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("actagent config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('actagent config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: ACTAgentConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("actagent security audit --deep")}`;

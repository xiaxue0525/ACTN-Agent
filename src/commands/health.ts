...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ACTAGENT_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: ACTAgentConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: ACTAgentConfig) => {
...
/** Runs the `actagent health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ACTAGENT_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<ACTAgentConfig> {
  const { readBestEffortConfig } = await loadConfigRuntime();
  return await readBestEffortConfig();
}

async function readRuntimeHealthConfig(): Promise<ACTAgentConfig> {

...
describe("channels list", () => {
  ...
  it("prefers reachable gateway account snapshots over command-local token state", async () => {
    ...
    const output = stripAnsi(loggedText(runtime));
    expect(output).toContain("Discord default (actagentsweeper):");
    expect(output).toContain("configured");
  ...
  it("default output shows configured official external channels when the plugin is missing", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      channelId: "discord",
      label: "Discord",
      installSpec: "@actagent/discord",
      installCommand: "actagent plugins install @actagent/discord",
      doctorFixCommand: "actagent doctor --fix",
      repairHint:
        "Install the official external plugin with: actagent plugins install @actagent/discord, or run: actagent doctor --fix.",
    });
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
    ...
    expect(output).toContain(
      "run actagent plugins install @actagent/discord or actagent doctor --fix",
    );

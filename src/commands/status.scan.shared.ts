...
function hasExplicitMemorySearchConfig(cfg: ACTAgentConfig, agentId: string): boolean {
...
export function resolveMemoryPluginStatus(cfg: ACTAgentConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: ACTAgentConfig, agentId: string) => { store: { path: string } } | null;

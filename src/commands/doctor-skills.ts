...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("actagent doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("actagent skills check --agent <id>")} or ${formatCliCommand("actagent skills info <name> --agent <id>")}`,
  );
  return lines;
}

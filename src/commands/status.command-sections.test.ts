...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:actagent security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:actagent security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:actagent status --all",
      "  Need to debug live? cmd:actagent logs --follow",
      "  Fix reachability first: cmd:actagent gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:actagent devices approve req-123)",
      "muted(Fallback: cmd:actagent devices approve --latest)",
      "muted(Inspect: cmd:actagent devices list)",
    ]);
  });

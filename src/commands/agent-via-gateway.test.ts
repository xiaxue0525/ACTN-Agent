...
function mockConfig(storePath: string, overrides?: Partial<ACTAgentConfig>) {
  const config = {
    agents: {
      defaults: {
        timeoutSeconds: 600,
...
async function withTempStore(
  ...
  overrides?: Partial<ACTAgentConfig>,
) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-agent-cli-"));

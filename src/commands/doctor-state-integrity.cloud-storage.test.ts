...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "ACTAgent", ".actagent");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/actagent-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".actagent");
  ...
  it("anchors cloud detection to OS homedir when ACTAGENT_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".actagent");
    const originalACTAgentHome = process.env.ACTAGENT_HOME;
    process.env.ACTAGENT_HOME = "/tmp/actagent-home-override";

...
export async function setupSkills(
  ...
  let next: ACTAgentConfig = cfg;
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("actagent skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("actagent skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      // visible through `actagent skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("actagent doctor")}\` to review skills + requirements.`,
      );

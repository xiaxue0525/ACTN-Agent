...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): ACTAgentConfig {
  ...
  function writeStore(cfg: ACTAgentConfig, entries: Record<string, unknown>) {

/**
 * Shared realtime voice controls for active ACTAgent agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "actagent_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal ACTAgent voice control result.",
    "Do not call actagent_agent_consult or any other tool for this message.",
    "Speak this exact ACTAgent status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "ACTAgent is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "ACTAgent has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "ACTAgent could not queue that follow-up."
    : "ACTAgent could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "ACTAgent finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `ACTAgent is starting ${name}.` : "ACTAgent is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `ACTAgent finished ${name} and is continuing.`
        : "ACTAgent finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `ACTAgent is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "ACTAgent is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `ACTAgent is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "ACTAgent is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "ACTAgent is working on the current voice request.";
  }

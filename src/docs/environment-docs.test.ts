...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "ACTAgent ignores provider credential environment variables from workspace `.env` files",
    );
    expect(markdown).toContain("~/.actagent/.env");
    expect(markdown).toContain("$ACTAGENT_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("ACTAGENT_LOAD_SHELL_ENV=1");

...
import type { Context, Model, SimpleStreamOptions } from "actagent/plugin-sdk/llm";
...
describe("applyExtraParamsToAgent", () => {
  ...
  it("passes agentDir and workspaceDir to provider stream wrappers", () => {
    ...
    applyExtraParamsToAgent(
      ...
      "high",
      "cass",
      "/tmp/actagent-workspace",
      model,
      "/tmp/actagent-agent",
    );

    expect(capturedContext?.agentDir).toBe("/tmp/actagent-agent");
    expect(capturedContext?.workspaceDir).toBe("/tmp/actagent-workspace");
  });

...
import { completeSimple, type Model } from "actagent/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.ACTAGENT_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "ACTAGENT_LIVE_SETUP_TOKEN did not look like a setup-token. Use ACTAGENT_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set ACTAGENT_LIVE_SETUP_TOKEN_VALUE or ACTAGENT_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureACTAgentModelsJson(cfg, tokenSource.agentDir);

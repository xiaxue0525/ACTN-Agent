...
import type { DB as ACTAgentAgentKyselyDatabase } from "../../state/actagent-agent-db.generated.js";
import {
  openACTAgentAgentDatabase,
  runACTAgentAgentWriteTransaction,
  type ACTAgentAgentDatabase,
...
// The auth database lives in the agent dir and shares the actagent-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "actagent-agent.sqlite"),
  };
}
...
export function openAuthProfileDatabase(agentDir?: string): ACTAgentAgentDatabase {
  return openACTAgentAgentDatabase(resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function readPersistedAuthProfileStoreRaw(
  agentDir?: string,
  database?: ACTAgentAgentDatabase,
): unknown {
  if (database) {
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: ACTAgentAgentDatabase) => {
  ...
  runACTAgentAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: ACTAgentAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runACTAgentAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runACTAgentAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}

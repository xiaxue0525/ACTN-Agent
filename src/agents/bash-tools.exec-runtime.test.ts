...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if ACTAGENT_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${ACTAGENT_PREPEND_PATH}${PATH:+:$PATH}"; unset ACTAGENT_PREPEND_PATH; echo test',
    );
  });

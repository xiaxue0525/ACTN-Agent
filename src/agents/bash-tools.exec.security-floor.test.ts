...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-exec-security-floor-"));
    process.env.HOME = tempRoot;
    process.env.USERPROFILE = tempRoot;
    process.env.ACTAGENT_HOME = tempRoot;
    process.env.ACTAGENT_STATE_DIR = path.join(tempRoot, "state");
  ...
  it("does not let host approval defaults deny implicit sandbox execution", async () => {
    const actagentDir = path.join(tempRoot ?? os.tmpdir(), ".actagent");
    fs.mkdirSync(actagentDir, { recursive: true });
    fs.writeFileSync(
      path.join(actagentDir, "exec-approvals.json"),

/**
 * ACTAgent stdio transport wrapper for MCP server subprocesses.
 */
...
export type ACTAgentStdioServerParameters = {
  command: string;
  args?: string[];
...
export class ACTAgentStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: ACTAgentStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "ACTAgentStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }

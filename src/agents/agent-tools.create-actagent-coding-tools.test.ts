/**
 * Broad coverage for createACTAgentCodingTools.
...
import "./test-helpers/fast-actagent-tools.js";
import { createACTAgentCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as actAgentPluginTools from "./actagent-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createACTAgentCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createACTAgentCodingTools>) {
...
type ACTAgentCodingTool = ReturnType<typeof createACTAgentCodingTools>[number];
type ACTAgentToolsOptions = NonNullable<Parameters<typeof createACTAgentTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: ACTAgentCodingTool[], name: string): ACTAgentCodingTool {
...
function requireToolExecute(tool: ACTAgentCodingTool): NonNullable<ACTAgentCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateACTAgentToolsOptions(): ACTAgentToolsOptions {
  const calls = vi.mocked(createACTAgentTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createACTAgentTools call");
  }
  return options;
}
...
describe("createACTAgentCodingTools", () => {
  const testConfig: ACTAgentConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createACTAgentCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-hook-channel-"));
  ...
  it("keeps Tool Search controls when core ACTAgent tools are not materialized", () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
    ...
    expect(createACTAgentToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateACTAgentToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:actagent"], ["*"]]) {
      const tools = createACTAgentCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to ACTAgent tool construction", () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateACTAgentToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into ACTAgent tool construction plans", () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateACTAgentToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the ACTAgent core factory", () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(actAgentPluginTools, "resolveACTAgentPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateACTAgentToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to ACTAgent tool factory planning", () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateACTAgentToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to ACTAgent plugin discovery", async () => {
    const createACTAgentToolsMock = vi.mocked(createACTAgentTools);
    createACTAgentToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `actagent-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateACTAgentToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createACTAgentToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateACTAgentToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("actagent-tools:test-helper")).toBeLessThan(
      stages.indexOf("actagent-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createACTAgentCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createACTAgentCodingTools({
      config: testConfig,
    ...
    const codexTools = createACTAgentCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: ACTAgentConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createACTAgentCodingTools({
    ...
    const anthropicTools = createACTAgentCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: ACTAgentConfig = {
      tools: {
        exec: {
    ...
    const allowed = createACTAgentCodingTools({
    ...
    const denied = createACTAgentCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createACTAgentCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createACTAgentCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createACTAgentCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createACTAgentCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createACTAgentCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createACTAgentCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createACTAgentCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createACTAgentCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createACTAgentCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createACTAgentCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createACTAgentCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-read-"));
    try {
      ...
      const contents = "Hello from actagent read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "actagent-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "actagent-workspace"),
    ...
    const tools = createACTAgentCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-canonical-"));
    try {
      const tools = createACTAgentCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-legacy-alias-"));
    try {
      const tools = createACTAgentCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-structured-write-"));
    try {
      const tools = createACTAgentCodingTools({ workspaceDir: tmpDir });
      ...
      await expect(
        writeExecute("tool-structured-write", {
          ...
          content: [
            ...
            { type: "input_text", text: "const root = path.join(process.env.HOME, 'actagentd');\n" },
          ],
        }),
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-structured-edit-"));
    try {

...
const EXEC_SHELL_SNAPSHOT_ENV = "ACTAGENT_EXEC_SHELL_SNAPSHOT";
...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-disabled-state-"));
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-disabled-home-"));
    ...
    expect(wrapped).toBe(command);
    expect(fs.existsSync(resolveShellSnapshotDir({ ACTAGENT_STATE_DIR: stateDir }))).toBe(false);
  });
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const trustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "actagent-snapshot-trusted-state-"),
    );
    const untrustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "actagent-snapshot-untrusted-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-state-home-"));
    const untrustedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "actagent-snapshot-untrusted-home-"),
    );
    ...
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      ...
      env: {
        ...
        ACTAGENT_STATE_DIR: untrustedStateDir,
    ...
    expect(fs.existsSync(resolveShellSnapshotDir({ ACTAGENT_STATE_DIR: untrustedStateDir }))).toBe(
      false,
    );
    expect(fs.existsSync(resolveShellSnapshotDir({ ACTAGENT_STATE_DIR: trustedStateDir }))).toBe(
      true,
    );
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-cwd-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '[ "$ACTAGENT_SHELL" = exec ] && alias oc_snap_exec_alias="printf marker-ok"',
  ...
  it("captures bash aliases behind common interactive-only guards", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-interactive-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-interactive-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-interactive-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("preserves per-call safe env overrides after trusted capture", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("preserves per-call env outside the snapshot allowlist", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-plugin-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-plugin-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-plugin-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("does not let non-fingerprinted env change captured shell state", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-branch-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-branch-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-branch-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("refreshes stale snapshot files when startup files source alias fragments", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-refresh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-refresh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-refresh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refreshes fresh cached snapshots that no longer parse", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-corrupt-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-corrupt-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-corrupt-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    ...
    fs.writeFileSync(
      snapshotPath,
      [
        "# ACTAgent exec shell snapshot. Generated; do not edit.",
  ...
  it("refuses to persist aliases or functions with literal secret-looking values", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-secret-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-secret-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-secret-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("captures zsh aliases in sourceable form", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zsh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zsh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zsh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
  ...
  it("captures zsh startup state from ZDOTDIR", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zdot-home-"));
    const zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zdot-dir-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zdot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-snapshot-zdot-cwd-"));

// Verifies OpenAI model selections route between ACTAgent and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  it("keeps explicit ACTAgent plus Codex auth profile under the unified OpenAI provider", () => {
  ...
  it("keeps explicit OpenAI ACTAgent API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible ACTAgent configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {

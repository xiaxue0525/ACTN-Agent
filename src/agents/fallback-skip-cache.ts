 ...
 * ACTAGENT_FALLBACK_SKIP_TTL_MS.
 */
export const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "ACTAGENT_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    actagentFallbackSkipCache?: SkipBySession;
    actagentFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.actagentFallbackSkipCacheState) {
    ...
    const buckets = globalStore.actagentFallbackSkipCache ?? new Map();
    globalStore.actagentFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.actagentFallbackSkipCache = buckets;
  }
  return globalStore.actagentFallbackSkipCacheState;
}

...
let ensureACTAgentModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    ensureACTAgentModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureACTAgentModelsJson: ensureACTAgentModelsJsonMock,
    }));
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: ACTAgentConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/actagent-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/actagent",
      resolveDefaultAgentId: (cfg: ACTAgentConfig) =>
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as ACTAgentConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as ACTAgentConfig });
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as ACTAgentConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as ACTAgentConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/actagent/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/actagent/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/actagent/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest model id policies once for discovered catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as ACTAgentConfig, useCache: false });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as ACTAgentConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/actagent-model-catalog-env" } as NodeJS.ProcessEnv;

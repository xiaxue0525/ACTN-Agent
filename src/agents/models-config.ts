...
import { planACTAgentModelsJson } from "./models-config.plan.js";
import {
...
function resolveModelsConfigInput(config?: ACTAgentConfig): {
  config: ACTAgentConfig;
  sourceConfigForSecrets: ACTAgentConfig;
} {
...
export async function ensureACTAgentModelsJson(
  config?: ACTAgentConfig,
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planACTAgentModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,

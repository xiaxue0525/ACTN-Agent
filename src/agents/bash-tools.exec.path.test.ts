...
const ENV_KEYS = ["ACTAGENT_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("ACTAGENT_SHELL")) {
        input.onStdout?.(env.ACTAGENT_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ACTAGENT_EXEC_SHELL_SNAPSHOT = "0";
  ...
  it("strips malformed XML arg-value suffixes from exec command and routing options", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-exec-xml-"));
    try {
  ...
  it("sets ACTAGENT_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-actagent-shell", {
      command: 'printf "%s" "${ACTAGENT_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/actagent-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/actagent-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a actagent /approve abc123 deny",
    ...
    "sudo ACTAGENT_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root ACTAGENT_APPROVE=1 bash -lc '/approve abc123 allow-once'",

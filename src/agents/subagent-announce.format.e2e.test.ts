...
let configOverride: ACTAgentConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: ACTAgentConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/actagent/actagent/issues/31298
    previousFastTestEnv = process.env.ACTAGENT_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ACTAGENT_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // ACTAGENT_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("ACTAgent runtime context (internal):");

...
const CLEANUP_STATE_KEY = Symbol.for("actagent.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("actagent.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("actagent.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "ACTAGENT_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "ACTAGENT_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "ACTAGENT_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.ACTAGENT_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isACTAgentSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "actagent" || exe.endsWith("/actagent") || exe.endsWith("/actagent-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "actagent" ||
        arg.endsWith("/actagent") ||
        arg === "actagent.mjs" ||
        arg.endsWith("/actagent.mjs"),
    )
  ) {
    return true;
  }
  ...
  const hasACTAgentCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasACTAgentCommandToken,
  );
}
...
function shouldTreatAsNonACTAgentOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isACTAgentSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-actagent-owner"],
    };
  }

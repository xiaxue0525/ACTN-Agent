// Verifies ACTAgent gateway tool schema, restart signaling, and config mutations.
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-test-"));

    try {
      await withEnvAsync(
        { ACTAGENT_STATE_DIR: stateDir, ACTAGENT_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Recommended follow-up: run actagent --profile isolated doctor --non-interactive in a terminal or approvals-capable ACTAgent surface.",
          );
        },
      );
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/actagent-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/actagent-bin"] } } }',
      }),
    ).rejects.toThrow(

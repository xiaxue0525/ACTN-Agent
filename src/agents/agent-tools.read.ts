...
import { detectMime } from "@actagent/media-core/mime";
...
function resolveAdaptiveReadMaxBytes(options?: ACTAgentReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".actagent/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.actagent/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with ACTAgent result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createACTAgentReadTool(base, {
...
/** Wrap the base read tool with ACTAgent paging, MIME, and image handling. */
export function createACTAgentReadTool(
  base: AnyAgentTool,
  options?: ACTAgentReadToolOptions,
): AnyAgentTool {
  return {
    ...base,

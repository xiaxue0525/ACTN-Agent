...
let testState: ACTAgentTestState | undefined;

beforeEach(async () => {
  testState = await createACTAgentTestState({
    layout: "state-only",
    prefix: "actagent-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ACTAGENT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ACTAGENT_HOME: "/srv/actagent-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/actagent-home"), ".actagent", "workspace"));
  });

  it("prefers ACTAGENT_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ACTAGENT_WORKSPACE_DIR: "/srv/actagent-workspace",
      ACTAGENT_HOME: "/srv/actagent-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/actagent-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".actagent", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("actagent-workspace-");
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".actagent"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an ACTAgent attestation marker", async () => {
    const tempDir = await makeTempWorkspace("actagent-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-workspace-hardlink-"));
    try {

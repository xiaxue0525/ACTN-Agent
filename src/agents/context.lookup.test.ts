...
vi.mock("./models-config.runtime.js", () => ({
  ensureACTAgentModelsJson: contextTestState.ensureACTAgentModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureACTAgentModelsJson.mockClear();
  return { ensureACTAgentModelsJson: contextTestState.ensureACTAgentModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(
      path.normalize(discoverAgentDir).endsWith(path.join(".actagent", "agents", "main", "agent")),
    ).toBe(true);

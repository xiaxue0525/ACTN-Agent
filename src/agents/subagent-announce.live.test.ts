...
import {
  createACTAgentTestState,
  type ACTAgentTestState,
} from "../test-utils/actagent-test-state.js";
...
const LIVE = isLiveTestEnabled() && isTruthyEnvValue(process.env.ACTAGENT_LIVE_SUBAGENT_E2E);
...
type LiveSubagentModelProviders = NonNullable<NonNullable<ACTAgentConfig["models"]>["providers"]>;

function resolveLiveSubagentModelConfig(): LiveSubagentModelConfig {
  const modelKey = process.env.ACTAGENT_LIVE_SUBAGENT_E2E_MODEL?.trim() || "openai/gpt-5.5";
...
function liveSubagentConfig(
  ...
  token: string,
  options?: {
    queue?: NonNullable<ACTAgentConfig["messages"]>["queue"];
    toolAllow?: string[];
  },
): ACTAgentConfig {
  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: { [modelKey]: { agentRuntime: { id: "actagent" }, params: { maxTokens: 1024 } } },
        sandbox: { mode: "off" },
        subagents: {
          allowAgents: ["*"],
...
describeLive("subagent announce live", () => {
  let state: ACTAgentTestState | undefined;
  ...
  it(
    ...
    async () => {
      if (!isTruthyEnvValue(process.env.ACTAGENT_SUBAGENT_ISSUE_82913_REPRO)) {
        console.warn(
          "[issue-82913] skip: set ACTAGENT_SUBAGENT_ISSUE_82913_REPRO=1 to run this focused repro",
        );
        return;
      }
      ...
      state = await createACTAgentTestState({
        ...
        env: {
          ...
          ACTAGENT_DISABLE_PERSISTED_PLUGIN_REGISTRY: "1",
          ...
          ACTAGENT_PLUGINS_PATHS: undefined,
        },
      });
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 240,
          message: [
            "Run this exact ACTAgent busy-parent subagent scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
    ...
    async () => {
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 180,
          message: [
            "Run this exact ACTAgent subagent steering scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
            `Step 1: call sessions_spawn with exactly this JSON input: ${JSON.stringify({
              task: childTask,
  ...
  it(
    ...
    async () => {
      ...
      if (!modelConfig.modelKey.startsWith("google/")) {
        console.warn(
          "[subagent-stress] skip: set ACTAGENT_LIVE_SUBAGENT_E2E_MODEL=google/gemini-3.1-pro-preview",
        );
        return;
      }
      requireLiveSubagentAuth(modelConfig);
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({ name: "actagent-gemini-stress-live", private: true }, null, 2)}\n`,
        "utf8",
      );
      await fs.writeFile(
        path.join(state.workspaceDir, "AGENTS.md"),
        "ACTAgent live stress test workspace. Keep responses concise.\n",
        "utf8",
      );
      await state.writeConfig(
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 420,
          message: [
            "Run this exact ACTAgent Gemini subagent stress scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
              ...
              return `Call sessions_spawn for child ${childNumber} with exactly this JSON input: ${JSON.stringify(
                {
                  task: [
                    ...
                    "Then run a tiny shell command if the bash tool is available: printf actagent.",

...
function createClaudeCliOverrideConfig(config: CliBackendConfig): ACTAgentConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from ACTAgent's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__actagent__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__actagent__*");
  ...
  it("keeps Claude permission mode unset when ACTAgent exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {

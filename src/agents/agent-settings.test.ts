...
describe("isSilentOverflowProneModel", () => {
  ...
  // actagent#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable ACTAgent runtime's secondary compaction path;
  // ACTAgent's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of actagent#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering ACTAgent runtime's
  ...
  // state.messages intact; ACTAgent's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep ACTAgent runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove ACTAgent runtime's

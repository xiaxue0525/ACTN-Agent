...
type SubagentRunsTable = ACTAgentStateKyselyDatabase["subagent_runs"];
type SubagentRegistryDatabase = Pick<ACTAgentStateKyselyDatabase, "subagent_runs">;
...
function readSubagentRegistryRows(): SubagentRunSqliteRow[] {
  const { db } = openACTAgentStateDatabase();

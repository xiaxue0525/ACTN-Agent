/**
 * ACTAgent built-in and plugin tool assembly.
 *
...
import { resolveACTAgentPluginToolsForOptions } from "./actagent-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./actagent-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./actagent-tools.nodes-workspace-guard.js";
import {
  collectPresentACTAgentTools,
...
type ACTAgentToolsDeps = {
  callGateway: typeof callGateway;
  config?: ACTAgentConfig;
};

const defaultACTAgentToolsDeps: ACTAgentToolsDeps = {
  callGateway,
};

let actAgentToolsDeps: ACTAgentToolsDeps = defaultACTAgentToolsDeps;

export function createACTAgentTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? actAgentToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("actagent-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("actagent-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("actagent-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("actagent-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("actagent-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("actagent-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("actagent-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("actagent-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("actagent-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("actagent-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : actAgentToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForACTAgentTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentACTAgentTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentACTAgentTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: actAgentToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentACTAgentTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("actagent-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveACTAgentPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("actagent-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("actagent-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<ACTAgentToolsDeps>) {
    actAgentToolsDeps = overrides
      ? {
          ...defaultACTAgentToolsDeps,
          ...overrides,
        }
      : defaultACTAgentToolsDeps;
  },

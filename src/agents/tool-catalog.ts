...
type CoreToolDefinition = {
  ...
  includeInACTAgentGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const actagentTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInACTAgentGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:actagent": actagentTools,

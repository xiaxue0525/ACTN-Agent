...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-managed-media-"));
  try {
    return await withEnvAsync({ ACTAGENT_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInACTAgentTmp<T>(
  params: {
    actAgentTmpDir: string;
  ...
  const hardlinkPath = path.join(params.actAgentTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.actAgentTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.actAgentTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.actAgentTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const actAgentTmpDir = resolvePreferredACTAgentTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred ACTAgent tmp root",
      media: path.join(actAgentTmpDir, "image.png"),
      expected: path.join(actAgentTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred ACTAgent tmp root",
      media: pathToFileURL(path.join(actAgentTmpDir, "photo.png")).href,
      expected: path.join(actAgentTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred ACTAgent tmp root",
      media: path.join(actAgentTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(actAgentTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(actAgentTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside actagent tmp root",
      media: path.join(os.tmpdir(), "outside-actagent", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked ACTAgent tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(actAgentTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(actAgentTmpDir, { recursive: true });
      const symlinkPath = path.join(actAgentTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked ACTAgent tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInACTAgentTmp(
      {
        actAgentTmpDir,
  ...
  it("rejects symlinked ACTAgent tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInACTAgentTmp(

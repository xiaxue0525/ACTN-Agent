...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.ACTAGENT_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.ACTAGENT_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "actagent" || forcedRuntime === "codex") {

...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/actagent-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): ACTAgentConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: ACTAgentConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planACTAgentModelsJsonWithDeps>>;
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planACTAgentModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/actagent-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planACTAgentModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/actagent-models-config-env-vars-test",
        env: {},
  ...
  it("keeps google-vertex static catalog rows when an auth profile supplies the API key", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-google-vertex-models-"));
    try {

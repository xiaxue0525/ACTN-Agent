...
function createDefaultSpawnConfig(): ACTAgentConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: ACTAgentConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects ACTAgent config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      acp: {
        enabled: true,
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an ACTAgent config agent, not an ACP harness. Use runtime="subagent" or omit runtime for ACTAgent config agents. Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps ACTAgent ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
  ...
  it("includes cwd in ACP thread intro banner when provided at spawn time", async () => {
    ...
    expect(metadata.introText).toContain("cwd: /home/bob/actagentd");
  });

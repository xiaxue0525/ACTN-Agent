...
describe("provider attribution", () => {
  it("resolves the canonical ACTAgent product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in ACTAgent runtime wrapper.",
      product: "ACTAgent",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "X-BILLING-INVOKE-ORIGIN": "ACTAgent",
      },
    });
    expect(resolveProviderAttributionHeaders("NVIDIA", { ACTAGENT_VERSION: "2026.3.22" })).toEqual({
      "X-BILLING-INVOKE-ORIGIN": "ACTAgent",
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ACTAGENT_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { ACTAGENT_VERSION: "2026.3.22" })).toEqual({
      originator: "actagent",
      version: "2026.3.22",
      "User-Agent": "actagent/2026.3.22",
    });
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { ACTAGENT_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard actagent User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "ACTAgent",
      version: "2026.3.22",
      headers: {
        originator: "actagent",
    ...
    expect(resolveProviderAttributionHeaders("xai", { ACTAGENT_VERSION: "2026.3.22" })).toEqual({
      originator: "actagent",
      version: "2026.3.22",
      "User-Agent": "actagent/2026.3.22",
    });
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ ACTAGENT_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,

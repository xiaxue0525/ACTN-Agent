...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "ACTAGENT_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { ACTAGENT_LIVE_MODEL_IMAGE_PROBE: "false" },
        "ACTAGENT_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { ACTAGENT_LIVE_MODEL_IMAGE_PROBE: "1" },
        "ACTAGENT_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });

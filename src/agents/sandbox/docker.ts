...
import { markACTAgentExecEnv } from "../../infra/actagent-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; ACTAgent will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "actagent.sandbox=1");
  args.push("--label", `actagent.sessionKey=${params.scopeKey}`);
  args.push("--label", `actagent.createdAtMs=${createdAtMs}`);
  args.push("--label", `actagent.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `actagent.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markACTAgentExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "actagent.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`actagent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`actagent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("actagent sandbox recreate --all");
}

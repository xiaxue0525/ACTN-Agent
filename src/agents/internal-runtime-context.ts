 ...
 */
/** Opening delimiter for protected ACTAgent runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_ACTAGENT_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected ACTAgent runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_ACTAGENT_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[ACTAGENT_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[ACTAGENT_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const ACTAGENT_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const ACTAGENT_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "ACTAgent runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const ACTAGENT_RUNTIME_EVENT_HEADER = "ACTAgent runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const ACTAGENT_RUNTIME_CONTEXT_CUSTOM_TYPE = "actagent.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["ACTAgent runtime context (internal):", ACTAGENT_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === ACTAGENT_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === ACTAGENT_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === ACTAGENT_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${ACTAGENT_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${ACTAGENT_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${ACTAGENT_RUNTIME_EVENT_HEADER}\n${ACTAGENT_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isACTAgentRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === ACTAGENT_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isACTAgentRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isACTAgentRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isACTAgentRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isACTAgentRuntimeContextCustomMessage(message)) {
      return true;
    }

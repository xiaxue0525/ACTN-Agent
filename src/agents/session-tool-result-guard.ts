 ...
 */
import { resolveIntegerOption } from "@actagent/normalization-core/number-coercion";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000ACTAGENT_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[ACTAgent persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[ACTAgent persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[ACTAgent persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[ACTAgent persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[ACTAgent persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyACTAgentAssistantMessage(message: AgentMessage): boolean {
  ...
  return provider === "actagent" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyACTAgentAssistantMessage(nextMessage);
    if (

...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: ACTAgentConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<ACTAgentConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<ACTAgentConfig["agents"]>["list"]>[number]["tools"];
  }): ACTAgentConfig {
    return {
      tools: params.tools,
      agents: {

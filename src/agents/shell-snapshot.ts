...
const CAPTURE_MARKER = "__ACTAGENT_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__ACTAGENT_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // ACTAGENT_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.ACTAGENT_SHELL === "exec") {
    env.ACTAGENT_SHELL = "exec";
  }
  return env;
}

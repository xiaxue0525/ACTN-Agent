...
function resolveAttachmentLimits(config: ACTAgentConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".actagent", "attachments");
  const relDir = path.posix.join(".actagent", "attachments", attachmentId);

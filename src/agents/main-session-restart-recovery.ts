...
export async function markRestartAbortedMainSessions(params: {
  cfg?: ACTAgentConfig;
  additionalCfgs?: Iterable<ACTAgentConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, ACTAGENT_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is ACTAgentConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "ACTAgent was waiting on tool/model work. Continue from the existing " +

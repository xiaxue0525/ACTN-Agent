 ...
 * private ACTAgent state directory with one hashed file per MCP server URL.
 */
...
/** Creates the MCP SDK OAuth provider backed by ACTAgent's private store. */
export function createMcpOAuthClientProvider(params: {
  ...
  const assertAuthorizationRedirectAllowed = () => {
    if (!allowAuthorizationRedirect) {
      throw new Error(
        `MCP server "${params.serverName}" requires OAuth authorization. Run actagent mcp login ${params.serverName}.`,
      );
    }
  };
  return {

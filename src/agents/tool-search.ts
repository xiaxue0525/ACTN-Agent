 ...
 *
 * Presents large ACTAgent/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "actagent" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    actagent: { value: controller.actagent, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("actagent.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: ACTAgentConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: ACTAgentConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "actagent", sourceName: pluginId };
  }
  return { source: "actagent", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with actagent.tools.search, actagent.tools.describe, and actagent.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The actagent.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through ACTAgent.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(

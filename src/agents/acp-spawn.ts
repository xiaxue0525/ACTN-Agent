...
} from "@actagent/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@actagent/acp-core/runtime/types";
import {
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: ACTAgentConfig;
  agentId: string;
}): NonNullable<NonNullable<ACTAgentConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an ACTAgent config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for ACTAgent config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: ACTAgentConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: ACTAgentConfig): string[] {

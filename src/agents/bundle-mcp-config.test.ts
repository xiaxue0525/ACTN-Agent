/** Tests merging bundled MCP defaults with ACTAgent user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets ACTAgent mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps ACTAgent transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled ACTAgent MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled ACTAgent MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {

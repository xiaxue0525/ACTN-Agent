...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to ACTAgent transport aliases when needed.
    const cases = [
      {
        ...
        alias: "actagent-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "actagent-openai-completions-transport",
      },
      {
        ...
        alias: "actagent-azure-openai-responses-transport",
      },
      {
        ...
        alias: "actagent-anthropic-messages-transport",
      },
      {
        ...
        alias: "actagent-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on ACTAgent transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createACTAgentTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the ACTAgent simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("actagent-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the ACTAgent transport until ACTAgent preserves attribution", () => {

...
const LIVE = isLiveTestEnabled(["ACTAGENT_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "actagent-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("actagent_live_probe", "ACTAgent Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an ACTAgent-configured streamable-http server", async () => {
    const geminiCommand = process.env.ACTAGENT_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("actagentLiveProbe");

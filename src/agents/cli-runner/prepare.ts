...
const prepareDeps = {
  ...
  resolveACTAgentReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveACTAgentReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveACTAgentReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ACTAGENT_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          ACTAGENT_MCP_AGENT_ID: sessionAgentId ?? "",
          ACTAGENT_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          ACTAGENT_MCP_SESSION_KEY: params.sessionKey ?? "",
          ACTAGENT_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          ACTAGENT_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          ACTAGENT_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          ACTAGENT_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          ACTAGENT_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          ACTAGENT_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          ACTAGENT_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  let actAgentHistoryMessages: unknown[] | undefined;
  const loadACTAgentHistoryMessages = async () => {
    actAgentHistoryMessages ??= await loadCliSessionHistoryMessages({
      sessionId: params.sessionId,
    ...
    return actAgentHistoryMessages;
  };
  ...
  const actAgentReferences = await prepareDeps.resolveACTAgentReferencePaths({
    workspaceDir,
  ...
  const builtSystemPrompt = buildCliAgentSystemPrompt({
    ...
    heartbeatPrompt,
    docsPath: actAgentReferences.docsPath ?? undefined,
    sourcePath: actAgentReferences.sourcePath ?? undefined,
  ...
  try {
    const hookResult = await resolvePromptBuildHookResult({
      ...
      messages: await loadACTAgentHistoryMessages(),
      hookCtx: {
        runId: params.runId,
        agentId: sessionAgentId,
        sessionKey: params.sessionKey,
  ...
  const shouldPrepareACTAgentHistoryPrompt =
    !reusableCliSession.sessionId || allowRawTranscriptReseed;
  const actAgentHistoryPrompt = shouldPrepareACTAgentHistoryPrompt
    ? buildCliSessionHistoryPrompt({
  ...
  try {
    ...
    return {
      ...
      ...(actAgentHistoryPrompt ? { actAgentHistoryPrompt } : {}),
      heartbeatPrompt,
      authEpoch,
      authEpochVersion: CLI_AUTH_EPOCH_VERSION,
      extraSystemPromptHash,

...
const CLI_BACKEND_PRESERVE_ENV = "ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ACTAGENT_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ACTAGENT_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ACTAGENT_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ACTAGENT_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ACTAGENT_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.actAgentHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.actAgentHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (env.ACTAGENT_MCP_TOKEN || env.ACTAGENT_MCP_SESSION_KEY || env.ACTAGENT_MCP_AGENT_ID) {
        ...
        if (result.exitCode !== 0 || result.reason !== "exit") {
          if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
            ...
            const deferWatchdogNoticeForFreshRetry =
              ...
              Boolean(context.actAgentHistoryPrompt) &&

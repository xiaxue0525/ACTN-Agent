/**
 * Materializes selected ACTAgent skills as a temporary Claude CLI plugin.
 */
...
const ACTAGENT_CLAUDE_PLUGIN_NAME = "actagent-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredACTAgentTmpDir(), "actagent-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ACTAGENT_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: ACTAGENT_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped ACTAgent skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(

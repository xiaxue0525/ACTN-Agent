...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["ACTAGENT_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("actagent-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("actagent-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("actagent-cli-bundle-mcp-bundled-");
    process.env.ACTAGENT_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): ACTAgentConfig {
  return {
    plugins: {
      entries: {

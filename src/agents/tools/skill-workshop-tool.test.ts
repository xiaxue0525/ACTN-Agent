...
let testState: ACTAgentTestState;
let stateDir = "";

beforeEach(async () => {
  testState = await createACTAgentTestState({
    layout: "state-only",
    prefix: "actagent-skill-workshop-state-",
  });
...
describe("skill_workshop tool", () => {
  it("is exposed in the ACTAgent tool set", async () => {
    const workspaceDir = await tempDirs.make("actagent-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed ACTAgent tool sets", async () => {
    const workspaceDir = await tempDirs.make("actagent-skill-workshop-tool-");
    const tools = createACTAgentTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("actagent-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("actagent-skill-workshop-tool-second-");

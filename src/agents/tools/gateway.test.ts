...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    actagent: process.env.ACTAGENT_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.actagent === undefined) {
      delete process.env.ACTAGENT_GATEWAY_TOKEN;
    } else {
      process.env.ACTAGENT_GATEWAY_TOKEN = envSnapshot.actagent;
    }
  });
  ...
  it("accepts allowlisted gatewayUrl overrides (SSRF hardening)", async () => {
    ...
    await callGatewayTool(
      "health",
      { gatewayUrl: "ws://127.0.0.1:19199", gatewayToken: "t", timeoutMs: 5000 },
      {},
    );
    const call = capturedGatewayCall();
    expect(call.method).toBe("health");
    expect(call.params).toEqual({});
    expect(call.url).toBe("ws://127.0.0.1:19199");
  ...
  it("uses ACTAGENT_GATEWAY_TOKEN for allowlisted local overrides", () => {
    process.env.ACTAGENT_GATEWAY_TOKEN = "env-token";
    const opts = resolveGatewayOptions({ gatewayUrl: "ws://127.0.0.1:19199" });
    expect(opts.url).toBe("ws://127.0.0.1:19199");
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    ...
    process.env.ACTAGENT_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
  ...
  it("does not send the local approval runtime token to gatewayUrl overrides", async () => {
    ...
    await callGatewayTool(
      ...
      { gatewayUrl: "ws://127.0.0.1:19199", gatewayToken: "t" },
      { id: "approval-id" },
    );

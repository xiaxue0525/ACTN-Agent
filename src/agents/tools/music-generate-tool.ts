...
function hasExplicitMusicGenerationModelConfig(cfg?: ACTAgentConfig): boolean {
...
export function createMusicGenerateTool(options?: {
  ...
  const cfg: ACTAgentConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,

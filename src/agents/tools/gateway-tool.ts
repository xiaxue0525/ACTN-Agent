...
import { isRecord as isPlainObject } from "@actagent/normalization-core/record-coerce";
import {
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `actagent security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as ACTAgentConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as ACTAgentConfig);

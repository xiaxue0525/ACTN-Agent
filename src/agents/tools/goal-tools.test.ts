...
async function createStoreConfig(): Promise<{ config: ACTAgentConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as ACTAgentConfig,
    template,
  };
}

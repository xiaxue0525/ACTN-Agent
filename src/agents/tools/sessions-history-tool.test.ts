...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.ACTAGENT_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.ACTAGENT_CONFIG_PATH = previousConfigPath;
    }
    if (tempDir) {

...
import type { ExtensionAPI, ExtensionContext } from "actagent/plugin-sdk/agent-sessions";
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("extracts opaque identifiers and audits summary quality", () => {
    const identifiers = extractOpaqueIdentifiers(
      "Track id a1b2c3d4e5f6 plus A1B2C3D4E5F6 and URL https://example.com/a and /tmp/x.log plus port host.local:19199",
    );
  ...
  it("filters ordinary short numbers and trims wrapped punctuation", () => {
    const identifiers = extractOpaqueIdentifiers(
      "Year 2026 count 42 port 19199 ticket 123456 URL https://example.com/a, path /tmp/x.log, and tiny /a with prose on/off.",
    );

    expect(identifiers).not.toContain("2026");
    expect(identifiers).not.toContain("42");
    expect(identifiers).not.toContain("19199");
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("actagent.runtime-context");
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-compaction-workspace-"));

// Verifies ACTAgent-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "actagent/plugin-sdk/agent-core";
import {
  installACTAgentOwnedToolHooks,
  resetACTAgentOwnedToolHooks,
  textToolResult,
} from "actagent/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "actagent/plugin-sdk/agent-sessions";
...
describe("ACTAgent-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetACTAgentOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installACTAgentOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installACTAgentOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/actagent-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/actagent-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/actagent-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installACTAgentOwnedToolHooks({ blockReason: "blocked by policy" });

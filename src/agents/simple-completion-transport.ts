 ...
 *
 * Registers provider-specific stream functions and rewrites models that need ACTAgent-managed transport semantics.
 */
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `actagent-anthropic-vertex-simple:${suffix}`;
}
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use ACTAgent's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createACTAgentTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }

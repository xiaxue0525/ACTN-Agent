...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: ACTAgentConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/actagent" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ACTAgentConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ACTAgentConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: ACTAgentConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: ACTAgentConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: ACTAgentConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/actagent");
  });

  it("uses ACTAGENT_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "actagent-home");
    withEnv({ ACTAGENT_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as ACTAgentConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".actagent", "workspace"));
    });
  });

  it("uses ACTAGENT_WORKSPACE_DIR for default agent workspace", () => {
    const workspaceDir = path.join(path.sep, "srv", "actagent-workspace");
    withEnv(
      {
        ACTAGENT_WORKSPACE_DIR: workspaceDir,
        ACTAGENT_HOME: path.join(path.sep, "srv", "actagent-home"),
      },
      () => {
        const workspace = resolveAgentWorkspaceDir({} as ACTAgentConfig, "main");
  ...
  it("uses ACTAGENT_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "actagent-home");
    withEnv({ ACTAGENT_HOME: home, ACTAGENT_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as ACTAgentConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".actagent", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ ACTAGENT_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ ACTAGENT_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/actagent-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/actagent-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-agent-scope-"));

...
describe("MCP OAuth provider", () => {
  it("stores token state under the ACTAgent state directory with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const tokenDir = `${home}/.actagent/mcp-oauth`;
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(provider.state?.()).rejects.toThrow("Run actagent mcp login Remote Docs.");
        await expect(provider.saveCodeVerifier?.("verifier")).rejects.toThrow(
          "Run actagent mcp login Remote Docs.",
        );
        await expect(
          provider.redirectToAuthorization?.(new URL("https://auth.example.com/authorize")),
        ).rejects.toThrow("Run actagent mcp login Remote Docs.");
      },
      {
        prefix: "actagent-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          ACTAGENT_CONFIG_PATH: undefined,
          ACTAGENT_STATE_DIR: undefined,
  ...
  it("clears stored credentials for a configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "actagent-mcp-oauth-clear-",
        skipSessionCleanup: true,
        env: {
          ACTAGENT_CONFIG_PATH: undefined,
          ACTAGENT_STATE_DIR: undefined,

...
function hasConfiguredProvider(params: { cfg?: ACTAgentConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
function isCliAgentRuntime(runtime: string | undefined, cfg: ACTAgentConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale ACTAgent
...
export function resolveImageFallbackDefaultProvider(cfg: ACTAgentConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: ACTAgentConfig | undefined): unknown {

/**
 * ACTAgent-owned tool registration filters.
 *
...
/**
 * Registration helpers for optional ACTAgent-owned tools.
 *
...
export function collectPresentACTAgentTools(
...
export function isUpdatePlanToolEnabledForACTAgentTools(params: {
...
function mergeACTAgentToolPolicyList(...lists: Array<string[] | undefined>): string[] | undefined {
  const merged = lists.flatMap((list) => (Array.isArray(list) ? list : []));
  return merged.length > 0 ? uniqueStrings(merged) : undefined;
}

function isToolExplicitlyAllowedByACTAgentToolPolicy(params: {
  toolName: string;
  allowlist?: string[];
  denylist?: string[];
}): boolean {
...
/** Decides whether update_plan should be included in the assembled ACTAgent tool set. */
export function shouldIncludeUpdatePlanToolForACTAgentTools(params: {
  config?: ACTAgentConfig;
  agentSessionKey?: string;
  agentId?: string | null;
  modelProvider?: string;

...
import { resolveOptionalMediaToolFactoryPlan } from "./actagent-tools.media-factory-plan.js";
...
type CreateACTAgentToolsOptions = Parameters<
  typeof import("./actagent-tools.js").createACTAgentTools
>[0];
let createACTAgentToolsForTestModule: typeof import("./actagent-tools.js").createACTAgentTools;
let legacyComfyToolNames: string[];

async function createACTAgentToolsForTest(options?: CreateACTAgentToolsOptions) {
  return createACTAgentToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/actagent.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): ACTAgentConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createACTAgentTools: createACTAgentToolsForTestModule } =
      await import("./actagent-tools.js"));
    ...
    legacyComfyToolNames = (
      await createACTAgentToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: ACTAgentConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createACTAgentToolsForTest({
      config,
      agentDir: "/tmp/actagent-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: ACTAgentConfig = {};
    const workspaceDir = "/tmp/actagent-workspace";

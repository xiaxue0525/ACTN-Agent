...
function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "actagent-agent-cache-"));
}
...
describe("SQLite agent cache store", () => {
  it("stores scoped JSON values and blobs in the agent database", () => {
    const env = { ACTAGENT_STATE_DIR: createTempStateDir() };
  ...
  it("hides invalid persisted expiries and ignores invalid clear clocks", () => {
    ...
    const database = openACTAgentAgentDatabase({ agentId: "main", env });
    database.db
  ...
  it("honors explicit per-agent database paths", () => {
    ...
    expect(
      listACTAgentRegisteredAgentDatabases({ env }).find((entry) => entry.path === dbPath),
    ).toMatchObject({
      agentId: "worker",
      path: dbPath,
    });

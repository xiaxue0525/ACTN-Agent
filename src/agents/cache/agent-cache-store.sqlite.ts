...
import {
  ...
  type ACTAgentAgentDatabaseOptions,
...
export type SqliteAgentCacheStoreOptions = ACTAgentAgentDatabaseOptions & {
  scope: string;
  now?: () => number;
};
...
type CacheEntriesTable = ACTAgentAgentKyselyDatabase["cache_entries"];
type AgentCacheDatabase = Pick<ACTAgentAgentKyselyDatabase, "cache_entries">;
...
function toDatabaseOptions(options: SqliteAgentCacheStoreOptions): ACTAgentAgentDatabaseOptions {
  return {
...
export function writeSqliteAgentCacheEntry(
  ...
  runACTAgentAgentWriteTransaction((database) => {
...
export function readSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  const database = openACTAgentAgentDatabase(toDatabaseOptions(options));
  const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
  const row =
    executeSqliteQueryTakeFirstSync(
      database.db,
...
export function deleteSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  return runACTAgentAgentWriteTransaction((database) => {
    const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
...
/** Agent runtime cache store implementation backed by ACTAgent's SQLite DB. */

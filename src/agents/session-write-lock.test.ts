...
import { MAX_TIMER_TIMEOUT_MS } from "@actagent/normalization-core/number-coercion";
...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live ACTAgent-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "actagent"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ACTAGENT_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            ACTAGENT_SESSION_WRITE_LOCK_STALE_MS: "60000",
            ACTAGENT_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ACTAGENT_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            ACTAGENT_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            ACTAGENT_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          ACTAGENT_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          ACTAGENT_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/actagent/actagent.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { ACTAGENT_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/actagent/actagent.mjs", "doctor"],
      });
  ...
  it("does not clean live ACTAgent locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/actagent/actagent.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-ACTAgent processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-actagent.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-actagent.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-actagent-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-ACTAgent process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-actagent-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-ACTAgent entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with ACTAgent or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const actagentLock = path.join(sessionsDir, "actagent-live.jsonl.lock");
    ...
    try {
      ...
      const actagentResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(actagentResult.cleaned).toEqual([]);
      await expect(fs.access(actagentLock)).resolves.toBeUndefined();

      await fs.rm(actagentLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;

/**
 * Builds the effective ACTAgent agent tool surface.
 * Assembles core, shell, channel, ACTAgent, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: ACTAgentConfig; agentId?: string }) {
...
export type ACTAgentCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeACTAgentTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createACTAgentCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep ACTAgent web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: ACTAgentCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeACTAgentTools: includeCoreTools,
  ...
  const includeACTAgentTools = includeCoreTools && toolConstructionPlan.includeACTAgentTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createACTAgentReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeACTAgentTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("actagent-tools");
  ...
  // Always normalize tool JSON Schemas before handing them to ACTAgent model runtime.

...
export function resolveCompactionReserveTokensFloor(cfg?: ACTAgentConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: ACTAgentConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers ACTAgent runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (actagent#75799).
 *
...
/**
 * Disable ACTAgent runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when ACTAgent or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (actagent#75799).
 * Default-mode runs against ordinary providers keep ACTAgent runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores ACTAgent runtime's

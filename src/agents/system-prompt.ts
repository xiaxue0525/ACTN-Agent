/**
 * ACTAgent system prompt renderer.
 *
...
} from "@actagent/normalization-core/string-normalization";
...
import {
  buildACTAgentToolFallbackText,
  shouldRenderACTAgentToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__actagent__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__actagent__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__actagent__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; ACTAgent handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Do not use `message(action=send)` to deliver the current source-channel reply; reply normally so ACTAgent can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.actagent.ai",
    docsPath ? "Mirror: https://docs.actagent.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/actagent/actagent",
    docsPath
      ? "ACTAgent behavior/config/architecture: read local docs first."
      : "ACTAgent behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `actagent status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to ACTAgent main. */
  ...
  const promptSurface = params.promptSurface ?? "actagent_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running ACTAgent process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List ACTAgent agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List ACTAgent agent ids allowed for sessions_spawn",
  ...
  const renderACTAgentToolWorkflowHints = shouldRenderACTAgentToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.actagent/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.actagent/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside ACTAgent.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    promptSurface,
    toolLines,
    renderACTAgentToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside ACTAgent.",
      "",
      "## Tooling",
      ...
      "CLI lifecycle only on explicit user request: `actagent gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## ACTAgent Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, ACTAgent pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by ACTAgent and included below in Project Context.",
      "",

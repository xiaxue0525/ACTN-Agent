...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const actAgentTmpDir = path.resolve(resolvePreferredACTAgentTmpDir());
  if (!isPathInside(actAgentTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: actAgentTmpDir });
  return resolved;
}

...
function isACTAgentLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "actagent" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isACTAgentLoopbackMcpServer(name, server)) {
    // ACTAgent's loopback MCP exposes local tools; Codex should ask for approval

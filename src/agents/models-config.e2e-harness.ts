...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousACTAgentAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousACTAgentAgentDir = process.env.ACTAGENT_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousACTAgentAgentDir === undefined) {
      delete process.env.ACTAGENT_AGENT_DIR;
    } else {
      process.env.ACTAGENT_AGENT_DIR = previousACTAgentAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "ACTAGENT_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: ACTAgentConfig = {
  models: {
    providers: {
      "custom-proxy": {

...
const ensureACTAgentModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;

vi.mock("actagent/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("actagent/plugin-sdk/llm")>("actagent/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureACTAgentModelsJson: (...args: Parameters<typeof ensureACTAgentModelsJsonMock>) =>
        ensureACTAgentModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("actagent/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("actagent/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("actagent-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureACTAgentModelsJsonMock.mockReset();
  ensureACTAgentModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  ...
  it("resolves explicit OpenAI ACTAgent runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      sessionId: "codex-first-actagent",
      sessionFile,
      workspaceDir,
      config: cfg,
      prompt: "hello",
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-actagent"),
      enqueue: immediateEnqueue,
    });

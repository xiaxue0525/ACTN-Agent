...
describe("gateway-hosted exec approvals", () => {
  ...
  it(
    "lets ACTAgent-style gateway tool calls request and wait for approval over separate connections",
    async () => {
      const envSnapshot = captureEnv(TEST_ENV_KEYS);
      cleanup.push(() => envSnapshot.restore());

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-exec-approval-e2e-"));

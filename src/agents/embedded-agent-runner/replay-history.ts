...
// `provider:"actagent"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_ACTAGENT_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyActagentAssistant(message: AgentMessage): boolean {
  ...
  return (
    provider === "actagent" &&
    typeof model === "string" &&
    TRANSCRIPT_ONLY_ACTAGENT_MODELS.has(model)
  );
}
...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyActagentAssistant(message)) {
...
function isReplayDroppableTrailingAssistant(message: AgentMessage | undefined): boolean {
  ...
  // (actagentsweeper review on #77287, P2).

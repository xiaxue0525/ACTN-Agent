...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __actagentSessionLockPromptReleaseInstalled?: boolean;
};
...
const TRANSCRIPT_ONLY_ACTAGENT_ASSISTANT_MODELS = new Set(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyACTAgentAssistantLine(line: string): boolean {
  try {
    ...
    return (
      ...
      TRANSCRIPT_ONLY_ACTAGENT_ASSISTANT_MODELS.has(message.model)
    );
  } catch {
    return false;
  }
...
async function sessionFenceAdvanceIsBenign(params: {
  ...
  return lines.length > 0 && lines.every(isTranscriptOnlyACTAgentAssistantLine);
}
...
async function sessionFenceRewriteIsBenign(params: {
  ...
  return appendedLines.every(isTranscriptOnlyACTAgentAssistantLine);
}
...
// Controllers in the same ACTAgent process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while ACTAgent held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "actagent.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__actagentSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__actagentSessionLockPromptReleaseInstalled"] = true;

...
import type { AgentMessage } from "actagent/plugin-sdk/agent-core";
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createACTAgentCodingToolsMock,
        0,
        "createACTAgentCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded ACTAgent runs when configured", async () => {
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createACTAgentCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.actagentCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_ACTAGENT_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_ACTAGENT_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "actagent.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "actagent.runtime-context",
        display: false,
      },
    );
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("ACTAGENT_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.actagent/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.actagent/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.actagent/skills/smaug",
              source: "actagent-workspace",
              sourceInfo: {
                path: "/Users/alice/.actagent/skills/smaug/SKILL.md",
                source: "actagent-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.actagent/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/actagent-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/actagent-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createACTAgentCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/actagent-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/actagent-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("ACTAGENT_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the ACTAgent runtime event.");
    expect(result.finalPromptText).toBe("Continue the ACTAgent runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the ACTAgent runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
      ...
      attemptOverrides: {
        ...
        currentInboundContext: {
          text: [
            ...
            "Current event:\n#2003 Bob: hey actagent summarize the plan",
    ...
    expect(seenPrompt).toContain("[ACTAgent room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[ACTAgent room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the ACTAgent runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[ACTAgent room event]");
  });

...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createACTAgentCodingToolsMock: UnknownMock;
...
vi.mock("../../docs-path.js", () => ({
  resolveACTAgentReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createACTAgentCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createACTAgentCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("actagent.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "actagent.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createACTAgentCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.ACTAGENT_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.ACTAGENT_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.ACTAGENT_TRAJECTORY;
    } else {
      process.env.ACTAGENT_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}

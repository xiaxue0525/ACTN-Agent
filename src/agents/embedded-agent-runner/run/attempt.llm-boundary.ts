...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const actagent = (message as unknown as Record<string, unknown>)["__actagent"];
    if (!actagent || typeof actagent !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (actagent as { beforeAgentRunBlocked?: unknown })
    ...
    const nextACTAgent = {
      ...(actagent as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __actagent: nextACTAgent,

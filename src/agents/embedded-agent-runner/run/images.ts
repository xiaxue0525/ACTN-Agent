...
function isACTAgentCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".actagent-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "actagent-cli-images" && /^actagent(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isACTAgentCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);

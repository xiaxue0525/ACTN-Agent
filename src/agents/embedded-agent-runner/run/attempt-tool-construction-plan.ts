...
import type { ACTAgentCodingToolConstructionPlan } from "../../agent-tools.js";
...
// Names here must be emitted directly by createACTAgentTools(). Catalog entries
...
const ACTAGENT_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: ACTAgentCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: ACTAgentCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: ACTAgentCodingToolConstructionPlan,
): ACTAgentCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  ...
  return (
    ...
    ACTAGENT_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeACTAgentTools = normalized.some((name) => ACTAGENT_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: ACTAgentCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeACTAgentTools;
...
/** Returns whether the allowlist requires any built-in coding/ACTAgent tools. */

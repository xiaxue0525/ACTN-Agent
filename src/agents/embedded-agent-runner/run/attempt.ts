...
import {
  ...
  ACTAGENT_EMBEDDED_CONTEXT_ENGINE_HOST,
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: ACTAGENT_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-actagent-coding-tools");
    ...
    try {
      ...
      // exact ACTAgent-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal ACTAgent agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        await sessionLockController.withSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/actagent/actagent/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("actagent:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/actagent/actagent/issues/8643

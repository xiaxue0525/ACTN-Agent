...
function authFingerprint(agentDir: string): object {
  return {
    authProfilesSqlite: fileFingerprint(path.join(agentDir, "actagent-agent.sqlite")),
    authProfilesSqliteWal: fileFingerprint(path.join(agentDir, "actagent-agent.sqlite-wal")),
  };
}

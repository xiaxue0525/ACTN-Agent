...
import { createACTAgentCodingTools, resolveProcessToolScopeKey } from "../agent-tools.js";
...
import { resolveACTAgentReferencePaths } from "../docs-path.js";
...
function summarizeCompactionMessages(messages: AgentMessage[]): CompactionMessageMetrics {
  ...
  const sources: Array<{ role: string; chars: number; tool?: string }> = [];
  let estTokens = 0;
  ...
  for (const msg of messages) {
    ...
    sources.push({ role, chars, tool: resolveMessageToolLabel(msg) });
  ...
  return {
    ...
    sources: selectTopSources(sources),
  };
}

function selectTopSources(
  sources: CompactionMessageMetrics["sources"],
): CompactionMessageMetrics["sources"] {
  const selected: CompactionMessageMetrics["sources"] = [];
  for (const entry of sources) {
    let insertAt = selected.length;
    for (let index = 0; index < selected.length; index += 1) {
      if (entry.chars > selected[index].chars) {
        insertAt = index;
        break;
      }
    }
...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  await ensureACTAgentModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createACTAgentCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createACTAgentCodingTools(...) also omit it, so both paths resolve
    ...
    const actAgentReferences = await resolveACTAgentReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop ACTAgent
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores ACTAgent runtime's auto-compaction (actagent#75799), so re-apply
      ...
      // exact ACTAgent-managed registrations so custom tools survive startup.
      ...
      while (true) {
        ...
        try {
          ...
          if (diagEnabled && preMetrics) {
            ...
            log.debug(
              `[compaction-diag] sources diagId=${diagId} top=${JSON.stringify(preMetrics.sources)}`,
            );
          }

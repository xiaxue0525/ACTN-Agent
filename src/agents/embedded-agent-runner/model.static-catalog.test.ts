...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listACTAgentPluginManifestMetadata: manifestMocks.listACTAgentPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listACTAgentPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : { ok: false, error: "missing manifest", manifestPath: `${pluginDir}/actagent.plugin.json` };
  });
}
...
beforeEach(() => {
  manifestMocks.listACTAgentPluginManifestMetadata.mockReset();

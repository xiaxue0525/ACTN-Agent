...
let actAgentNativeCodexResponsesStreamFnForTest: StreamFn | undefined;
...
function isDefaultACTAgentStreamFnForModel(
...
function resolveACTAgentNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  ...
  if (!isDefaultACTAgentStreamFnForModel(params.model, params.currentStreamFn)) {
    return undefined;
  }
  return actAgentNativeCodexResponsesStreamFnForTest ?? params.currentStreamFn ?? streamSimple;
}
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveACTAgentNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "actagent-native-codex-responses";
  }
  if (isDefaultACTAgentStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  const actAgentNativeCodexResponsesStreamFn = resolveACTAgentNativeCodexResponsesStreamFn({
    model: params.model,
    currentStreamFn: params.currentStreamFn,
  });
  if (actAgentNativeCodexResponsesStreamFn) {
    return wrapEmbeddedAgentStreamFn(actAgentNativeCodexResponsesStreamFn, {
      runSignal: params.signal,
      resolvedApiKey: params.resolvedApiKey,
      authProfileId: params.authProfileId,
      authStorage: params.authStorage,
  ...
  if (
    isDefaultACTAgentStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey)
  ) {
    const boundaryAwareStreamFn = createBoundaryAwareStreamFnForModel(params.model);
    if (boundaryAwareStreamFn) {
      // Some ACTAgent session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // ACTAgent's HTTP transport so provider-specific auth/header semantics
...
export const testing = {
  setACTAgentNativeCodexResponsesStreamFnForTest(streamFn: StreamFn | undefined): void {
    actAgentNativeCodexResponsesStreamFnForTest = streamFn;
  },
  resetACTAgentNativeCodexResponsesStreamFnForTest(): void {
    actAgentNativeCodexResponsesStreamFnForTest = undefined;
  },
};

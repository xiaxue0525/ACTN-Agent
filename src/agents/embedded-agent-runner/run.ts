 ...
 * See: https://github.com/actagent/actagent/issues/60552
 */
...
export async function runEmbeddedAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "actagent";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating ACTAgent models.json. This keeps one-shot model runs from
      ...
      const pluginHarnessNeedsACTAgentAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      const actAgentNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      let piExternalCliAuthScope = pluginHarnessOwnsTransport
        ? { ignoreAutoPreferredProfile: false }
        : actAgentNativeCodexResponsesNeedsAuthBootstrap
          ? {
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedsACTAgentAuthBootstrap &&
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsACTAgentAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsACTAgentAuthBootstrap
      ...
      // Plugin harnesses own their model transport/auth. Running ACTAgent's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsACTAgentAuthBootstrap) {
      ...
      const runAttemptAuthProfileStore = pluginHarnessOwnsTransport
        ...
        : attemptAuthProfileStore;
      const harnessBuildsACTAgentTools =
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison ACTAgent auth cooldowns.
          return;
        }
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip ACTAgent auth
            // bootstrap but drift back to ACTAgent when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            // These harnesses build ACTAgent tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildsACTAgentTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,

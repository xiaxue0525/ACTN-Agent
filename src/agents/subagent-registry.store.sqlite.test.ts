...
describe("subagent registry sqlite store", () => {
  ...
  beforeEach(async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-subagent-sqlite-"));
  });
  ...
  async function withTempStateEnv<T>(fn: () => Promise<T>): Promise<T> {
    ...
    return await withEnvAsync({ ACTAGENT_STATE_DIR: tempStateDir }, fn);
  }
  ...
  it("persists subagent runs in the shared sqlite state database", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(await fs.stat(path.join(tempStateDir!, "state", "actagent.sqlite"))).toBeTruthy();
  ...
  it("imports the legacy json registry when sqlite has no runs", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(
        openACTAgentStateDatabase().db.prepare("SELECT COUNT(*) AS count FROM subagent_runs").get(),
      ).toEqual({ count: 1 });
    });
  });
});

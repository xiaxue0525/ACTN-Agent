...
import type { AssistantMessage, Message, Tool } from "actagent/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.ACTAGENT_CACHE_TRACE,
      file: process.env.ACTAGENT_CACHE_TRACE_FILE,
      messages: process.env.ACTAGENT_CACHE_TRACE_MESSAGES,
      prompt: process.env.ACTAGENT_CACHE_TRACE_PROMPT,
      system: process.env.ACTAGENT_CACHE_TRACE_SYSTEM,
    };
    process.env.ACTAGENT_CACHE_TRACE = "1";
    process.env.ACTAGENT_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.ACTAGENT_CACHE_TRACE_MESSAGES = "0";
    process.env.ACTAGENT_CACHE_TRACE_PROMPT = "0";
    process.env.ACTAGENT_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("ACTAGENT_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("ACTAGENT_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("ACTAGENT_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("ACTAGENT_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("ACTAGENT_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ACTAGENT_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ACTAGENT_LIVE_ANTHROPIC_CACHE_MODEL",

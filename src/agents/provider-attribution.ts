...
import { listACTAgentPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const ACTAGENT_ATTRIBUTION_PRODUCT = "ACTAgent";
const ACTAGENT_ATTRIBUTION_ORIGINATOR = "actagent";
...
function formatACTAgentUserAgent(version: string): string {
  return `${ACTAGENT_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listACTAgentPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ACTAGENT_ATTRIBUTION_PRODUCT,
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": ACTAGENT_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ACTAGENT_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatACTAgentUserAgent(identity.version),
    },
  };
}

...
vi.mock("./builtin-actagent.js", () => ({
  createACTAgentAgentHarness: (): AgentHarness => ({
    id: "actagent",
...
const originalRuntime = process.env.ACTAGENT_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.ACTAGENT_AGENT_RUNTIME;
  } else {
    process.env.ACTAGENT_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: ACTAgentConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): ACTAgentConfig {
...
function groupDenyAllConfig(): ACTAgentConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): ACTAgentConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("runAgentHarnessAttempt", () => {
  it("fails when a forced plugin harness is unavailable and fallback is omitted", async () => {
    process.env.ACTAGENT_AGENT_RUNTIME = "codex";
  ...
  it("falls back to the ACTAgent harness in auto mode when no plugin harness matches", async () => {
    const result = await runAgentHarnessAttempt(createAttemptParams());

    expect(result.sessionIdUsed).toBe("actagent");
    expect(agentRunAttempt).toHaveBeenCalledTimes(1);
  });

  it("allows the selected ACTAgent harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "actagent")),
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through ACTAgent", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through ACTAgent", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to ACTAgent when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit ACTAgent runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "actagent")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("actagent");
  ...
  it("honors provider wildcard ACTAgent runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();

    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "actagent")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("actagent");
  ...
  it("leaves ACTAgent harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to ACTAgent", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("ignores session-level ACTAgent pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("honors explicit ACTAgent runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();

    const harness = selectAgentHarness({
      provider: "openai",
      modelId: "gpt-5.4",
      agentHarnessRuntimeOverride: "actagent",
    });

    expect(harness.id).toBe("actagent");
  ...
  it("treats legacy PI runtime overrides as the built-in ACTAgent harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects ACTAgent when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("actagent");
  });
  ...
  it("ignores existing session ACTAgent pins when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced ACTAgent for OpenAI default runtime selection", () => {
    process.env.ACTAGENT_AGENT_RUNTIME = "actagent";
    registerFailingCodexHarness();
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "actagent"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
  ...
  it("does not compact a selected plugin harness through ACTAgent when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns ACTAgent for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",

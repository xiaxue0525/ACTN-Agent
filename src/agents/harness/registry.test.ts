...
describe("agent harness registry", () => {
  ...
  it("keeps model-specific harnesses behind plugin registration in auto mode", () => {
    ...
    process.env.ACTAGENT_AGENT_RUNTIME = "auto";
  ...
  it("falls back to ACTAgent for other models", () => {
    process.env.ACTAGENT_AGENT_RUNTIME = "auto";
  ...
  it("honors explicit provider ACTAgent runtime policy", () => {
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        provider: "codex",
        modelId: "gpt-5.4",
        config: providerRuntimeConfig("codex", "actagent"),
      }).id,
    ).toBe("actagent");
  });

...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("actagent.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by ACTAgent",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveACTAgentCliExecutable();
  const argv =
    executable === "actagent"
      ? ["actagent"]
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all ACTAgent
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `actagent-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("actagent:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.actagent_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `actagent-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveACTAgentCliExecutable(): string {
  const envPath = process.env.ACTAGENT_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "actagent.mjs"),
  ...
  throw new Error("Cannot resolve ACTAgent CLI executable path for native hook relay");
}

...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("actagent-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("actagent-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("actagent-tilde-test-mkdir-");
  ...
  it("ignores ACTAGENT_HOME for write operations", async () => {
    const actagentHome = await createTempDir("actagent-home-override-", os.tmpdir());
    const dir = await createTempDir("actagent-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("ACTAGENT_HOME", actagentHome);

    createHostWorkspaceWriteTool(actagentHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(actagentHome, path.basename(testFile))));
  });

  it("ignores ACTAGENT_HOME for mkdir operations", async () => {
    const actagentHome = await createTempDir("actagent-home-override-", os.tmpdir());
    const dir = await createTempDir("actagent-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(actagentHome, path.basename(newDir))));
  });

  it("ignores ACTAGENT_HOME for readFile operations", async () => {
    const actagentHome = await createTempDir("actagent-home-override-", os.tmpdir());
    const dir = await createTempDir("actagent-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(actagentHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores ACTAGENT_HOME for access operations", async () => {
    const actagentHome = await createTempDir("actagent-home-override-", os.tmpdir());
    const dir = await createTempDir("actagent-tilde-test-edit-");

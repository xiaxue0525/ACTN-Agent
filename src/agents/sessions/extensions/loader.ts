...
import * as bundledLlm from "actagent/plugin-sdk/llm";
...
import { installACTAgentInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "actagent/plugin-sdk/agent-core": bundledAgentCore,
  "@actagent/plugin-sdk/agent-core": bundledAgentCore,
  "actagent/plugin-sdk/llm": bundledLlm,
  "@actagent/plugin-sdk/llm": bundledLlm,
  "actagent/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@actagent/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@actagent\/plugin-sdk|actagent\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "actagent/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@actagent/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installACTAgentInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.actagent && typeof pkg.actagent === "object") {
      return pkg.actagent as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "actagent.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "actagent" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "actagent" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with ACTAgent manifest or index.ts

...
type SkillDiscoveryMode = "actagent" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { actagent?: ResourceManifest };
    return pkg.actagent ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "actagent");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "actagent-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "actagent"),
      projectMetadata,
    ...
    // User extensions from ~/.actagent/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.actagent/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "actagent"),
      userMetadata,

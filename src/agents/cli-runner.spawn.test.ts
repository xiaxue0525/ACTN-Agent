...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cli-exec-approvals-"));
...
async function withTempACTAgentHome(run: (home: string) => Promise<void>): Promise<void> {
  const originalACTAgentHome = process.env.ACTAGENT_HOME;
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cli-home-"));
  process.env.ACTAGENT_HOME = home;
  try {
    await run(home);
  } finally {
    if (originalACTAgentHome === undefined) {
      delete process.env.ACTAGENT_HOME;
    } else {
      process.env.ACTAGENT_HOME = originalACTAgentHome;
    }
    await fs.rm(home, { recursive: true, force: true });
  }
}
...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the ACTAgent skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("actagent-cli-system-prompt-");
  ...
  it("passes ACTAgent skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("actagent-skills");
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("actagent-session");
...
`,
  ...
  it("does not create exec approvals file while resolving Claude live policy", async () => {
    await withTempACTAgentHome(async (home) => {
      const approvalsPath = path.join(home, ".actagent", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
...
`,
  ...
  it("answers Claude live control_request can_use_tool with allow when ACTAgent exec is YOLO despite raw --permission-mode default", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // and ACTAgent policy is authoritative over raw Claude permission-mode
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-live-skills-"));
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.ACTAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "actagent-cli-bootstrap-context-"),
    );

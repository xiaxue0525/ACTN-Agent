 ...
 *
 * Routes models that need ACTAgent-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "actagent-openai-responses-transport",
  "openai-chatgpt-responses": "actagent-openai-responses-transport",
  "openai-completions": "actagent-openai-completions-transport",
  "azure-openai-responses": "actagent-azure-openai-responses-transport",
  "anthropic-messages": "actagent-anthropic-messages-transport",
  "google-generative-ai": "actagent-google-generative-ai-transport",
};
...
function hasACTAgentTransportRequirement(model: Model): boolean {
  const request = getModelProviderRequestTransport(model);
  return Boolean(request?.proxy || request?.tls || getModelProviderLocalService(model));
}

/** Returns whether ACTAgent has a managed transport implementation for this API. */
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasACTAgentTransportRequirement(model)) {
    return undefined;
  }
...
/** Creates a managed ACTAgent transport stream for explicit fallback/runtime callers. */
export function createACTAgentTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need ACTAgent's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same ACTAgent request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);

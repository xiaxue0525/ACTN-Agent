...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.ACTAGENT_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.ACTAGENT_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "ACTAGENT_BUILD_PRIVATE_QA" | "ACTAGENT_QA_FORCE_RUNTIME",
...
function makeProviderRuntimeConfig(runtime: string): ACTAgentConfig {
  return {
    models: {
      providers: {
        openai: {
...
afterEach(() => {
  restoreEnv("ACTAGENT_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("ACTAGENT_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    delete process.env.ACTAGENT_BUILD_PRIVATE_QA;
    process.env.ACTAGENT_QA_FORCE_RUNTIME = "actagent";
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    ...
    process.env.ACTAGENT_BUILD_PRIVATE_QA = "1";
    process.env.ACTAGENT_QA_FORCE_RUNTIME = "actagent";
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    process.env.ACTAGENT_BUILD_PRIVATE_QA = "1";
    process.env.ACTAGENT_QA_FORCE_RUNTIME = "bogus";

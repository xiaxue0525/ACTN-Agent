...
import { ACTAGENT_CLI_ENV_VAR } from "../infra/actagent-exec-env.js";
import {
...
function stripACTAgentPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "actagent") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "actagent"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "actagent"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "actagent") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseACTAgentChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const actagentArgv = stripACTAgentPackageRunner(argv);
  return (
    normalizeCommandBaseName(actagentArgv[0]) === "actagent" &&
    (actagentArgv[1] === "channels" || actagentArgv[1] === "channel") &&
    actagentArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseACTAgentChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive ACTAgent channel login commands.",
          "Run `actagent channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
...
export function createExecTool(
  ...
  const defaultBackgroundMs = clampWithDefault(
    defaults?.backgroundMs ?? readEnvInt("ACTAGENT_BASH_YIELD_MS", "PI_BASH_YIELD_MS"),
    10_000,
    10,
    120_000,
  );

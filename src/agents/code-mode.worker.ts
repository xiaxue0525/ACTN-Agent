...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__actagentCatalog) ? globalThis.__actagentCatalog : [];
  const apiFiles = Array.isArray(globalThis.__actagentApiFiles) ? globalThis.__actagentApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__actagentNamespaces) ? globalThis.__actagentNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__actagentHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __actagentSettleBridge: { value: settle },
    __actagentTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__actagentResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__actagentHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "actagent-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__actagentTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__actagentResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  let output: unknown[] = [];
  try {
    const settle = vm.global.getProp("__actagentSettleBridge");
    try {

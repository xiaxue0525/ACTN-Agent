...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("actagent-bundled-");
  const workspaceDir = await createTempDir("actagent-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: ACTAgentConfig) {

...
import { readLocalSkillCardContentSync } from "../lifecycle/actagenthub.js";
...
describe("buildWorkspaceSkillStatus", () => {
  it("surfaces valid ACTAgentHub linkage and local Skill Card metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-status-"));
    try {
      const skillDir = path.join(workspaceDir, "skills", "agentreceipt");
      const originPath = path.join(skillDir, ".actagenthub", "origin.json");
      ...
      expect(report.skills[0]?.actagenthub).toEqual({
        status: "linked",
        valid: true,
        registry: "https://actagenthub.ai",
        slug: "agentreceipt",
  ...
  it("uses ACTAgentHub origin metadata for linkage when the skill name is a display name", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-status-"));
    try {
      const skillDir = path.join(workspaceDir, "skills", "agentreceipt");
      await writeACTAgentHubStatusFixture({
        workspaceDir,
        skillDir,
        slug: "agentreceipt",
      });
      ...
      expect(report.skills[0]?.actagenthub).toMatchObject({
        status: "linked",
        valid: true,
        registry: "https://actagenthub.ai",
        slug: "agentreceipt",
  ...
  it("does not link ACTAgentHub origin metadata from the wrong install directory", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-status-"));
    try {
      ...
      expect(report.skills[0]?.actagenthub).toMatchObject({
        ...
        valid: false,
        slug: "agentreceipt",
        reason: expect.stringContaining("expected ACTAgentHub install directory"),
      });
    } finally {
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("does not link ACTAgentHub origin metadata when the lockfile registry disagrees", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-status-"));
    try {
      const skillDir = path.join(workspaceDir, "skills", "agentreceipt");
      await writeACTAgentHubStatusFixture({
        ...
        skillDir,
        slug: "agentreceipt",
        originRegistry: "https://actagenthub.ai",
      ...
      expect(report.skills[0]?.actagenthub).toMatchObject({
        ...
        valid: false,
        slug: "agentreceipt",
        reason: expect.stringContaining("does not match the workspace ACTAgentHub lockfile"),
      });
    } finally {
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("surfaces malformed or mismatched ACTAgentHub linkage without trusting it", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-status-"));
    try {
      ...
      await fs.mkdir(path.join(malformedDir, ".actagenthub"), { recursive: true });
      await fs.writeFile(path.join(malformedDir, ".actagenthub", "origin.json"), "{not json", "utf8");
      ...
      expect(requireSkillStatus(byName, "malformed").actagenthub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("Malformed ACTAgentHub origin metadata"),
      });
      expect(requireSkillStatus(byName, "missing-lock").actagenthub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("not tracked by the workspace ACTAgentHub lockfile"),
      });
      expect(requireSkillStatus(byName, "mismatch").actagenthub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("does not match the workspace ACTAgentHub lockfile"),
      });
      expect(requireSkillStatus(byName, "local-only").actagenthub).toBeUndefined();
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "actagent-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@actagent/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "ACTAGENT_TEST_MISSING_SKILL_KEY",
            requires: { env: ["ACTAGENT_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "actagent-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @actagent/missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("ACTAGENT_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["ACTAGENT_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
...
async function writeACTAgentHubStatusFixture(params: {
  workspaceDir: string;
  skillDir: string;
  slug: string;
  ...
  const originPath = path.join(params.skillDir, ".actagenthub", "origin.json");
  await fs.mkdir(path.dirname(originPath), { recursive: true });
  await fs.writeFile(
    originPath,
    `${JSON.stringify(
      {
        version: 1,
        registry: params.originRegistry ?? "https://actagenthub.ai",
        slug: params.slug,
        installedVersion,
        installedAt,
      },
  ...
  const lockPath = path.join(params.workspaceDir, ".actagenthub", "lock.json");
  await fs.mkdir(path.dirname(lockPath), { recursive: true });
  await fs.writeFile(
    lockPath,
    `${JSON.stringify(
      {
        ...
        skills: {
          [params.slug]: {
            ...
            installedAt,
            registry: params.lockRegistry ?? "https://actagenthub.ai",
          },
        },
      },
      null,

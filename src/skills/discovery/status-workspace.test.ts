...
function makeEntry(params: {
  ...
  return {
    skill: createCanonicalFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "actagent-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("honors legacy actagentdbot skill metadata requirements and install hints", async () => {
    ...
    await writeSkill({
      ...
      metadata:
        '{"actagentdbot":{"requires":{"bins":["fakebin"]},"install":[{"id":"brew","kind":"brew","formula":"fakebin","bins":["fakebin"],"label":"Install fakebin"}]}}',
    });
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-bundled-"));
    ...
    await withEnvAsync({ ACTAGENT_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("actagent-workspace");

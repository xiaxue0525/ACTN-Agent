...
describe("installSkillFromSource", () => {
  it("installs a local skill directory using the SKILL.md frontmatter name", async () => {
    await withTempDir({ prefix: "actagent-skill-source-local-" }, async (root) => {
  ...
  it("uses --as slug override for local skill directories", async () => {
    await withTempDir({ prefix: "actagent-skill-source-as-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
  ...
  it("ignores oversized source-origin metadata while loading skill keys", async () => {
    await withTempDir({ prefix: "actagent-skill-source-origin-cap-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
      ...
      await fs.writeFile(
        path.join(workspaceDir, "skills", "custom-name", ".actagent", "source-origin.json"),
        "x".repeat(20 * 1024),
      );
  ...
  it("installs git: file repositories and records the resolved commit", async () => {
    await withTempDir({ prefix: "actagent-skill-source-git-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
  ...
  it("isolates git commands from inherited Git hook environment", async () => {
    await withTempDir({ prefix: "actagent-skill-source-git-env-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
  ...
  it("disables system git config while preserving sanitized git command env", async () => {
    await withTempDir({ prefix: "actagent-skill-source-git-system-config-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
      const poisonRepoDir = path.join(root, "poison");
      await fs.mkdir(repoDir, { recursive: true });
  ...
  it("installs slash-containing git branch refs from fresh clones", async () => {
    await withTempDir({ prefix: "actagent-skill-source-git-ref-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
      await fs.mkdir(repoDir, { recursive: true });
      await initGitSkillRepo(repoDir);
  ...
  it.each([
    ...
    async (entry) => {
      await withTempDir({ prefix: "actagent-skill-source-git-policy-" }, async (root) => {
        const workspaceDir = path.join(root, "workspace");
        const repoDir = path.join(root, "repo");
        await fs.mkdir(repoDir, { recursive: true });
        await initGitSkillRepo(repoDir);
  ...
  it("removes stale ACTAgentHub lock tracking after source installs", async () => {
    await withTempDir({ prefix: "actagent-skill-source-untrack-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
      await fs.mkdir(path.join(workspaceDir, ".actagenthub"), { recursive: true });
      await fs.mkdir(path.join(sourceDir, ".actagenthub"), { recursive: true });
      await fs.writeFile(
        path.join(sourceDir, ".actagenthub", "origin.json"),
        JSON.stringify({
          version: 1,
          registry: "https://actagenthub.example",
          slug: "frontmatter-skill",
      ...
      const lock = JSON.parse(
        await fs.readFile(path.join(workspaceDir, ".actagenthub", "lock.json"), "utf8"),
      ) as { skills: Record<string, unknown> };
      expect(lock.skills["frontmatter-skill"]).toBeUndefined();
      await expect(
        fs.access(path.join(workspaceDir, "skills", "frontmatter-skill", ".actagenthub")),
      ).rejects.toThrow();
    });
  });

  it("rejects missing local skill roots before treating them as ACTAgentHub slugs", async () => {
    await withTempDir({ prefix: "actagent-skill-source-missing-" }, async (root) => {
      const result = await installSkillFromSource({

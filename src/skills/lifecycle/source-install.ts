...
import { untrackACTAgentHubSkill } from "./actagenthub.js";
...
async function removeACTAgentHubInstallMetadata(targetDir: string): Promise<void> {
  await Promise.all([
    fs.rm(path.join(targetDir, ".actagenthub"), { recursive: true, force: true }),
    fs.rm(path.join(targetDir, ".actagentdhub"), { recursive: true, force: true }),
  ]);
}
...
async function installLocalSkillDir(params: {
  ...
  await removeACTAgentHubInstallMetadata(install.targetDir);
  ...
  await untrackACTAgentHubSkill(params.workspaceDir, slug);
...
async function installGitSkill(params: {
  ...
  return await withTempDir("actagent-git-skill-", async (tmpDir) => {

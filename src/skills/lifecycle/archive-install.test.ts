...
async function expectFlatRootMarkerRejected(params: {
  ...
  const result = await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "actagent-skill-actagenthub-test-",
    timeoutMs: 120_000,
...
describe("skill archive install", () => {
  it.each(["skill.md", "skills.md", "SKILL.MD"])(
    "installs a single-root ACTAgentHub archive with legacy marker %s",
    async (marker) => {
      const root = await tempDirs.make("actagent-skill-archive-install-");
  ...
  it("keeps skill archive policy installs independent from built-in scanner blocks", async () => {
    ...
    await fs.writeFile(path.join(extractedRoot, "SKILL.md"), skillFileContent("ACTAgentHub Policy"));
    ...
    const result = await installExtractedSkillRoot({
      workspaceDir,
      slug: "actagenthub-policy-only",
      extractedRoot,
      mode: "install",
      policy: {
        config: {},
        installId: "actagenthub",
        origin: { type: "actagenthub", slug: "actagenthub-policy-only", version: "1.0.0" },
        source: { kind: "actagenthub", authority: "actagent", mutable: false, network: true },
        requestedSpecifier: "actagenthub:actagenthub-policy-only@1.0.0",
      },
      rootMarkers: ACTAGENTHUB_SKILL_ARCHIVE_ROOT_MARKERS,
    });

...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "actagent");
  }
  return path.join(homedir(), ".actagent");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "actagent-bundled"
        ? { kind: "bundled", authority: "actagent", mutable: false, network: false }
        : skillSource === "actagent-managed" || skillSource === "actagent-extra"
          ? { kind: "managed", authority: "actagent", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["actagent-bundled", "actagent-managed", "actagent-extra"]);

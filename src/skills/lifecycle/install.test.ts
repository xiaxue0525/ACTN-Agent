...
import { resolveACTAgentMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"actagent":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveACTAgentMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("actagent-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["ACTAGENT_STATE_DIR"]);
  try {
...
describe("installSkill install policy hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("ACTAGENT_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an ACTAgent-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["ACTAGENT_STATE_DIR", "ACTAGENT_CONFIG_PATH"]);
    try {
      process.env.ACTAGENT_STATE_DIR = "/tmp/untrusted-state";
      process.env.ACTAGENT_CONFIG_PATH = "/tmp/untrusted-config/actagent.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("actagent-workspace");

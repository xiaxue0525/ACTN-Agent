    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "actagent.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "actagent.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "actagent.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("actagent-");
  const acpxRoot = await tempDirs.make("actagent-acpx-plugin-");
  const helperRoot = await tempDirs.make("actagent-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("actagent-");
  const pluginRoot = await tempDirs.make("actagent-plugin-");
  const outsideDir = await tempDirs.make("actagent-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("actagent-");
    const pluginRoot = await tempDirs.make("actagent-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("actagent-");
    const pluginRoot = await tempDirs.make("actagent-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });

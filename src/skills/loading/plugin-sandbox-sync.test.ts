...
beforeAll(async () => {
  fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "actagent-plugin-skills-sync-"));
});
...
describe("syncSkillsToWorkspace for plugin skills", () => {
  it("syncs plugin skills from symlinked directories to sandbox workspace", async () => {
    ...
    const pluginSkillsDir = path.join(sourceWorkspace, ".actagent", "plugin-skills");

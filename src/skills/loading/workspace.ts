...
import { resolveACTAgentMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".actagent", "source-origin.json");
...
function resolveSkillsLimits(config?: ACTAgentConfig, agentId?: string): ResolvedSkillsLimits {
...
export function resolveSkillRootScanLimit(config?: ACTAgentConfig): number {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "actagent-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "actagent-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  frontmatter: ParsedSkillFrontmatter;
  skillDir: string;
}): ACTAgentSkillMetadata | undefined {
  const metadata = resolveACTAgentMetadata(params.frontmatter);
...
function resolveAllowedSymlinkTargetRealPaths(config?: ACTAgentConfig): string[] {
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.actagent/plugin-skills/, so
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`actagent skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`actagent skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,

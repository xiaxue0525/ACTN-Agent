// ACTAgentHub verdict helpers normalize skill security verdicts from registry metadata.
import {
  fetchACTAgentHubSkillSecurityVerdicts,
  resolveACTAgentHubBaseUrl,
  type ACTAgentHubSkillSecurityVerdictItem,
} from "../../infra/actagenthub.js";
import type { buildWorkspaceSkillStatus } from "../discovery/status.js";

/** Public ACTAgentHub verdict item shape projected into local security scan verdicts. */
export type ACTAgentSkillSecurityVerdictItem = Omit<
  ACTAgentHubSkillSecurityVerdictItem,
...
export function projectACTAgentHubVerdictItem(
  item: ACTAgentHubSkillSecurityVerdictItem,
  registry: string,
): ACTAgentSkillSecurityVerdictItem {
  const projected: ACTAgentSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: ACTAgentSkillSecurityVerdictItem["error"] = {};
...
function canAutoFetchVerdictRegistry(registry: string): boolean {
  const configured = normalizeAutoVerdictRegistryBase(resolveACTAgentHubBaseUrl());
  const target = normalizeAutoVerdictRegistryBase(registry);
  return configured !== null && target === configured;
}

export function collectACTAgentHubVerdictTargets(
...
export async function fetchACTAgentSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<ACTAgentSkillSecurityVerdictItem[]> {
  ...
  const items: ACTAgentSkillSecurityVerdictItem[] = [];
  for (const [registry, registryTargets] of byRegistry) {
    const response = await fetchACTAgentHubSkillSecurityVerdicts({
      baseUrl: registry,
      items: registryTargets,
      skipAuth: true,
    });
    for (const item of response.items) {
      items.push(projectACTAgentHubVerdictItem(item, registry));
    }
  }
  return items;
}

...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded ACTAgent agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<ACTAgentConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    vi.stubEnv("ACTAGENT_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },

...
function configWithSlot(engineId: string): ACTAgentConfig {
...
function requireRegistryState() {
  const registryState = (globalThis as Record<symbol, unknown>)[
    Symbol.for("actagent.contextEngineRegistryState")
...
describe("Registry tests", () => {
  ...
  it("exposes the trusted plugin owner for a resolved registered engine", async () => {
    ...
    registerContextEngineForOwner(engineId, () => new MockContextEngine(), "plugin:lossless-actagent", {
      allowSameOwnerRefresh: true,
    });

    const engine = await resolveContextEngine(configWithSlot(engineId));

    expect(resolveContextEngineOwnerPluginId(engine)).toBe("lossless-actagent");
  });
...
describe("Invalid engine fallback", () => {
  ...
  it("accepts resolved engines whose info.id differs from the registered slot id (#66601)", async () => {
    // Regression for actagent/actagent#66601: third-party plugins like
    // lossless-actagent register under an external slot id ("lossless-actagent") but
...
// lossless-actagent registered successfully but resolution could not find it.
// ═══════════════════════════════════════════════════════════════════════════

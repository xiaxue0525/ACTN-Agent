    ...
    info: {
      ...
      hostRequirements: {
        "agent-run": {
          ...
          unsupportedMessage:
            "Use the native Codex or ACTAgent embedded runtime, or switch contextEngine to legacy.",
        },
      },
    },
    async ingest() {
...
describe("context engine host compatibility", () => {
  ...
  it("rejects generic CLI hosts when an engine requires pre-prompt assembly", () => {
    expect(() =>
      ...
      'Context engine "lossless-actagent" cannot run operation "agent-run" on CLI backend "claude-cli".',
    );
  });
  ...
  it("allows native Codex and ACTAgent embedded hosts to satisfy pre-prompt assembly", () => {

...
const SETUP_RE =
  /^(?:setup(?!\s+agent\b)|set\s+me\s+up|set\s+up\s+actagent|onboard|onboard\s+me|bootstrap|first\s+run)(?:\b|$)/i;
const PLUGIN_LIST_RE = /^(?:plugins?|actagenthub)\s+list$|^list\s+plugins?$/i;
const PLUGIN_SEARCH_RE =
  /^(?:(?:plugins?|actagenthub)\s+search|search\s+plugins?(?:\s+for)?)\s+(?<query>.+)$/i;
const PLUGIN_INSTALL_RE =
  /^(?:(?:plugins?)\s+install|install\s+(?:(?<source>npm|actagenthub)\s+)?plugins?)\s+(?<spec>\S+)$/i;
...
export function parseCrestodianOperation(input: string): CrestodianOperation {
  ...
  if (!trimmed) {
    return {
      kind: "none",
      message: "Tiny actagent tap: say status, doctor, models, agents, or talk to agent.",
    };
  }
...
function normalizePluginInstallSpec(spec: string, source: string | undefined): string {
  ...
  if (normalizedSource === "actagenthub" && !trimmed.toLowerCase().startsWith("actagenthub:")) {
    return `actagenthub:${trimmed}`;
  }
  return trimmed;
}
...
function validateCrestodianPluginInstallSpec(spec: string): string | null {
  ...
  if (/\s/.test(trimmed)) {
    return "Crestodian plugin install accepts one npm or ACTAgentHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    ...
    return "Crestodian plugin install accepts npm or ACTAgentHub package specs only.";
  }
  return null;
}
...
function formatSetupPlanDescription(
  ...
  return `bootstrap ACTAgent setup for workspace ${workspace}${model}`;
}

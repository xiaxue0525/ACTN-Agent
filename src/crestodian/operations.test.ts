...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/actagent.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @actagent/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@actagent/demo",
    });
    expect(parseCrestodianOperation("plugin install actagenthub:actagent-demo")).toEqual({
      kind: "plugin-install",
      spec: "actagenthub:actagent-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall actagent-demo")).toEqual({
      kind: "plugin-uninstall",
      pluginId: "actagent-demo",
    });
  });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/actagent.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "actagenthub:actagent-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin actagenthub:actagent-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("actagenthub:actagent-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin actagenthub:actagent-demo",
      },
      { rescue: true, spec: "actagenthub:actagent-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "actagent-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin actagent-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("actagent-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin actagent-demo",
      },
      { rescue: true, pluginId: "actagent-demo" },
    );
  });

...
type ACTAgentReferencePaths = Awaited<ReturnType<typeof resolveACTAgentReferencePaths>>;
...
type CrestodianOverviewDependencies = {
  ...
  resolveACTAgentReferencePaths?: typeof resolveACTAgentReferencePaths;
};
...
function buildAgentSummaries(cfg: ACTAgentConfig): CrestodianAgentSummary[] {
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): ACTAgentReferencePaths | undefined {
...
export async function loadCrestodianOverview(
  ...
  const resolveReferences = deps.resolveACTAgentReferencePaths ?? resolveACTAgentReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: ACTAGENT_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: ACTAGENT_SOURCE_URL,
    },
  };
}
...
export function formatCrestodianOverview(overview: CrestodianOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Crestodian online. Little actagents, typed tools.",
    "",
    `Config: ${configStatus}`,

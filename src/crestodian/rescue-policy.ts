...
function resolveAgentEntry(cfg: ACTAgentConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: ACTAgentConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: ACTAgentConfig, agentId?: string): boolean {
...
export function resolveCrestodianRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Crestodian rescue is blocked because ACTAgent sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  ...
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Crestodian rescue only accepts commands from an ACTAgent owner.",
    };
  }

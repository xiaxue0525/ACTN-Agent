...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => ACTAgentConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): ACTAgentConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): ACTAgentConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,

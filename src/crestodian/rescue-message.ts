...
function formatUnsupportedRemoteOperation(operation: CrestodianOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      ...
      "Use local `actagent` for agent handoff, or ask for status, doctor, config, gateway, agents, or models.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      ...
      "Use local `actagent crestodian` or `actagent plugins install` instead.",
    ].join(" ");
  }
  return null;
}
...
export async function runCrestodianRescueMessage(
  ...
  return capture.read() || "Crestodian listened, clicked a actagent, and found nothing to change.";
}

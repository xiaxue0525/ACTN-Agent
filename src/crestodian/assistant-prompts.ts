...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, ACTAgent's ring-zero setup helper.",
  "Turn the user's request into exactly one safe ACTAgent Crestodian command.",
  ...
  "Use the provided ACTAgent docs/source references when the user's request needs behavior, config, or architecture details.",
  ...
  "- plugin install <npm-or-actagenthub-spec>",
  "- plugin uninstall <id>",
  "- audit",
  "- validate config",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `ACTAgent docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `ACTAgent source: ${

...
describe("resolveCronPayloadOutcome", () => {
  ...
  it("lets preferred final assistant text recover a plain tool warning", () => {
    const result = resolveCronPayloadOutcome({
      ...
      finalAssistantVisibleText: "**actagentsweeper 6h report**\nClosed: 34 total",
      preferFinalAssistantVisibleText: true,
    });

    expect(result.hasFatalErrorPayload).toBe(false);
    expect(result.embeddedRunError).toBeUndefined();
    expect(result.summary).toBe("**actagentsweeper 6h report**\nClosed: 34 total");
    expect(result.outputText).toBe("**actagentsweeper 6h report**\nClosed: 34 total");
    expect(result.deliveryPayloads).toEqual([
      { text: "**actagentsweeper 6h report**\nClosed: 34 total" },
    ]);
  });

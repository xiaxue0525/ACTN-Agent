...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ACTAGENT_HOME for tilde expansion", () => {
    vi.stubEnv("ACTAGENT_HOME", "/srv/actagent-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/actagent-home", "cron", "jobs.json"));
  });
});

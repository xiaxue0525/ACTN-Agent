...
const ACTAGENT_DEBUG_PROXY_ENABLED = "ACTAGENT_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["ACTAGENT_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[ACTAGENT_DEBUG_PROXY_ENABLED])) {
    return;
  }

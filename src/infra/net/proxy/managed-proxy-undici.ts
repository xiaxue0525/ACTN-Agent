// Bridges ACTAgent-managed proxy TLS trust into Undici EnvHttpProxyAgent and
// explicit ProxyAgent options without changing unrelated operator proxies.
import { isRecord as isProxyTlsRecord } from "@actagent/normalization-core/record-coerce";
...
function resolveManagedProxyUrl(env: ManagedProxyTlsEnv = process.env): string | undefined {
  ...
  if (env["ACTAGENT_PROXY_ACTIVE"] !== "1") {
    return undefined;
  }
...
/** Resolves managed proxy TLS trust only when the target proxy is ACTAgent's active proxy. */
export function resolveActiveManagedProxyTlsOptions(
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    ...
    caFileOverride: env["ACTAGENT_PROXY_CA_FILE"],
  });
  try {

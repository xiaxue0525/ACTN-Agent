...
describe("startProxy", () => {
  ...
  const envKeysToClean = [
    ...
    "no_proxy",
    "NO_PROXY",
    "ACTAGENT_PROXY_ACTIVE",
    "ACTAGENT_PROXY_CA_FILE",
    "ACTAGENT_PROXY_LOOPBACK_MODE",
    "ACTAGENT_PROXY_URL",
  ];
  ...
  function writeTempCa(contents = "proxy-ca"): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), "actagent-proxy-lifecycle-ca-"));
    tempDirs.push(dir);
  ...
  it("uses ACTAGENT_PROXY_URL when config proxyUrl is omitted", async () => {
    process.env["ACTAGENT_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("prefers config proxyUrl over ACTAGENT_PROXY_URL", async () => {
    process.env["ACTAGENT_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("uses HTTPS proxy URLs from ACTAGENT_PROXY_URL", async () => {
    process.env["ACTAGENT_PROXY_URL"] = "https://127.0.0.1:3128";
  ...
  it("passes configured proxy CA trust to Proxyline", async () => {
    ...
    expect(process.env["ACTAGENT_PROXY_CA_FILE"]).toBe(caFile);
    expect(installGlobalProxyMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("does not load configured proxy CA files for plain HTTP proxy URLs", async () => {
    const missingCaFile = path.join(os.tmpdir(), "actagent-missing-http-proxy-ca.pem");
  ...
  it("loads inherited HTTPS proxy CA trust for child routing", () => {
    ...
    process.env["ACTAGENT_PROXY_ACTIVE"] = "1";
    process.env["ACTAGENT_PROXY_LOOPBACK_MODE"] = "gateway-only";
    process.env["HTTP_PROXY"] = "https://proxy.example:8443";
    process.env["ACTAGENT_PROXY_CA_FILE"] = caFile;
  ...
  it("sets process proxy env vars for inherited clients", async () => {
    ...
    expect(process.env["HTTP_PROXY"]).toBe("http://127.0.0.1:3128");
    expect(process.env["HTTPS_PROXY"]).toBe("http://127.0.0.1:3128");
    expect(process.env["ACTAGENT_PROXY_ACTIVE"]).toBe("1");
    expect(process.env["ACTAGENT_PROXY_LOOPBACK_MODE"]).toBe("gateway-only");
  });
  ...
  it("persists loopbackMode in env for forked child CLIs", async () => {
    ...
    expect(process.env["ACTAGENT_PROXY_LOOPBACK_MODE"]).toBe("block");
    expect(getActiveManagedProxyLoopbackMode()).toBe("block");

    await stopProxy(handle);
    process.env["ACTAGENT_PROXY_ACTIVE"] = "1";
    process.env["ACTAGENT_PROXY_LOOPBACK_MODE"] = "proxy";
  ...
  it("restores previous proxy env and stops Proxyline on stop", async () => {
    ...
    expect(process.env["ACTAGENT_PROXY_ACTIVE"]).toBeUndefined();
    expect(proxylineStopMock).toHaveBeenCalledOnce();
    expect(forceResetGlobalDispatcherMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("registers exact Gateway loopback URLs with Proxyline", async () => {
    ...
    const unregister = expectBypassUnregister(
      registerManagedProxyGatewayLoopbackBypass("ws://127.0.0.1:19199"),
    );
  ...
  it("accepts literal loopback IPs and localhost for Gateway bypass registration", async () => {
    ...
    const unregisterIpv6 = expectBypassUnregister(
      registerManagedProxyGatewayLoopbackBypass("ws://[::1]:19199"),
    );
    expect(proxylineRegisterBypassMock).toHaveBeenCalledWith({ url: "ws://[::1]:19199" });
    unregisterIpv6();

    const unregisterLocalhost = expectBypassUnregister(
      registerManagedProxyGatewayLoopbackBypass("ws://localhost.:19199"),
    );
    expect(proxylineRegisterBypassMock).toHaveBeenCalledWith({ url: "ws://localhost.:19199" });
    unregisterLocalhost();
  ...
  it("blocks Gateway bypass registration when active proxy loopbackMode is block", async () => {
    ...
    try {
      expect(() => registerManagedProxyGatewayLoopbackBypass("ws://127.0.0.1:19199")).toThrow(
  ...
  it("does not register Gateway bypass when active proxy loopbackMode is proxy", async () => {
    ...
    try {
      const unregister = registerManagedProxyGatewayLoopbackBypass("ws://127.0.0.1:19199");

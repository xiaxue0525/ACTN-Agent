...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    ACTAGENT_PROXY_ACTIVE: process.env["ACTAGENT_PROXY_ACTIVE"],
    ACTAGENT_PROXY_LOOPBACK_MODE: process.env["ACTAGENT_PROXY_LOOPBACK_MODE"],
    ACTAGENT_PROXY_CA_FILE: process.env["ACTAGENT_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["ACTAGENT_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["ACTAGENT_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["ACTAGENT_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["ACTAGENT_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or ACTAGENT_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["ACTAGENT_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["ACTAGENT_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance ACTAgent spawned itself.
 *

...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ACTAGENT_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*actagent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "actagent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "actagent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "actagent-gw");
  ...
  records.push(`_actagent-gw._tcp IN PTR ${instanceLabel}._actagent-gw._tcp`);
  records.push(`${instanceLabel}._actagent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._actagent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; actagent-content-hash: ${contentHash}\n${contentBody}`;
}

...
import { bundledDistPluginFile } from "actagent/plugin-sdk/test-fixtures";
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/actagent-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "actagent", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["ACTAGENT_UPDATE_PACKAGE_SPEC"]);
    process.env.ACTAGENT_UPDATE_PACKAGE_SPEC = "file:/tmp/actagent.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "actagent", tag: "latest" })).toBe(
      "file:/tmp/actagent.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "actagent",
        tag: "beta",
        env: { ACTAGENT_UPDATE_PACKAGE_SPEC: "actagent@next" },
      }),
    ).toBe("actagent@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "actagent"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "actagent", tag: "main" })).toBe(
      ACTAGENT_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "actagent",
        tag: "github:actagent/actagent#feature/my-branch",
      }),
    ).toBe("github:actagent/actagent#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "actagent",
        tag: "https://example.com/actagent-main.tgz",
      }),
    ).toBe("https://example.com/actagent-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "actagent-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:actagent/actagent#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/actagent-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/actagent-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/actagent-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("actagent-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:actagent/actagent#main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("/tmp/actagent-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "actagent-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "actagent");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "actagent"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "actagent"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "actagent"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "actagent"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "actagent-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "actagent@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "actagent@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "actagent@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "actagent@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "actagent-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "actagent-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".actagent", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "actagent");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "actagent"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "actagent-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "actagent");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "actagent-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "actagent");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "actagent-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "actagent");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "actagent-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "actagent@file+..+pack+actagent-2026.5.6.tgz",
        "node_modules",
        "actagent",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "actagent"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "actagent-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "actagent");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "actagent"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "actagent@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "actagent@latest",
    ...
    expect(globalInstallArgs("pnpm", "actagent@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "actagent@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:actagent/actagent#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=actagent",
      "github:actagent/actagent#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "actagent@git+https://github.com/actagent/actagent.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=actagent",
      "actagent@git+https://github.com/actagent/actagent.git",
    ]);
    expect(globalInstallArgs("bun", "actagent@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "actagent@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "actagent@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "actagent@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "actagent@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "actagent@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "actagent@latest"]);
    expect(globalInstallArgs("pnpm", "actagent@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "actagent@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "actagent@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/actagent/lib/node_modules")).toEqual({
      prefix: "/opt/actagent",
      globalRoot: "/opt/actagent/lib/node_modules",
      binDir: "/opt/actagent/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "actagent-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".actagent-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".actagent-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".actagent-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "actagent"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".actagent-123", ".actagent-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "actagent"));
      const markerFileStat = await fs.stat(path.join(root, ".actagent-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "actagent-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "actagent-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".actagent-install-stage", ".actagent-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.actagent-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.actagent-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "actagent-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "actagent");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "actagent-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "actagent-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "actagent-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "actagent-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@actagent/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "actagent-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@actagent/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "actagent-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@actagent/qa-lab");
        await writePackageDistInventory(packageRoot);

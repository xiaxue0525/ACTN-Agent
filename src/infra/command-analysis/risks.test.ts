...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "ACTAGENT_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "actagent", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("actagent channels login --channel whatsapp");
  });

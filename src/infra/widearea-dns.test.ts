...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 19199,
  displayName: "Mac Studio (ACTAgent)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "actagent.internal", expected: "actagent.internal." },
    { value: "actagent.internal.", expected: "actagent.internal." },
    { value: "  actagent.internal  ", expected: "actagent.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { ACTAGENT_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { ACTAGENT_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { ACTAGENT_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("actagent.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "actagent.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("actagent.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN actagent.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_actagent-gw._tcp IN PTR studio-london._actagent-gw._tcp`,
      `studio-london._actagent-gw._tcp IN SRV 0 0 19199 studio-london`,
      `displayName=Mac Studio (ACTAgent)`,
      `gatewayPort=19199`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/actagent`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN actagent.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._actagent-gw._tcp IN TXT`,
        `displayName=Mac Studio (ACTAgent)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("actagent.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("actagent.internal."),
      expectedZoneText,
      "utf-8",
    );
  });

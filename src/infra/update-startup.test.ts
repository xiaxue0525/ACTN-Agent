...
vi.mock("./actagent-root.js", async () => {
  const actual = await vi.importActual<typeof import("./actagent-root.js")>("./actagent-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "actagent-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    if (!loaded) {
      ({ resolveACTAgentPackageRoot } = await import("./actagent-root.js"));
    ...
    vi.mocked(resolveACTAgentPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveACTAgentPackageRoot).mockResolvedValue("/opt/actagent");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("actagent update")}`,
    );
  ...
  it("honors ACTAGENT_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.ACTAGENT_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by ACTAGENT_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by ACTAGENT_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/actagent/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });

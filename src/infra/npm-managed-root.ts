...
import { normalizeOptionalString as readOptionalString } from "@actagent/normalization-core/string-coerce";
...
import { resolveACTAgentPackageRootSync } from "./actagent-root.js";
...
type ManagedNpmRootACTAgentMetadata = {
...
type ManagedNpmRootACTAgentHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedACTAgentMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootACTAgentMetadata | undefined {
  const metadata: ManagedNpmRootACTAgentMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
/** Read host ACTAgent pnpm overrides for reuse inside a managed npm root. */
export async function readACTAgentManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.actagent)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const actagentMetadata = buildManagedACTAgentMetadata({
    current: manifest.actagent,
    managedOverrideKeys,
  });
  ...
  if (actagentMetadata) {
    next.actagent = actagentMetadata;
  } else {
    delete next.actagent;
  }
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "actagent" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.actagent;
  ...
  if (isRecord(value.peerDependenciesMeta) && "actagent" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.actagent;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])actagent(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.actagent);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-managed-peer-plan-"));
  try {
    delete dependencies.actagent;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_actagent-pack-archives"),
      path.join(tempRoot, "_actagent-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.actagent).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.actagent)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.actagent).toSorted();
  const actagentMetadata = buildManagedACTAgentMetadata({
    current: manifest.actagent,
    managedOverrideKeys,
...
/** Remove stale managed-root actagent peer installs while preserving active host links. */
export async function repairManagedNpmRootACTAgentPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootACTAgentHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "actagent" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasACTAgentPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "actagent"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootACTAgentPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall actagent" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall actagent" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootACTAgentPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootACTAgentHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootACTAgentHostState> {
  const packageRoot =
  ...
  const managedACTAgentPackageDir = path.join(params.npmRoot, "node_modules", "actagent");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedACTAgentPackageDir),
    lstatIfExists(managedACTAgentPackageDir),
  ]);
...
async function managedNpmRootLockfileHasACTAgentPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "actagent" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/actagent" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "actagent" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootACTAgentPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("actagent" in dependencies) {
    const { actagent: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("actagent" in dependenciesValue) {
          delete dependenciesValue.actagent;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/actagent" in parsed.packages) {
        delete parsed.packages["node_modules/actagent"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "actagent" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.actagent;
  ...
  const actagentPackageDir = path.join(params.npmRoot, "node_modules", "actagent");
  if (!params.preservePackageDir && (await pathExists(actagentPackageDir))) {
    await fs.rm(actagentPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["actagent", "actagent.cmd", "actagent.ps1"].map((binName) =>

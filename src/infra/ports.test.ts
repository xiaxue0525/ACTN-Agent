...
describe("ports helpers", () => {
  ...
  it("prints an ACTAgent-specific hint when port details look like another ACTAgent instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(19199, "node dist/index.js actagent gateway"),
      19199,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another ACTAgent instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/actagent/dist/index.js gateway --port 19199\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("actagent");
  ...
  it("reports established gateway client connections from lsof", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command.includes("lsof")) {
        return {
          stdout:
            "p111\ncnode\nnTCP 127.0.0.1:50123->127.0.0.1:19199 (ESTABLISHED)\n" +
            "p222\ncnode\nnTCP 127.0.0.1:19199->127.0.0.1:50123 (ESTABLISHED)\n" +
            "p444\ncnode\nnTCP 127.0.0.1:50125->[::ffff:127.0.0.1]:19199 (ESTABLISHED)\n" +
            "p555\ncnode\nnTCP 127.0.0.1:50126->127.0.0.1:19199abc (ESTABLISHED)\n" +
            "p666\ncnode\nnTCP 127.0.0.1:50127->127.0.0.1:99999 (ESTABLISHED)\n" +
            "p333\ncBrowser\nnTCP 127.0.0.1:50124->198.51.100.7:19199 (ESTABLISHED)\n",
          stderr: "",
          code: 0,
        };
      }
      if (command === "ps") {
        ...
        if (argv.includes("command=")) {
          return {
            stdout:
              pid === "111"
                ? "node /tmp/newer-actagent/dist/index.js logs --follow\n"
                : pid === "222"
                  ? "node /tmp/older-actagent/dist/index.js gateway run\n"
    ...
    const result = await inspectPortConnections(19199);
  ...
  it("deduplicates repeated lsof listener records for one process address", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/actagent/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports multiple lsof socket records for one process", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command.includes("lsof")) {
        return {
          stdout:
            "p111\ncnode\n" +
            "nTCP 127.0.0.1:50123->127.0.0.1:19199 (ESTABLISHED)\n" +
            "nTCP 127.0.0.1:50124->127.0.0.1:19199 (ESTABLISHED)\n",
          stderr: "",
          code: 0,
        };
      }
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/newer-actagent/dist/index.js logs --follow\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    expect(result.connections).toEqual([
      ...
      expect.objectContaining({
        pid: 111,
        direction: "client",
        address: "TCP 127.0.0.1:50124->127.0.0.1:19199 (ESTABLISHED)",
      }),
    ]);
  });
  ...
  it("falls back to ss for established gateway client connections", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ss") {
        return {
          stdout:
            '0 0 127.0.0.1:50123 127.0.0.1:19199 users:(("node",pid=111,fd=12))\n' +
            '0 0 127.0.0.1:50124 198.51.100.7:19199 users:(("browser",pid=333,fd=9))\n',
          stderr: "",
          code: 0,
        };
      }
...
describe("inspectPortUsage on Windows", () => {
  it("reports established gateway client connections from netstat", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "netstat") {
        return {
          stdout:
            "  TCP    127.0.0.1:50123    127.0.0.1:19199    ESTABLISHED    4242\r\n" +
            "  TCP    127.0.0.1:50124    198.51.100.7:19199  ESTABLISHED    5000\r\n",
          stderr: "",
          code: 0,
        };
      }
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\actagent\\dist\\index.js logs --follow\r\n',
          stderr: "",
          code: 0,
        };
      }
    ...
    expect(result.connections[0]?.commandLine).toContain("actagent");
  });

  it("uses PowerShell process command lines to classify ACTAgent listeners", async () => {
    setPlatform("win32");
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      const [command] = argv;
      if (command === "netstat") {
        return {
          stdout: "  TCP    127.0.0.1:19199    0.0.0.0:0    LISTENING    4242\r\n",
          stderr: "",
          code: 0,
        };
      }
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\actagent\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("does not match Windows listener ports by substring", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "netstat") {
        return {
          stdout:
            "  TCP    127.0.0.1:191990    0.0.0.0:0    LISTENING    9000\r\n" +
            "  TCP    [::1]:191990        [::]:0         LISTENING    9001\r\n",
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "wmic") {
        return {
          stdout: "CommandLine=node.exe C:\\actagent\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }

// Verifies state-dir migrations preserve existing ACTAgent runtime data.
...
async function withStateDirFixture(run: (root: string) => Promise<void>): Promise<void> {
  try {
    await withTempDir({ prefix: "actagent-state-dir-" }, async (root) => {
      await run(root);
    });
  } finally {
    resetAutoMigrateLegacyStateDirForTest();
...
describe("legacy state dir auto-migration", () => {
  it("skips a legacy symlinked state dir when it points outside supported legacy roots", async () => {
    await withStateDirFixture(async (root) => {
      const legacySymlink = path.join(root, ".actagentdbot");
      ...
      expect(fs.readFileSync(path.join(root, ".actagentdbot", "marker.txt"), "utf-8")).toBe("ok");
    });
  });

  it("skips state-dir migration when ACTAGENT_STATE_DIR is explicitly set", async () => {
    await withStateDirFixture(async (root) => {
      const legacyDir = path.join(root, ".actagentdbot");
      fs.mkdirSync(legacyDir, { recursive: true });

      const result = await autoMigrateLegacyStateDir({
        env: { ACTAGENT_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
        homedir: () => root,
      });
  ...
  it("migrates the legacy plugin install index from an explicit state dir", async () => {
    await withStateDirFixture(async (root) => {
      ...
      const result = await autoMigrateLegacyStateDir({
        env: { ACTAGENT_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
        homedir: () => root,
      });

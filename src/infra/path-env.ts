// Builds PATH values for ACTAgent child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current ACTAgent process is on.
  try {
  ...
  // Bundled macOS app: `actagent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ACTAGENT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "actagent"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `actagent`
...
/**
 * Best-effort PATH bootstrap so skills that require the `actagent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureACTAgentCliOnPath(opts: EnsureACTAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ACTAGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.ACTAGENT_PATH_BOOTSTRAPPED = "1";

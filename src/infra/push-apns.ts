...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open ACTAgent to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "actagent.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ACTAGENT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ACTAGENT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ACTAGENT_APNS_TEAM_ID and ACTAGENT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ACTAGENT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ACTAGENT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ACTAGENT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ACTAGENT_APNS_PRIVATE_KEY_P8 or ACTAGENT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ACTAGENT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}

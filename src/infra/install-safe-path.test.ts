...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@actagent/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@actagent/matrix", id: "matrix", expected: true },
    { packageName: "@actagent/matrix", id: "@actagent/matrix", expected: true },
    { packageName: "@actagent/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@actagent/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@actagent__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "actagent-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "actagent-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "actagent-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "actagent-install-safe-" }, async (parentDir) => {

  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.actagent.proxy.validation",
        "apns-topic": "ai.actagent.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "ACTAgent APNs proxy validation" } }));
    });
  } finally {

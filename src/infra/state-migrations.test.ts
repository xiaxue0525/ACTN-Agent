...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: ACTAgentConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: ACTAgentConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.ACTAGENT_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("actagent-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("migrates legacy delivery queue files into shared SQLite state", async () => {
    ...
    const { db } = openACTAgentStateDatabase({ env });
    const rows = db
      .prepare(

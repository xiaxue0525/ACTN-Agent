// Creates temporary ACTAgent directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared ACTAgent temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_ACTAGENT_TMP_DIR = "/tmp/actagent";
...
export type ResolvePreferredACTAgentTmpDirOptions = {
...
/** Resolves a safe ACTAgent temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredACTAgentTmpDir(
  options: ResolvePreferredACTAgentTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "actagent" : `actagent-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[actagent] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback ACTAgent temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback ACTAgent temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_ACTAGENT_TMP_DIR;

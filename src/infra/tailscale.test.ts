...
describe("tailscale helpers", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ACTAGENT_TEST_TAILSCALE_BINARY", "NODE_ENV", "VITEST"]);
    process.env.ACTAGENT_TEST_TAILSCALE_BINARY = "tailscale";
  ...
  it("allows the test binary override in explicit test environments", () => {
    process.env.ACTAGENT_TEST_TAILSCALE_BINARY = "/tmp/test-tailscale";
  ...
  it("ignores the test binary override outside test environments", () => {
    process.env.ACTAGENT_TEST_TAILSCALE_BINARY = "/tmp/attacker-tailscale";
  ...
  it("enableTailscaleServe passes a configured service name", async () => {
    ...
    await enableTailscaleServe(3000, exec as never, "svc:actagent");
    ...
    expectExecCall(
      ...
      1,
      tailscaleBin,
      ["serve", "--service=svc:actagent", "--bg", "--yes", "3000"],
      {
        maxBuffer: 200_000,
        timeoutMs: 15_000,
      },
  ...
  it("disableTailscaleServe disables only the configured service name", async () => {
    ...
    await disableTailscaleServe(exec as never, "svc:actagent");

    expect(exec).toHaveBeenCalledTimes(1);
    expectExecCall(exec, 1, tailscaleBin, ["serve", "clear", "svc:actagent"], {
      maxBuffer: 200_000,
      timeoutMs: 15_000,
    });
  });
...
describe("tailscaleFunnelStatusCoversPort", () => {
  ...
  it("matches a Funnel route whose Proxy is a full http URL", () => {
    const status = buildFunnelStatus({ "/": { Proxy: "http://127.0.0.1:19199" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });
  ...
  it("matches a Proxy URL with a longer path", () => {
    const status = buildFunnelStatus({ "/api": { Proxy: "http://127.0.0.1:19199/api" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });

  it("matches the localhost loopback alias", () => {
    const status = buildFunnelStatus({ "/": { Proxy: "http://localhost:19199" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });

  it("matches an IPv6 loopback Proxy", () => {
    const status = buildFunnelStatus({ "/": { Proxy: "http://[::1]:19199" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });

  it("matches the documented https+insecure target scheme", () => {
    const status = buildFunnelStatus({
      "/": { Proxy: "https+insecure://localhost:19199" },
    });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });
  ...
  it("matches https+insecure with a trailing path", () => {
    const status = buildFunnelStatus({
      "/api": { Proxy: "https+insecure://127.0.0.1:19199/api" },
    });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });
  ...
  it("does not match https+insecure on a non-loopback host", () => {
    const status = buildFunnelStatus({
      "/": { Proxy: "https+insecure://10.0.0.5:19199" },
    });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(false);
  });

  it("matches a bare port form", () => {
    const status = buildFunnelStatus({ "/": { Proxy: "19199" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(true);
  });
  ...
  it("does not match a non-loopback host on the right port", () => {
    const status = buildFunnelStatus({ "/": { Proxy: "http://10.0.0.5:19199" } });
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(false);
  });
    ...
    const status = {
      ...
      Web: {
        "device.tailnet.ts.net:443": {
          Handlers: { "/": { Proxy: "http://127.0.0.1:19199" } },
        },
      },
    } as Record<string, unknown>;
    expect(tailscaleFunnelStatusCoversPort(status, 19199)).toBe(false);
  ...
  it("returns false on an empty status payload", () => {
    expect(tailscaleFunnelStatusCoversPort({}, 19199)).toBe(false);
  });
});

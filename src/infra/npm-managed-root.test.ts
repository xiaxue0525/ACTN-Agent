...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "actagent-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs ACTAgent-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readACTAgentManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readACTAgentManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@actagent/discord",
          version: "2026.5.2",
          resolvedSpec: "@actagent/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@actagent/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@actagent/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for actagent@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed actagent peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    await fs.mkdir(path.join(npmRoot, "node_modules", "actagent"), { recursive: true });
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
        {
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "actagent", "package.json"),
      `${JSON.stringify({ name: "actagent", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootACTAgentPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@actagent/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.actagent).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "actagent"));
    for (const binName of ["actagent", "actagent.cmd", "actagent.ps1"]) {
  ...
  it("does not repair the active ACTAgent host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/actagent-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "actagent", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "actagent"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "actagent.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/actagent-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "actagent"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/actagent"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/actagent-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.actagent).toBeUndefined();
    for (const binName of ["actagent", "actagent.cmd", "actagent.ps1"]) {

...
const WIDE_AREA_DOMAIN = "actagent.internal.";
...
describe("bonjour-discovery", () => {
    ...
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B") {
        if (domain === "local.") {
          return {
            stdout: [
              "Add 2 3 local. _actagent-gw._tcp. Peter\\226\\128\\153s Mac Studio Gateway",
              "Add 2 3 local. _actagent-gw._tcp. Laptop Gateway",
              "",
            ].join("\n"),
            stderr: "",
            code: 0,
        ...
        if (domain === WIDE_AREA_DOMAIN) {
          return {
            stdout: [`Add 2 3 ${WIDE_AREA_DOMAIN} _actagent-gw._tcp. Tailnet Gateway`, ""].join(
              "\n",
            ),
            stderr: "",
            code: 0,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        ...
        return {
          stdout: [
            `${instance}._actagent-gw._tcp. can be reached at ${host}:19199`,
            txtParts.join(" "),
            "",
          ].join("\n"),
          stderr: "",
  ...
  it("decodes dns-sd octal escapes in TXT displayName", async () => {
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B" && domain === "local.") {
        return {
          stdout: ["Add 2 3 local. _actagent-gw._tcp. Studio Gateway", ""].join("\n"),
          stderr: "",
          code: 0,
          signal: null,
          killed: false,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        return {
          stdout: [
            "Studio Gateway._actagent-gw._tcp. can be reached at studio.local:19199",
            "txtvers=1 displayName=Peter\\226\\128\\153s\\032Mac\\032Studio lanHost=studio.local gatewayPort=19199 sshPort=22",
            "",
          ].join("\n"),
          stderr: "",
          code: 0,
  ...
  it("rejects malformed and out-of-range advertised ports", async () => {
    const run = vi.fn(async (argv: string[]) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B" && domain === "local.") {
        return {
          stdout: ["Add 2 3 local. _actagent-gw._tcp. Broken Gateway", ""].join("\n"),
          stderr: "",
          code: 0,
          signal: null,
          killed: false,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        return {
          stdout: [
            "Broken Gateway._actagent-gw._tcp. can be reached at broken.local:19199abc",
  ...
  it("falls back to tailnet DNS probing for wide-area when split DNS is not configured", async () => {
    ...
    const serviceBase = `_actagent-gw._tcp.${zone}`;
    ...
    expect(beacon.port).toBe(19199);
    expect(beacon.tailnetDns).toBe("peters-mac-studio-1.sheep-coho.ts.net");
    expect(beacon.gatewayPort).toBe(19199);
    expect(beacon.sshPort).toBe(22);
    expect(beacon.cliPath).toBe("/opt/homebrew/bin/actagent");
  ...
  it("normalizes domains and respects domains override", async () => {
    ...
    await discoverGatewayBeacons({
      platform: "darwin",
      timeoutMs: 1,
      domains: ["local", "actagent.internal"],
      run: run as unknown as typeof runCommandWithTimeout,
    });
    ...
    expect(browseDomains).toContain("actagent.internal.");

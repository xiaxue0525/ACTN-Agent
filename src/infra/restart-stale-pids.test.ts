...
const mockResolveGatewayPort = vi.hoisted(() => vi.fn(() => 19199));
...
function createACTAgentBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "actagent-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createACTAgentBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  beforeEach(() => {
    ...
    mockResolveGatewayPort.mockReturnValue(19199);
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses actagent-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "actagent-gateway" },
          { pid: process.pid, cmd: "actagent-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of actagent", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/actagent/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
      ...
      expect(findGatewayPidsOnPortSync(19199)).toEqual([stalePid]);
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: actagent-weixin sidecar (child of the gateway) invoked
      // cleanStaleGatewayProcessesSync during init. lsof reported the parent
      // gateway on port 19199, its PID was not process.pid, so the cleanup
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "actagent-gateway" },
          { pid: unrelatedStalePid, cmd: "actagent-gateway" },
        ]),
        stderr: "",
      });
      const pids = withStubbedPpid(parentGatewayPid, () => findGatewayPidsOnPortSync(19199));
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "actagent-gateway" },
            { pid: grandparentPid, cmd: "actagent-gateway" },
            { pid: benignStalePid, cmd: "actagent-gateway" },
          ]),
          stderr: "",
        });
        const pids = withStubbedPpid(directParentPid, () => findGatewayPidsOnPortSync(19199));
    ...
    it("excludes PID 1 when the direct parent gateway is the container entrypoint — container topology", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: 1, cmd: "actagent-gateway" },
          { pid: benignStalePid, cmd: "actagent-gateway" },
        ]),
        stderr: "",
      });
      const pids = withStubbedPpid(1, () => findGatewayPidsOnPortSync(19199));
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "actagent-gateway" },
            { pid: gatewayGrandparentPid, cmd: "actagent-gateway" },
          ]),
          stderr: "",
        });
        const pids = withStubbedPpid(pluginHostPid, () => findGatewayPidsOnPortSync(19199));
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "actagent-gateway" },
              { pid: gatewayGrandparentPid, cmd: "actagent-gateway" },
              { pid: benignStalePid, cmd: "actagent-gateway" },
            ]),
            stderr: "",
        ...
        const pids = withStubbedPpid(toolHostPid, () => findGatewayPidsOnPortSync(19199));
    ...
    it("excludes pids whose command does not include 'actagent'", () => {
    ...
    it("forwards the spawnTimeoutMs argument to spawnSync", () => {
      ...
      findGatewayPidsOnPortSync(19199, 400);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "actagent-gateway" }]),
            stderr: "",
          };
        });

        withStubbedPpid(gatewayParentPid, () => findGatewayPidsOnPortSync(19199, 400));
    ...
    it("deduplicates pids from dual-stack listeners (IPv4+IPv6 emit same pid twice)", () => {
      ...
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(19199);
    ...
    it("delegates to Windows port helpers on win32 and skips lsof", () => {
      ...
      try {
        mockReadWindowsListeningPids.mockReturnValue([]);
        expect(findGatewayPidsOnPortSync(19199)).toStrictEqual([]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(19199, undefined);
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["actagent", "gateway"]);
        expect(findGatewayPidsOnPortSync(19199)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(19199, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple actagent pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "actagent-gateway" },
          { pid: pid2, cmd: "actagent-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(19199);
    ...
    it("returns [] when status 0 but only non-actagent pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks actagent processes — non-actagent listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty actagent stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty actagent stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + actagent pid in stdout — container-restricted lsof reports partial results
          return createACTAgentBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "actagent-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("cleanStaleGatewayProcessesSync", () => {
    ...
    it("treats failed Windows port probes as inconclusive, not free", () => {
      ...
      try {
        ...
        expect(mockReadWindowsListeningPidsResult).toHaveBeenCalledWith(19199, 400);
        expectWarningContaining("port 19199 still in use after 2000ms");
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'actagent'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("actagent")
      // inside the mid-loop flush: a non-actagent cmd between two entries must not
      // be pushed, but the following actagent entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-actagent entry first, then actagent entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid actagent entry must still be found.
      const stdout = `p0\ncactagent-gateway\np${stalePid}\ncactagent-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(19199);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero actagent pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-actagent stdout (line 145)", () => {
    it("treats status 1 + non-actagent stdout as port-free (not an actagent process)", () => {
      // status 1 + non-empty stdout where no actagent pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill actagent pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-actagent output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // No actagent pids in status-1 output means the port is free for this cleanup.
      expect(cleanStaleGatewayProcessesSync()).toContain(stalePid);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return createLsofResult({
              stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "actagent-gateway" }]),
            });
          }
          return createLsofResult();
        });

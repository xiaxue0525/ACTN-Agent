...
const ACTAGENT_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "actagent.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.ACTAGENT_DIAGNOSTICS)) &&
    typeof env.ACTAGENT_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.ACTAGENT_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: ACTAGENT_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.ACTAGENT_DIAGNOSTICS_RUN_ID ? { runId: env.ACTAGENT_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.ACTAGENT_DIAGNOSTICS_ENV ? { envName: env.ACTAGENT_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.ACTAGENT_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }

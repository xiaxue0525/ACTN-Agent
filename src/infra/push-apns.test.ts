...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.actagent.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const actagentPayload = requireRecord(payload.actagent, "actagent payload");
    expectRecordFields(actagentPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof actagentPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.actagent.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open ACTAgent to review this request.",
      },
      sound: "default",
      category: "actagent.exec-approval",
      "content-available": 1,
    });
    const actagentPayload = requireRecord(payload.actagent, "actagent payload");
    expectRecordFields(actagentPayload, {
    ...
    expect(typeof actagentPayload.ts).toBe("number");
    expectNoProperties(actagentPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.actagent, "actagent payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });

...
vi.mock("./actagent-root.js", () => ({
  resolveACTAgentPackageRoot: vi.fn(async () => null),
  resolveACTAgentPackageRootSync: vi.fn(() => null),
}));
...
let actagentRoot: typeof import("./actagent-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    actagentRoot = await import("./actagent-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/actagent");
    const wrapperArgv1 = abs("fixtures/bin/actagent");
  ...
  it("uses resolveACTAgentPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/actagent");
    (
      actagentRoot.resolveACTAgentPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/actagent"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "actagent" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "actagent.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-actagent");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/actagent-bundle");
    (
      actagentRoot.resolveACTAgentPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/ACTAgent.app/Contents/MacOS/ACTAgent");
    const bundledUiDir = abs("fixtures/ACTAgent.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/actagent-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (

...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevACTAgentHome = process.env.ACTAGENT_HOME;

    try {
      process.env.ACTAGENT_HOME = dir;
      const approvalsPath = path.join(dir, ".actagent", "exec-approvals.json");
      ...
      if (prevACTAgentHome === undefined) {
        delete process.env.ACTAGENT_HOME;
      } else {
        process.env.ACTAGENT_HOME = prevACTAgentHome;
      }
    }
  });
});

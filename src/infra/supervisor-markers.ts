...
const SUPERVISOR_HINTS = {
  launchd: ["ACTAGENT_LAUNCHD_LABEL"],
  systemd: ["ACTAGENT_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["ACTAGENT_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "ACTAGENT_SERVICE_MARKER",
  "ACTAGENT_SERVICE_KIND",
] as const;
...
function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.ACTAGENT_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.ACTAGENT_SERVICE_MARKER?.trim();

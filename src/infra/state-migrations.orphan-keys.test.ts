...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".actagent");
...
function sharedMainOpsConfig(sharedStorePath: string): ACTAgentConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: ACTAgentConfig = OPS_WORK_CONFIG) {

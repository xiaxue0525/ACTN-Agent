// Validates the current runtime against ACTAgent's Node engine floor.
...
/** Returns whether a detected runtime meets ACTAgent's minimum runtime contract. */
...
/** Checks a Node version label against ACTAgent's current minimum Node version. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "actagent requires Node >=22.19.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run actagent.",
    ].join("\n"),
  );
  runtime.exit(1);
}

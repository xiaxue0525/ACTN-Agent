...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/actagent-state", "devices")).toEqual({
      dir: path.join("/tmp/actagent-state", "devices"),
      pendingPath: path.join("/tmp/actagent-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/actagent-state", "devices", "paired.json"),
    });
  });

...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerACTAgentRestart() uses schtasks to restart the gateway.
...
export function triggerACTAgentRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ACTAGENT_SYSTEMD_UNIT,
      process.env.ACTAGENT_PROFILE,
    );
  ...
  const label =
    process.env.ACTAGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ACTAGENT_PROFILE);

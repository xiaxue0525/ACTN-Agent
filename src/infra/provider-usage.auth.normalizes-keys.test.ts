...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "actagent-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".actagent");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".actagent", "agents", "main", "agent");
  }

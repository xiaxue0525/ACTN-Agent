...
import { asNullableObjectRecord as readRecord } from "@actagent/normalization-core/record-coerce";
import {
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ACTAGENT_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for ACTAgent. You can keep running the web gateway without it: \`${formatCliCommand("actagent gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
...
function tailscaleProxyMatchesLoopbackPort(proxy: string, port: number): boolean {
  ...
  // "http://127.0.0.1:19199", "http://127.0.0.1:19199/",
  // "https+insecure://localhost:19199/api"), or as the bare forms accepted
  // by `tailscale funnel/serve` ("localhost:19199", "19199"). Strip any

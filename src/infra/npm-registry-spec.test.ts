...
import {
  ...
  isACTAgentStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@actagent/voice-call",
    "@actagent/voice-call@1.2.3",
    "@actagent/voice-call@1.2.3-beta.4",
    "@actagent/voice-call@latest",
    "@actagent/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    {
      spec: "@actagent/voice-call@^1.2.3",
    ...
    {
      spec: "git+ssh://github.com/actagent/actagent",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@actagent/voice-call@2026.5.3-1",
      expected: {
        name: "@actagent/voice-call",
        raw: "@actagent/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@actagent/voice-call@1.2.3-beta.1",
      expected: {
        name: "@actagent/voice-call",
        raw: "@actagent/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@actagent/voice-call", expected: true },
    { spec: "@actagent/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@actagent/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects ACTAgent-org npm specs for %s", ({ spec, expected }) => {
    expect(isACTAgentOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects ACTAgent stable correction versions for %s", ({ value, expected }) => {
    expect(isACTAgentStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares ACTAgent release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareACTAgentReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("npm prerelease resolution policy", () => {
  it.each([
    ...
    {
      spec: "@actagent/voice-call@latest",
  ...
  it.each([
    {
      spec: "@actagent/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@actagent/voice-call@beta"`,
    },
    {
      spec: "@actagent/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",

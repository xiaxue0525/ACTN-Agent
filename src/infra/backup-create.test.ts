...
import { closeACTAgentAgentDatabasesForTest } from "../state/actagent-agent-db.js";
import {
  closeACTAgentStateDatabase,
  openACTAgentStateDatabase,
} from "../state/actagent-state-db.js";
import { withACTAgentTestState } from "../test-utils/actagent-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "actagent-backup-2026-01-01",
    archivePath: "/tmp/actagent-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/actagent-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.actagent",
        "Skipped 1 path:",
        "- workspace: ~/Projects/actagent (covered by ~/.actagent)",
        "Created /tmp/actagent-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.actagent/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.actagent/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.actagent/config.json",
        "- credentials: ~/.actagent/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/actagent-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.actagent",
      "Created /tmp/actagent-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/actagent.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.actagent\\");

    expect(filter(String.raw`C:\Users\me\.actagent\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.actagent\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("falls back when injected nowMs is outside Date range", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-invalid-now-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("actagent-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("skips current live volatile state files while preserving workspace locks", async () => {
    await withACTAgentTestState(
      {
        layout: "split",
        prefix: "actagent-backup-volatile-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("scrubs transient SQLite delivery queue rows from archive snapshots", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-sqlite-queue-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const { db } = openACTAgentStateDatabase({ env: state.env });
        db.prepare(
          `
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/actagent.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/actagent.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores into the archive", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-agent-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/actagent-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/actagent-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "actagent.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/actagent.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "actagent-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withACTAgentTestState(
      {
        layout: "state-only",
        prefix: "actagent-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");

// Tests ACTAgent home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ACTAGENT_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands ACTAGENT_HOME when set to ~", () => {
    const env = {
      ACTAGENT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ACTAGENT_HOME",
      env: { ACTAGENT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ACTAGENT_HOME is tilde-only and no fallback home exists",
      env: { ACTAGENT_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ACTAGENT_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ACTAGENT_HOME: "/srv/actagent-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/actagent-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/actagent-home " },
      expected: "/srv/actagent-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/actagent-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ACTAGENT_HOME", () => {
    expect(

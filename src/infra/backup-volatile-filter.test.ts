...
const stateDir = "/opt/actagent/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/actagent/state/sessions/../config.jsonl` resolves to
    // `/opt/actagent/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\actagent\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/actagent/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });

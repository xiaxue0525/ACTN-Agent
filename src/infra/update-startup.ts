...
import { resolveACTAgentPackageRoot } from "./actagent-root.js";
...
function resolveAutoUpdatePolicy(cfg: ACTAgentConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: ACTAgentConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.ACTAGENT_NO_AUTO_UPDATE);
  ...
  const root = await resolveACTAgentPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("actagent update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by ACTAGENT_NO_AUTO_UPDATE", {

...
const DEFAULT_SOCKET = "~/.actagent/exec-approvals.sock";
const DEFAULT_FILE = "~/.actagent/exec-approvals.json";
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "actagent" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "actagent") {
    return false;
  }
  offset += 1;

 ...
 * returning control to the caller (typically `triggerACTAgentRestart` →
 ...
 * the parent gateway listening on 19199, the parent's PID passes the
...
function parsePidsFromLsofOutput(stdout: string, spawnTimeoutMs: number): number[] {
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("actagent")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an actagent
 ...
 * Returns only PIDs that belong to actagent gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only actagent gateway processes.

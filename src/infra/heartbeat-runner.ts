...
import { timestampMsToIsoString } from "@actagent/normalization-core/number-coercion";
import {
...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: ACTAgentConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ACTAgentConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: ACTAgentConfig) {
...
function resolveHeartbeatAgents(cfg: ACTAgentConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: ACTAgentConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: ACTAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: ACTAgentConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: ACTAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: ACTAgentConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: ACTAgentConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ACTAgentConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();

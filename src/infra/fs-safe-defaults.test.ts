// Covers ACTAgent's default fs-safe Python helper configuration.
...
vi.mock("@actagent/fs-safe/config", () => ({
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.ACTAGENT_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in ACTAgent", async () => {
  ...
  it("honors the ACTAgent-specific env mode override", async () => {
    process.env.ACTAGENT_FS_SAFE_PYTHON_MODE = "auto";

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ACTAgentConfig | null = null;
   ...
   */
  get(key: string, cfg: ACTAgentConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: ACTAgentConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ACTAgentConfig): void {

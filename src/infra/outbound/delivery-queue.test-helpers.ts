...
export function installDeliveryQueueTmpDirHooks(): { readonly tmpDir: () => string } {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(resolvePreferredACTAgentTmpDir(), "actagent-dq-suite-"));
  });
...
export function readQueuedEntry(tmpDir: string, id: string): Record<string, unknown> {
  const { db } = openACTAgentStateDatabase({ env: { ...process.env, ACTAGENT_STATE_DIR: tmpDir } });
  const row = db
    .prepare(

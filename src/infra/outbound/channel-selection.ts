...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `actagent agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: ACTAgentConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: actagent doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run actagent channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use actagent channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: ACTAgentConfig): Promise<boolean> {

...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "ACTAGENT_LIVE_ANTHROPIC_KEY",
  "ACTAGENT_LIVE_ANTHROPIC_KEYS",
  "ACTAGENT_LIVE_GEMINI_KEY",
  "ACTAGENT_LIVE_OPENAI_KEY",
  "ACTAGENT_GATEWAY_TOKEN",
  "ACTAGENT_GATEWAY_PASSWORD",
  "ACTAGENT_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ACTAGENT_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ACTAGENT_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ACTAGENT_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ACTAGENT_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.actagent/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.actagent/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".actagent");
        process.env.ACTAGENT_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "actagent", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ACTAGENT_STATE_DIR=./evil-state",
            "ACTAGENT_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.ACTAGENT_STATE_DIR).toBe(stateDir);
        expect(process.env.ACTAGENT_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ACTAGENT_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ACTAGENT_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ACTAGENT_LIVE_GEMINI_KEY=sk-gemini-live",
            "ACTAGENT_LIVE_OPENAI_KEY=sk-openai-live",
            "ACTAGENT_GATEWAY_TOKEN=attacker-token",
            "ACTAGENT_GATEWAY_PASSWORD=attacker-password",
            "ACTAGENT_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.ACTAGENT_STATE_DIR).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ACTAGENT_AGENT_DIR=./evil-agent",
            `ACTAGENT_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "ACTAGENT_OAUTH_DIR=./evil-oauth",
            "PI_CODING_AGENT_DIR=./evil-pi-agent",
          ].join("\n"),
        );

        delete process.env.ACTAGENT_AGENT_DIR;
        delete process.env.ACTAGENT_BUNDLED_PLUGINS_DIR;
        delete process.env.ACTAGENT_OAUTH_DIR;
        delete process.env.PI_CODING_AGENT_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ACTAGENT_AGENT_DIR).toBeUndefined();
        expect(process.env.ACTAGENT_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.ACTAGENT_OAUTH_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ACTAGENT_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ACTAGENT_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ACTAGENT_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ACTAGENT_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'ACTAGENT_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.ACTAGENT_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.ACTAGENT_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ACTAGENT_PINNED_PYTHON=./attacker-python",
            "ACTAGENT_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ACTAGENT_PINNED_PYTHON;
        delete process.env.ACTAGENT_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ACTAGENT_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ACTAGENT_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ACTAGENT_PINNED_PYTHON=/trusted/python",
            "ACTAGENT_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ACTAGENT_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ACTAGENT_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ACTAGENT_LIVE_GEMINI_KEY=sk-gemini-live",
            "ACTAGENT_LIVE_OPENAI_KEY=sk-openai-live",
            "ACTAGENT_GATEWAY_TOKEN=trusted-token",
            "ACTAGENT_GATEWAY_PASSWORD=trusted-password",
            "ACTAGENT_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ACTAGENT_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ACTAGENT_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ACTAGENT_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ACTAGENT_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ACTAGENT_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ACTAGENT_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ACTAGENT_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ACTAGENT_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks ACTAGENT_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "ACTAGENT_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when ACTAGENT_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.ACTAGENT_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "actagent", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.ACTAGENT_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".actagentdbot");
      process.env.HOME = base;
      delete process.env.ACTAGENT_STATE_DIR;
      delete process.env.ACTAGENT_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/actagent.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ACTAGENT_MDNS_HOSTNAME",
          "ACTAGENT_SESSION_CACHE_TTL_MS",
          "ACTAGENT_UPDATE_PACKAGE_SPEC",
          "ACTAGENT_GATEWAY_PORT",
          "ACTAGENT_GATEWAY_URL",
          "ACTAGENT_ACTAGENTHUB_URL",
          "ACTAGENTHUB_URL",
          "ACTAGENT_ACTAGENTHUB_TOKEN",
          "ACTAGENTHUB_TOKEN",
          "ACTAGENTHUB_AUTH_TOKEN",
          "ACTAGENTHUB_CONFIG_PATH",
          "ACTAGENT_DISABLE_BUNDLED_PLUGINS",
          "ACTAGENT_ALLOW_INSECURE_PRIVATE_WS",
          "ACTAGENT_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "ACTAGENT_SKIP_PROVIDERS",
          "ACTAGENT_SKIP_CRON",
          "ACTAGENT_RAW_STREAM",
          "ACTAGENT_RAW_STREAM_PATH",
          "ACTAGENT_CACHE_TRACE",
          "ACTAGENT_CACHE_TRACE_FILE",
          "ACTAGENT_CACHE_TRACE_MESSAGES",
          "ACTAGENT_CACHE_TRACE_PROMPT",
          "ACTAGENT_CACHE_TRACE_SYSTEM",
          "ACTAGENT_SHOW_SECRETS",
          "ACTAGENT_PLUGIN_CATALOG_PATHS",
          "ACTAGENT_MPM_CATALOG_PATHS",
          "ACTAGENT_NODE_EXEC_HOST",
          "ACTAGENT_NODE_EXEC_FALLBACK",
          "ACTAGENT_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("actagent/actagent");

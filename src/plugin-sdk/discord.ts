...
/**
 * @deprecated Compatibility facade for the `actagent/plugin-sdk/discord` subpath.
...
export type { ACTAgentPluginApi, PluginRuntime } from "./channel-plugin-common.js";
...
/** Discord channel config shape for one account in ACTAgent config. */
export type DiscordAccountConfig = NonNullable<NonNullable<ACTAgentConfig["channels"]>["discord"]>;
...
type DiscordApiFacadeModule = {
  ...
  inspectDiscordAccount: (params: { cfg: ACTAgentConfig; accountId?: string | null }) => unknown;
  listDiscordAccountIds: (cfg: ACTAgentConfig) => string[];
  ...
  resolveDefaultDiscordAccountId: (cfg: ACTAgentConfig) => string;
...
function resolveCompatRuntimeConfig(params: { cfg?: ACTAgentConfig }): ACTAgentConfig {
...
/** List configured Discord account ids from ACTAgent config. */
export function listDiscordAccountIds(cfg: ACTAgentConfig): string[] {
...
export function resolveDefaultDiscordAccountId(cfg: ACTAgentConfig): string {

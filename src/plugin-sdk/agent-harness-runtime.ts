...
export { VERSION as ACTAGENT_VERSION } from "../version.js";
...
export { resolveACTAgentAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@actagent/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded ACTAgent uses for progress logs.
 */
 ...
 * preserve the same ACTAgent-owned fallback signals as the built-in ACTAgent path

...
let cachedACTAgentPackageRoot: string | undefined;

function getACTAgentPackageRoot() {
  if (cachedACTAgentPackageRoot) {
    return cachedACTAgentPackageRoot;
  }
  cachedACTAgentPackageRoot =
...
function resolveFacadeModuleLocation(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getACTAgentPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getACTAgentPackageRoot()
        ? "ACTAgent package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      preparedLocation.boundaryRoot === getACTAgentPackageRoot()
        ? "ACTAgent package root"
        : "plugin root",
    rejectHardlinks: false,
  });
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedACTAgentPackageRoot = undefined;
}

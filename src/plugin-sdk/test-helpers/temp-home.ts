// Temp home test helpers create isolated ACTAgent home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ACTAGENT_HOME.
  delete process.env.ACTAGENT_HOME;
  process.env.ACTAGENT_STATE_DIR = path.join(base, ".actagent");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "actagent-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".actagent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {

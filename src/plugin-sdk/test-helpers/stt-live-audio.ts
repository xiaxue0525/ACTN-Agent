...
export const ACTAGENT_LIVE_TRANSCRIPT_MARKER_RE = /open(?:actagent|cl|flaw|clar|core)/;

export function expectACTAgentLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(ACTAGENT_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? ACTAGENT_LIVE_TRANSCRIPT_MARKER_RE;

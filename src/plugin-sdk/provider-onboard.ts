...
import { findNormalizedProviderKey } from "@actagent/model-catalog-core/provider-id";
...
export type { ACTAgentConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: ACTAgentConfig, ...args: TArgs) => ACTAgentConfig;
  applyConfig: (cfg: ACTAgentConfig, ...args: TArgs) => ACTAgentConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: ACTAgentConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: ACTAgentConfig, preset: TParams) => ACTAgentConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: ACTAgentConfig): {
  next: ACTAgentConfig;
  changed: boolean;
} {

...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "actagent", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "actagent", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "actagent", "--profile", "work", "gateway", "status"],

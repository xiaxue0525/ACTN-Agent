...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveACTAgentHubBaseUrlMock: vi.fn((baseUrl?: string) =>
...
vi.mock("../infra/actagenthub.js", () => ({
  downloadACTAgentHubSkillArchive: mocks.noopAsync,
  fetchACTAgentHubSkillCard: (...args: unknown[]) => mocks.fetchACTAgentHubSkillCardMock(...args),
  fetchACTAgentHubSkillDetail: mocks.noopAsync,
  fetchACTAgentHubSkillVerification: (...args: unknown[]) =>
    mocks.fetchACTAgentHubSkillVerificationMock(...args),
  resolveACTAgentHubBaseUrl: (baseUrl?: string) => mocks.resolveACTAgentHubBaseUrlMock(baseUrl),
  searchACTAgentHubSkills: mocks.noopAsync,
}));
...
describe("skills verify CLI", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-skill-verify-cli-"));
    ...
    mocks.fetchACTAgentHubSkillVerificationMock.mockReset();
    mocks.fetchACTAgentHubSkillCardMock.mockReset();
  ...
  async function writeInstalledGeneratedCardSkill() {
    ...
    await fs.writeFile(
      ...
      "# Generated Skill Card\n\nThis file is added by ACTAgentHub during bundle assembly.\n",
      "utf8",
    );
    await fs.writeFile(
  ...
  it("does not reject an installed bundle just because ACTAgentHub generated skill-card.md", async () => {
    await writeInstalledGeneratedCardSkill();
    mocks.fetchACTAgentHubSkillVerificationMock.mockResolvedValueOnce({
    ...
    expect(mocks.fetchACTAgentHubSkillVerificationMock).toHaveBeenCalledWith({
      slug: "agentreceipt",
      version: "1.2.3",
      tag: undefined,

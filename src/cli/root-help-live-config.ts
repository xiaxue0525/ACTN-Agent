...
export function hasPluginHelpAffectingConfig(config: ACTAgentConfig | null | undefined): boolean {
...
export function hasPluginHelpAffectingEnv(env: NodeJS.ProcessEnv): boolean {
  return Boolean(
    env.ACTAGENT_BUNDLED_PLUGINS_DIR?.trim() || env.ACTAGENT_DISABLE_BUNDLED_PLUGINS?.trim(),
  );
}

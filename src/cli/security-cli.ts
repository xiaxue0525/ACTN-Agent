...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent security audit", "Run a local security audit."],
          [
            "actagent security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["actagent security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "actagent security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "actagent security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["actagent security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["actagent security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.actagent.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("ACTAgent security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("actagent security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("actagent security audit --fix")}`));
        if (!fixResult) {

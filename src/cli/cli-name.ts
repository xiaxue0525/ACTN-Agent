...
const DEFAULT_CLI_NAME = "actagent";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(actagent)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `actagent`. */
...
/** Replace a leading `actagent` command prefix with the active CLI name. */

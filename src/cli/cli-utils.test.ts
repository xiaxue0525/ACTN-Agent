...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "actagent", "terminal"] },
    { argv: ["node", "actagent", "chat"] },
    { argv: ["node", "actagent", "gateway"] },
    { argv: ["node", "actagent", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "actagent", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "actagent", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "actagent", "gateway", "status"] },
    { argv: ["node", "actagent", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "actagent", "--help"] },
    { argv: ["node", "actagent", "gateway"] },
    { argv: ["node", "actagent", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "actagent.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("actagent.internal");
    } finally {
      log.mockRestore();
    }
  });

...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.actagent.ai/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`actagent doctor --fix\` to normalize legacy cron job storage.\n`,
    );

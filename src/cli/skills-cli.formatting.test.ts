...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-bundled-skills-test-"));
    process.env.ACTAGENT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

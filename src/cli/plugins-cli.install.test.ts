...
const CLI_STATE_ROOT = "/tmp/actagent-state";
const ORIGINAL_ACTAGENT_STATE_DIR = process.env.ACTAGENT_STATE_DIR;
const ORIGINAL_ACTAGENT_NIX_MODE = process.env.ACTAGENT_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/actagent-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.ACTAGENT_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): ACTAgentConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): ACTAgentConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createACTAgentHubInstallResult(params: {
  pluginId: string;
  packageName: string;
  version: string;
  channel: string;
  ...
  return {
    ...
    actagenthub: {
      ...
      actagenthubPackage: params.packageName,
      actagenthubFamily: "code-plugin",
      actagenthubChannel: params.channel,
      version: params.version,
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@actagent/${pluginId}`,
    npmTarballName: `actagent-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@actagent/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@actagent/${pluginId}@1.2.3`,
...
function mockACTAgentHubPackageNotFound(packageName: string) {
  installPluginFromACTAgentHub.mockResolvedValue({
    ok: false,
    error: `ACTAgentHub /api/v1/packages/${packageName} failed (404): Package not found`,
...
function primeNpmPluginFallback(pluginId = "demo") {
  ...
  mockACTAgentHubPackageNotFound(pluginId);
...
function createPathHookPackInstalledConfig(tmpRoot: string): ACTAgentConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): ACTAgentConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  const cfg = {} as ACTAgentConfig;
  const installedCfg = createNpmHookPackInstalledConfig();

  loadConfig.mockReturnValue(cfg);
  mockACTAgentHubPackageNotFound("@acme/demo-hooks");
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing actagent.plugin.json",
  });
...
function primeBlockedNpmPluginInstall(params: {
  ...
  loadConfig.mockReturnValue({} as ACTAgentConfig);
  mockACTAgentHubPackageNotFound(params.spec);
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
...
function actAgentHubInstallCall(callIndex = 0): PluginInstallCall {
  return mockCallArg(installPluginFromACTAgentHub, callIndex) as PluginInstallCall;
}
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: ACTAgentConfig } {
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_ACTAGENT_STATE_DIR === undefined) {
      delete process.env.ACTAGENT_STATE_DIR;
    } else {
      process.env.ACTAGENT_STATE_DIR = ORIGINAL_ACTAGENT_STATE_DIR;
    }
    if (ORIGINAL_ACTAGENT_NIX_MODE === undefined) {
      delete process.env.ACTAGENT_NIX_MODE;
    } else {
      process.env.ACTAGENT_NIX_MODE = ORIGINAL_ACTAGENT_NIX_MODE;
    }
  });
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `actagent doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("installs ACTAgentHub plugins and persists source metadata", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
    ...
    loadConfig.mockReturnValue(cfg);
    parseACTAgentHubPluginSpec.mockReturnValue({ name: "demo" });
    installPluginFromACTAgentHub.mockResolvedValue(
      createACTAgentHubInstallResult({
        pluginId: "demo",
        packageName: "demo",
        version: "1.2.3",
        channel: "official",
    ...
    await runPluginsCommand(["plugins", "install", "actagenthub:demo"]);

    expect(actAgentHubInstallCall().spec).toBe("actagenthub:demo");
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("actagenthub");
    expect(record.spec).toBe("actagenthub:demo");
    expect(record.installPath).toBe(cliInstallPath("demo"));
    expect(record.version).toBe("1.2.3");
    expect(record.actagenthubPackage).toBe("demo");
    expect(record.actagenthubFamily).toBe("code-plugin");
    expect(record.actagenthubChannel).toBe("official");
    expect(record.actagentpackSha256).toBe(
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    );
    expect(record.actagentpackSpecVersion).toBe(1);
    expect(record.actagentpackManifestSha256).toBe(
      "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
    );
    expect(record.actagentpackSize).toBe(4096);
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
    expect(runtimeLogsContain("Installed plugin: demo")).toBe(true);
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
  });

  it("passes the active profile extensions dir to ACTAgentHub installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("passes force through as overwrite mode for ACTAgentHub installs", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
    ...
    await runPluginsCommand(["plugins", "install", "actagenthub:demo", "--force"]);

    expect(actAgentHubInstallCall().spec).toBe("actagenthub:demo");
    expect(actAgentHubInstallCall().mode).toBe("update");
  });

  it("keeps explicit ACTAgentHub versions pinned in install records", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
    ...
    loadConfig.mockReturnValue(cfg);
    parseACTAgentHubPluginSpec.mockReturnValue({ name: "demo", version: "1.2.3" });
    installPluginFromACTAgentHub.mockResolvedValue(
      createACTAgentHubInstallResult({
        pluginId: "demo",
        packageName: "demo",
    ...
    await runPluginsCommand(["plugins", "install", "actagenthub:demo@1.2.3"]);

    expect(actAgentHubInstallCall().spec).toBe("actagenthub:demo@1.2.3");
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("actagenthub");
    expect(record.spec).toBe("actagenthub:demo@1.2.3");
    expect(record.installPath).toBe(cliInstallPath("demo"));
    expect(record.version).toBe("1.2.3");
    expect(record.actagenthubPackage).toBe("demo");
    expect(record.actagentpackSha256).toBe(
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(installPluginFromACTAgentHub).not.toHaveBeenCalled();
    expect(npmInstallCall().spec).toBe("@actagent/brave-plugin");
    ...
    expect(record.spec).toBe("@actagent/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-actagent-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-actagent-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-actagent-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-actagent-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing actagent.extensions",
      code: "missing_actagent_extensions",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-actagent-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-actagent-plugin@2026.5.7");
    expect(hookNpmInstallCall().expectedIntegrity).toBe(
      "sha512-TCkP9as00WfEhgFWG8YL/rcmaWGIshAki2HQh83nTRccGfVBCoGjrEboTTqq3yDmK9koWTV11zi8u8A4dNtvug==",
    );
  });

  it("installs ordinary bare plugin specs through npm without ACTAgentHub lookup", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    loadConfig.mockReturnValue(cfg);
  ...
  it("passes bare npm selectors through npm without ACTAgentHub lookup", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    loadConfig.mockReturnValue(cfg);
    installPluginFromNpmSpec.mockResolvedValue(createNpmPluginInstallResult("demo"));
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/actagent-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@actagent/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("actagent-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@actagent/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@actagent/discord"]);
  ...
  it("uses bundled ACTAgent package specs instead of pinning stale managed npm overrides", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("discord");
    ...
    findBundledPluginSourceMock.mockImplementation((params: unknown) => {
      ...
      return lookup.kind === "npmSpec" && lookup.value === "@actagent/discord"
        ? {
            pluginId: "discord",
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@actagent/discord@2026.5.20" },
    });
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@actagent/discord" },
    });
    const record = persistedInstallRecord("discord");
    expect(record.source).toBe("path");
    expect(record.spec).toBe("@actagent/discord@2026.5.20");
    expect(record.sourcePath).toBe(bundledPath);
    expect(record.installPath).toBe(bundledPath);
    expect(runtimeLogsContain("ships with the current ACTAgent build")).toBe(true);
    expect(runtimeLogsContain("npm:@actagent/discord@2026.5.20")).toBe(true);
  });
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-actagent-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-actagent-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-actagent-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromACTAgentHub).not.toHaveBeenCalled();
  ...
  it("passes npm: prefix installs through npm options without ACTAgentHub lookup", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("reports npm install failures without trying ACTAgentHub when npm: prefix is used", async () => {
    loadConfig.mockReturnValue({} as ACTAgentConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing actagent.hooks",
    });
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@actagent/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as ACTAgentConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("actagent plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    loadConfig.mockReturnValue({} as ACTAgentConfig);
    mockACTAgentHubPackageNotFound("@example/lossless-actagent");
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/actagent/.actagent/extensions/lossless-actagent (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing actagent.hooks",
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "@example/lossless-actagent"]),
    ).rejects.toThrow("__exit__:1");

    expect(runtimeErrors.at(-1)).toContain(
      "Use `actagent plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("does not fall back to hook pack for npm installs when dangerous force unsafe install is set", async () => {
    ...
    installPluginFromACTAgentHub.mockResolvedValue({
      ok: false,
      error: "ACTAgentHub /api/v1/packages/demo failed (404): Package not found",
      code: "package_not_found",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-local-hook-pack-"));
    const cfg = {} as ACTAgentConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("still falls back to npm hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    ...
    installPluginFromACTAgentHub.mockResolvedValue({
      ok: false,
      error: "ACTAgentHub /api/v1/packages/@acme/demo-hooks failed (404): Package not found",
      code: "package_not_found",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("does not fall back to npm when explicit ACTAgentHub rejects a real package", async () => {
    parseACTAgentHubPluginSpec.mockReturnValue({ name: "demo" });
    installPluginFromACTAgentHub.mockResolvedValue({
      ok: false,
      error: 'Use "actagent skills install demo" instead.',
      code: "skill_package",
    });

    await expect(runPluginsCommand(["plugins", "install", "actagenthub:demo"])).rejects.toThrow(
      "__exit__:1",
    );

    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "actagent skills install demo" instead.');
  });

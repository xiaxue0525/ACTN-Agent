...
const CLI_STATE_ROOT = "/tmp/actagent";
...
describe("npm-resolution helpers", () => {
  it("keeps the requested selector when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@actagent/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@actagent/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@actagent/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@actagent/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @actagent/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@actagent/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@actagent/plugin-alpha@latest",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@actagent/plugin-alpha@1.2.3",
      installPath: ALPHA_INSTALL_PATH,
      version: "1.2.3",
      resolvedName: "@actagent/plugin-alpha",
      resolvedVersion: "1.2.3",
    ...
    expect(logs).toEqual(["Pinned npm install record to @actagent/plugin-alpha@1.2.3."]);
  ...
  it("keeps install record selector for CLI unless --pin is requested", () => {
    ...
    expect(record.spec).toBe("@actagent/plugin-alpha");
    expect(logs).toEqual([]);
  });
});

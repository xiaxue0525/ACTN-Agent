...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/actagent-stability.json",
  path: "/tmp/actagent-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/actagent-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/actagent-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/actagent-test-missing-config.json",
  normalizeStateDirEnv: (env?: NodeJS.ProcessEnv) => normalizeStateDirEnv(env),
  resolveStateDir: () => "/tmp",
  resolveGatewayPort: (cfg?: { gateway?: { port?: number } }) => cfg?.gateway?.port ?? 19199,
}));
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/actagent-control-ui";
  ...
  function gatewayStartOptions(index = 0) {
    expect(startGatewayServer.mock.calls[index]?.[0]).toBe(19199);
  ...
  it("forwards parent-captured options to `gateway run` subcommand", async () => {
    ...
    expect(callArg(forceFreePortAndWait, 0, 0)).toBe(19199);
    expect(callArg(waitForPortBindable, 0, 0)).toBe(19199);
    expect(
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.ACTAGENT_SERVICE_MARKER;
    process.env.ACTAGENT_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        delete process.env.ACTAGENT_SERVICE_MARKER;
      } else {
        process.env.ACTAGENT_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.ACTAGENT_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.ACTAGENT_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("does not write startup failure bundles for expected gateway lock conflicts", async () => {
    const err = Object.assign(new Error("gateway already running on port 19199"), {
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `actagent onboard --mode local` or `actagent setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/actagent-test-missing-config.json",
      config: {},
      parsed: null,

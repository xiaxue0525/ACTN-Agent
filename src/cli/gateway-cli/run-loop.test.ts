...
>(() => ({ mode: "disabled", detail: "ACTAGENT_NO_RESPAWN" }));
...
describe("runGatewayLoop", () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(gatewayLog.warn).toHaveBeenNthCalledWith(
        ...
        "An unauthorized SIGUSR1 restart signal was received and ignored. " +
          "If a pending gateway restart needs to be applied, run `actagent gateway restart` " +
  ...
  it("releases the lock before exiting on spawned restart", async () => {
    vi.clearAllMocks();
    peekGatewaySigusr1RestartReason.mockReturnValue(undefined);
    const originalTraceEnv = process.env.ACTAGENT_GATEWAY_RESTART_TRACE;
    process.env.ACTAGENT_GATEWAY_RESTART_TRACE = "1";
    ...
    try {
      await withIsolatedSignals(async ({ captureSignal }) => {
        ...
        expect(respawnOpts?.env?.ACTAGENT_GATEWAY_RESTART_TRACE_STARTED_AT_MS).toMatch(/^\d/u);
        expect(respawnOpts?.env?.ACTAGENT_GATEWAY_RESTART_TRACE_LAST_AT_MS).toMatch(/^\d/u);
        expect(writeGatewayRestartHandoffSync).not.toHaveBeenCalled();
      });
    } finally {
      if (originalTraceEnv === undefined) {
        delete process.env.ACTAGENT_GATEWAY_RESTART_TRACE;
      } else {
        process.env.ACTAGENT_GATEWAY_RESTART_TRACE = originalTraceEnv;
      }
    }
  });
  ...
  it("waits briefly before exiting on launchd supervised restart", async () => {
    ...
    try {
      ...
      vi.useRealTimers();
      delete process.env.ACTAGENT_LAUNCHD_LABEL;
      if (originalPlatformDescriptor) {
  ...
  it("forwards lockPort to initial and restart lock acquisitions", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(1, { port: 19199 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(2, { port: 19199 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(3, { port: 19199 });
  ...
  it("hard-respawns update restarts and exits only after the replacement becomes healthy", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      const { start, started } = createSignaledStart(close);
      const { runtime, exited } = createRuntimeWithExitSignal();
      await runLoopWithStart({ start, runtime, lockPort: 19199, waitForHealthyChild });
      await waitForStart(started);
      const sigusr1 = captureSignal("SIGUSR1");
      ...
      await expect(exited).resolves.toBe(0);
      expect(waitForHealthyChild).toHaveBeenCalledWith(19199, 7777, "127.0.0.1");
  ...
  it("probes the configured gateway host for update respawn health", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      await expect(exited).resolves.toBe(0);
      expect(waitForHealthyChild).toHaveBeenCalledWith(19199, 7778, "10.0.0.25");
    });
  });
  ...
  it("marks update respawn failures and falls back to in-process restart", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(waitForHealthyChild).toHaveBeenCalledWith(19199, 8888, "127.0.0.1");
...
describe("gateway discover routing helpers", () => {
  ...
  it("prefers resolved service port over TXT gatewayPort", () => {
    ...
    expect(pickGatewayPort(beacon)).toBe(19199);
  });

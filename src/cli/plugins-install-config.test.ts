...
import { bundledPluginRootAt, repoInstallSpec } from "actagent/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { ACTAgentConfig } from "../config/config.js";
import type { ConfigFileSnapshot } from "../config/types.actagent.js";
import {
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: ACTAgentConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as ACTAgentConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as ACTAgentConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@actagent/discord",
    normalizedSpec: "@actagent/discord",
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(async (cfg: ACTAgentConfig) => [
      {
        config: cfg,
        changes: [],
      },
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as ACTAgentConfig;
  ...
  it("does not run stale Discord cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as ACTAgentConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        valid: true,
        sourceConfig: cfg,
  ...
  it("allows Brave official plugin reinstall recovery from source-only runtime shadows", async () => {
    const snapshotCfg = {
      plugins: { installs: { brave: { source: "actagenthub", installPath: "/bad/brave" } } },
    } as unknown as ACTAgentConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        ...
        issues: [
          ...
          },
          {
            ...
            message:
              'web_search provider is not available: brave (install or enable plugin "brave", then run actagent doctor --fix)',
          },
        ],
      }),
    );
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as ACTAgentConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `actagent doctor --fix` before installing plugins.");
  });

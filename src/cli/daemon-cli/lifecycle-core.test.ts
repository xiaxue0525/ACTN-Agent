...
const loadConfig = vi.fn<() => ACTAgentConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      ACTAGENT_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { ACTAGENT_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("ACTAGENT_CONTAINER_HINT", "actagent-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for actagent-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { ACTAGENT_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("ACTAGENT_GATEWAY_TOKEN", "env-token");
  ...
  it("emits stopped when an unmanaged process handles stop", async () => {
    ...
    await runServiceStop({
      ...
      onNotLoaded: async () => ({
        result: "stopped",
        message: "Gateway stop signal sent to unmanaged process on port 19199: 4200.",
      }),
    });
  ...
  it("runs restart health checks after an unmanaged restart signal", async () => {
    ...
    await runServiceRestart({
      ...
      onNotLoaded: async () => ({
        result: "restarted",
        message: "Gateway restart signal sent to unmanaged process on port 19199: 4200.",
      }),
      postRestartCheck,
    });
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["actagent", "gateway"],
    ...
    const repairLoadedService = vi.fn(async () => ({
      ...
      warnings: ["service was installed by ACTAgent 2026.4.24, current CLI is 2026.5.2"],
      loaded: true,
    }));
    ...
    expect(payload.warnings?.[0]).toContain("service was installed by ACTAgent");
    expect(payload.service?.loaded).toBe(true);
  });
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["actagent gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["actagent gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints?.includes("actagent gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "actagent gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });

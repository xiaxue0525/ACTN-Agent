...
async function confirmGatewayReachable(params: {
  ...
  const token = normalizeOptionalString(params.auth?.token ?? process.env.ACTAGENT_GATEWAY_TOKEN);
  const password = normalizeOptionalString(
    params.auth?.password ?? process.env.ACTAGENT_GATEWAY_PASSWORD,
  );

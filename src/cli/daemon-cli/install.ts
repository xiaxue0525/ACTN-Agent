...
import { ACTAGENT_WRAPPER_ENV_KEY, resolveACTAgentWrapperPath } from "../../daemon/program-args.js";
...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === ACTAGENT_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[ACTAGENT_WRAPPER_ENV_KEY] = value;
      }
      continue;
    }
    if (
...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (opts.wrapper !== undefined) {
    try {
      wrapperPath = await resolveACTAgentWrapperPath(opts.wrapper);
      if (!wrapperPath) {
  ...
  if (!wrapperPath) {
    try {
      wrapperPath = await resolveACTAgentWrapperPath(installEnv[ACTAGENT_WRAPPER_ENV_KEY]);
    } catch (err) {
      fail(`Invalid ${ACTAGENT_WRAPPER_ENV_KEY}: ${String(err)}`);
      return;
    }
  }
  if (loaded) {
    if (!opts.force) {
      ...
      if (autoRefreshMessage) {
        ...
        if (!json) {
          ...
          defaultRuntime.log(
            `Reinstall with: ${formatCliCommand("actagent gateway install --force")}`,
          );
        }
        return;
      }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.ACTAGENT_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service ACTAGENT_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[ACTAGENT_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await buildGatewayInstallPlan({
        env: params.installEnv,
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[ACTAGENT_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[ACTAGENT_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${ACTAGENT_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }

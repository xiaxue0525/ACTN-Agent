...
import type { StaleACTAgentUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleACTAgentUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleACTAgentUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ACTAGENT_STATE_DIR: "/tmp/actagent-daemon",
    ACTAGENT_CONFIG_PATH: "/tmp/actagent-daemon/actagent.json",
  },
}));
...
const resolveGatewayPort = vi.fn((_cfg?: unknown, _env?: unknown) => 19199);
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ACTAGENT_STATE_DIR ?? "/tmp/actagent-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ACTAGENT_CONFIG_PATH ?? `${stateDir}/actagent.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/actagent-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleACTAgentUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleACTAgentUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ACTAGENT_STATE_DIR = "/tmp/actagent-cli";
    process.env.ACTAGENT_CONFIG_PATH = "/tmp/actagent-cli/actagent.json";
    ...
    findStaleACTAgentUpdateLaunchdJobs.mockReset();
    findStaleACTAgentUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/actagent-daemon/actagent.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/actagent-cli/actagent.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ACTAGENT_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.ACTAGENT_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.ACTAGENT_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.ACTAGENT_STATE_DIR).toBe("/tmp/actagent-daemon");
    expect(handoffInput.ACTAGENT_CONFIG_PATH).toBe("/tmp/actagent-daemon/actagent.json");
    ...
    async () => {
      serviceReadCommand.mockResolvedValueOnce({
        ...
        environment: {
          ACTAGENT_STATE_DIR: "/tmp/actagent-daemon",
          ACTAGENT_CONFIG_PATH: "/tmp/actagent-daemon/actagent.json",
          ACTAGENT_LAUNCHD_LABEL: "ai.actagent.manual-update.gateway",
        },
      });
      findStaleACTAgentUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleACTAgentUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.ACTAGENT_STATE_DIR).toBe("/tmp/actagent-daemon");
      expect(staleScanEnv?.ACTAGENT_CONFIG_PATH).toBe("/tmp/actagent-daemon/actagent.json");
      expect(staleScanEnv?.ACTAGENT_LAUNCHD_LABEL).toBe("ai.actagent.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleACTAgentUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-actagent/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-status-config-"));
    const configPath = path.join(tmp, "actagent.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.ACTAGENT_STATE_DIR = tmp;
    process.env.ACTAGENT_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        ACTAGENT_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.ACTAGENT_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "actagent-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets ACTAGENT_STATE_DIR=/tmp/actagent-daemon,
    // distinct from the CLI process ACTAGENT_STATE_DIR=/tmp/actagent-cli. Drift

...
const resolveControlUiLinksMock = vi.hoisted(() =>
  vi.fn((_opts?: unknown) => ({ httpUrl: "http://127.0.0.1:19199" })),
);
...
vi.mock("./status.gather.js", () => ({
  ...
  resolvePortListeningAddresses: () => ["127.0.0.1:19199"],
}));
...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    printDaemonStatus(
      {
        ...
        logFile: "/tmp/actagent.log",
        gateway: {
          bindMode: "loopback",
          bindHost: "127.0.0.1",
          port: 19199,
          portSource: "env/config",
          probeUrl: "ws://127.0.0.1:19199",
        },
        port: {
          port: 19199,
          status: "busy",
          listeners: [{ pid: 9000, ppid: 8999, address: "127.0.0.1:19199" }],
          hints: [],
        },
        rpc: {
          ok: false,
    ...
    expectMockLineContains(runtime.error, formatCliCommand("actagent gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
    ...
    expectMockLineContains(runtime.log, "newer-actagent");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale ACTAgent updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.actagent.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.actagent.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "Gateway port 19199 is not listening");
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/actagent/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/actagent",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/actagent)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this ACTAgent command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `actagent` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/actagent-cli/actagent.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/actagent-daemon/actagent.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.actagent.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.actagent.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "actagent gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "- whatsapp: 2026.5.3 (actagenthub)");
    expectMockLineContains(runtime.log, "actagent plugins update <plugin-id>");
    expectMockLineContains(runtime.log, "actagent gateway restart");
  });

...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("actagent-daemon-install-int-");
    configPath = path.join(tempHome, "actagent.json");
    process.env.HOME = tempHome;
    process.env.ACTAGENT_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer ACTAgent", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.ACTAGENT_GATEWAY_TOKEN).toBeUndefined();
  });
});

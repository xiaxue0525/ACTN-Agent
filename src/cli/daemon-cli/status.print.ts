...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("actagent doctor")}" (or "${formatCliCommand("actagent doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("actagent gateway install --force")}\` from the same --profile / ACTAGENT_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this ACTAgent command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `actagent --version`, `which actagent`, and `actagent gateway status --deep`; if this mismatch is unexpected, update PATH so `actagent` points to the version you want, or reinstall the Gateway service from that same ACTAgent install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("actagent gateway restart")} and re-check with ${formatCliCommand("actagent gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale ACTAgent client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ACTAGENT_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("actagent gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale ACTAgent updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("actagent gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.ACTAGENT_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      for (const entry of drift.drifts) {
        const sourceLabel = entry.source === "actagenthub" ? "actagenthub" : "npm";
        defaultRuntime.log(
          `- ${warnText(entry.pluginId)}: ${entry.installedVersion} (${sourceLabel}) → expected ${drift.gatewayVersion}`,
        );
      }
      defaultRuntime.log(
        `${label("Fix:")} ${formatCliCommand("actagent plugins update <plugin-id>")} for each drifted plugin, then ${formatCliCommand("actagent gateway restart")}.`,
      );
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("actagent gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("actagent status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.actagent.ai/troubleshooting`);
}

...
const resolveGatewayPort = vi.hoisted(() => vi.fn((_cfg?: unknown, _env?: unknown) => 19199));
...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ACTAGENT_CONTAINER_HINT", "ACTAGENT_PROFILE"]);
    delete process.env.ACTAGENT_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["actagent", "gateway", "--port", "19199"],
      environment: {},
    });
    ...
    waitForGatewayHealthyListener.mockResolvedValue({
      healthy: true,
      portUsage: { port: 19199, status: "busy", listeners: [], hints: [] },
    });
  ...
  it("repairs stale loaded service definitions from gateway start", async () => {
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { ACTAGENT_SERVICE_VERSION: "2026.4.24" } } },
    ...
    expect(repairParams.state?.command?.environment).toEqual({
      ACTAGENT_SERVICE_VERSION: "2026.4.24",
    });
  ...
  it("fails restart when gateway remains unhealthy after the full timeout", async () => {
    ...
    const unhealthy: RestartHealthSnapshot = {
      ...
      staleGatewayPids: [],
      runtime: { status: "stopped" },
      portUsage: { port: 19199, status: "free", listeners: [], hints: [] },
      waitOutcome: "timeout",
      elapsedMs: 60_000,
    };
  ...
  it("waits longer for Windows gateway restart health", async () => {
    ...
    expect(waitParams.port).toBe(19199);
  });
  ...
  it("signals an unmanaged gateway process on stop", async () => {
    ...
    expect(findVerifiedGatewayListenerPidsOnPortSync).toHaveBeenCalledWith(19199);
  ...
  it("fails unmanaged restart when multiple gateway listeners are present", async () => {
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      "multiple gateway processes are listening on port 19199",
    );
  });
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (actagent#87577)", async () => {
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (actagent#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "actagent.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "actagent.service is a system-scope unit (/etc/systemd/system/actagent.service); run `sudo systemctl restart actagent.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      /sudo systemctl restart actagent\.service/,
    );
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (actagent#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "actagent-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (actagent#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "actagent-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "actagent-gateway.service is a system-scope unit (/etc/systemd/system/actagent-gateway.service); run `sudo systemctl stop actagent-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    runServiceStop.mockImplementation(async (params: { onNotLoaded?: () => Promise<unknown> }) => {

...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveACTAgentWrapperPath(installEnv[ACTAGENT_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `actagent dashboard` or copy a fresh auth URL with `actagent dashboard --no-open`.",

...
type DaemonConfigContext = {
  ...
  cliCfg: ACTAgentConfig;
  daemonCfg: ACTAgentConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): ACTAgentConfig | null {
...
function coerceStatusConfig(value: unknown): ACTAgentConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleACTAgentUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g actagent@<v>` updates the gateway binary
   * without a corresponding `actagent plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleACTAgentUpdateLaunchdJobs }) =>
            findStaleACTAgentUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];

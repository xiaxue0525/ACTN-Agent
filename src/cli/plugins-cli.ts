...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage ACTAgent plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.actagent.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    .command("search")
    .description("Search ACTAgentHub plugin packages")
  ...
  plugins
    .command("install")
    .description(
      "Install a plugin or hook pack (path, archive, npm spec, git repo, actagenthub:package, or marketplace entry)",
    )
    .argument(

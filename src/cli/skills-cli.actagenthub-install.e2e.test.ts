...
describe("actagent skills install ACTAgentHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-actagenthub-cli-e2e-"));
    try {
      const result = await spawnACTAgent(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          ACTAGENT_STATE_DIR: stateDir,
          ACTAGENT_CONFIG_PATH: path.join(stateDir, "actagent.json"),
          ACTAGENT_ACTAGENTHUB_URL: registry,
          ACTAGENT_ACTAGENTHUB_TOKEN: "test-token",
          ACTAGENT_ACTAGENTHUB_GITHUB_CODELOAD_BASE_URL: registry,
          ACTAGENTHUB_DISABLE_TELEMETRY: "",
          actagentdHUB_DISABLE_TELEMETRY: "",

...
describe("registerQrCli", () => {
  ...
  function expectLoggedLocalSetupCode() {
    expectLoggedSetupCode("ws://127.0.0.1:19199");
  }
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("ACTAGENT_GATEWAY_TOKEN", "");
    vi.stubEnv("ACTAGENT_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("actagent devices approve <requestId>");
  });
  ...
  it("allows private LAN IP cleartext setup urls", async () => {
    ...
    expectLoggedSetupCode("ws://192.168.1.8:19199");
  });
  ...
  it("allows android emulator cleartext override urls", async () => {
    ...
    await runQr(["--setup-code-only", "--url", "ws://10.0.2.2:19199"]);

    expectLoggedSetupCode("ws://10.0.2.2:19199");
  });
  ...
  it("uses ACTAGENT_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("ACTAGENT_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),

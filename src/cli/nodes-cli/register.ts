    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent nodes status", "List known nodes with live status."],
          ["actagent nodes pairing pending", "Show pending node pairing requests."],
          ["actagent nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'actagent nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["actagent nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.actagent.ai/cli/nodes")}\n`,
    );

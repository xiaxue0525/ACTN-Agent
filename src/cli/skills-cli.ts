// Skills CLI for workspace status, install/update, ACTAgentHub verification, and workshop proposals.
...
>;
type ResolvedACTAgentHubSkillVerificationTarget = Extract<
  Awaited<ReturnType<typeof resolveACTAgentHubSkillVerificationTarget>>,
  { ok: true }
>;
...
function resolveACTAgentHubTargetWorkspaceDir(
  command: Command | undefined,
...
function shouldFailSkillVerification(result: ACTAgentHubSkillVerificationResponse): boolean {
...
function buildSkillVerificationOutput(
  result: ACTAgentHubSkillVerificationResponse,
  target: ResolvedACTAgentHubSkillVerificationTarget,
...
function readVerifiedSkillCardUrl(
  ...
  if (!result.card || typeof result.card !== "object" || Array.isArray(result.card)) {
    return { ok: false, error: "ACTAgentHub verification response did not include a Skill Card URL." };
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.actagent.ai/cli/skills")}\n`,
    );

  skills
    .command("search")
    .description("Search ACTAgentHub skills")
    ...
    .action(async (queryParts: string[], opts: { limit?: number; json?: boolean }) => {
      try {
        const results = await searchSkillsFromACTAgentHub({
        ...
        if (results.length === 0) {
          defaultRuntime.log("No ACTAgentHub skills found.");
          return;
        }
  ...
  skills
    .command("install")
    .description("Install a skill from ACTAgentHub, git, or a local directory")
    .argument("<slug>", "ACTAgentHub skill slug, git:<repo>, or local skill directory")
    .option("--version <version>", "Install a specific version")
    .option("--force", "Overwrite an existing workspace skill", false)
    .option(
      "--force-install",
      "Install a pending GitHub-backed skill before ACTAgentHub scan completes",
      false,
    )
    ...
    .action(
      async (
        ...
        try {
          const workspaceDir = resolveACTAgentHubTargetWorkspaceDir(command, opts);
          if (!workspaceDir) {
            return;
          }
          if (isSkillSourceInstallSpec(slug)) {
            if (opts.version) {
              defaultRuntime.error("--version is only supported for ACTAgentHub skill installs.");
              defaultRuntime.exit(1);
              return;
            }
  ...
  skills
    .command("update")
    .description("Update ACTAgentHub-installed skills in the active or shared managed directory")
    .argument("[slug]", "Single skill slug")
    .option("--all", "Update all tracked ACTAgentHub skills", false)
    .option(
      "--force-install",
      "Install a pending GitHub-backed skill before ACTAgentHub scan completes",
      false,
    ...
    .action(
      async (
        ...
        try {
          ...
          const tracked = await readTrackedACTAgentHubSkillSlugs(workspaceDir);
          if (opts.all && tracked.length === 0) {
            defaultRuntime.log("No tracked ACTAgentHub skills to update.");
            return;
          }
  ...
  skills
    .command("verify")
    .description("Verify a ACTAgentHub skill with ACTAgentHub")
    .argument("<slug>", "ACTAgentHub skill slug")
    ...
    .action(
      async (
        ...
        try {
          ...
          const target = await resolveACTAgentHubSkillVerificationTarget({
            workspaceDir,
            slug,
            version: opts.version,
            tag: opts.tag,
          ...
          if (!target.ok) {
            ...
            const verification = await fetchACTAgentHubSkillVerification({
              slug: target.slug,
            ...
            if (opts.card) {
              ...
              if (!cardUrl.ok) {
                ...
                const card = await fetchACTAgentHubSkillCard({
                  url: cardUrl.url,
                  baseUrl: target.baseUrl,
                });

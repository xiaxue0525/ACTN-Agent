...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.actagent${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ACTAGENT_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.ACTAGENT_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.ACTAGENT_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.ACTAGENT_CONFIG_PATH)) {
    env.ACTAGENT_CONFIG_PATH = path.join(stateDir, "actagent.json");
  }

  if (profile === "dev" && !env.ACTAGENT_GATEWAY_PORT?.trim()) {
    env.ACTAGENT_GATEWAY_PORT = "19001";
  }
}

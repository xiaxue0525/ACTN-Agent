// `actagent plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `actagent plugins inspect`. */
...
function formatInstallLines(install: PluginInstallRecord | undefined): string[] {
  ...
  if (install.actagenthubPackage) {
    lines.push(`ACTAgentHub package: ${install.actagenthubPackage}`);
  }
  if (install.actagenthubChannel) {
    lines.push(`ACTAgentHub channel: ${install.actagenthubChannel}`);
  }
  ...
  if (install.actagentpackSha256) {
    lines.push(`actagentpack sha256: ${install.actagentpackSha256}`);
  }
  if (install.actagentpackSpecVersion !== undefined) {
    lines.push(`actagentpack spec: ${install.actagentpackSpecVersion}`);
  }
  if (install.actagentpackManifestSha256) {
    lines.push(`actagentpack manifest sha256: ${install.actagentpackManifestSha256}`);
  }
  if (install.actagentpackSize !== undefined) {
    lines.push(`actagentpack size: ${install.actagentpackSize} bytes`);
  }
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "actagent plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "actagent"}`);

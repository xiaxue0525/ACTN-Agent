...
const mocks = vi.hoisted(() => ({
  ...
  relinkACTAgentPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkACTAgentPeerDependenciesInManagedNpmRoot:
    mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with ACTAGENT_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as ACTAgentConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { ACTAGENT_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        ACTAGENT_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { ACTAGENT_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as ACTAgentConfig,
      env: {},
    });
  ...
  it("repairs managed npm actagent peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/actagent-state/npm",
      "/tmp/actagent-state/npm/projects/codex",
    ]);
    mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as ACTAgentConfig,
      env: { ACTAGENT_STATE_DIR: "/tmp/actagent-state" },
    });

    expect(mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/actagent-state/npm",
      logger: {},
    });
    expect(mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/actagent-state/npm/projects/codex",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired ACTAgent host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkACTAgentPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @actagent/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @actagent/discord: ENETUNREACH.',
        guidance: ["Run `actagent doctor --fix` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from actagenthub:@actagent/matrix@beta: ACTAgentHub actagentpack download for @actagent/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `actagent doctor --fix` to retry plugin repair.",
          "Run `actagent plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `actagent doctor --fix`."],
        },
        {

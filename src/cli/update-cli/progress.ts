...
const STEP_LABELS: Record<string, string> = {
  ...
  "actagent doctor entry": "Checking doctor entrypoint",
  "actagent doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g actagent@latest");
    hints.push(
      "System install outline: actagent gateway stop -> sudo <system-npm> i -g actagent@latest -> actagent gateway install --force -> actagent gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g actagent@latest --omit=optional");
  }

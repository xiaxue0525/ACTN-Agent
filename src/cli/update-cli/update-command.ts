...
import { disableCurrentACTAgentUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "ACTAGENT_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "ACTAGENT_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "ACTAGENT_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "ACTAGENT_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "ACTAGENT_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_SOURCE_CONFIG_PATH_ENV = "ACTAGENT_UPDATE_POST_CORE_SOURCE_CONFIG_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "ACTAGENT_UPDATE_POST_CORE_STARTED_AT_MS";
...
const POST_UPDATE_PLUGIN_REPAIR_GUIDANCE = "Run actagent doctor --fix to attempt automatic repair.";
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as ACTAgentConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run actagent plugins inspect ${pluginId} --runtime --json for details.`;
}
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("actagent gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("actagent gateway install --force"),
    CLI_NAME,
  );
  const statusCommand = replaceCliName(
    formatCliCommand("actagent gateway status --deep"),
    CLI_NAME,
  );
  if (platform === "darwin") {
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall ACTAgent ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("actagent gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `actagent update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("actagent update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
function isRunningInsideGatewayService(
  ...
  if (env.ACTAGENT_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.ACTAGENT_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for actagent@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `actagent update`."
      : "Upgrade Node to 22.19+ or Node 24, then rerun `actagent update`.",
    "Bare `npm i -g actagent` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g actagent@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.ACTAGENT_SERVICE_MARKER;
  delete resolvedEnv.ACTAGENT_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("actagent completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        return await runUpdateStep({
          ...
          env: {
            ...
            ...(candidateHostVersion === null
              ? {}
              : { ACTAGENT_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
          progress: params.progress,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as ACTAgentConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("actagent gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        await createUpdateConfigSnapshot();
        process.env.ACTAGENT_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.ACTAGENT_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("actagent gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("actagent doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("actagent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("actagent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.ACTAGENT_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.ACTAGENT_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
  ...
  const preFinalizeConfig = configSnapshot.valid
    ? {
        ...
        authoredConfig: isRecord(configSnapshot.parsed)
          ? (configSnapshot.parsed as ACTAgentConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as ACTAgentConfig,
      authoredConfig: authoredConfig as ACTAgentConfig,
    };
  }
  const authored = parsed as ACTAgentConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as ACTAgentConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-update-post-core-"));
    ...
    const child = spawn(params.nodeRunner ?? resolveNodeRunner(), argv, {
      ...
      env: {
        ...
        ...(postCoreHostVersion === null
          ? {}
          : { ACTAGENT_COMPATIBILITY_HOST_VERSION: postCoreHostVersion }),
...
async function updateCommandInternal(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun !== true) {
    await disableCurrentACTAgentUpdateLaunchdJob().catch(() => undefined);
    assertConfigWriteAllowedInCurrentMode();
  }
  ...
  if (postCoreUpdateResume) {
    ...
    process.env.ACTAGENT_COMPATIBILITY_HOST_VERSION = (await readPackageVersion(root)) ?? VERSION;
  ...
  const devTargetRef =
    channel === "dev" ? process.env.ACTAGENT_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (updateInstallKind === "package") {
    ...
    if (managedServiceRootRedirect) {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.warn(
            `Shell ACTAgent root differs from the managed gateway service root: ${managedServiceRootRedirect.previousRoot}`,
          ),
        );
        defaultRuntime.log(
          theme.muted(
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating ACTAgent..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `actagent update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this ACTAgent install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("actagent doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("actagent gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g actagent@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g actagent@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (!pluginsUpdatedInFreshProcess) {
    ...
    const previousCompatibilityHostVersion = process.env.ACTAGENT_COMPATIBILITY_HOST_VERSION;
    if (compatibilityDowngradeTarget) {
      process.env.ACTAGENT_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          ...
          process.env.ACTAGENT_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  }

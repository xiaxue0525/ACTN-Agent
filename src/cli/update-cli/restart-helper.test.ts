...
describe("restart-helper", () => {
  ...
  async function prepareAndReadScript(env: Record<string, string>, gatewayPort = 19199) {
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 19199) {
    const stateCheck = "$taskState = Get-ACTAgentScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-ACTAgentSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "actagent restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-ACTAgentListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-ACTAgentSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'actagent-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `actagent-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("actagent-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^actagent-restart-/);
    ...
    it("uses ACTAGENT_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ACTAGENT_PROFILE: "default",
        ACTAGENT_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("actagent-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        ACTAGENT_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        ACTAGENT_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped actagent gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart actagent-gateway.service");
      expect(calls).toContain("--user is-active --quiet actagent-gateway.service");
      expect(calls).toContain("is-active --quiet actagent-gateway.service");
      expect(calls).not.toContain("--user restart actagent-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.actagent.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.actagent.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.actagent/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.actagent/logs/gateway-restart.log' 2>&1");
    ...
    it("uses ACTAGENT_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/actagent-state/logs' 2>/dev/null && : >>'/tmp/actagent-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/actagent-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("actagent restart attempt source=update target=ai.actagent.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.actagent.gateway");
      expect(log).toContain("actagent restart failed source=update status=42");
      expect(log).not.toContain("actagent restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        ACTAGENT_LAUNCHD_LABEL: "ai.actagent.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        ACTAGENT_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.actagent.$(echo injected)");
      expect(log).not.toContain("target=ai.actagent.injected");
    });

    it("uses ACTAGENT_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ACTAGENT_PROFILE: "default",
        ACTAGENT_LAUNCHD_LABEL: "com.custom.actagent",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.actagent'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'ACTAgent Gateway'");
      expect(content).toContain("function Invoke-ACTAgentSchtasksWithTimeout");
      expect(content).toContain("function Get-ACTAgentScheduledTaskState");
      expect(content).toContain("function Invoke-ACTAgentStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("actagent restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".actagent\\gateway.cmd"',
      );
      expect(content).toContain("actagent restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%ACTAGENT_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses ACTAGENT_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        ACTAGENT_PROFILE: "default",
        ACTAGENT_WINDOWS_TASK_NAME: "ACTAgent Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'ACTAgent Gateway (custom)'");
      expect(content).toContain("Get-ACTAgentScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-ACTAgentSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-ACTAgentStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ACTAGENT_PROFILE: "production",
      });
      expect(content).toContain("actagent-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ACTAGENT_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.actagent.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'ACTAgent Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ACTAGENT_LAUNCHD_LABEL: "ai.actagent.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.actagent.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });

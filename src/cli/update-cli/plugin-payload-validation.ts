...
import { auditACTAgentPeerDependencyLink } from "../../plugins/plugin-peer-link.js";
...
const TRACKED_SOURCES: ReadonlySet<string> = new Set(["npm", "actagenthub", "git", "marketplace"]);
...
export async function runPluginPayloadSmokeCheck(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records).toSorted(([a], [b]) =>
    ...
    if (manifestDeclaresACTAgentPeer(manifest)) {
      const peerIssue = await auditACTAgentPeerDependencyLink({
      ...
      if (peerIssue) {
        failures.push({
          ...
          detail: `Plugin declares peerDependency "actagent" but peer link audit failed: ${peerIssue.reason}.`,
        });
      }
    }
      ...
      failures.push({
        ...
        detail: `Plugin extension entry validation failed: ${
          extensionResolution.status === "invalid"
            ...
            : "package.json actagent.extensions is empty"
        }`,
      });
      continue;
...
function manifestDeclaresACTAgentPeer(manifest: PackageManifest): boolean {
  const peerDependencies = (manifest as { peerDependencies?: unknown }).peerDependencies;
  return (
    typeof peerDependencies === "object" &&
    peerDependencies !== null &&
    !Array.isArray(peerDependencies) &&
    typeof (peerDependencies as Record<string, unknown>).actagent === "string"
  );
}

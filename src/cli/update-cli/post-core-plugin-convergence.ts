...
import { relinkACTAgentPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `actagent doctor --fix` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`actagent plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmACTAgentPeerLinks(params: {
  ...
  try {
    ...
    const message = `Failed to repair managed npm ACTAgent host peer links: ${err instanceof Error ? err.message : String(err)}`;
    return {
      changes: [],
      warnings: [
        {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmACTAgentPeerLinks({ env });
 ...
 * resolve as a trusted-source-linked official install (npm or ACTAgentHub)
...
export function filterRecordsToActive(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records)) {
    ...
    const officialACTAgentHub = resolveTrustedSourceLinkedOfficialACTAgentHubSpec({ pluginId, record });
    if (officialNpm || officialACTAgentHub) {
      filtered[pluginId] = record;
    }
  }
  return filtered;

// Formatting layer for `actagent skills` commands; keeps discovery data separate from terminal UI.
...
function appendACTAgentHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`actagent skills search\`, \`actagent skills install\`, and \`actagent skills update\` for ACTAgentHub-backed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("actagent skills list")}\` to see all skills.`
      : "No skills found.";
    return appendACTAgentHubHint(message, opts.json);
  }
  ...
  return appendACTAgentHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendACTAgentHubHint(
      `Skill "${safeRequestedName}" not found. Run \`${formatCliCommand("actagent skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`actagent config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$ACTAGENT_CONFIG_PATH")} ${theme.muted("(default: ~/.actagent/actagent.json)")}`,
    );
  }

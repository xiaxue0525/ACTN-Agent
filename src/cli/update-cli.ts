// Commander wiring for `actagent update`, its status/finalize subcommands, and help text.
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update ACTAgent and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["actagent update", "Update a source checkout (git)"],
        ["actagent update --channel beta", "Switch to beta channel (git + npm)"],
        ["actagent update --channel dev", "Switch to dev channel (git + npm)"],
        ["actagent update --tag beta", "One-off update to a dist-tag or version"],
        ["actagent update --tag main", "One-off package update from GitHub main"],
        ["actagent update --dry-run", "Preview actions without changing anything"],
        ["actagent update --no-restart", "Update without restarting the service"],
        ["actagent update --json", "Output result as JSON"],
        ["actagent update --yes", "Non-interactive (accept downgrade prompts)"],
        ["actagent update wizard", "Interactive update wizard"],
        ["actagent --update", "Shorthand for actagent update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run actagent update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.actagent.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .description("Run ACTAgent update finalization after an external core runtime change")
    .option("--json", "Output result as JSON", false)
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent update status", "Show channel + version status."],
          ["actagent update status --json", "JSON output."],
          ["actagent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.actagent.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

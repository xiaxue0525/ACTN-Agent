...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent channels list", "List configured channels."],
          ["actagent channels list --all", "Show configured, bundled, and installable channels."],
          ["actagent channels add", "Open guided channel setup."],
          ["actagent channels status --probe", "Run channel status checks and probes."],
          [
            "actagent channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["actagent channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.actagent.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent channels add", "Open guided setup for available chat channels."],
          [
            "actagent channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["actagent channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)

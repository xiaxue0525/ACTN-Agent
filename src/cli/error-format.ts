...
const DEFAULT_GATEWAY_PORT_EXAMPLE = 19199;
...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "actagent channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `actagent channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON, for example ${formatInlineCliCommand(
      "actagent config set gateway.port 19199 --strict-json",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?actagent doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "actagent gateway status --deep";
...
/** Format a plugin lookup miss with optional ACTAgentHub search guidance. */
export function formatMissingPluginMessage(params: {
  id: string;
  listCommand?: string;
  includeSearch?: boolean;
}): string {
  const listCommand = params.listCommand ?? "actagent plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("actagent plugins search " + params.id)} to look for installable plugins`
    : "";

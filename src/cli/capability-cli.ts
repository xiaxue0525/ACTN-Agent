...
import { detectMime, extensionForMime, normalizeMimeType } from "@actagent/media-core/mime";
import {
...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside ACTAgent.";
...
function getAuthProfileIdsForProvider(cfg: ACTAgentConfig, providerId: string): string[] {
...
function pinRuntimeConfigSnapshot(config: ACTAgentConfig): void {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.actagent.ai/cli/infer")}\n`,
    );

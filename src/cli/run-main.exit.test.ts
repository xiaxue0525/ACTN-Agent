...
vi.mock("../infra/path-env.js", () => ({
  ensureACTAgentCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.ACTAGENT_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.ACTAGENT_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    await runCli(["node", "actagent", "status"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "actagent", "status"]);
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "actagent", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "actagent", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "actagent", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "actagent", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading ACTAgent CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "actagent", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "actagent", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "actagent", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "actagent", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "actagent", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.ACTAGENT_HIDE_BANNER = "1";

    await runCli(["node", "actagent", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "actagent", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "actagent", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "actagent", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "actagent", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "actagent", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "actagent", "doctor", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "actagent", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "actagent", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "actagent", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "actagent", "gateway", "run"]],
    ["bare gateway runtime", ["node", "actagent", "gateway"]],
    ["node runtime", ["node", "actagent", "node", "run"]],
    ["local agent runtime", ["node", "actagent", "agent", "--local"]],
    ["provider inference", ["node", "actagent", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "actagent", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "actagent", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "actagent", "skills", "search", "browser"]],
    ["update command", ["node", "actagent", "update", "check"]],
    ["channel probe", ["node", "actagent", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "actagent", "channels", "capabilities"]],
    ["directory plugin command", ["node", "actagent", "directory", "peers", "list"]],
    ["message plugin command", ["node", "actagent", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "actagent", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "actagent", "--help"]],
    ["root version", ["node", "actagent", "--version"]],
    ["gateway help", ["node", "actagent", "gateway", "--help"]],
    ["gateway run help", ["node", "actagent", "gateway", "run", "--help"]],
    ["status", ["node", "actagent", "status"]],
    ["health", ["node", "actagent", "health"]],
    ["gateway status", ["node", "actagent", "gateway", "status"]],
    ["gateway health", ["node", "actagent", "gateway", "health"]],
    ["remote agent control-plane", ["node", "actagent", "agent", "run"]],
    ["chat control-plane", ["node", "actagent", "chat"]],
    ["terminal control-plane", ["node", "actagent", "terminal"]],
    ["config", ["node", "actagent", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "actagent", "channels"]],
    ["completion", ["node", "actagent", "completion", "zsh"]],
    ["debug proxy cli", ["node", "actagent", "proxy", "start"]],
    ["agents list", ["node", "actagent", "agents", "list"]],
    ["models list", ["node", "actagent", "models", "list"]],
    ["models status without live probe", ["node", "actagent", "models", "status"]],
    ["skills check", ["node", "actagent", "skills", "check"]],
    ["skills info", ["node", "actagent", "skills", "info", "weather"]],
    ["skills list", ["node", "actagent", "skills", "list"]],
    ["tasks list", ["node", "actagent", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "actagent", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "actagent", "tools", "effective"]],
    ["migrate", ["node", "actagent", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "actagent", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "actagent", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "actagent", "foo"])).rejects.toThrow(
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "actagent", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "actagent", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "actagent", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "actagent", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "actagent", "lcm_recent"])).rejects.toThrow(
      '"lcm_recent" is an agent tool available from the "lossless-actagent" plugin',
    );
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "actagent", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "actagent", "auth", "--help"]],
    ["tool", ["node", "actagent", "tool", "image_generate"]],
    ["tools", ["node", "actagent", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "actagent", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "actagent", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "actagent", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "actagent", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "actagent", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://actagent.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `actagent onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "actagent", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "actagent", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "actagent", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "actagent", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "actagent", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "actagent", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[actagent] ACTAgent hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[actagent] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[actagent] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);

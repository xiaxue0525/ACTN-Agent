...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(19199)).toStrictEqual([]);
  });

  it("skips lsof when the port is already bindable", async () => {
    tryListenOnPortMock.mockResolvedValue(undefined);

    const result = await forceFreePortAndWait(19199, { timeoutMs: 500, intervalMs: 100 });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(19199)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(19199);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(19199, {
      timeoutMs: 500,
      intervalMs: 100,
  ...
  it("falls back to fuser when lsof is permission denied", async () => {
    ...
    expect(termCall?.[1]).toEqual(["-k", "-TERM", "19199/tcp"]);
  ...
  it("uses fuser SIGKILL escalation when port stays busy", async () => {
    ...
    expect(killCall?.[1]).toEqual(["-k", "-KILL", "19199/tcp"]);
  ...
  it("throws when lsof is unavailable and fuser is missing", async () => {
    ...
    await expect(forceFreePortAndWait(19199, { timeoutMs: 200, intervalMs: 100 })).rejects.toThrow(
      /fuser not found/i,
    );
  });
});
...
describe("gateway --force helpers (Windows netstat path)", () => {
  ...
  it("parses PIDs from netstat output correctly", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(19199, 42, 99));
    expect(listPortListeners(19199)).toEqual<PortProcess[]>([{ pid: 42 }, { pid: 99 }]);
  });
  ...
  it("deduplicates PIDs that appear multiple times", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(19199, 42, 42));
    expect(listPortListeners(19199)).toEqual<PortProcess[]>([{ pid: 42 }]);
  });
  ...
  it("throws a descriptive error when netstat fails", () => {
    ...
    expect(() => listPortListeners(19199)).toThrow(/netstat failed/);
  });

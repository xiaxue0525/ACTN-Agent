// Runtime implementations for `actagent plugins` subcommands. Heavy plugin modules stay
...
function formatConfiguredRuntimePluginInstallSpec(params: {
  ...
  const actagenthubSpec = params.actagenthubSpec?.trim();
  const npmSpec = params.npmSpec?.trim();
  if (actagenthubSpec && params.defaultChoice !== "npm") {
    return actagenthubSpec;
  }
  return npmSpec ?? actagenthubSpec ?? params.pluginId;
}
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "actagent"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "actagent doctor --fix" to install ${installSpec}, or install it manually with "actagent plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(idInput: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as ACTAgentConfig;
  ...
  let next: ACTAgentConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("actagent plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    actagent plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    actagent plugins registry --refresh");
      lines.push("    actagent gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.actagent.ai/plugin");
  lines.push("");

...
const mocks = vi.hoisted(() => {
  ...
  let configState: ACTAgentConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: ACTAgentConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: ACTAgentConfig }>
    >(async () => ({
      path: "/tmp/actagent.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: ACTAgentConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/actagent.json");
    ...
    expect(output).not.toContain("/tmp/actagent.json\nforged");

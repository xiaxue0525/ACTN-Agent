...
describe("registerOnboardCommand", () => {
  ...
  it("parses numeric gateway port and drops invalid values", async () => {
    await runCli(["onboard", "--gateway-port", "19199"]);
    expect(setupWizardOptions(0).gatewayPort).toBe(19199);

    await runCli(["onboard", "--gateway-port", "nope"]);
    expect(setupWizardOptions(1).gatewayPort).toBeUndefined();

    await runCli(["onboard", "--gateway-port", "19199x"]);
  ...
  it("forwards --gateway-token-ref-env", async () => {
    await runCli(["onboard", "--gateway-token-ref-env", "ACTAGENT_GATEWAY_TOKEN"]);
    expect(setupWizardOptions().gatewayTokenRefEnv).toBe("ACTAGENT_GATEWAY_TOKEN");
  });

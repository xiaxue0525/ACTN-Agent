...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.ACTAGENT_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.ACTAGENT_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.ACTAGENT_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.ACTAGENT_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.ACTAGENT_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.ACTAGENT_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.ACTAGENT_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "actagent", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "actagent"]);

    const names = program.commands.map((cmd) => cmd.name());
    expect(names).toContain("acp");
    expect(names).toContain("gateway");
    expect(names).toContain("actagentbot");
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "actagent", "nodes", "list"], "actagent");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "actagent", "infer"], "actagent");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "actagent", "acp", "--help"], "actagent");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "actagent", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("actagent");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "actagent", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("actagent");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "actagent", "completion", "--write-state"];
    const program = new Command().name("actagent");
  ...
  it.each([
    ["plugins update", ["plugins", "update", "lossless-actagent"]],
    ["plugins update --all", ["plugins", "update", "--all"]],
    ["plugins install", ["plugins", "install", "lossless-actagent"]],
    ["plugins list", ["plugins", "list"]],
    ["plugins inspect", ["plugins", "inspect", "lossless-actagent"]],
    ...
    process.argv = ["node", "actagent", ...args];
    const program = new Command().name("actagent");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "actagent", "plugins"];
    const program = new Command().name("actagent");

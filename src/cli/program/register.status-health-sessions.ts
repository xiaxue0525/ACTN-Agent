...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent status", "Show channel health + session summary."],
          ["actagent status --all", "Full diagnosis (read-only)."],
          ["actagent status --json", "Machine-readable output."],
          ["actagent status --usage", "Show model provider usage/quota snapshots."],
          [
            "actagent status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["actagent status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.actagent.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.actagent.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent sessions", "List all sessions."],
          ["actagent sessions --agent work", "List sessions for one agent."],
          ["actagent sessions --all-agents", "Aggregate sessions across agents."],
          ["actagent sessions --active 120", "Only last 2 hours."],
          ["actagent sessions --limit 25", "Show the newest 25 sessions."],
          ["actagent sessions --json", "Machine-readable output."],
          ["actagent sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.actagent.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "actagent sessions cleanup --dry-run --fix-missing",
          ...
          ["actagent sessions cleanup --enforce", "Apply maintenance now."],
          ["actagent sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["actagent sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "actagent sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .actagent/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["actagent commitments", "List pending inferred follow-ups."],
          ["actagent commitments --all", "List all inferred follow-ups."],
          ["actagent commitments --agent work", "List one agent's inferred follow-ups."],
          ["actagent commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {

...
describe("ensureConfigReady", () => {
  ...
  const originalACTAgentStateDir = process.env.ACTAGENT_STATE_DIR;
  ...
  function useTempACTAgentHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-config-guard-"));
    tempRoots.push(root);
    process.env.ACTAGENT_HOME = root;
    delete process.env.ACTAGENT_STATE_DIR;
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".actagent", "tasks", "runs.sqlite");
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "");
  }

  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".actagent", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }
  ...
  afterEach(() => {
    ...
    if (originalACTAgentStateDir === undefined) {
      delete process.env.ACTAGENT_STATE_DIR;
    } else {
      process.env.ACTAGENT_STATE_DIR = originalACTAgentStateDir;
    }
    for (const root of tempRoots.splice(0)) {
      fs.rmSync(root, { recursive: true, force: true });
    }
  ...
  it("runs doctor flow when lightweight startup detection finds legacy state", async () => {
    const root = useTempACTAgentHome();
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempACTAgentHome();
    fs.mkdirSync(path.join(root, ".actagent", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for ACTAGENT_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-config-guard-home-"));
    tempRoots.push(root);
    process.env.HOME = root;
    process.env.ACTAGENT_HOME = "~/svc";
    delete process.env.ACTAGENT_STATE_DIR;
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "ACTAgent config is invalid",
      "File: /tmp/actagent.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("actagent doctor --fix")}`,
      `Inspect: ${formatCliCommand("actagent config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("actagent doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempACTAgentHome());

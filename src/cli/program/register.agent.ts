...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.actagent.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['actagent agents set-identity --agent main --name "ACTAgent" --emoji "🐲"', "Set name + emoji."],
  ["actagent agents set-identity --agent main --avatar avatars/actagent.png", "Set avatar path."],
  [
    "actagent agents set-identity --workspace ~/.actagent/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "actagent agents set-identity --identity-file ~/.actagent/workspace/IDENTITY.md --agent main",

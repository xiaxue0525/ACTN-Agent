...
describe("command-path-policy", () => {
  ...
  it("applies exact overrides after broader channel plugin rules", () => {
    ...
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "actagent", "channels", "status"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("bypass");
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "actagent", "channels", "status", "--probe"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("default");
  ...
  it("keeps config-only agent commands on config-only startup", () => {
    ...
    expect(
      agentPolicy.loadPlugins({
        argv: ["node", "actagent", "agent", "--local"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(true);
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "googlemeet", "login"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "tool", "image_generate"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "tools", "effective"])).toBe("bypass");
  });

  it("resolves static network proxy bypass policies from the catalog", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "actagent", "config", "get", "proxy.enabled"]),
    ).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "proxy", "start"])).toBe("bypass");
  });

  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "gateway"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "gateway", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "gateway", "health"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "node", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "node", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "agent", "--local"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "agent", "run"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "channels", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "actagent", "channels", "status", "--probe"]),
    ).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "models", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "models", "status", "--probe"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "skills", "info", "browser"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "skills", "list"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "skills", "search", "browser"])).toBe(
      "default",
    );
  });
  ...
  it("uses the longest catalog command path for deep network proxy overrides", async () => {
    ...
    expect(
      resolveCliCatalogCommandPathLocal(["node", "actagent", "nodes", "camera", "snap"]),
    ).toEqual(["nodes", "camera", "snap"]);
    expect(resolveCliNetworkProxyPolicyLocal(["node", "actagent", "nodes", "camera", "snap"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicyLocal(["node", "actagent", "nodes", "camera", "list"])).toBe(
      "bypass",
    );
  });
  ...
  it("stops catalog command path resolution before positional arguments", () => {
    expect(
      resolveCliCatalogCommandPath(["node", "actagent", "config", "get", "proxy.enabled"]),
    ).toEqual(["config", "get"]);
    expect(
      resolveCliCatalogCommandPath(["node", "actagent", "message", "send", "--to", "demo"]),
    ).toEqual(["message"]);
  });

  it("treats bare gateway invocations with options as the gateway runtime", () => {
    const argv = ["node", "actagent", "gateway", "--port", "1234"];
  ...
  it("does not let gateway run option values spoof bypass subcommands", () => {
    for (const argv of [
      ["node", "actagent", "gateway", "--token", "status"],
      ["node", "actagent", "gateway", "--token=status"],
      ["node", "actagent", "gateway", "--password", "health"],
      ["node", "actagent", "gateway", "--password-file", "status"],
      ["node", "actagent", "gateway", "--ws-log", "compact"],
    ]) {
  ...
  it("still resolves real gateway bypass subcommands after their command token", () => {
    expect(resolveCliCatalogCommandPath(["node", "actagent", "gateway", "status"])).toEqual([
      "gateway",
      "status",
    ]);
    expect(
      resolveCliCatalogCommandPath(["node", "actagent", "gateway", "status", "--token", "secret"]),
    ).toEqual(["gateway", "status"]);
    expect(resolveCliNetworkProxyPolicy(["node", "actagent", "gateway", "status"])).toBe("bypass");
  });
});

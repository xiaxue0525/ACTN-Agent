...
describe("pickTagline", () => {
  ...
  it("keeps ACTAGENT_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { ACTAGENT_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
      "Your terminal just grew actagents\u2014type something and let the bot pinch the busywork.",
    );
    expect(value).not.toBe(DEFAULT_TAGLINE);
  });

  it("ignores partial ACTAGENT_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { ACTAGENT_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
    ).toBe("Your terminal just grew actagents\u2014type something and let the bot pinch the busywork.");
  });
});

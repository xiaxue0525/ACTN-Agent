...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "ACTAGENT_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# ACTAgent Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# ACTAgent Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the ACTAgent completion block. */
...
/** Returns whether a shell profile already contains an ACTAgent completion block or source line. */
 ...
 * Returns true if profile has `source <(actagent completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "actagent") {

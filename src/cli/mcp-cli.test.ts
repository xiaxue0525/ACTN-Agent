...
const serveACTAgentChannelMcp = mocks.serveACTAgentChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveACTAgentChannelMcp: mocks.serveACTAgentChannelMcp,
}));
...
async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("actagent-cli-mcp-home-", async (home) => {
  ...
  it("adds a configured MCP server from flags without replacing operator knobs", async () => {
    await withTempHome("actagent-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
  ...
  it("fails named probes for disabled MCP servers", async () => {
    await withTempHome("actagent-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `MCP server "docs" is disabled in ${configPath}. Run actagent mcp configure docs --enable before probing it.`,
      );
    });
  });
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("actagent-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run actagent mcp list to see configured servers.`,
      );
    });
  });
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("actagent-cli-mcp-home-", async () => {
      ...
      expect(serveACTAgentChannelMcp).toHaveBeenCalledWith({

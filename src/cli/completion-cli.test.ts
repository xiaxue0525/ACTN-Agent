...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_actagent_gateway()");
    expect(script).toContain("(status) _actagent_gateway_status ;;");
    expect(script).toContain("(restart) _actagent_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "actagent.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[actagent]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_actagent_register_completion]}" = "_actagent_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _actagent_register_completion
            [[ -z "\${precmd_functions[(r)_actagent_register_completion]}" ]] || exit 12
            [[ "\${_comps[actagent]-}" = "_actagent_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'actagent gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("actagent");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("actagent").option("--json", "JSON output");
    const empty = new Command().name("actagent");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c actagent -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c actagent -n "__actagent_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c actagent -n \"__actagent_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c actagent -n \"__actagent_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__actagent_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__actagent_command_path_matches agent -- --verbose");
    expect(script).toContain("__actagent_command_path_matches sessions cleanup --");
    expect(script).not.toContain("__actagent_command_path_matches sessions cleanup -- --verbose");
    expect(script).toContain(
      "complete -c actagent -n \"__actagent_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });

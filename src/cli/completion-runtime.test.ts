...
describe("completion-runtime", () => {
  ...
  it("formats PowerShell source and reload commands with single-quoted paths", () => {
    expect(
      formatCompletionSourceLine("powershell", "actagent", "C:\\Users\\Ada\\open'actagent.ps1"),
    ).toBe(". 'C:\\Users\\Ada\\open''actagent.ps1'");
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-completion-state-"));

    process.env.HOME = homeDir;
    process.env.ACTAGENT_STATE_DIR = stateDir;

    try {
      const cachePath = resolveCompletionCachePath("powershell", "actagent");
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

      await installCompletion("powershell", true, "actagent");

      const profilePath = resolveCompletionProfilePath("powershell");
      const profile = await fs.readFile(profilePath, "utf-8");
      expect(profile).toBe(`# ACTAgent Completion\n. '${cachePath}'\n`);
    } finally {
  ...
  it("rejects install when the completion cache is missing", async () => {
    ...
    try {
      await expect(installCompletion("zsh", true, "actagent")).rejects.toThrow(

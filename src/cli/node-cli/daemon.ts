...
function renderNodeServiceStartHints(): string[] {
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("actagent node install"),
    startCommand: formatCliCommand("actagent node start"),
...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 19199;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("actagent node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ACTAGENT_LOG_PREFIX: baseEnv.ACTAGENT_LOG_PREFIX ?? "node",

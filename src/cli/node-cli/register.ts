    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "actagent node run --host 127.0.0.1 --port 19199",
            "Run the node host in the foreground.",
          ],
          ["actagent node status", "Check node host service status."],
          ["actagent node install", "Install the node host service."],
          ["actagent node start", "Start the installed node host service."],
          ["actagent node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.actagent.ai/cli/node")}\n`,
    );
  ...
  node
    ...
    .action(async (opts) => {
      ...
      const host =
        ...
        "127.0.0.1";
      const port = parsePortOption(opts.port, existing?.gateway?.port ?? 19199);
      if (port === null) {

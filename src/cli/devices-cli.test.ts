...
describe("devices cli approve", () => {
  ...
  it("prints selected details and exits when implicit approval is used", async () => {
    ...
    expect(readRuntimeErrorOutput()).toContain("actagent devices approve req-abc");
  ...
  it.each([
    ...
    expectGatewayCall(0, { method: "device.pair.list" });
    expect(hasGatewayMethod("device.pair.approve")).toBe(false);
    expect(readRuntimeErrorOutput()).toContain(`actagent devices approve ${expectedRequestId}`);
  });
  ...
  it("falls back to device id when selected pending display name is blank", async () => {
    ...
    expect(readRuntimeErrorOutput()).toContain("actagent devices approve req-blank");
    expect(hasGatewayMethod("device.pair.approve")).toBe(false);
  });
  ...
  it("includes explicit gateway flags in the rerun approval command", async () => {
    ...
    await runDevicesApprove([
      "--latest",
      "--url",
      "ws://gateway.example:19199/actagent?cluster=qa lab",
      "--timeout",
      "3000",
      "--token",
      "secret-token",
    ...
    expect(errorOutput).toContain(
      "actagent devices approve req-url --url 'ws://gateway.example:19199/actagent?cluster=qa lab' --timeout 3000",
    );
  ...
  it("returns JSON for implicit approval preview in JSON mode", async () => {
    ...
    await runDevicesApprove(["--latest", "--json", "--url", "ws://gateway.example:19199"]);
    ...
    expect(runtime.writeJson).toHaveBeenCalledWith({
      ...
      approveCommand: "actagent devices approve req-json --url ws://gateway.example:19199 --json",
      requiresAuthFlags: {
        token: false,
        password: false,
      },
...
describe("devices cli local fallback", () => {
  ...
  it("refuses local fallback when the gateway request is absent from local pairing state", async () => {
    ...
    await expect(runDevicesCommand(["list"])).rejects.toThrow(
      "different ACTAGENT_PROFILE or ACTAGENT_STATE_DIR",
    );
  ...
  it("does not use local fallback when an explicit --url is provided", async () => {
    ...
    await expect(
      runDevicesCommand(["list", "--json", "--url", "ws://127.0.0.1:19199"]),

...
export function buildToolPluginPackageManifest(params: {
  ...
  const actagent =
    params.packageManifest.actagent &&
    typeof params.packageManifest.actagent === "object" &&
    !Array.isArray(params.packageManifest.actagent)
      ? { ...(params.packageManifest.actagent as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(actagent.extensions)
    ? actagent.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("actagent.plugin.json generated metadata is stale. Run actagent plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `actagent.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("actagent.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`actagent.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `actagent.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include actagent.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json actagent.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run actagent plugins build.");
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    name: `actagent-plugin-${id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && actagent plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && actagent plugins validate --entry ./dist/index.js",
      test: "vitest run",
    },
  ...
  const descriptionLiteral = jsStringLiteral(`Add ${name} tools to ACTAgent.`);
...
import { getToolPluginMetadata } from "actagent/plugin-sdk/tool-plugin";
...
`;
  ...
  writeJsonFile(path.join(rootDir, PLUGIN_MANIFEST_FILENAME), {
    id,
    name,
    description: `Add ${name} tools to ACTAgent.`,

...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "actagent", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "actagent", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "actagent", "nodes", "--", "--json"])).toBe(false);
  });

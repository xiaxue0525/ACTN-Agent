...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    errors,
    runtime,
    searchACTAgentHubPackages: vi.fn(),
  };
});
...
vi.mock("../infra/actagenthub.js", () => ({
  searchACTAgentHubPackages: mocks.searchACTAgentHubPackages,
}));
...
describe("plugins search command", () => {
  beforeEach(() => {
    ...
    mocks.searchACTAgentHubPackages.mockReset();
  });

  it("searches ACTAgentHub code and bundle plugin families", async () => {
    mocks.searchACTAgentHubPackages
      .mockResolvedValueOnce([
        {
          score: 12,
          package: {
            name: "actagent-calendar",
    ...
    expect(mocks.searchACTAgentHubPackages).toHaveBeenCalledWith({
      query: "calendar",
      family: "code-plugin",
      limit: 5,
    });
    ...
    expect(mocks.logs.join("\n")).toContain("actagent-calendar");
    expect(mocks.logs.join("\n")).toContain(
      "Install: actagent plugins install actagenthub:actagent-calendar",
    );
  });

  it("writes JSON results when requested", async () => {
    mocks.searchACTAgentHubPackages.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
  ...
  it("rejects partial numeric search limits", async () => {
    ...
    expect(mocks.searchACTAgentHubPackages).not.toHaveBeenCalled();
  });
});

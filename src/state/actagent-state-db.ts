// ACTAgent state database manages shared persisted state and migrations.
...
import {
  resolveACTAgentStateSqliteDir,
  resolveACTAgentStateSqlitePath,
} from "./actagent-state-db.paths.js";
import { ACTAGENT_STATE_SCHEMA_SQL } from "./actagent-state-schema.generated.js";

/**
 * Shared ACTAgent SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const ACTAGENT_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const ACTAGENT_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const ACTAGENT_STATE_DIR_MODE = 0o700;
const ACTAGENT_STATE_FILE_MODE = 0o600;
const ACTAGENT_STATE_SIDECAR_SUFFIXES = ["", "-shm", "-wal"] as const;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type ACTAgentStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type ACTAgentStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

/** Status stored for a state migration run. */
export type ACTAgentMigrationRunStatus = "completed" | "warning" | "failed";
/** Status stored for a state backup run. */
export type ACTAgentBackupRunStatus = "completed" | "failed";

/** Input for recording one state migration run summary. */
export type RecordACTAgentStateMigrationRunOptions = ACTAgentStateDatabaseOptions & {
  id?: string;
  startedAt: number;
  finishedAt?: number;
  status: ACTAgentMigrationRunStatus;
  report: Record<string, unknown>;
};

/** Input for recording one migrated source file/table pair. */
export type RecordACTAgentStateMigrationSourceOptions = ACTAgentStateDatabaseOptions & {
  runId: string;
...
export type RecordACTAgentStateBackupRunOptions = ACTAgentStateDatabaseOptions & {
  id?: string;
  createdAt: number;
  archivePath: string;
  status: ACTAgentBackupRunStatus;
  manifest: Record<string, unknown>;
};

const cachedDatabases = new Map<string, ACTAgentStateDatabase>();

type ACTAgentStateMetadataDatabase = Pick<
  ACTAgentStateKyselyDatabase,
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > ACTAGENT_STATE_SCHEMA_VERSION) {
    throw new Error(
      `ACTAgent state database ${pathname} uses newer schema version ${userVersion}; this ACTAgent build supports ${ACTAGENT_STATE_SCHEMA_VERSION}.`,
    );
  }
}

function ensureACTAgentStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveACTAgentStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveACTAgentStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`ACTAgent state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: ACTAGENT_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    chmodSync(dir, ACTAGENT_STATE_DIR_MODE);
  }
  for (const suffix of ACTAGENT_STATE_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (existsSync(candidate)) {
      chmodSync(candidate, ACTAGENT_STATE_FILE_MODE);
    }
  }
}
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(ACTAGENT_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${ACTAGENT_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<ACTAgentStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: ACTAGENT_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: ACTAgentStateDatabaseOptions = {}): string {
  return options.path ?? resolveACTAgentStateSqlitePath(options.env ?? process.env);
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openACTAgentStateDatabase(
  options: ACTAgentStateDatabaseOptions = {},
): ACTAgentStateDatabase {
  ...
  ensureACTAgentStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  const walMaintenance = configureSqliteWalMaintenance(db, {
    databaseLabel: "actagent-state",
...
export function runACTAgentStateWriteTransaction<T>(
  operation: (database: ACTAgentStateDatabase) => T,
  options: ACTAgentStateDatabaseOptions = {},
): T {
  const database = openACTAgentStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureACTAgentStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function recordACTAgentStateMigrationRun(
  options: RecordACTAgentStateMigrationRunOptions,
): string {
  const id = options.id ?? randomUUID();
  runACTAgentStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ACTAgentStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
      database.db,
...
export function recordACTAgentStateMigrationSource(
  options: RecordACTAgentStateMigrationSourceOptions,
): void {
  runACTAgentStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ACTAgentStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function recordACTAgentStateBackupRun(options: RecordACTAgentStateBackupRunOptions): string {
  const id = options.id ?? randomUUID();
  runACTAgentStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ACTAgentStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function closeACTAgentStateDatabase(): void {
...
export function isACTAgentStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeACTAgentStateDatabaseForTest = closeACTAgentStateDatabase;

// ACTAgent state database tests cover state DB migrations and persistence.
...
import { resolveACTAgentStateSqlitePath } from "./actagent-state-db.paths.js";
import {
  collectSqliteSchemaShape,
  createSqliteSchemaShapeFromSql,
} from "./sqlite-schema-shape.test-support.js";

type StateDbTestDatabase = Pick<ACTAgentStateKyselyDatabase, "diagnostic_events" | "schema_meta">;

function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "actagent-state-db-"));
}
...
describe("actagent state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveACTAgentStateSqlitePath({ ACTAGENT_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "actagent.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "actagent-test-state", `${process.pid}-7`, "state", "actagent.sqlite"),
    );
  });

  it("creates the shared state schema from the committed SQL shape", () => {
    const stateDir = createTempStateDir();
    const database = openACTAgentStateDatabase({
      env: { ACTAGENT_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./actagent-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "actagent.sqlite"));
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `actagent-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openACTAgentStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openACTAgentStateDatabase({ path: firstPath });
    const second = openACTAgentStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openACTAgentStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { ACTAGENT_STATE_DIR: stateDir } };
    ...
    runACTAgentStateWriteTransaction((database) => {
      ...
      expect(() =>
        runACTAgentStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runACTAgentStateWriteTransaction(async () => {
        return "not sync";
      }, options),

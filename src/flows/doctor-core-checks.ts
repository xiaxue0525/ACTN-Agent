...
import {
  ...
  type LegacyactagentdBrowserProfileResidue,
...
import type { ConfigValidationIssue, ACTAgentConfig } from "../config/types.actagent.js";
...
const BROWSER_actagentd_PROFILE_RESIDUE_CHECK_ID = "core/doctor/browser-actagentd-profile-residue";
...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: ACTAgentConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: ACTAgentConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: ACTAgentConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "actagent.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `actagent configure` and set Gateway mode (local/remote), or `actagent config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `actagent config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `actagent config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: ACTAgentConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `actagent doctor --allow-exec` to verify exec SecretRefs during doctor, or `actagent secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `actagent doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `actagent doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.blockedOutsideEntry
          ? [
              ...
              "or set the affected OpenAI models to an ACTAgent runtime policy.",
            ].join(" ")
          : [
              "Run `actagent doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an ACTAgent runtime policy.",
            ].join(" "),
      }),
    );
    ...
    return [
      {
        ...
        path: workspaceDir,
        fixHint:
          "Inspect the legacy directories and migrate or remove them; see `actagent doctor` for the detailed migration prompt.",
      },
    ];
  },
};
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `actagent doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyactagentdBrowserProfileResidue): HealthFinding {
  return {
    checkId: BROWSER_actagentd_PROFILE_RESIDUE_CHECK_ID,
    severity: "warning",
    ...
    fixHint:
      "Run `actagent doctor --fix` to archive the stale actagentd profile safely instead of deleting it in place.",
  };
}

function formatWouldArchiveBrowserResidue(residue: LegacyactagentdBrowserProfileResidue): string {
  return [
    "Would archive legacy actagentd managed browser profile residue.",
    `- legacy profile: ${residue.legacyProfileDir}`,
    `- canonical profile: ${residue.canonicalUserDataDir}`,
  ].join("\n");
}

const browseractagentdProfileResidueCheck: HealthCheck = {
  id: BROWSER_actagentd_PROFILE_RESIDUE_CHECK_ID,
  kind: "core",
  description:
    "Legacy actagentd managed browser profile residue has been archived after the ACTAgent rename.",
  source: "doctor",
  async detect(ctx, scope) {
    const residue = await detectLegacyactagentdBrowserProfileResidue(ctx.cfg, browserResidueDeps(ctx));
    if (!residue) {
      return [];
    }
  ...
  async repair(ctx) {
    ...
    if (!residue) {
      return {
        status: "skipped",
        reason: "legacy actagentd browser profile residue no longer exists",
        changes: [],
      };
    }
    const effect = {
    ...
    const result = await maybeArchiveLegacyactagentdBrowserProfileResidue(
      ctx.cfg,
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active actagent.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
...
export function createCoreHealthChecks(
  ...
  return [
    ...
    browseractagentdProfileResidueCheck,

...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/actagent-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/actagent-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["actagent-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("registerCoreHealthChecks", () => {
  ...
  it("registers only implemented core health targets from the doctor conversion inventory", () => {
    ...
    for (const id of CORE_HEALTH_CHECKS.map((check) => check.id)) {
      if (id === "core/doctor/browser-actagentd-profile-residue") {
        continue;
      }
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: ACTAgentConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/actagent-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
    ...
    const findings = await check.detect({
      ...
      runtime,
      cfg,
      cwd: "/tmp/actagent-test-workspace",
    });
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves in lint checks", async () => {
    ...
    await withEnvAsync({ ACTAGENT_TEST_GATEWAY_TOKEN: "resolved-test-token" }, async () => {
      const findings = await check?.detect({
        mode: "lint",
          ...
          gateway: {
            ...
            auth: {
              ...
              token: {
                source: "env",
                provider: "default",
                id: "ACTAGENT_TEST_GATEWAY_TOKEN",
              },
            },
          },
          secrets: {
  ...
  it("reports unresolved SecretRefs even when ACTAGENT_GATEWAY_TOKEN is set", async () => {
    const check = CORE_HEALTH_CHECKS.find((entry) => entry.id === "core/doctor/gateway-auth");
    await withEnvAsync(
      {
        ACTAGENT_GATEWAY_TOKEN: "fallback-token",
        ACTAGENT_MISSING_GATEWAY_REF_TOKEN: undefined,
      },
      async () => {
        const findings = await check?.detect({
          mode: "lint",
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "actagent-health-exec-ref-"));
  ...
  it("reports exec SecretRef failures when gateway auth lint explicitly allows exec checks", async () => {
    ...
    const findings = await withEnvAsync({ ACTAGENT_GATEWAY_TOKEN: "fallback-token" }, async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Gateway token SecretRef could not be resolved:"),
        fixHint:
          "Run `actagent doctor --allow-exec` to verify exec SecretRefs during doctor, or `actagent secrets audit --allow-exec` to audit all exec SecretRefs.",
      }),
    );
  });

...
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<ACTAgentConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.actagent.ai/tools/web";
...
function rawKeyValue(config: ACTAgentConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: ACTAgentConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: ACTAgentConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: ACTAgentConfig, result: ACTAgentConfig): ACTAgentConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as ACTAgentConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "ACTAgent will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.actagent.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — ACTAgent will store a reference instead of the API key.",

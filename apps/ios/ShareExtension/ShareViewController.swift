...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.actagent.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to ACTAgent", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to ACTAgent gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to ACTAgent.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "ACTAgentShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "ACTAgent is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "ACTAgent Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("actagent-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in

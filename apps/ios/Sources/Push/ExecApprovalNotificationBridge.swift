...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "actagent.exec-approval"
    static let reviewActionIdentifier = "actagent.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.actAgentPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.actAgentPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func actAgentPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {

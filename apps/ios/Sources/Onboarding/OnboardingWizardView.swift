...
struct OnboardingWizardView: View {
    ...
    @State private var manualPort: Int = 19199
    @State private var manualPortText: String = "19199"
    @State private var manualTLS: Bool = true
    @State private var gatewayToken: String = ""
    @State private var gatewayPassword: String = ""
    @State private var connectMessage: String?
    @State private var statusLine: String = "In your ACTAgent chat, run /pair qr, then scan the code here."
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `actagent devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "actagent devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your ACTAgent chat\n"
                            + "\(requestLine)\n"
                            + "ACTAgent will also retry automatically when you return to this app.")
                }
            }
...
extension OnboardingWizardView {
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your ACTAgent chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "actagent.local"
                    self.manualPort = 19199
                    self.manualTLS = true
                }
            } else {
                self.manualHost = "actagent.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "actagent.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your ACTAgent chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "actagent.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "actagent.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 19199 }
        case .remoteDomain:
            if host == "actagent.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 19199 }
        case .developerLocal:

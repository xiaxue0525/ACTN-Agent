...
@MainActor
enum ACTAgentAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class ACTAgentAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.actagent.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.actagent.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.actagent.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? ACTAgentAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "actagent.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "actagent.watch.promptId"
    static let sessionKeyKey = "actagent.watch.sessionKey"
    static let actionPrimaryIDKey = "actagent.watch.action.primary.id"
    static let actionPrimaryLabelKey = "actagent.watch.action.primary.label"
    static let actionSecondaryIDKey = "actagent.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "actagent.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "actagent.watch.action.primary"
    static let actionSecondaryIdentifier = "actagent.watch.action.secondary"
    static let actionIdentifierPrefix = "actagent.watch.action."
    static let actionIDKeyPrefix = "actagent.watch.action.id."
    static let actionLabelKeyPrefix = "actagent.watch.action.label."
    static let categoryPrefix = "actagent.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ACTAgentWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ACTAgentWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ACTAgentWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "ACTAgent" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ACTAgentWatchAction]) -> [UNNotificationAction] {
...
struct ACTAgentApp: App {
    ...
    init() {
        ...
        ACTAgentAppModelRegistry.appModel = appModel
...
extension ACTAgentApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("ACTAgent: installing uncaught exception handler")

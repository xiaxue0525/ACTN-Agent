...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ACTAgentContactsSearchParams) async throws -> ACTAgentContactsSearchPayload {
        ...
        return ACTAgentContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ACTAgentContactsAddParams) async throws -> ACTAgentContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ACTAgentContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ACTAgentContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ACTAgentContactPayload {
        ACTAgentContactPayload(

...
extension SettingsProTab {
    ...
    var gatewayConnectionRow: some View {
        HStack(spacing: 12) {
            ProIconBadge(
                ...
                color: self.gatewayConnected ? ACTAgentBrand.ok : .secondary)
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(self.gatewayConnected ? "Connected" : self.appModel.gatewayDisplayStatusText)
                    .font(.caption)
                    .foregroundStyle(self.gatewayConnected ? ACTAgentBrand.ok : .secondary)
            }
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            ...
            self.settingsListRow(
                icon: "info.circle",
                title: "About",
                detail: DeviceInfoHelper.actAgentVersionString(),
                route: .about)
        }
        .padding(.horizontal, ACTAgentProMetric.pagePadding)
    }
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while ACTAgent is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while ACTAgent is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? ACTAgentBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayConnected ? ACTAgentBrand.ok : ACTAgentBrand.warn)
            ...
            self.detailListCard {
                ...
                Divider()
                self.detailRow("App", value: DeviceInfoHelper.actAgentVersionString())
                Divider()
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context ACTAgent can expose to the gateway.",
    ...
    var notificationsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "bell",
                title: "Notifications",
                detail: "Approvals and event alerts from ACTAgent.",
                value: self.notificationStatusText,
                color: self.notificationStatusText == "Allowed" ? ACTAgentBrand.ok : .secondary)
            ...
            ProCard(radius: SettingsLayout.cardRadius) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("ACTAgent uses notifications for approval prompts and mirrored event alerts.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    var aboutDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                value: DeviceInfoHelper.actAgentVersionString(),
                color: ACTAgentBrand.accent)

            self.detailListCard {
                self.detailRow("Version", value: DeviceInfoHelper.actAgentVersionString())
                Divider()
                self.detailRow("Device", value: DeviceInfoHelper.deviceFamily())
                Divider()
                self.detailRow("Platform", value: DeviceInfoHelper.platformStringForDisplay())
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? ACTAgentBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == ACTAgentLocationMode.off.rawValue ? .secondary : ACTAgentBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(ACTAgentLocationMode.off.rawValue)
                    Text("While Using").tag(ACTAgentLocationMode.whileUsing.rawValue)
                    Text("Always").tag(ACTAgentLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)

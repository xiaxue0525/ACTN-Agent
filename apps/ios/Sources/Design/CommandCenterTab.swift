...
struct CommandCenterTab: View {
    ...
    @State private var activeChatSessions: [ACTAgentChatSessionEntry] = []
    var openChat: () -> Void
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                .safeAreaPadding(.bottom, ACTAgentProMetric.bottomScrollInset)
            }
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ACTAgentProMark(size: 31, shadowRadius: 9)
            Text("ACTAgent")
    ...
    private var pendingApprovalsContent: some View {
        CommandPanel(
            tint: self.pendingApproval == nil ? nil : ACTAgentBrand.warn,
    ...
    private var startWorkAction: some View {
        CommandPanel(tint: ACTAgentBrand.accent, isProminent: true, padding: 9) {
                ...
                Label("Start work", systemImage: "play.fill")
                    ...
                    .background {
                        RoundedRectangle(cornerRadius: 13, style: .continuous)
                            .fill(LinearGradient(
                                colors: [ACTAgentBrand.accentHot, ACTAgentBrand.accent],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing))
                            .shadow(color: ACTAgentBrand.accentHot.opacity(0.34), radius: 18, y: 8)
                    }
            }
            .buttonStyle(.plain)
        }
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(ACTAgentBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? ACTAgentBrand.ok : .secondary
    }
    ...
    private var liveActivityColor: Color {
        if self.pendingApproval != nil { return ACTAgentBrand.warn }
        return self.gatewayConnected ? ACTAgentBrand.ok : .secondary
    }
        ...
        return self.activeChatSessions
            ...
            .map { session in
                ...
                return WorkItem(
                    ...
                    trailing: "chat",
                    color: isCurrent ? ACTAgentBrand.accent : ACTAgentBrand.ok,
                    progress: nil,
    ...
    private var defaultSessionItems: [WorkItem] {
        [
            ...
            WorkItem(
                ...
                trailing: "voice",
                color: self.appModel.talkMode.isEnabled ? ACTAgentBrand.ok : .secondary,
                progress: nil,
                route: .settings),
            WorkItem(
                id: "device-capture",
                ...
                trailing: "device",
                color: self.appModel.screenRecordActive ? ACTAgentBrand.warn : .secondary,
                progress: nil,
                route: .settings),
            WorkItem(
                id: "agent-roster",
    ...
    private static func sessionChoices(
        _ sessions: [ACTAgentChatSessionEntry],
        currentSessionKey: String) -> [ACTAgentChatSessionEntry]
    {
    ...
    private static func sessionTitle(_ session: ACTAgentChatSessionEntry) -> String {
    ...
    private static func sessionDetail(_ session: ACTAgentChatSessionEntry) -> String {

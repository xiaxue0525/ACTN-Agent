...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking ACTAgent" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return ACTAgentBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return ACTAgentBrand.warn
        default:
            if !self.isConfigLoaded { return ACTAgentBrand.warn }
            return self.isEnabled ? ACTAgentBrand.ok : ACTAgentBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(ACTAgentBrand.danger)
        case .waiting:
            AnyShapeStyle(ACTAgentBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), ACTAgentBrand.accent],

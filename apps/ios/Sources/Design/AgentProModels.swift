...
struct ACTAgentHubSearchParams: Encodable {
    let query: String?
    let limit: Int
}

struct ACTAgentHubSearchResponseLite: Decodable {
    let results: [ACTAgentHubSearchResultLite]
}

struct ACTAgentHubSearchResultLite: Decodable {
    let slug: String
...
struct ACTAgentHubInstallParams: Encodable {
    let source = "actagenthub"
    let slug: String
}

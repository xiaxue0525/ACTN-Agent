...
extension AgentProTab {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let skillMutationStatusText {
                    Text(skillMutationStatusText)
                        .font(.caption2)
                        .foregroundStyle(ACTAgentBrand.accent)
                }
                if let skillMutationErrorText {
                    Text(skillMutationErrorText)
                        .font(.caption2)
                        .foregroundStyle(ACTAgentBrand.warn)
                }
            }
        }
        .padding(.horizontal, ACTAgentProMetric.pagePadding)
    }
    ...
    var actAgentHubSearchCard: some View {
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    ProIconBadge(systemName: "square.and.arrow.down", color: ACTAgentBrand.accent)
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Install Skills")
                            .font(.headline)
                        Text("Search ACTAgentHub and install into this workspace.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    Spacer(minLength: 8)
                    Button {
                        Task { await self.searchACTAgentHubSkills() }
                    } label: {
                        Image(systemName: "magnifyingglass")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                    .disabled(self.actAgentHubLoading || !self.gatewayConnected)
                    .accessibilityLabel("Search ACTAgentHub")
                }

                TextField("Search ACTAgentHub", text: self.$actAgentHubQuery)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                    .font(.subheadline)
                    .submitLabel(.search)
                ...
                if self.actAgentHubLoading {
                    ProgressView()
                        .controlSize(.small)
                }
                if let actAgentHubErrorText {
                    Text(actAgentHubErrorText)
                        .font(.caption2)
                        .foregroundStyle(ACTAgentBrand.warn)
                }
                if !self.actAgentHubResults.isEmpty {
                    VStack(spacing: 0) {
                        let results = Array(self.actAgentHubResults.prefix(8))
                        ForEach(Array(results.enumerated()), id: \.element.slug) { index, result in
                            self.actAgentHubResultRow(result)
    ...
    func actAgentHubResultRow(_ result: ACTAgentHubSearchResultLite) -> some View {
        let installing = self.actAgentHubInstallSlug == result.slug
        return HStack(alignment: .top, spacing: 10) {
            ProIconBadge(systemName: "sparkles", color: ACTAgentBrand.accent)
            VStack(alignment: .leading, spacing: 3) {
            ...
            Spacer(minLength: 8)
            Button {
                Task { await self.installACTAgentHubSkill(result) }
            } label: {
    ...
    func isSkillConfigBusy(_ skill: SkillStatusEntryLite) -> Bool {
        self.skillConfigBusyKeys.contains(skill.effectiveSkillKey)
            || self.actAgentHubInstallSlug != nil
    }
        ...
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let message = self.skillConfigMessages[skill.effectiveSkillKey] {
                    Text(message.text)
                        .font(.caption2)
                        .foregroundStyle(message.kind == .success ? ACTAgentBrand.accent : ACTAgentBrand.warn)
                }
            }
        }
        .padding(.horizontal, ACTAgentProMetric.pagePadding)
    ...
    @MainActor
    func installACTAgentHubSkill(_ result: ACTAgentHubSearchResultLite) async {
        guard self.gatewayConnected else { return }
        self.actAgentHubInstallSlug = result.slug
        self.actAgentHubErrorText = nil
        defer { self.actAgentHubInstallSlug = nil }
        do {
            let params = ACTAgentHubInstallParams(slug: result.slug)
            _ = try await self.requestGateway(method: "skills.install", params: params, timeoutSeconds: 125)
            await self.appModel.refreshGatewayOverviewIfConnected()
            await self.refreshOverview(force: true)
        } catch {
            self.actAgentHubErrorText = Self.skillMutationMessage(error)
        }
    }

    @MainActor
    func searchACTAgentHubSkills() async {
        guard self.gatewayConnected else { return }
        self.actAgentHubLoading = true
        self.actAgentHubErrorText = nil
        defer { self.actAgentHubLoading = false }
        do {
            let query = self.actAgentHubQuery.trimmingCharacters(in: .whitespacesAndNewlines)
            let params = ACTAgentHubSearchParams(query: query.isEmpty ? nil : query, limit: 20)
            let data = try await self.requestGateway(method: "skills.search", params: params, timeoutSeconds: 20)
            self.actAgentHubResults = try JSONDecoder().decode(ACTAgentHubSearchResponseLite.self, from: data).results
        } catch {
            self.actAgentHubErrorText = Self.skillMutationMessage(error)
        }
    }
    ...
    func skillStatus(_ skill: SkillStatusEntryLite) -> (text: String, color: Color) {
        ...
        if skill.hasMissingRequirements {
            return ("setup", ACTAgentBrand.warn)
        }
        return ("enabled", ACTAgentBrand.accent)
    }
}

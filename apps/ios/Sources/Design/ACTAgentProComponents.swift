...
struct ACTAgentProBackground: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        LinearGradient(
            colors: ACTAgentBrand.canvasColors(for: self.colorScheme),
            startPoint: .top,
            endPoint: .bottom)
            .ignoresSafeArea()
            .overlay(alignment: .top) {
                if self.colorScheme == .light {
                    LinearGradient(
                        colors: [
                            ACTAgentBrand.accent.opacity(0.05),
                            .clear,
                        ],
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = ACTAgentProMetric.cardRadius
    ...
    private var borderStyle: AnyShapeStyle {
        ...
        let gradient = LinearGradient(
            colors: [
                ...
                (self.tint ?? ACTAgentBrand.accent).opacity(0.10),
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = ACTAgentProMetric.cardRadius,
...
struct ACTAgentProMark: View {
    ...
    var body: some View {
        Image("ACTAgentIcon")
            ...
            .shadow(color: ACTAgentBrand.accent.opacity(0.28), radius: self.shadowRadius, y: self.shadowRadius / 2)
            .accessibilityLabel("ACTAgent")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = ACTAgentBrand.accentHot
        ...
        Button(action: self.action) {
            HStack(spacing: 12) {
                Image(systemName: self.systemImage)
                    ...
                    .background(ACTAgentBrand.accentHot, in: RoundedRectangle(cornerRadius: 13, style: .continuous))
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundStyle(ACTAgentBrand.accentHot)
            }
            .padding(12)
            .proGlassSurface(
                fill: self.colorScheme == .dark ? Color.white.opacity(0.045) : Color.white.opacity(0.68),
                stroke: ACTAgentBrand.accent.opacity(self.colorScheme == .dark ? 0.22 : 0.14),
                radius: 18,
                isProminent: true,
                interactive: true)
        }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ProIconBadge(
                ...
                color: self.done ? ACTAgentBrand.ok : ACTAgentBrand.warn)
            VStack(alignment: .leading, spacing: 3) {
                Text(self.title)

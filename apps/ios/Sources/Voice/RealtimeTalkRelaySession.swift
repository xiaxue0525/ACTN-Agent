...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "actagent_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.actagent", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "ACTAgent tool call failed"]
                : ["text": completion.text ?? "ACTAgent finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: ACTAgentChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }

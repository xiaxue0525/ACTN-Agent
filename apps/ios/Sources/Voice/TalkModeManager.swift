...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ACTAGENT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ACTAgentTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.actagent", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ACTAgentTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ACTAgentTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return ACTAgentTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ACTAgentTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ACTAgentTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ACTAgentTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ACTAgentTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ACTAgentTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ACTAgentTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = ACTAgentChatEventText.assistantText(from: chatEvent) {

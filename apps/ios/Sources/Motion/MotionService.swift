...
final class MotionService: MotionServicing {
    func activities(params: ACTAgentMotionActivityParams) async throws -> ACTAgentMotionActivityPayload {
        ...
        let mapped: [ACTAgentMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ACTAgentMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = ACTAgentPedometerPayload(

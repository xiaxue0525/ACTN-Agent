...
final class RemindersService: RemindersServicing {
    func list(params: ACTAgentRemindersListParams) async throws -> ACTAgentRemindersListPayload {
        ...
        let payload: [ACTAgentReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ACTAgentReminderPayload(
        ...
        return ACTAgentRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ACTAgentRemindersAddParams) async throws -> ACTAgentRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ACTAgentReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ACTAgentRemindersAddPayload(reminder: payload)
    }

...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.actagent.ios", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<ACTAgentActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = ACTAgentActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<ACTAgentActivityAttributes>.activities
    ...
    private func updateCurrent(state: ACTAgentActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<ACTAgentActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> ACTAgentActivityAttributes.ContentState {
        ACTAgentActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> ACTAgentActivityAttributes.ContentState {
        ACTAgentActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func idleState() -> ACTAgentActivityAttributes.ContentState {
        ACTAgentActivityAttributes.ContentState(
            statusText: "Idle",
            isIdle: true,
            isDisconnected: false,
    ...
    private func disconnectedState() -> ACTAgentActivityAttributes.ContentState {
        ACTAgentActivityAttributes.ContentState(

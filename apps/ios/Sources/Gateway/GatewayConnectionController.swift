...
extension GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [
            ...
            ACTAgentCapability.screen.rawValue,
        ]
        ...
        if cameraEnabled { caps.append(ACTAgentCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ACTAgentCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ACTAgentLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ACTAgentCapability.location.rawValue) }

        caps.append(ACTAgentCapability.device.rawValue)
        caps.append(ACTAgentCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(ACTAgentCapability.watch.rawValue)
        }
        caps.append(ACTAgentCapability.photos.rawValue)
        caps.append(ACTAgentCapability.contacts.rawValue)
        caps.append(ACTAgentCapability.calendar.rawValue)
        caps.append(ACTAgentCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(ACTAgentCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ACTAgentScreenCommand.record.rawValue,
            ACTAgentSystemCommand.notify.rawValue,
            ACTAgentChatCommand.push.rawValue,
        ...
        if caps.contains(ACTAgentCapability.camera.rawValue) {
            commands.append(ACTAgentCameraCommand.list.rawValue)
            commands.append(ACTAgentCameraCommand.snap.rawValue)
            commands.append(ACTAgentCameraCommand.clip.rawValue)
        }
        if caps.contains(ACTAgentCapability.location.rawValue) {
            commands.append(ACTAgentLocationCommand.get.rawValue)
        }
        if caps.contains(ACTAgentCapability.device.rawValue) {
            commands.append(ACTAgentDeviceCommand.status.rawValue)
            commands.append(ACTAgentDeviceCommand.info.rawValue)
        }
        if caps.contains(ACTAgentCapability.watch.rawValue) {
            commands.append(ACTAgentWatchCommand.status.rawValue)
            commands.append(ACTAgentWatchCommand.notify.rawValue)
        }
        if caps.contains(ACTAgentCapability.photos.rawValue) {
            commands.append(ACTAgentPhotosCommand.latest.rawValue)
        }
        if caps.contains(ACTAgentCapability.contacts.rawValue) {
            commands.append(ACTAgentContactsCommand.search.rawValue)
            commands.append(ACTAgentContactsCommand.add.rawValue)
        }
        if caps.contains(ACTAgentCapability.calendar.rawValue) {
            commands.append(ACTAgentCalendarCommand.events.rawValue)
            commands.append(ACTAgentCalendarCommand.add.rawValue)
        }
        if caps.contains(ACTAgentCapability.reminders.rawValue) {
            commands.append(ACTAgentRemindersCommand.list.rawValue)
            commands.append(ACTAgentRemindersCommand.add.rawValue)
        }
        if caps.contains(ACTAgentCapability.motion.rawValue) {
            commands.append(ACTAgentMotionCommand.activity.rawValue)
            commands.append(ACTAgentMotionCommand.pedometer.rawValue)
        }

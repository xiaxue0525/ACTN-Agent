...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ACTAgentDeviceStatusPayload {
        ...
        return ACTAgentDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> ACTAgentDeviceInfoPayload {
        ...
        return ACTAgentDeviceInfoPayload(
    ...
    private func batteryStatus() -> ACTAgentBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ACTAgentBatteryState = switch device.batteryState {
        ...
        return ACTAgentBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ACTAgentThermalStatusPayload {
        let state: ACTAgentThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ACTAgentThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ACTAgentStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ACTAgentStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }

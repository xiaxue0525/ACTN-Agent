...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: ACTAgentWatchNotifyParams) -> ACTAgentWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [ACTAgentWatchAction]?,
        kind: String?,
        promptId: String?) -> [ACTAgentWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> ACTAgentWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ACTAgentWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                ACTAgentWatchAction(id: "approve", label: "Approve"),
                ACTAgentWatchAction(id: "decline", label: "Decline", style: "destructive"),
                ACTAgentWatchAction(id: "open_phone", label: "Open iPhone"),
                ACTAgentWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            ACTAgentWatchAction(id: "done", label: "Done"),
            ACTAgentWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            ACTAgentWatchAction(id: "open_phone", label: "Open iPhone"),
            ACTAgentWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: ACTAgentWatchRisk?,
        priority: ACTAgentNotificationPriority?) -> ACTAgentWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: ACTAgentNotificationPriority?,
        risk: ACTAgentWatchRisk?) -> ACTAgentNotificationPriority?
    {

...
struct IOSGatewayChatTransport: ACTAgentChatTransport {
    static let logger = Logger(subsystem: "ai.actagent", category: "ios.chat.transport")
    ...
    private struct ChatSendParams: Codable {
        ...
        var message: String
        var thinking: String
        var attachments: [ACTAgentChatAttachmentPayload]?
        var timeoutMs: Int
    ...
    static func makeChatSendParamsJSON(
        ...
        attachments: [ACTAgentChatAttachmentPayload]) throws -> String
    {
        ...
        parentSessionKey: String?) async throws -> ACTAgentChatCreateSessionResponse
    {
        ...
        return try JSONDecoder().decode(ACTAgentChatCreateSessionResponse.self, from: res)
    }
    ...
    func listSessions(limit: Int?) async throws -> ACTAgentChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ACTAgentChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ACTAgentChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ACTAgentChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ACTAgentChatAttachmentPayload]) async throws -> ACTAgentChatSendResponse
    {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ACTAgentChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ACTAgentGatewayHealthOK.self, from: res))?.ok ?? true
    }
    ...
    static func mapEventFrame(_ evt: EventFrame) -> ACTAgentChatTransportEvent? {
        switch evt.event {
        case "tick":
            return .tick
        case "seqGap":
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: ACTAgentGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = evt.payload else { return nil }
        ...
        case "session.message":
            ...
            guard let message = try? GatewayPayloadDecoding.decode(
                payload,
                as: ACTAgentSessionMessageEventPayload.self)
            else {
                return nil
            }
        ...
        case "agent":
            ...
            guard let agentPayload = try? GatewayPayloadDecoding.decode(
                payload,
                as: ACTAgentAgentEventPayload.self)
            else {
                return nil
            }

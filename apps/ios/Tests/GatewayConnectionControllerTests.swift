...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": ACTAgentLocationMode.always.rawValue,
            ...
            #expect(caps.contains(ACTAgentCapability.canvas.rawValue))
            #expect(caps.contains(ACTAgentCapability.screen.rawValue))
            #expect(caps.contains(ACTAgentCapability.camera.rawValue))
            #expect(caps.contains(ACTAgentCapability.location.rawValue))
            #expect(caps.contains(ACTAgentCapability.voiceWake.rawValue))
            #expect(caps.contains(ACTAgentCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": ACTAgentLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(ACTAgentLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(ACTAgentSystemCommand.notify.rawValue))
            #expect(!commands.contains(ACTAgentSystemCommand.run.rawValue))
            #expect(!commands.contains(ACTAgentSystemCommand.which.rawValue))
            #expect(!commands.contains(ACTAgentSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(ACTAgentSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.actagent.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.actagent.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.actagent.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.actagent.gateway", account: "lastConnection")

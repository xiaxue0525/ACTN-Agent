...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_actagent-gw._tcp", domain: "local.", interface: nil)
    ...
    @Test @MainActor func manualConnectionsAllowPrivateLanPlaintext() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "actagent.local", useTLS: false) == false)
    ...
    @Test @MainActor func manualDefaultPortUses443OnlyForTailnetTLSHosts() async {
        ...
        #expect(controller._test_resolveManualPort(host: "gateway.example.com", port: 0, useTLS: true) == 19199)
        #expect(controller._test_resolveManualPort(host: "device.sample.ts.net", port: 0, useTLS: true) == 443)
        #expect(controller._test_resolveManualPort(host: "device.sample.ts.net.", port: 0, useTLS: true) == 443)
        #expect(controller._test_resolveManualPort(host: "device.sample.ts.net", port: 19199, useTLS: true) == 19199)
    }

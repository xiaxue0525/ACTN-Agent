# ACTAgent iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. It connects to an ACTAgent Gateway as a `role: node` on iPhone and iPad.
...
- Beta release uses canonical `ai.actagent.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `ACTAgentPushTransport=relay`, `ACTAgentPushDistribution=official`, and `ACTAgentPushAPNsEnvironment=production`.
...
- Beta builds default to `https://ios-push-relay.actagent.ai`.
- Optional custom relay override: `ACTAGENT_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
3. Optional: set a custom official/TestFlight relay URL for the build. If unset, the beta flow uses `https://ios-push-relay.actagent.ai`.

```bash
export ACTAGENT_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/ACTAgent-<version>.ipa`
   - `apps/ios/build/beta/ACTAgent-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/ACTAgent.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `ACTAgentPushTransport=direct` and `ACTAgentPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `ACTAGENT_APNS_TEAM_ID`, `ACTAGENT_APNS_KEY_ID`, and either `ACTAGENT_APNS_PRIVATE_KEY_P8` or `ACTAGENT_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.actagent/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `ACTAGENT_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `ACTAgentPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `actagent.json`. `ACTAGENT_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine ACTAgent official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as ACTAgent node commands through the gateway. Agents can drive the iPhone or iPad canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.

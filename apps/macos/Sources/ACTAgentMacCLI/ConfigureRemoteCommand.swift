...
private let appDefaultsSuites = ["ai.actagent.mac", "ai.actagent.mac.debug"]
private let appOnboardingVersion = 7

struct ConfigureRemoteOptions {
    var sshTarget: String?
    var directUrl: String?
    var localPort: Int = 19199
    var remotePort: Int = 19199
    var token: String?
    var password: String?
    var identity: String?
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              actagent-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              actagent-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --local-port <p>    Local tunnel port for the mac app/UI. Default: 19199.
              --remote-port <p>   Gateway port on the remote host. Default: 19199.
              --token <token>     Remote gateway token.
              --password <pw>     Remote gateway password.
              --identity <path>   SSH identity file.
              --project-root <p>  Remote ACTAgent checkout for CLI commands.
              --cli-path <path>   Remote actagent executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func configureSSHRemote(_ opts: ConfigureRemoteOptions) throws -> ConfigureRemoteOutput {
    ...
    let configURL = actAgentConfigURL()
...
private func actAgentConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["ACTAGENT_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".actagent/actagent.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "actagent.connectionMode")
        setDefaultString(defaults, key: "actagent.remoteTarget", value: target)
        defaults.set(true, forKey: "actagent.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "actagent.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "actagent.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "actagent.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "actagent.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("ACTAgent macOS Remote Config")

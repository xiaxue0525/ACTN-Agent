...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.actagent", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ACTAgentSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ACTAgentSessionPreviewEntry,
    ...
    private static func cache(payload: ACTAgentSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ACTAgentChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ACTAgentKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ACTAgentKit.AnyCodable]) -> [ACTAgentChatMessage] {
    ...
    private static func previewText(for message: ACTAgentChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ACTAgentChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ACTAgentChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ACTAgentChatMessage) -> String? {

...
enum DebugActions {
    private static let verboseDefaultsKey = "actagent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "actagent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ACTAgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ACTAgent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ACTAgentPaths.configURL
        guard

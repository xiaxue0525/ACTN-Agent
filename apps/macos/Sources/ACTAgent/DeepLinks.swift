...
private let deepLinkLogger = Logger(subsystem: "ai.actagent", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ACTAgent is paused", message: "Unpause ACTAgent to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ACTAgent agent?", message: body) else { return }
        }

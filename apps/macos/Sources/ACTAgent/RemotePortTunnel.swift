...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.actagent", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(defaultRemotePort: Int, for sshHost: String) -> Int? {
        ...
        guard let sshKey = ACTAgentConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = ACTAgentConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.actagent.remote.tunnel.port", qos: .utility)
            do {

...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.actagent.mac`) even if the shipped
    ...
    {
        if environment["ACTAGENT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "actagent.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "actagent.nixMode") { return true }

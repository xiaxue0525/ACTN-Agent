...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.actagent", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["ACTAGENT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = ACTAgentConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        let stored = UserDefaults.standard.integer(forKey: "gatewayPort")
        return stored > 0 ? stored : 19199
    }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.actagentExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "actagent CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["ACTAGENT_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "actagent@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "actagent@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "actagent@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "actagent@\(target)"])
            }

        statusHandler("Installing actagent@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed actagent@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("actagent ") {
            normalized = String(normalized.dropFirst("actagent ".count))
        }

...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ACTAgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.actagent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ACTAgentKit.AnyCodable]()) {
                $0[$1.key] = ACTAgentKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking ACTAgent Local Network access.
                    Allow ACTAgent in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ACTAgentProtocol.AnyCodable?) -> [String: ACTAgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ACTAgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ACTAgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ACTAgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("actagent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("actagent.control.agent")
}

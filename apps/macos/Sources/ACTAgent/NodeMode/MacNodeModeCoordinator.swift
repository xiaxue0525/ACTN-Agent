...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.actagent", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = ACTAgentConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ACTAgentNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: ACTAgentLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(ACTAgentCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(ACTAgentCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(ACTAgentCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: ACTAgentConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: ACTAgentLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            ACTAgentSystemCommand.which.rawValue,
            ACTAgentSystemCommand.run.rawValue,
            ACTAgentSystemCommand.execApprovalsGet.rawValue,
            ACTAgentSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(ACTAgentCapability.browser.rawValue) {
            commands.append(ACTAgentBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(ACTAgentCapability.camera.rawValue) {
        ...
        if capsSet.contains(ACTAgentCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: ACTAgentConfigFile.loadDict(),

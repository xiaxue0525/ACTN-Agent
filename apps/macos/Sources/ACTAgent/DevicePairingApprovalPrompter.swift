...
final class DevicePairingApprovalPrompter {
    ...
    private let logger = Logger(subsystem: "ai.actagent", category: "device-pairing")
    ...
    static func alertSummary(for req: PendingRequest) -> String {
        ...
        return "Approve \(subject) to control ACTAgent. Only approve if this is yours; you can remove it later in Settings."
    }
    ...
    static func deviceName(for req: PendingRequest) -> String {
        ...
        return self.isMac(req.platform) ? "ACTAgent Mac app" : "New device"
    }

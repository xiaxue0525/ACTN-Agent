...
private let webChatSwiftLogger = Logger(subsystem: "ai.actagent", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "actagent.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: ACTAgentChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> ACTAgentChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(ACTAgentChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            ACTAgentChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return ACTAgentChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ACTAgentChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ACTAgentGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    ACTAgentSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ACTAgentAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: ACTAgentProtocol.ModelChoice) -> ACTAgentChatModelChoice {
        ACTAgentChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ACTAgentChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ACTAgentChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ACTAgentChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: ACTAgentChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ACTAgentChatView>) -> NSViewController
    {

...
final class AppState {
    private static let logger = Logger(subsystem: "ai.actagent", category: "app-state")
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        let fallback = "ws://127.0.0.1:19199"
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            ACTAgentConfigFile.canonicalHostForComparison(host) ==
                ACTAgentConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = ACTAgentConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: ACTAgentConfigFile.loadDict(),
            draft: .init(
        ...
        guard ACTAgentConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/actagent"
        state.remoteCliPath = ""
        return state
    }
}

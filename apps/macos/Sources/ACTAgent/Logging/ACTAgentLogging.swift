...
enum ACTAgentLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ACTAgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ACTAgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ACTAgentLogging.bootstrapIfNeeded()
        let label = ACTAgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ACTAgentOSLogHandler: AppLogLevelBackedHandler {
...
struct ACTAgentFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ACTAgentLogging.parseLabel(self.label)

...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ACTAGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/actagent")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("actagent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("actagent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/actagent/ (rolling or stdout), or nil if none exist.

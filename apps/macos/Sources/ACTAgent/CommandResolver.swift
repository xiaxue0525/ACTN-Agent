...
enum CommandResolver {
    private static let projectRootDefaultsKey = "actagent.gatewayProjectRootPath"
    private static let helperName = "actagent"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let actagentEntry = root.appendingPathComponent("actagent.mjs").path
        if FileManager().isReadableFile(atPath: actagentEntry) { return actagentEntry }
        let binEntry = root.appendingPathComponent("bin/actagent.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/actagent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let actagentPaths = self.actagentManagedPaths(home: home)
        if !actagentPaths.isEmpty {
            extras.insert(contentsOf: actagentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + actagentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func actagentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".actagent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func actagentExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectACTAgentExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("actagent.mjs").path,
            root.appendingPathComponent("bin/actagent.js").path,
        ]
    ...
    static func hasAnyACTAgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.actagentExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func actagentNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let actagentPath = self.projectACTAgentExecutable(projectRoot: root) {
            return [actagentPath, subcommand] + extraArgs
        }
        if let actagentPath = self.actagentExecutable(searchPaths: searchPaths) {
            return [actagentPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "actagent", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            actagent entrypoint missing (looked for dist/index.js or actagent.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real actagent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/actagent"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/actagent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/actagent.mjs"
            node "$PRJ/actagent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/actagent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/actagent.js"
            node "$PRJ/bin/actagent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent actagent"
          pnpm --silent actagent \(quotedArgs);
        else
          echo "actagent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ACTAgentConfigFile.loadDict()

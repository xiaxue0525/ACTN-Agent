...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(ACTAgentCapability.browser.rawValue))
        #expect(!commands.contains(ACTAgentBrowserCommand.proxy.rawValue))
        #expect(commands.contains(ACTAgentCanvasCommand.present.rawValue))
    ...
    @Test func `local tls params ignore remote configured fingerprint`() throws {
        let url = try #require(URL(string: "wss://127.0.0.1:19199"))
        let root: [String: Any] = [
            "gateway": [
                "remote": [

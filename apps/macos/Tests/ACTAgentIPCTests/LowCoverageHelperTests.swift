...
struct LowCoverageHelperTests {
    private typealias ProtoAnyCodable = ACTAgentProtocol.AnyCodable
    ...
    @Test func `port guardian parses listeners and builds reports`() {
        ...
        let emptyReport = PortGuardian._testBuildReport(port: 19199, mode: .local, listeners: [])
    ...
    @Test func `port guardian local mode still rejects unexpected`() {
        ...
        #expect(PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "actagent-gateway",
            port: 19199, mode: .local) == true)

...
struct GatewayEndpointStoreTests {
    ...
    @Test func `resolve gateway token prefers env and falls back to launchd`() {
        let snapshot = self.makeLaunchAgentSnapshot(
            env: ["ACTAGENT_GATEWAY_TOKEN": "launchd-token"],
            token: "launchd-token",
            password: nil)
        ...
        let envToken = GatewayEndpointStore._testResolveGatewayToken(
            isRemote: false,
            root: [:],
            env: ["ACTAGENT_GATEWAY_TOKEN": "env-token"],
    ...
    @Test func `resolve gateway password falls back to launchd`() {
        let snapshot = self.makeLaunchAgentSnapshot(
            env: ["ACTAGENT_GATEWAY_PASSWORD": "launchd-pass"],
            token: nil,
    ...
    @Test func `dashboard URL uses local base path in local mode`() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://127.0.0.1:19199")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://127.0.0.1:19199/control/")
    }

    @Test func `dashboard URL skips local base path in remote mode`() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://gateway.example:19199")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://gateway.example:19199/")
    }
    ...
    @Test func `dashboard URL uses fragment token and omits password`() throws {
        ...
        let url = try GatewayEndpointStore.dashboardURL(
            ...
            mode: .local,
            localBasePath: "/control")
        #expect(url.absoluteString == "http://127.0.0.1:19199/control/#token=abc123")
    ...
    @Test func `dashboard URL can use native auth token override`() throws {
        ...
        #expect(url.absoluteString == "http://127.0.0.1:19199/control/#token=device-token")
        #expect(url.query == nil)
    }
    ...
    @Test func `normalize gateway url adds default port for loopback ws`() {
        ...
        #expect(url?.port == 19199)
        #expect(url?.absoluteString == "ws://127.0.0.1:19199")
    }

    @Test func `normalize gateway url accepts private network ws`() {
        let url = GatewayRemoteConfig.normalizeGatewayUrl("ws://192.168.0.202:19199")
        #expect(url?.absoluteString == "ws://192.168.0.202:19199")
    }

    @Test func `normalize gateway url accepts tailnet ws`() {
        let url = GatewayRemoteConfig.normalizeGatewayUrl("ws://100.123.224.76:19199")
        #expect(url?.absoluteString == "ws://100.123.224.76:19199")
    }
    ...
    @Test func `missing transport infers direct from private remote URL`() {
        ...
        #expect(resolution.directURL?.absoluteString == "ws://192.168.0.202:19199")
    }
    ...
    @Test func `normalize gateway url rejects public host ws`() {
        let url = GatewayRemoteConfig.normalizeGatewayUrl("ws://gateway.example:19199")
        #expect(url == nil)
    }

    @Test func `normalize gateway url rejects private ipv4 suffix host bypasses`() {
        #expect(GatewayRemoteConfig.normalizeGatewayUrl("ws://192.168.0.202.attacker.example:19199") == nil)
        #expect(GatewayRemoteConfig.normalizeGatewayUrl("ws://100.123.224.76.attacker.example:19199") == nil)
    }

    @Test func `normalize gateway url rejects ipv6 prefix hostname bypasses`() {
        #expect(GatewayRemoteConfig.normalizeGatewayUrl("ws://fcorp.example:19199") == nil)
        #expect(GatewayRemoteConfig.normalizeGatewayUrl("ws://fd-example.com:19199") == nil)
    }

...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["ACTAGENT_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "actagent-gateway",
                executablePath: "/tmp/actagent-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 actagent-gateway @ /tmp/actagent-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error

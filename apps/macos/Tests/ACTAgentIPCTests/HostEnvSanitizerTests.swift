...
struct HostEnvSanitizerTests {
    ...
    @Test func `sanitize shell wrapper allows only explicit override keys`() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["ACTAGENT_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func `sanitize non shell wrapper keeps regular overrides`() {
        let env = HostEnvSanitizer.sanitize(overrides: ["ACTAGENT_TOKEN": "secret"])
        #expect(env["ACTAGENT_TOKEN"] == "secret")
    }

...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: ACTAgentProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ACTAgentProtocol.AnyCodable] = [
            "presence": .init([ACTAgentProtocol.AnyCodable(entry)]),
        ]
        let payload = ACTAgentProtocol.AnyCodable(payloadMap)

...
@Suite(.serialized)
struct ACTAgentConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("actagent-config-\(UUID().uuidString)")
            .appendingPathComponent("actagent.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["ACTAGENT_CONFIG_PATH": override]) {
            #expect(ACTAgentConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["ACTAGENT_CONFIG_PATH": override]) {
            ...
            #expect(ACTAgentConfigFile.remoteGatewayPort() == 19999)
            #expect(ACTAgentConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(ACTAgentConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(ACTAgentConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(ACTAgentConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(ACTAgentConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["ACTAGENT_CONFIG_PATH": override]) {
            ...
            ACTAgentConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:19199")
            let root = ACTAgentConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
            #expect(url == "ws://127.0.0.1:19199")
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["ACTAGENT_CONFIG_PATH": override]) {
            ...
            ACTAgentConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = ACTAgentConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "ACTAGENT_CONFIG_PATH": nil,
            "ACTAGENT_STATE_DIR": dir,
        ]) {
            #expect(ACTAgentConfigFile.stateDirURL().path == dir)
            #expect(ACTAgentConfigFile.url().path == "\(dir)/actagent.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("actagent-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("actagent.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "ACTAGENT_STATE_DIR": stateDir.path,
            "ACTAGENT_CONFIG_PATH": configPath.path,
        ]) {
            ACTAgentConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-actagent-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = ACTAgentConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try ACTAgentConfigFile.withTestingFileLock {
                ACTAgentConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = ACTAgentConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = ACTAgentConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [

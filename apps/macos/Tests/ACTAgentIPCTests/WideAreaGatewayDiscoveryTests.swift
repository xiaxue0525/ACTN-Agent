...
import Foundation
import Testing
@testable import ACTAgentDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("ACTAGENT_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("ACTAGENT_WIDE_AREA_DOMAIN", "actagent.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("ACTAGENT_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("ACTAGENT_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._actagent-gw._tcp.actagent.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 19199 steipetacstudio.actagent.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (ACTAgent)\" \"gatewayPort=19199\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/actagent/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (ACTAgent)"
        #expect(beacon.displayName == expectedDisplay)
        #expect(beacon.port == 19199)
        #expect(beacon.gatewayPort == 19199)
        #expect(beacon.tailnetDns == "peters-mac-studio-1.sheep-coho.ts.net")
        #expect(beacon.cliPath == "/Users/steipete/actagent/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._actagent-gw._tcp.actagent.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {

...
struct DashboardWindowSmokeTests {
    @Test func `dashboard window controller shows and closes`() throws {
        let url = try #require(URL(string: "http://127.0.0.1:19199/control/#token=device-token"))
        let controller = DashboardWindowController(
            url: url,
            auth: DashboardWindowAuth(
                gatewayUrl: "ws://127.0.0.1:19199/control/",
                token: "device-token",
                password: nil))
        controller.show()
    ...
    @Test func `dashboard navigation stays on same endpoint`() throws {
        let dashboard = try #require(URL(string: "http://127.0.0.1:19199/control/"))
        #expect(DashboardWindowController.shouldAllowNavigation(
            to: try #require(URL(string: "http://127.0.0.1:19199/control/chat")),
            dashboardURL: dashboard))
        #expect(!DashboardWindowController.shouldAllowNavigation(
            to: try #require(URL(string: "https://docs.actagent.ai/")),
            dashboardURL: dashboard))
    }

    @Test func `dashboard origin brackets ipv6 literals`() throws {
        let url = try #require(URL(string: "http://[fd12:3456:789a::1]:19199/control/"))
        #expect(DashboardWindowController.originString(for: url) == "http://[fd12:3456:789a::1]:19199")
    }

    @Test func `dashboard failure state opens in dashboard window`() throws {
        let url = try #require(URL(string: "http://127.0.0.1:19199/control/"))
        let controller = DashboardWindowController(
            url: url,

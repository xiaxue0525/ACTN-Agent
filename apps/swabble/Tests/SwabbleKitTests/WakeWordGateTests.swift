...
final class WakeWordGateTests: XCTestCase {
    func testMatchRequiresGapAfterTrigger() {
        let transcript = "hey actagentd do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["actagentd"], minPostTriggerGap: 0.3)
    ...
    func testMatchHandlesMultiWordTriggers() {
        let transcript = "hey actagentd do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey actagentd"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    func testMatchPrefersMostSpecificTriggerWhenOverlapping() {
        ...
        let config = WakeWordGateConfig(triggers: ["actagentd", "hey actagentd"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
        XCTAssertEqual(match?.trigger, "hey actagentd")
    }
    ...
    func testCommandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "actagentd", start: 0.2, duration: 0.1, range: transcript.range(of: "actagentd")),

...
- Play build: `apps/android/build/release-bundles/actagent-<version>-play-release.aab`
- Third-party build: `apps/android/build/release-bundles/actagent-<version>-third-party-release.aab`
...
Use `adb reverse` so Android `localhost:19199` tunnels to your laptop `localhost:19199`.

Terminal A (gateway):

```bash
pnpm actagent gateway --port 19199 --verbose
```

Terminal B (USB tunnel):

```bash
adb reverse tcp:19199 tcp:19199
```
...
- Canvas web content already supports live reload when loaded from Gateway `__actagent__/canvas/` (see `docs/platforms/android.md`).
...
Current ACTAgent Android implication:
...
2) Android app is connected to that gateway and `actagent nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `ACTAGENT_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__actagent__/`).
...
- `ACTAGENT_ANDROID_GATEWAY_URL=ws://...` (default: from your local ACTAgent config)
- `ACTAGENT_ANDROID_GATEWAY_TOKEN=...`
- `ACTAGENT_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - list pending requests (`actagent devices list`), then approve with the exact ID (`actagent devices approve <requestId>`) and rerun.

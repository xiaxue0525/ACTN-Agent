...
val androidStoreFile = providers.gradleProperty("ACTAGENT_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ACTAGENT_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ACTAGENT_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ACTAGENT_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set ACTAGENT_ANDROID_STORE_FILE, " +
      "ACTAGENT_ANDROID_STORE_PASSWORD, ACTAGENT_ANDROID_KEY_ALIAS, and " +
      "ACTAGENT_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/ACTAgentKit/Sources/ACTAgentKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.actagent.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "actagent-$versionName-$buildType.apk"
          } else {
            "actagent-$versionName-$flavorName-$buildType.apk"
          }

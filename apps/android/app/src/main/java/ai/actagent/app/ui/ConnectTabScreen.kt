    ...
    if (showDiagnostics) {
      Surface(
        ...
        Column(
          ...
          if (pairingRequired) {
            Text(
              "Approve this phone on the gateway. ACTAgent retries automatically while this screen stays open.",
              style = mobileCallout,
              color = mobileTextSecondary,
            )
            CommandBlock("actagent devices list")
            CommandBlock("actagent devices approve <requestId>")
          }
          Text("ACTAgent Android ${actAgentAndroidVersionLabel()}", style = mobileCaption1, color = mobileTextSecondary)
          Button(
            onClick = {
            ...
            Spacer(modifier = Modifier.width(8.dp))
            Text("Copy Report for actagent", style = mobileCallout.copy(fontWeight = FontWeight.Bold))
          }
        }
      }
    }
    ...
    AnimatedVisibility(visible = advancedOpen) {
      Surface(
        ...
        Column(
          ...
          CommandBlock("actagent qr --setup-code-only")
          CommandBlock("actagent qr --json")
          Text(
          ...
          if (inputMode == ConnectInputMode.SetupCode) {
            ...
            OutlinedTextField(
              ...
              placeholder = { Text(if (manualTlsInput) "443" else "19199", style = mobileBody, color = mobileTextTertiary) },
              modifier = Modifier.fillMaxWidth(),
              singleLine = true,

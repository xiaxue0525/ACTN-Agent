...
import ai.actagent.app.SensitiveFeatureConfig
...
import ai.actagent.app.ui.design.ACTAgentErrorState
...
fun OnboardingFlow(
  ...
  actagentdesignTheme {
    ...
    var manualPort by rememberSaveable { mutableStateOf("19199") }
...
private fun WelcomeScreen(
  ...
  ACTAgentScaffold(modifier = modifier, contentPadding = PaddingValues(horizontal = 24.dp, vertical = 18.dp)) {
    Column(
      ...
      Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(18.dp)) {
        Text(
          text = "ACTAGENT",
          style = ACTAgentTheme.type.display.copy(fontSize = 34.sp, lineHeight = 38.sp, fontWeight = FontWeight.Black),
      ...
      Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Surface(onClick = onConnect, color = Color.Transparent, contentColor = ACTAgentTheme.colors.text) {
          Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
            Text(text = "Already have a setup?  ", style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted)
            Text(text = "Sign in", style = ACTAgentTheme.type.body.copy(fontWeight = FontWeight.SemiBold), color = ACTAgentTheme.colors.text)
          }
        }
      }
...
private fun GatewaySetupScreen(
  ...
  ACTAgentScaffold(modifier = modifier, contentPadding = PaddingValues(horizontal = 18.dp, vertical = 16.dp)) {
          ...
          Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Surface(
              ...
              Row(
                ...
                Text(text = "Advanced", style = ACTAgentTheme.type.section, color = ACTAgentTheme.colors.text)
                Icon(
            ...
            if (advancedOpen) {
              ACTAgentTextField(value = setupCode, onValueChange = onSetupCodeChange, placeholder = "Setup code")
              Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                ACTAgentTextField(value = manualHost, onValueChange = onManualHostChange, placeholder = "Host", modifier = Modifier.weight(1f))
                ACTAgentTextField(value = manualPort, onValueChange = onManualPortChange, placeholder = "Port", modifier = Modifier.width(104.dp))
              }
              Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
              ...
              ACTAgentTextField(value = token, onValueChange = onTokenChange, placeholder = "Token optional")
              ACTAgentTextField(value = password, onValueChange = onPasswordChange, placeholder = "Password optional")
            }
          }
        }
      }
      ACTAgentPrimaryButton(text = "Pair with Gateway", icon = Icons.Default.Security, onClick = onPair, modifier = Modifier.fillMaxWidth())
    }
  }
}
    ...
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(18.dp)) {
      ...
      Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Icon(
          ...
          tint =
            when (recoveryState) {
              GatewayRecoveryUiState.Connected -> ACTAgentTheme.colors.success
              GatewayRecoveryUiState.Pairing -> ACTAgentTheme.colors.text
              GatewayRecoveryUiState.Finishing -> ACTAgentTheme.colors.text
              GatewayRecoveryUiState.Failed -> ACTAgentTheme.colors.warning
            },
        )
        Text(text = recoveryState.title, style = ACTAgentTheme.type.display, color = ACTAgentTheme.colors.text)
        Text(
      ...
      ACTAgentPanel {
        Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
          Text(text = "Last gateway", style = ACTAgentTheme.type.caption, color = ACTAgentTheme.colors.textMuted)
          Text(text = serverName?.takeIf { it.isNotBlank() } ?: "Home Gateway", style = ACTAgentTheme.type.section, color = ACTAgentTheme.colors.text)
          Text(text = recoveryGatewayDetail(ready = ready, remoteAddress = remoteAddress, statusText = statusText), style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted)
          ACTAgentStatusPill(
            text =
              when (recoveryState) {
            ...
            status =
              when (recoveryState) {
                GatewayRecoveryUiState.Connected -> ACTAgentStatus.Success
                GatewayRecoveryUiState.Pairing -> ACTAgentStatus.Neutral
                GatewayRecoveryUiState.Finishing -> ACTAgentStatus.Neutral
                GatewayRecoveryUiState.Failed -> ACTAgentStatus.Warning
              },
          )
        }
      }
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Text(
              ...
              style = ACTAgentTheme.type.title.copy(fontSize = 20.sp, lineHeight = 25.sp, fontWeight = FontWeight.Bold),
              color = ACTAgentTheme.colors.text,
            )
            Text(
              text = "These permissions keep ACTAgent secure\nand useful.",
              style = ACTAgentTheme.type.body,
              color = ACTAgentTheme.colors.textMuted,
            )
          }
...
private fun HeroPrimaryAction(
  ...
  Surface(
    ...
    Row(
      ...
      Text(text = title, style = ACTAgentTheme.type.title.copy(fontSize = 14.5.sp, lineHeight = 18.sp), modifier = Modifier.weight(1f), textAlign = TextAlign.Center)
...
private fun OnboardingHeader(
  ...
  Row(modifier = modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
    onBack?.let {
      Surface(onClick = it, modifier = Modifier.size(34.dp), color = Color.Transparent, contentColor = ACTAgentTheme.colors.text) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
      Text(text = title, style = ACTAgentTheme.type.display, color = ACTAgentTheme.colors.text)
      subtitle?.let { Text(text = it, style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted) }
    }
  }
}
...
private fun GatewayOption(
  ...
  ACTAgentPanel(contentPadding = PaddingValues(horizontal = 12.dp, vertical = 8.dp)) {
    ACTAgentListItem(
      title = title,
      subtitle = subtitle,
      metadata = status,
      leading = { Icon(imageVector = icon, contentDescription = null, modifier = Modifier.size(22.dp), tint = ACTAgentTheme.colors.text) },
      trailing = {
        Icon(imageVector = Icons.AutoMirrored.Filled.KeyboardArrowRight, contentDescription = "Open $title", modifier = Modifier.size(20.dp), tint = ACTAgentTheme.colors.text)
      },
      onClick = onClick,
    )
  }
...
private fun OutlinedAction(
  ...
  Surface(
    ...
    Row(
      ...
      Text(text = title, style = ACTAgentTheme.type.section, modifier = Modifier.weight(1f))
    }
  }
}
...
private fun TogglePill(
  ...
  Surface(
    ...
    Text(text = text, modifier = Modifier.padding(horizontal = 12.dp, vertical = 7.dp), style = ACTAgentTheme.type.label)
  }
}
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      containerColor = ACTAgentTheme.colors.surfaceRaised,
      title = { Text("Permissions", style = ACTAgentTheme.type.section, color = ACTAgentTheme.colors.text) },
      text = {
        Text(
          "Choose what this phone can share with ACTAgent. You can change these later in Settings.",
          style = ACTAgentTheme.type.body,
          color = ACTAgentTheme.colors.textMuted,
        )
      },
  ...
  Box(modifier = Modifier.fillMaxWidth().height(38.dp), contentAlignment = Alignment.Center) {
    ...
    Text(
      ...
      style = ACTAgentTheme.type.title.copy(fontSize = 15.2.sp, lineHeight = 19.sp),
      color = ACTAgentTheme.colors.text,
      maxLines = 1,
    )
    Surface(
      ...
      color = Color.Transparent,
      contentColor = ACTAgentTheme.colors.text,
      border = BorderStroke(1.dp, ACTAgentTheme.colors.text),
    ) {
      Box(contentAlignment = Alignment.Center) {
        Text(text = "?", style = ACTAgentTheme.type.label.copy(fontSize = 13.sp, lineHeight = 16.sp), color = ACTAgentTheme.colors.text)
      }
    }
  }
}
...
private fun PermissionRow(row: PermissionRowModel) {
  Surface(
    ...
    Row(
      ...
      Surface(modifier = Modifier.size(30.dp), shape = CircleShape, color = ACTAgentTheme.colors.surfacePressed, border = BorderStroke(1.dp, ACTAgentTheme.colors.border)) {
        Box(contentAlignment = Alignment.Center) {
          Icon(imageVector = row.icon, contentDescription = null, modifier = Modifier.size(17.dp), tint = ACTAgentTheme.colors.text)
        }
      }
      ...
      Text(
        ...
        style = ACTAgentTheme.type.body,
        color = if (row.granted) ACTAgentTheme.colors.success else ACTAgentTheme.colors.textMuted,
        maxLines = 1,
      )
      Icon(
...
private fun PermissionContinueButton(onClick: () -> Unit) {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
      Text(text = "Continue", style = ACTAgentTheme.type.title.copy(fontSize = 18.sp, lineHeight = 23.sp), color = ACTAgentTheme.colors.primaryText)
      Icon(
        imageVector = Icons.AutoMirrored.Filled.KeyboardArrowRight,
        contentDescription = null,
        modifier = Modifier.align(Alignment.CenterEnd).padding(end = 20.dp).size(19.dp),
        tint = ACTAgentTheme.colors.primaryText,
      )
    }
  }
}
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    ...
    message = "Approval is in progress.\nACTAgent will reconnect automatically.",
    canAutoRetry = true,
  ),
  Finishing(
    title = "Finishing Setup",
    message = "Gateway approved this phone.\nACTAgent is bringing the node online.",
    canAutoRetry = true,
  ),
  Failed(
...
private fun recoveryGatewayDetail(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      "Gateway approval is in progress. ACTAgent will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOf(
      "ACTAgent Android gateway diagnostic",
      "Status: $statusText",
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("ACTAgent gateway diagnostic", diagnostic))

...
import ai.actagent.app.GatewayDeviceTokenSummary
import ai.actagent.app.GatewayNodeSummary
import ai.actagent.app.GatewayNodesDevicesSummary
import ai.actagent.app.GatewayPairedDeviceSummary
import ai.actagent.app.GatewayPendingDeviceSummary
...
internal fun NodesDevicesSettingsScreen(
  ...
  SettingsDetailFrame(
    ...
    when {
      !isConnected ->
        ACTAgentPanel {
          Text(text = "Connect the gateway to load nodes and paired devices.", style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted)
        }
      summary.isEmpty() ->
        ACTAgentPanel {
          Column(verticalArrangement = Arrangement.spacedBy(3.dp)) {
            Text(text = "No nodes or paired devices.", style = ACTAgentTheme.type.section, color = ACTAgentTheme.colors.text)
            Text(text = "Linked phones and node hosts will appear here after pairing.", style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted)
          }
        }
...
private fun NodesDevicesPanel(summary: GatewayNodesDevicesSummary) {
  Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
    if (!summary.devicePairingAvailable) {
      ACTAgentPanel {
        Text(text = "Device pairing admin needs elevated access. Connected nodes still work.", style = ACTAgentTheme.type.body, color = ACTAgentTheme.colors.textMuted)
      }
    }
...
private fun NodeRow(node: GatewayNodeSummary) {
  DeviceListRow(
    ...
    status = if (node.connected) ACTAgentStatus.Success else ACTAgentStatus.Warning,
  )
}
...
private fun DeviceListRow(
  ...
  actagentdetailRow(
    title = title,
    subtitle = subtitle,
    leading = { ACTAgentTextBadge(text = badge) },
    trailing = { ACTAgentStatusPill(text = statusText, status = status) },
  )
}
...
private fun nodeSubtitle(node: GatewayNodeSummary): String {
  ...
  val version = node.version?.let { "ACTAgent $it" }
...
private fun pairedDeviceStatus(tokens: List<GatewayDeviceTokenSummary>): ACTAgentStatus =
  when {
    tokens.isEmpty() -> ACTAgentStatus.Neutral
    tokens.any { !it.revoked } -> ACTAgentStatus.Success
    else -> ACTAgentStatus.Warning
  }

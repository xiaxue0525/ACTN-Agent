...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "ACTAgent Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "ACTAgent Node · Talk"
              state.connected -> "ACTAgent Node · Connected"
              else -> "ACTAgent Node"
            }
          val text =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "ACTAgent node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.actagent.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.actagent.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.actagent.app.extra.VOICE_CAPTURE_MODE"

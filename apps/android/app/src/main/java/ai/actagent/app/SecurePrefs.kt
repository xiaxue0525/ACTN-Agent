...
class SecurePrefs(
  ...
  companion object {
    val defaultWakeWords: List<String> = listOf("actagent", "claude")
    private const val displayNameKey = "node.displayName"
    private const val locationModeKey = "location.enabledMode"
    private const val voiceWakeModeKey = "voiceWake.mode"
    private const val plainPrefsName = "actagent.node"
    private const val securePrefsName = "actagent.node.secure"
  ...
  private val _manualPort =
    MutableStateFlow(plainPrefs.getInt("gateway.manual.port", 19199))
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block ACTAgent's own notifications in blocklist mode to prevent forwarding loops.

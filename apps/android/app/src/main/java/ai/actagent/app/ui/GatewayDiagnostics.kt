...
internal fun actAgentAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this ACTAgent Android gateway connection failure.
    ...
    - tell me whether `actagent devices list` should show a pending pairing request
    - if more signal is needed, ask for `actagent qr --json`, `actagent devices list`, and `actagent nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${actAgentAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("ACTAgent gateway diagnostics", report))

...
private const val NOTIFICATION_CHANNEL_BASE_ID = "actagent.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "ACTAgent Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "ACTAgent Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "ACTAgent Active")
      }

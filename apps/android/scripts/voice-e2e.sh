...
PACKAGE_NAME="ai.actagent.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="ai.actagent.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="ai.actagent.app.debug.OPEN_VOICE_E2E"
PORT=19199
HOST="127.0.0.1"
MODE="both"
...
Options:
  ...
  --port PORT                     Gateway port. Default: 19199.
...
ARTIFACT_DIR="/tmp/actagent-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    ACTAGENT_OPENAI_API_KEY="$(bws_get_secret ACTAGENT_OPENAI_API_KEY)"
  else
    ACTAGENT_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret ACTAGENT_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$ACTAGENT_OPENAI_API_KEY" \
      pnpm actagent gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
...
adb logcat -d -v time |
  rg -i 'ACTAgent|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |

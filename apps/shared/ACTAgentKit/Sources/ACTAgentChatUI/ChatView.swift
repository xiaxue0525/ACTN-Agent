...
@MainActor
public struct ACTAgentChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: ACTAgentChatViewModel
    ...
    private let talkControl: ACTAgentChatTalkControl?
    ...
    public init(
        viewModel: ACTAgentChatViewModel,
        ...
        talkControl: ACTAgentChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [ACTAgentChatMessage] {
        let base: [ACTAgentChatMessage]
    ...
    private func mergeToolResults(in messages: [ACTAgentChatMessage]) -> [ACTAgentChatMessage] {
        ...
        for message in messages {
            ...
            let merged = ACTAgentChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: ACTAgentChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: ACTAgentChatMessage) -> Bool {
    ...
    private func primaryText(in message: ACTAgentChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return ACTAgentChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: ACTAgentChatMessage) -> Bool {
    ...
    private func toolCalls(in message: ACTAgentChatMessage) -> [ACTAgentChatMessageContent] {
    ...
    private func inlineToolResults(in message: ACTAgentChatMessage) -> [ACTAgentChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: ACTAgentChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ACTAgentChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(ACTAgentChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(ACTAgentChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)

...
private let chatUILogger = Logger(subsystem: "ai.actagent", category: "ACTAgentChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class ACTAgentChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [ACTAgentChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [ACTAgentChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [ACTAgentChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [ACTAgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ACTAgentChatPendingToolCall] = []
    public private(set) var sessions: [ACTAgentChatSessionEntry] = []
    private let transport: any ACTAgentChatTransport
    private var sessionDefaults: ACTAgentChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: ACTAgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any ACTAgentChatTransport,
    ...
    public var sessionChoices: [ACTAgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [ACTAgentChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [ACTAgentChatThinkingLevelOption] = [
        ACTAgentChatThinkingLevelOption(id: "off", label: "off"),
        ACTAgentChatThinkingLevelOption(id: "minimal", label: "minimal"),
        ACTAgentChatThinkingLevelOption(id: "low", label: "low"),
        ACTAgentChatThinkingLevelOption(id: "medium", label: "medium"),
        ACTAgentChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: ACTAgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ACTAgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ACTAgentChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ACTAgentChatMessage) -> ACTAgentChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ACTAgentChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ACTAgentChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: ACTAgentChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: ACTAgentChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: ACTAgentChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: ACTAgentChatMessage) -> Bool {
        ...
        updated[matchIndex] = ACTAgentChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [ACTAgentChatMessage],
        incoming: [ACTAgentChatMessage]) -> [ACTAgentChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ACTAgentChatMessage]) -> [ACTAgentChatMessage] {
        var result: [ACTAgentChatMessage] = []
    ...
    private static func dedupeKey(for message: ACTAgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ACTAgentChatMessageContent] = [
            ACTAgentChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ACTAgentChatAttachmentPayload in
            ACTAgentChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "ACTAgentChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: ACTAgentChatSessionEntry?) -> [ACTAgentChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: ACTAgentChatSessionEntry,
        defaults: ACTAgentChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [ACTAgentChatThinkingLevelOption]?) -> [ACTAgentChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return ACTAgentChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [ACTAgentChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return ACTAgentChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [ACTAgentChatThinkingLevelOption],
        current: String) -> [ACTAgentChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [ACTAgentChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [ACTAgentChatThinkingLevelOption]) -> [ACTAgentChatThinkingLevelOption]
    {
        var result: [ACTAgentChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> ACTAgentChatSessionEntry {
        ACTAgentChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = ACTAgentChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: ACTAgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: ACTAgentSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: ACTAgentChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: ACTAgentChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = ACTAgentChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: ACTAgentChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: ACTAgentChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: ACTAgentChatMessage) -> ACTAgentChatMessage {
    ...
    private func handleAgentEvent(_ evt: ACTAgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ACTAgentChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: ACTAgentAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: ACTAgentAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }

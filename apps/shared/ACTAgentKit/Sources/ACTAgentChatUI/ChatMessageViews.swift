...
struct ChatMessageBubble: View {
    let message: ACTAgentChatMessage
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ACTAgentChatTheme.userText : ACTAgentChatTheme.assistantText
    ...
    private var inlineAttachments: [ACTAgentChatMessageContent] {
    ...
    private var toolCalls: [ACTAgentChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ACTAgentChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ACTAgentChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ACTAgentChatTheme.onboardingAssistantBubble
        }
        return ACTAgentChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ACTAgentChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ACTAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ACTAgentChatTheme.userText : ACTAgentChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ACTAgentChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ACTAgentChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ACTAgentChatTheme.assistantText)
            }
        }
    }
}

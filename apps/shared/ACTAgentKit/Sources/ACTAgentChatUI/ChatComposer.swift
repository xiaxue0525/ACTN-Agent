...
public struct ACTAgentChatTalkControl {
...
@MainActor
struct ACTAgentChatComposer: View {
    @Bindable var viewModel: ACTAgentChatViewModel
    let style: ACTAgentChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: ACTAgentChatView.ComposerChrome
    let messagePlaceholder: String?
    let talkControl: ACTAgentChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(ACTAgentChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(ACTAgentChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(ACTAgentChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ACTAgentPendingAttachment.id)
                { (att: ACTAgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ACTAgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ACTAgentChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(ACTAgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(ACTAgentChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: ACTAgentChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: ACTAgentChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: ACTAgentChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(ACTAgentChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return ACTAgentChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: ACTAgentChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return ACTAgentChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: ACTAgentChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: ACTAgentChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(ACTAgentChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? ACTAgentChatTheme.userBubble
    }

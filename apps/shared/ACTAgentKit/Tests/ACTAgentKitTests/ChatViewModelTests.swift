...
import ACTAgentKit
import Testing
@testable import ACTAgentChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> ACTAgentChatHistoryPayload
{
    ACTAgentChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> ACTAgentChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> ACTAgentChatThinkingLevelOption {
    ACTAgentChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> ACTAgentChatSessionEntry
{
    ACTAgentChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> ACTAgentChatModelChoice {
    ACTAgentChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [ACTAgentChatHistoryPayload],
    sessionsResponses: [ACTAgentChatSessionsListResponse] = [],
    modelResponses: [[ACTAgentChatModelChoice]] = [],
    ...
    -> (TestChatTransport, ACTAgentChatViewModel)
{
...
private func sendUserMessage(_ vm: ACTAgentChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, ACTAgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ACTAgentChatHistoryPayload]
    private let sessionsResponses: [ACTAgentChatSessionsListResponse]
    private let modelResponses: [[ACTAgentChatModelChoice]]
    ...
    private let stream: AsyncStream<ACTAgentChatTransportEvent>
    private let continuation: AsyncStream<ACTAgentChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<ACTAgentChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> ACTAgentChatCreateSessionResponse
    {
        ...
        return ACTAgentChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ACTAgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ACTAgentChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [ACTAgentChatAttachmentPayload]) async throws -> ACTAgentChatSendResponse
    {
        await self.state.sentSessionKeysAppend(sessionKey)
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return ACTAgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ACTAgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ACTAgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ACTAgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func displaysErrorMessageFallbackOnlyForAssistantErrorTurns() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> ACTAgentChatMessage {
        ...
        #expect(
            ACTAgentChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                ACTAgentSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: ACTAgentChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = ACTAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = ACTAgentChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: ACTAgentChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = ACTAgentChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ACTAgentChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(ACTAgentChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == ACTAgentChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = ACTAgentChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ACTAgentChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = ACTAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = ACTAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = ACTAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func decodesGatewayThinkingMetadataFromSessionList() throws {
        ...
        let decoded = try JSONDecoder().decode(
            ACTAgentChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        ...
        let vm = await MainActor.run { ACTAgentChatViewModel(sessionKey: "main", transport: transport) }

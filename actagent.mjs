...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `actagent: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[actagent] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.ACTAGENT_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.ACTAGENT_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    ACTAGENT_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["actagent: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:actagent/actagent#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g actagent@latest`.");
  return lines.join("\n");
};
...
const isHelpFastPathDisabled = () =>
  process.env.ACTAGENT_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.ACTAGENT_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.ACTAGENT_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.ACTAGENT_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "actagent.json"), path.join(stateDir, "actagentdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".actagent", "actagent.json"),
    path.join(homeDir, ".actagent", "actagentdbot.json"),
    path.join(homeDir, ".actagentdbot", "actagent.json"),
    path.join(homeDir, ".actagentdbot", "actagentdbot.json"),
  ];
};

const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.ACTAGENT_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.ACTAGENT_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.ACTAGENT_CONTAINER)) {
      return false;
    }
    ...
    process.stdout.write(commit ? `ACTAgent ${version} (${commit})\n` : `ACTAgent ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.ACTAGENT_BUNDLED_VERSION) ?? "0.0.0";
}

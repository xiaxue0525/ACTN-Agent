...
- Repo: `https://github.com/actagent/actagent`
...
- Dependency-touching work: direct dependency inspection is mandatory when feasible; do not rely on assumptions, wrappers, or memory. Most dependencies are OSS, so read their source/docs/types. Codex-related work has a hard gate: the acting agent must personally inspect sibling `../codex` source for the exact protocol/runtime behavior before any verdict, comment, approval, merge recommendation, code change, or `proof sufficient` claim. If missing, clone `https://github.com/openai/codex.git` there first. Subagent reports, PR text, ACTAgent wrappers, generated schemas, memory, and prior bot reviews do not satisfy this gate. No direct `../codex` check means no Codex verdict. Cite Codex files/lines checked in final/review/comment.
...
## ACTAgentSweeper Review Policy

- ACTAgent-specific review rules live here; generic ACTAgentSweeper prompts stay repo-agnostic.
- ACTAgentSweeper-owned schema, labels, close reasons, protected-label gates, maintainer-item gates, and mutation rules live in `actagent/actagentsweeper`.
...
- Optional integrations, providers, channels, skill bundles, MCP surfaces, and service workflows route to plugins, ACTAgentHub, or owner repos when current seams suffice. Keep core items for missing core/plugin APIs, bundled regressions, security/core hardening, or maintainer product decisions.
...
- Public ACTAgentSweeper comments prefer `https://docs.actagent.ai/...` when a public docs page exists; structured evidence still cites repo files, lines, SHAs.
...
- State/storage migrations are database-first. Runtime reads/writes the canonical store only. Old file stores, sidecars, aliases, and fallback readers belong in `actagent doctor --fix` migration code only, never steady-state runtime.
- Storage default: SQLite only. Do not add JSON/JSONL/TXT/sidecar files for ACTAgent-owned runtime state, caches, queues, registries, indexes, cursors, checkpoints, or plugin scratch data.
...
- Use the shared state DB (`state/actagent.sqlite`) for global runtime state and plugin KV data. Use the per-agent DB (`agents/<agentId>/agent/actagent-agent.sqlite`) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- CLI: `pnpm actagent ...` or `pnpm dev`; build: `pnpm build`.
...
- Checks in a Codex worktree or linked/sparse checkout: avoid direct local `pnpm check*`; use `node scripts/crabbox-wrapper.mjs run ... -- env ACTAGENT_CHECK_CHANGED_REMOTE_CHILD=1 ACTAGENT_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed` so pnpm runs inside Testbox, not locally.
...
- Use `$actagent-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Pre-land/pre-commit code changes: mandatory fresh `$autoreview` until no accepted/actionable findings remain. Do not land code on CI, ACTAgentSweeper, prior review comments, or your own manual review alone unless user explicitly opts out or scope is truly trivial/docs-only. If findings want refactor, refactor; no ugly fixes.
...
- Use `$actagent-pr-maintainer` immediately for maintainer-side ACTAgent issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. External PR creation/refresh follows the requested workflow; linked refs alone do not require maintainer archive tooling.
- Issue/PR start: `git status -sb`; if clean, `git pull --ff-only`; if dirty, yell before pull/rebase.
- PR refs: `gh pr view/diff` or `gh api`, not web search. Prefer `gitcrawl` for maintainer discovery; missing/stale `gitcrawl` falls through to live `gh`, not external setup. Verify live with `gh` before mutation.
...
- PR artifacts/screenshots: attach to PR/comment/external artifact store. Never push screenshots, videos, proof images, or proof assets to ACTAgent or any product repo branch, including temp artifact branches. Use Crabbox artifact publishing plus the manifest URL. Do not commit `.github/pr-assets`.
- CI polling: exact SHA, relevant checks only, minimal fields. Skip routine noise (`Auto response`, `Labeler`, docs agents, performance/stale). Logs only after failure/completion or concrete need.
- ACTAgent write-access maintainers may skip `Real behavior proof` when local tests or Crabbox verified behavior; record proof in PR verification.
...
- Naming: **ACTAgent** product/docs; `actagent` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `actagent/plugin-sdk/*`.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run independent `pnpm test`/Vitest commands concurrently in one worktree; Vitest cache races with `ENOTEMPTY`. Group one command or use distinct `ACTAGENT_VITEST_FS_MODULE_CACHE_PATH`.
- Test workers max 16. Memory pressure: `ACTAGENT_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `ACTAGENT_LIVE_TEST=1 pnpm test:live`; verbose `ACTAGENT_LIVE_TEST_QUIET=0`.
...
- Docs final answers: include relevant full `https://docs.actagent.ai/...` URL(s). If issue/PR work too, GitHub URL last.
- `CHANGELOG.md`: release-only. Do not edit for normal PRs, direct `main` fixes, or `ship it`; release generation owns it. Do not ask agents for changelog edits.
...
- Release generation: derive `CHANGELOG.md` from merged PRs + all direct `main` commits. Entries: active `### Changes`/`### Fixes`, single-line, thank credited humans; never thank bots/forbidden handles: `@actagent`, `@actagentsweeper`, `@codex`, `@admin1douyin`.
...
- Secrets: channel/provider creds in `~/.actagent/credentials/`; model auth profiles in `~/.actagent/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Use `$release-actagent-maintainer`.
- Backport means apply to newest open `release/` branch unless user names another target.
- GHSA/advisories: `$actagent-ghsa-maintainer` / `$security-triage`. Secret scanning: `$actagent-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `actagent gateway restart/status --deep`; logs = `./scripts/actagentlog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$release-actagent-maintainer`.
- Parallels: `$actagent-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- ACTAgentSweeper ops: `$actagentsweeper`. Deployed hook sessions may post one concise `#actagentsweeper` note only when surprising/actionable/risky; if using message tool, reply exactly `NO_REPLY`.
...
- Rebrand/migration/config warnings: run `actagent doctor`.

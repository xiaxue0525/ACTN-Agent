// ACTAgent Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "actagent-control-";
const EMBEDDED_CACHE_VERSION = "__ACTAGENT_CONTROL_UI_BUILD_ID__";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__ACTAGENT_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "ACTAgent", body: event.data.text() };
  }

  const title = data.title || "ACTAgent";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "actagent-notification",
    data: { url: data.url || "./" },
  };

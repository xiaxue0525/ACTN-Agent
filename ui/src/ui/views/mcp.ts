...
function renderServerRow(props: McpViewProps, server: McpServerRow) {
  ...
  const probeCommand = `actagent mcp probe ${quotedName}`;
  const loginCommand = `actagent mcp login ${quotedName}`;
  return html`
    ...
    <section class="mcp-page">
      ...
      <section class="card mcp-command-card">
        ...
        <div class="mcp-command-card__grid">
          <code>actagent mcp status --verbose</code>
          <code>actagent mcp doctor --probe</code>
          <code>actagent mcp login &lt;name&gt;</code>
          <code>actagent mcp reload</code>
        </div>
      </section>

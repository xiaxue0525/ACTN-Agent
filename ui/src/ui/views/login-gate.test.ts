...
describe("resolveLoginFailureFeedback", () => {
  ...
  it("explains missing auth credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Paste the token from actagent dashboard --no-open or enter the configured password.",
      "If no token is configured, run actagent doctor --generate-gateway-token on the gateway host.",
  ...
  it("explains rejected stale credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run actagent dashboard --no-open and open the fresh URL or paste its token.",
  ...
  it("preserves pairing request ids in the approval command", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run actagent devices list on the Gateway host.",
      "Approve this request: actagent devices approve req-123.",
  ...
  it("explains insecure HTTP device identity failures", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Use HTTPS/Tailscale Serve, or open http://127.0.0.1:19199 on the Gateway host.",
  ...
  it("explains browser WebSocket security failures as insecure context", () => {
    const feedback = resolveLoginFailureFeedback({
      connected: false,
      lastError:
        "Browser refused the Gateway WebSocket for security reasons. Use wss:// when the Control UI is served over HTTPS/Tailscale Serve, or open the loopback dashboard at http://127.0.0.1:19199.",
  ...
  it("keeps generic browser WebSocket constructor failures on the network path", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Confirm the Gateway is running with actagent status or actagent gateway run.",
      "Check the WebSocket URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
      "Reopen the dashboard with actagent dashboard --no-open to recopy the current URL and auth details.",
    ]);
  });
  ...
  it("explains protocol mismatch without requiring a gateway protocol change", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Reopen the served dashboard with actagent dashboard so the UI and Gateway come from the same install.",
      "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
      "Restart the Gateway after updating ACTAgent so it serves the current protocol.",
    ]);
  });
  ...
  it("redacts credential-shaped values from displayed raw errors", () => {
    const feedback = resolveLoginFailureFeedback({
      connected: false,
      lastError:
        "failed ws://host/actagent#token=secret-token Authorization: Bearer secret-bearer token=inline-secret",
      lastErrorCode: null,
      hasToken: false,
      hasPassword: false,
    });
    ...
    expect(feedback?.rawError).toBe(
      "failed ws://host/actagent#[redacted-credential] Authorization: Bearer [redacted] token=[redacted]",
    );
  });
});
...
describe("renderLoginGate", () => {
  ...
  it("renders an accessible structured failure panel with raw error details", async () => {
    ...
    expect(docsLink?.getAttribute("href")).toBe(
      "https://docs.actagent.ai/web/control-ui#debuggingtesting-dev-server--remote-gateway",
    );
  });
});

...
function createProps(overrides: Partial<SkillsProps> = {}): SkillsProps {
  ...
  return {
    ...
    skillCardErrors: {},
    actagenthubQuery: "",
    actagenthubResults: null,
    actagenthubSearchLoading: false,
    actagenthubSearchError: null,
    ...
    onACTAgentHubQueryChange: () => undefined,
    onACTAgentHubDetailOpen: () => undefined,
    onACTAgentHubDetailClose: () => undefined,
    onACTAgentHubInstall: () => undefined,
    ...overrides,
  };
}
...
describe("renderSkills", () => {
  ...
  it("opens detail dialogs and routes ACTAgentHub actions", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    ...
    const onACTAgentHubDetailOpen = vi.fn();
    const onACTAgentHubInstall = vi.fn();
    ...
    render(
      renderSkills(
        createProps({
          actagenthubQuery: "git",
          actagenthubResults: [
            {
              score: 0.95,
              slug: "github",
              displayName: "GitHub",
              summary: "GitHub integration for ACTAgent",
              version: "1.2.3",
            },
          ],
          onACTAgentHubDetailOpen,
          onACTAgentHubInstall,
        }),
      ),
      container,
    );
    ...
    expect(resultItem?.querySelector(".list-sub")?.textContent?.trim()).toBe(
      "GitHub integration for ACTAgent",
    );
    ...
    expect(onACTAgentHubDetailOpen).toHaveBeenCalledTimes(1);
    expect(onACTAgentHubDetailOpen).toHaveBeenCalledWith("github");
    expect(onACTAgentHubInstall).toHaveBeenCalledTimes(1);
    expect(onACTAgentHubInstall).toHaveBeenCalledWith("github");

    onACTAgentHubInstall.mockClear();
    showModal.mockClear();

    render(
      renderSkills(
        createProps({
          actagenthubSearchError: "rate limited",
          actagenthubInstallMessage: { kind: "success", text: "Installed github" },
          actagenthubDetailSlug: "github",
          actagenthubDetail: {
            skill: {
              slug: "github",
              displayName: "GitHub",
    ...
    expect(normalizeText(container.querySelector(".md-preview-dialog__body")!)).toBe(
      "GitHub integration for ACTAgent By ACTAgent (@actagent) Latest: v1.2.3 Added search support Platforms: macos, linux Install GitHub",
    );
  ...
  it("renders installed ACTAgentHub verdicts and the local Skill Card tab", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    ...
    const verdictKey = "https://actagenthub.ai\u0000agentreceipt\u00001.2.3";
        ...
        createProps({
          ...
          actagenthubVerdicts: {
            [verdictKey]: {
              ...
              version: "1.2.3",
              securityAuditUrl:
                "https://actagenthub.ai/actagent/agentreceipt/security-audit?version=1.2.3",
  ...
  it("fails closed for inconsistent ACTAgentHub verdict envelopes", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    installDialogMethod("showModal", function (this: HTMLDialogElement) {

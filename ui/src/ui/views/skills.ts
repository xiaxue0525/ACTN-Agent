...
import type {
  ACTAgentHubSkillSecurityVerdict,
  ACTAgentHubSearchResult,
  ACTAgentHubSkillDetail,
  SkillMessageMap,
} from "../controllers/skills.ts";
import { actagenthubVerdictKey } from "../controllers/skills.ts";
...
export type SkillsProps = {
  ...
  actagenthubQuery: string;
  actagenthubResults: ACTAgentHubSearchResult[] | null;
  ...
  onACTAgentHubQueryChange: (query: string) => void;
  onACTAgentHubDetailOpen: (slug: string) => void;
  onACTAgentHubDetailClose: () => void;
  onACTAgentHubInstall: (slug: string) => void;
};
...
function verdictForSkill(skill: SkillStatusEntry, verdicts: SkillsProps["actagenthubVerdicts"]) {
  const link = skill.actagenthub;
...
function verdictLabel(verdict: ACTAgentHubSkillSecurityVerdict | null | undefined): string {
  if (!verdict) {
    return "Unavailable";
  }
...
function verdictChipClass(verdict: ACTAgentHubSkillSecurityVerdict | null | undefined): string {
  if (!verdict) {
    return "chip-warn";
  }
  if (verdict.ok && verdict.decision === "pass") {
...
export function renderSkills(props: SkillsProps) {
  ...
  return html`
    <section class="card">
      ...
      <div style="margin-top: 16px; border-top: 1px solid var(--border); padding-top: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
          <div style="font-weight: 600;">ACTAgentHub</div>
        ...
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          <label class="field" style="flex: 1; min-width: 180px;">
            <input
              .value=${props.actagenthubQuery}
              @input=${(e: Event) =>
                props.onACTAgentHubQueryChange((e.target as HTMLInputElement).value)}
              placeholder="Search ACTAgentHub skills…"
              autocomplete="off"
              name="actagenthub-search"
            />
          </label>
          ${props.actagenthubSearchLoading ? html`<span class="muted">Searching…</span>` : nothing}
        </div>
        ${props.actagenthubSearchError
          ? html`<div class="callout danger" style="margin-top: 8px;">
              ${props.actagenthubSearchError}
            </div>`
          : nothing}
        ${props.actagenthubInstallMessage
          ? html`<div
              class="callout ${props.actagenthubInstallMessage.kind === "error" ? "danger" : "success"}"
              style="margin-top: 8px;"
            >
              ${props.actagenthubInstallMessage.text}
            </div>`
          : nothing}
        ${renderACTAgentHubResults(props)}
      </div>
    ...
    ${props.actagenthubDetailSlug ? renderACTAgentHubDetailDialog(props) : nothing}
  `;
}

function renderACTAgentHubResults(props: SkillsProps) {
  const results = props.actagenthubResults;
  if (!results) {
    return nothing;
  }
  if (results.length === 0) {
    return html`<div class="muted" style="margin-top: 8px;">No skills found on ACTAgentHub.</div>`;
  }
  return html`
      ...
      ${results.map(
        (r) => html`
          <div
            ...
            @click=${() => props.onACTAgentHubDetailOpen(r.slug)}
          >
            <div class="list-main">
            ...
            <div class="list-meta" style="display: flex; align-items: center; gap: 8px;">
              ...
              <button
                class="btn btn--sm"
                ?disabled=${props.actagenthubInstallSlug !== null}
                @click=${(e: Event) => {
                  e.stopPropagation();
                  props.onACTAgentHubInstall(r.slug);
                }}
              >
                ${props.actagenthubInstallSlug === r.slug ? "Installing\u2026" : "Install"}
              </button>
            </div>
          </div>
        `,
...
function renderACTAgentHubDetailDialog(props: SkillsProps) {
  const detail = props.actagenthubDetail;
  ...
  return html`
    <dialog
      ...
      @close=${props.onACTAgentHubDetailClose}
    >
      <div class="md-preview-dialog__panel">
        <div class="md-preview-dialog__header">
          <div class="md-preview-dialog__title">
            ${detail?.skill?.displayName ?? props.actagenthubDetailSlug}
          </div>
          <button
            class="btn btn--sm"
            @click=${(e: Event) => {
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ${props.actagenthubDetailLoading
            ? html`<div class="muted">${t("common.loading")}</div>`
            : props.actagenthubDetailError
              ? html`<div class="callout danger">${props.actagenthubDetailError}</div>`
              : detail?.skill
                ? html`
                    ...
                    <button
                      ...
                      ?disabled=${props.actagenthubInstallSlug !== null}
                      @click=${() => {
                        if (props.actagenthubDetailSlug) {
                          props.onACTAgentHubInstall(props.actagenthubDetailSlug);
                        }
                      }}
                    >
                      ${props.actagenthubInstallSlug === props.actagenthubDetailSlug
                        ? "Installing\u2026"
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const verdict = verdictForSkill(skill, props.actagenthubVerdicts);
  ...
  return html`
    <div class="list-item list-item-clickable" @click=${() => props.onDetailOpen(skill.skillKey)}>
      ...
      >
        ${skill.actagenthub?.status === "linked"
          ? html`<span class="chip ${verdictChipClass(verdict)}">${verdictLabel(verdict)}</span>`
          : skill.actagenthub?.status === "invalid"
            ? html`<span class="chip chip-warn">ACTAgentHub link invalid</span>`
            : nothing}
...
function renderSkillDetail(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const showBundledBadge = Boolean(skill.bundled && skill.source !== "actagent-bundled");
    ...
    >
      <div class="md-preview-dialog__panel">
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ...
          ${skill.actagenthub || skill.skillCard?.present
            ? html`
            ...
            : nothing}
          ${detailTab === "overview"
            ? renderInstalledACTAgentHubOverview(skill, props, verdict)
...
function renderInstalledACTAgentHubOverview(
  skill: SkillStatusEntry,
  props: SkillsProps,
  verdict: ACTAgentHubSkillSecurityVerdict | null,
) {
  const link = skill.actagenthub;
  if (!link) {
    return nothing;
  ...
  if (link.status === "invalid") {
    return html`<div class="callout danger">
      <div style="font-weight: 600; margin-bottom: 4px;">ACTAgentHub link invalid</div>
      <div>${link.reason}</div>
    </div>`;
  }
  ...
  return html`
    ...
    >
      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        ...
        ${props.actagenthubVerdictsLoading ? html`<span class="muted">Refreshing…</span>` : nothing}
      </div>
      ${props.actagenthubVerdictsError
        ? html`<div class="muted" style="font-size: 13px;">${props.actagenthubVerdictsError}</div>`
        : reasonText

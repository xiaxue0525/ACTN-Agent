...
import type { ACTAgentFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  ...
  const modal = container.querySelector<ACTAgentFilePreviewModal>("actagent-file-preview-modal");
...
function shadowText(modal: ACTAgentFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("actagent-file-preview-modal", () => {
  beforeEach(() => {

...
import type { ACTAgentModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: ACTAgentModalDialog, id: string): HTMLElement {
...
describe("actagent-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("actagent-modal-dialog-label");
    expect(descriptionId).toBe("actagent-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "actagent-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "actagent-modal-dialog-description").textContent).toBe(

...
const SETTINGS_KEY_PREFIX = "actagent.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "actagent.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "actagent.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "actagent.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "actagent.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "actagent.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__ACTAGENT_CONTROL_UI_BASE_PATH__"]);
  ...
  const effectiveUrl = `${proto}://${formatHostWithPort(location.hostname, "19199")}`;
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const settings = {
      ...
      theme: theme === "custom" && !customTheme ? "actagent" : theme,
      themeMode: mode,
      chatShowThinking:
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("actagent.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;

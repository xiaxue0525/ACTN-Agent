...
export type ThemeName = "actagent" | "knot" | "dash" | "custom";
...
export const VALID_THEME_NAMES = new Set<ThemeName>(["actagent", "knot", "dash", "custom"]);
const VALID_THEME_MODES = new Set<ThemeMode>(["system", "light", "dark"]);

type ThemeSelection = { theme: ThemeName; mode: ThemeMode };

const LEGACY_MAP: Record<string, ThemeSelection> = {
  defaultTheme: { theme: "actagent", mode: "dark" },
  docsTheme: { theme: "actagent", mode: "light" },
  lightTheme: { theme: "knot", mode: "dark" },
  landingTheme: { theme: "knot", mode: "dark" },
  newTheme: { theme: "knot", mode: "dark" },
  dark: { theme: "actagent", mode: "dark" },
  light: { theme: "actagent", mode: "light" },
  openknot: { theme: "knot", mode: "dark" },
  fieldmanual: { theme: "dash", mode: "dark" },
  actagentdash: { theme: "dash", mode: "light" },
  system: { theme: "actagent", mode: "system" },
};
...
export function parseThemeSelection(
  ...
  const normalizedTheme = VALID_THEME_NAMES.has(theme as ThemeName)
    ? (theme as ThemeName)
    : (LEGACY_MAP[theme]?.theme ?? "actagent");

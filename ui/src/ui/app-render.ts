...
import {
  closeACTAgentHubDetail,
  installFromACTAgentHub,
  loadSkillCard,
  installSkill,
...
const SKILL_WORKSHOP_MODE_KEY = "actagent:control-ui:skill-workshop-mode:v1";
const SKILL_WORKSHOP_CURRENT_CHAT_REVISIONS_KEY =
  "actagent:control-ui:skill-workshop-current-chat-revisions:v1";
...
let actagenthubSearchTimer: ReturnType<typeof setTimeout> | null = null;

const UPDATE_BANNER_DISMISS_KEY = "actagent:control-ui:update-banner-dismissed:v1";
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">ACTAgent</span>
                      </span>
                    `}
              </div>
              <button
        ...
        ${state.tab === "skills"
          ? renderLazyView(lazySkills, (m) =>
              m.renderSkills({
                ...
                actagenthubVerdicts: state.actagenthubVerdicts,
                actagenthubVerdictsLoading: state.actagenthubVerdictsLoading,
                actagenthubVerdictsError: state.actagenthubVerdictsError,
                ...
                actagenthubQuery: state.actagenthubSearchQuery,
                actagenthubResults: state.actagenthubSearchResults,
                actagenthubSearchLoading: state.actagenthubSearchLoading,
                actagenthubSearchError: state.actagenthubSearchError,
                actagenthubDetail: state.actagenthubDetail,
                actagenthubDetailSlug: state.actagenthubDetailSlug,
                actagenthubDetailLoading: state.actagenthubDetailLoading,
                actagenthubDetailError: state.actagenthubDetailError,
                actagenthubInstallSlug: state.actagenthubInstallSlug,
                actagenthubInstallMessage: state.actagenthubInstallMessage,
                ...
                onACTAgentHubQueryChange: (query) => {
                  setACTAgentHubSearchQuery(state, query);
                  if (actagenthubSearchTimer) {
                    clearTimeout(actagenthubSearchTimer);
                  }
                  actagenthubSearchTimer = setTimeout(() => {
                    void searchACTAgentHub(state, query);
                  }, 300);
                },
                onACTAgentHubDetailOpen: (slug) => void loadACTAgentHubDetail(state, slug),
                onACTAgentHubDetailClose: () => closeACTAgentHubDetail(state),
                onACTAgentHubInstall: (slug) => void installFromACTAgentHub(state, slug),
              }),
            )
          : nothing}
        ${state.tab === "skillWorkshop"

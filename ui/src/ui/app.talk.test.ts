...
describe("ACTAgentApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { ACTAgentApp } = await import("./app.ts");
    const app = Object.create(ACTAgentApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await ACTAgentApp.prototype.toggleRealtimeTalk.call(app as never);
  ...
  it("keeps the Talk options toggle inside the open-panel click guard", async () => {
    await import("./app.ts");
    const app = document.createElement("actagent-app");

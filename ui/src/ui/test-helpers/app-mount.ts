...
import "../app.ts";
import type { ACTAgentApp } from "../app.ts";
...
const mountedApps = new Set<ACTAgentApp>();

function collectMountedApps() {
  return new Set<ACTAgentApp>([
    ...mountedApps,
    ...document.querySelectorAll<ACTAgentApp>("actagent-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<ACTAgentApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<ACTAgentApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("actagent-app") as ACTAgentApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__ACTAGENT_CONTROL_UI_BASE_PATH__"] = undefined;

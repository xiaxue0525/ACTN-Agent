...
declare global {
  interface Window {
    __ACTAGENT_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class ACTAgentApp extends LitElement {
  ...
  @state() theme: ThemeName = this.settings.theme ?? "actagent";
  ...
  @state() actagenthubSearchQuery = "";
  @state() actagenthubSearchResults: ACTAgentHubSearchResult[] | null = null;
  @state() actagenthubSearchLoading = false;
  @state() actagenthubSearchError: string | null = null;
  @state() actagenthubDetail: ACTAgentHubSkillDetail | null = null;
  @state() actagenthubDetailSlug: string | null = null;
  @state() actagenthubDetailLoading = false;
  @state() actagenthubDetailError: string | null = null;
  @state() actagenthubInstallSlug: string | null = null;
  @state() actagenthubInstallMessage: { kind: "success" | "error"; text: string } | null = null;
  @state() actagenthubVerdicts: Record<string, ACTAgentHubSkillSecurityVerdict> = {};
  @state() actagenthubVerdictsLoading = false;
  @state() actagenthubVerdictsError: string | null = null;
  ...
  clearCustomTheme() {
    const nextTheme = this.theme === "custom" ? "actagent" : this.theme;
    this.customThemeImportExpanded = true;
    this.customThemeImportSelectOnSuccess = false;
    applySettingsInternal(this as unknown as Parameters<typeof applySettingsInternal>[0], {
      ...this.settings,
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "ACTAgent"}: ${entry.text}`;
...
if (!customElements.get("actagent-app")) {
  customElements.define("actagent-app", ACTAgentApp);
}

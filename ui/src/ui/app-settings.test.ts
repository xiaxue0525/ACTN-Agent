...
const createHost = (tab: Tab): SettingsHost => ({
  ...
  theme: "actagent" as unknown as ThemeName & ThemeMode,
  themeMode: "system",
  themeResolved: "dark",
...
describe("setTabFromRoute", () => {
  ...
  it("falls back to actagent when custom is selected without a stored custom theme", () => {
    const host = createHost("chat");
    ...
    expect(host.theme).toBe("actagent");
    expect(host.settings.theme).toBe("actagent");
...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/actagent";

    applySettingsFromUrl(host);

    expect(host.settings.token).toBe("abc123");
    expect(window.location.search).toBe("");
    expect(JSON.parse(localStorage.getItem("actagent.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  });
  ...
  it("hydrates native Mac app auth before the first connection", () => {
    ...
    (
      window as unknown as {
        __ACTAGENT_NATIVE_CONTROL_AUTH__?: {
          gatewayUrl?: string;
          token?: string;
          password?: string;
        };
    ...
    expect(
      (
        window as unknown as {
          __ACTAGENT_NATIVE_CONTROL_AUTH__?: unknown;
        }
      )["__ACTAGENT_NATIVE_CONTROL_AUTH__"],
    ).toBeUndefined();
  });
    ...
    const scenarios = [
      {
        ...
        settingsGatewayUrl: "ws://gateway-a.example:19199",
        settingsToken: "",
      ...
      },
      {
        ...
        url: "https://control.example/chat?gatewayUrl=ws%3A%2F%2Fgateway-b.example%3A19199&session=agent%3Atest_new%3Amain#token=token-b",
        settingsGatewayUrl: "ws://gateway-a.example:19199",
        settingsToken: "",
        expectedToken: "",
        expectedSession: "agent:test_new:main",
        expectedPendingGatewayUrl: "ws://gateway-b.example:19199",
        expectedPendingGatewayToken: "token-b",
        expectedSearch: "?session=agent%3Atest_new%3Amain",
      },
      {
        name: "different gateway defers token without changing session",
        url: "https://control.example/chat?gatewayUrl=ws%3A%2F%2Fgateway-b.example%3A19199#token=token-c",
        settingsGatewayUrl: "ws://gateway-a.example:19199",
        settingsToken: "",
        expectedToken: "",
      ...
      },
      {
        ...
        url: "https://control.example/chat?gatewayUrl=ws%3A%2F%2Fgateway-b.example%3A19199&session=agent%3Atest_new%3Amain",
        settingsGatewayUrl: "ws://gateway-a.example:19199",

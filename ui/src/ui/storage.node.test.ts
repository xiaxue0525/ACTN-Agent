  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __ACTAGENT_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window["__ACTAGENT_CONTROL_UI_BASE_PATH__"];
    return;
  }
  Object.defineProperty(window, "__ACTAGENT_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /actagent/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/actagent"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      protocol: "http:",
      host: "gateway.example:19199",
      pathname: "/apps/actagent/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/actagent"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("actagent.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "actagent.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/actagent",
    ...
    const settings = loadSettings();
    expect(settings.gatewayUrl).toBe("wss://gateway.example:8443/actagent");
    expect(settings.token).toBe("");
    expect(settings.sessionKey).toBe("agent");
    const scopedKey = "actagent.control.settings.v1:wss://gateway.example:8443/actagent";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      textScale: 100,
      sessionsByGateway: {
        "wss://gateway.example:8443/actagent": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `actagent.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "actagent",
      themeMode: "system",
  ...
  it("normalizes persisted text scale to the nearest supported stop", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `actagent.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        textScale: 123,
      }),
  ...
  it("falls back to actagent when persisted custom theme data is invalid", () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/",
    ...
    const settings = loadSettings();
    expect(settings.theme).toBe("actagent");
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `actagent.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", () => {
    ...
    expect(JSON.parse(localStorage.getItem("actagent.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🐲",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", () => {
    ...
    expect(localStorage.getItem("actagent.control.user.v1")).toBeNull();
  });
});

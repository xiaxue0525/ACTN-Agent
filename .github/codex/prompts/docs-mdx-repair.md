# ACTAgent Docs MDX Repair Agent

You are repairing generated ACTAgent documentation after a fast MDX validation failure.
...
1. Read `.actagent-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".actagent-sync/mdx/${LOCALE}.json"`.

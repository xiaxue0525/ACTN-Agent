...
Explicit copy flows, such as `actagent agents add`, use this portability policy:
...
has such a marker, `actagent doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but ACTAgent cannot resolve a probeable model

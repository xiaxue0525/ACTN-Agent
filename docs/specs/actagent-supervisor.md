---
title: ACTAgent Supervisor
description: Fleet supervision plan for Codex app-server sessions controlled by ACTAgent.
readWhen:
  - Designing Codex fleet supervision
  - Building ACTAgent tools that read, steer, or spawn Codex sessions
...
ACTAgent Supervisor lets one always-on ACTAgent instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into ACTAgent through MCP.

## Product Model

Codex remains the primary work surface. ACTAgent supervises Codex rather than hiding Codex inside an opaque ACTAgent subagent.

The ACTAgent plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor actagent: an always-on ACTAgent agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

ACTAgent may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- MCP server for Codex-to-actagent calls.
- ACTAgent tools for actagent-to-Codex control.
...
Every supervised Codex gets an MCP server named `actagent-codex-supervisor`.
...
- `actagent_report_progress`: publish current task state to the supervisor.
- `actagent_ask`: ask the supervisor for help or delegation.
...
## actagent Control Surface

The always-on actagent gets the same primitives as internal tools:
...
6. Supervisor actagent can read and steer immediately.
...
- Supervisor state lives in ACTAgent Gateway storage.
...
- Transcript reads redact secrets at ACTAgent boundary.
...
- Durable state owner: ACTAgent Gateway DB, Cloudflare Durable Object, or VPS database.
- Approval policy granularity for supervisor-originated turns.
- How much transcript summary should be injected into the always-on actagent context versus kept as a tool/resource.

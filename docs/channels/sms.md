...
read_when:
  - You want to connect ACTAgent to SMS through Twilio
  - You need SMS webhook or allowlist setup
title: "SMS"
---

ACTAgent can receive and send SMS through a Twilio phone number or Messaging Service. The Gateway registers an inbound webhook route, validates Twilio request signatures by default, and sends replies back through Twilio's Messages API.
...
- A public HTTPS URL that reaches your ACTAgent Gateway.
...
```bash
actagent config patch --file ./sms.patch.json5 --dry-run
actagent config patch --file ./sms.patch.json5
```
...
```bash
actagent pairing list sms
actagent pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef. Use this when the Gateway should resolve the Twilio Auth Token from the ACTAgent secrets runtime instead of storing plaintext config:
...
```bash
actagent message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
actagent message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. ACTAgent strips markdown, flattens fenced code blocks, preserves readable links, and chunks long replies before sending them through Twilio.
...
```bash
actagent channels capabilities --channel sms
actagent channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `actagent pairing list sms`.
5. Approve the pairing code with `actagent pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
actagent message send --channel sms --target sms:+15557654321 --message "ACTAgent SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "ACTAgent SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
actagent pairing list sms
actagent pairing approve sms <CODE>
...
By default, ACTAgent validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string.
...
### Twilio returns 403 or ACTAgent rejects the webhook

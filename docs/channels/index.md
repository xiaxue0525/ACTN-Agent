---
summary: "Messaging platforms ACTAgent can connect to"
read_when:
  - You want to choose a chat channel for ACTAgent
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

ACTAgent can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
- WhatsApp setup is install-on-demand: onboarding can show the setup flow before
  ...
  ACTAgentHub/npm plugin only when the channel is actually active.
...
- [iMessage](/channels/imessage) - Native macOS integration via the `imsg` bridge on a signed-in Mac (or SSH wrapper when the Gateway runs elsewhere), including private API actions for replies, tapbacks, effects, attachments, and group management. Preferred for new ACTAgent iMessage setups when host permissions and Messages access fit.
...
- Channels can run simultaneously; configure multiple and ACTAgent will route per chat.

...
```bash
actagent plugins install @actagent/googlechat
```

Local checkout (when running from a git repo):

```bash
actagent plugins install ./path/to/local/googlechat-plugin
```
...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `actagent-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.actagent/googlechat-service-account.json`).
5. Create a Google Chat app in the [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   - Fill in the **Application info**:
     - **App name**: (e.g. `ACTAgent`)
     - **Avatar URL**: (e.g. `https://actagent.ai/logo.png`)
     - **Description**: (e.g. `Personal AI Assistant`)
   - Enable **Interactive features**.
   - Under **Functionality**, check **Join spaces and group conversations**.
   - Under **Connection settings**, select **HTTP endpoint URL**.
   - Under **Triggers**, select **Use a common HTTP endpoint URL for all triggers** and set it to your gateway's public URL followed by `/googlechat`.
     - _Tip: Run `actagent status` to find your gateway's public URL._
...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure ACTAgent with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the ACTAgent dashboard and other sensitive endpoints on your private network.
...
2. **Expose the dashboard to the tailnet only (port 8443):**
   ...
   tailscale serve --bg --https 8443 http://127.0.0.1:19199

   # If bound to Tailscale IP only (e.g., 100.106.161.80):
   tailscale serve --bg --https 8443 http://100.106.161.80:19199
   ```
...
3. **Expose only the webhook path publicly:**
   ...
   ```bash
   # If bound to localhost (127.0.0.1 or 0.0.0.0):
   tailscale funnel --bg --set-path /googlechat http://127.0.0.1:19199/googlechat

   # If bound to Tailscale IP only (e.g., 100.106.161.80):
   tailscale funnel --bg --set-path /googlechat http://100.106.161.80:19199/googlechat
   ```
...
```caddy
your-domain.com {
    reverse_proxy /googlechat* localhost:19199
}
```

With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to ACTAgent.

### Option C: Cloudflare Tunnel

Configure your tunnel's ingress rules to only route the webhook path:

- **Path**: `/googlechat` -> `http://localhost:19199/googlechat`
- **Default Rule**: HTTP 404 (Not Found)

## How it works

1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
   - ACTAgent verifies bearer auth before reading/parsing full webhook bodies when the header is present.
...
2. ACTAgent verifies the token against the configured `audienceType` + `audience`:
...
1. **Channel not configured**: The `channels.googlechat` section is missing from your config. Verify with:
   ...
   ```bash
   actagent config get channels.googlechat
   ```
...
2. **Plugin not enabled**: Check plugin status:
   ...
   ```bash
   actagent plugins list | grep googlechat
   ```
...
- Check `actagent channels status --probe` for auth errors or missing audience config.
- If no messages arrive, confirm the Chat app's webhook URL + event subscriptions.
- If mention gating blocks replies, set `botUser` to the app's user resource name and verify `requireMention`.
- Use `actagent logs --follow` while sending a test message to see if requests reach the gateway.

...
read_when:
  - You want to connect ACTAgent to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to ACTAgent via the LINE Messaging API. The plugin runs as a webhook
...
```bash
actagent plugins install @actagent/line
```

Local checkout (when running from a git repo):

```bash
actagent plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so ACTAgent applies strict pre-auth body limits and timeout before verification.
- ACTAgent processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
actagent pairing list line
actagent pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.actagent/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. ACTAgent validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.

...
Tencent Yuanbao is Tencent's AI assistant platform. The ACTAgent channel plugin
connects Yuanbao bots to ACTAgent over WebSocket so they can interact with users
...
> **Requires ACTAgent 2026.4.10 or above.** Run `actagent --version` to check. Upgrade with `actagent update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  actagent channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
actagent channels login --channel yuanbao
```
...
```bash
actagent pairing list yuanbao
actagent pairing approve yuanbao <CODE>
```

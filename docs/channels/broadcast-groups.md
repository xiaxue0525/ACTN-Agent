...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when ACTAgent would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When ACTAgent would reply in this chat, it will run all three agents.
...
<Tabs>
  <Tab title="Alfred's context">
    ...
    Workspace: /Users/user/actagent-alfred/
  ...
  <Tab title="Bärbel's context">
    ...
    Workspace: /Users/user/actagent-baerbel/
    Tools: read only
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Agents not responding">
    ...
    **Debug:**
    ...
    ```bash
    tail -f ~/.actagent/logs/gateway.log | grep broadcast
    ```

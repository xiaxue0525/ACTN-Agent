...
- bot user = the ACTAgent Matrix account that sends the reply
...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    ACTAgent marks finalized text-only preview edits with `content["com.actagent.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:

```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/actagent-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.actagent\\.finalized_preview",
        "value": true
      },
...
```
    ...
    - `actagent-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `actagent-finalized-preview-<botname>`)
    - `@bot:example.org`: your ACTAgent bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple ACTAgent bots notifying the same recipient, create one rule per bot with a distinct sender match.

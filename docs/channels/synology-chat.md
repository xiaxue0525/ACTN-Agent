---
summary: "Synology Chat webhook setup and ACTAgent config"
read_when:
  - Setting up Synology Chat with ACTAgent
...
Synology Chat ships as a bundled plugin in current ACTAgent releases, so normal
...
```bash
actagent plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged ACTAgent releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `actagent onboard` now shows Synology Chat in the same channel setup list as `actagent channels add`.
   - Non-interactive setup: `actagent channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your ACTAgent gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in ACTAgent.
   - Guided: `actagent onboard`
   - Direct: `actagent channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- ACTAgent accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `actagent pairing list synology-chat`
  - `actagent pairing approve synology-chat <CODE>`
...
```bash
actagent message send --channel synology-chat --target 123456 --text "Hello from ACTAgent"
actagent message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
actagent message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before ACTAgent forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. ACTAgent now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached ACTAgent
- `Rate limit exceeded`:

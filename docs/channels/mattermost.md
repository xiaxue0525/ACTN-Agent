---
summary: "Mattermost bot setup and ACTAgent config"
read_when:
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    actagent plugins install @actagent/mattermost
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    actagent plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged ACTAgent releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure ACTAgent and start the gateway">
    Minimal config:
...
Native slash commands are opt-in. When enabled, ACTAgent registers `oc_*` slash commands via the Mattermost API and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, ACTAgent derives one from gateway host/port + `callbackPath`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when ACTAgent registers `oc_*` commands.
    - ACTAgent refreshes current Mattermost command registration before accepting each callback so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as ACTAgent.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to ACTAgent.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from ACTAgent, not `404`.
...
- Approve via:
  - `actagent pairing list mattermost`
  - `actagent pairing approve mattermost <CODE>`
...
Use these target formats with `actagent message send` or cron/webhooks:
...
ACTAgent resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), ACTAgent sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When ACTAgent sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), ACTAgent falls back to sending a fresh final post so the reply is never lost.
...
Normal agent replies can also include semantic `presentation` payloads. ACTAgent renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, ACTAgent derives the callback URL from `gateway.customBindHost` + `gateway.port`, then falls back to `http://localhost:<port>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and ACTAgent run on the same host/network namespace.
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="actagent-mattermost-interactions", data=botToken)`
  </Step>
...
secret = hmac.new(
    b"actagent-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `actagent message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: ACTAgent did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to `http://127.0.0.1:19199/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as ACTAgent. Set an explicit externally reachable `commands.callbackUrl` instead.

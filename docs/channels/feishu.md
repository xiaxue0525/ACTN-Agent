...
<Note>
Requires ACTAgent 2026.5.29 or above. Run `actagent --version` to check. Upgrade with `actagent update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  actagent channels login --channel feishu
  ```
...
```bash
actagent pairing list feishu
actagent pairing approve feishu <CODE>
```
...
4. Check logs: `actagent logs --follow`
...
5. Ensure the gateway is running: `actagent gateway status`
6. Check logs: `actagent logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `actagent channels login --channel feishu`
...
3. Restart the gateway: `actagent gateway restart`
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.actagent/workspace-{agentId}",
        agentDirTemplate: "~/.actagent/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.actagent/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.actagent/agents/{agentId}/agent` |
...
workspace: /Users/you/.actagent/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, ACTAgent
...
conversion fails, ACTAgent falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, ACTAgent
hydrates it from Feishu before routing the turn. Normal group replies that
ACTAgent turns into threads keep using the reply root message ID (`om_*`) so the

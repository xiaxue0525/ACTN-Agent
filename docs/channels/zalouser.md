...
read_when:
  - Setting up Zalo Personal for ACTAgent
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside ACTAgent.
...
Zalo Personal ships as a bundled plugin in current ACTAgent releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `actagent plugins install @actagent/zalouser`
- Pinned version: `actagent plugins install @actagent/zalouser@2026.5.2`
- Or from a source checkout: `actagent plugins install ./path/to/local/zalouser-plugin`
...
- `actagent pairing list zalouser`
- `actagent pairing approve zalouser <code>`
...
- On startup, ACTAgent resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, ACTAgent stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in ACTAgent state. Example:
...
Profile names select the saved Zalo login credentials in ACTAgent state. Resolution order is:
...
- ACTAgent sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, ACTAgent sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `actagent channels logout --channel zalouser && actagent channels login --channel zalouser`
...
- The channel now runs fully in ACTAgent without external CLI binaries.

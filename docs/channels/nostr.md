...
read_when:
  - You want ACTAgent to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ACTAgent to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current ACTAgent releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`actagent onboard`) and `actagent channels add` still surface
...
```bash
actagent plugins install --link <path-to-local-nostr-plugin>
```
...
actagent channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "actagent@example.com",
        lud16: "actagent@example.com",
      },
    },
  },
}

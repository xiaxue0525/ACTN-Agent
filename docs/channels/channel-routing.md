...
ACTAgent routes replies **back to the channel where a message came from**. The
...
ACTAgent infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, ACTAgent still records inbound session metadata, but it
...
when a guarded path must not create a new ACTAgent session. In that mode,
ACTAgent may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when ACTAgent would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.actagent/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.actagent`):

- `~/.actagent/agents/<agentId>/sessions/sessions.json`

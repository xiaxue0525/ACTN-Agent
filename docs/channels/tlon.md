...
Tlon is a decentralized messenger built on Urbit. ACTAgent connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current ACTAgent releases, so normal packaged
...
```bash
actagent plugins install @actagent/tlon
```
...
```bash
actagent plugins install ./path/to/local/tlon-plugin
```
...
By default, ACTAgent blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `actagent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ACTAgent replies in-thread.

...
Nextcloud Talk ships as a bundled plugin in current ACTAgent releases, so
...
```bash
actagent plugins install @actagent/nextcloud-talk
```
...
```bash
actagent plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ACTAgent" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure ACTAgent:
   ...
   ```bash
   actagent channels add --channel nextcloud-talk \
...
- Approve via:
  - `actagent pairing list nextcloud-talk`
  - `actagent pairing approve nextcloud-talk <CODE>`

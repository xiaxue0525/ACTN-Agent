...
ACTAgent sources most Go catalog rows from the bundled ACTAgent model registry and
supplements current upstream rows while the registry catches up. Run
`actagent models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    ACTAgent handles per-model routing automatically when the model ref uses

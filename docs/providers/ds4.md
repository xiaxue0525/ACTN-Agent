---
summary: "Run ACTAgent through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run ACTAgent against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the ACTAgent config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. ACTAgent connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled ACTAgent provider plugin. Configure it under
...
<Warning>
ACTAgent agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the ACTAgent provider config">
    ...
    actagent infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: actagent-ds4-ok" \
      --json
    ```
...
aligned with `--tokens` unless you intentionally want ACTAgent to request less
output than the server default.

## On-demand startup

ACTAgent can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and ACTAgent model
metadata:
...
Then test ACTAgent model routing:
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the ACTAgent turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when ACTAgent starts the server on
    demand.
  </Accordion>
</AccordionGroup>

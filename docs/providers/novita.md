---
summary: "Use NovitaAI's OpenAI-compatible API with ACTAgent"
read_when:
  - You want to run ACTAgent with NovitaAI models
...
API. In ACTAgent it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. ACTAgent handles
...
```bash
actagent onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for ACTAgent model selection. Your account,
...
```bash
actagent models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `actagent onboard --auth-choice novita-api-key` if the stored profile is
  stale.

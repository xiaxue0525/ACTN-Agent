---
summary: "Use Ollama Cloud directly with ACTAgent"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets ACTAgent call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. ACTAgent registers it
...
- You want the same native Ollama chat API shape ACTAgent uses for local
...
ACTAgent discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
actagent models list --provider ollama-cloud
actagent models set ollama-cloud/kimi-k2.6
```
...
ACTAGENT_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `actagent models list --provider ollama-cloud` and

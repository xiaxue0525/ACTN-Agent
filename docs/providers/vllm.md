---
summary: "Run ACTAgent with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run ACTAgent against a local vLLM server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "vLLM"
---

vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. ACTAgent connects to vLLM using the `openai-completions` API.

ACTAgent can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth). Use `vllm/*` in `agents.defaults.models` to keep discovery dynamic when you also configure a custom vLLM base URL.

ACTAgent treats `vllm` as a local OpenAI-compatible provider that supports
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    actagent models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, ACTAgent queries:
...
<Note>
If you set `models.providers.vllm` explicitly, ACTAgent uses your declared models by default. Add `"vllm/*": {}` to `agents.defaults.models` when you want ACTAgent to query that configured provider's `/models` endpoint and include all advertised vLLM models.
</Note>
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden ACTAgent attribution headers | Not injected on custom base URLs |
  ...
  <Accordion title="Nemotron 3 thinking controls">
    ...
    returned as hidden reasoning or visible answer text. When an ACTAgent session
  ...
  <Accordion title="Qwen tool calls appear as text">
    ...
    - vLLM returns an empty `tool_calls` array when ACTAgent sends
      `tool_choice: "auto"`
    ...
    ```bash
    actagent config set agents.defaults.models '{"vllm/Qwen-Qwen2.5-Coder-32B-Instruct":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Server not reachable">
    ...
    For explicit loopback, LAN, or Tailscale endpoints, ACTAgent trusts the
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for ACTAgent.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set. If you have defined `models.providers.vllm`, ACTAgent uses only your declared models unless `agents.defaults.models` includes `"vllm/*": {}`.
  </Accordion>
  ...
  <Accordion title="Tools render as raw text">
    ...
    - confirm the exact model id with `actagent models list --provider vllm`

...
read_when:
  - You want to use Volcano Engine or Doubao models with ACTAgent
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    actagent onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    actagent models list --provider volcengine
    actagent models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `actagent.json`:
...
For voice-note targets, ACTAgent asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `actagent onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, ACTAgent falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.actagent/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running ACTAgent as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about ACTAgent setup.
  </Card>
</CardGroup>

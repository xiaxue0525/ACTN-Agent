...
SenseAudio can transcribe inbound audio and voice-note attachments through ACTAgent's shared `tools.media.audio` pipeline. ACTAgent posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. ACTAgent uploads the
...
<Note>
SenseAudio is batch STT only in ACTAgent. Voice Call realtime transcription

---
summary: "Use Mistral models and Voxtral transcription with ACTAgent"
read_when:
  - You want to use Mistral models in ACTAgent
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

ACTAgent includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    actagent onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    actagent onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    actagent models list --provider mistral
    ```
  </Step>
</Steps>
...
ACTAgent currently ships this bundled Mistral catalog:
...
```bash
actagent infer model run --local \
...
```bash
actagent models list --all --provider mistral --plain
```
...
<Note>
ACTAgent defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    ACTAgent maps the session **thinking** level to Mistral's API:

    | ACTAgent thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so ACTAgent sends

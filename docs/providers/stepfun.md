---
summary: "Use StepFun models with ACTAgent"
read_when:
  - You want StepFun models in ACTAgent
  - You need StepFun setup guidance
title: "StepFun"
---

ACTAgent includes a bundled StepFun provider plugin with two provider ids:
    ...
    **Best for:** general-purpose use via the standard StepFun endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        actagent onboard --auth-choice stepfun-standard-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        actagent onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        actagent models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    **Best for:** Step Plan reasoning endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        actagent onboard --auth-choice stepfun-plan-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        actagent onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        actagent models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - The provider is bundled with ACTAgent, so there is no separate plugin install step.
    ...
    - Use `actagent models list` and `actagent models set <provider/model>` to inspect or switch models.

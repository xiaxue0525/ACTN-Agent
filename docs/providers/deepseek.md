...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with ACTAgent
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    actagent onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    actagent models list --provider deepseek
    ```
    ...
    ```bash
    actagent models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. ACTAgent also replays
...
`reasoning_content` on follow-up requests. ACTAgent handles this inside the
...
DeepSeek `reasoning_content`. ACTAgent fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in ACTAgent (including the UI **None** selection),
ACTAgent sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
ACTAGENT_LIVE_PROVIDERS=deepseek \
ACTAGENT_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \

...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    actagent config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
...
<Tabs>
  <Tab title="Native Perplexity API (pplx-)">
    When your key starts with `pplx-`, ACTAgent uses the native Perplexity Search
  ...
  <Tab title="OpenRouter / Sonar (sk-or-)">
    When your key starts with `sk-or-`, ACTAgent routes through OpenRouter using
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    ACTAgent will detect the prefix and switch to the Sonar transport
    automatically.

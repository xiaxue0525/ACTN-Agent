---
summary: "Use Synthetic's Anthropic-compatible API in ACTAgent"
read_when:
...
ACTAgent registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    actagent onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
ACTAgent's Anthropic client appends `/v1` to the base URL automatically, so use
...
`actagent models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that ACTAgent appends `/v1` automatically.

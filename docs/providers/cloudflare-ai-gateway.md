...
read_when:
  - You want to use Cloudflare AI Gateway with ACTAgent
  - You need the account ID, gateway ID, or API key env var
---

Cloudflare AI Gateway sits in front of provider APIs and lets you add analytics, caching, and controls. For Anthropic, ACTAgent uses the Anthropic Messages API through your Gateway endpoint.
...
When thinking is enabled for Anthropic Messages models, ACTAgent strips trailing
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    actagent onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your ACTAgent config:
  ...
  <Step title="Verify the model is available">
    ```bash
    actagent models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key exported only in an interactive shell will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.actagent/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>

---
summary: "Use OpenRouter's unified API to access many models in ACTAgent"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in ACTAgent
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    actagent onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    actagent models set openrouter/<provider>/<model>
    ```
...
ACTAgent submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. ACTAgent sends `modalities: ["text",
...
ACTAgent sends OpenRouter STT requests as JSON with base64 audio under
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), ACTAgent also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://actagent.ai`                                                                                  |
| `X-OpenRouter-Title`      | `ACTAgent`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, ACTAgent
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    ACTAgent sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that ACTAgent uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, ACTAgent maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. ACTAgent sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: ACTAgent keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    ACTAgent forwards that object to OpenRouter as the request `provider`

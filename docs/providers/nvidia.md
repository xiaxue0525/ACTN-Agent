---
summary: "Use NVIDIA's OpenAI-compatible API in ACTAgent"
read_when:
  - You want to use open models in ACTAgent for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). ACTAgent
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    actagent onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    actagent models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
actagent onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, ACTAgent setup and model-selection paths
...
ACTAgent release. When the live feed is available, the first returned model is
...
malformed, ACTAgent falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in ACTAgent. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    ACTAgent prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. ACTAgent's bundled Ultra row

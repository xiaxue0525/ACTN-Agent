...
summary: "Specification for making the bundled Codex app-server harness honor ACTAgent context-engine plugins"
read_when:
  - You are wiring context-engine lifecycle behavior into the Codex harness
  - You need lossless-actagent or another context-engine plugin to work with codex/* embedded harness sessions
  - You are comparing embedded ACTAgent and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same ACTAgent context-engine
lifecycle contract that embedded ACTAgent turns already honor.
...
`lossless-actagent`, control context assembly, post-turn ingest, maintenance, and
ACTAgent-level compaction policy as far as the Codex app-server boundary allows.

## Non-goals

- Do not reimplement Codex app-server internals.
- Do not make Codex native thread compaction produce a lossless-actagent summary.
...
as built-in ACTAgent attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in ACTAgent-controlled code. The external
boundary is the Codex app-server protocol itself: ACTAgent can control what it
...
Built-in ACTAgent attempts call the context-engine lifecycle directly:
...
For Codex harness turns, ACTAgent should preserve this lifecycle:

1. Read the mirrored ACTAgent session transcript.
...
8. Mirror the Codex result back into the ACTAgent transcript.
...
11. Preserve Codex native compaction signals and ACTAgent compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. ACTAgent should
not try to mutate the app-server's internal history except through supported
protocol calls.

ACTAgent's transcript mirror remains the source for ACTAgent features:
...
The context-engine interface returns ACTAgent `AgentMessage[]`, not a Codex
...
For engines like lossless-actagent, the assembled context should be deterministic
...
- Label it clearly as ACTAgent assembled context.
...
```text
ACTAgent assembled context for this turn:
...
inside ACTAgent and preserves context-engine semantics.
...
we need strict ACTAgent parity, run context-engine assembly before hook
...
1. ACTAgent context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit ACTAgent compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
ACTAgent compaction should prefer the context engine's `compact()` result for
the ACTAgent transcript mirror and plugin state.
...
the ACTAgent session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
ACTAgent session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in ACTAgent semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole ACTAgent compaction when the context engine is primary.
...
Avoid requiring lossless-actagent in ACTAgent core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for ACTAgent/plugin state.

...
This hub links the core docs for how ACTAgent connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`actagent gateway`), a single long-running process that owns channel connections and the WebSocket control plane.

- **Loopback first**: the Gateway WS defaults to `ws://127.0.0.1:19199`.
...
- **Canvas host** is served on the same port as the Gateway (`/__actagent__/canvas/`, `/__actagent__/a2ui/`), protected by Gateway auth when bound beyond loopback.

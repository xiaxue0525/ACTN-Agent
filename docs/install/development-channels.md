...
ACTAgent ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/actagent`, or
  `$ACTAGENT_HOME/actagent` when `ACTAGENT_HOME` is set; override with
  `ACTAGENT_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
actagent update --tag 2026.4.1-beta.1
...
actagent update --tag actagent@2026.4.1-beta.1
...
- The tag is not persisted. Your next `actagent update` uses your configured
  channel as usual.
- For package installs, ACTAgent pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  ACTAgent prompts for confirmation (skip with `--yes`).
...
Preview what `actagent update` would do without making changes:

```bash
actagent update --dry-run
actagent update --channel beta --dry-run
actagent update --tag 2026.4.1-beta.1 --dry-run
actagent update --dry-run --json
```
...
When you switch channels with `actagent update`, ACTAgent also syncs plugin
sources:

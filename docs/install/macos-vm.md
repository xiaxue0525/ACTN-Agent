---
summary: "Run ACTAgent in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want ACTAgent isolated from your main macOS environment
...
Run ACTAgent in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create actagent --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run actagent --no-display`
5. SSH in, install ACTAgent, configure channels
6. Done
...
```bash
lume create actagent --os macos --ipsw latest
```
...
```bash
lume stop actagent
lume run actagent --no-display
```

The VM runs in the background. ACTAgent's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "actagent status"
```
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to ACTAgent.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running ACTAgent/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your ACTAgent config:
...
```bash
lume stop actagent
lume clone actagent actagent-golden
```

Reset anytime:

```bash
lume stop actagent && lume delete actagent
lume clone actagent-golden actagent
lume run actagent --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get actagent` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `actagent channels login` |

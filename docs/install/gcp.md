---
summary: "Run ACTAgent Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want ACTAgent running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent ACTAgent Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "ACTAgent 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the ACTAgent Gateway in Docker
- Persist `~/.actagent` + `~/.actagent/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.actagent` state includes `actagent.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-actagent-project --name="ACTAgent Gateway"
    gcloud config set project my-actagent-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create actagent-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh actagent-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the ACTAgent repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ACTAGENT_IMAGE=actagent:latest
    ACTAGENT_GATEWAY_TOKEN=
    ACTAGENT_GATEWAY_BIND=lan
    ACTAGENT_GATEWAY_PORT=19199

    ACTAGENT_CONFIG_DIR=/home/$USER/.actagent
    ACTAGENT_WORKSPACE_DIR=/home/$USER/.actagent/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.actagent
    ```

    Set `ACTAGENT_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, ACTAgent uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ACTAGENT_GATEWAY_TOKEN`.
    ...
    services:
      actagent-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ACTAGENT_GATEWAY_BIND=${ACTAGENT_GATEWAY_BIND}
          - ACTAGENT_GATEWAY_PORT=${ACTAGENT_GATEWAY_PORT}
          - ACTAGENT_GATEWAY_TOKEN=${ACTAGENT_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${ACTAGENT_GATEWAY_PORT}:19199"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ACTAGENT_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm actagent-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:19199"]' --strict-json
    ```

    If you changed the gateway port, replace `19199` with your configured port.
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh actagent-gateway --zone=us-central1-a -- -L 19199:127.0.0.1:19199
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop actagent-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type actagent-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start actagent-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create actagent-deploy \
     --display-name="ACTAgent Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-actagent-project \
     --member="serviceAccount:actagent-deploy@my-actagent-project.iam.gserviceaccount.com" \

---
summary: "Deploy ACTAgent Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run ACTAgent on a Kubernetes cluster
  - You want to test ACTAgent in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running ACTAgent on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

ACTAgent is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/actagent 19199:19199 -n actagent
open http://localhost:19199
```
...
```bash
kubectl get secret actagent-secrets -n actagent -o jsonpath='{.data.ACTAGENT_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: actagent (configurable via ACTAGENT_NAMESPACE)
├── Deployment/actagent        # Single pod, init container + gateway
├── Service/actagent           # ClusterIP on port 19199
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/actagent-config  # actagent.json + AGENTS.md
└── Secret/actagent-secrets    # Gateway token + API keys
```
...
Edit `actagent.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret actagent-secrets -n actagent \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/actagent -n actagent
```

### Custom namespace

```bash
ACTAGENT_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/actagent/actagent:latest # or pin to a specific version from https://github.com/actagent/actagent/releases
```
...
- The default config keeps the Control UI on the safer local-access path: loopback bind plus `kubectl port-forward` to `http://127.0.0.1:19199`
...
└── manifests/
    ...
    ├── configmap.yaml          # actagent.json + AGENTS.md
    ├── deployment.yaml         # Pod spec with security hardening
    ├── pvc.yaml                # 10Gi persistent storage
    └── service.yaml            # ClusterIP on 19199
```

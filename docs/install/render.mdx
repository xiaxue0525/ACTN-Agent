---
summary: "Deploy ACTAgent on Render with Infrastructure-as-Code"
read_when:
  - Deploying ACTAgent to Render
...
Deploy ACTAgent on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/actagent/actagent)
...
repository configures everything needed to run ACTAgent:
...
services:
  - type: web
    ...
    envVars:
      - key: ACTAGENT_GATEWAY_PORT
        value: "8080"
      - key: ACTAGENT_STATE_DIR
        value: /data/.actagent
      - key: ACTAGENT_WORKSPACE_DIR
        value: /data/workspace
      - key: ACTAGENT_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means ACTAgent state
...
`ACTAGENT_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original ACTAgent repository, Render will not auto-deploy your ACTAgent. To update it, run a manual Blueprint sync from the dashboard.
...
For ACTAgent, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `ACTAGENT_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ACTAGENT_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `actagent backup create` in the Render shell.

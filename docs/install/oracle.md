---
summary: "Host ACTAgent on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up ACTAgent on Oracle Cloud
  - Looking for free VPS hosting for ACTAgent
  - Want 24/7 ACTAgent on a small server
title: "Oracle Cloud"
---

Run a persistent ACTAgent Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname actagent
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=actagent
    ```

    From now on, connect via Tailscale: `ssh ubuntu@actagent`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    actagent config set gateway.bind loopback
    ...
    actagent config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    curl http://localhost:19199
    ```

    Access the Control UI from any device on your tailnet:

    ```
    https://actagent.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.actagent` to restrict credential file permissions.
- `actagent security audit` for an ACTAgent-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most ACTAgent features work fine; a small number of native binaries need ARM builds:
...
```bash
ssh -L 19199:127.0.0.1:19199 ubuntu@actagent
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=actagent --reset` to re-authenticate.

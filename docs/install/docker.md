---
summary: "Optional Docker-based setup and onboarding for ACTAgent"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run ACTAgent on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ACTAGENT_IMAGE="ghcr.io/actagent/actagent:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/actagent/actagent/pkgs/container/actagent).
  ...
  <Step title="Complete onboarding">
    ...
    `actagent-gateway` directly. `actagent-cli` is for commands you run after
    the gateway container already exists.

  </Step>

  <Step title="Open the Control UI">
    Open `http://127.0.0.1:19199/` in your browser and paste the configured
    ...
    ```bash
    docker compose run --rm actagent-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm actagent-cli channels login

    # Telegram
    docker compose run --rm actagent-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm actagent-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t actagent:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node actagent-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node actagent-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:19199","http://127.0.0.1:19199"]}]'
docker compose up -d actagent-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `ACTAGENT_EXTRA_MOUNTS`
or `ACTAGENT_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `actagent-cli` shares `actagent-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d actagent-gateway`, run onboarding
and setup-time config writes through `actagent-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `ACTAGENT_IMAGE`                           | Use a remote image instead of building locally                        |
| `ACTAGENT_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `ACTAGENT_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `ACTAGENT_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `ACTAGENT_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `ACTAGENT_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `ACTAGENT_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `ACTAGENT_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `ACTAGENT_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `ACTAGENT_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `ACTAGENT_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `ACTAGENT_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, ACTAgent
...
`ACTAGENT_IMAGE_APT_PACKAGES` during image build. The legacy
`ACTAGENT_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `ACTAGENT_IMAGE_PIP_PACKAGES`. This runs
...
`ACTAGENT_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export ACTAGENT_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="actagent-gateway"
./scripts/docker/setup.sh
```

Install the official `@actagent/diagnostics-otel` plugin from ACTAgentHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`ACTAGENT_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`actagenthub:@actagent/diagnostics-prometheus`, enable the
`diagnostics-prometheus` plugin, then scrape:

```text
http://<gateway-host>:19199/api/diagnostics/prometheus
```
...
```bash
curl -fsS http://127.0.0.1:19199/healthz   # liveness
curl -fsS http://127.0.0.1:19199/readyz     # readiness
```
...
```bash
docker compose exec actagent-gateway node dist/index.js health --token "$ACTAGENT_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `ACTAGENT_GATEWAY_BIND=lan` so host access to
`http://127.0.0.1:19199` works with Docker port publishing.
...
When ACTAgent runs in Docker, `127.0.0.1` inside the container is the container
...
`ACTAGENT_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `ACTAGENT_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `ACTAGENT_CONFIG_DIR` to `/home/node/.actagent`,
`ACTAGENT_WORKSPACE_DIR` to `/home/node/.actagent/workspace`, and
`ACTAGENT_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/actagent`, so those
...
That mounted config directory is where ACTAgent keeps:

- `actagent.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `ACTAGENT_GATEWAY_TOKEN`
...
but separate from `ACTAGENT_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
ACTAgent home, so plugin install records and package roots survive container
...
For easier day-to-day Docker management, install `actagentdock`:
...
If you installed actagentdock from the older `scripts/shell-helpers/actagentdock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `actagentdock-start`, `actagentdock-stop`, `actagentdock-dashboard`, etc. Run
`actagentdock-help` for all commands.
See [actagentdock](/install/actagentdock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export ACTAGENT_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export ACTAGENT_SANDBOX=1
    export ACTAGENT_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the ACTAgent sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm actagent-cli gateway probe
    docker compose run -T --rm actagent-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `actagent-cli` uses `network_mode: "service:actagent-gateway"` so CLI
    ...
    `no-new-privileges` on both `actagent-gateway` and `actagent-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in actagent-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `actagent-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `actagent plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm actagent-cli plugins install <package>
    ```

    If you already created a long-running `actagent-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.actagent`, make sure your host bind mounts are owned by uid 1000:
    ...
    `/path/to/actagent-config/npm` to `root:root` if you intentionally run
    ACTAgent as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ACTAGENT_HOME_VOLUME="actagent_home"`
    2. **Bake system deps**: `export ACTAGENT_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export ACTAGENT_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export ACTAGENT_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm actagent-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `ACTAGENT_HOME_VOLUME` or
       `ACTAGENT_EXTRA_MOUNTS`. ACTAgent auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/actagent/actagent/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    ACTAgent runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm actagent-cli dashboard --no-open
    docker compose run --rm actagent-cli devices list
    docker compose run --rm actagent-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm actagent-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm actagent-cli devices list --url ws://127.0.0.1:19199
    ```
...
- [actagentdock](/install/actagentdock) — Docker Compose community setup
- [Updating](/install/updating) — keeping ACTAgent up to date

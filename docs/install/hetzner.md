---
summary: "Run ACTAgent Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want ACTAgent running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running ACTAgent in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent ACTAgent Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "ACTAgent 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.actagent/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.actagent
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ACTAGENT_CONFIG_DIR=/root/.actagent
    ACTAGENT_WORKSPACE_DIR=/root/.actagent/workspace
  ...
  <Step title="Hetzner-specific access">
    ...
    **Prerequisite:** Ensure your VPS sshd config allows TCP forwarding. If you
    ...
    ```bash
    ssh -N -L 19199:127.0.0.1:19199 root@YOUR_VPS_IP
    ```
...
- Infrastructure: [actagent-terraform-hetzner](https://github.com/andreesg/actagent-terraform-hetzner)
- Docker config: [actagent-docker-config](https://github.com/andreesg/actagent-docker-config)

---
summary: "Developer workflow for ACTAgent agent runtime: build, test, and live validation"
title: "ACTAgent agent runtime workflow"
read_when:
  - Working on ACTAgent agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the ACTAgent agent runtime in ACTAgent.
...
```bash
ACTAGENT_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm actagent agent --message "Hello" --thinking low`
...
State lives under the ACTAgent state directory. Default is `~/.actagent`. If `ACTAGENT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `actagent.json` for config
...
- [ACTAgent agent runtime architecture](/agent-runtime-architecture)

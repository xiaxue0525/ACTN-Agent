---
summary: "CLI reference for `actagent agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
actagent agent --to +15555550123 --message "status update" --deliver
actagent agent --agent ops --message "Summarize logs"
actagent agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
actagent agent --session-key agent:ops:incident-42 --message "Summarize status"
actagent agent --agent ops --session-key incident-42 --message "Summarize status"
actagent agent --session-id 1234 --message "Summarize inbox" --thinking medium
actagent agent --to +15555550123 --message "Trace logs" --verbose on --json
actagent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
actagent agent --agent ops --message "Run locally" --local
```

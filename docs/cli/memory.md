---
summary: "CLI reference for `actagent memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
actagent memory status
actagent memory status --deep
actagent memory status --fix
actagent memory index --force
actagent memory search "meeting notes"
actagent memory search --query "deployment" --max-results 20
actagent memory promote --limit 10 --min-score 0.75
actagent memory promote --apply
actagent memory promote --json --min-recall-count 0 --min-unique-queries 0
actagent memory promote-explain "router vlan"
actagent memory promote-explain "router vlan" --json
actagent memory rem-harness
actagent memory rem-harness --json
actagent memory status --json
actagent memory status --deep --index
actagent memory status --deep --index --verbose
actagent memory status --agent main
actagent memory index --agent main --verbose
```
...
```bash
actagent memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `actagent cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
actagent memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
actagent memory rem-harness [--agent <id>] [--include-promoted] [--json]
```

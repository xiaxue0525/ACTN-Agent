---
summary: "CLI reference for `actagent tui` (Gateway-backed or local embedded terminal UI)"
read_when:
  - You want a terminal UI for the Gateway (remote-friendly)
  - You want to pass url/token/session from scripts
  - You want to run the TUI in local embedded mode without a Gateway
  - You want to use actagent chat or actagent tui --local
title: "TUI"
---
...
Aliases: `actagent chat` and `actagent terminal` invoke the same command with `--local` implied.

Notes:

- `chat` and `terminal` are aliases for `actagent tui --local`.
...
actagent tui --local
actagent tui
actagent tui --url ws://127.0.0.1:19199 --token <token>
actagent tui --session main --deliver
actagent chat --message "Compare my config to the docs and tell me what to fix"
# when run inside an agent workspace, infers that agent automatically
actagent tui --session bugfix
```
...
If `actagent config validate` is already failing, use `actagent configure` or
`actagent doctor --fix` first. `actagent chat` does not bypass the invalid-
config guard.
...
Apply targeted fixes with `actagent config set` or `actagent configure`, then
rerun `actagent config validate`. See [TUI](/web/tui) and [Config](/cli/config).

---
summary: "CLI reference for `actagent configure` (interactive configuration prompts)"
read_when:
...
Use `actagent onboard` for the full guided first-run journey, `actagent setup` for the baseline config/workspace only, and `actagent channels add` when you only need channel account setup.
...
<Tip>
`actagent config` without a subcommand opens the same wizard. Use `actagent config get|set|unset` for non-interactive edits.
</Tip>

For web search, `actagent configure --section web` lets you choose a provider
...
actagent configure --section model --section channels
actagent configure --section gateway --section daemon
```

---
summary: "CLI reference for `actagent channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `actagent sessions`, Gateway `sessions.list`, or the agent
...
```bash
actagent channels add --channel telegram --token <bot-token>
actagent channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
actagent channels remove --channel telegram --delete
```

<Tip>
`actagent channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, ACTAgent uses the channel's default install source without opening the interactive plugin install prompt.

When you run `actagent channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `actagent agents bindings`, `actagent agents bind`, and `actagent agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, ACTAgent promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `actagent status --deep` for a broad probe.
- Use `actagent doctor` for guided fixes.
- `actagent channels list` no longer prints model provider usage/quota snapshots. For those, use `actagent status` (overview) or `actagent models list` (per-provider).
- `actagent channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
actagent channels resolve --channel slack "#general" "@jane"
actagent channels resolve --channel discord "My Server/#support" "@someone"
actagent channels resolve --channel matrix "Project Room"
```

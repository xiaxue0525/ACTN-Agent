---
summary: "CLI reference for `actagent sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `actagent channels status --probe`,
`actagent status --deep`, or `actagent health --verbose` when you need live
channel connectivity.

`actagent sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
actagent sessions
actagent sessions --agent work
actagent sessions --all-agents
actagent sessions --active 120
actagent sessions --limit 25
actagent sessions --verbose
actagent sessions --json
```
...
```bash
actagent sessions tail
actagent sessions tail --follow
actagent sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
actagent sessions --agent work tail --follow
actagent sessions --all-agents tail --follow
```

`actagent sessions tail` renders recent trajectory JSONL events as compact progress lines. Without `--session-key`, it tails running sessions first, then the latest stored session. `--tail <count>` controls how many existing events print before follow mode; the default is `80`, and `0` starts at the current end. `--follow` keeps watching the selected trajectory files, including relocated files referenced by `<session>.trajectory-path.json`.
...
```bash
actagent sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
actagent sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.actagent/trajectory-exports/` under the selected workspace.

`actagent sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`actagent sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.actagent/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.actagent/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
actagent sessions cleanup --agent work --dry-run
actagent sessions cleanup --all-agents --dry-run
actagent sessions cleanup --enforce
actagent sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
actagent sessions cleanup --dry-run --fix-dm-scope
actagent sessions cleanup --json
```

`actagent sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `actagent sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`actagent sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.actagent/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.actagent/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,

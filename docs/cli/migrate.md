---
summary: "CLI reference for `actagent migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into ACTAgent
...
```bash
actagent migrate list
actagent migrate claude --dry-run
actagent migrate codex --dry-run
actagent migrate codex --skill gog-vault77-google-workspace
actagent migrate codex --plugin google-calendar --dry-run
actagent migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
actagent migrate hermes --dry-run
actagent migrate hermes
actagent migrate apply codex --yes --skill gog-vault77-google-workspace
actagent migrate apply codex --yes --plugin google-calendar
actagent migrate apply codex --yes
actagent migrate apply claude --yes
actagent migrate apply hermes --yes
actagent migrate apply hermes --include-secrets --yes
actagent onboard --flow import
actagent onboard --import-from claude --import-source ~/.claude
actagent onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `actagent migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local ACTAgent state exists.
</ParamField>
...
`actagent migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `actagent migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an ACTAgent backup before applying the migration. If no local ACTAgent state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the ACTAgent agent workspace.
...
- Claude command Markdown files converted into ACTAgent skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. ACTAgent does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the ACTAgent Codex harness and you want to
...
Running `actagent migrate codex` in an interactive terminal previews the full
...
start checked unless the target ACTAgent Codex plugin config already has that
...
```bash
actagent migrate codex --dry-run --skill gog-vault77-google-workspace
actagent migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
actagent migrate codex --dry-run --plugin google-calendar
actagent migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  ACTAgent agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to ACTAgent provider runs,
...
- `SOUL.md` and `AGENTS.md` into the ACTAgent agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for ACTAgent file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that ACTAgent cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live ACTAgent config or credentials. This preserves opaque or unsafe state without pretending ACTAgent can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `actagent.plugin.json`:
...
Provider plugins can use `actagent/plugin-sdk/migration` for item construction and summary counts, plus `actagent/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `actagent onboard --flow import` and `actagent setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh ACTAgent setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move ACTAgent to a new machine.

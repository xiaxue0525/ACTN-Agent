---
summary: "CLI reference for `actagent path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
actagent plugins enable oc-path
```

## Why use it

ACTAgent state is spread across human-edited markdown, commented JSONC config,
...
`actagent path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `actagent path` for ordinary whole-file edits, rich
...
```bash
actagent path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
actagent path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
actagent path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
actagent path resolve 'oc://AGENTS.md/runtime-safety/actagent-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
actagent path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`actagent path` does four things:
...
# Read a leaf
actagent path resolve 'oc://gateway.jsonc/version'

# Wildcard search
actagent path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
actagent path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
actagent path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
actagent path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
actagent path emit ./AGENTS.md
```
...
actagent path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
actagent path set 'oc://actagent.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
actagent path set 'oc://actagent.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
actagent path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
actagent path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
actagent path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
actagent path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
actagent path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
actagent path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
actagent path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
actagent path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
actagent path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
actagent path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
actagent path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ actagent path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ actagent path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ actagent path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ actagent path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ actagent path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ actagent path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ actagent path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: actagent.invoke
```

```bash
$ actagent path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ actagent path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
actagent path resolve 'oc://AGENTS.md/tools/gh/risk' --human
actagent path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
actagent path find 'oc://AGENTS.md/tools/**/risk'
actagent path find 'oc://session.jsonl/[event=action]/userId'
actagent path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
actagent path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
actagent path set 'oc://gateway.jsonc/version' '2.0'
actagent path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ actagent path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
actagent path emit ./AGENTS.md
actagent path emit ./gateway.jsonc --json
```
...
`actagent path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__ACTAGENT_REDACTED__` (verbatim or as a substring) is refused at write
  time.

---
summary: "CLI reference for `actagent approvals` and `actagent exec-policy`"
read_when:
...
## `actagent exec-policy`

`actagent exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.actagent/exec-approvals.json`

Examples:

```bash
actagent exec-policy show
actagent exec-policy show --json

actagent exec-policy preset yolo
actagent exec-policy preset cautious --json

actagent exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `actagent exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `actagent approvals set --gateway`
or `actagent approvals set --node <id|name|ip>`.

## Common commands

```bash
actagent approvals get
actagent approvals get --node <id|name|ip>
actagent approvals get --gateway
```

`actagent approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
actagent approvals set --file ./exec-approvals.json
actagent approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
actagent approvals set --node <id|name|ip> --file ./exec-approvals.json
actagent approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested ACTAgent policy aligned, also set:
...
```bash
actagent approvals allowlist add "~/Projects/**/bin/rg"
actagent approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
actagent approvals allowlist add --agent "*" "/usr/bin/uname"

actagent approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `actagent nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.actagent/exec-approvals.json`.

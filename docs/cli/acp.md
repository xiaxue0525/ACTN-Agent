...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an ACTAgent Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`actagent acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to ACTAgent channel
conversations instead of hosting an ACP harness session, use
[`actagent mcp serve`](/cli/mcp) instead.
...
- ACTAgent acts as an ACP server
- an IDE or ACP client connects to ACTAgent
- ACTAgent forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where ACTAgent runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to ACTAgent: use `actagent acp`
- ACTAgent should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include ACTAgent parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the ACTAgent gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
actagent acp --url wss://gateway-host:19199 --token <token>

# Remote Gateway (token from file)
actagent acp --url wss://gateway-host:19199 --token-file ~/.actagent/gateway.token

# Attach to an existing session key
actagent acp --session agent:main:main

# Attach by label (must already exist)
actagent acp --session-label "support inbox"

# Reset the session key before the first prompt
actagent acp --session agent:main:main --reset-session
```
...
actagent acp client --server-args --url wss://gateway-host:19199 --token-file ~/.actagent/gateway.token

# Override the server command (default: actagent)
actagent acp client --server "node" --server-args actagent.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run ACTAgent through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`actagent acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `actagent gateway call sessions.list` as the only ACP proof. That
...
it to drive an ACTAgent Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `actagent acp` over stdio.

Example config (persisted):

```bash
actagent config set gateway.remote.url wss://gateway-host:19199
actagent config set gateway.remote.token <token>
```
...
```bash
actagent acp --session agent:main:main
actagent acp --session agent:design:main
actagent acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see ACTAgent plugin tools or selected
...
[ACTAgent tools MCP bridge](/tools/acp-agents-setup#actagent-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
ACTAgent bot over ACP, use `acpx` with its built-in `actagent` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx actagent` at `actagent acp`.
3. Target the ACTAgent session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default ACTAgent ACP session
acpx actagent exec "Summarize the active ACTAgent session state."

# Persistent named session for follow-up turns
acpx actagent sessions ensure --name codex-bridge
acpx actagent -s codex-bridge --cwd /path/to/repo \
  "Ask my ACTAgent work agent for recent context relevant to this repo."
```

If you want `acpx actagent` to target a specific Gateway and session key every
time, override the `actagent` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "actagent": {
      "command": "env ACTAGENT_HIDE_BANNER=1 ACTAGENT_SUPPRESS_NOTES=1 actagent acp --url ws://127.0.0.1:19199 --token-file ~/.actagent/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local ACTAgent checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ACTAGENT_HIDE_BANNER=1 ACTAGENT_SUPPRESS_NOTES=1 node actagent.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an ACTAgent agent without scraping a terminal.
...
In Zed, open the Agent panel and select "ACTAgent ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ACTAGENT_GATEWAY_TOKEN`, `ACTAGENT_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ACTAGENT_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ACTAGENT_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `actagent acp client` sets `ACTAGENT_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `actagent`).

---
summary: "CLI reference for `actagent wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `actagent wiki`
title: "Wiki"
---
...
Use `actagent wiki` when you want a compiled knowledge vault with:
...
actagent wiki search "who should I ask about Teams?" --mode route-question
...
- `.actagent-wiki/cache/agent-digest.json`
- `.actagent-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `actagent memory search` when the
...
```bash
actagent wiki search "bgroux" --mode find-person
actagent wiki search "who knows Teams rollout?" --mode route-question
actagent wiki search "maintainer-whois" --mode source-evidence
actagent wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
actagent wiki get entity.alpha
actagent wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`actagent wiki` behavior is shaped by:

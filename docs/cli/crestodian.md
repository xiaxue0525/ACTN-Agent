...
read_when:
  - You run actagent with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair ACTAgent
...
Crestodian is ACTAgent's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `actagent` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `actagent`
with no command starts Crestodian in an interactive terminal. Running
`actagent crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`actagent tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same ACTAgent reference discovery as regular agents. In a Git checkout,
...
[https://github.com/actagent/actagent](https://github.com/actagent/actagent), with explicit
...
actagent
actagent crestodian
actagent crestodian --json
actagent crestodian --message "models"
actagent crestodian --message "validate config"
actagent crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
actagent crestodian --message "set default model openai/gpt-5.5" --yes
actagent onboard --modern
```
...
config set-ref gateway.auth.token env ACTAGENT_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install actagenthub:actagent-codex-app-server
plugin uninstall actagent-codex-app-server
talk to work agent
...
- `actagent.json` is missing
- `actagent.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`actagent --help` and `actagent --version` still use the normal fast paths.
Noninteractive bare `actagent` exits with a short message instead of printing
...
- create agents
- install plugins from ACTAgentHub or npm
- uninstall plugins
...
```text
~/.actagent/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`actagent onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `actagent onboard` still runs classic onboarding.
...
planner turn through ACTAgent's normal runtime paths. It first uses the
configured ACTAgent model. If no configured model is usable yet, it can fall
...
`actagent tui`, `actagent chat`, and `actagent terminal` still open the normal
...
ACTAgent: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
ACTAgent: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
ACTAgent: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `actagent` for agent handoff.

---
summary: "CLI reference for `actagent devices` (device pairing + token rotation/revocation)"
read_when:
...
### `actagent devices remove <deviceId>`
...
```
actagent devices remove <deviceId>
actagent devices remove <deviceId> --json
```

### `actagent devices clear --yes [--pending]`

Clear paired devices in bulk.

```
actagent devices clear --yes
actagent devices clear --yes --pending
actagent devices clear --yes --pending --json
```

### `actagent devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, ACTAgent only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), ACTAgent supersedes the previous pending entry and issues a new `requestId`. Run `actagent devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
ACTAgent keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `actagent devices list`
or use `actagent devices approve --latest` to preview the exact upgrade before
approving it.
...
```
actagent devices approve
actagent devices approve <requestId>
actagent devices approve --latest
```

## Paperclip / `actagent_gateway` first-run approval

When a new Paperclip agent connects through the `actagent_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `actagent_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `actagent devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
actagent devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `actagent devices list` first to confirm a pending request exists.
...
### `actagent devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
actagent devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `actagent devices revoke --device <id> --role <role>`
...
```
actagent devices revoke --device <deviceId> --role node
```
...
```bash
actagent devices rotate --device <deviceId> --role operator
```

---
summary: "CLI reference for `actagent policy` conformance checks"
read_when:
  - You want to check ACTAgent settings against an authored policy.jsonc
...
`actagent policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing ACTAgent settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
ACTAgent observes the active workspace as evidence, and policy health checks
...
data-handling posture, ACTAgent config secret provider/auth profile posture, and governed tool
...
posture, require ACTAgent config SecretRefs to use managed providers, require
...
repeatable way to prove that ACTAgent still conforms to that statement. Use
...
```bash
actagent plugins enable policy
```
...
exposure, agent workspace posture, configured sandbox runtime posture, ACTAgent
...
when a concrete rule is present. ACTAgent reads current `channels.*` settings
...
posture, ACTAgent config agent sandbox workspace access and tool deny posture,
...
`edit`, and `apply_patch`; ACTAgent config `group:fs` covers file mutation tools
...
present in `policy.jsonc`. The observed state is existing ACTAgent config or
...
present in `agents.list[]`, ACTAgent evaluates the scoped rule against inherited
...
Container posture policy is evaluated only against evidence ACTAgent can
...
```bash
actagent policy check
actagent policy check --json
actagent policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `actagent doctor --lint`
when the Policy plugin is enabled.

Compare an operator policy file to an authored baseline policy file:

```bash
actagent policy compare --baseline official.policy.jsonc
actagent policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```

`policy compare` compares policy file syntax to policy file syntax. It does not
inspect ACTAgent runtime state, evidence, credentials, or secrets. The command
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://actagent.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://actagent.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://actagent.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://actagent.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://actagent.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://actagent.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://actagent.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://actagent.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://actagent.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://actagent.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed ACTAgent state used by the policy checks. The
...
2. Run `actagent policy check --json`.
...
5. Re-run `actagent doctor --lint` in CI or release gates.
...
`actagent policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
actagent policy watch --json
```
...
```json
{
  ...
  "target": "oc://actagent.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://actagent.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://actagent.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://actagent.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://actagent.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in ACTAgent config

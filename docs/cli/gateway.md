---
summary: "ACTAgent Gateway CLI (`actagent gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is ACTAgent's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `actagent gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How ACTAgent advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.actagent/actagent.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `actagent onboard --mode local` and `actagent setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--port <port>" type="number">
  WebSocket port (default comes from config/env; usually `19199`).
</ParamField>
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `ACTAGENT_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point ACTAgent at that IPv4 endpoint.
</ParamField>
...
actagent gateway restart --safe
actagent gateway restart --safe --skip-deferral
actagent gateway restart --force
```

`actagent gateway restart --safe` asks the running Gateway to preflight active ACTAgent work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`actagent gateway restart --safe --skip-deferral` runs the same ACTAgent-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `ACTAGENT_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `ACTAGENT_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `ACTAGENT_DIAGNOSTICS=timeline` with `ACTAGENT_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `ACTAGENT_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
actagent gateway health --url ws://127.0.0.1:19199
```
...
```bash
actagent gateway usage-cost
actagent gateway usage-cost --days 7
actagent gateway usage-cost --json
```
...
```bash
actagent gateway stability
actagent gateway stability --type payload.large
actagent gateway stability --bundle latest
actagent gateway stability --bundle latest --export
actagent gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, ACTAgent writes the same diagnostic snapshot to `~/.actagent/logs/stability/actagent-stability-*.json` when the recorder has events. Inspect the newest bundle with `actagent gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
actagent gateway diagnostics export --output actagent-diagnostics.zip
actagent gateway diagnostics export --json
```
...
actagent gateway status --require-rpc
```
...
```bash
actagent gateway call status
actagent gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `actagent` or Node with those args.

```bash
cat > ~/.local/bin/actagent-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- actagent "$@"
EOF
chmod +x ~/.local/bin/actagent-doppler

actagent gateway install --wrapper ~/.local/bin/actagent-doppler --force
actagent gateway restart
```
...
`ACTAGENT_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
ACTAGENT_WRAPPER="$HOME/.local/bin/actagent-doppler" actagent gateway install --force
actagent doctor
```

To remove a persisted wrapper, clear `ACTAGENT_WRAPPER` while reinstalling:

```bash
ACTAGENT_WRAPPER= actagent gateway install --force
actagent gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active ACTAgent work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the ACTAgent-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `ACTAGENT_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `ACTAGENT_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_actagent-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `actagent.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
- `gatewayPort` (WebSocket port, usually `19199`)
...
```bash
actagent gateway discover --timeout 4000
actagent gateway discover --json | jq '.beacons[].wsUrl'
```

---
summary: "CLI reference for `actagent message` (send + channel actions)"
read_when:
...
```
actagent message <subcommand> [flags]
```
...
- `actagent message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, ACTAgent will attempt a live directory lookup when the provider supports it.
...
- `actagent message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, ACTAgent does not force a `default` account SecretRef scope
...
```
actagent message send --channel discord \
...
```bash
actagent message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
actagent message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
actagent message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
actagent message poll --channel msteams \
...
```
actagent message react --channel slack \
...
```
actagent message react --channel signal \
...
```
actagent message send --channel telegram --target @mychat --message "Choose:" \
...
```
actagent message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
actagent message send --channel telegram --target @mychat \

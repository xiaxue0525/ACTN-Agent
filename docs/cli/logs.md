---
summary: "CLI reference for `actagent logs` (tail gateway logs via RPC)"
read_when:
...
`actagent logs` also accepts the standard Gateway client flags:
...
actagent logs --follow --interval 2000
actagent logs --limit 500 --max-bytes 500000
actagent logs --json
actagent logs --plain
actagent logs --no-color
actagent logs --limit 500
actagent logs --local-time
actagent logs --utc
actagent logs --follow --local-time
actagent logs --url ws://127.0.0.1:19199 --token "$ACTAGENT_GATEWAY_TOKEN"
```

## Notes

- Timestamps render in your local timezone by default. Use `--utc` for UTC output.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `actagent logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
- `actagent logs --follow` does not follow configured-file fallbacks after implicit local Gateway RPC failures. On Linux, it uses the active user-systemd Gateway journal by PID when available and prints the selected log source; otherwise it keeps retrying the live Gateway instead of tailing a potentially stale side-by-side file.

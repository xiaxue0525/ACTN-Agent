---
summary: "ACTAgent CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `actagent` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`actagent` is the main CLI entry point. Each core command has either a
...
- `actagent setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `actagent onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `actagent configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `actagent channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Legacy aliases       | [`daemon`](/cli/daemon) (gateway service) · [`actagentbot`](/cli/actagentbot) (namespace)                                                                                                                                                         |
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.actagent-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.actagent-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`actagent update`](/cli/update) (source installs only) |
...
```
actagent [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
[`actagent workboard`](/cli/workboard) or `actagent voicecall`.
...
`actagent status --usage` and the Control UI surface provider usage/quota when

---
summary: "CLI reference for `actagent node` (headless node host)"
read_when:
...
- `--port <port>`: Gateway WebSocket port (default: `19199`)
...
`actagent node run` and `actagent node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `ACTAGENT_GATEWAY_TOKEN` / `ACTAGENT_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `actagent.json` config
key.
`actagent node install` persists it into the supervised node service when it is
...
```bash
actagent node install --host <gateway-host> --port 19199
```
...
```bash
actagent node status
actagent node start
actagent node stop
actagent node restart
actagent node uninstall
```

Use `actagent node run` for a foreground node host (no service).
...
- `actagent approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, ACTAgent prepares a canonical `systemRunPlan`

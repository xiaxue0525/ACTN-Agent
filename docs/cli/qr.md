---
summary: "CLI reference for `actagent qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
actagent qr
actagent qr --setup-code-only
actagent qr --json
actagent qr --remote
actagent qr --url wss://gateway.example/ws
```
...
- With `--remote`, ACTAgent requires either `gateway.remote.url` or

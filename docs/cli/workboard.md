---
summary: "CLI reference for `actagent workboard` cards, dispatch, and worker runs"
read_when:
...
`actagent workboard` is the terminal surface for the bundled
...
```bash
actagent plugins enable workboard
...
```bash
actagent workboard list [--board <id>] [--status <status>] [--json]
actagent workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
actagent workboard show <id> [--json]
actagent workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
actagent workboard list
actagent workboard list --board default --status ready
actagent workboard list --json
```
...
```bash
actagent workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
actagent workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
actagent workboard show 7f4a2c10
actagent workboard show 7f4a2c10 --json
```
...
```bash
actagent workboard dispatch
actagent workboard dispatch --json
actagent workboard dispatch --url http://127.0.0.1:19199 --token "$ACTAGENT_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local ACTAgent state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `actagent` command when you need a different state root.
...
```bash
actagent plugins inspect workboard --runtime --json
```
...
Then retry `actagent workboard dispatch`. Data-only fallback is useful for local
...
```bash
actagent workboard list --status ready
```

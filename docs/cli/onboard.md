---
summary: "CLI reference for `actagent onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want ACTAgent to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How ACTAgent onboarding fits together.
  </Card>
...
actagent onboard --flow quickstart
actagent onboard --flow manual
actagent onboard --flow import
actagent onboard --import-from hermes --import-source ~/.hermes
actagent onboard --skip-bootstrap
actagent onboard --mode remote --remote-url wss://gateway-host:19199
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh ACTAgent setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `actagent onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `actagent` also starts the classic
onboarding flow. Once a config file has authored settings, bare `actagent`
...
`ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
ACTAGENT_LOCALE=zh-CN actagent onboard
```
...
ACTAgent marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export ACTAGENT_GATEWAY_TOKEN="your-token"
actagent onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `actagent gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `actagent dashboard` (Control UI, no channel setup).
...
actagent configure
actagent agents add <name>
```

Use `actagent setup` instead when you only need the baseline config/workspace. Use `actagent configure` later for targeted changes and `actagent channels add` for channel-only setup.

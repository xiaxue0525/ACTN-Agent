---
summary: "CLI reference for `actagent cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `actagent cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>

## Create jobs quickly

`actagent cron create` is an alias for `actagent cron add`. For new jobs, put the schedule first and the prompt second:
...
`actagent cron list` and `actagent cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `actagent cron show`
and `actagent cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `actagent cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Note: cron jobs, pending runtime state, and run history live in the shared SQLite state database. Legacy `jobs.json`, `jobs-state.json`, and `runs/*.jsonl` files are imported once and renamed with a `.migrated` suffix. After import, edit schedules with `actagent cron add|edit|remove` instead of editing JSON files.

### Manual runs

`actagent cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
actagent cron run <job-id>
actagent cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
actagent cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, ACTAgent passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a cron run `skipped`.

`actagent doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
actagent cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
actagent cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
actagent cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
actagent cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
actagent cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
actagent cron list
actagent cron list --agent ops
actagent cron get <job-id>
actagent cron show <job-id>
actagent cron run <job-id>
actagent cron run <job-id> --due
actagent cron run <job-id> --wait --wait-timeout 10m
actagent cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
actagent cron runs --id <job-id> --limit 50
actagent cron runs --id <job-id> --run-id <run-id>
```

`actagent cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`actagent cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
actagent cron edit <job-id> --agent ops
actagent cron edit <job-id> --clear-agent
actagent cron edit <job-id> --session current
actagent cron edit <job-id> --session "session:daily-brief"
```

`actagent cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
actagent cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
actagent cron edit <job-id> --webhook "https://example.invalid/actagent/cron"
actagent cron edit <job-id> --best-effort-deliver
actagent cron edit <job-id> --no-best-effort-deliver
actagent cron edit <job-id> --no-deliver
```

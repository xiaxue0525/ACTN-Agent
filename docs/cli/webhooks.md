---
summary: "CLI reference for `actagent webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into ACTAgent
...
```bash
actagent webhooks gmail setup --account <email> [...]
actagent webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the ACTAgent webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and ACTAgent webhook delivery.

```bash
actagent webhooks gmail setup --account you@example.com
actagent webhooks gmail setup --account you@example.com --project my-gcp-project --json
actagent webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### ACTAgent delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | ACTAgent webhook URL.                      |
| `--hook-token <token>` | (none)  | ACTAgent webhook token.                    |
...
```bash
actagent webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, ACTAgent delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| ACTAgent delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |

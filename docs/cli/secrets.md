---
summary: "CLI reference for `actagent secrets` (reload, audit, configure, apply)"
read_when:
...
Use `actagent secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
actagent secrets reload
actagent secrets reload --json
actagent secrets reload --url ws://127.0.0.1:19199 --token <token>
```
...
Scan ACTAgent state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `actagent.json` refs)
...
```bash
actagent secrets audit
actagent secrets audit --check
actagent secrets audit --json
...
```bash
actagent secrets configure
actagent secrets configure --plan-out /tmp/actagent-secrets-plan.json
actagent secrets configure --apply --yes
actagent secrets configure --providers-only
actagent secrets configure --skip-provider-setup
actagent secrets configure --agent ops
actagent secrets configure --json
```
...
- `configure` targets secret-bearing fields in `actagent.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, ACTAgent fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
actagent secrets apply --from /tmp/actagent-secrets-plan.json --json
```
...
- `actagent.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.actagent/.env` known secret keys whose values were migrated

---
summary: "CLI reference for `actagent models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
actagent models set <model-or-alias>
actagent models scan
```

`actagent models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise ACTAgent falls back to matching OAuth/API-key
...
the command uses `ACTAGENT_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For OpenAI ChatGPT/Codex OAuth troubleshooting, `actagent models status`,
`actagent models auth list --provider openai`, and
`actagent config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, ACTAgent resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, ACTAgent
...
By default ACTAgent tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ACTAGENT_AGENT_DIR`)
...
- `no_model`: provider auth exists, but ACTAgent could not resolve a probeable
...
```bash
actagent models auth add
actagent models auth list [--provider <id>] [--json]
actagent models auth login --provider <id>
actagent models auth login --provider openai --profile-id openai:work
actagent models auth paste-api-key --provider <id>
actagent models auth setup-token --provider <id>
actagent models auth paste-token
```
...
`actagent plugins list` to see which providers are installed.
Use `actagent models auth --agent <id> <subcommand>` to write auth results to a
...
usually as a backup for Codex subscription limits. Run `actagent doctor --fix`
to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
actagent models auth login --provider openai --set-default
actagent models auth login --provider openai --method api-key
actagent models auth paste-api-key --provider openai
...
- `paste-api-key` accepts API keys generated elsewhere, prompts for the key
  ...
  `printf "%s\n" "$OPENAI_API_KEY" | actagent models auth paste-api-key --provider openai`.
...
- Anthropic note: Anthropic staff told us ACTAgent-style Claude CLI usage is allowed again, so ACTAgent treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported ACTAgent token path, but ACTAgent now prefers Claude CLI reuse and `claude -p` when available.

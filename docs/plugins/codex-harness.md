---
summary: "Run ACTAgent embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the bundled Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to ACTAgent
---

The bundled `codex` plugin lets ACTAgent run embedded OpenAI agent turns
through Codex app-server instead of the built-in ACTAgent harness.
...
app-server execution. ACTAgent still owns chat channels, session files, model
selection, ACTAgent dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
When no ACTAgent sandbox is active, ACTAgent starts Codex app-server threads
...
ACTAgent dynamic tools continue through the app-server `item/tool/call` bridge.
Active ACTAgent sandboxing and restricted tool policies disable native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[ACTAgent code mode](/reference/code-mode), which is an opt-in QuickJS-WASI
runtime for generic ACTAgent runs with a different `exec` input shape.
...
- ACTAgent with the bundled `codex` plugin available.
...
- Codex auth available through `actagent models auth login --provider openai`,
...
Most users who want Codex in ACTAgent want this path: sign in with a
...
```bash
actagent models auth login --provider openai
```
...
harness options in ACTAgent config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                              |
| -------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | ACTAgent config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | ACTAgent config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | ACTAgent agent config              |
| Sign in with ChatGPT/Codex OAuth       | `actagent models auth login --provider openai`                                   | CLI auth profile                   |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + ACTAgent config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | ACTAgent model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "actagent"` with normal OpenAI auth          | ACTAgent model/provider config     |
...
Codex compacts through its native app-server thread state, so ACTAgent ignores
those local summarizer overrides at runtime and `actagent doctor --fix` removes
...
`plugins.slots.contextEngine: "lossless-actagent"` and
`plugins.entries.lossless-actagent.config.summaryModel`, not through
`agents.defaults.compaction.provider`. `actagent doctor --fix` migrates the old
`compaction.provider: "lossless-actagent"` shape to the Lossless context-engine slot
...
the bound thread. ACTAgent does not wait for completion, impose an ACTAgent
...
turns. The API key is only an auth fallback, not a request to switch to ACTAgent or
...
- Do not use legacy Codex GPT refs in config. Run `actagent doctor --fix` to
...
- `agentRuntime.id: "actagent"` opts a provider or model into the ACTAgent
...
| Direct OpenAI API-key traffic through ACTAgent       | Provider or model `agentRuntime.id: "actagent"` and normal OpenAI auth | `/status` shows ACTAgent runtime        | Use only when ACTAgent is intentional |
| Legacy config                                        | legacy Codex GPT refs                                                  | `actagent doctor --fix` rewrites it     | Do not write new config this way      |
...
With Codex forced, ACTAgent fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts ACTAgent's managed Codex binary locally with stdio
...
implicit YOLO posture, ACTAgent selects allowed guardian permissions instead.
When an ACTAgent sandbox is active for the session, ACTAgent disables Codex
...
through ACTAgent sandbox-backed dynamic tools such as `sandbox_exec` and
`sandbox_process` when the normal exec/process tools are available.

Use normalized ACTAgent exec mode when you want Codex native auto-review before
...
For Codex app-server sessions, ACTAgent maps `tools.exec.mode: "auto"` to Codex
...
In `tools.exec.mode: "auto"`, ACTAgent does not preserve legacy unsafe Codex
...
works, but `tools.exec.mode: "auto"` is the normalized ACTAgent surface.
...
supports ACTAgent text commands.
...
- `/codex resume <thread-id>` attaches the current ACTAgent session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `actagent doctor --fix` to migrate older
...
When ACTAgent sees a ChatGPT subscription-style Codex auth profile, it removes
...
If a subscription profile hits a Codex usage limit, ACTAgent records the reset
...
For local stdio app-server launches, ACTAgent sets `CODEX_HOME` to a per-agent
...
default. ACTAgent preserves the normal process `HOME`; Codex-run subprocesses
...
ACTAgent removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. ACTAgent does not expose
...
ACTAgent integration tools such as messaging, media, cron, browser, nodes,
gateway, `heartbeat_respond`, and `web_search` are available through Codex tool
search under the `actagent` namespace, keeping the initial model context
smaller.
...
ACTAgent or ACP delegation is still available through the `actagent` dynamic
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put ACTAgent dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional ACTAgent dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after ACTAgent builds its inherited environment. ACTAgent keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                 |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. ACTAgent dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                 |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                        |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while ACTAgent waits for `turn/completed`.                                                                                                                                                                                       |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, or post-tool raw assistant progress while ACTAgent waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget. |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an ACTAgent sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the ACTAgent sandbox egress setting.        |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                 |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                    |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an ACTAgent sandbox-backed Codex environment with Codex app-server 0.132.0 or newer so native Codex execution can run inside the active ACTAgent sandbox.                                                                                                                            |

ACTAgent-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90 second
ACTAgent watchdog by default. A positive per-call `timeoutMs` argument extends
...
budgets are capped at 600000 ms. On timeout, ACTAgent aborts the tool signal
...
After Codex accepts a turn, and after ACTAgent responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, ACTAgent best-effort
interrupts the Codex turn, records a diagnostic timeout, and releases the
ACTAgent session lane so follow-up chat messages are not queued behind a stale
...
longer post-tool idle budget: after ACTAgent returns an `item/tool/call`
...
without `turn/completed`, ACTAgent best-effort interrupts the native turn and
...
timeouts still retire the stuck app-server client and release the ACTAgent
...
- `ACTAGENT_CODEX_APP_SERVER_BIN`
- `ACTAGENT_CODEX_APP_SERVER_ARGS`
- `ACTAGENT_CODEX_APP_SERVER_MODE=yolo|guardian`
- `ACTAGENT_CODEX_APP_SERVER_APPROVAL_POLICY`
- `ACTAGENT_CODEX_APP_SERVER_SANDBOX`

`ACTAGENT_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`ACTAGENT_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`ACTAGENT_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the ACTAgent harness turn. ACTAgent
does not translate Codex plugins into synthetic `codex_plugin_*` ACTAgent
dynamic tools.
...
Thread app config is computed when ACTAgent establishes a Codex harness session
...
The short version: ACTAgent does not vendor the desktop-control app or execute
...
- ACTAgent dynamic tools are supported. Codex asks ACTAgent to execute those
  tools, so ACTAgent remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  ACTAgent can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. ACTAgent keeps a transcript mirror for channel
  history, search, `/new`, `/reset`, and future model or harness switching, but
  it does not replace Codex compaction with an ACTAgent or context-engine
  summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching ACTAgent provider/model settings.
- `tool_result_persist` applies to ACTAgent-owned transcript tool results, not
...
**ACTAgent uses the built-in harness instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their ACTAgent host:

```bash
(
  pattern='openai/gpt-5\.[45]|openai[-]codex|agentRuntime(\.id)?|harnessRuntime|Runtime: OpenAI Codex|legacy OpenAI Codex prefix|resolveSelectedOpenAIRuntimeProvider|candidateProvider[": ]+openai|status[": ]+401|Incorrect API key|No API key|api-key path|API-key path|OAuth'

  if ls /tmp/actagent/actagent-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/actagent/actagent-*.log 2>/dev/null || true
  else
    journalctl --user -u actagent-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `actagent doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because ACTAgent tests the
...
the Codex thread is still trying to use a native hook relay id that ACTAgent no
...
app-server or ACTAgent Gateway so old threads are dropped and native hook

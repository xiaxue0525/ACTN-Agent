---
summary: "Step-by-step guide to building a messaging channel plugin for ACTAgent"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect ACTAgent to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects ACTAgent to a
...
<Info>
  If you have not built any ACTAgent plugin before, read
...
Channel plugins do not need their own send/edit/react tools. ACTAgent keeps one
...
`defineChannelMessageAdapter` from `actagent/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `actagent/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`actagent/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `actagent/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `actagent/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `actagent/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `actagent/plugin-sdk/approval-auth-runtime`
- `actagent/plugin-sdk/approval-client-runtime`
- `actagent/plugin-sdk/approval-delivery-runtime`
- `actagent/plugin-sdk/approval-gateway-runtime`
- `actagent/plugin-sdk/approval-handler-adapter-runtime`
- `actagent/plugin-sdk/approval-handler-runtime`
- `actagent/plugin-sdk/approval-native-runtime`
- `actagent/plugin-sdk/approval-reply-runtime`
- `actagent/plugin-sdk/channel-runtime-context`

Likewise, prefer `actagent/plugin-sdk/setup-runtime`,
`actagent/plugin-sdk/setup-runtime`,
`actagent/plugin-sdk/reply-runtime`,
`actagent/plugin-sdk/reply-dispatch-runtime`,
`actagent/plugin-sdk/reply-reference`, and
`actagent/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `actagent/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `actagent/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `actagent/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `actagent.setupEntry` in
...
- use the broader `actagent/plugin-sdk/setup` seam only when you also need the
...
- `actagent/plugin-sdk/account-core`,
  `actagent/plugin-sdk/account-id`,
  `actagent/plugin-sdk/account-resolution`, and
  `actagent/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `actagent/plugin-sdk/inbound-envelope` and
  `actagent/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `actagent/plugin-sdk/channel-targets` for target parsing helpers
- `actagent/plugin-sdk/outbound-media` for media loading and
  `actagent/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `actagent/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `actagent/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `actagent/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `actagent/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `actagent/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `actagent/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`actagent/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#actagent-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/actagent-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "actagent": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect ACTAgent to Acme Chat."
        }
      }
    }
    ```

    ```json actagent.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { ACTAgentConfig } from "actagent/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "actagent/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so ACTAgent
  ...
  <Step title="Add a setup entry">
    ...
    ACTAgent loads this instead of the full entry when the channel is disabled
    ...
    `actagent/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    ACTAgent. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to ACTAgent.
...
├── package.json              # actagent.channel metadata
├── actagent.plugin.json      # Manifest with config schema

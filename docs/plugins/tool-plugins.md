---
summary: "Build simple typed agent tools with defineToolPlugin and actagent plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple ACTAgent plugin that only adds agent tools
...
Tool plugins add agent-callable tools to ACTAgent without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want ACTAgent to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `actagent plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `actagent.plugin.json` and `package.json` metadata with
   `actagent plugins build`.
...
- `actagent >=2026.5.17`, the first ACTAgent version that exports
  `actagent/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `actagent.plugin.json`, and
  `package.json`.
...
- `actagent.plugin.json`: generated manifest metadata for the initial tool.
...
`actagent plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so ACTAgent can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the ACTAgent tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and ACTAgent to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, ACTAgent uses a strict empty object
...
ACTAgent reads plugin config from the plugin entry in the Gateway config. Do not
...
ACTAgent discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `actagent plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells ACTAgent which
...
For the simple tool-plugin workflow, `actagent plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "actagent.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
- `actagent.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `actagent.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate ACTAgent checkout or installed CLI, install the package path:

```bash
actagent plugins install ./stock-quotes
actagent plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
actagent plugins install npm-pack:./actagent-plugin-stock-quotes-0.1.0.tgz
actagent plugins inspect stock-quotes --runtime --json
```
...
Publish through ACTAgentHub when the package is ready:

```bash
actagenthub package publish your-org/stock-quotes --dry-run
actagenthub package publish your-org/stock-quotes
```

Install with an explicit ACTAgentHub locator:

```bash
actagent plugins install actagenthub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ACTAgentHub
is the preferred discovery and distribution surface for ACTAgent plugins.
...
`actagent plugins build --entry ./dist/index.js` or
`actagent plugins validate --entry ./dist/index.js`.
...
### `actagent.plugin.json generated metadata is stale`
...
Commit both `actagent.plugin.json` and `package.json` changes.

### `package.json actagent.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`actagent plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `actagent plugins inspect <plugin-id> --runtime`
2. `actagent plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `actagent.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `actagent.extensions: ["./dist/index.js"]`.
...
- [ACTAgentHub publishing](/actagenthub/publishing)

...
If the active memory plugin exposes shared recall artifacts, ACTAgent can search
...
currently exposing public bridge inputs yet. Run `actagent wiki doctor` first,
...
`actagent wiki status`, `actagent wiki doctor`, and `actagent wiki bridge
...
actagent wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha

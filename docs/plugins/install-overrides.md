...
[`actagent plugins install`](/cli/plugins).
...
```bash
export ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export ACTAGENT_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/actagent-codex-2026.5.8.tgz",
  "actagent-web-search": "npm:@actagent/web-search@2026.5.8"
}'
```
...
ACTAgent uses the override source instead of the catalog, bundled, or default
...
entry normally represents an ACTAgent-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches ACTAgent.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal ACTAgent state:

```bash
npm pack extensions/codex --pack-destination /tmp

ACTAGENT_STATE_DIR="$(mktemp -d)" \
ACTAGENT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
ACTAGENT_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/actagent-codex-2026.5.8.tgz"}' \
pnpm actagent onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$ACTAGENT_STATE_DIR/npm/projects" -path '*/node_modules/@actagent/codex/package.json' -print
grep -R '"@actagent/codex"' "$ACTAGENT_STATE_DIR/npm/projects"/*/package-lock.json
```

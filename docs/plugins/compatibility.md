...
read_when:
  - You maintain an ACTAgent plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

ACTAgent keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core ACTAgent repo as a separate
...
```sh
actagent-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. ACTAgent
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `actagent` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against ACTAgent plugin packages.
Run it from a clean ACTAgent checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @actagent/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @actagent/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @actagent/plugin-inspector@0.1.0 -- <actagenthub-plugin-dir> --json"
```
...
ACTAgent should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `actagent/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `actagent/extension-api`,
  `actagent/plugin-sdk/channel-runtime`, `actagent/plugin-sdk/command-auth`
  status builders, `actagent/plugin-sdk/test-utils` (replaced by focused
  `actagent/plugin-sdk/*` test subpaths), and the `ACTAgentBotConfig` /
  `ACTAgentSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `actagent/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `actagent plugins registry --refresh` and
  `actagent doctor --fix`

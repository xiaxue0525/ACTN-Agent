---
summary: "Testing utilities and patterns for ACTAgent plugins"
...
Reference for test utilities, patterns, and lint enforcement for ACTAgent
plugins.
...
These test-helper subpaths are repo-local source entrypoints for ACTAgent's own
...
**Plugin API mock import:** `actagent/plugin-sdk/plugin-test-api`

**Agent runtime contract import:** `actagent/plugin-sdk/agent-runtime-test-contracts`

**Channel contract import:** `actagent/plugin-sdk/channel-contract-testing`

**Channel test helper import:** `actagent/plugin-sdk/channel-test-helpers`

**Channel target test import:** `actagent/plugin-sdk/channel-target-testing`

**Plugin contract import:** `actagent/plugin-sdk/plugin-test-contracts`

**Plugin runtime test import:** `actagent/plugin-sdk/plugin-test-runtime`

**Provider contract import:** `actagent/plugin-sdk/provider-test-contracts`

**Provider HTTP mock import:** `actagent/plugin-sdk/provider-http-test-mocks`

**Environment/network test import:** `actagent/plugin-sdk/test-env`

**Generic fixture import:** `actagent/plugin-sdk/test-fixtures`

**Node builtin mock import:** `actagent/plugin-sdk/test-node-mocks`

Inside the ACTAgent repo, prefer the focused subpaths below for new bundled
plugin tests. The broad
`actagent/plugin-sdk/testing` barrel is legacy compatibility only.
...
import { installCommonResolveTargetErrorCases } from "actagent/plugin-sdk/channel-target-testing";
import { AUTH_PROFILE_RUNTIME_CONTRACT } from "actagent/plugin-sdk/agent-runtime-test-contracts";
...
import { describePluginRegistrationContract } from "actagent/plugin-sdk/plugin-test-contracts";
...
import { getProviderHttpMocks } from "actagent/plugin-sdk/provider-http-test-mocks";
import { withEnv, withFetchPreconnect, withServer } from "actagent/plugin-sdk/test-env";
import {
  bundledPluginRoot,
  createCliRuntimeCapture,
  typedCases,
} from "actagent/plugin-sdk/test-fixtures";
import { mockNodeBuiltinModule } from "actagent/plugin-sdk/test-node-mocks";
```
...
suites that depend on bundled ACTAgent inventory stay under `src/plugins/contracts`.
...
import type { MockFn, PluginRuntime, RuntimeEnv } from "actagent/plugin-sdk/plugin-test-runtime";
```
...
ACTAgent's loader acceptance gates. Add at least one loader-backed smoke test
...
Prefer the shared plugin runtime mock from `actagent/plugin-sdk/channel-test-helpers`
...
1. **No monolithic root imports** -- `actagent/plugin-sdk` root barrel is rejected
...
ACTAgent uses Vitest with V8 coverage thresholds. For plugin tests:

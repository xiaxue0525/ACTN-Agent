...
The plugin is published to npm and is not bundled into the ACTAgent runtime image.
...
Set `dimensions` for non-standard embedding models. ACTAgent knows the
...
`memory_forget` query path, and `actagent ltm search`. Auto-recall prefers the
...
```bash
actagent ltm list
actagent ltm search "project preferences"
actagent ltm stats
```
...
```bash
actagent ltm query --cols id,text,createdAt --limit 20
actagent ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.actagent/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.actagent/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/actagent",
          storageOptions: {
...
ACTAgent treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade ACTAgent and restart the
Gateway.
...
```bash
actagent ltm stats
actagent ltm search "recent preference"
```

...
read_when:
  - You want an ACTAgent agent to join a Google Meet call
  - You want an ACTAgent agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for ACTAgent — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured ACTAgent agent answers, and regular ACTAgent TTS speaks into Meet.
...
provider plus regular ACTAgent TTS. OpenAI is the default transcription
...
```bash
actagent googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `actagent googlemeet setup --json` for
...
```bash
actagent googlemeet setup --transport twilio
```
...
```bash
actagent googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
actagent googlemeet create --transport chrome-node --mode agent
```
...
```bash
actagent googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. ACTAgent uses the
  ...
  the ACTAgent Chrome profile on the node to already be signed in to Google.
...
ACTAgent's microphone/camera permission grant and avoid the Meet **Use
...
Use `actagent googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in ACTAgent browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by ACTAgent. For
...
You do **not** need a full ACTAgent Gateway or model API key inside a macOS VM
...
- Gateway host: ACTAgent Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: ACTAgent CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update ACTAgent in the VM, then enable the bundled plugin there:

```bash
actagent plugins enable google-meet
```

Start the node host in the VM:

```bash
actagent node run --host <gateway-host> --port 19199 --display-name parallels-macos
```
...
```bash
ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1 \
  actagent node run --host <gateway-lan-ip> --port 19199 --display-name parallels-macos
```
...
```bash
ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1 \
  actagent node install --host <gateway-lan-ip> --port 19199 --display-name parallels-macos --force
actagent node restart
```

`ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`actagent.json` setting. `actagent node install` stores it in the LaunchAgent
...
```bash
actagent googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, ACTAgent browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, ACTAgent auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `actagent node run` in the VM,
  approve pairing, and make sure `actagent plugins enable google-meet` and
  `actagent plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses ACTAgent
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. ACTAgent
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by ACTAgent; use separate virtual devices or Loopback-style routing
...
ACTAgent does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with ACTAgent, review BlackHole's
...
Chrome transport opens the Meet URL through ACTAgent browser control and joins
as the signed-in ACTAgent browser profile. On macOS, the plugin checks for
...
```bash
actagent googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
actagent googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local ACTAgent audio
...
meeting; ACTAgent does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an ACTAgent agent. Be brief.",
...
actagent plugins list | grep -E 'google-meet|voice-call'
actagent googlemeet setup
```
...
`ACTAGENT_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes ACTAgent requests:
...
`meetings.space.readonly` lets ACTAgent resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets ACTAgent pass `SpaceConfig` settings such as
...
```bash
actagent googlemeet auth login --json
```
...
```bash
ACTAGENT_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ACTAGENT_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
actagent googlemeet auth login --json
```
...
ACTAGENT_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ACTAGENT_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
actagent googlemeet auth login --json --manual
```
...
support existed, rerun `actagent googlemeet auth login --json` so the refresh
...
```bash
actagent googlemeet doctor --oauth --json
```
...
```bash
actagent googlemeet doctor --oauth --create-space --json
actagent googlemeet create --no-join --json
```
...
```bash
actagent googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
actagent googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `actagent googlemeet auth login
...
- `ACTAGENT_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `ACTAGENT_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `ACTAGENT_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `ACTAGENT_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `ACTAGENT_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `ACTAGENT_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `ACTAGENT_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
actagent googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
actagent googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
actagent googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
actagent googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
actagent googlemeet latest --today
actagent googlemeet calendar-events --today --json
actagent googlemeet artifacts --event "Weekly sync"
actagent googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
actagent googlemeet latest --meeting https://meet.google.com/abc-defg-hij
actagent googlemeet artifacts --conference-record conferenceRecords/abc123 --json
actagent googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
actagent googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
ACTAgent accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops ACTAgent's local/session
...
```bash
actagent googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
actagent googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
actagent googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
actagent googlemeet export --conference-record conferenceRecords/abc123 \
...
ACTAGENT_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
actagent googlemeet setup --transport chrome-node --mode transcribe
actagent googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `ACTAGENT_LIVE_TEST=1` enables guarded live tests.
- `ACTAGENT_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `ACTAGENT_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `ACTAGENT_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `ACTAGENT_GOOGLE_MEET_CLIENT_SECRET`,
  `ACTAGENT_GOOGLE_MEET_ACCESS_TOKEN`, and
  `ACTAGENT_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `ACTAGENT_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the ACTAgent browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the ACTAgent browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, ACTAgent reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured ACTAgent TTS provider.
...
- `chrome.guestName: "ACTAgent Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through ACTAgent browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  ACTAgent agent in a per-meeting sub-agent session, and spoken back through the
  normal ACTAgent TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `actagent_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `actagent_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional ACTAgent agent id for
  `actagent_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `actagent_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured ACTAgent
agent produces the answer, and regular ACTAgent TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means ACTAgent did
...
transcripts are routed through the configured ACTAgent agent, and the answer is
spoken through the normal ACTAgent TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured ACTAgent agent | Normal ACTAgent TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal ACTAgent tools, it can call `actagent_agent_consult`.

The consult tool runs the regular ACTAgent agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
ACTAgent sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated ACTAgent agent workspace, model defaults,
...
```bash
actagent googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `actagent logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
actagent plugins list | grep google-meet
actagent googlemeet setup
```
...
```bash
actagent plugins enable google-meet
actagent plugins enable browser
ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1 \
  actagent node run --host <gateway-lan-ip> --port 19199 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; ACTAgent
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, ACTAgent
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `ACTAGENT_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `actagent googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the ACTAgent Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. ACTAgent should click **Use microphone** or, for
...
```bash
actagent googlemeet setup
actagent googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> ACTAgent agent -> TTS talk-back path,
...
run `actagent googlemeet status --json <session-id>` after participants speak
...
utterance but ACTAgent did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  ACTAgent. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
actagent googlemeet recover-tab
actagent googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
actagent googlemeet setup --transport twilio
actagent voicecall setup
actagent voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
actagent voicecall smoke --to "+15555550123"
```
...
```bash
actagent voicecall smoke --to "+15555550123" --yes
```
...
- Run `actagent googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `actagent voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `actagent voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `actagent logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `actagent googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `actagent logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: ACTAgent owns the

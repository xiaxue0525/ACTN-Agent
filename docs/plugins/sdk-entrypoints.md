...
when ACTAgent loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, ACTAgent will use a
...
**Import:** `actagent/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the ACTAgent tool-result format, and
exposes static metadata that `actagent plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, ACTAgent uses a strict empty object
...
- Tool names are static. `actagent plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `actagent.plugin.json` and `package.json` `actagent.extensions`; ACTAgent does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `actagent/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `ACTAgentPluginConfigSchema \| () => ACTAgentPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: ACTAgentPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `actagent.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- ACTAgent resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `actagent/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: ACTAgentPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: ACTAgentPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. ACTAgent may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and ACTAgent
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `actagent nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; ACTAgent resolves it to the
...
ACTAgent loads this instead of the full entry when a channel is disabled,
...
- `actagent/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `actagent/plugin-sdk/channel-setup` for optional-install setup surfaces
- `actagent/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`actagent/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so ACTAgent can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want ACTAgent to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; ACTAgent rejects descriptor names outside
...
ACTAgent classifies loaded plugins by their registration behavior:
...
Use `actagent plugins inspect <id>` to see a plugin's shape.

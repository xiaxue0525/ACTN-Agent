...
read_when:
  ...
  - You are defining plugin config schemas or package.json actagent metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`actagent.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `actagent` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/actagent-my-channel",
      "version": "1.0.0",
      "type": "module",
      "actagent": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ACTAgentHub baseline">
    ```json actagent-actagenthub-package.json
    {
      "name": "@myorg/actagent-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "actagent": {
        "extensions": ["./index.ts"],
...
<Note>
If you publish the plugin externally on ACTAgentHub, those `compat` and `build` fields are required. The canonical publish snippets live in `docs/snippets/plugin-publish/`.
</Note>
...
`actagent.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`actagent.install` is package metadata, not manifest metadata.

| Field                        | Type                                | What it means                                                                     |
| ---------------------------- | ----------------------------------- | --------------------------------------------------------------------------------- |
| `actagenthubSpec`                | `string`                            | Canonical ACTAgentHub spec for install/update and onboarding install-on-demand flows. |
...
| `defaultChoice`              | `"actagenthub"` \| `"npm"` \| `"local"` | Preferred install source when multiple sources are available.                     |
| `minHostVersion`             | `string`                            | Minimum supported ACTAgent version in the form `>=x.y.z` or `>=x.y.z-prerelease`. |
...
<AccordionGroup>
  <Accordion title="Onboarding behavior">
    Interactive onboarding also uses `actagent.install` for install-on-demand surfaces. If your plugin exposes provider auth choices or channel setup/catalog metadata before runtime loads, onboarding can show that choice, prompt for ACTAgentHub, npm, or local install, install or enable the plugin, then continue the selected flow. ACTAgentHub onboarding choices use `actagenthubSpec` and are preferred when present; npm choices require trusted catalog metadata with a registry `npmSpec`; exact versions and `expectedIntegrity` are optional npm pins. If `expectedIntegrity` is present, install/update flows enforce it for npm. Keep the "what to show" metadata in `actagent.plugin.json` and the "how to install it" metadata in `package.json`.
  </Accordion>
  ...
  <Accordion title="Pinned npm installs">
    ...
    ```json
    {
      "actagent": {
        "install": {
          "npmSpec": "@wecom/wecom-actagent-plugin@1.2.3",
...
When enabled, ACTAgent loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `actagent.plugin.json` in the package root. ACTAgent uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to ACTAgent",
  "configSchema": {
    "type": "object",
...
## ACTAgentHub publishing

For plugin packages, use the package-specific ACTAgentHub command:
...
<Note>
The legacy skill-only publish alias is for skills. Plugin packages should always use `actagenthub package publish`.
</Note>

## Setup entry

The `setup-entry.ts` file is a lightweight alternative to `index.ts` that ACTAgent loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).

```typescript
// setup-entry.ts
import { defineSetupPluginEntry } from "actagent/plugin-sdk/channel-core";
...
CLI wizard locale (`ACTAGENT_LOCALE`, then system locale variables) and falls
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so ACTAgent can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "actagent/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `actagent.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `actagent onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "actagent/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `actagent/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `actagent/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `actagent/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "actagent/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/actagent-my-channel",
      docsPath: "/channels/my-channel",
    });
...
**External plugins:** publish to [ACTAgentHub](/actagenthub), then install:

<Tabs>
  <Tab title="npm">
    ```bash
    actagent plugins install @myorg/actagent-my-plugin
    ```

    Bare package specs install from npm during the launch cutover.

  </Tab>
  <Tab title="ACTAgentHub only">
    ```bash
    actagent plugins install actagenthub:@myorg/actagent-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    Use npm when a package has not moved to ACTAgentHub yet, or when you need a
    direct npm install path during migration:

    ```bash
    actagent plugins install npm:@myorg/actagent-my-plugin
    ```
...
```bash
actagent plugins install <package-name>
```

<Info>
For npm-sourced installs, `actagent plugins install` installs the package into a per-plugin project under `~/.actagent/npm/projects` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>

<Note>
Gateway startup does not install plugin dependencies. npm/git/ACTAgentHub install flows own dependency convergence; local plugins must already have their dependencies installed.
</Note>

Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged ACTAgent startup never repairs or mirrors plugin dependencies.

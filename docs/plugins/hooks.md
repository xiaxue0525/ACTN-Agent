...
Plugin hooks are in-process extension points for ACTAgent plugins. Use them
...
current plugin options; ACTAgent injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when ACTAgent has resolved the child session's native model before launch.
...
use `actagent sessions` or the Gateway session/status surfaces. When debugging
...
- ACTAgent strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, ACTAgent stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when ACTAgent can
...
identifier when ACTAgent can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and ACTAgent continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when ACTAgent can derive a
...
Codex native `Stop` hooks are relayed into this hook as ACTAgent
...
reach the next model turn exactly once. ACTAgent drains queued injections before
...
compatibility, but ACTAgent no longer runs built-in install-time dangerous-code
...
context when syncing external wake schedulers, and keep ACTAgent as the

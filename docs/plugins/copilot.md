---
summary: "Run ACTAgent embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / actagent / copilot) and want it to run through the Copilot CLI
---

The external `@actagent/copilot` plugin lets ACTAgent run embedded subscription
...
ACTAgent still owns chat channels, session files, model selection, ACTAgent
...
[Side questions (`/btw`)](#side-questions-btw)), and `actagent doctor`.
...
- ACTAgent with the `@actagent/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@actagent/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `~/.actagent/agents/<agentId>/copilot` for full per-agent isolation. The
...
`actagent doctor` runs the plugin
...
The Copilot runtime is an external plugin so the core `actagent` package does
...
```bash
actagent plugins install @actagent/copilot
```
...
Without the opt-in, actagent uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@actagent/copilot`
   package.
2. The well-known fallback dir `~/.actagent/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `ACTAGENT_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the ACTAgent harness without disturbing
...
(isolating SDK state from ACTAgent's `models.json` / `auth-profiles.json` in
the same directory), or `~/.actagent/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, ACTAgent uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  ACTAgent's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `infiniteSessionConfig` — optional override for the SDK
  ...
  leave as-is.
- `hooksConfig` — optional bridge config exposing ACTAgent
...
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged ACTAgent tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through ACTAgent's
...
Nothing in the rest of ACTAgent needs to know about these fields. Other
...
The ACTAgent side transcript mirror (see below) continues to receive the
...
deliberately leaves `harness.runSideQuestion` undefined, so ACTAgent's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/actagent/actagent/blob/main/extensions/copilot/harness.test.ts)
...
that hosts (including `actagent doctor`) can call to verify the environment:
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the ACTAgent channel/TUI prompt path; the
...
Permission enforcement for bridged ACTAgent tools happens **inside the
...
`createACTAgentCodingTools` to every coding tool: loop detection,
...
`createACTAgentCodingTools` — identity (`senderIsOwner`,

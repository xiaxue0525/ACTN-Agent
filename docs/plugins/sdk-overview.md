...
read_when:
  ...
  - You want a reference for all registration methods on ACTAgentPluginApi
...
<Note>
  This page is for plugin authors using `actagent/plugin-sdk/*` inside
  ACTAgent. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [ACTAgent App SDK](/concepts/actagent-sdk) and the `@actagent/sdk` package
  instead.
</Note>
...
prefer `actagent/plugin-sdk/channel-core`; keep `actagent/plugin-sdk/core` for
...
`actagent.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. ACTAgent's
...
<Warning>
  ...
  `actagent/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`actagent/plugin-sdk/discord` and `actagent/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `ACTAgentPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main ACTAgent prompt.", surfaces: ["actagent_main"] },
];
```

Structured `surfaces` may include `actagent_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `actagent_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `actagent nodes` |
...
targeted runtime, for example `["actagent", "codex"]`. External plugins
cannot register this middleware; keep normal ACTAgent plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. ACTAgent calls the
...
`actagent nodes canvas` explicit plugin-owned node features.
...
- User config still wins. ACTAgent merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping ACTAgent thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `actagent/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep ACTAgent as the source of truth for due checks and execution.
...
| `api.config`             | `ACTAgentConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `actagent/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when ACTAgent is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through ACTAgent's plugin
...
- **`@actagent/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@actagent/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `actagent/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `actagent/plugin-sdk/speech`, `.../provider-model-shared`, or another

...
At startup, ACTAgent does roughly this:
...
The manifest is the control-plane source of truth. ACTAgent uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, ACTAgent
...
ACTAgent does not cache plugin discovery results or direct manifest registry
...
ACTAgent still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, ACTAgent calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that ACTAgent no longer calls, such as
...
| --  | _(built-in model lookup)_         | ACTAgent tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared ACTAgent refreshers                                                                                          |
...
that still runs on ACTAgent's normal inference loop.

`resolveUsageAuth` decides whether ACTAgent should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of ACTAgent's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as ACTAgent adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while ACTAgent owns the
...
- ACTAgent only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "ACTAgent plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-actagent-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-actagent-scopes` only when the header is explicitly present
  - if `x-actagent-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-actagent-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `actagent/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `actagent/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `actagent/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `actagent/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `actagent/plugin-sdk/config-schema` | Root `actagent.json` Zod schema (`ACTAgentSchema`) |
...
<Info>
`actagent/plugin-sdk/channel-runtime`, `actagent/plugin-sdk/channel-lifecycle`,
small channel helper facades, `actagent/plugin-sdk/outbound-runtime`,
`actagent/plugin-sdk/outbound-send-deps`, `actagent/plugin-sdk/config-runtime`,
and `actagent/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `actagent/plugin-sdk/*` subpaths. Never
...
`actagent/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape ACTAgent writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to ACTAgent's
...
- if both `catalog` and `discovery` are registered, ACTAgent uses `catalog`
...
- Read-only command paths such as `actagent status`, `actagent status --all`,
  `actagent channels status`, `actagent channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `actagent.extensions`:
...
Security guardrail: every `actagent.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `actagent plugins install` installs plugin dependencies with a
...
Optional: `actagent.setupEntry` can point at a lightweight setup-only module.
When ACTAgent needs setup surfaces for a disabled channel plugin, or
...
Optional: `actagent.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let ACTAgent load the
...
Channel plugins can advertise setup/discovery metadata via `actagent.channel` and
install hints via `actagent.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@actagent/nextcloud-talk",
  "actagent": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@actagent/nextcloud-talk",
...
Useful `actagent.channel` fields beyond the minimal example:
...
ACTAgent can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.actagent/mpm/plugins.json`
- `~/.actagent/mpm/catalog.json`
- `~/.actagent/plugins/catalog.json`

Or point `ACTAGENT_PLUGIN_CATALOG_PATHS` (or `ACTAGENT_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "actagent": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `actagent.install` block. The
...
export default function (api) {
  api.registerContextEngine("lossless-actagent", (ctx) => ({
    info: { id: "lossless-actagent", name: "Lossless actagent", ownsCompaction: true },
    async ingest() {
...
2. add typed plugin registration/runtime surfaces
   Extend `ACTAgentPluginApi` and/or `api.runtime` with the smallest useful
...
This is how ACTAgent stays opinionated without becoming hardcoded to one

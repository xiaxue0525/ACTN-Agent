...
read_when:
  - You want to place an outbound voice call from ACTAgent
...
Voice calls for ACTAgent via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        actagent plugins install @actagent/voice-call
        ```
      </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `actagent doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `actagent doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `actagent_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal ACTAgent tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `actagent_agent_consult`.
...
configured ACTAgent agent without paying a full agent-consult round trip on
...
`actagent_agent_consult` still run the full ACTAgent agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call actagent_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `actagent doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
actagent voicecall call --to "+15555550123" --message "Hello from ACTAgent"
actagent voicecall start --to "+15555550123"   # alias for call
actagent voicecall continue --call-id <id> --message "Any questions?"
actagent voicecall speak --call-id <id> --message "One moment"
actagent voicecall dtmf --call-id <id> --digits "ww123456#"
actagent voicecall end --call-id <id>
actagent voicecall status --call-id <id>
actagent voicecall tail
actagent voicecall latency                      # summarize turn latency from logs
actagent voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL ACTAgent reconstructs
...
Use `actagent logs --follow` for the live phase trace. A healthy Twilio Meet
...
`actagent voicecall tail` still shows persisted call records; it is useful for
...
- `actagent logs --follow` shows realtime TwiML served, the realtime bridge

...
The plugin is included with ACTAgent, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for ACTAgent clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    actagent plugins enable admin-http-rpc
...
```bash
actagent plugins disable admin-http-rpc
actagent gateway restart
```
...
- For `token` and `password` auth, narrower `x-actagent-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-actagent-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for ACTAgent clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.

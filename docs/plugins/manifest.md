...
read_when:
  - You are building an ACTAgent plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native ACTAgent plugin manifest** only.
...
ACTAgent auto-detects those bundle layouts too, but they are not validated
against the `actagent.plugin.json` schema described here.

For compatible bundles, ACTAgent currently reads bundle metadata plus declared
...
ACTAgent runtime expectations.

Every native ACTAgent plugin **must** ship a `actagent.plugin.json` file in the
**plugin root**. ACTAgent uses this manifest to validate configuration
...
`actagent.plugin.json` is the metadata ACTAgent reads **before it loads your
...
- QA runner metadata the shared `actagent qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; ACTAgent still reads this during the deprecation window.                                                             |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                      |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that ACTAgent can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                        |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `actagent qa` host before plugin runtime loads.                                                                                                                                                  |
...
ACTAgent reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so ACTAgent can
...
If a tool has no `toolMetadata`, ACTAgent preserves the existing behavior and
...
ACTAgent reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, ACTAgent falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. ACTAgent
...
the shared `actagent qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `actagent qa`, for example `matrix`.    |
...
ACTAgent also includes `setup.providers[].envVars` in generic provider auth and
...
ACTAgent can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. ACTAgent treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `actagent.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
ACTAgent reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that ACTAgent can
read without importing the plugin runtime.

```json
{
  "contracts": {
    "agentToolResultMiddleware": ["actagent", "codex"],
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `actagent migrate`.                                         |
...
ACTAgent reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, ACTAgent can still load the plugin, but
...
the same defaults through `package.json#actagent.channel.commands` alongside
...
When `channels.chat` is configured, ACTAgent considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, ACTAgent disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, ACTAgent
...
Use `modelSupport` when ACTAgent should infer your provider plugin from
...
ACTAgent applies this precedence:
...
Use `modelCatalog` when ACTAgent should know provider model metadata before
...
provider-filtered list uses an alias, ACTAgent can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching ACTAgent model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when ACTAgent
...
exec provider preset. ACTAgent reads this metadata before plugin runtime loads,
...
from the managed plugin install roots, such as git and ACTAgentHub installs.
...
The map key is the integration id. If `providerAlias` is omitted, ACTAgent uses
...
When an operator selects the preset, ACTAgent writes a provider reference like:
...
At startup/reload, ACTAgent resolves that provider by loading current plugin
...
ACTAgent materializes it at startup/reload to the current Node executable and
...
ACTAgent derives `trustedDirs` for manifest presets from the plugin root and,
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the ACTAgent provider id, for example `z-ai` for a `zai` provider. |
...
### ACTAgent Provider Index

The ACTAgent Provider Index is ACTAgent-owned preview metadata for providers
...
4. ACTAgent Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `actagent doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `actagent.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `actagent` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if ACTAgent must know it before loading plugin code, put it in `actagent.plugin.json`
...
`actagent` block instead of `actagent.plugin.json`.
`actagent.bundle` and `actagent.bundle.json` are not ACTAgent plugin contracts;
native plugins must use `actagent.plugin.json` plus the supported
`package.json#actagent` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `actagent.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `actagent.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `actagent.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `actagent.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `actagent.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `actagent.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `actagent.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `actagent.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `actagent.install.actagenthubSpec` / `actagent.install.npmSpec` / `actagent.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `actagent.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `actagent.install.minHostVersion`                                                          | Minimum supported ACTAgent host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `actagent.compat.pluginApi`                                                                | Minimum ACTAgent plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `actagent.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `actagent.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `actagent.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#actagent.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `actagent.plugin.json`.

`actagent.install.minHostVersion` is enforced during install and manifest
...
`actagent.compat.pluginApi` is enforced during package install for non-bundled
plugin sources. Use it for the ACTAgent plugin SDK/runtime API floor that the
...
flows. Official ACTAgent release sync bumps existing official plugin API floors
to the ACTAgent release version by default, but plugin-only releases can keep a
lower floor when the package intentionally supports older hosts. Do not use the
package version alone as the compatibility contract. `peerDependencies.actagent`
remains npm package metadata; ACTAgent uses the `actagent.compat.pluginApi`
contract for install compatibility decisions.

Official install-on-demand metadata should use `actagenthubSpec` when the plugin is
published on ACTAgentHub; onboarding treats that as the preferred remote source and
records ACTAgentHub artifact facts after install. `npmSpec` remains the compatibility
fallback for packages that have not moved to ACTAgentHub yet.

Exact npm version pinning already lives in `npmSpec`, for example
`"npmSpec": "@wecom/wecom-actagent-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `actagent.setupEntry` when status, channel list,
...
entrypoint fields. For example, `actagent.runtimeExtensions` cannot make an
escaping `actagent.extensions` path loadable.

`actagent.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `actagent doctor --fix`.

`actagent.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`actagent.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
ACTAgent discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with ACTAgent
3. **Global install** — plugins installed into the global ACTAgent plugin root
...
- The manifest is **required for native ACTAgent plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `ACTAgentPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.

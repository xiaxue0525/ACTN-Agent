...
read_when:
  - You see the ACTAGENT_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ACTAGENT_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before ACTAgent 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external ACTAgent plugin
---

ACTAgent has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`actagent/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`actagent/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`actagent/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`actagent/extension-api`** - a bridge that gave plugins direct access to
...
ACTAgent does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`actagent/plugin-sdk/\<subpath\>`)
...
`actagent/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `actagent doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `actagent/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `ACTAgentConfig` | `actagent/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `actagent/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `actagent/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `actagent/plugin-sdk/config-mutation` |
    | Session store helpers | `actagent/plugin-sdk/session-store-runtime` |
    | Markdown table config | `actagent/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `actagent/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `actagent/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `actagent/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // ACTAgent and Codex runtime dynamic tools
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `actagent/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `actagent/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "actagent/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "actagent/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "actagent/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "actagent/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "actagent/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "actagent/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `actagent/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `actagent/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `actagent/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `actagent/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `actagent/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `actagent/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `actagent/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `actagent/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `actagent/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `actagent/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `actagent/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `actagent/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `actagent/plugin-sdk/error-runtime` |
    | Transport readiness waits | `actagent/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `actagent/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `actagent/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `actagent/plugin-sdk/number-runtime` |
    | Process-local async lock | `actagent/plugin-sdk/async-lock-runtime` |
    | File locks | `actagent/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `actagent/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `ACTAgentSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | ACTAgent-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@actagent/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`actagent/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`actagent/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "actagent/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "actagent/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `actagent/plugin-sdk/channel-inbound` or
    `actagent/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `actagent/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `actagent/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `actagent/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `actagent/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    ACTAgent no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. ACTAgent downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="ACTAgentSchemaType alias → ACTAgentConfig">
    `ACTAgentSchemaType` re-exported from `actagent/plugin-sdk` is now a
    one-line alias for `ACTAgentConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { ACTAgentSchemaType } from "actagent/plugin-sdk";
    // After
    import type { ACTAgentConfig } from "actagent/plugin-sdk/config-schema";
    ```
...
```bash
ACTAGENT_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 actagent gateway run
ACTAGENT_SUPPRESS_EXTENSION_API_WARNING=1 actagent gateway run
```

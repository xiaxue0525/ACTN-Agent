---
summary: "How ACTAgent installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged ACTAgent installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

ACTAgent keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the ACTAgent
package directory.
...
- SDK/core imports are peer or supplied ACTAgent imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into ACTAgent-owned package roots

ACTAgent owns only the plugin lifecycle:
...
ACTAgent uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.actagent/npm/projects/<encoded-package>`
- git packages clone under `~/.actagent/git`
...
```bash
cd ~/.actagent/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`actagent plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. ACTAgent reads the tarball's npm
...
`node_modules` beside the plugin package. ACTAgent scans the managed project
...
publishable lockfile during install, and ACTAgent's managed npm project root
supports it through the normal npm install path. ACTAgent-owned publishable
...
ACTAgent does not require it for community packages, but npm will respect it
when present.

ACTAgent-owned npm plugin packages can also publish with explicit
...
with `actagent.release.bundleRuntimeDependencies: false`; those packages still
...
`actagent` package does not bundle its full dependency tree.

Plugins that import `actagent/plugin-sdk/*` declare `actagent` as a peer
dependency. ACTAgent does not let npm install a separate registry copy of the
...
resolution/materialization and ACTAgent reasserts plugin-local
`node_modules/actagent` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. ACTAgent does not
...
actagent plugins install <source>
actagent doctor --fix
```

`doctor --fix` can clean legacy ACTAgent-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of ACTAgent.
They should either have no heavy runtime dependency tree or be moved out to a
downloadable package on ACTAgentHub/npm.
...
the same npm/git/ACTAgentHub path as third-party plugins.

In source checkouts, ACTAgent treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g actagent`        | Built runtime tree inside the package | ACTAgent package and explicit plugin install/update/doctor flows     |
...
| `actagent plugins install ...`   | Managed npm project/git/ACTAgentHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older ACTAgent versions generated bundled-plugin dependency roots at startup or
...
`.actagent-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.actagent/npm/node_modules` root.

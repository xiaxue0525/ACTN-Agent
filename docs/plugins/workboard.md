...
ACTAgent Gateway; it is not a replacement for GitHub Issues, Linear, Jira, or
...
state directory and move with the rest of that Gateway's ACTAgent state.
...
under the ACTAgent state directory. Boards, cards, labels, lifecycle events,
...
`actagent doctor --fix` to migrate the shipped legacy plugin-state namespaces
...
system processes; normal ACTAgent subagent sessions still own execution. The
...
ACTAgent records the orchestration intent and exposes it in worker context; the
...
actagent workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
actagent workboard show <card-id>
actagent workboard dispatch
```

`actagent workboard dispatch` calls the running Gateway so worker starts use the
...
```bash
actagent plugins disable workboard
actagent gateway restart
```

---
summary: "Create your first ACTAgent plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new ACTAgent plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend ACTAgent without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the ACTAgent repository. Publish
the package to [ACTAgentHub](/actagenthub) and users install it with:
...
`actagenthub:` prefix when you want ACTAgentHub resolution.
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because ACTAgent loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect ACTAgent to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through ACTAgent model fallback.
  </Card>
...
```json actagent.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to ACTAgent",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so ACTAgent can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    actagent plugins inspect my-plugin --runtime --json
    ```
    ...
    `actagent demo-plugin ping`.

    For a bundled plugin in this repository, ACTAgent discovers source-checkout
  ...
  <Step title="Publish">
    ...
    The canonical ACTAgentHub snippets live in `docs/snippets/plugin-publish/`.

  </Step>

  <Step title="Install">
    Install the published package through ACTAgentHub:

    ```bash
    actagent plugins install actagenthub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified ACTAgent runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so ACTAgent can avoid
...
```typescript
import { definePluginEntry } from "actagent/plugin-sdk";
```
...
`actagent/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `actagent` metadata</Check>
<Check>**actagent.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [actagent/actagent](https://github.com/actagent/actagent/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official ACTAgent X account [@actagent](https://x.com/actagent) for release announcements.

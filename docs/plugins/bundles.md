---
summary: "Install and use Codex, Claude, and Cursor bundles as ACTAgent plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how ACTAgent maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

ACTAgent can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
ACTAgent maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native ACTAgent plugins. Native plugins run
...
of requiring authors to rewrite them as native ACTAgent plugins, ACTAgent
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    actagent plugins install ./my-bundle

    # Archive
    actagent plugins install ./my-bundle.tgz
...
## What ACTAgent maps from bundles

Not every bundle feature runs in ACTAgent today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal ACTAgent skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | ACTAgent-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded ACTAgent settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded ACTAgent LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded ACTAgent defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal ACTAgent skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal ACTAgent skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal ACTAgent hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded ACTAgent

- enabled bundles can contribute MCP server config
- ACTAgent merges bundle MCP config into the effective embedded ACTAgent settings as
  `mcpServers`
- ACTAgent exposes supported bundle MCP tools during embedded ACTAgent agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, ACTAgent uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in ACTAgent config. `actagent mcp set` and `actagent doctor --fix` normalize the common alias.
...
ACTAgent registers bundle MCP tools with provider-safe names in the form
...
#### Embedded ACTAgent settings

- Claude `settings.json` is imported as default embedded ACTAgent settings when the
  bundle is enabled
- ACTAgent sanitizes shell override keys before applying them
...
#### Embedded ACTAgent LSP

- enabled Claude bundles can contribute LSP server config
- ACTAgent loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded ACTAgent LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `actagent plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but ACTAgent does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit ACTAgent best when they use skill roots and ACTAgent-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded ACTAgent settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded ACTAgent
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded ACTAgent LSP defaults
...
ACTAgent checks for native plugin format first:

1. `actagent.plugin.json` or valid `package.json` with `actagent.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, ACTAgent uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `actagent plugins install` and ship everything
  they need in the installed plugin directory.
- ACTAgent-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `actagent doctor --fix` removes legacy staged dependency directories and can
...
- ACTAgent does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `actagent plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded ACTAgent settings from `settings.json` are supported. ACTAgent does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    ACTAgent hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>

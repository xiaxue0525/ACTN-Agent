...
read_when:
  ...
  - You need to map a third-party CLI into ACTAgent's text fallback runner
---

CLI backend plugins let ACTAgent call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points ACTAgent at the plugin runtime module              |
| Manifest ownership   | `actagent.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `actagent.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json actagent.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through ACTAgent",
    ...
    `cliBackends` is the runtime ownership list. It lets ACTAgent auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "actagent/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how ACTAgent should launch and parse the CLI:
...
| `modelAliases`                            | Map ACTAgent model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields ACTAgent reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific ACTAgent auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into ACTAgent's loopback MCP tool bridge           |
| `ownsNativeCompaction`             | Backend owns its own compaction - ACTAgent defers      |

Keep these hooks provider-owned. Do not add CLI-specific branches to core when a
backend hook can express the behavior.

### `ownsNativeCompaction`: opting out of ACTAgent compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so ACTAgent's safeguard summarizer never runs against its
...
stay over budget / go stale (ACTAgent no longer rescues it):
...
CLI backends do not receive ACTAgent tools by default. If the CLI can consume an
...
"always-on"` so ACTAgent can fail closed when a caller requires no native tools.
...
```bash
actagent plugins inspect acme-cli --runtime --json
actagent agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `actagent.extensions` and built runtime entries for published packages</Check>
<Check>`actagent.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>

---
summary: "Bundled `oc-path` plugin: ships the `actagent path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`actagent path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the ACTAgent repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `actagent path … --json` instead of carrying separate markdown, JSONC,
...
actagent path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
actagent path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `actagent` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/actagent.plugin.json`:
...
`actagent path …`, so installs that never use the verb pay no cost.
...
state. Bare `actagent path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
actagent plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `actagent path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__ACTAGENT_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`actagent path` CLI reference](/cli/path)

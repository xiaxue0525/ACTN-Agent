---
summary: "Set up Codex Computer Use for Codex-mode ACTAgent agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode ACTAgent agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. ACTAgent
...
Use this page when ACTAgent is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## ACTAgent.app and Peekaboo

ACTAgent.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want ACTAgent.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode ACTAgent agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an ACTAgent node and exposes mobile
...
ACTAgent-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through ACTAgent's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the ACTAgent command:

```bash
cua-driver mcp-config --client actagent
```
...
available as a normal ACTAgent MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. ACTAgent
...
Computer Use in and lets ACTAgent install or re-enable it before the turn:
...
With this config, ACTAgent checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, ACTAgent asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, ACTAgent also tries to
...
plugin command surface is available. These are ACTAgent chat/runtime commands,
not `actagent codex ...` CLI subcommands:
...
ACTAgent uses the same app-server API that Codex itself exposes. The
...
During install, ACTAgent polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, ACTAgent prefers
...
`computer-use` is registered, ACTAgent tries to add the standard bundled
...
## What ACTAgent checks

ACTAgent reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If ACTAgent says Computer Use
...
ACTAgent intentionally fails closed when `computerUse.enabled` is true. A
...
fresh ACTAgent session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active ACTAgent relay through the local bridge or
Gateway fallback. Start a fresh ACTAgent session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or ACTAgent Gateway so old threads

...
By default, ACTAgent starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after ACTAgent builds its inherited environment.                                                                                                                                                                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active ACTAgent sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows ACTAgent sandbox egress.                                                                                                          |
...
unattended ACTAgent turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, ACTAgent treats the implicit default as guardian
...
When an ACTAgent sandbox is active, the local Codex app-server process still
runs on the Gateway host. ACTAgent therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the ACTAgent sandbox
backend. Shell access is exposed through ACTAgent sandbox-backed dynamic tools
...
`workspace-write` without active ACTAgent sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`actagent doctor` and fix the reported host namespace policy for the ACTAgent
...
The stable default is fail-closed: active ACTAgent sandboxing disables native
...
try Codex's remote environment support with ACTAgent's sandbox backend. This
...
When the flag is on and the current ACTAgent session is sandboxed, ACTAgent
...
ACTAgent-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so ACTAgent rejects
that combination.
...
1. An explicit ACTAgent Codex auth profile for the agent.
...
Stdio app-server launches inherit ACTAgent's process environment by default.
ACTAgent owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's ACTAgent state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the ACTAgent agent
instead of leaking in from the operator's personal `~/.codex` home.

ACTAgent does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `actagent`, `gh`, `git`, cloud CLIs, and shell commands see
...
ACTAgent plugins and ACTAgent skill snapshots still flow through ACTAgent's own
...
part of an ACTAgent agent, inventory them explicitly:

```bash
actagent migrate codex --dry-run
actagent migrate apply codex --yes
```
...
Most remaining ACTAgent integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `actagent` namespace. This keeps the initial
...
surface, while explicit ACTAgent or ACP delegation is still available through
the `actagent` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, ACTAgent aborts the
...
post-tool idle budget: after ACTAgent returns an `item/tool/call` response, after
...
`turn/completed`, ACTAgent best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when ACTAgent
...
If discovery fails or times out, ACTAgent uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. ACTAgent
...
For ACTAgent workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
ACTAgent Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact ACTAgent skills
...
`BOOTSTRAP.md` when present is forwarded as ACTAgent turn input reference
context.

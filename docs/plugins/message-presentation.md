...
Message presentation is ACTAgent's shared contract for rich outbound chat UI.
...
```bash
actagent message send --channel slack \
  --target channel:C123 \
...
```bash
actagent message send --channel telegram \
...
Use helpers from `actagent/plugin-sdk/interactive-runtime` when bridging older
code:

...
- With `gateway.tls.enabled: true`, use `https://127.0.0.1:19199/` and
  `wss://127.0.0.1:19199` for the WebSocket endpoint.
...
- Re-open anytime: `actagent dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `actagent dashboard` still prints the
  clean URL and tells you to use the token from `ACTAGENT_GATEWAY_TOKEN` or
...
- **Localhost**: open `http://127.0.0.1:19199/`.
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `ACTAGENT_GATEWAY_TOKEN`); `actagent dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `actagent dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `actagent dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `ACTAGENT_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `actagent status`; remote: SSH tunnel `ssh -N -L 19199:127.0.0.1:19199 user@host` then open `http://127.0.0.1:19199/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `actagent config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `ACTAGENT_GATEWAY_TOKEN` in this shell, then rerun `actagent dashboard`
  - No shared secret configured: `actagent doctor --generate-gateway-token`

...
- default: `http://<host>:19199/`
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/actagent`)
...
- [http://127.0.0.1:19199/](http://127.0.0.1:19199/) (or [http://localhost:19199/](http://localhost:19199/))

If the page fails to load, start the Gateway first: `actagent gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `actagent devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. ACTAgent keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `actagent devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.

Paperclip agents that connect through the `actagent_gateway` adapter use the same first-run approval flow. After the initial connection attempt, run `actagent devices approve --latest` to preview the pending request, then rerun the printed `actagent devices approve <requestId>` command to approve it. Pass explicit `--url` and `--token` values for a remote gateway. To keep approvals stable across restarts, configure a persistent `adapterConfig.devicePrivateKeyPem` in Paperclip instead of letting it generate a new ephemeral device identity each run.
...
Imported themes are stored only in the current browser profile. They are not written to gateway config and do not sync across devices. Replacing the imported theme updates the one local slot; clearing it switches the active theme back to actagent if the imported theme was selected.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.actagent/actagent.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - Deprecated fallback: run `actagent doctor --fix` to migrate stored legacy jobs with `notify: true` from `cron.webhook` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for ACTAgent-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `actagent mcp doctor --probe` when you need live server proof.
...
7. Run `actagent mcp status --verbose`, `actagent mcp doctor --probe`, or `actagent mcp reload` from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop actagent`, `please stop`) to abort out-of-band.
...
If the page shows **Protocol mismatch** right after an ACTAgent update, first reopen the dashboard with `actagent dashboard` and hard-refresh the page. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `push/vapid-keys.json` (under the ACTAgent state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `ACTAGENT_VAPID_PUBLIC_KEY`
- `ACTAGENT_VAPID_PRIVATE_KEY`
- `ACTAGENT_VAPID_SUBJECT` (defaults to `https://actagent.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    actagent gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```

    Then open:

    - `http://<tailscale-ip>:19199/` (or your configured `gateway.controlUi.basePath`)
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, ACTAgent **blocks** Control UI connections without device identity.
...
- `http://127.0.0.1:19199/` (on the gateway host)
...
- `GET /__actagent__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
ACTAGENT_CONTROL_UI_BASE_PATH=/actagent/ pnpm ui:build
```
...
Then point the UI at your Gateway WS URL (e.g. `ws://127.0.0.1:19199`).

## Blank Control UI page

If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<actagent-app>` is not registered after startup.
...
<Steps>
  ...
  <Step title="Open with gatewayUrl">
    ```text
    http://localhost:5173/?gatewayUrl=ws%3A%2F%2F<gateway-host>%3A19199
    ```

    Optional one-time auth (if needed):

    ```text
    http://localhost:5173/?gatewayUrl=wss%3A%2F%2F<gateway-host>%3A19199#token=<gateway-token>
    ```

...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `actagent.internal.`), **or**
...
Run `actagent devices list` again before approval.
...
actagent gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.actagent.ai`.
...
- `ACTAGENT_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `ACTAGENT_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine ACTAgent iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight ACTAgent builds.
...
```bash
export ACTAGENT_APNS_TEAM_ID="TEAMID"
export ACTAGENT_APNS_KEY_ID="KEYID"
export ACTAGENT_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.actagent/credentials/apns
chmod 700 ~/.actagent/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.actagent/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.actagent/credentials/apns/AuthKey_KEYID.p8
export ACTAGENT_APNS_PRIVATE_KEY_PATH="$HOME/.actagent/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_actagent-gw._tcp` on `local.` and, when configured, the same
...
`actagent.internal.`) and Tailscale split DNS.
See [Bonjour](/gateway/bonjour) for the CoreDNS example.

### Manual host/port

In Settings, enable **Manual Host** and enter the gateway host + port (default `19199`).
...
```bash
actagent nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:19199/__actagent__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__actagent__/canvas/` and `/__actagent__/a2ui/`.
- It is served from the Gateway HTTP server (same port as `gateway.port`, default `19199`).
...
tools; the iOS app exposes iPhone and iPad capabilities through ACTAgent node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through ACTAgent by invoking node
...
```bash
actagent nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__actagent; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
actagent nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `actagent devices list` and approve manually.

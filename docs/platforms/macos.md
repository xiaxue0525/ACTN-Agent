---
summary: "ACTAgent macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for ACTAgent. It owns permissions,
...
- Installs the global CLI (`actagent`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `actagent gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.actagent.gateway`
(or `ai.actagent.<profile>` when using `--profile`/`ACTAGENT_PROFILE`; legacy `com.actagent.*` still unloads).
...
Replace the label with `ai.actagent.<profile>` when running a named profile.
...
The app registers the `actagent://` URL scheme for local actions.
...
```bash
open 'actagent://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **ACTAgent.app**.
...
Avoid putting your ACTAgent state dir in iCloud or other cloud-synced folders.
...
If `actagent doctor` detects state under:
...
```bash
cd apps/macos
swift run actagent-mac connect --json
swift run actagent-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `actagent gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
...
- **Local port:** the Gateway port (default `19199`), always stable.

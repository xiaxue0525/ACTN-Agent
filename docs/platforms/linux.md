...
2. `npm i -g actagent@latest`
3. `actagent onboard --install-daemon`
4. From your laptop: `ssh -N -L 19199:127.0.0.1:19199 <user>@<host>`
5. Open `http://127.0.0.1:19199/` and authenticate with the configured shared secret (token by default; password if you set `gateway.auth.mode: "password"`)
...
ACTAgent installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `actagent gateway install` and
`actagent onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/actagent-gateway[-<profile>].service`:

```
[Unit]
Description=ACTAgent Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. ACTAgent therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, ACTAgent starts the child through a short
...
- ACTAgent-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `ACTAGENT_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.

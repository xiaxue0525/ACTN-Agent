---
summary: "Run the ACTAgent Gateway on EasyRunner with Podman and Caddy"
read_when:
  - Deploying ACTAgent on EasyRunner
  - Running the Gateway behind EasyRunner's Caddy proxy
  - Choosing persistent volumes and auth for a hosted Gateway
title: "EasyRunner"
---

EasyRunner can host the ACTAgent Gateway as a small containerized app behind its
...
- A built or published ACTAgent container image.
- A persistent config volume for `/home/node/.actagent`.
...
services:
  actagent:
    image: ghcr.io/actagent/actagent:latest
    restart: unless-stopped
    environment:
      ACTAGENT_GATEWAY_TOKEN: ${ACTAGENT_GATEWAY_TOKEN}
      ACTAGENT_HOME: /home/node
      ACTAGENT_STATE_DIR: /home/node/.actagent
      ACTAGENT_CONFIG_PATH: /home/node/.actagent/actagent.json
      ACTAGENT_WORKSPACE_DIR: /workspace
    volumes:
      - actagent-config:/home/node/.actagent
      - actagent-workspace:/workspace
    labels:
      caddy: actagent.example.com
      caddy.reverse_proxy: "{{upstreams 1455}}"
    command: ["actagent", "gateway", "--bind", "lan", "--port", "1455"]
...
Replace `actagent.example.com` with your Gateway hostname. Store
`ACTAGENT_GATEWAY_TOKEN` in EasyRunner's secret/environment manager instead of
...
```bash
actagent gateway probe --url https://actagent.example.com --token <token>
actagent gateway status --url https://actagent.example.com --token <token>
```
...
- Pull or build the new ACTAgent image, then redeploy the EasyRunner app.
- Back up the `actagent-config` volume before updates.
- Back up `actagent-workspace` if agents write durable project data there.
- Run `actagent doctor` after major updates to catch config migrations and
  service warnings.
...
- Files are root-owned after restore: repair the mounted volumes so the
  container user can write `/home/node/.actagent` and `/workspace`.

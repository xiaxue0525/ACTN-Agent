...
- Same path: run the app from a fixed location (for ACTAgent, `dist/ACTAgent.app`).
...
Prefer granting Accessibility to ACTAgent.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.actagent.mac
sudo tccutil reset ScreenCapture ai.actagent.mac
...
Workaround: move files into the ACTAgent workspace (`~/.actagent/workspace`) if you want to avoid per-folder grants.

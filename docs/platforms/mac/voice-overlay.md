...
Audience: macOS app developers. Goal: keep the voice overlay predictable when wake-word and push-to-talk overlap.
...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `ai.actagent`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.actagent" AND category CONTAINS "voicewake"' --level info --style compact
  ```

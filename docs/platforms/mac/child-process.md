...
service via the external `actagent` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.actagent.gateway`
  (or `ai.actagent.<profile>` when using `--profile`/`ACTAGENT_PROFILE`; legacy `com.actagent.*` is supported).

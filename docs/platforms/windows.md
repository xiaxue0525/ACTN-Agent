...
x64 and ARM64 installers on ACTAgent releases.

Download the latest stable installer:

- [ACTAgentCompanion-Setup-x64.exe](https://github.com/actagent/actagent/releases/latest/download/ACTAgentCompanion-Setup-x64.exe)
- [ACTAgentCompanion-Setup-arm64.exe](https://github.com/actagent/actagent/releases/latest/download/ACTAgentCompanion-Setup-arm64.exe)
- [Checksums](https://github.com/actagent/actagent/releases/latest/download/ACTAgentCompanion-SHA256SUMS.txt)

After install, launch **ACTAgent Companion** from the Start menu or the system
...
`ACTAgentGateway` WSL distro, installs the Gateway inside it, and pairs the app.
...
Windows Hub can register as a first-class ACTAgent node. The agent can then use
...
actagent devices approve <request-id>
...
Windows capabilities without a running ACTAgent Gateway.
...
For terminal-first use, install ACTAgent from PowerShell:
...
```powershell
actagent onboard --non-interactive --skip-health
...
Then install ACTAgent inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled actagent-gateway.service
systemctl --user status actagent-gateway.service --no-pager
```
...
Check Task Manager for `ACTAgent.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If it is not running, launch **ACTAgent
...
```powershell
notepad "$env:LOCALAPPDATA\ACTAgentTray\Logs\Setup\easy-setup-latest.txt"
```

---
summary: "Symptom first troubleshooting hub for ACTAgent"
read_when:
  - ACTAgent is not working and you need the fastest path to a fix
...
- `actagent status` → shows configured channels and no obvious auth errors.
- `actagent status --all` → full report is present and shareable.
- `actagent gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `actagent gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `actagent doctor` → no blocking config/service errors.
- `actagent channels status --probe` → reachable gateway returns live per-account
...
- `actagent logs --follow` → steady activity, no repeating fatal errors.
...
and run `actagent status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in ACTAgent

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `actagent infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on ACTAgent agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger ACTAgent prompts crash the
...
## Plugin install fails with missing actagent extensions

If install fails with `package.json missing actagent.extensions`, the plugin package
is using an old shape that ACTAgent no longer accepts.

Fix in the plugin package:

1. Add `actagent.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `actagent plugins install <package>` again.

Example:

```json
{
  "name": "@actagent/my-plugin",
  "version": "1.2.3",
  "actagent": {
...
Install policy runs on plugin installs and updates. ACTAgent-owned plugin
versions normally move with the ACTAgent release, so an ACTAgent update can
also need matching `@actagent/*` plugin updates during post-update sync.

Avoid these broad policy shapes unless you also maintain the matching upgrade
rule:

- Freezing ACTAgent-owned plugins to one exact old version, such as allowing
  only `@actagent/*@2026.5.3`.
...
- Approving plugin versions without considering the policy request's
  `actagentVersion` and the plugin candidate metadata.

Safer policy rules allow trusted ACTAgent-owned plugin updates when the
candidate is compatible with the current ACTAgent host, instead of pinning a
single release forever. If you block npm by default, make a narrow exception
for the trusted `@actagent/*` plugin packages or plugin ids you use. If you
...
If the policy is intentionally strict, relax it for the trusted ACTAgent upgrade
window, rerun `actagent plugins update --all`, then restore the stricter rule.
...
If `actagent doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run ACTAgent as
...
```mermaid
flowchart TD
  A[ACTAgent is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `actagent gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    actagent config get tools.exec.host
    actagent config get tools.exec.security
    actagent config get tools.exec.ask
    ...
    ```bash
    actagent config set tools.exec.host gateway
    actagent config set tools.exec.security full
    actagent config set tools.exec.ask off
    actagent gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `actagent` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `actagent browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.

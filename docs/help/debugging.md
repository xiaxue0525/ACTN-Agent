...
This is handy when you need to toggle obscure settings without editing `actagent.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[actagent]"
...
Use `ACTAGENT_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
ACTAGENT_PLUGIN_LIFECYCLE_TRACE=1 actagent plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm actagent ...`
...
`ACTAGENT_RUN_NODE_CPU_PROF_DIR`:

```bash
ACTAGENT_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm actagent status
```
...
```bash
ACTAGENT_TRACE_SYNC_IO=1 pnpm actagent gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `ACTAGENT_TRACE_SYNC_IO=1` when you explicitly want Node
...
`actagent-gateway-watch-main` (or a profile/port-specific variant such as
`actagent-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t actagent-gateway-watch-main
```
...
# or
ACTAGENT_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
ACTAGENT_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`ACTAGENT_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`ACTAGENT_PROFILE`, `ACTAGENT_CONFIG_PATH`, `ACTAGENT_STATE_DIR`,
`ACTAGENT_GATEWAY_PORT`, and `ACTAGENT_SKIP_CHANNELS` into the pane. Put
...
`actagent doctor --fix --non-interactive` once and restarts the Gateway child.
Use `ACTAGENT_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `actagent.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.actagent-dev` and
...
```bash
pnpm gateway:dev
ACTAGENT_PROFILE=dev actagent tui
```

If you don't have a global install yet, run the CLI via `pnpm actagent ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `ACTAGENT_PROFILE=dev`
   - `ACTAGENT_STATE_DIR=~/.actagent-dev`
   - `ACTAGENT_CONFIG_PATH=~/.actagent-dev/actagent.json`
   - `ACTAGENT_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ACTAGENT_SKIP_CHANNELS=1`).
...
```bash
ACTAGENT_PROFILE=dev actagent gateway --dev --reset
```
...
## Raw stream logging (ACTAgent)

ACTAgent can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.actagent/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
ACTAGENT_RAW_STREAM=1
ACTAGENT_RAW_STREAM_PATH=~/.actagent/logs/raw-stream.jsonl
```

Default file:

`~/.actagent/logs/raw-stream.jsonl`
...
```bash
ACTAGENT_RAW_STREAM_PATH=~/.actagent/logs/raw-openai-completions.jsonl
```

Default file:

`~/.actagent/logs/raw-openai-completions.jsonl`
...
- If you need to use the built ACTAgent CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./actagent.mjs` or create a shell alias like `alias actagent-build="node $(pwd)/actagent.mjs"`

---
summary: "How ACTAgent validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing ACTAgent update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `actagent doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  local ACTAgentHub fixture installs and update no-ops, marketplace update behavior,
  and Claude-bundle enable/inspect. Set `ACTAGENT_PLUGINS_E2E_ACTAGENTHUB=0` to
  keep the ACTAgentHub block hermetic/offline.
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `actagent plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `actagent config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `actagent update --yes --json`, and requires the candidate update command to
...
```bash
ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC=actagent@2026.4.23 \
ACTAGENT_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC=actagent@latest \
ACTAGENT_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`ACTAGENT_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `actagent@beta`, `actagent@latest`, or an exact
  published version.
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `ACTAGENT_TRUSTED_PACKAGE_TOKEN` secret.
...
`package_acceptance_package_spec=actagent@YYYY.M.D` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published ACTAgent
...
- Registry/package source behavior: `test:docker:plugins` fixture or ACTAgentHub
  fixture server.

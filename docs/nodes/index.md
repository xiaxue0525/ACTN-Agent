...
`actagent nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`actagent node run` or the
...
actagent devices reject <requestId>
...
`actagent devices list` before approving.
...
- `node.pair.*` (CLI: `actagent nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `actagent nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.actagent/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, ACTAgent also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If ACTAgent cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
actagent node run --host <gateway-host> --port 19199 --display-name "Build Node"
```
...
```bash
# Terminal A (keep running): forward local 18790 -> gateway 127.0.0.1:19199
ssh -N -L 18790:127.0.0.1:19199 user@gateway-host

# Terminal B: export the gateway token and connect through the tunnel
export ACTAGENT_GATEWAY_TOKEN="<gateway-token>"
actagent node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `actagent node run` supports token or password auth.
- Env vars are preferred: `ACTAGENT_GATEWAY_TOKEN` / `ACTAGENT_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `ACTAGENT_GATEWAY_*` env vars.

### Start a node host (service)

```bash
actagent node install --host <gateway-host> --port 19199 --display-name "Build Node"
actagent node start
...
If the node retries with changed auth details, re-run `actagent devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `actagent node run` / `actagent node install` (persists in `~/.actagent/node.json` on the node).
- `actagent nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
actagent approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
actagent approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.actagent/exec-approvals.json`.
...
```bash
actagent config set tools.exec.host node
actagent config set tools.exec.security allowlist
actagent config set tools.exec.node "<id-or-name>"
```
...
```bash
actagent nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
actagent nodes canvas snapshot --node <idOrNameOrIp> --format png
actagent nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
actagent nodes canvas present --node <idOrNameOrIp> --target https://example.com
actagent nodes canvas hide --node <idOrNameOrIp>
actagent nodes canvas navigate https://example.com --node <idOrNameOrIp>
actagent nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
actagent nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
actagent nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
actagent nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
actagent nodes camera list --node <idOrNameOrIp>
actagent nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
actagent nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
actagent nodes camera clip --node <idOrNameOrIp> --duration 10s
actagent nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
actagent nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
actagent nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
actagent nodes location get --node <idOrNameOrIp>
actagent nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
actagent nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from ACTAgent"}'
```
...
```bash
actagent nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
actagent nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
actagent nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
actagent nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
actagent nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
actagent nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.actagent/exec-approvals.json`).
...
```bash
actagent config set tools.exec.node "node-id-or-name"
```
...
```bash
actagent config unset tools.exec.node
actagent config unset 'agents.list[0].tools.exec.node'
```
...
ACTAgent can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
actagent node run --host <gateway-host> --port 19199
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.actagent/node.json`.
- Exec approvals are enforced locally via `~/.actagent/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `ACTAGENT_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `ACTAGENT_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `actagent nodes …` works against this Mac).

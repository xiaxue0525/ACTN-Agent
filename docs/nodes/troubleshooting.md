...
```bash
actagent nodes describe --node <idOrNameOrIp>
actagent nodes canvas snapshot --node <idOrNameOrIp>
actagent logs --follow
```
...
actagent nodes status
actagent approvals get --node <idOrNameOrIp>
actagent approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`actagent approvals get --node ...`), not in the gateway pairing record.

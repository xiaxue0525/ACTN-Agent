...
ACTAgent supports **camera capture** for agent workflows:
...
```bash
actagent nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
actagent nodes camera snap --node <id> --facing front
actagent nodes camera clip --node <id> --duration 3000
actagent nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`actagent.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `actagent` CLI to invoke camera commands on the macOS node.

Examples:

```bash
actagent nodes camera list --node <id>            # list camera ids
actagent nodes camera snap --node <id>            # prints saved path
actagent nodes camera snap --node <id> --max-width 1280
actagent nodes camera snap --node <id> --delay-ms 2000
actagent nodes camera snap --node <id> --device-id <id>
actagent nodes camera clip --node <id> --duration 10s          # prints saved path
actagent nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
actagent nodes camera clip --node <id> --device-id <id>
actagent nodes camera clip --node <id> --no-audio
```

Notes:

- `actagent nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
actagent nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```

...
- `consultThinkingLevel`: optional thinking level override for the full ACTAgent agent run behind realtime `actagent_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `actagent_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `actagent_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through ACTAgent instead.
- `realtime.instructions`: appends provider-facing system instructions to ACTAgent's built-in realtime prompt. Use it for voice style and tone; ACTAgent keeps the default `actagent_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `actagent_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `actagent-mlx-tts` helper when present, or an executable on `PATH`. Set `ACTAGENT_MLX_TTS_BIN` to point at a custom helper binary during development.

...
read_when:
  ...
  - Enabling the plugin-tools or ACTAgent-tools MCP bridge
...
When ACTAgent uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal ACTAgent `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by ACTAgent before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, ACTAgent/acpx cannot force a model selection.
...
Packaged installs use the official `@actagent/acpx` runtime plugin for ACP.
...
```bash
actagent plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `ACTAGENT_ACPX_RUNTIME_STARTUP_PROBE=0` or
`ACTAGENT_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the ACTAgent workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before ACTAgent passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install ACTAgent globally with `npm install -g actagent`, the acpx
...
normally and reports the missing dependency through `actagent acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose ACTAgent plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
ACTAgent plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
actagent config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `actagent-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled ACTAgent
  plugins.
...
### ACTAgent tools MCP bridge

By default, ACPX sessions also do **not** expose built-in ACTAgent tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
actagent config set plugins.entries.acpx.config.actAgentToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `actagent-tools` into ACPX session
  bootstrap.
- Exposes selected built-in ACTAgent tools. The initial server exposes `cron`.
...
```bash
actagent config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use ACTAgent agent/run timeouts, including `/acp timeout`.
...
```bash
actagent config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from ACTAgent exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between ACTAgent `tools.exec.mode`, Codex Guardian
...
<Warning>
ACTAgent defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.

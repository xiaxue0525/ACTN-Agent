...
ACTAgent supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, ACTAgent also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `actagent configure --section web`. It stores the key in
`~/.actagent/actagent.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.actagent/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).

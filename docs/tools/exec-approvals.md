...
host-local `ask: "always"` in `~/.actagent/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `actagent approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `actagent exec-policy show`                                      | Local-machine merged view.                                                             |
| `actagent exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `actagent` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, ACTAgent also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.actagent/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through ACTAgent's native auto reviewer before falling back to a human approval route.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, ACTAgent treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  ACTAgent may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in ACTAgent config
...
- In YOLO mode, ACTAgent does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when ACTAgent's effective exec
policy is YOLO. For ACTAgent-managed Claude live sessions, ACTAgent's
...
If you want a more conservative setup, tighten ACTAgent exec policy back to
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    actagent approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
actagent exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.actagent/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `actagent approvals set --gateway` or
`actagent approvals set --node <id|name|ip>`.
...
```bash
actagent approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `actagent exec-policy` does not synchronize node approvals.
- `actagent exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `actagent approvals --node ...`.
...
specific argument shape. ACTAgent evaluates the regular expression
...
entries instead of hand-editing the encoded value. If ACTAgent cannot
...
edit its local `~/.actagent/exec-approvals.json` directly.

CLI: `actagent approvals` supports gateway or node editing - see
...
ACTAgent posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, ACTAgent can still
...
When an async exec approval is denied, ACTAgent treats the host command as
...
session delivery is unavailable, ACTAgent falls back to a concise operator or

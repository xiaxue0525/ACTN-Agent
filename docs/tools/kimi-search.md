...
ACTAgent supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), ACTAgent reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.actagent/.env`.

If you omit `baseUrl`, ACTAgent defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, ACTAgent defaults to `kimi-k2.6`.
...
ACTAgent treats Kimi `web_search` as successful only after Moonshot returns
...
ACTAgent returns a structured `kimi_web_search_ungrounded` error instead of

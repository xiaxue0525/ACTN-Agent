...
<Steps>
  <Step title="Install the plugin">
    ```bash
    actagent plugins install diffs
    ```
  </Step>
...
ACTAgent includes syntax highlighting for common source, config, and documentation languages:
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/actagent-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in ACTAgent config.
  </Step>

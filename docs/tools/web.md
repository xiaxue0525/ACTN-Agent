...
ACTAgent also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    actagent configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "ACTAgent plugin SDK" });
    ```
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when ACTAgent web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of ACTAgent's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, ACTAgent keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use ACTAgent's guarded fetch path. For
trusted provider API hosts, ACTAgent allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, ACTAgent checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, ACTAgent resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `actagent models auth login
...
ACTAgent keeps startup resilient and reports a warning so you can reinstall the
plugin or run `actagent doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let ACTAgent auto-detect the first ready web-fetch
...
When you choose **Kimi** during `actagent onboard` or
`actagent configure --section web`, ACTAgent can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `actagent doctor --fix`.
When you choose Grok during `actagent onboard` or `actagent configure --section web`,
ACTAgent can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, ACTAgent does not
...
<Tabs>
  <Tab title="Config file">
    Run `actagent configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.actagent/.env`.
...
optional structured filters. ACTAgent only enables the built-in xAI `x_search`

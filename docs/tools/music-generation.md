...
For session-backed agent runs, ACTAgent starts music generation as a
...
from the completion reply, ACTAgent sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. ACTAgent still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, ACTAgent
...
Provider request timeouts are operator configuration only. ACTAgent uses
...
- **Status lookup:** `actagent tasks list` or `actagent tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** ACTAgent injects an internal completion event back
...
```bash
ACTAGENT_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```

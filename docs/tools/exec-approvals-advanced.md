...
- allowlist entries live in host-local `~/.actagent/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `actagent approvals allowlist ...`).
- `actagent security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `actagent doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, ACTAgent still rejects the `env` builtin in safe-bin
...
session, ACTAgent also resumes that session with an internal followup so the agent observes that
the command did not run instead of later repairing a missing result.

### Followup delivery behavior

After an approved async exec finishes, ACTAgent sends a followup `agent` turn to the same session.
...
the originating channel, ACTAgent keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, ACTAgent sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.actagent.approval` custom event content on the first prompt
  event so ACTAgent-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. ACTAgent first tries a private route on the
...
topics, ACTAgent preserves the topic for the approval prompt and the post-approval follow-up.

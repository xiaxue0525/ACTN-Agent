...
read_when:
  - Looking for an overview of ACTAgent's media capabilities
...
ACTAgent generates images, videos, and music, understands inbound media
...
For async tools, ACTAgent submits the request to the provider, returns a task
...
ACTAgent wakes the agent with the generated media paths so it can tell the
...
ACTAgent sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until ACTAgent has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in ACTAgent until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>

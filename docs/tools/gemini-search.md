...
ACTAgent supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.actagent/.env`.
...
`pd`, `pw`, `pm`, and `py`. ACTAgent converts these values, or an explicit

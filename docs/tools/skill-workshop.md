...
Skill Workshop is ACTAgent's governed path for creating and updating workspace
skills.
...
plugin, ACTAgentHub, extra-root, managed, personal-agent, or system skills.
...
```bash
actagent skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
```
...
- `autonomous.enabled`: allows ACTAgent to create pending proposals from durable
...
```text
<ACTAGENT_STATE_DIR>/skill-workshop/
  proposals.json
...
Default state directory: `~/.actagent`.
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `ACTAGENT_STATE_DIR`.                             |
...
- [Skills CLI](/cli/skills) for `actagent skills` commands

...
`~/.actagent/actagent.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  for WhatsApp/Telegram). Use `actagent setup --node-manager` for npm, pnpm,
...
<ParamField path="skills.install.allowUploadedArchives" type="boolean" default="false">
  ...
  archives staged through `skills.upload.*`. Normal ACTAgentHub installs do not
  need this setting.
</ParamField>
...
runs after ACTAgent has staged source material and before the install or update
continues. It applies to ACTAgentHub skills, uploaded skills, Git/local skills,
...
```json5
{
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/actagent-install-policy",
        args: ["--json"],
        timeoutMs: 10000,
        noOutputTimeoutMs: 10000,
        maxOutputBytes: 1048576,
        passEnv: ["ACTAGENT_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. ACTAgent runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the ACTAgent process into the policy
...
`actagentVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
ACTAgent does not execute install policy during normal Gateway startup. Installs
and updates fail closed when policy is enabled but unavailable. `actagent doctor`
performs static validation, and `actagent doctor --deep` executes a synthetic
...
{
  "protocolVersion": 1,
  "actagentVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../actagent-skill-actagenthub/root",
  ...
  "origin": {
    "type": "actagenthub",
    "registry": "https://actagenthub.actagent.ai",
    "slug": "weather",
    "version": "1.0.0"
  },
  "request": {
    "kind": "skill-install",
    "mode": "install",
    "requestedSpecifier": "actagenthub:weather@1.0.0"
  },
  "skill": {
...
`metadata.actagent.skillKey`, use that key instead. Quote hyphenated names
...
Managed `~/.actagent/skills` and personal `~/.agents/skills` directories

...
read_when:
  - You want ACTAgent to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current ACTAgent session.
...
ACTAgent waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal ACTAgent
usage and model controls.

## Model tools

ACTAgent exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable ACTAgent sessions, including the
...
token snapshot yet. Usage refreshes as ACTAgent records session usage and

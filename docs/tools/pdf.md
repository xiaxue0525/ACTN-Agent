...
The tool is only registered when ACTAgent can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  ACTAgent can actually authenticate that provider for the agent.
...
- `file://` URL
- `http://` and `https://` URL
- ACTAgent-managed inbound refs such as `media://inbound/<id>`
...
- Managed inbound refs and replayed paths under ACTAgent's inbound media store are allowed with workspace-only file policy.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, ACTAgent drops the rendered images and continues with the
  extracted text.

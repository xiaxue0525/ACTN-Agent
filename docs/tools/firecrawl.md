...
ACTAgent can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `actagent configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `actagent doctor --fix`.
...
when you run Firecrawl yourself. ACTAgent accepts `http://` only for loopback,
...
ACTAgent always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, ACTAgent

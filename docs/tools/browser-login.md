...
When a site requires login, **sign in manually** in the **host** browser profile (the actagent browser).
...
ACTAgent controls a **dedicated Chrome profile** (named `actagent`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `actagent` browser.
...
actagent browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `actagent`).
...
```bash
actagent browser open https://x.com --browser-profile actagent
```

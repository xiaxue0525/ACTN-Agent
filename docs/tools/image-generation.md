...
ACTAgent records a background task, returns the task id immediately, and wakes
...
generated images are still missing from the completion reply, ACTAgent sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, ACTAgent routes image requests
...
nearby geometry option instead of the exact requested one, ACTAgent remaps to
...
ACTAgent tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when ACTAgent can
  ...
  <Accordion title="Timeouts">
    ...
    honor the same timeout budget when configured, bounded by ACTAgent's 600000
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, ACTAgent reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. ACTAgent
    ...
    ACTAgent forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible ACTAgent maps
    ...
    transparency-capable OpenAI image model. ACTAgent routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    ACTAgent forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. ACTAgent sends:
    ...
    Krea; ACTAgent maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as ACTAgent-managed image attachments

    ACTAgent intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for ACTAgent image generation" size=3840x2160 count=1
```
  </Tab>
...
`actagent infer image edit`; `--openai-background` remains as an

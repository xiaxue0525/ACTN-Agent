...
summary: "ACTAgent tools, skills, and plugins overview: what agents can call and how to extend them"
read_when:
  - You want to understand what tools ACTAgent provides
...
| Search a large ACTAgent tool catalog        | [Tool Search](/tools/tool-search)              | [Tool Search](/tools/tool-search)                                                                               |
...
<Steps>
  ...
  <Step title="Use a skill when the agent needs instructions">
    ...
    Skills can live in a workspace, shared skill directory, managed ACTAgent
  ...
  <Step title="Use a plugin when ACTAgent needs a new capability">
    ...
    plugins can be installed from ACTAgentHub, npm, git, local directories, or
    archives.
...
| Large ACTAgent catalogs | Search and call many eligible tools without sending every schema to the model | `tool_search_code`, `tool_search`, `tool_describe`                   | [Tool Search](/tools/tool-search)                                                           |

<Note>
Tool Search is an experimental ACTAgent agent surface. Codex harness runs use
...
Choose the extension path by the job you need ACTAgent to do:
...
5. For delegated runs, check per-agent restrictions in
   [Per-agent sandbox and tool restrictions](/tools/multi-agent-sandbox-tools).
6. For large ACTAgent catalogs, confirm whether the run uses direct tool exposure or
...
- [Tool Search](/tools/tool-search) for compact ACTAgent tool catalog discovery

...
summary: "Build, test, and publish custom SKILL.md workspace skills for your ACTAgent agents."
read_when:
...
ACTAgent loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.actagent/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.actagent/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    ACTAgent watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ...
    ```bash
    actagent agent --message "give me a greeting"
    ```
...
metadata: { "actagent": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `actagent.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `actagent.json`:
...
actagent skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
## Publishing to ACTAgentHub

<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.actagent` gating fields
    are set. Add a `homepage` URL if you have a project page.
  </Step>
  <Step title="Install the ACTAgentHub skill">
    The ACTAgentHub skill documents the current publish command shape and required
    metadata:

    ```bash
    actagent skills install actagenthub-publish
    ```
  ...
  <Step title="Publish">
    ...
    See [ACTAgentHub — Publishing](/actagenthub/publishing) for the full flow.
...
<Tip>
  ...
  - **Test locally** — use `actagent agent --message "..."` before sharing.
  - **Use ACTAgentHub** — browse community skills at [actagenthub.ai](https://actagenthub.ai)

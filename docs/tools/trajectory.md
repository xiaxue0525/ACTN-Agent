---
summary: "Export redacted trajectory bundles for debugging an ACTAgent agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an ACTAgent session
...
Trajectory capture is ACTAgent's per-session flight recorder. It records a
...
ACTAgent writes the bundle under the workspace:

```text
.actagent/trajectory-exports/actagent-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.actagent/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, ACTAgent sends the
...
Trajectory capture is on by default for ACTAgent agent runs.
...
```json
{
  "traceSchema": "actagent-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | ACTAgent version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
ACTAgent also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `ACTAGENT_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export ACTAGENT_TRAJECTORY_DIR=/var/lib/actagent/trajectories
```

When this variable is set, ACTAgent writes one JSONL file per session id in that
directory.
...
Set `ACTAGENT_TRAJECTORY=0` before starting ACTAgent:

```bash
export ACTAGENT_TRAJECTORY=0
```
...
ACTAgent flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`ACTAGENT_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting ACTAgent:

```bash
export ACTAGENT_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when ACTAgent logs an `actagent-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `ACTAGENT_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
ACTAgent redacts sensitive values before writing export files:
...
- confirm ACTAgent was started without `ACTAGENT_TRAJECTORY=0`
- check whether `ACTAGENT_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.actagent/trajectory-exports/`

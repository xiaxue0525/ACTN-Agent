...
read_when:
  - Running ACTAgent Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, ACTAgent Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/actagent/actagent/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- ACTAgent and Chrome are on the same machine
...
- WSL2 runs the Gateway on `127.0.0.1:19199`
- Windows opens the Control UI in a normal browser at `http://127.0.0.1:19199/`
- Windows Chrome exposes a CDP endpoint on port `9222`
- WSL2 can reach that Windows CDP endpoint
- ACTAgent points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, ACTAgent is not the problem yet.
...
Fix that before touching ACTAgent config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point ACTAgent at the address that is reachable from WSL2:
...
- use HTTP(S) when you want ACTAgent to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting ACTAgent to succeed
...
```bash
actagent browser open https://example.com --browser-profile remote
actagent browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `actagent browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `actagent browser stop --browser-profile remote`
...
3. ACTAgent config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
4. Control UI: are you opening `http://127.0.0.1:19199/` instead of a LAN IP?
...
- only then debug ACTAgent config or Control UI auth

...
frontmatter and a markdown body. ACTAgent loads bundled skills plus any local
...
<CardGroup cols={2}>
  ...
  <Card title="ACTAgentHub" href="/actagenthub" icon="cloud">
...
ACTAgent loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.actagent/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. ACTAgent discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an ACTAgent
  skill root. Use `actagent migrate plan codex` to inventory those skills, then
  `actagent migrate codex` to copy them into your ACTAgent workspace.
</Note>
...
| Shared managed | `~/.actagent/skills`         | All agents on this machine  |
...
`actagent.plugin.json` (paths relative to the plugin root). Plugin skills load
...
skill overrides them. Gate them via `metadata.actagent.requires.config` on the
...
## Installing from ACTAgentHub

[ACTAgentHub](https://actagenthub.ai) is the public skills registry. Use
`actagent skills` commands for install and update, or the `actagenthub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `actagent skills install <slug>`                       |
| Install from a Git repository      | `actagent skills install git:owner/repo@ref`           |
| Install a local skill directory    | `actagent skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `actagent skills install <slug> --global`              |
| Update all workspace skills        | `actagent skills update --all`                         |
| Update a shared managed skill      | `actagent skills update <slug> --global`               |
| Update all shared managed skills   | `actagent skills update --all --global`                |
| Verify a skill's trust envelope    | `actagent skills verify <slug>`                        |
| Print the generated Skill Card     | `actagent skills verify <slug> --card`                 |
| Publish / sync via ACTAgentHub CLI     | `actagenthub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `actagent skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.actagent/skills` directory, visible to all local agents unless agent
    ...
    `actagent skills update` tracks ACTAgentHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `actagent skills verify <slug>` asks ACTAgentHub for the skill's
    `actagenthub.skill.verify.v1` trust envelope. Installed ACTAgentHub skills verify
    against the version and registry recorded in `.actagenthub/origin.json`.

    ACTAgentHub skill pages expose the latest security scan state before install,
    with detail pages for VirusTotal, ACTAgentScan, and static analysis. The
    command exits non-zero when ACTAgentHub marks verification as failed. Publishers
    recover false positives through the ACTAgentHub dashboard or
    `actagenthub skill rescan <slug>`.

  </Accordion>
  <Accordion title="Private archive installs">
    Gateway clients that need non-ACTAgentHub delivery can stage a zip skill archive
    ...
    `actagent.json`. Normal ACTAgentHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.actagent/skills` and personal `~/.agents/skills` may contain
  ...
  <Accordion title="Operator install policy">
    ...
    source path, applies to ACTAgentHub, uploaded, Git, local, update, and
...
<Note>
  ACTAgent follows the [AgentSkills](https://agentskills.io) spec. The
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, ACTAgent keeps the skill's instructions out of the agent's normal
...
ACTAgent filters skills at load time using `metadata.actagent` (single-line
JSON in the frontmatter). A skill with no `metadata.actagent` block is always
...
<ParamField path="requires.config" type="string[]">
  Each `actagent.json` path must be truthy.
</ParamField>
...
<Note>
  Legacy `metadata.actagentdbot` blocks are still accepted when
  `metadata.actagent` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, ACTAgent lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `actagent.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** ACTAgent does not auto-install Homebrew or translate brew
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.actagent/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.actagent.primaryEnv`.
...
<Note>
  ...
  `metadata.actagent.skillKey`, use that key under `skills.entries`. Quote
  hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, ACTAgent:

<Steps>
  <Step title="Reads skill metadata">
    ACTAgent resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, ACTAgent also materializes the same
...
ACTAgent snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, ACTAgent refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, ACTAgent watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, ACTAgent can treat macOS-only skills as eligible when
    ...
    answering bin probes, ACTAgent clears its cached bin matches.
...
When skills are eligible, ACTAgent injects a compact XML block into the system

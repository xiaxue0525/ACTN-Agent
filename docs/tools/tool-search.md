---
summary: "Tool Search: compact large ACTAgent tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want ACTAgent agents to use a large tool catalog without adding every tool schema to the prompt
  - You want ACTAgent tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for ACTAgent runs
---

Tool Search is an experimental ACTAgent agent runtime feature. It gives agents one
...
This page documents ACTAgent Tool Search. It is not the Codex-native tool
...
When enabled for ACTAgent runs, the model receives one `tool_search_code` tool by default.
That tool runs a short JavaScript body in an isolated Node subprocess with an
`actagent.tools` bridge:

```js
const hits = await actagent.tools.search("create a GitHub issue");
const tool = await actagent.tools.describe(hits[0].id);
return await actagent.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include ACTAgent tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through ACTAgent.

Codex harness runs do not receive these experimental ACTAgent Tool Search
controls. ACTAgent passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the ACTAgent embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible ACTAgent and plugin tools.
...
6. Expose either the ACTAgent code bridge or the structured fallback tools to the
   model.

At execution time every real tool call returns to ACTAgent. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`actagent.tools.call(...)` crosses the bridge back into the Gateway, where the
...
Both modes are experimental. Prefer direct tool exposure for small ACTAgent tool
...
catalog includes eligible ACTAgent, MCP, and client tools after normal policy
filtering.
...
`actagent.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await actagent.tools.search("calendar event", { limit: 5 });
```

`actagent.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await actagent.tools.describe("mcp:calendar:create_event");
```

`actagent.tools.call(id, args)`

Calls a selected tool through ACTAgent.

```js
await actagent.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. ACTAgent enforces a
...
- `actagent.tools.search`
- `actagent.tools.describe`
- `actagent.tools.call`

Normal ACTAgent behavior still applies to final calls:
...
Enable Tool Search for ACTAgent runs with the default code bridge:

```bash
actagent config set tools.toolSearch true
```
...
Use the structured fallback tools instead for ACTAgent runs:
...
- final tool calls executed through ACTAgent
...
- whether the result came from ACTAgent, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the ACTAgent runtime:

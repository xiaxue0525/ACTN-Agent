...
This page lists every configuration knob for ACTAgent memory search. For conceptual overviews, see:
...
All memory search settings live under `agents.defaults.memorySearch` in `actagent.json` unless noted otherwise.
...
When `provider` is not set, ACTAgent uses OpenAI embeddings. Set `provider`
...
ACTAgent pauses vector search and reports an index identity warning instead of
automatically re-embedding everything. Rebuild when you are ready with
`actagent memory status --index --agent <id>` or
`actagent memory index --force --agent <id>`.
</Warning>
...
`memorySearch.provider` can point at a custom `models.providers.<id>` entry for memory-specific provider adapters such as `ollama`, or for OpenAI-compatible model APIs such as `openai-responses` / `openai-completions`. ACTAgent resolves that provider's `api` owner for the embedding adapter while preserving the custom provider id for endpoint, auth, and model-prefix handling. This lets multi-GPU or multi-host setups dedicate memory embeddings to a specific local endpoint:
...
<AccordionGroup>
  <Accordion title="Gemini">
    ...
    <Warning>
    Changing model or `outputDimensionality` changes the index identity. ACTAgent
  ...
  <Accordion title="Bedrock">
    ...
    Bedrock uses the AWS SDK default credential chain — no API keys needed. If ACTAgent runs on EC2 with a Bedrock-enabled instance role, just set the provider and model:
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    ```bash
    actagent memory status --deep --agent main
    actagent memory index --force --agent main
    ```
...
When sqlite-vec is unavailable, ACTAgent falls back to in-process cosine similarity automatically.
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.actagent/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
`searchMode: "search"` is lexical/BM25-only. ACTAgent does not run semantic vector readiness probes or QMD embedding maintenance for that mode, including during `memory status --deep`; `vsearch` and `query` continue to require QMD vector readiness and embeddings.

`rerank: false` only changes QMD `query` mode and requires QMD 2.1 or newer. In direct CLI mode ACTAgent passes `--no-rerank`; in mcporter-backed MCP mode it passes `rerank: false` to QMD's unified query tool. Leave it unset to use QMD's default query reranking behavior.
...
<Note>
QMD model overrides stay on the QMD side, not ACTAgent config. If you need to override QMD's models globally, set environment variables such as `QMD_EMBED_MODEL`, `QMD_RERANK_MODEL`, and `QMD_GENERATE_MODEL` in the gateway runtime environment.
</Note>

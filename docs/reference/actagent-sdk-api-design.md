---
summary: "Reference design for the public ACTAgent App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "ACTAgent App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public ACTAgent app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level ACTAgent App SDK wrapper
---

This page is the detailed API reference design for the public
[ACTAgent App SDK](/concepts/actagent-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@actagent/sdk` is the external app/client package for talking to the
  Gateway. `actagent/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `ACTAgent`, `Agent`, `Session`, `Run`,
...
- source: ACTAgent, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@actagent/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@actagent/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@actagent/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `actagent/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `ACTAgent`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [ACTAgent App SDK](/concepts/actagent-sdk)

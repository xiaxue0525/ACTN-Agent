...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. ACTAgent normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, ACTAgent seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so ACTAgent can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. ACTAgent does not need to inject block-level cache markers.
- ACTAgent uses `prompt_cache_key` to keep cache routing stable across turns. Direct OpenAI hosts use `prompt_cache_retention: "24h"` when `cacheRetention: "long"` is selected.
...
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). ACTAgent maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, ACTAgent injects Anthropic
...
handles provider-side prompt caching automatically. ACTAgent does not inject
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, ACTAgent
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; ACTAgent maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, ACTAgent automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  ACTAgent manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  ACTAgent maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, ACTAgent derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean ACTAgent is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

ACTAgent splits the system prompt into a **stable prefix** and a **volatile
...
## ACTAgent cache-stability guards

ACTAgent also keeps several cache-sensitive payload shapes deterministic before
...
ACTAgent exposes dedicated cache-trace diagnostics for embedded agent runs.
...
ACTAgent keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.actagent/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ACTAGENT_STATE_DIR/logs/cache-trace.jsonl`
...
- `ACTAGENT_CACHE_TRACE=1` enables cache tracing.
- `ACTAGENT_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ACTAGENT_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ACTAGENT_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ACTAGENT_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. ACTAgent's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.

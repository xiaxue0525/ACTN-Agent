---
summary: "ACTAgent code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable ACTAgent code mode for an agent run
...
Code mode is an experimental ACTAgent agent-runtime feature. It is off by
default. When you enable it, ACTAgent changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents ACTAgent code mode. It is not Codex Code mode. The two
...
- ACTAgent code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the ACTAgent generic agent runtime, where the model
...
surfaces. ACTAgent code mode is an ACTAgent-owned experimental tool-surface
adapter for generic ACTAgent runs. It uses `quickjs-wasi`, a hidden ACTAgent
tool catalog, and the normal ACTAgent tool executor.

## What is this?

ACTAgent code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal ACTAgent tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same ACTAgent execution path used by normal agent turns.
...
surface. It does not replace ACTAgent tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for ACTAgent, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through ACTAgent
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, ACTAgent fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
ACTAGENT_DEBUG_CODE_MODE=1 \
ACTAGENT_DEBUG_MODEL_TRANSPORT=1 \
ACTAGENT_DEBUG_MODEL_PAYLOAD=tools \
actagent gateway
```
...
`ACTAGENT_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental ACTAgent surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic ACTAgent agent runs.
...
**Code mode** is the ACTAgent runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, ACTAgent fails closed for
...
2. Build the effective ACTAgent tool list.
...
- If `language` is `"typescript"`, ACTAgent transpiles before evaluation.
...
final value is JSON-compatible after ACTAgent's output adapter runs.
...
`wait` exists because nested ACTAgent tools can be slow, interactive, approval
...
2. On suspension, ACTAgent snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. ACTAgent re-registers host callbacks by stable names.
5. ACTAgent delivers nested tool results into the restored VM.
6. ACTAgent drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "actagent" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "ACTAgent code mode" });
```
...
state directory. For each code-mode `exec` call, ACTAgent builds the run-scoped
...
inside the QuickJS program while ACTAgent still shows only `exec` and `wait` to
the model.
...
7. ACTAgent auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__actagent*`
...
catalog for arbitrary enabled ACTAgent, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary ACTAgent tools, not through the
  code-mode bridge
...
Code mode supersedes the ACTAgent Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- ACTAgent does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same ACTAgent executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the ACTAgent compact
catalog bridge. Code mode is the generic ACTAgent alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal ACTAgent
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("actagent:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, ACTAgent omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters ACTAgent.
...
ACTAgent loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside ACTAgent's main event loop in a worker. A guest
...
inputs beyond existing ACTAgent trajectory policy.
...
ACTAGENT_DEBUG_MODEL_TRANSPORT=1 \
ACTAGENT_DEBUG_MODEL_PAYLOAD=tools \
ACTAGENT_DEBUG_SSE=events \
actagent gateway
```

For payload-shape debugging, use `ACTAGENT_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `ACTAGENT_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for ACTAgent tools
...
5. Send an agent turn with ACTAgent, plugin, MCP, and client test tools.
...
8. In `exec`, call ACTAgent/plugin/client tools through `tools.search`,

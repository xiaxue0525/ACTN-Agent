...
ACTAgent has three public release lanes:
...
- Every stable ACTAgent release ships the npm package, macOS app, and signed
...
6. Run `ACTAgent NPM Release` with `preflight_only=true`. Before a tag exists,
...
vYYYY.M.D-beta.N` from the matching `release/YYYY.M.D` branch. The helper runs
   ...
   proof, records plugin npm and ACTAgentHub plans, and prints the exact
   `ACTAgent Release Publish` command only after the evidence bundle is green.
   `ACTAgent Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ACTAgentHub in parallel, and then promotes the
   prepared ACTAgent npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the ACTAgent npm publish child succeeds, it creates or updates the
   ...
   `actagent-<version>-dependency-evidence.zip` for post-release incident
   ...
   evidence as soon as ACTAgent npm publish succeeds, waits for ACTAgentHub whenever
   ACTAgent npm is being published, then runs `pnpm release:verify-beta` and
   uploads postpublish evidence for the GitHub release, npm package, selected
   plugin npm packages, selected ACTAgentHub packages, child workflow run IDs, and
   optional NPM Telegram run ID. The ACTAgentHub path retries transient CLI
   ...
   `actagent@YYYY.M.D-beta.N` or
   `actagent@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `ACTAgent Release Publish`, reusing the successful preflight artifact via
    ...
    readiness requires the signed `ACTAgentCompanion-Setup-x64.exe`,
    `ACTAgentCompanion-Setup-arm64.exe`, and
    `ACTAgentCompanion-SHA256SUMS.txt` assets on the ACTAgent GitHub release;
    promote them with the `Windows Node Release` workflow after the matching
    `actagent/actagent-windows-node` release has passed its signing workflow.
...
- Plugin version sync updates official plugin package versions and existing
  `actagent.compat.pluginApi` floors to the ACTAgent release version by
  ...
  compatible with older ACTAgent hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `ACTAgent Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `actagent@beta`, `actagent@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=actagent@beta -f suite_profile=product -f published_upgrade_survivor_baseline=actagent@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
  - `smoke`: install/channel/agent, gateway network, and config reload lanes
  - `package`: artifact-native package/update/restart/plugin lanes without OpenWebUI or live ACTAgentHub
...
- `ACTAgent NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `actagent-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `actagent-<version>-dependency-evidence.zip`.
- Run `ACTAgent Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful ACTAgent npm
  ...
  workflow serializes plugin npm publish, plugin ACTAgentHub publish, and ACTAgent
...
- Run the manual `Windows Node Release` workflow for stable releases after the
  matching `actagent/actagent-windows-node` release exists. It downloads the
  ...
  uploads the installers plus manifest onto the canonical ACTAgent GitHub
  release. Website download links should target exact ACTAgent release asset
...
- Release checks now run in a separate manual workflow:
  `ACTAgent Release Checks`
- `ACTAgent Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `ACTAgent Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/actagent-cross-os-release-checks-reusable.yml` directly
...
- `ACTAgent Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an ACTAgent branch or release tag
- `ACTAgent NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `ACTAGENT_LIVE_TEST=1 ACTAGENT_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  runs the fast release guardrails, plugin npm/ACTAgentHub release checks, build,
  UI build, and `release:actagent:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/actagent-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/actagent-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `ACTAGENT_NPM_TELEGRAM_PACKAGE_SPEC=actagent@YYYY.M.D-beta.N ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex ACTAGENT_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `ACTAGENT_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `actagent/releases/.github/workflows/actagent-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=actagent@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `ACTAgent Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `ACTAgent Release
...
`ACTAgent Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=actagent@YYYY.M.D-beta.N`
...
The cross-OS OpenAI install smoke uses `ACTAGENT_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=actagent@YYYY.M.D-beta.N \
  -f evidence_package_spec=actagent@YYYY.M.D-beta.N \
...
required ACTAgent dynamic tools drift or disappear from the standard tier
summary.
...
The Docker box lives in `ACTAgent Release Checks` through
`actagent-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `ACTAgent Release Checks`. It is the agentic
...
`scripts/resolve-actagent-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `actagent@beta`, `actagent@latest`, or an exact ACTAgent release
  version
...
`ACTAgent Release Checks` runs Package Acceptance with `source=artifact`, the
...
configured-auth update restart, live ACTAgentHub skill install, stale plugin dependency cleanup, offline plugin
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=actagent@2026.4.26
```
...
- `smoke`: quick package install/channel/agent, gateway network, and config
  reload lanes
- `package`: install/update/restart/plugin package contracts plus live ACTAgentHub
...
`ACTAgent Release Publish` is the normal mutating publish entrypoint. It
...
4. Dispatch `Plugin NPM Release` with `publish_scope=all-publishable` and
   `ref=<release-sha>`.
5. Dispatch `Plugin ACTAgentHub Release` with the same scope and SHA.
6. Dispatch `ACTAgent NPM Release` with the release tag, npm dist-tag, and
...
Use the lower-level `Plugin NPM Release` and `Plugin ACTAgentHub Release` workflows
only for focused repair or republish work. `ACTAgent Release Publish` rejects
`plugin_publish_scope=selected` when `publish_actagent_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@actagent/diffs-language-pack`. For a selected plugin repair, set
`publish_actagent_npm=false` with `plugin_publish_scope=selected` and
`plugins=@actagent/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`ACTAgent NPM Release` accepts these operator-controlled inputs:
...
`ACTAgent Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `ACTAgent NPM Release` preflight run id;
  required when `publish_actagent_npm=true`
- `npm_dist_tag`: npm target tag for the ACTAgent package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_actagent_npm=false`
- `plugins`: comma-separated `@actagent/*` package names when
  `plugin_publish_scope=selected`
- `publish_actagent_npm`: defaults to `true`; set `false` only when using the
  workflow as a plugin-only repair orchestrator
- `wait_for_actagenthub`: defaults to `false` so npm availability is not blocked by
  the ACTAgentHub sidecar; set `true` only when workflow completion must include
  ACTAgentHub completion

`ACTAgent Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an ACTAgent branch or
  release tag.
...
- For `ACTAgent NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `ACTAgent Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `ACTAgent NPM Release` with `preflight_only=true`
...
6. Run `ACTAgent Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ACTAgentHub before promoting the ACTAgent npm package
7. If the release landed on `beta`, use the
   `actagent/releases/.github/workflows/actagent-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/actagent/actagent/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/actagent/actagent/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/actagent-npm-release.yml`](https://github.com/actagent/actagent/blob/main/.github/workflows/actagent-npm-release.yml)
- [`.github/workflows/actagent-release-checks.yml`](https://github.com/actagent/actagent/blob/main/.github/workflows/actagent-release-checks.yml)
- [`.github/workflows/actagent-cross-os-release-checks-reusable.yml`](https://github.com/actagent/actagent/blob/main/.github/workflows/actagent-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-actagent-package-candidate.mjs`](https://github.com/actagent/actagent/blob/main/scripts/resolve-actagent-package-candidate.mjs)
- [`scripts/actagent-npm-release-check.ts`](https://github.com/actagent/actagent/blob/main/scripts/actagent-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/actagent/actagent/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/actagent/actagent/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`actagent/maintainers/release/README.md`](https://github.com/actagent/maintainers/blob/main/release/README.md)

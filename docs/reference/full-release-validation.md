...
beta publish, pass `release_package_spec=actagent@YYYY.M.D-beta.N` to reuse the
...
`release_package_spec` values derive `codex_plugin_spec=npm:@actagent/codex@<version>`;
...
`ACTAgent Release Checks` is the largest child workflow. It resolves the target
...
| `release-checks`    | All ACTAgent Release Checks stages.                                                             |
...
gate. Required ACTAgent dynamic tool drift in the standard tier blocks the
...
- `release-package-under-test` from the Full Release Validation parent and `ACTAgent Release Checks`
...
- `.github/workflows/actagent-live-and-e2e-checks-reusable.yml`

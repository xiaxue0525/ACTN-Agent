...
read_when:
  - Moving ACTAgent runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.actagent/state/actagent.sqlite`
...
- Configuration stays file-backed: `actagent.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside ACTAgent's database.
...
- Raw stream logging uses `ACTAGENT_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of ACTAgent
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/actagent.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/actagent-agent.sqlite`.
- Config remains file-backed. `actagent.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/actagent.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/actagent-state-schema.sql`,
      `src/state/actagent-agent-schema.sql`,
      `src/state/*generated*`.
...
- `actagent doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `actagent migrate` should not carry legacy ACTAgent
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the ACTAgent `sessionId` as the canonical
...
former stores now write to `state/actagent.sqlite` or the owning
`actagent-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `actagent-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/actagent-agent-db.ts` opens
  `agents/<agentId>/agent/actagent-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one ACTAgent session to one or more external
  ...
  ACTAgent session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `actagent.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.actagent/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.actagent/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.actagent/exec-approvals.json`;
  ...
  `state/actagent.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `actagent-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and ACTAgent keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`ACTAgentKit`) uses the same
  `state/actagent.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/actagent.sqlite#table/device_identities` rows. It never
  reads or writes `actagent/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/actagent.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `actagent update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/actagent.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/actagent.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `actagent sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `ACTAGENT_CACHE_TRACE_FILE`,
  `ACTAGENT_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `ACTAGENT_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.actagent-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.actagent-wiki/state.json` or the unused
  `.actagent-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `actagent.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  ACTAgent session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `actagent-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the ACTAgent temp root, not durable ACTAgent state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$ACTAGENT_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$ACTAGENT_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.actagent/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `actagent-cli-images` cache
...
- `actagent secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/actagent.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.actagent-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.actagent-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.actagent-wiki/cache/agent-digest.json` and
  `.actagent-wiki/cache/claims.jsonl`. The migration provider imports old cache
  files and removes the cache directory when it becomes empty.
- ACTAgentHub skill install tracking now stores one SQLite plugin-state row per
  workspace/skill instead of writing or reading `.actagenthub/lock.json` and
  `.actagenthub/origin.json` sidecars at runtime. Runtime code uses tracked-install
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  ACTAgent temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `actagent backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `actagent backup restore` validates the archive before extraction, reuses the
...
- `actagent reset --scope config+creds+sessions` removes per-agent
  `actagent-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureACTAgentModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `actagent/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.actagent/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.actagent/memory/lancedb` as an implicit ACTAgent-managed store. It is an
...
- `check:database-first-legacy-stores` fails new runtime source that pairs
  ...
  `state/actagent.sqlite` location instead of pretending those features own
...
`actagent doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `actagent migrate` must not import legacy ACTAgent state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `actagent doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/actagent.sqlite`:
...
```text
~/.actagent/state/actagent.sqlite
~/.actagent/agents/<agentId>/agent/actagent-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `actagent backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `actagent backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `actagent sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `actagent backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  ACTAgent test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `actagent-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.actagent-wiki/log.jsonl`
- Memory Wiki `.actagent-wiki/state.json`
- Memory Wiki `.actagent-wiki/locks/`
- Memory Wiki `.actagent-wiki/source-sync.json`
- Memory Wiki `.actagent-wiki/import-runs/*.json`
- Memory Wiki `.actagent-wiki/cache/agent-digest.json`
- Memory Wiki `.actagent-wiki/cache/claims.jsonl`
- ACTAgentHub `.actagenthub/lock.json`
- ACTAgentHub `.actagenthub/origin.json`
- Browser profile decoration `.actagent-profile-decorated`

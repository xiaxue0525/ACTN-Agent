---
summary: "Get ACTAgent installed and run your first chat in minutes."
read_when:
...
Install ACTAgent, run onboarding, and chat with your AI assistant — all in
...
/>
  ...
  <Step title="Verify the Gateway is running">
    ...
    You should see the Gateway listening on port 19199.
...
```bash
mkdir -p "$HOME/.actagent/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.actagent/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run ACTAgent as a service account or want custom paths:

- `ACTAGENT_HOME` — home directory for internal path resolution
- `ACTAGENT_STATE_DIR` — override the state directory
- `ACTAGENT_CONFIG_PATH` — override the config file path

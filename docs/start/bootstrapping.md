...
On the first agent run, ACTAgent bootstraps the workspace (default
`~/.actagent/workspace`):
...
For embedded/local model runs, ACTAgent keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `actagent onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.actagent/workspace`).
</Note>

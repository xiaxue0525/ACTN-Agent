...
CLI onboarding is the **recommended** terminal setup path for ACTAgent on
...
`ACTAGENT_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `ACTAgent`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`actagent dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`actagent configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.actagent/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `actagent doctor` first.
</Note>
...
Use `actagent agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.actagent/workspace-<agentId>`.
...
- CLI command reference: [`actagent onboard`](/cli/onboard)

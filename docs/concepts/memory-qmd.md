...
alongside ACTAgent. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, ACTAgent falls back to the
...
ACTAgent creates a self-contained QMD home under
`~/.actagent/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- ACTAgent creates collections from your workspace memory files and any
...
- If QMD fails entirely, ACTAgent falls back to the builtin SQLite engine.
  ...
  `actagent memory status` and one-shot CLI probes still recheck QMD directly.
...
ACTAgent keeps the QMD search path compatible with both current and older QMD
installs.

On startup, ACTAgent checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, ACTAgent searches all
...
Older QMD builds only accept one collection filter. When ACTAgent detects one
...
process, so you can tune QMD globally without adding new ACTAgent config:
...
collection under `~/.actagent/agents/<id>/qmd/sessions/`.
...
When scope denies a search, ACTAgent logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If ACTAgent
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but ACTAgent still reports
...
with `actagent memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs ACTAgent uses.

**Many QMD subprocesses during search?** Update QMD if possible. ACTAgent uses
...
`memory.qmd.searchMode = "search"`. ACTAgent treats that mode as lexical-only,
...
the next startup reconciliation. ACTAgent recreates stale managed collections
...
ACTAgent's builtin symlink rules. Keep temporary monorepo checkouts under

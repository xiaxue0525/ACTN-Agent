---
summary: "How ACTAgent remembers things across sessions"
...
ACTAgent remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.actagent/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, ACTAgent keeps the file on
...
`actagent doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use ACTAgent approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. ACTAgent infers them in a hidden background pass, scopes them to
...
<Info>
ACTAgent uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, ACTAgent
...
```bash
actagent memory rem-backfill --rollback
actagent memory rem-backfill --rollback-short-term
```

## CLI

```bash
actagent memory status          # Check index status and provider
actagent memory search "query"  # Search from the command line
actagent memory index --force   # Rebuild the index
```

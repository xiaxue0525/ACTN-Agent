---
summary: "How ACTAgent manages conversation sessions"
read_when:
...
ACTAgent organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `actagent security audit`.
...
ACTAgent automatically bounds session storage over time. By default, it runs
...
`actagent sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `actagent sessions cleanup --dry-run`.

## Inspecting sessions

- `actagent status` -- session store path and recent activity.
- `actagent sessions --json` -- all sessions (filter with `--active <minutes>`).

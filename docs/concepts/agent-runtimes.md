---
summary: "How ACTAgent separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between ACTAgent, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot`      | How ACTAgent authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`                 | The model selected for the agent turn.                              |
| Agent runtime | `actagent`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp           | Where messages enter and leave ACTAgent.                            |
...
`actagent doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside ACTAgent's prepared agent loop. Today this
  is the built-in `actagent` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | ACTAgent name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `actagent doctor --fix` owns legacy
...
That means ACTAgent selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or ACTAgent session store
becomes Codex.
...
3. If the user explicitly chooses **ACTAgent for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "actagent"`. A selected `openai` OAuth profile is routed
   internally through ACTAgent's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `actagent doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; ACTAgent no longer keeps a bundled Codex CLI backend.
...
| Surface                     | ACTAgent embedded                             | Codex app-server                                                            |
| --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | ACTAgent through the ACTAgent embedded runner | Codex app-server                                                            |
| Canonical thread state      | ACTAgent transcript                           | Codex thread, plus ACTAgent transcript mirror                               |
| ACTAgent dynamic tools      | Native ACTAgent tool loop                     | Bridged through the Codex adapter                                           |
| Native shell and file tools | ACTAgent path                                 | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native ACTAgent context assembly              | ACTAgent projects assembled context into the Codex turn                     |
| Compaction                  | ACTAgent or selected context engine           | Codex-native compaction, with ACTAgent notifications and mirror maintenance |
| Channel delivery            | ACTAgent                                      | ACTAgent                                                                    |

This ownership split is the main design rule:

- If ACTAgent owns the surface, ACTAgent can provide normal plugin hook behavior.
- If the native runtime owns the surface, ACTAgent needs runtime events or native hooks.
- If the native runtime owns canonical thread state, ACTAgent should mirror and project context, not rewrite unsupported internals.

## Runtime selection

ACTAgent chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, ACTAgent uses `actagent` as the
...
`ACTAGENT_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`actagent doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where ACTAgent can preserve the intent.
...
selection/runtime error; it is never silently routed back to ACTAgent.
...
harness. Explicit ACTAgent runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
ACTAgent routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `actagent doctor --fix`.

If `actagent doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`actagent doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@actagent/copilot` plugin registers an opt-in `copilot` runtime
...
`actagent doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not ACTAgent, it should document what ACTAgent surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether ACTAgent can edit history or only mirror it.                                   |
| Do ACTAgent dynamic tools work?        | Messaging, sessions, cron, and ACTAgent-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around ACTAgent-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume ACTAgent equivalence where the native runtime owns more state.            |

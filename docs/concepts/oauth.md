---
summary: "OAuth in ACTAgent: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand ACTAgent OAuth end-to-end
...
ACTAgent supports "subscription auth" via OAuth for providers that offer it
(notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic, the practical split
is now:

- **Anthropic API key**: normal Anthropic API billing
- **Anthropic Claude CLI / subscription auth inside ACTAgent**: Anthropic staff
  told us this usage is allowed again

OpenAI Codex OAuth is explicitly supported for use in external tools like
ACTAgent.

ACTAgent stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`actagent doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
ACTAgent also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:

```bash
actagent models auth login --provider <id>
```
...
- you log in via ACTAgent _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, ACTAgent treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai:default` profile, but once ACTAgent has a local OAuth profile,
  the local refresh token is canonical. If that local refresh token is rejected,
  ACTAgent can use a usable same-account Codex CLI token as a runtime-only
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.actagent/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.actagent/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.actagent/credentials/oauth.json` (imported into `auth-profiles.json` on first use)

All of the above also respect `$ACTAGENT_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, ACTAgent uses read-through
...
Claude subscription limits, and Anthropic staff told us ACTAgent-style Claude
CLI usage is allowed again. ACTAgent therefore treats Claude CLI reuse and
...
If you want other subscription-style options in ACTAgent, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/zai).
</Warning>

ACTAgent also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

ACTAgent supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

ACTAgent's interactive login flows are implemented in `actagent/plugin-sdk/llm` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from ACTAgent
2. ACTAgent stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including ACTAgent workflows.
...
`actagent models auth list --provider openai` after repair before copying
...
Wizard path is `actagent onboard` → auth choice `openai`.
...
- exception: some external CLI credentials stay externally managed; ACTAgent
  ...
  `openai:default` profile, then ACTAgent-owned refreshes keep the local
  profile canonical. If the local Codex refresh fails and Codex CLI has a
  usable token for the same account, ACTAgent may use that token for the current
...
- `actagent channels list --json` (shows `auth[]`)

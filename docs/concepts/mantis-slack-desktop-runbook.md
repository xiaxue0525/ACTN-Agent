...
Linux desktop, VNC rescue, Slack Web, a real ACTAgent gateway, screenshots,
...
- Mantis artifacts: owned by the ACTAgent run. They live under
...
- remote logs such as `slack-desktop-command.log`, `actagent-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, ACTAgent install/build or
...
remote status after Mantis has copied metadata proving that either the ACTAgent

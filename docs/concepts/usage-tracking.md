...
- `/usage cost` in chats: local cost summary aggregated from ACTAgent session logs.
- CLI: `actagent status --usage` prints a full per-provider breakdown.
- CLI: `actagent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. ACTAgent treats
  ...
  quota, so ACTAgent inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, ACTAgent prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  ACTAgent calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise ACTAgent falls back to

...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.actagent/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.actagent/agents/<agentId>/sessions`.
...
a local profile, but ACTAgent does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.actagent/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.actagent` (or `ACTAGENT_STATE_DIR`)
- Workspace: `~/.actagent/workspace` (or `~/.actagent/workspace-<agentId>`)
- Agent dir: `~/.actagent/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.actagent/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, ACTAgent runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.actagent/workspace` (or `~/.actagent/workspace-<profile>` when `ACTAGENT_PROFILE` is set).
- State defaults to `~/.actagent/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    actagent agents add coding
    actagent agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.actagent/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.actagent/workspace-alex" },
      { id: "mia", workspace: "~/.actagent/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, ACTAgent upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, ACTAgent falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.actagent/workspace-main" },
          { id: "coding", workspace: "~/.actagent/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.actagent/workspace-main" },
          { id: "alerts", workspace: "~/.actagent/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    actagent channels login --channel whatsapp --account personal
    actagent channels login --channel whatsapp --account biz
    ```

    `~/.actagent/actagent.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.actagent/workspace-home",
            agentDir: "~/.actagent/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.actagent/workspace-work",
            agentDir: "~/.actagent/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.actagent/credentials/whatsapp/personal
              // authDir: "~/.actagent/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.actagent/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.actagent/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.actagent/workspace-opus",

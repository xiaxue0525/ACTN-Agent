---
summary: "Public ACTAgent App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "ACTAgent App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to ACTAgent
...
The **ACTAgent App SDK** is the public client API for apps outside the
ACTAgent process. Use `@actagent/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@actagent/sdk` talks to the Gateway from outside ACTAgent.
  `actagent/plugin-sdk/*` is only for plugins that run inside ACTAgent and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@actagent/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `ACTAgent`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `ACTAgentEvent`,
`ACTAgentEventType`, `GatewayEvent`, `ACTAgentTransport`,
...
```typescript
import { ACTAgent } from "@actagent/sdk";

const oc = new ACTAgent({
...
`new ACTAgent({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `ACTAgentTransport`:
...
The SDK normalizes raw Gateway events into a stable `ACTAgentEvent` envelope:
...
Task helpers use the durable task ledger that also backs `actagent tasks`:
...
Use the App SDK when code lives outside ACTAgent:
...
Use the Plugin SDK when code runs inside ACTAgent:
...
App SDK code should import from `@actagent/sdk`. Plugin code should import from
documented `actagent/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [ACTAgent App SDK API design](/reference/actagent-sdk-api-design)

---
summary: "Use SOUL.md to give your ACTAgent agent an actual voice instead of generic assistant sludge"
read_when:
...
ACTAgent injects it on normal sessions, so it has real weight. If your agent
...
For ACTAgent, `SOUL.md` is that layer.
...
Path fixed for ACTAgent workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files ACTAgent injects into model context.
  </Card>
  <Card title="System prompt" href="/concepts/system-prompt" icon="message-lines">
    How `SOUL.md` is composed into ACTAgent and Codex runtime context.
  </Card>

---
summary: "Delegate architecture: running ACTAgent as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run ACTAgent as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an ACTAgent agent that:
...
ACTAgent's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of ACTAgent's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.actagent/workspace-delegate",
  tools: {
...
- Cron run history: ACTAgent shared SQLite state database
- Session transcripts: `~/.actagent/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through ACTAgent's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
actagent agents add delegate
```

This creates:

- Workspace: `~/.actagent/workspace-delegate`
- State: `~/.actagent/agents/delegate/agent`
- Sessions: `~/.actagent/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.actagent/workspace" },
      {
        id: "delegate",
        workspace: "~/.actagent/workspace-delegate",
        tools: {
...
~/.actagent/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.actagent/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.actagent/workspace-org",
        agentDir: "~/.actagent/agents/org-assistant/agent",
...
recall view. ACTAgent redacts credential/token-like text, truncates long

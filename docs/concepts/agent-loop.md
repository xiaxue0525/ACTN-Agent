...
In ACTAgent, a loop is a single, serialized run per session that emits lifecycle and stream events
...
2. `agentCommand` runs the agent:
   ...
   - calls `runEmbeddedAgent` (ACTAgent agent runtime)
   - emits **lifecycle end/error** if the embedded loop does not emit one
3. `runEmbeddedAgent`:
   - serializes runs via per-session + global queues
   - resolves model + auth profile and builds the ACTAgent session
...
4. `subscribeEmbeddedAgentSession` bridges agent runtime events to ACTAgent `agent` stream:
...
- System prompt is built from ACTAgent's base prompt, skills prompt, bootstrap context, and per-run overrides.
- Model-specific limits and compaction reserve tokens are enforced.
- See [System prompt](/concepts/system-prompt) for what the model sees.

## Hook points (where you can intercept)

ACTAgent has two hook systems:
...
- **`tool_result_persist`**: synchronously transform tool results before they are written to an ACTAgent-owned session transcript.
...
ACTAgent plugin hooks as the compatibility contract for documented mirrored

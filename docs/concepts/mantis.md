---
summary: "Mantis is the visual end-to-end verification system for reproducing ACTAgent bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for ACTAgent bugs
...
Mantis is the ACTAgent end-to-end verification system for bugs that need a real
...
Mantis lives in the ACTAgent QA stack.

- ACTAgent owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm actagent qa mantis`.
...
- ACTAgentSweeper owns GitHub comment routing: parsing maintainer commands,
  dispatching the workflow, and posting the final PR comment.
- ACTAgent agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in ACTAgent, machine scheduling in
Crabbox, and maintainer workflow glue in ACTAgentSweeper.
...
```bash
pnpm actagent qa mantis discord-smoke \
...
thread, calls ACTAgent's `message.thread-reply` action with a repo-local
...
```bash
pnpm actagent qa mantis desktop-browser-smoke \
...
`ACTAGENT_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `ACTAGENT_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `ACTAGENT_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `ACTAGENT_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `ACTAGENT_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `ACTAGENT_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm actagent qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT ACTAgent gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable ACTAgent
home at `$HOME/.actagent-mantis/slack-actagent`, patches Slack Socket Mode
configuration for the selected channel, starts `actagent gateway run` on port
`38973`, and keeps Chrome running in the VNC session. This is the "leave me a
Linux desktop with Slack and a actagent running" mode; the bot-to-bot Slack QA lane
...
- `ACTAGENT_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `ACTAGENT_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `ACTAGENT_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `ACTAGENT_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent ACTAgent Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `ACTAGENT_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/actagent-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`actagent/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm actagent qa mantis slack-desktop-smoke --gateway-setup` against that
...
this lane when you want "a Linux desktop with Slack and a actagent running" instead
...
worktree, runs `pnpm actagent qa telegram --credential-source convex
...
`@actagent-mantis telegram desktop proof`, from the Actions UI with freeform
...
ACTAgent with the leased Telegram SUT bot token, starts `actagent gateway run`
...
token never logs Telegram Desktop in; it only configures ACTAgent. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring ACTAgent.
...
comment or creates one. The workflows publish to `actagent-crabbox-artifacts`
with public URLs under `https://artifacts.actagent.ai`. They provide bucket,
...
```text
@actagent-mantis discord status reactions
```
...
```text
@actagent-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
ACTAgentSweeper command examples:

```text
@actagentsweeper mantis discord discord-status-reactions-tool-only
@actagentsweeper verify e2e discord
```
...
ACTAgentSweeper review findings.
...
6. Start a child ACTAgent Gateway with an isolated state directory.
...
- It exercises a real ACTAgent Gateway, Discord bot auth, message dispatch,
...
- `pnpm actagent qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or ACTAgent browser control captures screenshots.
...
- Node 22 and pnpm
- ACTAgent checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one ACTAgent Gateway, one browser, and one model run
...
- `ACTAGENT_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `ACTAGENT_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `ACTAGENT_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `ACTAGENT_QA_CONVEX_SITE_URL`
- `ACTAGENT_QA_CONVEX_SECRET_CI`
- `ACTAGENT_QA_MANTIS_CRABBOX_COORDINATOR`
- `ACTAGENT_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`ACTAGENT_QA_REDACT_PUBLIC_METADATA=1` for this reason.
...
- When should ACTAgentSweeper automatically recommend Mantis instead of waiting for a
  maintainer command?

...
read_when:
  ...
  - Explaining how ACTAgent updates one channel message while work is in progress
...
When progress drafts are enabled, ACTAgent creates one visible work-in-progress
...
That is usually enough. ACTAgent will pick an automatic one-word label, wait
...
ACTAgent sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from ACTAgent's built-in
...
ACTAgent renders only the `progress.text` in the channel progress UI. The
...
ACTAgent uses the same formatter for progress drafts and `/verbose`:
...
ACTAgent truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, ACTAgent still suppresses the older standalone
...
When the final answer is ready, ACTAgent tries to keep the chat clean:

- If the draft can safely become the final answer, ACTAgent edits it in place.
- If the channel uses native progress streaming, ACTAgent finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, ACTAgent sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, ACTAgent keeps the

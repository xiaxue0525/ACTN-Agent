---
summary: "What the ACTAgent system prompt contains and how it is assembled"
read_when:
...
ACTAgent builds a custom system prompt for every agent run. The prompt is **ACTAgent-owned** and does not use a runtime default prompt.

The prompt is assembled by ACTAgent and injected into each agent run.
...
the full ACTAgent-owned prompt. The provider runtime can:
...
- **ACTAgent Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **ACTAgent Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to ACTAgent docs/source and when to read them.
...
ACTAgent keeps large stable content, including **Project Context**, above the
...
ACTAgent can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **ACTAgent
...
For channel auto-reply runs, ACTAgent omits the generic **Silent Replies**
...
ACTAgent keeps committed prompt snapshots for the Codex runtime happy path under
...
ACTAgent developer instructions, turn-scoped collaboration-mode instructions
when ACTAgent provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after ACTAgent sends
...
On the native Codex harness, ACTAgent avoids repeating stable workspace files
...
Codex developer instructions. The compact ACTAgent skills list is also forwarded
...
ACTAgent prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, ACTAgent can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, ACTAgent injects a compact **available skills list**
...
points to the local ACTAgent docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.actagent.ai](https://docs.actagent.ai).

The same section also includes the ACTAgent source location. Git checkouts expose the local
...
stale. The prompt also notes the public docs mirror, community Discord, and ACTAgentHub
([https://actagenthub.ai](https://actagenthub.ai)) for skills discovery. It tells the model to
consult docs first for ACTAgent behavior, commands, configuration, or architecture, and to
run `actagent status` itself when possible (asking the user only when it lacks access).

...
read_when:
  - You want ACTAgent to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, ACTAgent can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. ACTAgent may check in afterward.
- You say you are exhausted. ACTAgent may ask later whether you slept.
- The agent says it will follow up after something changes. ACTAgent may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: ACTAgent remembers a
...
```bash
actagent config set commitments.enabled true
actagent config set commitments.maxPerDay 3
```

Equivalent `actagent.json`:
...
After an agent reply, ACTAgent may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, ACTAgent stores a commitment with:
...
without ACTAgent tools.

ACTAgent never delivers an inferred commitment immediately after writing it.
...
actagent commitments dismiss cm_abc123
```

See [`actagent commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local ACTAgent state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
actagent config set commitments.enabled false
```
...
- Check `actagent commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`actagent commitments`](/cli/commitments)

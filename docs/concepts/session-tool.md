...
ACTAgent gives agents tools to work across sessions, inspect status, and
...
After the target responds, ACTAgent can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `actagent-tui` are
not session keys.
...
- `deliveryContext` is the persisted delivery route stored on the session,
  which ACTAgent can reuse for later delivery even when the active surface
  differs.
...
`subagents` is the visibility helper for already spawned ACTAgent
...
the completion origin only identifies a channel ACTAgent can still reuse the

---
summary: "Move one ACTAgent session's reply route between linked chat channels"
...
Channel docking is call forwarding for one ACTAgent session.
...
Alice can message ACTAgent on Telegram and Discord:
...
ACTAgent keeps the current session context and changes the reply route:
...
4. Keep the same ACTAgent session, but receive future replies in Discord.

...
`node-llama-cpp` runtime package next to ACTAgent, then point `local.modelPath`
at a GGUF file:
...
ACTAgent indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.actagent/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `actagent memory index --force`
...
**Memory search disabled?** Check `actagent memory status`. If no provider is
...
**Stale results?** Run `actagent memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** ACTAgent falls back to in-process cosine similarity
automatically. `actagent memory status --deep` reports the local vector store

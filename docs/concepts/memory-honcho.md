...
[Honcho](https://honcho.dev) adds AI-native memory to ACTAgent. It persists
...
```bash
actagent plugins install @honcho-ai/actagent-honcho
actagent honcho setup
...
Settings live under `plugins.entries["actagent-honcho"].config`:
    ...
    entries: {
      "actagent-honcho": {
        config: {
          ...
          workspaceId: "actagent", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `actagent honcho setup` detects and
...
```bash
actagent honcho setup                        # Configure API key and migrate files
actagent honcho status                       # Check connection status
actagent honcho ask <question>               # Query Honcho about the user
actagent honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/actagent-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho ACTAgent integration guide](https://docs.honcho.dev/v3/guides/integrations/actagent)
- [Memory](/concepts/memory) -- ACTAgent memory overview

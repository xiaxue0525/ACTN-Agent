...
Paste this into `actagent.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, ACTAgent can show:
...
case-insensitive. If `allowedChatIds` is non-empty and ACTAgent cannot resolve a
...
Lossless actagent is a context-engine plugin with its own recall tools. Install and
configure it as a context engine first; see [Context engine](/concepts/context-engine).
Then let Active Memory use the Lossless actagent recall tools:
...
Lossless actagent uses that as a lower-level delegated expansion tool.
...
`config.promptOverride` replaces the default Active Memory prompt. ACTAgent
...
5. Verify memory search itself works with `actagent memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, ACTAgent uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `actagent memory status --deep` to inspect the memory-search backend
      and index health.

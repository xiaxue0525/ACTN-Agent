...
ACTAgent can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **ACTAgent tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
actagent agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, ACTAgent now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), ACTAgent will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw ACTAgent transcript history before compaction.
...
2. **Builds a system prompt** using the same ACTAgent prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   ACTAgent session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us ACTAgent-style Claude CLI usage is allowed again, so ACTAgent treats
...
resolver for ACTAgent skills. When the current skills snapshot includes at least
one selected skill with a materialized path, ACTAgent passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate ACTAgent skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, ACTAgent keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by ACTAgent to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. ACTAgent maps that
to the existing exec policy instead of adding Claude-specific policy config.
For ACTAgent-managed Claude live sessions, the effective ACTAgent exec policy is
...
Claude launches normalize that flag to match the effective ACTAgent exec policy.

The bundled Anthropic `claude-cli` backend also maps ACTAgent `/think` levels
...
Before ACTAgent can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
actagent models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, ACTAgent resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; ACTAgent clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from ACTAgent's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. ACTAgent still keeps raw
...
- ACTAgent drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, ACTAgent lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), ACTAgent seeds
...
provider would start cold because ACTAgent's own session transcript is empty
for `claude-cli` runs.
...
ACTAgent will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, ACTAgent appends the
...
- For Gemini CLI JSON output, ACTAgent reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; ACTAgent no
...
- `stats.cached` is normalized into ACTAgent `cacheRead`.
- If `stats.input` is missing, ACTAgent derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before ACTAgent handles
...
Some CLI backends run an agent that compacts its **own** transcript, so ACTAgent must
...
`ownsNativeCompaction: true`, and ACTAgent returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep ACTAgent's safeguard from firing on a
...
CLI backends do **not** receive ACTAgent tool calls directly, but a backend can
...
When bundle MCP is enabled, ACTAgent:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`ACTAGENT_MCP_TOKEN`)
...
If no MCP servers are enabled, ACTAgent still injects a strict config when a
...
When a fresh CLI session is seeded from a prior ACTAgent transcript (for
...
- **No direct ACTAgent tool calls.** ACTAgent does not inject tool calls into

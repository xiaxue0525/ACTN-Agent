---
summary: "Pre-flight and rollback checklist before exposing an ACTAgent Gateway beyond loopback"
...
| -------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Loopback + SSH tunnel      | Personal use, admin access, debugging           | Keep `gateway.bind: "loopback"` and tunnel `127.0.0.1:19199`                                        |
...
- Backup location for `~/.actagent/actagent.json` and credentials.
...
```bash
actagent gateway probe --url ws://127.0.0.1:19199 --token "$ACTAGENT_GATEWAY_TOKEN"
```
...
Run `actagent security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `actagent security audit --deep`.
...
5. Re-run `actagent security audit --deep`.

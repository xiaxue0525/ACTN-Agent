...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  ACTAgent is **not** a hostile multi-tenant security boundary for multiple
...
ACTAgent security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `actagent security audit`
...
```bash
actagent security audit
actagent security audit --deep
actagent security audit --fix
actagent security audit --json
```
...
ACTAgent is both a product and an experiment: you're wiring frontier-model behavior into real messaging surfaces and real tools. **There is no "perfectly secure" setup.** The goal is to be deliberate about:
...
ACTAgent source checkouts use `pnpm-lock.yaml`. The published `actagent` npm
package and ACTAgent-owned npm plugin packages include `npm-shrinkwrap.json`,
...
ACTAgent assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.actagent`, including `actagent.json`), treat them as a trusted operator.
...
ACTAgent uses `@actagent/fs-safe` for root-bounded file access, atomic writes, archive extraction, temp workspaces, and secret-file helpers. ACTAgent defaults fs-safe's optional POSIX Python helper to **off**; set `ACTAGENT_FS_SAFE_PYTHON_MODE=auto` or `require` only when you want the extra fd-relative mutation hardening and can support a Python runtime.
...
- ACTAgent's product default for trusted single-operator setups is that host exec on `gateway`/`node` is allowed without approval prompts (`security="full"`, `ask="off"` unless you tighten it). That default is intentional UX, not a vulnerability by itself.
...
ACTAgent separates two concepts:
...
If you run `--deep`, ACTAgent also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.actagent/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.actagent/credentials/<channel>-allowFrom.json` (default account)
  - `~/.actagent/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.actagent/agents/<agentId>/agent/auth-profiles.json`
- **Codex runtime state**: `~/.actagent/agents/<agentId>/agent/codex-home/`
- **File-backed secrets payload (optional)**: `~/.actagent/secrets.json`
- **Legacy OAuth import**: `~/.actagent/credentials/oauth.json`
...
`actagent security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`actagent security audit` raises `config.insecure_or_dangerous_flags` when
...
gateway:
  ...
  auth:
    mode: password
    password: ${ACTAGENT_GATEWAY_PASSWORD}
```
...
- ACTAgent gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from ACTAgent responses.
...
ACTAgent stores session transcripts on disk under `~/.actagent/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.actagent` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If ACTAgent cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
ACTAgent can refresh the skills list mid-session:
...
- If you install or update plugins (`actagent plugins install <package>`, `actagent plugins update <id>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - ACTAgent does not run built-in local dangerous-code blocking during install/update. Use `security.installPolicy` for operator-owned local allow/block decisions and `actagent security audit --deep` for diagnostic scanning.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained plugin packages; ACTAgent copies/references them without running `npm install`.
  - Prefer pinned, exact versions (`@scope/pkg@1.2.3`), and inspect the unpacked code on disk before enabling.
  - `--dangerously-force-unsafe-install` is deprecated and no longer changes plugin install/update behavior.
  - Configure `security.installPolicy` when operators need a trusted local command to make host-specific allow/block decisions for skill and plugin installs. This policy runs after source material is staged but before installation continues, applies to ACTAgentHub skills too, and is not bypassed by deprecated unsafe flags.
...
By default, ACTAgent routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ACTAgent has two separate "who can trigger me?" layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.actagent/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- "Paste the full contents of ~/.actagent or your logs."

## External content special-token sanitization

ACTAgent strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
ACTAgent includes explicit bypass flags that disable external-content safety wrapping:
...
ACTAgent strips common model-family special-token literals from wrapped
...
- `~/.actagent/actagent.json`: `600` (user read/write only)
- `~/.actagent`: `700` (user only)

`actagent doctor` can warn and offer to tighten these permissions.
...
- Default: `19199`
- Config/flags/env: `gateway.port`, `--port`, `ACTAGENT_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__actagent__/canvas/` and `/__actagent__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run ACTAgent with Docker on a VPS, remember that published container ports
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts its presence via mDNS (`_actagent-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
5. **Environment variable** (alternative): set `ACTAGENT_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `actagent doctor --generate-gateway-token`.
...
`ACTAGENT_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
This is intentionally process environment only, not an `actagent.json` config
key.
...
- ACTAgent also has a narrow backend/container-local self-connect path for
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `ACTAGENT_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `ACTAGENT_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ACTAgent
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. ACTAgent verifies the identity by resolving the
...
- In those identity-bearing modes, omitting `x-actagent-scopes` falls back to the normal operator default scope set; send the header explicitly when you want a narrower scope set.
...
- ACTAgent will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.actagent/` (or `$ACTAGENT_STATE_DIR/`) may contain secrets or private data:

- `actagent.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking provider credentials prevents a cloned workspace from substituting attacker-controlled provider accounts. Blocking the whole `ACTAGENT_*` prefix means adding a new `ACTAGENT_*` flag later can never regress into silent inheritance from workspace state.
...
- When sharing diagnostics, prefer `actagent status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@actagent", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.actagent`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.actagent/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `actagent` profile).
...
ACTAgent's browser navigation policy is strict by default: private/internal destinations stay blocked unless you explicitly opt in.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.actagent/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.actagent/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.actagent/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default ACTAgent limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `actagent gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `ACTAGENT_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/actagent/actagent-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.actagent/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `actagent security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + ACTAgent version
...
Found a vulnerability in ACTAgent? Please report responsibly:

1. Email: [security@actagent.ai](mailto:security@actagent.ai)

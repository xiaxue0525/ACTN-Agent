...
ACTAgent supports OAuth and API keys for model providers. For always-on gateway
...
setup, but ACTAgent also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `actagent gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.actagent/.env` so the daemon can read it:

```bash
cat >> ~/.actagent/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `actagent onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.actagent/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in ACTAgent as a supported token
path. Anthropic staff has since told us that ACTAgent-style Claude CLI usage is
allowed again, so ACTAgent treats Claude CLI reuse and `claude -p` usage as
...
2. Tell ACTAgent to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching ACTAgent auth profile.
...
```bash
actagent models auth paste-token --provider openrouter
```
...
- If auth exists but ACTAgent cannot resolve a probeable model candidate for
...
- Anthropic staff told us this ACTAgent integration path is allowed again.
- ACTAgent therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `ACTAGENT_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- ACTAgent retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, ACTAgent deletes
...
Use `actagent models auth login --provider <id> --profile-id <profileId>` for
...
```bash
actagent models auth login --provider anthropic --force
```
...
When you debug order issues, `actagent models status --probe` shows omitted
...
Run `actagent models status` to confirm which profile is expiring. If an

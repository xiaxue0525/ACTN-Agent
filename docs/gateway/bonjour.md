...
ACTAgent can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_actagent-gw._tcp` under a dedicated zone
   (example: `actagent.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

ACTAgent supports any discovery domain; `actagent.internal.` is just an example.
...
- serve your chosen domain (example: `actagent.internal.`) from `~/.actagent/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _actagent-gw._tcp actagent.internal.
dig @<TAILNET_IPV4> -p 53 _actagent-gw._tcp.actagent.internal PTR +short
```
...
`_actagent-gw._tcp` in your discovery domain without multicast.

### Gateway listener security (recommended)

The Gateway WS port (default `19199`) binds to loopback by default. For LAN/tailnet
access, bind explicitly and keep auth enabled.

For tailnet-only setups:

- Set `gateway.bind: "tailnet"` in `~/.actagent/actagent.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_actagent-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_actagent-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _actagent-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _actagent-gw._tcp local.
  ```
...
in-progress states. If the service never reaches `announced`, ACTAgent eventually
...
invalid DNS-label character, ACTAgent falls back to `actagent.local`. Set
`ACTAGENT_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_actagent-gw._tcp`.
...
```bash
actagent plugins enable bonjour
```
...
discovery plugin for that ACTAgent config:

```bash
actagent plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `ACTAGENT_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `ACTAGENT_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `ACTAGENT_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `ACTAGENT_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep ACTAGENT_DISABLE_BONJOUR
   ```
...
3. Use a direct target when Bonjour is disabled:
   - Control UI or local tools: `http://127.0.0.1:19199`
   - LAN clients: `http://<gateway-host>:19199`
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `ACTAGENT_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `ACTAGENT_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. ACTAgent retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `ACTAGENT_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `actagent plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `actagent plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `ACTAGENT_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `ACTAGENT_DISABLE_BONJOUR`).
- `ACTAGENT_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `ACTAGENT_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.actagent/actagent.json` controls the Gateway bind mode.
- `ACTAGENT_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `ACTAGENT_SSH_PORT`).
- `ACTAGENT_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `ACTAGENT_TAILNET_DNS`).
- `ACTAGENT_CLI_PATH` overrides the advertised CLI path (legacy: `ACTAGENT_CLI_PATH`).

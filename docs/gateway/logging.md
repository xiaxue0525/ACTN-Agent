...
- Default rolling log file is under `/tmp/actagent/` (one file per day): `actagent-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.actagent/actagent.json`:
  - `logging.file`
  - `logging.level`
...
ACTAgent can mask sensitive tokens before log or transcript output leaves the
...
`actagent gateway` supports a per-gateway style switch:

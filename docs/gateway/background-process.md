...
ACTAgent runs shell commands through the `exec` tool and keeps long-running tasks in memory. The `process` tool manages those background sessions.
...
- Spawned exec commands receive `ACTAGENT_SHELL=exec` for context-aware shell/profile rules.
...
- `ACTAGENT_BASH_YIELD_MS`: default yield (ms)
- `ACTAGENT_BASH_MAX_OUTPUT_CHARS`: in-memory output cap (chars)
- `ACTAGENT_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
- `ACTAGENT_BASH_JOB_TTL_MS`: TTL for finished sessions (ms, bounded to 1m–3h)
- `ACTAGENT_PROCESS_INPUT_WAIT_IDLE_MS`: idle-output threshold before writable background sessions are marked as likely waiting for input (default 15000 ms)

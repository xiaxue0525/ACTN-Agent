---
summary: "Schema-accurate configuration examples for common ACTAgent setups"
read_when:
  - Learning how to configure ACTAgent
...
Save to `~/.actagent/actagent.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.actagent/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 19199,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/actagent" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
    ...
    remote: { url: "ws://gateway.tailnet:19199", token: "remote-token" },
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.actagent/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```

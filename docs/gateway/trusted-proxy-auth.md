...
read_when:
  - Running ACTAgent behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ACTAgent
...
- You run ACTAgent behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth).
...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    ACTAgent checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    ACTAgent extracts the user identity from the configured header.
  </Step>
...
cannot create the device identity that ACTAgent uses to bind operator scopes,
...
This is a break-glass flag (`actagent security audit` reports it as critical).
...
- Internal Gateway clients that do not travel through the reverse proxy should use `gateway.auth.password` / `ACTAGENT_GATEWAY_PASSWORD`, not trusted-proxy identity headers.
...
<Tabs>
  <Tab title="Proxy TLS termination (recommended)">
    ...
    - ACTAgent can stay on loopback HTTP behind the proxy.
  ...
  <Tab title="Gateway TLS termination">
    If ACTAgent itself serves HTTPS directly (no TLS-terminating proxy), set:
...
<AccordionGroup>
  <Accordion title="Pomerium">
    ...
    ```yaml
    routes:
      - from: https://actagent.example.com
        to: http://actagent-gateway:19199
        policy:
          - allow:
              or:
                - email:
  ...
  <Accordion title="Caddy with OAuth">
    ...
    ```
    actagent.example.com {
        authenticate with oauth2_provider
        authorize with policy1

        reverse_proxy actagent:19199 {
  ...
  <Accordion title="nginx + oauth2-proxy">
    ...
    location / {
        ...
        proxy_pass http://actagent:19199;
...
ACTAgent rejects ambiguous configurations where both a `gateway.auth.token` (or `ACTAGENT_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
Loopback trusted-proxy identity headers still fail closed: same-host callers are not silently authenticated as proxy users. Internal ACTAgent callers that bypass the proxy may authenticate with `gateway.auth.password` / `ACTAGENT_GATEWAY_PASSWORD` instead. Token fallback remains intentionally unsupported in trusted-proxy mode.

## Operator scopes header

Trusted-proxy auth is an **identity-bearing** HTTP mode, so callers may optionally declare operator scopes with `x-actagent-scopes`.

Note: `x-actagent-scopes` applies to HTTP endpoints only. WebSocket scopes are
...
- `x-actagent-scopes: operator.read`
- `x-actagent-scopes: operator.read,operator.write`
- `x-actagent-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, ACTAgent honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-actagent-scopes` is absent, their runtime scope falls back to `operator.write`.
...
Practical rule: send `x-actagent-scopes` explicitly when you want a trusted-proxy request to be narrower than the defaults, or when a gateway-auth plugin route needs something stronger than write scope.
...
`actagent security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    ACTAgent rejected a loopback-source trusted-proxy request.
...
<Steps>
  ...
  <Step title="Update ACTAgent config">
    Update ACTAgent config with trusted-proxy auth.
  </Step>
  ...
  <Step title="Audit">
    Run `actagent security audit` and review findings.
  </Step>
</Steps>

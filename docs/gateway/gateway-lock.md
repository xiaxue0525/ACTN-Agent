...
- The gateway then binds the HTTP/WebSocket listener (default `ws://127.0.0.1:19199`) using an exclusive TCP listener.
...
- If the port is occupied by _another_ process, the error is the same; free the port or choose another with `actagent gateway --port <port>`.

...
actagent nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), ACTAgent treats
...
Pairing state is stored under the Gateway state directory (default `~/.actagent`):

- `~/.actagent/nodes/paired.json`
- `~/.actagent/nodes/pending.json`

If you override `ACTAGENT_STATE_DIR`, the `nodes/` folder moves with it.

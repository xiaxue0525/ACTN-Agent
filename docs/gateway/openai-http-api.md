...
ACTAgent's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `actagent agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `ACTAGENT_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `ACTAGENT_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use
  `gateway.auth.password` / `ACTAGENT_GATEWAY_PASSWORD` as a local direct
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, ACTAgent treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-actagent-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-actagent-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-actagent-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-actagent-scopes` when the header is present
...
ACTAgent treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "actagent"` routes to the configured default agent.
- `model: "actagent/default"` also routes to the configured default agent.
- `model: "actagent/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-actagent-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent.
- `x-actagent-agent-id: <agentId>` remains supported as a compatibility override.
- `x-actagent-session-key: <sessionKey>` fully controls session routing.
- `x-actagent-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.

Compatibility aliases still accepted:

- `model: "actagent:<agentId>"`
- `model: "agent:<agentId>"`
...
For custom apps, the safest default is to reuse the same `user` value per conversation thread. Avoid account-level identifiers unless you explicitly want multiple conversations or devices to share one ACTAgent session. Use `x-actagent-session-key` when you need explicit routing control across multiple clients or threads.
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An ACTAgent agent-target list.

    The returned ids are `actagent`, `actagent/default`, and `actagent/<agentId>` entries.
  ...
  <Accordion title="Why is `actagent/default` included?">
    `actagent/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    Use `x-actagent-model`.

    Examples:
    `x-actagent-model: openai/gpt-5.4`
    `x-actagent-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "actagent/default"` or `model: "actagent/<agentId>"`.
    When you need a specific embedding model, send it in `x-actagent-model`.
...
- Base URL: `http://127.0.0.1:19199/v1`
- Docker on macOS base URL: `http://host.docker.internal:19199/v1`
- API key: your Gateway bearer token
- Model: `actagent/default`

Expected behavior:

- `GET /v1/models` should list `actagent/default`
- Open WebUI should use `actagent/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-actagent-model`

Quick smoke:

```bash
curl -sS http://127.0.0.1:19199/v1/models \
  -H 'Authorization: Bearer YOUR_TOKEN'
```

If that returns `actagent/default`, most Open WebUI setups can connect with the same base URL and token.
...
```bash
curl -sS http://127.0.0.1:19199/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
...
```bash
curl -N http://127.0.0.1:19199/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-actagent-model: openai/gpt-5.4' \
  -d '{
    "model": "actagent/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:19199/v1/models/actagent%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
```bash
curl -sS http://127.0.0.1:19199/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-actagent-model: openai/text-embedding-3-small' \
  -d '{
    "model": "actagent/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns ACTAgent agent targets, not raw provider catalogs.
- `actagent/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-actagent-model`, not the OpenAI `model` field.

---
summary: "How ACTAgent sandboxing works: modes, scopes, workspace access, and images"
title: "Sandboxing"
...
ACTAgent can run **tools inside sandbox backends** to reduce blast radius. This is **optional** and controlled by configuration (`agents.defaults.sandbox` or `agents.list[].sandbox`). If sandboxing is off, tools run on the host. The Gateway stays on the host; tool execution runs in an isolated sandbox when enabled.
...
<AccordionGroup>
  <Accordion title="Sandboxed browser details">
    ...
    - By default, sandbox browser containers use a dedicated Docker network (`actagent-sandbox-browser`) instead of the global `bridge` network. Configure with `agents.defaults.sandbox.browser.network`.
    ...
    - noVNC observer access is password-protected by default; ACTAgent emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Sandboxing is off by default. If you enable sandboxing and do not choose a backend, ACTAgent uses the Docker backend. It executes tools and sandbox browsers locally via the Docker daemon socket (`/var/run/docker.sock`). Sandbox container isolation is determined by Docker namespaces.
...
If you deploy the ACTAgent Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config requires host paths**: The `actagent.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.actagent/workspaces`), not the internal Gateway container path. When ACTAgent asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS bridge parity (identical volume map)**: The ACTAgent Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.actagent:/home/user/.actagent`).
- **Codex code mode**: When an ACTAgent sandbox is active, ACTAgent disables Codex app-server native Code Mode, user MCP servers, and app-backed plugin execution for that turn because those native surfaces run from the Gateway-host app-server process instead of the ACTAgent sandbox backend. Shell access is exposed through ACTAgent sandbox-backed tools such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools are available. Do not mount the host Docker socket into agent sandbox containers or custom Codex sandboxes.
...
ACTAgent sandboxing and the service user is not allowed to create unprivileged
...
`actagent doctor`; if it reports a Codex bwrap namespace probe failure, prefer
an AppArmor profile that grants the required namespaces to the ACTAgent service
...
If you map paths internally without absolute host parity, ACTAgent natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.
</Warning>

### SSH backend

Use `backend: "ssh"` when you want ACTAgent to sandbox `exec`, file tools, and media reads on an arbitrary SSH-accessible machine.
    ...
    defaults: {
      sandbox: {
        ...
        ssh: {
          ...
          workspaceRoot: "/tmp/actagent-sandboxes",
...
<AccordionGroup>
  <Accordion title="How it works">
    - ACTAgent creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
    - On first use after create or recreate, ACTAgent seeds that remote workspace from the local workspace once.
    ...
    - ACTAgent does not sync remote changes back to the local workspace automatically.
  ...
  <Accordion title="Authentication material">
    ...
    - `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. ACTAgent resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
  ...
  <Accordion title="Remote-canonical consequences">
    ...
    - Host-local edits made outside ACTAgent after the seed step are not visible remotely until you recreate the sandbox.
    - `actagent sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
...
Use `backend: "openshell"` when you want ACTAgent to sandbox tools in an OpenShell-managed remote environment. For the full setup guide, configuration reference, and workspace mode comparison, see the dedicated [OpenShell page](/gateway/openshell).
...
- `mirror` (default): local workspace stays canonical. ACTAgent syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. ACTAgent seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

<AccordionGroup>
  <Accordion title="Remote transport details">
    - ACTAgent asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
<Tabs>
  <Tab title="mirror (local canonical)">
    ...
    - Before `exec`, ACTAgent syncs the local workspace into the OpenShell sandbox.
    - After `exec`, ACTAgent syncs the remote workspace back to the local workspace.
    - File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

    Use this when:

    - you edit files locally outside ACTAgent and want those changes to show up in the sandbox automatically
  ...
  <Tab title="remote (OpenShell canonical)">
    ...
    - When the sandbox is first created, ACTAgent seeds the remote workspace from the local workspace once.
    - After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
    - ACTAgent does **not** sync remote changes back into the local workspace after exec.
    ...
    - If you edit files on the host outside ACTAgent after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `actagent sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `actagent sandbox recreate` deletes the current runtime and lets ACTAgent recreate it on next use
...
<Tabs>
  <Tab title="none (default)">
    Tools see a sandbox workspace under `~/.actagent/sandboxes`.
  </Tab>
  <Tab title="ro">
...
<Note>
**Skills note:** the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, ACTAgent mirrors eligible skills into the sandbox workspace (`.../skills`) so they can be read. With `"rw"`, workspace skills are readable from `/workspace/skills`.
</Note>
...
- ACTAgent blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- ACTAgent also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. ACTAgent normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `actagent-sandbox:bookworm-slim`

<Note>
**Source checkout vs npm install**

The `scripts/sandbox-setup.sh`, `scripts/sandbox-common-setup.sh`, and `scripts/sandbox-browser-setup.sh` helper scripts are only available when running from a [source checkout](https://github.com/actagent/actagent). They are not included in the npm package.

If you installed ACTAgent via `npm install -g actagent`, use the inline `docker build` commands shown below instead.
</Note>
...
<Steps>
  <Step title="Build the default image">
    ...
    ```bash
    docker build -t actagent-sandbox:bookworm-slim - <<'DOCKERFILE'
    ...
    ACTAgent does not silently substitute plain `debian:bookworm-slim` when `actagent-sandbox:bookworm-slim` is missing. Sandbox runs that target the default image fail fast with a build instruction until you build it, because the bundled image carries `python3` for sandbox write/edit helpers.
  ...
  <Step title="Optional: build the common image">
    ...
    From an npm install, build the default image first (see above), then build the common image on top using the [`scripts/docker/sandbox/Dockerfile.common`](https://github.com/actagent/actagent/blob/main/scripts/docker/sandbox/Dockerfile.common) from the repository.

    Then set `agents.defaults.sandbox.docker.image` to `actagent-sandbox-common:bookworm-slim`.
  ...
  <Step title="Optional: build the sandbox browser image">
    ...
    From an npm install, build using the [`scripts/docker/sandbox/Dockerfile.browser`](https://github.com/actagent/actagent/blob/main/scripts/docker/sandbox/Dockerfile.browser) from the repository.
...
<AccordionGroup>
  <Accordion title="Sandbox browser Chromium defaults">
    ...
    - `--remote-debugging-port=<derived from ACTAGENT_BROWSER_CDP_PORT>`
    ...
    - The three graphics hardening flags (`--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu`) are optional and are useful when containers lack GPU support. Set `ACTAGENT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if your workload requires WebGL or other 3D/browser features.
    - `--disable-extensions` is enabled by default and can be disabled with `ACTAGENT_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
    - `--renderer-process-limit=2` is controlled by `ACTAGENT_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
For Docker gateway deployments, `scripts/docker/setup.sh` can bootstrap sandbox config. Set `ACTAGENT_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can override socket location with `ACTAGENT_DOCKER_SOCKET`. Full setup and env reference: [Docker](/install/docker#agent-sandbox).
...
- Use `actagent sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.

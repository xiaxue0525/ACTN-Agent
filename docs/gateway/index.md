...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `actagent gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `ACTAGENT_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `ACTAGENT_GATEWAY_TOKEN` / `ACTAGENT_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

ACTAgent's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `actagent`, `actagent/default`, and `actagent/<agentId>`.
- `actagent/default` is the stable alias that always maps to the configured default agent.
- Use `x-actagent-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `ACTAGENT_GATEWAY_PORT` → `gateway.port` → `19199` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                    |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `actagent doctor --fix` or `actagent gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
- Unique `gateway.port`
- Unique `ACTAGENT_CONFIG_PATH`
- Unique `ACTAGENT_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
ACTAGENT_CONFIG_PATH=~/.actagent/a.json ACTAGENT_STATE_DIR=~/.actagent-a actagent gateway --port 19001
ACTAGENT_CONFIG_PATH=~/.actagent/b.json ACTAGENT_STATE_DIR=~/.actagent-b actagent gateway --port 19002
```
...
```bash
ssh -N -L 19199:127.0.0.1:19199 user@host
```

Then connect clients locally to `ws://127.0.0.1:19199`.
...
Use `actagent gateway restart` for restarts. Do not chain `actagent gateway stop` and `actagent gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `actagent gateway stop --disable`.

LaunchAgent labels are `ai.actagent.gateway` (default) or `ai.actagent.<profile>` (named profile). `actagent doctor` audits and repairs service config drift.
...
```bash
actagent gateway install
systemctl --user enable --now actagent-gateway[-<profile>].service
actagent gateway status
```
...
[Service]
ExecStart=/usr/local/bin/actagent gateway --port 19199
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `ACTAgent Gateway`
(or `ACTAgent Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, ACTAgent falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now actagent-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/actagent-gateway[-<profile>].service` and adjust
`ExecStart=` if your `actagent` binary lives elsewhere.

Do not also let `actagent doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level ACTAgent gateway service; use `ACTAGENT_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
actagent --dev setup
actagent --dev gateway --allow-unconfigured
actagent --dev status
```

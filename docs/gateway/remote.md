...
- The Gateway WebSocket usually binds to **loopback** on your configured port (defaults to 19199).
...
- Use the macOS app's remote mode (Settings → General → ACTAgent runs).
...
- `actagent health` and `actagent status --deep` now reach the remote gateway via `ws://127.0.0.1:19199`.
- `actagent gateway status`, `actagent gateway health`, `actagent gateway probe`, and `actagent gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `19199` with your configured `gateway.port` (or `--port` or `ACTAGENT_GATEWAY_PORT`).
</Note>
...
When the gateway is loopback-only, keep the URL at `ws://127.0.0.1:19199` and open the SSH tunnel first.
...
- URL override safety:
  ...
  - Env URL overrides (`ACTAGENT_GATEWAY_URL`) may use env credentials only (`ACTAGENT_GATEWAY_TOKEN` / `ACTAGENT_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `ACTAGENT_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `ACTAGENT_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `ACTAGENT_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `ACTAGENT_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `ACTAGENT_GATEWAY_*` only.

## Chat UI remote access

WebChat no longer uses a separate HTTP port. The SwiftUI chat UI connects directly to the Gateway WebSocket.

- Forward `19199` over SSH (see above), then connect clients to `ws://127.0.0.1:19199`.
...
<Note>
If you have a leftover `com.actagent.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>

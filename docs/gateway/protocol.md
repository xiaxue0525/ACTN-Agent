...
ACTAgent. All clients (CLI, web UI, macOS app, iOS/Android nodes, headless
...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to Gateway policy. The first supported tool is `actagent_agent_consult`; clients receive a run id and wait for normal chat lifecycle events before submitting the provider-specific tool result.
...
- Operators may call `skills.search` and `skills.detail` (`operator.read`) for
  ACTAgentHub discovery metadata.
- Operators may call `skills.upload.begin`, `skills.upload.chunk`, and
  ...
  not the normal ACTAgentHub skill install flow, and is disabled by default unless
...
- Operators may call `skills.install` (`operator.admin`) in three modes:
  - ACTAgentHub mode: `{ source: "actagenthub", slug, version?, force? }` installs a
  ...
  - Upload mode: `{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }`
    ...
    affect ACTAgentHub installs.
  - Gateway installer mode: `{ name, installId, timeoutMs? }`
    runs a declared `metadata.actagent.install` action on the gateway host.
...
- Operators may call `skills.update` (`operator.admin`) in two modes:
  - ACTAgentHub mode updates one tracked slug or all tracked ACTAgentHub installs in
...
- Final `agent` results may include `result.deliveryStatus` when delivery was
  ...
  statuses documented for [`actagent agent --json --deliver`](/cli/agent#json-delivery-status).

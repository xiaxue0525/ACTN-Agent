---
summary: "Use OpenShell as a managed sandbox backend for ACTAgent agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for ACTAgent. Instead of running Docker
containers locally, ACTAgent delegates sandbox lifecycle to the `openshell` CLI,
...
- OpenShell plugin installed (`actagent plugins install @actagent/openshell-sandbox`)
- The `openshell` CLI installed and on `PATH` (or set a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- ACTAgent Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, ACTAgent creates an OpenShell
...
- You edit files locally outside ACTAgent and want those changes visible in the
...
- When the sandbox is first created, ACTAgent seeds the remote workspace from
...
- ACTAgent does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside ACTAgent after the initial seed, the remote sandbox does **not** see those changes. Use `actagent sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"actagent"`  | Sandbox source for first-time create                  |
...
1. ACTAgent calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. ACTAgent calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `actagent sandbox` commands

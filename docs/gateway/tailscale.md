...
ACTAgent can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. ACTAgent requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this ACTAgent Serve/Funnel
mode. `off` means ACTAgent is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `ACTAGENT_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ACTAGENT_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. ACTAgent verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
ACTAgent only treats a request as Serve when it arrives from loopback with
...
```json5
{
  gateway: {
    bind: "loopback",
    tailscale: { mode: "serve", serviceName: "svc:actagent" },
  },
}
```

With the example above, startup reports the Service URL as
`https://actagent.<tailnet-name>.ts.net/` instead of the device hostname.
...
- Control UI: `http://<tailscale-ip>:19199/`
- WebSocket: `ws://<tailscale-ip>:19199`

<Note>
Loopback (`http://127.0.0.1:19199`) will **not** work in this mode.
</Note>
...
Prefer `ACTAGENT_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
actagent gateway --tailscale serve
actagent gateway --tailscale funnel --auth password
```
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to
  ...
  `svc:<dns-label>` Service name format, for example `svc:actagent`.
...
- Set `gateway.tailscale.resetOnExit` if you want ACTAgent to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, ACTAgent checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The ACTAgent-managed Funnel password-only policy is unchanged.

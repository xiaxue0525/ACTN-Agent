...
read_when:
  - You need a beginner-friendly overview of ACTAgent logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

ACTAgent has two main log surfaces:
...
ACTAgent keeps up to five numbered archives beside the active file, such as
`actagent-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.actagent/actagent.json`:

```json
{
  "logging": {
    "file": "/path/to/actagent.log"
  }
}
```
...
or times out before `logs.tail` answers, `actagent logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `actagent logs --follow` is stricter: on Linux it uses the active
...
```bash
actagent channels logs --channel whatsapp
```
...
ACTAgent preserves the original structured log arguments alongside these fields
...
`actagent gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
actagent gateway
actagent gateway --verbose --ws-log compact
actagent gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.actagent/actagent.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/actagent/actagent-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`ACTAGENT_LOG_LEVEL`** environment variable (e.g. `ACTAGENT_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `actagent.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `actagent --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
ACTAGENT_DEBUG_MODEL_TRANSPORT=1 actagent gateway
ACTAGENT_DEBUG_MODEL_PAYLOAD=tools ACTAGENT_DEBUG_SSE=events actagent gateway
```

Available flags:

- `ACTAGENT_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `ACTAGENT_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `ACTAGENT_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `ACTAGENT_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `ACTAGENT_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `ACTAGENT_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `ACTAGENT_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal ACTAgent logging, so `actagent logs --follow`
...
ACTAgent writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
ACTAgent can redact sensitive tokens before they hit console output, file logs,
...
policy. ACTAgent still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `ACTAGENT_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `actagent doctor` first.

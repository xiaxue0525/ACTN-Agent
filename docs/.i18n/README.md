...
- repo: `actagent/docs`
- local checkout: `~/Projects/actagent-docs`

## Source of truth

- English docs are authored in `actagent/actagent`.
...
1. Edit English docs in `actagent/actagent`.
2. Push to `main`.
3. `actagent/actagent/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `actagent/docs`.
...
5. `actagent/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `actagent/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `actagent/docs` instead.
...
- Sync metadata is written to `.actagent-sync/source.json` in the publish repo.
- Source repo secret: `ACTAGENT_DOCS_SYNC_TOKEN`
- Publish repo secret: `ACTAGENT_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `actagent/docs` first.

...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ACTAgent
...
Running ACTAgent via Node with `tsx` fails at startup with:

```
[actagent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `actagent status` and `gateway:watch` worked.

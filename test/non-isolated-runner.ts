...
const SHARED_TEST_SETUP = Symbol.for("actagent.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("actagent.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("actagent.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.ACTAGENT_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.ACTAGENT_TEST_HOME = testHome;
...
function resetACTAgentGlobalRunState(): void {
...
export default class ACTAgentNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.ACTAGENT_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {

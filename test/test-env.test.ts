...
function createTempHome(): string {
  return makeTempDir(tempDirs, "actagent-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-actagent.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.actagent/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.actagent/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".actagent", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".actagent", "external-plugins", "glueactagent", "actagent.plugin.json"),
      '{"id":"glueactagent"}\n',
    );
    writeFile(
      path.join(realHome, ".actagent", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ACTAGENT_LIVE_TEST_QUIET = "1";
    process.env.ACTAGENT_CONFIG_PATH = "~/custom-actagent.json5";
    process.env.ACTAGENT_TEST_HOME = priorIsolatedHome;
    process.env.ACTAGENT_STATE_DIR = path.join(priorIsolatedHome, ".actagent");
    ...
    expect(process.env.ACTAGENT_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".actagent", "actagent.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".actagent", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".actagent", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ACTAGENT_LIVE_TEST = "1";
    process.env.ACTAGENT_LIVE_USE_REAL_HOME = "1";
    process.env.ACTAGENT_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ACTAGENT_LIVE_TEST;
    delete process.env.ACTAGENT_LIVE_GATEWAY;
    delete process.env.ACTAGENT_LIVE_USE_REAL_HOME;
    delete process.env.ACTAGENT_LIVE_TEST_QUIET;

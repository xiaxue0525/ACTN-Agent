// ACTAgent test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it } from "vitest";
import { createACTAgentTestInstance, testing } from "./actagent-test-instance.js";
...
describe("actagent test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createACTAgentTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".actagent"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "actagent.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.ACTAGENT_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.ACTAGENT_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.ACTAGENT_SKIP_CRON).toBe("0");

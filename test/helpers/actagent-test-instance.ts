// ACTAgent test instance helper spawns isolated ACTAgent processes.
...
import {
  ...
  type ACTAgentTestStateOptions,
} from "../../src/test-utils/actagent-test-state.js";
import { sleep } from "../../src/utils.js";

export type ACTAgentTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<ACTAgentTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type ACTAgentTestInstanceCommandResult = {
  code: number | null;
...
export type ACTAgentTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<ACTAgentTestInstanceCommandResult>;
...
export async function createACTAgentTestInstance(
  options: ACTAgentTestInstanceOptions,
): Promise<ACTAgentTestInstance> {
  ...
  const instance: ACTAgentTestInstance = {
    name: options.name,
    port,

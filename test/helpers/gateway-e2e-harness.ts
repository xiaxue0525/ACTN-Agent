...
import { createACTAgentTestInstance, type ACTAgentTestInstance } from "./actagent-test-instance.js";
...
export type GatewayInstance = ACTAgentTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createACTAgentTestInstance({ name });
  try {

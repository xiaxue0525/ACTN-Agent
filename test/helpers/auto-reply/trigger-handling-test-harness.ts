...
const embeddedAgentMocks = getSharedMocks("actagent.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("actagent.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("actagent.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("actagent.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    actagentHome: process.env.ACTAGENT_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("ACTAGENT_HOME", snapshot.actagentHome);
  restoreKey("ACTAGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ACTAGENT_STATE_DIR = join(home, ".actagent");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "actagent-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".actagent", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): ACTAgentConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "actagent"),

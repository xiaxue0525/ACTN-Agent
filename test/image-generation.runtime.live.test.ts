...
import { loadConfig, type ACTAgentConfig } from "../src/config/config.js";
import {
...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.ACTAGENT_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.ACTAGENT_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.ACTAGENT_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.ACTAGENT_LIVE_IMAGE_GENERATION_MODELS);
const DEFAULT_LIVE_IMAGE_GENERATION_TIMEOUT_MS = 120_000;
const LIVE_IMAGE_GENERATION_TIMEOUT_MS = resolvePositiveIntegerEnv(
  process.env.ACTAGENT_LIVE_IMAGE_GENERATION_TIMEOUT_MS,
...
function withPluginsEnabled(cfg: ACTAgentConfig): ACTAgentConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,

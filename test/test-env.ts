...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ACTAGENT_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ACTAGENT_TEST_FAST", value: process.env.ACTAGENT_TEST_FAST },
    {
      key: "ACTAGENT_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ACTAGENT_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ACTAGENT_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ACTAGENT_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ACTAGENT_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ACTAGENT_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ACTAGENT_STATE_DIR", value: process.env.ACTAGENT_STATE_DIR },
    { key: "ACTAGENT_CONFIG_PATH", value: process.env.ACTAGENT_CONFIG_PATH },
    { key: "ACTAGENT_GATEWAY_PORT", value: process.env.ACTAGENT_GATEWAY_PORT },
    { key: "ACTAGENT_BRIDGE_ENABLED", value: process.env.ACTAGENT_BRIDGE_ENABLED },
    { key: "ACTAGENT_BRIDGE_HOST", value: process.env.ACTAGENT_BRIDGE_HOST },
    { key: "ACTAGENT_BRIDGE_PORT", value: process.env.ACTAGENT_BRIDGE_PORT },
    { key: "ACTAGENT_CANVAS_HOST_PORT", value: process.env.ACTAGENT_CANVAS_HOST_PORT },
    { key: "ACTAGENT_TEST_HOME", value: process.env.ACTAGENT_TEST_HOME },
    { key: "ACTAGENT_AGENT_DIR", value: process.env.ACTAGENT_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ACTAGENT_TEST_HOME = tempHome;
  process.env.ACTAGENT_TEST_FAST = "1";
  process.env.ACTAGENT_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ACTAGENT_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ACTAGENT_BRIDGE_ENABLED;
  delete process.env.ACTAGENT_BRIDGE_HOST;
  delete process.env.ACTAGENT_BRIDGE_PORT;
  delete process.env.ACTAGENT_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ACTAGENT_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ACTAGENT_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".actagent");
  const priorIsolatedHome = params.env.ACTAGENT_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".actagent")
  ) {
    realStateDir = path.join(params.realHome, ".actagent");
  }
  const tempStateDir = path.join(params.tempHome, ".actagent");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ACTAGENT_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ACTAGENT_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "actagent.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "actagent.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ACTAGENT_LIVE_TEST === "1" ||
    process.env.ACTAGENT_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ACTAGENT_LIVE_USE_REAL_HOME);

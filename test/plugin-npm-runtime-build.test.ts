...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.actagent).toBe(
        plan.packageJson.actagent.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.actagent.optional).toBe(true);
    }
  });

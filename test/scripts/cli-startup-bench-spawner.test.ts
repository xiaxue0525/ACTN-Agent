...
describe("CLI startup benchmark script spawners", () => {
  ...
  it("does not require unrelated fixture cases for a narrowed preset", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-bench-budget-test-"));
    try {
      ...
      expect(() =>
        execFileSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              ACTAGENT_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH: "1",
            },
            stdio: "pipe",
          },
        ),
  ...
  it("skips x64 startup budgets on noncanonical architectures", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-bench-budget-arch-test-"));
    try {
  ...
  it("rejects malformed startup budget env vars before reading reports", () => {
    const result = spawnSync(process.execPath, ["scripts/test-cli-startup-bench-budget.mjs"], {
      ...
      env: {
        ...process.env,
        ACTAGENT_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT: "20pct",
      },
    });
    ...
    expect(result.stderr).toContain(
      "ACTAGENT_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT must be a non-negative number",
    );

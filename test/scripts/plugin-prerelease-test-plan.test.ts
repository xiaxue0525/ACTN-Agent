...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ACTAgentHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "ACTAGENT_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@actagent/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@actagent/kitchen-sink@beta");
    expect(script).toContain("actagenthub:@actagent/kitchen-sink@latest");
    expect(script).toContain("actagenthub:@actagent/kitchen-sink@beta");
    expect(script).toContain("ACTAGENT_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    ...
    expect(sweepScript).toContain("ACTAGENT_KITCHEN_SINK_PERSONALITY");
    ...
    expect(assertionsScript).toContain("record.actagenthubPackage !== packageName");
    expect(assertionsScript).toContain("record.actagentpackSha256");
    expect(assertionsScript).toContain("record.artifactKind");
    expect(assertionsScript).toContain("record.npmIntegrity");
    expect(assertionsScript).toContain("assertACTAgentHubExternalInstallContract");
    ...
    expect(readFileSync("scripts/e2e/lib/actagenthub-fixture-server.cjs", "utf8")).toContain(
      'from "actagent/plugin-sdk/plugin-entry"',
    );
    expect(readFileSync("scripts/e2e/lib/actagenthub-fixture-server.cjs", "utf8")).toContain(
      "X-ACTAgentHub-Artifact-Sha256",
    );
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("ACTAGENT_ENTRY=/app/actagent.mjs");
    expect(script).toContain("ACTAGENT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltACTAgentEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    const actagenthubScript = readFileSync("scripts/e2e/lib/plugins/actagenthub.sh", "utf8");
    const assertionsScript = readFileSync("scripts/e2e/lib/plugins/assertions.mjs", "utf8");
    const fixtureServer = readFileSync("scripts/e2e/lib/actagenthub-fixture-server.cjs", "utf8");
    const prereleasePlan = createPluginPrereleaseTestPlan();

    expect(lane).toEqual({
      command: "ACTAGENT_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(sweepScript).toContain("run_plugins_actagenthub_scenario");
    expect(actagenthubScript).toContain('plugins install "$ACTAGENTHUB_PLUGIN_SPEC"');
    expect(assertionsScript).toContain("assertACTAgentHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "actagent');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('actagent: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(inspectorRun.env).toEqual({
      ACTAGENT_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      ACTAGENT_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@actagent/plugin-inspector@${ACTAGENT_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/actagent-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "actagent-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
      "cancel-in-progress": "${{ startsWith(github.ref, 'refs/heads/tideactagent/alpha/') }}",
    });
    expect(fullReleaseWorkflow.concurrency).toEqual({
      group: "full-release-validation-${{ inputs.ref }}-${{ inputs.rerun_group }}",
      "cancel-in-progress":
        "${{ (inputs.ref == 'main' && inputs.rerun_group == 'all') || startsWith(inputs.ref, 'tideactagent/alpha/') }}",
    });
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg ACTAGENT_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
  ...
  it("keeps runtime tool coverage blocking in release checks", () => {
    const releaseChecksSource = readFileSync(
      ".github/workflows/actagent-release-checks.yml",
      "utf8",
    );
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm actagent qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, ACTAGENT_DISCORD_TOKEN)");

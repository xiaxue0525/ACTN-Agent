...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "actagent-restart-mac-test-"));
  tempRoots.push(root);
  ...
  return spawnSync(
    "bash",
    ["-c", `source ${shellQuote(helperPath)}; verify_gateway_port_listening 19199`],
    {
      encoding: "utf8",
      env: {
        ...process.env,
...
describe("scripts/restart-mac.sh", () => {
  it("fails the gateway verification when lsof finds no listener", () => {
    ...
    expect(result.stderr).toContain("No process is listening on gateway port 19199.");
  ...
  it("prints listener diagnostics when the gateway port is open", () => {
    const result = runGatewayPortCheck(
      [
        ...
        "printf '%s\\n' 'node    12345 user   21u  IPv4 0x123      0t0  TCP 127.0.0.1:19199 (LISTEN)'",
      ].join("\n"),
    );

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("127.0.0.1:19199 (LISTEN)");
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${ACTAGENT_RESTART_LOG:-/tmp/actagent-restart.log}"');
  });
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(chooseBlock).toContain('fail "ACTAGENT_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/ACTAgent.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/ACTAgent.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/ACTAgent.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/ACTAgent.app"),
    );
  });
});

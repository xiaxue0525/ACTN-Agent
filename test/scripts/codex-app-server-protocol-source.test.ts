...
const originalACTAgentCodexRepo = process.env.ACTAGENT_CODEX_REPO;

afterEach(() => {
  if (originalACTAgentCodexRepo === undefined) {
    delete process.env.ACTAGENT_CODEX_REPO;
  } else {
    process.env.ACTAGENT_CODEX_REPO = originalACTAgentCodexRepo;
  }
});
...
describe("codex app-server protocol source resolver", () => {
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ ACTAGENT_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ ACTAGENT_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses ACTAGENT_CODEX_REPO when provided", async () => {
    const root = createTempDir("actagent-protocol-source-root-");
    const codexRepo = createTempDir("actagent-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.ACTAGENT_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("actagent-protocol-source-parent-");
    const primaryACTAgent = path.join(parentDir, "actagent");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("actagent-protocol-source-worktree-");
    fs.mkdirSync(path.join(primaryACTAgent, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primaryACTAgent, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.ACTAGENT_CODEX_REPO;

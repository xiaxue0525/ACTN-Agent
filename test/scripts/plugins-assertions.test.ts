...
function writeFixtureServerShims(binDir: string, pidPath: string): void {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/bash",
      'printf "%s\\n" "$$" >"$ACTAGENT_TEST_FIXTURE_SERVER_PID"',
      "trap 'exit 0' TERM",
...
describe("plugins Docker assertions", () => {
  it("rejects loose ACTAgentHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "actagenthub-preflight"], {
      encoding: "utf8",
      env: {
        ...process.env,
        ACTAGENTHUB_PLUGIN_SPEC: "actagenthub:@actagent/kitchen-sink",
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );

    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "actagenthub-preflight"], {
      encoding: "utf8",
      env: {
        ...process.env,
        ACTAGENTHUB_PLUGIN_SPEC: "actagenthub:@actagent/kitchen-sink",
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    const scripts = [
      ...
      "scripts/e2e/lib/plugins/actagenthub.sh",
    ];

    for (const scriptPath of scripts) {
      const script = readFileSync(scriptPath, "utf8");
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/actagent-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("ACTAGENT_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|actagent-plugin|actagent-actagenthub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export ACTAGENT_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$ACTAGENT_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$ACTAGENT_PLUGINS_TMP_DIR"
cleanup_actagent_plugins_sweep
test ! -e "$ACTAGENT_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/actagent-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-plugin-sweep-caller-"));
...
set -euo pipefail
export ACTAGENT_PLUGINS_SWEEP_SOURCE_ONLY=1
export ACTAGENT_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$ACTAGENT_PLUGINS_TMP_DIR"
cleanup_actagent_plugins_sweep
test -d "$ACTAGENT_PLUGINS_TMP_DIR"
`,
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ACTAGENT_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("cleans ACTAgentHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-plugin-actagenthub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            "set +e",
            `( set -e; trap 'printf caller-cleanup > ${shellQuote(cleanupPath)}' EXIT; run_plugins_actagenthub_scenario )`,
            'status="$?"',
            "set -e",
            '[ "$status" != "0" ]',
          ].join("\n"),
        ],
        {
          ...
          env: {
            ...process.env,
            ACTAGENT_PLUGINS_E2E_LIVE_ACTAGENTHUB: "0",
            ACTAGENT_PLUGINS_TMP_DIR: tmpDir,
            ACTAGENT_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".actagent", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          ACTAGENT_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".actagent", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".actagent", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("times out stalled ACTAgentHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["actagenthub-preflight"], {
        ACTAGENTHUB_PLUGIN_ID: "actagent-kitchen-sink-fixture",
        ACTAGENTHUB_PLUGIN_SPEC: "actagenthub:@actagent/kitchen-sink",
        ACTAGENT_ACTAGENTHUB_URL: `http://127.0.0.1:${address.port}`,
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ACTAgentHub package preflight for @actagent/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled ACTAgentHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["actagenthub-preflight"], {
        ...
        ACTAGENTHUB_PLUGIN_SPEC: "actagenthub:@actagent/kitchen-sink",
        ACTAGENT_ACTAGENTHUB_URL: `http://127.0.0.1:${address.port}`,
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ACTAgentHub package preflight response for @actagent/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds ACTAgentHub package metadata response bodies", async () => {
    const server = createServer((_request, response) => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["actagenthub-preflight"], {
        ...
        ACTAGENTHUB_PLUGIN_SPEC: "actagenthub:@actagent/kitchen-sink",
        ACTAGENT_ACTAGENTHUB_URL: `http://127.0.0.1:${address.port}`,
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        ACTAGENT_PLUGINS_E2E_ACTAGENTHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ACTAgentHub package preflight response for @actagent/kitchen-sink response body exceeded 16 bytes",
      );

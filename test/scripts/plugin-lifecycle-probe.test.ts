...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "actagent-plugin-lifecycle-probe-"));
  tempDirs.push(dir);
  return dir;
}
...
function runProbe(args: string[], home = makeTempDir()) {
  return spawnSync(process.execPath, [probePath, ...args], {
    ...
    env: {
      ...process.env,
      HOME: home,
      ACTAGENT_CONFIG_PATH: path.join(home, ".actagent", "actagent.json"),
      USERPROFILE: home,
    },
  });
}
...
describe("plugin lifecycle matrix probe", () => {
  it("accepts inspect JSON for an enabled loaded plugin", () => {
    ...
    writeFileSync(
      inspectPath,
      `${JSON.stringify({ plugin: { enabled: true, id: "lifecycle-actagent", status: "loaded" } })}\n`,
      "utf8",
    );

    const result = runProbe(["assert-inspect-loaded", "lifecycle-actagent", inspectPath], dir);
  ...
  it("rejects inspect JSON that does not prove the runtime loaded", () => {
    ...
    writeFileSync(
      inspectPath,
      `${JSON.stringify({ plugin: { enabled: true, id: "lifecycle-actagent", status: "pending" } })}\n`,
      "utf8",
    );
    ...
    expect(result.stderr).toContain("expected lifecycle-actagent inspect status loaded, got pending");
  });

...
describe("happy path prompt snapshots", () => {
  ...
  it("deletes stale generated snapshot artifacts", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-prompt-snapshot-stale-"));
    try {
  ...
  it("renders the Codex model-bound prompt layers", async () => {
    ...
    expect(telegram).toContain("ACTAgent runtime context for this turn:");
  ...
  it("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    const agentSoulHeading = "## ACTAgent Agent Soul";
    ...
    expect(direct).not.toContain("This is an ACTAgent heartbeat turn.");
    expect(group).not.toContain("This is an ACTAgent heartbeat turn.");

    expect(heartbeat).toContain('"collaborationMode": {');
    expect(heartbeat).toContain('"developer_instructions": "This is an ACTAgent heartbeat turn.');
    expect(heartbeat).toContain(agentSoulHeading);
    const actAgentRuntimeInstructions = renderedPromptSection(
      heartbeat,
    ...
    expect(actAgentRuntimeInstructions).not.toContain(heartbeatPhrase);
    ...
    expect(collaborationModeInstructions).toContain(
      "/tmp/actagent-happy-path/workspace/HEARTBEAT.md",
    );
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {

...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/actagent-pr-maintainer/scripts/github-activity.sh",
);
...
describe("actagent-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("actagent/actagent last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/actagent\/actagent\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/actagent\/actagent\/commits/g)).toHaveLength(1);

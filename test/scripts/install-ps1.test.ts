...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "ACTAgent installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-ACTAgent");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const actAgentPathBody = extractFunctionBody(source, "Ensure-ACTAgentOnPath");
    ...
    expect(actAgentPathBody).toContain('Invoke-NpmCommand -Arguments @("config", "get", "prefix")');
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "actagent")');
  ...
  it("rejects ACTAgent GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-ACTAgent");
    const sourceTargetBody = extractFunctionBody(source, "Test-ACTAgentSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:actagent/actagent");
    expect(npmInstallBody).toContain("Test-ACTAgentSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain("npm installs do not support ACTAgent GitHub source targets");
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmACTAgentInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-ACTAgentDepsRoot");
    ...
    expect(depsRootBody).toContain("ACTAgent\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-ACTAgentDepsRoot");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-ACTAgentFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-ACTAgentFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-ACTAgent)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] ACTAgent build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveACTAgentCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveACTAgentCommand onboard");
  });

  runIfPowerShell("fails install when interactive onboarding exits non-zero", () => {
    const tempDir = harness.createTempDir("actagent-install-ps1-");
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingACTAgent { return $false }",
        "function Get-NpmCommandPath { return 'npm.cmd' }",
        "function Install-ACTAgent { return $true }",
        "function Ensure-ACTAgentOnPath { return $true }",
        "function Add-ToUserPath { param([string]$Path) }",
        "function Get-ACTAgentCommandPath { return 'cmd.exe' }",
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "actagent onboard failed with exit code 17",
    );
  });
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=ACTAgent installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "  return $false",
        "}",
        "function Ensure-ACTAgentOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\actagent-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-ACTAgent { Write-Output 'npm stdout'; return $true }",
        "function Ensure-ACTAgentOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-ACTAgentCommand { return 'ACTAgent test-version' }",
        "$NoOnboard = $true",
        "$result = Main",

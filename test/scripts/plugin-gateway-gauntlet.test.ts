...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("discovers bundled plugin manifests into lifecycle matrix rows", async () => {
    ...
    expect(matrix[0]).toEqual({
      ...
      id: "alpha",
      manifestPath: path.join("extensions", "alpha", "actagent.plugin.json"),
      name: "alpha",
  ...
  it("keeps manifest ids separate from bounded build entry ids", async () => {
    await writeManifest("kimi-coding", "actagent.plugin.json", JSON.stringify({ id: "kimi" }));
    ...
    expect(buildGauntletPrebuildEnv({}, { buildIds: [matrix[0].buildId] })).toEqual({
      ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS: "kimi-coding",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });

  it("skips source-only plugin dirs that are excluded from the built runtime", async () => {
    await writeManifest("qa-lab", "actagent.plugin.json", JSON.stringify({ id: "qa-lab" }));
    await writeManifest("qqbot", "actagent.plugin.json", JSON.stringify({ id: "qqbot" }));
    await writeManifest("telegram", "actagent.plugin.json", JSON.stringify({ id: "telegram" }));
  ...
  it("prebuilds private QA dist when QA chunks are enabled", () => {
    expect(buildGauntletPrebuildEnv({ EXISTING: "1" }, { includePrivateQa: true })).toEqual({
      ...
      ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS: "qa-channel,qa-lab,qa-matrix",
      ACTAGENT_ENABLE_PRIVATE_QA_CLI: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  ...
  it("marks gauntlet prebuilds as runtime-only when requested", () => {
    expect(
      ...
      EXISTING: "1",
      ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS: "acpx",
  ...
  it("prebuilds only selected plugin dist entries for bounded gauntlet runs", () => {
    expect(
      ...
      EXISTING: "1",
      ACTAGENT_BUILD_PRIVATE_QA: "1",
      ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS: "acpx,active-memory,qa-channel,qa-lab,qa-matrix",
      ACTAGENT_ENABLE_PRIVATE_QA_CLI: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
...
`,
  ...
  it("probes plugin-owned slash help while the plugin is installed", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const stateDir = process.env.ACTAGENT_STATE_DIR ?? process.cwd();",
        ...
        '    console.log("Usage: actagent workboard");',
        "    process.exit(0);",
        "  }",
  ...
  it("carries bounded build ids into QA run-node chunks", async () => {
    ...
    await writeManifest("alpha", "actagent.plugin.json", JSON.stringify({ id: "alpha" }));
    ...
    await fs.writeFile(
      ...
      [
        ...
        'fs.writeFileSync(path.join(outputDir, "env.txt"), process.env.ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS ?? "", "utf8");',

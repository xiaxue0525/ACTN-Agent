...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'actagent';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("actagent-startup-metadata-timeout-");
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("actagent-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: actagent\n",
      renderSourceBrowserHelpText: () => "Usage: actagent browser\n",
      renderSourceSecretsHelpText: () => "Usage: actagent secrets\n",
      renderSourceNodesHelpText: () => "Usage: actagent nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: actagent doctor\n",
        gateway: "Usage: actagent gateway\n",
        models: "Usage: actagent models\n",
        plugins: "Usage: actagent plugins\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("actagent browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("actagent secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("actagent nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("actagent");
    expect(written.subcommandHelpText.doctor).toContain("actagent doctor");
    expect(written.subcommandHelpText.gateway).toContain("actagent gateway");
    expect(written.subcommandHelpText.models).toContain("actagent models");
    expect(written.subcommandHelpText.plugins).toContain("actagent plugins");
  });

  it("renders independent startup help snapshots concurrently", async () => {
    const tempRoot = createTempDir("actagent-startup-metadata-concurrency-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writePromise = writeCliStartupMetadata({
      ...
      renderBundledRootHelpText: async () => "Usage: actagent\n",
      renderSourceBrowserHelpText: renderAfterUnblock("browser", "Usage: actagent browser\n"),
      renderSourceSecretsHelpText: renderAfterUnblock("secrets", "Usage: actagent secrets\n"),
      renderSourceNodesHelpText: renderAfterUnblock("nodes", "Usage: actagent nodes\n"),
  ...
  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("actagent-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: actagent nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: actagent doctor\n",
          gateway: "Usage: actagent gateway\n",
    ...
    expect(written.nodesHelpText).toContain("actagent nodes 3");
  });
});

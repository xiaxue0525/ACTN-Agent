...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "actagent-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "actagenthub",
          spec: "actagenthub:@actagent/matrix",
          installPath: matrixInstallDir,
          actagenthubPackage: "@actagent/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        ACTAGENT_TEST_WORKSPACE_DIR: workspace,
        ACTAGENT_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        ACTAGENT_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  it("accepts official ACTAgentHub npm-pack installs for configured external plugins", () => {
    expect(() => assertConfiguredPluginState()).not.toThrow();
  });

  it("rejects ACTAgentHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "actagent-upgrade-survivor-outside-"));
    try {
      expect(() =>

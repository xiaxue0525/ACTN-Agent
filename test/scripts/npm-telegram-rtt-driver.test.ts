...
function runDriver(env: Record<string, string>) {
  return spawnSync(process.execPath, [DRIVER_SCRIPT], {
    ...
    env: {
      ...process.env,
      ACTAGENT_QA_TELEGRAM_API_BASE_URL: "http://127.0.0.1:9",
      ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver-token",
      ACTAGENT_QA_TELEGRAM_GROUP_ID: "-100123",
      ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN: "sut-token",
      ...env,
    },
  });
}
...
async function runStalledTelegramBodyCase(): Promise<DriverCaseResult> {
  const root = mkdtempSync(path.join(tmpdir(), "actagent-telegram-rtt-driver-"));
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [DRIVER_SCRIPT], {
      ...
      env: {
        ...process.env,
        ACTAGENT_NPM_TELEGRAM_BOT_API_TIMEOUT_MS: "100",
        ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR: outputDir,
        ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES: "1",
        ACTAGENT_QA_TELEGRAM_API_BASE_URL: `http://127.0.0.1:${port}`,
        ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS: "1000",
        ACTAGENT_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver-token",
        ACTAGENT_QA_TELEGRAM_GROUP_ID: "-100123",
        ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "1000",
        ACTAGENT_QA_TELEGRAM_SUT_BOT_TOKEN: "sut-token",
      },
      killSignal: "SIGKILL",
      timeout: 2500,
...
async function runOversizedTelegramBodyCase(): Promise<DriverCaseResult> {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [DRIVER_SCRIPT], {
      ...
      env: {
        ...process.env,
        ACTAGENT_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES: "16",
        ACTAGENT_NPM_TELEGRAM_BOT_API_TIMEOUT_MS: "1000",
        ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR: outputDir,
        ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES: "1",
        ACTAGENT_QA_TELEGRAM_API_BASE_URL: `http://127.0.0.1:${port}`,
        ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS: "1000",
...
describe("npm Telegram RTT driver", () => {
  ...
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    for (const [name, value] of [
      ["ACTAGENT_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", "180000ms"],
      ["ACTAGENT_QA_TELEGRAM_CANARY_TIMEOUT_MS", "1e3"],
      ["ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES", "20samples"],
      ["ACTAGENT_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS", "30000ms"],
      ["ACTAGENT_NPM_TELEGRAM_BOT_API_TIMEOUT_MS", "100ms"],
      ["ACTAGENT_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES", "1mb"],
      ["ACTAGENT_NPM_TELEGRAM_MAX_FAILURES", "2failures"],
    ]) {
  ...
  it("rejects zero where positive numeric env values are required", () => {
    const result = runDriver({ ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES: "0" });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid ACTAGENT_NPM_TELEGRAM_WARM_SAMPLES: 0");
  });

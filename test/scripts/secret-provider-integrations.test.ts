...
function makeTempDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-secret-provider-proof-"));
  tempDirs.push(root);
  return root;
}

function writeStallingACTAgent(
  root: string,
  options: {
  ...
  const descendantScript = options.gatewayDescendantMarkerPath
    ...
    : "";
  const scriptPath = path.join(root, "fake-actagent.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
      ...
      "  process.exit(0);",
      "}",
      "console.error(`unexpected fake actagent args: ${args.join(' ')}`);",
      "process.exit(2);",
      "",
    ].join("\n"),
    { mode: 0o755 },
...
function writeLeakingStartupACTAgent(root: string): string {
  const scriptPath = path.join(root, "fake-leaking-actagent.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeSignaledStartupACTAgent(root: string): string {
  const scriptPath = path.join(root, "fake-signaled-actagent.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      ACTAGENT_ENTRY: fakeACTAgent,
      ACTAGENT_SECRET_PROOF_READY_MS: "60",
      ACTAGENT_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed ACTAgent commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveACTAgentCommand(
      ["gateway", "status"],
      { ...process.env, ACTAGENT_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["actagent"], label: "pnpm actagent" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "actagent", "gateway", "status"]);
    expect(command.options.env.ACTAGENT_SECRET_PROOF_SENTINEL).toBe("1");
    expect(command.options.shell).toBe(false);
  });

  it("keeps stalled startup health probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeACTAgent = writeStallingACTAgent(root);
    const result = runProofHarness(root, fakeACTAgent, "start");
  ...
  it("fails fast when startup exits by signal", () => {
    const root = makeTempDir();
    const fakeACTAgent = writeSignaledStartupACTAgent(root);
    const result = runProofHarness(root, fakeACTAgent, "start", {
      ACTAGENT_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeACTAgent, "start", {
      ACTAGENT_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.ACTAGENT_SECRET_PROOF_OUTPUT_BYTES;
    process.env.ACTAGENT_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.ACTAGENT_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.ACTAGENT_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });
  ...
  it("fails when proof temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        proof.cleanupEnv("/tmp/actagent-secret-provider-proof-stuck", {
          attempts: 3,
          retryDelayMs: 1,
        }),
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    const root = makeTempDir();
    const fakeACTAgent = writeLeakingStartupACTAgent(root);
    const result = runProofHarness(root, fakeACTAgent, "startup-fails", {
      ACTAGENT_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
  ...
  it("keeps stalled managed status probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeACTAgent = writeStallingACTAgent(root);
    const result = runProofHarness(root, fakeACTAgent, "status");

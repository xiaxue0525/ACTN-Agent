...
describe("gh-read helpers", () => {
  ...
  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "actagent/actagent"])).toBe("actagent/actagent");
    expect(parseRepoArg(["run", "list", "--repo=actagent/docs"])).toBe("actagent/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("actagent/actagent")).toBe("actagent/actagent");
    expect(normalizeRepo("github.com/actagent/actagent")).toBe("actagent/actagent");
    expect(normalizeRepo("https://github.com/actagent/actagent.git")).toBe("actagent/actagent");
    expect(normalizeRepo("git@github.com:actagent/actagent.git")).toBe("actagent/actagent");
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /ACTAGENT_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});

...
const PLUGINS_DOCKER_ACTAGENTHUB_PATH = "scripts/e2e/lib/plugins/actagenthub.sh";
...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("ACTAGENT_DOCKER_BUILD_RETRIES");
    expect(helper).toContain("ACTAGENT_DOCKER_BUILD_TIMEOUT");
    expect(helper).toContain('docker_build_run_logged "$label" "$timeout_value" "$log_file"');
    expect(helper).toContain("ACTAGENT_DOCKER_BUILD_REQUIRE_TIMEOUT");
  ...
  it("routes standalone Docker smoke runs through the timeout-aware helper", () => {
    ...
    expect(cleanupSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${ACTAGENT_CLEANUP_SMOKE_DOCKER_TIMEOUT:-600s}}"',
    );
    expect(cleanupSmoke).toContain(
  ...
  it("runs cleanup smoke on the native ARM platform instead of pulling an amd64 tag", () => {
    ...
    expect(
      runCleanupDefaultPlatform({ ACTAGENT_CLEANUP_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("ACTAGENT_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("ACTAGENT_TESTBOX");
    ...
    expect(liveBuild).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${ACTAGENT_LIVE_DOCKER_PULL_TIMEOUT:-180s}}"',
    );
    expect(liveBuild).toContain(
      'LIVE_IMAGE_PULL_ATTEMPTS="${ACTAGENT_LIVE_DOCKER_PULL_ATTEMPTS:-3}"',
    );
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (ACTAGENT_SKIP_DOCKER_BUILD=1)"',
    );
  });

  it("wraps centralized Docker builds with the timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-timeout-"));
...
export ACTAGENT_DOCKER_BUILD_TIMEOUT=17s
...
`;
  ...
  it("prints heartbeat progress for long successful centralized Docker builds", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ACTAGENT_DOCKER_BUILD_HEARTBEAT_SECONDS=1
...
`;
  ...
  it("stops the tracked build command without retrying when interrupted", async () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-signal-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ACTAGENT_DOCKER_BUILD_RETRIES=3
source "$ROOT_DIR/scripts/lib/docker-build.sh"
docker_build_run e2e-build -t demo-image .
`,
      );
  ...
  it("does not delay fast successful centralized Docker builds until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-fast-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ACTAGENT_DOCKER_BUILD_HEARTBEAT_SECONDS=30
...
ROOT_DIR=${shellQuote(rootDir)}
export ROOT_DIR
export ACTAGENT_DOCKER_BUILD_HEARTBEAT_SECONDS=08
...
`;
  ...
  it("fails centralized Docker builds fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-timeout-required-"));
...
export ACTAGENT_DOCKER_BUILD_TIMEOUT=19s
...
`;
  ...
  it("keeps setup-style Docker builds compatible when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-timeout-optional-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ACTAGENT_DOCKER_BUILD_TIMEOUT=23s
...
`;
  ...
  it("keeps reused Docker image probes behind the timeout-aware helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-image-reuse-timeout-"));
...
export ACTAGENT_SKIP_DOCKER_BUILD=1
...
grep -q '^image inspect actagent-reuse-image$' "$TMPDIR/docker-seen"
grep -q '^pull actagent-reuse-image$' "$TMPDIR/docker-seen"
`;
  ...
  it("fails Docker commands fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-timeout-required-"));
...
`;
  ...
  it("uses a Node watchdog for Docker commands when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-node-timeout-"));
...
`;
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Docker watchdog children that ignore parent SIG${shellSignal}`, () => {
      const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-node-signal-"));
...
export ACTAGENT_DOCKER_TIMEOUT_KILL_GRACE_MS=100
...
`;
  ...
  it("uses plain timeout when kill-after is unsupported", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-plain-timeout-"));
...
`;
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ACTAGENT_DOCKER_E2E_RUN_TIMEOUT=13s
...
`;
  ...
  it("keeps package-backed Docker runs bounded without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-package-timeout-required-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ACTAGENT_DOCKER_E2E_RUN_TIMEOUT=11s
...
`;
  ...
  it("uses gtimeout for package-backed Docker runs without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-package-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ACTAGENT_DOCKER_E2E_RUN_TIMEOUT=15s
...
`;
  ...
  it("removes functional Docker build package inputs after the build", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-cleanup-"));
...
node() {
  local script="$1"
  shift
  if [[ "$script" != "$ROOT_DIR/scripts/package-actagent-for-docker.mjs" ]]; then
    command node "$script" "$@"
    return
  fi
...
docker_build_run() {
  ...
  for arg in "$@"; do
    case "$arg" in
      actagent_package=*)
        build_context="\${arg#actagent_package=}"
        ;;
    esac
  done

  test -n "$build_context"
  test -f "$build_context/actagent-current.tgz"
...
leftovers="$(find "$TMPDIR" -maxdepth 1 \\( \\
  -name 'actagent-docker-e2e-pack.*' \\
  -o -name 'actagent-docker-e2e-package-context.*' \\
\\) -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("keeps caller-provided functional Docker build packages", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-build-external-package-"));
...
printf fixture >"$external_dir/actagent-current.tgz"
ACTAGENT_CURRENT_PACKAGE_TGZ="$external_dir/actagent-current.tgz"
export ACTAGENT_CURRENT_PACKAGE_TGZ
...
test -f "$TMPDIR/build-context-seen"
test -f "$ACTAGENT_CURRENT_PACKAGE_TGZ"
leftovers="$(find "$TMPDIR" -maxdepth 1 -name 'actagent-docker-e2e-package-context.*' -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("cleans generated package mounts after harness Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-package-mount-cleanup-"));
...
test ! -e "$pack_dir"
test -z "$(find "$TMPDIR" -maxdepth 1 -name 'actagent-docker-e2e-container.*' -print)"

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/actagent-current.tgz"
docker_e2e_package_mount_args "$external_dir/actagent-current.tgz"
...
test -f "$external_dir/actagent-current.tgz"
`;
  ...
  it("propagates shared E2E command timeouts into package-backed containers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-package-timeout-env-"));
...
package="$TMPDIR/actagent-current.tgz"
printf fixture >"$package"
export ACTAGENT_E2E_NPM_INSTALL_TIMEOUT=42s
export ACTAGENT_E2E_COMMAND_TIMEOUT=23s
docker_e2e_package_mount_args "$package"
printf "%s\\n" "\${DOCKER_E2E_PACKAGE_ARGS[@]}" >"$TMPDIR/package-args"

grep -qx -- "-e" "$TMPDIR/package-args"
grep -qx -- "ACTAGENT_CURRENT_PACKAGE_TGZ=/tmp/actagent-current.tgz" "$TMPDIR/package-args"
grep -qx -- "ACTAGENT_E2E_NPM_INSTALL_TIMEOUT=42s" "$TMPDIR/package-args"
grep -qx -- "ACTAGENT_E2E_COMMAND_TIMEOUT=23s" "$TMPDIR/package-args"
`;
  ...
  it("passes plugin lifecycle sampler timeout overrides into Docker", () => {
    ...
    expect(runner).toContain('if [ -n "${ACTAGENT_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS:-}" ]; then');
  ...
  it("cleans plugin lifecycle matrix temp roots on exit", () => {
    ...
    expect(sweep).toContain("actagent_plugins_cleanup_fixture_servers");
    expect(sweep).toContain(
      'resource_dir="$(mktemp -d "/tmp/actagent-plugin-lifecycle-matrix.XXXXXX")"',
    );
    expect(sweep).toContain('tarball_v1="$resource_dir/lifecycle-actagent-1.0.0.tgz"');
    expect(sweep).toContain('tarball_v2="$resource_dir/lifecycle-actagent-2.0.0.tgz"');
    ...
    expect(sweep).not.toContain('resource_dir="/tmp/actagent-plugin-lifecycle-matrix"');
    expect(sweep).not.toContain("/tmp/lifecycle-actagent-1.0.0.tgz");
    expect(sweep).not.toContain("/tmp/lifecycle-actagent-2.0.0.tgz");
  ...
  it("wraps direct Docker E2E npm installs with the shared timeout helper", () => {
    ...
    expect(multiNode).toContain(
      'actagent_e2e_install_package "$ARTIFACTS/install-a.log" "ACTAgent package under node-A prefix" "$NPM_PREFIX_A"',
    );
    expect(updateChannel).toContain(
      'actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(updateChannel).toContain(
      'actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path"',
    );
    expect(doctorSwitch).toContain(
      'actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(doctorSwitch).toContain(
      'actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz"',
    );
    for (const script of [releaseUpgrade, upgradeSurvivor, pluginCorrupt]) {
      expect(script).toContain(
        'actagent_e2e_maybe_timeout "${ACTAGENT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g',
      );
    }
  });
  ...
  it("keeps upgrade survivor mutable state off the host-mounted artifact tree", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("actagent-upgrade-survivor-runtime");
      expect(script).toContain("ACTAGENT_UPGRADE_SURVIVOR_TMPDIR");
      expect(script).toContain("ACTAGENT_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR");
      expect(script).toContain(
      ...
      expect(script).not.toContain('export TMPDIR="$ACTAGENT_UPGRADE_SURVIVOR_ARTIFACT_ROOT/tmp"');
      expect(script).not.toContain('export npm_config_cache="$ARTIFACT_ROOT/npm-cache"');
      expect(script).not.toContain(
        'export npm_config_cache="$ACTAGENT_UPGRADE_SURVIVOR_ARTIFACT_ROOT/npm-cache"',
      );
    }
  });

  it("wraps package-backed scenario ACTAgent CLI calls with the shared timeout helper", () => {
    const paths = [
    ...
    for (const path of paths) {
      ...
      expect(script, path).toContain("actagent_e2e_enable_actagent_cli_timeout");
    }
    expect(readFileSync(RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH, "utf8")).toContain(
      'actagent_e2e_run_command node "$baseline_entry" onboard',
    );
  });
  ...
  it("keeps append-only mock E2E state under per-run scratch roots", () => {
    const scripts = [
      {
        ...
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/actagent-release-typed-onboarding.XXXXXX")"',
        ...
        removed: [
          "/tmp/actagent-release-typed-onboarding-openai.jsonl",
          "/tmp/actagent-release-typed-onboarding-install.log",
          "/tmp/actagent-release-typed-onboarding.log",
          "/tmp/actagent-release-typed-onboarding-openai.log",
          "/tmp/actagent-release-typed-onboarding-agent.log",
          'mktemp -d "/tmp/actagent-release-typed-onboarding.XXXXXX"',
        ],
      },
      {
        path: RELEASE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/actagent-release-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        ...
        removed: [
          "/tmp/actagent-release-user-journey-openai.jsonl",
          "/tmp/actagent-release-user-journey-clickclack.json",
          "/tmp/actagent-release-user-journey-install.log",
          "/tmp/actagent-release-user-journey-onboard.log",
          "/tmp/actagent-release-user-journey-agent.log",
          "/tmp/actagent-release-user-journey-plugin-a-install-path.txt",
          "/tmp/actagent-release-user-journey-plugin-a-source-path.txt",
          'mktemp -d "/tmp/actagent-release-journey-plugin-a.XXXXXX"',
          'mktemp -d "/tmp/actagent-release-journey-plugin-b.XXXXXX"',
        ],
      },
      {
        path: RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/actagent-release-upgrade-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        extraState: 'CLICKCLACK_STATE="$scenario_tmp/clickclack.json"',
        expectedPaths: [
        ...
        removed: [
          "/tmp/actagent-release-upgrade-user-journey-openai.jsonl",
          "/tmp/actagent-release-upgrade-user-journey-clickclack.json",
          "/tmp/actagent-release-upgrade-baseline-install.log",
          "/tmp/actagent-release-upgrade-candidate-install.log",
          "/tmp/actagent-release-upgrade-onboard.log",
          "/tmp/actagent-release-upgrade-agent.log",
          'mktemp -d "/tmp/actagent-release-upgrade-plugin.XXXXXX"',
        ],
      },
      {
        ...
        removed: ["/tmp/actagent-mock-openai-requests.jsonl"],
      },
    ];
    ...
    for (const {
      ...
      expect(script, path).not.toMatch(/\/tmp\/actagent-release-[\w-]+\.(?:log|json|err|txt)/u);
    }
  });
  ...
  it("keeps multi-node update Docker artifacts isolated by default", () => {
    ...
    expect(multiNode).toContain(
      'RUN_ID="${ACTAGENT_MULTI_NODE_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(multiNode).toContain(
      'ARTIFACT_DIR="${ACTAGENT_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update/$RUN_ID}"',
    );
    expect(multiNode).toContain('-v "$ARTIFACT_DIR:/tmp/artifacts"');
    expect(multiNode).not.toContain(
      'ARTIFACT_DIR="${ACTAGENT_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"',
    );
  });

  it("bounds upgrade survivor foreground ACTAgent CLI calls", () => {
    const runner = readFileSync(UPGRADE_SURVIVOR_DOCKER_E2E_PATH, "utf8");
    const publishedRunner = readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8");
    ...
    expect(runner).toContain('-e ACTAGENT_UPGRADE_SURVIVOR_COMMAND_TIMEOUT="$COMMAND_TIMEOUT"');
    expect(runner).toContain(
      'command_timeout="${ACTAGENT_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" env -u ACTAGENT_GATEWAY_TOKEN',
    );
    expect(runner).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" actagent doctor --fix --non-interactive',
    );
    expect(runner).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" actagent config validate',
    );
    expect(runner).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" actagent gateway status',
    );
    expect(runner).toContain(
      'actagent gateway --port "$PORT" --bind loopback --allow-unconfigured',
    );
    ...
    expect(publishedRunner).toContain(
      'actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent --version',
    );
    expect(publishedRunner).toContain(
      'actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent config validate >"$BASELINE_CONFIG_VALIDATE_LOG"',
    );
    expect(publishedRunner).toContain(
      'actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" actagent',
    );
    expect(publishedRunner).toContain(
      'actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" actagent',
    );
    expect(publishedRunner).toContain(
      'actagent_e2e_maybe_timeout "$COMMAND_TIMEOUT" actagent doctor --fix --non-interactive',
    );
    ...
    expect(publishedRunner).toContain('actagent gateway --port "$port" --bind loopback');
    ...
    expect(updateRestartAuth).toContain('actagent gateway --port "$port" --bind loopback');
  });

  it("keeps the harness run wrapper available with pre-sourced Docker command helpers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-package-helper-guard-"));
...
`;
  ...
  it("prints plugins Docker E2E logs on successful runs", () => {
    ...
    expect(helper).toContain("ACTAGENT_DOCKER_E2E_LOG_HEARTBEAT_SECONDS");
  ...
  it("prints heartbeat progress for long successful Docker E2E log captures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-e2e-log-heartbeat-"));
...
`;
  ...
  it("does not delay fast successful Docker E2E log captures until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-e2e-log-fast-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E log heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "actagent-docker-e2e-log-zero-heartbeat-"));
...
`;
  ...
  it("keeps onboarding Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("ACTAGENT_ONBOARD_MAX_MEMORY_MIB");
    expect(runner).toContain("ACTAGENT_ONBOARD_MAX_CPU_PERCENT");
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${ACTAGENT_ONBOARD_COMMAND_TIMEOUT:-${ACTAGENT_E2E_COMMAND_TIMEOUT:-300s}}"',
    );
    expect(runner).toContain('-e "ACTAGENT_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
  ...
  it("cleans resource-sampled Docker E2E temp logs on every exit path", () => {
    for (const { path, label } of [
      ...
      expect(runner, path).toContain('DOCKER_RUN_TIMEOUT="${ACTAGENT_');
  ...
  it("bounds kitchen-sink plugin CLI commands inside the Docker sweep", () => {
    ...
    expect(sweep).toContain("run_kitchen_sink_actagent_logged()");
    expect(sweep).toContain("run_kitchen_sink_actagent_capture()");
    expect(sweep).toContain(
      'run_logged_print "$label" actagent_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" "$@"',
    );
    for (const line of sweep.split("\n")) {
      if (!line.includes('node "$ACTAGENT_ENTRY" plugins')) {
        continue;
      }

      expect(line).toContain("actagent_e2e_maybe_timeout");
    }
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("ACTAGENT_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(scenarios).toContain(
      '"ACTAGENT_INSTALL_TAG=beta ACTAGENT_E2E_MODELS=openai ACTAGENT_INSTALL_E2E_IMAGE=actagent-install-e2e-openai:local ACTAGENT_INSTALL_E2E_AGENT_TOOL_SMOKE=0 ACTAGENT_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini ACTAGENT_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 ACTAGENT_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 pnpm test:install:e2e"',
    );
    expect(scenarios).toContain(
      '"ACTAGENT_INSTALL_TAG=beta ACTAGENT_E2E_MODELS=anthropic ACTAGENT_INSTALL_E2E_IMAGE=actagent-install-e2e-anthropic:local pnpm test:install:e2e"',
    );
    expect(scenarios).toContain(
      '"ACTAGENT_OPENWEBUI_MODEL=openai/gpt-5.4-mini OPENWEBUI_SMOKE_MODE=models ACTAGENT_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 ACTAGENT_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui"',
    );
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${ACTAGENT_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="${ACTAGENT_INSTALL_E2E_AGENT_TURNS_PARALLEL:-1}"',
    );
    ...
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_AGENT_TURNS_PARALLEL");
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_AGENT_TOOL_SMOKE");
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_OPENAI_MODEL");
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300");
    expect(wrapper).toContain("ACTAGENT_INSTALL_E2E_PROFILE_FILE");
    expect(wrapper).toContain("ACTAGENT_PROFILE_FILE");
    expect(wrapper).toContain("ACTAGENT_TESTBOX_PROFILE_FILE");
    ...
    expect(runner).toContain("ACTAGENT_INSTALL_E2E_OPENAI_MODEL");
    expect(runner).toContain("ACTAGENT_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="${ACTAGENT_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300}"',
    );
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("ACTAGENT_PLUGINS_E2E_ACTAGENTHUB=0");
  });
  ...
  it("fails the multi-node update probe on update or restart regressions", () => {
    ...
    expect(runner).toContain("ACTAGENT_NO_RESPAWN=1");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    const pluginsActagenthub = readFileSync(PLUGINS_DOCKER_ACTAGENTHUB_PATH, "utf8");
    ...
    expect(scripts.join("\n")).toContain("ACTAGENT_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("routes doctor install switch commands through the E2E timeout helper", () => {
    ...
    expect(scenario).toContain(
      'command_timeout="${ACTAGENT_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
    );
    expect(scenario).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd"',
    );
    expect(scenario).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd"',
    );
    expect(scenario).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force',
    );
    expect(scenario).toContain(
      'actagent_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes',
    );
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = mkdtempSync(join(tmpdir(), "actagent-update-channel-fixture-"));
    try {
      mkdirSync(join(root, "patches"));
      writeFileSync(
        join(root, "package.json"),
        `${JSON.stringify({ name: "actagent", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("ACTAGENT_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("ACTAGENT_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("ACTAGENT_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT");
    expect(runner).toContain("ACTAGENT_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("ACTAGENT_PLUGIN_LIFECYCLE_TRACE");
    ...
    expect(probe).toContain('"actagent.plugin.json"');
    expect(runtimeSmoke).toContain("process.env.ACTAGENT_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runtimeSmoke).toContain("900000");
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$ACTAGENT_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$ACTAGENT_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ACTAGENT_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ACTAGENT_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('ACTAGENT_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'ACTAGENT_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("cleans OpenAI web search smoke processes through the E2E helpers", () => {
    ...
    expect(scenario).toContain('actagent_e2e_terminate_gateways "${gateway_pid:-}"');
    expect(scenario).toContain('actagent_e2e_stop_process "${mock_pid:-}"');
    expect(scenario).toContain(
      'gateway_pid="$(actagent_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"',
    );
    expect(scenario).toContain('actagent_e2e_wait_mock_openai "$MOCK_PORT"');
    expect(scenario).toContain('actagent_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360');
  ...
  it("keeps OpenAI web search smoke logs isolated per run", () => {
    ...
    expect(scenario).toContain(
      'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/actagent-openai-web-search-minimal.XXXXXX")"',
    );
    ...
    expect(scenario).not.toContain("/tmp/actagent-openai-web-search-minimal-requests.jsonl");
    expect(scenario).not.toContain("/tmp/actagent-openai-web-search-minimal-client-success.log");
    expect(scenario).not.toContain("/tmp/actagent-openai-web-search-minimal-client-reject.log");
  });

  it("keeps ACTAgentHub plugin Docker smoke hermetic by default", () => {
    const runner = readFileSync(PLUGINS_DOCKER_E2E_PATH, "utf8");
    const sweep = readFileSync(PLUGINS_DOCKER_SWEEP_PATH, "utf8");
    const actagenthub = readFileSync(PLUGINS_DOCKER_ACTAGENTHUB_PATH, "utf8");

    expect(runner).toContain("scripts/e2e/lib/plugins/sweep.sh");
    expect(runner).toContain("ACTAGENT_PLUGINS_E2E_LIVE_ACTAGENTHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/actagenthub.sh");
    expect(actagenthub).toContain("start_actagenthub_fixture_server()");
    expect(actagenthub).toContain('ACTAGENT_ACTAGENTHUB_URL="http://127.0.0.1:');
    expect(actagenthub).toContain("ACTAGENT_PLUGINS_E2E_LIVE_ACTAGENTHUB");
    expect(actagenthub).toContain("ACTAGENT_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(actagenthub).toContain("live ACTAgentHub can rate-limit CI");
    expect(actagenthub).toContain('[[ -n "${ACTAGENT_ACTAGENTHUB_URL:-}" || -n "${ACTAGENTHUB_URL:-}" ]]');
    expect(actagenthub).toContain("Ignoring ambient ACTAgentHub URL for fixture-mode plugin E2E");
    expect(actagenthub).toContain("unset ACTAGENT_ACTAGENTHUB_URL ACTAGENTHUB_URL");
  });
  ...
  it("keeps the plugin binding command escape Docker smoke focused", () => {
    ...
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${ACTAGENT_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
    ...
    expect(dockerfile).toContain("ACTAGENT_DISABLE_BUNDLED_PLUGIN_POSTINSTALL=1");
    expect(dockerfile).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter actagent",
    );
  });
  ...
  it("covers plugin CLI sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('ACTAGENT_PLUGINS_CLI_TIMEOUT="${ACTAGENT_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(sweep).toContain(
      'run_logged "$label" actagent_e2e_maybe_timeout "$ACTAGENT_PLUGINS_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" "$@"',
    );
    expect(sweep).toContain("run_plugins_actagent_capture()");
    expect(sweep).toContain(
      'actagent_e2e_maybe_timeout "$ACTAGENT_PLUGINS_CLI_TIMEOUT" node "$ACTAGENT_ENTRY" "$@" >"$output_file"',
    );
    expect(sweep).not.toContain('run_logged install-npm node "$ACTAGENT_ENTRY"');
    for (const [path, script] of [
      [PLUGINS_DOCKER_SWEEP_PATH, sweep],
      [PLUGINS_DOCKER_MARKETPLACE_PATH, marketplace],
      [PLUGINS_DOCKER_ACTAGENTHUB_PATH, actagenthub],
    ] as const) {
      const unboundedPluginCliLines = script
        .split("\n")
        .filter((line) => line.includes('node "$ACTAGENT_ENTRY" plugins'))
        .filter((line) => !line.includes("actagent_e2e_maybe_timeout"));
    ...
    expect(sweep).toContain('plugins install "npm:@actagent/demo-plugin-npm@0.0.1"');
    ...
    expect(actagenthub).toContain('plugins install "$ACTAGENTHUB_PLUGIN_SPEC"');
    expect(actagenthub).toContain('plugins update "$ACTAGENTHUB_PLUGIN_ID"');
    expect(actagenthub).toContain("run_plugins_actagent_logged install-actagenthub");
    expect(actagenthub).toContain('actagent_e2e_maybe_timeout "$ACTAGENT_PLUGINS_CLI_TIMEOUT"');
    expect(actagenthub).toContain("actagenthub:@actagent/kitchen-sink");
    expect(assertions).toContain("actagenthub-updated");
    expect(assertions).toContain("record.actagentpackSha256");

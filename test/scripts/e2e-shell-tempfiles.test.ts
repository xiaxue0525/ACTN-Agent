...
describe("e2e shell tempfile hygiene", () => {
  ...
  it("preserves wizard exit status when reporting failures", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-onboard-status-test-"));
...
export ACTAGENT_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ACTAGENT_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
ACTAGENT_ENTRY=node
actagent_test_state_create() { :; }
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('ACTAGENT_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$ACTAGENT_E2E_LAST_LOG_PATH"');
    expect(contents).not.toContain(
      "validate_local_basic_log /tmp/actagent-onboard-local-basic.log",
    );
    expect(contents).toContain(
  ...
  it("probes onboarding gateway readiness through the isolated scratch log", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-onboard-gateway-log-"));
...
actagent_e2e_probe_tcp() { return 1; }
sleep 30 &
GATEWAY_PID="$!"
printf 'listening on ws://127.0.0.1:19199\\n' >"$GATEWAY_LOG_PATH"
wait_for_gateway
...
`,
  ...
  it("removes fallback ACTAgentHub skill install HOME on failure", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "actagent-actagenthub-home-test-"));
...
`,
    ...
    try {
      const result = spawnSync("bash", ["scripts/e2e/lib/skills/actagenthub-install-proof.sh"], {
        cwd: process.cwd(),
        encoding: "utf8",
        env: {
          ...process.env,
          ACTAGENT_CURRENT_PACKAGE_TGZ: "",
      ...
      expect(
        scratchEntries.filter((entry) => entry.startsWith("actagent-skill-install-home.")),
      ).toEqual([]);
    } finally {
      await rm(tempRoot, { force: true, recursive: true });
    }

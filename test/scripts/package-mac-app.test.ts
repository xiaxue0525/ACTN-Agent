...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "actagent-plistbuddy-"));
  tempDirs.push(dir);
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "actagent-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "actagent-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      ACTAGENT_TEST_LOG=${JSON.stringify(logPath)}
      export ACTAGENT_TEST_LOG
  ...
  it("does not kill unrelated ACTAgent processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q ACTAgent");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/ACTAgent"');
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const actAgentKitBlock = script.slice(
      script.indexOf(
    ...
    expect(actAgentKitBlock).toContain("ERROR: ACTAgentKit resource bundle not found");
    expect(actAgentKitBlock).toContain("exit 1");
    expect(actAgentKitBlock).not.toContain("WARN:");
    expect(actAgentKitBlock).not.toContain("continuing");
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.actagent.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.actagent.test");

...
describe("scripts/test-extension.mjs", () => {
  ...
  it("can fail safe to all extensions when the base revision is unavailable", () => {
    const extensionIds = listChangedExtensionIds({
      base: "refs/heads/actagent-test-missing-base",
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(
      ...
      {
        env: { ACTAGENT_EXTENSION_BATCH_PARALLEL: "2" },
        runGroup,
    ...
    expect(firstRunGroupParams).toEqual({
      ...
      config: "heavy",
      env: {
        ACTAGENT_EXTENSION_BATCH_PARALLEL: "2",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { ACTAGENT_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { ACTAGENT_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
  ...
  it("rejects malformed extension batch parallelism", () => {
    for (const value of ["nope", "2x", "0"]) {
      expect(() =>
        resolveExtensionBatchParallelism(3, { ACTAGENT_EXTENSION_BATCH_PARALLEL: value }),
      ).toThrow("ACTAGENT_EXTENSION_BATCH_PARALLEL must be a positive integer");
    }
  });
  ...
  posixIt(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "actagent-test-extension-signal-"));

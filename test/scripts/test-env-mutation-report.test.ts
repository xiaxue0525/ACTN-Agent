...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("actagent-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.ACTAGENT_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.ACTAGENT_STATE_DIR = "state";
delete process.env["ACTAGENT_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("ACTAGENT_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, ACTAGENT_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "ACTAGENT_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.ACTAGENT_HOME = "not code"';
process.env.NOT_ACTAGENT = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ACTAGENT_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ACTAGENT_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/actagent-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active ACTAgent env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        key: "ACTAGENT_CONFIG_PATH",
        operation: "assign",
      },
      {
        file: "src/agents/auth-profiles/oauth-test-utils.ts",
        key: "ACTAGENT_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "ACTAGENT_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "ACTAGENT_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ACTAGENT_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ACTAGENT_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("ACTAgent test env mutation report");
    ...
    expect(rendered).toContain("L2 ACTAGENT_STATE_DIR assign process.env");

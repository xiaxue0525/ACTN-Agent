...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectSteps[1].env?.ACTAGENT_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubSteps[3].env?.ACTAGENT_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubSteps[3].env?.ACTAGENT_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalSteps[1].env?.ACTAGENT_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.ACTAGENT_SECURITY_TEAM_SLUG).toBe("actagent-secops");
    expect(runStep.env?.ACTAGENT_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("actagent-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @actagent/actagent-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @actagent/actagent-secops",
    );
    expect(codeowners).toContain("/scripts/github/dependency-guard.mjs @actagent/actagent-secops");
    expect(codeowners).toContain("/package-lock.json @actagent/actagent-secops");
    expect(codeowners).toContain("/npm-shrinkwrap.json @actagent/actagent-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @actagent/actagent-secops");
    expect(codeowners).toContain("/extensions/*/npm-shrinkwrap.json @actagent/actagent-secops");
  });
});

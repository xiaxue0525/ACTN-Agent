...
function runAssertionCommand(command: string, root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, command], {
    ...
    env: {
      ...
      ACTAGENT_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH: path.join(root, "agent.err"),
      ACTAGENT_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH: path.join(root, "agent.json"),
      ACTAGENT_STATE_DIR: path.join(root, "state"),
      PLUGIN_ID: "e2e-live-plugin-tool",
      PLUGIN_NAME: "@actagent/e2e-live-plugin-tool",
...
describe("live plugin tool assertions", () => {
  it("rejects loose timeout env values instead of parsing numeric prefixes", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-live-plugin-tool-"));
    try {
      const result = runAssertionCommand("configure", root, {
        ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "1e3",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: 1e3");
    } finally {
  ...
  it("writes strict positive timeout values into generated config", () => {
    ...
    try {
      const result = runAssertionCommand("configure", root, {
        ACTAGENT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "240",
      });

      expect(result.status).toBe(0);
      const config = JSON.parse(readFileSync(path.join(root, "state", "actagent.json"), "utf8"));

...
const LIVE_E2E_WORKFLOW = ".github/workflows/actagent-live-and-e2e-checks-reusable.yml";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/actagent-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/actagent-release-publish.yml";
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/actagent-scheduled-live-checks.yml";
...
describe("package acceptance workflow", () => {
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydrateWindowsPnpm.run).toContain('"actagent",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("ACTAGENT_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("ACTAGENT_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-actagent-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/actagent-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      "node scripts/check-actagent-package-tarball.mjs .artifacts/docker-e2e-package/actagent-current.tgz",
    );
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: actagent@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@actagent/codex@${BASH_REMATCH[1]}"',
    );
    expect(releaseChecksWorkflow).toContain(
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'actagent@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("ACTAGENT_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "ACTAGENT_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run ACTAgent Docker E2E package");
    expect(workflow).toContain("Download previous-run ACTAgent Docker E2E package");
    ...
    expect(workflow).toContain("ACTAGENT_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      "published_upgrade_survivor_baseline=${shellQuote(process.env.ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC)}",
    );
    expect(scheduler).toContain(
      "published_upgrade_survivor_baselines=${shellQuote(process.env.ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS)}",
    );
    expect(scheduler).toContain(
      '["ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["ACTAGENT_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["ACTAGENT_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("ACTAGENT_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("ACTAGENT_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("ACTAGENT_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u ACTAGENT_GATEWAY_TOKEN -u ACTAGENT_GATEWAY_PASSWORD actagent",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("actagent@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-actagent"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("ACTAGENT_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("ACTAGENT_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${ACTAGENT_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${ACTAGENT_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain(
    ...
    expect(dockerE2ePlanAction).not.toContain('docker pull "${ACTAGENT_DOCKER_E2E_');
  });
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("ACTAGENT_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("ACTAGENT_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("ACTAGENT_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("ACTAGENT_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("ACTAGENT_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'ACTAGENT_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: ACTAGENT_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_MODELS=anthropic/claude-opus-4-8");
    ...
    expect(workflow).toContain(
      "command: ACTAGENT_LIVE_GATEWAY_THINKING=off ACTAGENT_LIVE_GATEWAY_PROVIDERS=openai ACTAGENT_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "ACTAGENT_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "ACTAGENT_LIVE_GATEWAY_MODELS=minimax/MiniMax-M3,minimax-portal/MiniMax-M3 ACTAGENT_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "ACTAGENT_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "ACTAGENT_LIVE_GATEWAY_THINKING=off ACTAGENT_LIVE_GATEWAY_PROVIDERS=openai ACTAGENT_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ACTAGENT_LIVE_GATEWAY_MAX_MODELS=1 ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "ACTAGENT_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 ACTAGENT_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("ACTAGENT_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?ACTAGENT_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?ACTAGENT_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("image: ghcr.io/actagent/actagent-live-media-runner:ubuntu-24.04");
    ...
    expect(workflow).toContain("ACTAGENT_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("ACTAGENT_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("ACTAGENT_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("ACTAGENT_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: ACTAGENT_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: ACTAGENT_LIVE_GATEWAY_THINKING=off ACTAGENT_LIVE_GATEWAY_PROVIDERS=openai ACTAGENT_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ACTAGENT_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      'command: ACTAGENT_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: ACTAGENT_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: ACTAGENT_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    expect(workflow).toContain(
      'command: ACTAGENT_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.ACTAGENT_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${ACTAGENT_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("actagent_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("actagent_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("actagent_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("actagent_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('actagent_live_truthy "${ACTAGENT_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${ACTAGENT_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${ACTAGENT_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("actagent_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("actagent_live_prepare_bind_dir_for_container_user");
    ...
    for (const script of sharedLiveScripts) {
      expect(script).toContain("actagent_live_uses_managed_bind_dirs");
    ...
    for (const script of [
      ...
      expect(script).not.toContain('timeout --kill-after=30s "${ACTAGENT_LIVE_');
    }
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'timeout_value="${ACTAGENT_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'timeout_value="${ACTAGENT_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'timeout_value="${ACTAGENT_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "ACTAGENT_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${ACTAGENT_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${ACTAGENT_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    ...
    expect(stage).toContain(
      'local scripts_dir="${ACTAGENT_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("actagent_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("actagent_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    expectTextToIncludeAll(dockerPlanAction, [
      ...
      "require_any Codex ACTAGENT_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",anthropic,"* ]]; then',
      "require_any Anthropic ANTHROPIC_API_TOKEN ANTHROPIC_API_KEY ACTAGENT_CLAUDE_CREDENTIALS_JSON ACTAGENT_CLAUDE_JSON",
      'if [[ "$credentials" == *",factory,"* ]]; then',
      "require_any Factory FACTORY_API_KEY",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY ACTAGENT_GEMINI_SETTINGS_JSON",
  ...
  it("fails Testbox changed-check delegation when the remote command fails", () => {
    ...
    expect(workflow).toContain('PNPM_CONFIG_STORE_DIR: "/tmp/actagent-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("ACTAGENT_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "ACTAGENT_QA_CONVEX_SITE_URL: ${{ secrets.ACTAGENT_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "ACTAGENT_QA_CONVEX_SECRET_CI: ${{ secrets.ACTAGENT_QA_CONVEX_SECRET_CI }}",
    );
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm actagent qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm actagent qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.ACTAGENT_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      expect(releaseWorkflow).toContain(`qa_live_${lower}_enabled="$qa_live_${lower}_ci_enabled"`);
      expect(releaseWorkflow).toContain(
        `vars.ACTAGENT_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`ACTAGENT_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-actagent-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-actagent@beta}"',
    ...
    expectTextToIncludeAll(workflow, [
      ...
      "cancel-in-progress: ${{ (inputs.ref == 'main' && inputs.rerun_group == 'all') || startsWith(inputs.ref, 'tideactagent/alpha/') }}",
      "Verify release checks accepted tideactagent alpha advisory lanes",
      "release_checks_advisory_only",
      "release_check_blocking_job",
      "is a package-safety tideactagent alpha release-check lane",
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(runStep.run, [
      'export ACTAGENT_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("lets CI Telegram consumers wait on Convex leases instead of GitHub concurrency", () => {
    ...
    for (const [workflowPath, jobName, stepName] of telegramJobs) {
      ...
      expect(step.env?.ACTAGENT_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("1800000");
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "ACTAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "ACTAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/actagent-npm-release.yml", "utf8");
    const fullReleaseWorkflow = readFileSync(FULL_RELEASE_VALIDATION_WORKFLOW, "utf8");

    expect(workflow).toContain("timeout-minutes: 60");
    expect(workflow).toContain("environment: npm-release");
    expect(workflow).toContain("Download ACTAgent npm preflight manifest");
    expect(workflow).toContain("Validate ACTAgent npm preflight manifest");
    ...
    expect(workflow).toContain(
      "publish_actagent_npm=true requires plugin_publish_scope=all-publishable",
    );
    ...
    expect(fullReleaseWorkflow).not.toContain("node /app/actagent.mjs agent");
    expect(fullReleaseWorkflow).toContain('ACTAGENT_EXTENSIONS="diagnostics-otel,codex"');
  ...
  it("keeps beta release verification and ACTAgentHub publish repair hooks wired", () => {
    const packageJson = JSON.parse(readFileSync("package.json", "utf8")) as {
      scripts?: Record<string, string>;
    };
    const releaseWorkflow = readFileSync(RELEASE_PUBLISH_WORKFLOW, "utf8");
    const actAgentHubWorkflow = readFileSync(".github/workflows/plugin-actagenthub-release.yml", "utf8");
    const pluginNpmWorkflow = readFileSync(".github/workflows/plugin-npm-release.yml", "utf8");
    const actagentNpmWorkflow = readFileSync(".github/workflows/actagent-npm-release.yml", "utf8");
    ...
    expect(actAgentHubWorkflow).toContain("Verify published ACTAgentHub package");
    expect(actAgentHubWorkflow).toContain("bun install failed while preparing ACTAgentHub CLI; retrying");
    expect(actAgentHubWorkflow).toContain("max-parallel: 32");
    expect(releaseWorkflow).toContain("Plugin npm run ID");
    expect(releaseWorkflow).toContain("Plugin ACTAgentHub run ID");
    expect(releaseWorkflow).toContain("ACTAgent npm run ID");
    ...
    expect(releaseWorkflow).toContain("actagent-release-postpublish-evidence");
    expect(releaseWorkflow).toContain("Failed child job summary");
    expect(releaseWorkflow).toContain("Workflow completion waits for ACTAgentHub");
    expect(releaseWorkflow).toContain("Workflow completion does not wait for ACTAgentHub");
    expect(releaseWorkflow).toContain('[[ "${WAIT_FOR_ACTAGENTHUB}" == "true" ]]');
    expect(releaseWorkflow).toContain("--skip-actagenthub");
    expect(pluginNpmWorkflow).toContain("Validate release publish approval run");
    expect(actAgentHubWorkflow).toContain("Validate release publish approval run");
    expect(actagentNpmWorkflow).toContain("Validate release publish approval run");
    ...
    expect(actAgentHubWorkflow).toContain("Check ACTAgentHub package version");
    expect(actAgentHubWorkflow).toContain("already_published=true");
    expect(actAgentHubWorkflow).toContain(
      "steps.actagenthub_package_version.outputs.already_published != 'true'",
    );
    expect(pluginNpmWorkflow).toContain("Direct Plugin NPM Release dispatch");
    expect(actAgentHubWorkflow).toContain("Direct Plugin ACTAgentHub Release dispatch");
    expect(actagentNpmWorkflow).toContain("Direct ACTAgent npm publish");
    expect(pluginNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(actAgentHubWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(actagentNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(pluginNpmWorkflow).toContain("Direct Plugin NPM Release recovery");
    expect(actAgentHubWorkflow).toContain("Direct Plugin ACTAgentHub Release recovery");
    expect(actagentNpmWorkflow).toContain("Direct ACTAgent npm recovery");
    expect(pluginNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    expect(actAgentHubWorkflow).toContain("validate-release-publish-approval.mjs");
    expect(actagentNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    ...
    expect(actAgentHubWorkflow).toContain("environment: actagenthub-plugin-release");
    expect(actagentNpmWorkflow).toContain("environment: npm-release");
    ...
    expect(releaseWorkflow.lastIndexOf("create_or_update_github_release")).toBeLessThan(
      releaseWorkflow.indexOf('if [[ -n "${actagenthub_pid}" ]] && ! wait "${actagenthub_pid}"'),
    );
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/actagent-cross-os-release-checks-reusable.yml");
    const liveE2e = readWorkflow(LIVE_E2E_WORKFLOW);
    const releaseWorkflowPaths = [
      FULL_RELEASE_VALIDATION_WORKFLOW,
      RELEASE_CHECKS_WORKFLOW,
      ".github/workflows/actagent-cross-os-release-checks-reusable.yml",
      LIVE_E2E_WORKFLOW,
      NPM_TELEGRAM_WORKFLOW,
      ".github/workflows/actagent-release-publish.yml",
      ".github/workflows/actagent-npm-release.yml",
  ...
  it("kills timed TUI PTY workflow runs after the grace period", () => {
    ...
    expect(job.env?.ACTAGENT_TUI_PTY_INCLUDE_LOCAL).toBe("1");

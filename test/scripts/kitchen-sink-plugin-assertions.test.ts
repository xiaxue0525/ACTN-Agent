...
function runAssertInstalled({
  ...
  const home = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-install-"));
  ...
  const installsPath = path.join(home, ".actagent", "plugins", "installs.json");
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          installPath,
          resolvedSpec: "@actagent/kitchen-sink@latest",
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "npm:@actagent/kitchen-sink@latest",
...
function runAssertActagenthubInstalled({
  contextEngineIds = [],
}: {
  contextEngineIds?: string[];
} = {}) {
  const label = `actagenthub-context-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const pluginId = "actagent-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-install-"));
  const scratchRoot = tmpdir();
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          ...
          actagenthubPackage: "@actagent/kitchen-sink",
          integrity: "sha256-test",
          installPath,
          resolvedSpec: "actagenthub:@actagent/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          resolvedAt: 1,
          source: "actagenthub",
          spec: "actagenthub:@actagent/kitchen-sink@latest",
          version: "1.0.0",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_ID: pluginId,
        KITCHEN_SINK_LABEL: label,
        KITCHEN_SINK_SOURCE: "actagenthub",
        KITCHEN_SINK_SPEC: "actagenthub:@actagent/kitchen-sink@latest",
...
describe("kitchen-sink plugin assertions", () => {
  ...
  it("requires ACTAgentHub kitchen-sink fixtures to expose context engines", () => {
    const result = runAssertActagenthubInstalled({ contextEngineIds: [] });

    expect(result.status).not.toBe(0);
    expect(`${result.stdout}\n${result.stderr}`).toContain("context engines missing");
  });

  it("accepts ACTAgentHub kitchen-sink fixtures with a context engine", () => {
    const result = runAssertActagenthubInstalled({
      contextEngineIds: ["actagent-kitchen-sink-fixture"],
    });
  ...
  it("scans only the configured kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-scan-"));
  ...
  it("allocates an isolated scratch root by default", () => {
    ...
    expect(sweep).toContain('mktemp -d "/tmp/actagent-kitchen-sink.XXXXXX"');
    expect(sweep).toContain('mktemp -d "${KITCHEN_SINK_TMP_DIR}/actagenthub.XXXXXX"');
    expect(sweep).not.toContain('KITCHEN_SINK_TMP_DIR="${KITCHEN_SINK_TMP_DIR:-/tmp}"');
    expect(sweep).not.toContain('mktemp -d "/tmp/actagent-kitchen-sink-actagenthub.XXXXXX"');
  });

  it("cleans the default kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-cleanup-"));
...
`,
      ...
      expect(scratchRoot).toContain("/tmp/actagent-kitchen-sink.");
  ...
  it("cleans a ACTAgentHub fixture server that times out before readiness", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-sink-actagenthub-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "actagenthub-fixture");
...
export ACTAGENT_ACTAGENTHUB_FIXTURE_WAIT_ATTEMPTS=1
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_actagenthub_fixture_server "$FIXTURE_DIR"
status="$?"
set -e
if [[ "$status" -eq 0 ]]; then
...
server_pid="$(cat "$FIXTURE_DIR/actagenthub-fixture-pid")"
kill -0 "$server_pid"
cleanup_kitchen_sink_sweep
...
`,
      ...
      expect(result.status).toBe(0);
      expect(`${result.stdout}\n${result.stderr}`).toContain(
        "Timed out waiting for kitchen-sink ACTAgentHub fixture server.",
      );

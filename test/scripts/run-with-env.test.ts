...
describe("run-with-env", () => {
  it("parses leading env assignments before the command separator", () => {
    expect(
      parseRunWithEnvArgs([
        "ACTAGENT_GATEWAY_PROJECT_SHARDS=1",
        "EMPTY=",
        "--",
        "node",
  ...
  it("rejects missing command separators", () => {
    expect(() => parseRunWithEnvArgs(["ACTAGENT_GATEWAY_PROJECT_SHARDS=1", "node"])).toThrow(
      /Usage:/u,
    );
  });
  ...
  it("keeps command help passthrough after the separator", () => {
    expect(
      isRunWithEnvHelpRequest(["ACTAGENT_GATEWAY_PROJECT_SHARDS=1", "--", "node", "--help"]),
    ).toBe(false);
  });
  ...
  it.runIf(process.platform !== "win32").each(["SIGTERM", "SIGHUP", "SIGINT"] as const)(
    ...
    async (signal) => {
      const tempDir = mkdtempSync(path.join(tmpdir(), "actagent-run-with-env-signals-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(path.join(tmpdir(), "actagent-run-with-env-descendants-"));
      const readyFile = path.join(tempDir, "ready");
      ...
      const wrapper = spawn(
        ...
        {
          cwd: process.cwd(),
          env: { ...process.env, ACTAGENT_RUN_WITH_ENV_FORCE_KILL_MS: "200" },
          stdio: "ignore",
        },
      );
      let grandchildPid = 0;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(path.join(tmpdir(), "actagent-run-with-env-grace-"));
      const readyFile = path.join(tempDir, "ready");
      ...
      const wrapper = spawn(
        ...
        {
          cwd: process.cwd(),
          env: { ...process.env, ACTAGENT_RUN_WITH_ENV_FORCE_KILL_MS: "1000" },
          stdio: "ignore",
        },
      );
  ...
  it.runIf(process.platform !== "win32")("preserves wrapped command signal exits", () => {
    const result = spawnSync(
      ...
      [
        "scripts/run-with-env.mjs",
        "ACTAGENT_RUN_WITH_ENV_SIGNAL_TEST=1",
        "--",
        "node",
        "-e",

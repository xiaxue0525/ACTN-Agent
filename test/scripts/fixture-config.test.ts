...
function makeTempRoot(): string {
  return mkdtempSync(path.join(tmpdir(), "actagent-fixture-config-"));
}
...
function runFixture(
  ...
  return spawnSync(process.execPath, [fixturePath, command, ...args], {
    ...
    env: {
      ...process.env,
      ACTAGENT_CONFIG_BATCH_PATH: path.join(root, "batch.json"),
      ACTAGENT_CONFIG_PATH: path.join(root, "actagent.json"),
      ACTAGENT_GATEWAY_TOKEN: "test-token",
      ACTAGENT_OPENWEBUI_MODEL: "openai/gpt-5.4-mini",
...
describe("scripts/e2e/lib/fixture.mjs config commands", () => {
  it("rejects loose gateway port env values instead of parsing prefixes", () => {
    ...
    try {
      const result = runFixture(root, "config-reload", [], { PORT: "19199tcp" });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid PORT: 19199tcp");
    } finally {
  ...
  it("rejects loose Open WebUI provider timeout values", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        ACTAGENT_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300s",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid ACTAGENT_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: 300s");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("writes strict positive Open WebUI provider timeouts into generated config", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        ACTAGENT_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300",
      });

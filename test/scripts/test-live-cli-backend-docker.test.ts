...
describe("scripts/test-live-cli-backend-docker.sh", () => {
  ...
  it("forwards both fresh and resume CLI arg overrides into the Docker container", () => {
    ...
    expect(forwardedVars).toContain("ACTAGENT_LIVE_CLI_BACKEND_ARGS");
    expect(forwardedVars).toContain("ACTAGENT_LIVE_CLI_BACKEND_RESUME_ARGS");
    expect(forwardedVars).toContain("ACTAGENT_TEST_CONSOLE");
  });
});

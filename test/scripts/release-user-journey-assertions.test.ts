...
describe("release user journey assertions", () => {
  it("scans large files when checking release user journey output text", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-release-user-assertions-"));
  ...
  it("fails when uninstall leaves the managed plugin directory behind", () => {
    ...
    const installPath = path.join(home, ".actagent", "extensions", pluginId);
    const installPathFile = path.join(root, "install-path.txt");

    try {
      writeJson(path.join(home, ".actagent", "actagent.json"), {
        plugins: {
          entries: {},
          allow: [],
          deny: [],
  ...
  it("passes after uninstall clears config, records, and managed files", () => {
    ...
    try {
      ...
      writeFileSync(
        installPathFile,
        path.join(home, ".actagent", "extensions", "journey-plugin-a"),
        "utf8",
      );
  ...
  it("accepts ready ClickClack fixture state", async () => {
    ...
    try {
      await expect(
        withEnv({ HOME: home, ACTAGENT_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000" }, () =>
  ...
  it("bounds stalled ClickClack fixture HTTP probes", async () => {
    ...
    try {
      ...
      await expect(
        withEnv({ HOME: home, ACTAGENT_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100" }, () =>
          runReleaseUserJourneyAssertion("wait-clickclack-socket", [
            `http://127.0.0.1:${server.port}`,
            "0.2",
          ]),
  ...
  it("rejects loose HTTP timeout env values instead of parsing prefixes", async () => {
    ...
    try {
      await expect(
        withEnv({ HOME: home, ACTAGENT_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100ms" }, () =>
          runReleaseUserJourneyAssertion("wait-clickclack-socket", [
            `http://127.0.0.1:${server.port}`,
            "0.2",
          ]),
    ...
    try {
      await expect(
        withEnv(
          {
            HOME: home,
            ACTAGENT_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16",
            ACTAGENT_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
    ...
    try {
      await expect(
        withEnv(
          {
            HOME: home,
            ACTAGENT_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16bytes",
            ACTAGENT_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
            runReleaseUserJourneyAssertion("post-clickclack-inbound", [

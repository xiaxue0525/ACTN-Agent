...
describe("verify-docker-attestations", () => {
  it("resolves digest refs from tagged image refs", () => {
    expect(imageRefForDigest("ghcr.io/actagent/actagent:2026.4.26", imageDigest)).toBe(
      `ghcr.io/actagent/actagent@${imageDigest}`,
    );
    expect(imageRefForDigest("localhost:5000/actagent:main", imageDigest)).toBe(
      `localhost:5000/actagent@${imageDigest}`,
    );
  });

  it("accepts an image index with SBOM and provenance predicates", () => {
    const errors = collectDockerAttestationErrors({
      imageRef: "ghcr.io/actagent/actagent:test",
      index: createIndex(),
  ...
  it("reports unexpected attestation artifact types", () => {
    ...
    expect(errors).toEqual([
      `ghcr.io/actagent/actagent:test: linux/amd64 attestation ${attestationDigest} has unexpected artifactType "application/vnd.unknown"`,
    ]);
  });
  ...
  it("reports missing attestation manifests", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/actagent/actagent:test: missing attestation manifest for linux/amd64",
    ]);
  });
  ...
  it("reports missing SBOM or provenance predicates", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/actagent/actagent:test: linux/amd64 missing predicate https://slsa.dev/provenance/v1",
    ]);
  });
});

...
describe("dependency guard script", () => {
  ...
  it("recognizes trusted dependency guard actors automatically", async () => {
    ...
    const untrustedAuthorCandidate = dependencyGuardTrustedActorCandidates({
      pullRequest: { user: { login: "external" } },
    ...
    expect(untrustedAuthorCandidate).toEqual([
      { login: "external", source: "pull request author" },
    ]);
    ...
    await expect(
      findTrustedDependencyGuardActor({
        ...
        isDependencyApprover: async (login) =>
          login === "security-user" || login === "repo-admin" ? "actagent-secops" : null,
      }),
    ).resolves.toBeNull();
    await expect(
      findTrustedDependencyGuardActor({
  ...
  it("renders trusted dependency graph comments without blocker language", () => {
    ...
    expect(body).toContain("<!-- actagent:dependency-graph-guard -->");
  ...
  it("trusts only configured dependency guard marker comment authors", () => {
    const trustedAuthors = dependencyGuardCommentAuthors(
      "github-actions[bot], actagent-autoscrub[bot]",
    );

    expect(
      isDependencyGuardMarkerComment(
        {
          body: "<!-- actagent:dependency-graph-guard -->",
          user: { login: "actagent-autoscrub[bot]" },
        },
        "<!-- actagent:dependency-graph-guard -->",
        trustedAuthors,
      ),
    ).toBe(true);
    expect(
  ...
  it("attempts autoscrub on PR branches maintainers can modify", () => {
    const sameRepoPullRequest = {
      head: {
        ref: "external/change",
        repo: { full_name: "actagent/actagent" },
        sha: headSha,
      },
    };
    const forkPullRequest = {
      head: {
        ref: "external/change",
        repo: { full_name: "external/actagent" },
        sha: headSha,
      },
    };
  ...
  it("reads base lockfiles with the base API before writing autoscrub commits", async () => {
    ...
    const commit = await createAutoscrubCommit(
      { baseApi, writeApi },
      {
        ...
        pullRequest: {
          base: { sha: "base-sha" },
          head: { ref: "external/change", sha: headSha },
        },
        lockfileChanges: ["pnpm-lock.yaml"],
        targetRepository: { owner: "external", repo: "actagent" },
      },
    );

    expect(commit).toEqual({ sha: staleSha });
    expect(calls.map((call) => `${call.api}:${call.path}`)).toEqual([
      "base:/repos/actagent/actagent/contents/pnpm-lock.yaml?ref=base-sha",
      "write:graphql",
    ]);
    expect(calls[1].variables).toMatchObject({
      input: {
        branch: {
          repositoryNameWithOwner: "external/actagent",
          branchName: "external/change",
        },
  ...
  it("preserves GitHub status when an error body exceeds the cap", async () => {
    ...
    try {
      await expect(githubApi("token").request("/repos/actagent/actagent")).rejects.toMatchObject({
  ...
  it("aborts stalled GitHub API fetches at the request timeout", async () => {
    ...
    }).request("/repos/actagent/actagent");
    const rejection = expect(request).rejects.toThrow(
      /GitHub API GET \/repos\/actagent\/actagent exceeded timeout 5ms/u,
    );

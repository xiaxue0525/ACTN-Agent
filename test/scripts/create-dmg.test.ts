...
function makeApp(plistEntries: string[]): string {
  const dir = mkdtempSync(path.join(tmpdir(), "actagent-create-dmg-"));
  tempDirs.push(dir);
  const app = path.join(dir, "ACTAgent.app");
...
describe("create-dmg plist validation", () => {
  ...
  it("keeps temporary DMG artifacts scoped to one run", () => {
    ...
    expect(script).toContain('DMG_TEMP="$(mktemp -d "${TMPDIR:-/tmp}/actagent-dmg.XXXXXX")"');
    ...
    expect(script).not.toContain("/tmp/actagent-dmg-limits.txt");
  });

  it.runIf(process.platform === "darwin")(
    "fails before hdiutil when required plist keys are missing",
    () => {
      const app = makeApp(["<key>CFBundleName</key>", "<string>ACTAgent</string>"]);

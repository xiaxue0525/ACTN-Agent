...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "actagent-test-live-signal-"));
    ...
    const runner = spawn(process.execPath, ["scripts/test-live.mjs", "--", "fake.live.test.ts"], {
      env: {
        ...process.env,
        ACTAGENT_FAKE_PNPM_PID_PATH: childPidPath,
        ACTAGENT_FAKE_PNPM_SIGNALED_PATH: signaledPath,
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid ACTAGENT_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
...
function writeFakePnpm(filePath: string): void {
  writeFileSync(
    ...
    [
      ...
      "fs.writeFileSync(process.env.ACTAGENT_FAKE_PNPM_PID_PATH, String(process.pid));",
      'process.on("SIGTERM", () => {',
      '  fs.writeFileSync(process.env.ACTAGENT_FAKE_PNPM_SIGNALED_PATH, "SIGTERM");',
      "  process.exit(0);",
      "});",
      "setInterval(() => {}, 1000);",
      "",

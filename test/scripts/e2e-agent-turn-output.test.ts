...
describe("scripts/e2e/lib/agent-turn-output", () => {
  it("extracts local and gateway agent reply payload text", () => {
    expect(
      extractAgentReplyTexts(
        JSON.stringify({
          payloads: [{ text: "ACTAGENT_E2E_OK_LOCAL" }],
          meta: { finalAssistantVisibleText: "visible" },
        }),
      ),
    ).toEqual(["visible", "ACTAGENT_E2E_OK_LOCAL"]);

    expect(
      extractAgentReplyTexts(
        JSON.stringify({
          result: {
            payloads: [{ text: "ACTAGENT_E2E_OK_GATEWAY" }],
            meta: { finalAssistantRawText: "raw" },
          },
        }),
      ),
    ).toEqual(["raw", "ACTAGENT_E2E_OK_GATEWAY"]);
  });
    ...
    expect(
      extractAgentReplyTexts(
        [
          ...
          JSON.stringify({ payloads: [{ text: "ACTAGENT_E2E_OK_COMBINED" }] }),
        ].join("\n"),
      ),
    ).toEqual(["ACTAGENT_E2E_OK_COMBINED"]);
  });
  ...
  it("reads pretty JSON replies from combined stdout and stderr logs", () => {
    expect(
      extractAgentReplyTexts(
        [
          ...
          JSON.stringify(
            {
              payloads: [{ text: "ACTAGENT_E2E_OK_PRETTY" }],
            },
            null,
            2,
          ),
    ...
    ).toEqual(["ACTAGENT_E2E_OK_PRETTY"]);
  });

  it("does not accept markers that only appear outside reply payloads", () => {
    const dir = mkdtempSync(join(tmpdir(), "actagent-e2e-agent-output-"));
    try {
      const outputPath = join(dir, "agent.log");
      writeFileSync(
        outputPath,
        [
          "Return marker ACTAGENT_E2E_OK_PROMPT_ECHO",
          JSON.stringify({ payloads: [{ text: "wrong reply" }] }),
        ].join("\n"),
      );

      expect(() =>
        assertAgentReplyContainsMarker("ACTAGENT_E2E_OK_PROMPT_ECHO", outputPath),
  ...
  it("ignores stale reply markers outside the recent output tail", () => {
    ...
    try {
      ...
      writeFileSync(
        outputPath,
        [
          JSON.stringify({ payloads: [{ text: "ACTAGENT_E2E_OK_STALE" }] }),
          "x".repeat(2_200_000),
          JSON.stringify({ payloads: [{ text: "current reply without marker" }] }),
        ].join("\n"),
      );

      expect(() => assertAgentReplyContainsMarker("ACTAGENT_E2E_OK_STALE", outputPath)).toThrow(
  ...
  it("bounds missing marker diagnostics to the recent output tail", () => {
    ...
    try {
      ...
      expect(() => assertAgentReplyContainsMarker("ACTAGENT_E2E_OK_MISSING", outputPath)).toThrow(
        /agent reply payload did not contain marker/u,
      );
      try {
        assertAgentReplyContainsMarker("ACTAGENT_E2E_OK_MISSING", outputPath);
      } catch (error) {
  ...
  it("checks that the mock OpenAI endpoint was actually hit", () => {
    const dir = mkdtempSync(join(tmpdir(), "actagent-e2e-request-log-"));
    try {

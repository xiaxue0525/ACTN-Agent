...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      ACTAGENT_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "actagent.json"),
      ACTAGENT_ENABLE_PRIVATE_QA_CLI: "1",
      ACTAGENT_HOME: path.join(outputDir, "qa-state-root", "home"),
      ACTAGENT_RUN_NODE_SKIP_DTS_BUILD: "1",
      ACTAGENT_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      ACTAGENT_TEST_DISABLE_UPDATE_CHECK: "1",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.ACTAGENT_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        ACTAGENT_CONFIG_PATH: "/real/user/.actagent/actagent.json",
        ACTAGENT_HOME: "/real/user/home",
        ACTAGENT_STATE_DIR: "/real/user/.actagent",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });

...
function realBehaviorProofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local ACTAgent gateway, redacted Discord token.",
    steps: "pnpm actagent gateway restart and pnpm actagent gateway status",
    evidence,
...
describe("barnacle-auto-response", () => {
  it("keeps Barnacle-owned labels documented and ACTAgentHub spelled correctly", () => {
    expect(managedLabelSpecs["r: skill"].description).toContain("ACTAgentHub");
    expect(managedLabelSpecs["r: skill"].description).not.toContain("actagentdhub");
    ...
    expect(managedLabelSpecs["r: third-party-extension"].description).toContain("ACTAgentHub");
  ...
  it("does not classify a linked core plugin auto-enable fix as an external plugin candidate", () => {
    const labels = classifyPullRequestCandidateLabels(
      pr(
        ...
        [
          "- Problem: actagent doctor --fix adds duplicate installed plugin entries",
  ...
  it("does not respond to maintainer comments on external items", async () => {
  ...
  it("does not close automation PRs for the active PR limit", async () => {
    for (const automationPullRequest of [
      { head: { ref: "actagentsweeper/actagent-actagent-73880" }, login: "app/actagent-actagentsweeper" },
      { headRefName: "actagentsweeper/actagent-actagent-73880", login: "app/actagent-actagentsweeper" },
      {
        head: { ref: "clownfish/ghcrawl-156993-autonomous-smoke" },
        login: "app/actagent-clownfish",
      },
      { headRefName: "clownfish/ghcrawl-156993-autonomous-smoke", login: "app/actagent-clownfish" },
    ]) {
      const { calls, github } = barnacleGithub([]);
  ...
  it("still adds candidate labels to broad external PRs", async () => {
    const { calls, github } = barnacleGithub([
      file("ui/src/app.ts"),
      file("src/gateway/server.ts"),
      file("extensions/slack/src/index.ts"),
  ...
  it("preserves sufficient proof on synchronize when ACTAgentSweeper passed the exact head", async () => {
      ...
      comments: [
        {
          ...
          body: `<!-- actagentsweeper-verdict:pass item=123 sha=${headSha} confidence=high -->`,
        },
      ],
    });
  ...
  it("preserves stale sufficient proof while ACTAgentSweeper automerge owns the PR", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [PROOF_SUPPLIED_LABEL, PROOF_SUFFICIENT_LABEL, "actagentsweeper:automerge"],
        { action: "synchronize" },
      ),
      core: {
        info: () => undefined,
  ...
  it("preserves stale sufficient proof on ACTAgentSweeper branch updates", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        {
          head: {
            ref: "actagentsweeper/repair-pr-83758",
            sha: "0ede3d716805e7d2ced8df37c6666af510dc9e19",
          },
          body: realBehaviorProofBody(
            "![after](https://github.com/user-attachments/assets/gateway-ready)",
  ...
  it("preserves stale sufficient proof on ACTAgentSweeper-authored PR updates", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
  ...
  it("preserves ACTAgentSweeper's sufficient proof label on ordinary label events", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [PROOF_SUPPLIED_LABEL, PROOF_SUFFICIENT_LABEL],
        {
          ...
          sender: { login: "actagent-actagentsweeper[bot]", type: "Bot" },
        },
      ),
      core: {
        info: () => undefined,
  ...
  it("does not let Barnacle veto ACTAgentSweeper's sufficient proof label add", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
  ...
  it("actions manually applied candidate labels", async () => {
    const { calls, github } = barnacleGithub([file("extensions/example/actagent.plugin.json")]);
    ...
    expect(calls.createComment[0]?.body).toContain("ACTAgentHub");
  ...
  it("keeps bot-applied candidate labels passive", async () => {
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext({}, [candidateLabels.externalPluginCandidate], {
        action: "labeled",
        label: { name: candidateLabels.externalPluginCandidate },
        sender: { login: "actagent-bot[bot]", type: "Bot" },
      }),
      core: {
        info: () => undefined,
      },

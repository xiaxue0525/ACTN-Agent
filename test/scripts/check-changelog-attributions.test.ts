...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "actagent-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "actagent-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$ACTAGENT_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$ACTAGENT_TEST_CONTRIB"',
    ],
    {
      ACTAGENT_PR_CHANGELOG_SH: changelogScriptPath,
      ACTAGENT_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$ACTAGENT_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      ACTAGENT_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @actagent.",
      "- Maintainer-owned fix. Thanks @admin1douyin.",
      "- Mixed credit. Thanks @author and @ACTAgent.",
      "- Bot repair. Thanks @actagentsweeper[bot].",
      "- Dependency bump. Thanks @dependabot[bot].",
      "- App repair. Thanks @app/actagentsweeper.",
    ].join("\n");

    expect(findForbiddenChangelogThanks(content)).toEqual([
      { line: 1, handle: "codex", text: "- Internal cleanup. Thanks @codex." },
      { line: 2, handle: "actagent", text: "- Org-owned fix. Thanks @actagent." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @admin1douyin." },
      { line: 4, handle: "actagent", text: "- Mixed credit. Thanks @author and @ACTAgent." },
      { line: 5, handle: "actagentsweeper[bot]", text: "- Bot repair. Thanks @actagentsweeper[bot]." },
      { line: 6, handle: "dependabot[bot]", text: "- Dependency bump. Thanks @dependabot[bot]." },
      { line: 7, handle: "app/actagentsweeper", text: "- App repair. Thanks @app/actagentsweeper." },
    ]);
  });

  it("allows external author thanks attributions", () => {
    expect(
      findForbiddenChangelogThanks(
        "- User-facing fix. Fixes #123. Thanks @external-author and @other-user.",
      ),
    ).toStrictEqual([]);
  });
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @actagent and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "actagent",
        text: "- Mixed credit (#123). Thanks @actagent and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("actagent")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("steipete")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("app/actagentsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("actagentsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("actagentsweeper[bot]")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("actagent-actagentsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("actagent-actagentsweeper[bot]")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("human-actagentsweeper-fan")).toBe(false);
    expect(
      isForbiddenChangelogThanksHandle("human-actagentsweeper-fan", { strictBotHandle: true }),
    ).toBe(false);

    expect(requiresExplicitHumanChangelogThanks("actagentsweeper")).toBe(true);
    expect(requiresExplicitHumanChangelogThanks("actagentsweeper[bot]")).toBe(true);
    expect(requiresExplicitHumanChangelogThanks("dependabot[bot]")).toBe(true);
    expect(requiresExplicitHumanChangelogThanks("app/actagentsweeper")).toBe(true);
    expect(requiresExplicitHumanChangelogThanks("human-actagentsweeper-fan")).toBe(false);
  ...
  it("keeps non-bot forbidden authors on the no-thanks fallback", () => {
    ...
    try {
      const output = run(
        ...
        [
          ...
          `
set -euo pipefail
source "$ACTAGENT_PR_COMMON_SH"
source "$ACTAGENT_PR_CHANGELOG_SH"
source "$ACTAGENT_PR_GATES_SH"
...
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$ACTAGENT_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$ACTAGENT_TEST_CALLS"; }

prepare_gates 123
`,
        ],
        {
          ACTAGENT_PR_COMMON_SH: commonScriptPath,
          ACTAGENT_PR_CHANGELOG_SH: changelogScriptPath,
          ACTAGENT_PR_GATES_SH: gatesScriptPath,
          ACTAGENT_TEST_CALLS: callsPath,
        },
      );

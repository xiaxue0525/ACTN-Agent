...
describe("qa-e2e script", () => {
  it("enables private QA plugin SDK subpaths before loading QA Lab", () => {
    ...
    expect(env.ACTAGENT_BUILD_PRIVATE_QA).toBe("1");
    expect(env.ACTAGENT_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(env.ACTAGENT_DISABLE_BUNDLED_PLUGINS).toBe("0");
  });

  it("overrides inherited environment that would break the private QA self-check", () => {
    const env: NodeJS.ProcessEnv = {
      ACTAGENT_BUILD_PRIVATE_QA: "0",
      ACTAGENT_ENABLE_PRIVATE_QA_CLI: "0",

...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("ACTAGENT_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e ACTAGENT_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e ACTAGENT_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain("ACTAGENT_QA_CONVEX_SECRET_CI");
    expect(script).toContain("ACTAGENT_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("bounds installed-package hot path ACTAgent commands", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    ...
    expect(runtimeRun).toContain("source scripts/lib/actagent-e2e-instance.sh");
    expect(runtimeRun).toContain("actagent_e2e_run_command actagent --version");
    expect(runtimeRun).toContain("actagent_e2e_run_command actagent onboard");
    ...
    expect(runtimeRun).toContain("actagent_e2e_run_command actagent channels add");
    expect(runtimeRun).toContain("actagent_e2e_run_command actagent doctor --fix");
    expect(runtimeRun).toContain("actagent_e2e_run_command actagent doctor --non-interactive");
    expect(runtimeRun).not.toMatch(/^\s*actagent (onboard|channels add|doctor )/mu);
  });

  it("can install a resolved package tarball instead of a registry spec", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");

    expect(script).toContain("ACTAGENT_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("ACTAGENT_CURRENT_PACKAGE_TGZ");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_actagent_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_actagent_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("keeps live Docker artifacts isolated by default", () => {
    ...
    expect(script).not.toContain(
      'OUTPUT_DIR="${ACTAGENT_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"',
    );
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$actagent_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$actagent_package_dir/package.json" /app/package.json');
    expect(script).toContain('-v "$ROOT_DIR/extensions/qa-lab:/app/extensions/qa-lab:ro"');
    expect(script).not.toContain('ln -sfnT /app/extensions "$actagent_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/actagent/package.json");
    ...
    expect(gatewayRpcClient).toContain('from "actagent/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "actagent/plugin-sdk/gateway-runtime"');
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      testing.resolveCredentialSource({
        ACTAGENT_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        ACTAGENT_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      testing.resolveCredentialRole({
        ACTAGENT_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",

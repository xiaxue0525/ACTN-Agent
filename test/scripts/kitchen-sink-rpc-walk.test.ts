...
describe("kitchen-sink RPC isolated state", () => {
  it("prints help without creating temp state or installing the plugin", async () => {
    ...
    expect(result.stdout).toContain("ACTAGENT_KITCHEN_SINK_NPM_SPEC");
  ...
  it("cleans up the generated temporary home tree", async () => {
    ...
    expect(root).toContain("actagent-kitchen-sink-rpc-");
    expect(env.HOME).toBe(path.join(root, "home"));
    expect(env.USERPROFILE).toBe(env.HOME);
    expect(env.ACTAGENT_HOME).toBe(env.HOME);
    expect(env.ACTAGENT_STATE_DIR).toBe(path.join(env.HOME, ".actagent"));
    expect(env.ACTAGENT_CONFIG_PATH).toBe(path.join(env.ACTAGENT_STATE_DIR, "actagent.json"));
    expect(existsSync(env.ACTAGENT_STATE_DIR)).toBe(true);
  ...
  it("can fail the walk when generated temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        cleanupKitchenSinkEnv("/tmp/actagent-kitchen-sink-rpc-stuck", {
          attempts: 3,
          delayMs: 1,
          throwOnFailure: true,
          warn: false,
        ...
        "failed to remove Kitchen Sink RPC temp root: /tmp/actagent-kitchen-sink-rpc-stuck",
      );
...
describe("kitchen-sink RPC gateway teardown", () => {
  ...
  it("fails readiness waits before polling after signaled gateway exits", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-signal-ready-"));
    try {
  ...
  it("aborts stalled readiness probes when the gateway exits mid-probe", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-exit-during-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("keeps stalled readiness probes inside the caller deadline", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-stalled-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC gateway readiness logs", () => {
  it("scans gateway readiness logs incrementally across appended chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-scan-"));
    try {
      const logPath = path.join(root, "gateway.log");
      writeFileSync(logPath, "booting\n".repeat(1000));
  ...
  it("resets the readiness scanner after log rotation", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-rotate-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("tails large gateway logs without returning older content", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-tail-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("scans gateway error logs incrementally and keeps the latest findings", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-errors-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("does not allowlist dirty error lines that mention zero errors", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-zero-error-smuggle-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("bounds scanner memory for very long log lines", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-log-long-line-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC command output capture", () => {
  ...
  posixIt("kills timed command process groups", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-timeout-"));
...
`,
  ...
  posixIt("rejects timed commands that exit cleanly after SIGTERM", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-kitchen-rpc-timeout-zero-"));
...
`,
  ...
  it("rejects command spawn failures as Error objects", async () => {
    await expect(runCommand("actagent-definitely-missing-command", [])).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
});
...
describe("kitchen-sink RPC caller loading", () => {
  ...
  it("uses built callGateway chunks for dist and packaged entries", () => {
    expect(usesBuiltACTAgentEntry({ command: "node", baseArgs: ["dist/index.js"] })).toBe(true);
    expect(
      usesBuiltACTAgentEntry({ command: "node", baseArgs: ["/app/actagent.mjs"] }, "/repo", {
        ACTAGENT_ENTRY: "/app/actagent.mjs",
      }),
    ).toBe(true);
  });

  it("does not deep-import gateway TypeScript for source pnpm runners", () => {
    expect(usesBuiltACTAgentEntry({ pnpm: true, baseArgs: ["actagent"] })).toBe(false);
    expect(usesBuiltACTAgentEntry({ command: "node", baseArgs: ["scripts/dev.mjs"] })).toBe(false);
  });

  it("finds only built callGateway chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "actagent-rpc-call-chunks-"));
    try {
...
describe("kitchen-sink RPC command catalog assertions", () => {
  ...
  it("requires every expected Kitchen Sink plugin tool", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [{ id: "kitchen_sink_text", source: "plugin", pluginId: "actagent-kitchen-sink-fixture" }],
  ...
  it("requires plugin provenance for expected catalog tools", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [
          { id: "kitchen_sink_text", source: "plugin", pluginId: "actagent-kitchen-sink-fixture" },
          { id: "kitchen_sink_search", source: "core", pluginId: "actagent-kitchen-sink-fixture" },
...
describe("kitchen-sink RPC process sampling", () => {
  ...
  it("samples the POSIX gateway child instead of the pnpm launcher", async () => {
    const sample = await sampleProcess(4321, {
      ...
      runCommand: async (command: string, args: string[]) => {
        ...
        return {
          stdout: [
            " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm actagent gateway --port 19080",
  ...
  it("falls back to the POSIX gateway process title when the port arg is rewritten", async () => {
    const sample = await sampleProcess(4321, {
      ...
      runCommand: async () => ({
        stdout: [
          " 4321     1 1048576   0.0 node /usr/local/bin/corepack pnpm actagent gateway --port 19080",
          " 4322  4321  262144  12.5 actagent-gateway",
          " 4323  4322   32768   1.5 node helper.js",
        ].join("\n"),
        stderr: "",
      }),
  ...
  it("does not accept a POSIX launcher sample when the gateway child is missing", async () => {
    const sample = await sampleProcess(4321, {
      ...
      posixCommandLineNeedles: ["gateway", "--port", "19080"],
      runCommand: async () => ({
        stdout: " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm actagent status\n",
        stderr: "",
      }),
    });

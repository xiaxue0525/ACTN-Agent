...
describe("check-memory-fd-repro", () => {
  ...
  it("rejects loose numeric environment limits before generating files", () => {
    expect(
      withEnv(
        {
          ACTAGENT_MEMORY_FD_REPRO_FILES: "17",
          ACTAGENT_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "0",
          ACTAGENT_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "0",
        },
        () => parseArgs([]),
      ),
    ).toMatchObject({
    ...
    expect(() =>
      withEnv({ ACTAGENT_MEMORY_FD_REPRO_FILES: "17files" }, () => parseArgs([])),
    ).toThrow("ACTAGENT_MEMORY_FD_REPRO_FILES must be a non-negative integer");
    expect(() =>
      withEnv({ ACTAGENT_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3" }, () => parseArgs([])),
    ).toThrow("ACTAGENT_MEMORY_FD_REPRO_TIMEOUT_MS must be a non-negative integer");
  });
  ...
  it("lets explicit CLI numeric flags override malformed inherited env defaults", () => {
    expect(
      withEnv(
        {
          ACTAGENT_MEMORY_FD_REPRO_FILES: "17files",
          ACTAGENT_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "4fds",
          ACTAGENT_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3",
          ACTAGENT_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "soon",
          ACTAGENT_MEMORY_FD_REPRO_SETTLE_DELAY_MS: "later",
        },
        () =>
          parseArgs([
            "--files",
  ...
  it("stops parsing options after the argument terminator", () => {
    ...
    expect(
      withEnv({ ACTAGENT_MEMORY_FD_REPRO_FILES: "17" }, () => parseArgs(["--", "--unknown"])),
    ).toMatchObject({
      fileCount: 17,
    });
  });
  ...
  it("writes an offline FTS-only memory search config for repro indexing", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "actagent-memory-fd-config-"));
    try {
      ...
      expect(memorySearch).toMatchObject({
        ...
        model: "",
        store: {
          path: path.join(homeDir, ".actagent", "memory", "main.sqlite"),

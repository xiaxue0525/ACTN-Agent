...
describe("zai fallback repro command resolution", () => {
  it("wraps Windows pnpm.cmd without Node shell argv", () => {
    expect(
      resolveZaiFallbackPnpmCommand(["actagent", "agent", "--message", "hello world"], {
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\pnpm.cmd" actagent agent --message "hello world""`,
      ],
  ...
  it("cleans temporary repro state after fallback proof", async () => {
    ...
    const exitCode = await runZaiFallbackRepro({
      env: {
        ...
        ACTAGENT_ZAI_FALLBACK_SESSION_ID: "session-test",
      ...
      log: () => {},
      mkdtemp: async () => {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "actagent-zai-fallback-test-"));
        tempRoots.push(root);
        return root;
      },

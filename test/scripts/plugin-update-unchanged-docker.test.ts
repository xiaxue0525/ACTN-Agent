...
function runProbe(command: string, payload: unknown): void {
  const root = mkdtempSync(path.join(tmpdir(), "actagent-plugin-update-probe-"));
...
describe("plugin update unchanged Docker E2E", () => {
  it("seeds current plugin install ledger state before checking config stability", () => {
    ...
    expect(probe).toContain("writeJson(process.env.ACTAGENT_CONFIG_PATH, { plugins: {} });");
    expect(probe).not.toContain(
      "writeJson(process.env.ACTAGENT_CONFIG_PATH, { plugins: { installs",
    );
    expect(probe).toContain("installRecords: {");
    expect(probe).toContain('"lossless-actagent": {');
  });

  it("bounds the update command and prints diagnostics on hangs", () => {
    const script = readFileSync(PLUGIN_UPDATE_SCENARIO_SCRIPT, "utf8");

    expect(script).toContain("ACTAGENT_PLUGIN_UPDATE_TIMEOUT_SECONDS");
    expect(script).toContain(
      'actagent_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update',
    );
  ...
  it("waits for the local registry process during cleanup", () => {
    ...
    expect(script).toContain('actagent_e2e_stop_process "${registry_pid:-}"');
  ...
  it("bounds corrupt plugin update commands and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("ACTAGENT_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS");
    expect(
      script.match(/actagent_e2e_maybe_timeout "\$\{update_timeout_seconds\}s" \\/gu)?.length,
    ).toBe(2);
    expect(script).toContain("--channel beta");
    expect(script).toContain("ACTAGENT_UPDATE_POST_CORE=1");
    expect(script).not.toContain(
      'node "$entry" update --channel beta --tag "${ACTAGENT_CURRENT_PACKAGE_TGZ',
    );
    expect(script).toContain(
      "actagent update failed or timed out after ${update_timeout_seconds}s",
    );
    expect(script).toContain(
      "updated ACTAgent entry failed or timed out after ${update_timeout_seconds}s",
    );
  });
        ...
        outcomes: [
          {
            ...
            status: "skipped",
            message: `Disabled "${CORRUPT_PLUGIN_ID}" after plugin update failure; ACTAgent will continue without it. Failed to update ${CORRUPT_PLUGIN_ID}: registry timeout`,
          },
        ],
      },
    };
        ...
        warnings: [
          {
            ...
            message:
              ...
              `Run actagent plugins inspect ${CORRUPT_PLUGIN_ID} --runtime --json for details.`,
          },
        ],
      }),
    ).not.toThrow();

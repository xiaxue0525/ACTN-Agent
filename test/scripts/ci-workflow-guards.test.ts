...
describe("ci workflow guards", () => {
  it("kills timed manual checkout fetches after the grace period", () => {
    ...
    for (const workflowPath of workflowPaths) {
      ...
      const fetchTimeouts = workflow.match(
        /timeout --signal=TERM[^\n]* 30s git(?: -C "(?:\$workdir|\$GITHUB_WORKSPACE|actagenthub-source)")?/g,
      );
  ...
  it("fails and retries quiet Node test shard stalls quickly", () => {
    ...
    expect(runStep.env.ACTAGENT_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("300000");
    expect(runStep.env.ACTAGENT_VITEST_NO_OUTPUT_RETRY).toBe("1");
    expect(runStep.env.ACTAGENT_TEST_PROJECTS_PARALLEL).toBe("2");
  });
  ...
  it("keeps push docs validation ACTAgentHub-backed", () => {
    const workflow = readFileSync(".github/workflows/docs.yml", "utf8");

    expect(workflow).toContain("repository: actagent/actagenthub");
    expect(workflow).toContain("path: actagenthub-source");
    expect(workflow).toContain(
      "ACTAGENT_DOCS_SYNC_ACTAGENTHUB_REPO: ${{ github.workspace }}/actagenthub-source",
    );
  });

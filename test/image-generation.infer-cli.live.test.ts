...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.ACTAGENT_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through actagent infer", () => {
    const outputBase = path.join(os.tmpdir(), `actagent-infer-image-${process.pid}.png`);

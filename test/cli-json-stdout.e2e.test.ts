...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".actagentdbot");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "actagentdbot.json"), "{}", "utf8");
        ...
        delete env.ACTAGENT_HOME;
        delete env.ACTAGENT_STATE_DIR;
        delete env.ACTAGENT_CONFIG_PATH;
        delete env.VITEST;

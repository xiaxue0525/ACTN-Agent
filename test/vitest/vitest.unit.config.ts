...
export function loadIncludePatternsFromEnv(
  ...
  return loadPatternListFromEnv("ACTAGENT_VITEST_INCLUDE_FILE", env);
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  return loadPatternListFromEnv("ACTAGENT_VITEST_EXTRA_EXCLUDE_FILE", env) ?? [];
}
...
export function createUnitVitestConfigWithOptions(
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(sharedTest.setupFiles ?? []), "test/setup-actagent-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],

---
name: node-connect
description: "Diagnose ACTAgent Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify ACTAgent is advertising that route, then fix pairing/auth.
...
- whether `actagent devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `actagent qr --json`. It uses the same setup-code payload Android scans.

```bash
actagent config get gateway.mode
actagent config get gateway.bind
actagent config get gateway.tailscale.mode
actagent config get gateway.remote.url
actagent config get gateway.auth.mode
actagent config get gateway.auth.allowTailscale
actagent config get plugins.entries.device-pair.config.publicUrl
actagent qr --json
actagent devices list
actagent nodes status
```

If this ACTAgent instance is pointed at a remote gateway, also run:

```bash
actagent qr --remote --json
```
...
`actagent qr --json` success means:
...
If `actagent qr --json` says `Gateway is only bound to loopback`:
...
```bash
actagent devices list
actagent devices approve --latest   # preview only; copy the requestId from output
...
- `actagent devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run actagent qr again, rescan, then approve the pending device pairing.`

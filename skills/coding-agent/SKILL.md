...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.actagent`, `$ACTAGENT_STATE_DIR`, or active ACTAgent state dirs.
...
- Worker must send completion/failure via `actagent message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/actagent`; use an isolated checkout.
...
actagent message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use actagent system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t actagent-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF

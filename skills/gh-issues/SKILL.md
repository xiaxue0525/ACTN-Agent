...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${ACTAGENT_CONFIG_PATH:-${ACTAGENT_STATE_DIR:-$HOME/.actagent}/actagent.json}"
...
```text
${ACTAGENT_STATE_DIR:-$HOME/.actagent}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with actagent message send if route provided.
```

...
1. Download the runtime for your OS (extracts into `$ACTAGENT_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.actagent/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ACTAGENT_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.actagent/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active ACTAgent config file (`$ACTAGENT_CONFIG_PATH`, default `~/.actagent/actagent.json`):

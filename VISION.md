...
ACTAgent is the AI that actually does things.
...
ACTAgent started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> actagentdbot -> Moltbot -> ACTAgent.
...
ACTAgent runtime code reads the current configuration schema only.
...
`actagent doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in ACTAgent is a deliberate tradeoff: strong defaults without killing capability.
...
ACTAgent has an extensive plugin API.
...
- Code plugins run ACTAgent plugin code and are appropriate for deeper runtime extension.
...
Plugin discovery, official publisher status, provenance, and security review live in [ACTAgentHub](https://actagenthub.ai/).
ACTAgent docs should document core extension points; plugin promotion belongs in ACTAgentHub, preferably under vetted org publishers for official plugins.
...
New skills should be published through [ACTAgentHub](https://actagenthub.ai/) first, not added to core by default.
Official or bundled promotion should require a clear product, security, or maintainer-ownership reason.

### MCP Support

ACTAgent supports MCP as both a server and a runtime integration surface.
...
tool, ACPX, plugin, or ACTAgentHub paths.

### Setup

ACTAgent is currently terminal-first by design.
...
ACTAgent is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep ACTAgent hackable by default.
It is widely known, fast to iterate in, and easy to read, modify, and extend.

## What We Will Not Merge (For Now)

- New core skills when they can live on [ACTAgentHub](https://actagenthub.ai/)
- Full-doc translation sets for all docs (deferred; we plan AI-generated translations later)
- Commercial service integrations that do not clearly fit the model-provider category
- Wrapper channels around already supported channels without a clear capability or security gap
- MCP work that duplicates existing MCP, ACPX, plugin, or ACTAgentHub paths without a clear product or security gap
